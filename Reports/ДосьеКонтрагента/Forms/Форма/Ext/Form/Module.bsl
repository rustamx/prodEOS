...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		ТекстИсключения = НСтр("ru='Формирование отчета ""Досье контрагента"" в данной конфигурации невозможно.'; en = '""Counterparty profile"" report is not available in this configuration.'");
...
Процедура СформироватьОтчет(Команда)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаПоиска) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Поле ""ИНН или наименование контрагента"" не заполнено'; en = 'Field ""INN or name of counterparty"" is not specified'"), , "СтрокаПоиска");
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормойНаСервере()
	...
	Если ЗначениеЗаполнено(НаименованиеКонтрагента) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Досье контрагента: %1'; en = 'Counterparty profile: %1'"),
	...
	ИначеЕсли ЗначениеЗаполнено(Контрагент) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Контрагент);
	Иначе
		Заголовок = НСтр("ru='Досье контрагента'; en = 'Counterparty profile'");
	КонецЕсли;
	Элементы.ДекорацияРазделДанныеГосРеестров.Заголовок = ?(ЭтоЮридическоеЛицо, 
		НСтр("ru='ЕГРЮЛ'; en = 'USRLE'"), 
		НСтр("ru='ЕГРИП'; en = 'USRIE'"));
...
Процедура СформироватьОтчетНаКлиенте(ТекстПоиска)
	...
	Если ПоискПоИНН Тогда
		...
		Если СтрДлина(ТекстПоиска) <> 10 
			...
			ТекстОшибки = НСтр("ru='Для поиска по ИНН необходимо ввести 10 или 12 цифр.'; en = 'To search by INN you must enter 10 or 12 digits.'");
	...
	Иначе // Поиск по наименованию
		...
		ПараметрыФормы.Вставить("Заголовок",    НСтр("ru='Поиск контрагента'; en = 'Counterparty search'"));
...
Функция СформироватьОтчетНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Формирование отчета: Досье контрагента: %1'; en = 'Generating report: Counterparty profile: %1'"), 
		ИННПоиска);
...
Процедура ОбработатьОшибкиФормированияОтчета()
	...
	Если ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
		...
		ТекстВопроса = НСтр("ru='Для формирования ""Досье контрагента""
			...
			|Подключиться сейчас?';
			|en = 'To form a ""Counterparty profile"" 
			|you must connect to the online customer support. 
			|Connect now?'");
