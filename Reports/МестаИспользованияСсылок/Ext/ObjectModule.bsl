	...
	Если ТипЗнч(НаборСсылок) = Тип("СписокЗначений") Тогда
		...
		Если ВсегоСсылок >= 0 Тогда
			...
			Если ОдинаковыйТип Тогда
				Результат = СтрШаблон(НСтр("ru = 'Места использования %1'; en = 'Usage %1'"), НаборСсылок[0].Значение.Метаданные().Представление());
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если Результат = Неопределено Тогда
		Результат = НСтр("ru = 'Места использования элементов'; en = 'Items usage'");
	КонецЕсли;
