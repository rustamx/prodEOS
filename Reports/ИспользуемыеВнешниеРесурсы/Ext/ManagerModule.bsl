...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	НастройкиОтчета.Описание = НСтр("ru = 'Внешние ресурсы, используемые программой и дополнительными модулями'; en = 'External resources used by the program and additional modules'");
	...
	НастройкиОтчета.НастройкиДляПоиска.НаименованияПолей = 
		...
		|Порт';
		|en = 'Name and COM-class ID
		|Computer name
		|Address
		|Read data
		|Write data
		|Template name and component file
		|Checksum
...
Функция СловарьМодуляКонфигурации() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("ru = 'Программа'; en = 'Program'"));
	Результат.Вставить("Родительный", НСтр("ru = 'Программы'; en = 'Programs'"));
...
Функция ПредставлениеЗапросовРазрешенийНаИспользованиеВнешнихРесурсов(Знач ОперацииАдминистрирования, Знач ОписаниеДобавляемыхРазрешений, Знач ОписаниеУдаляемыхРазрешений, Знач КакТребуемые = Ложь) Экспорт
	...
	Для Каждого СтрокаТаблицыМодулей Из ТаблицаМодулей Цикл
		...
		Если Добавляемые.Количество() > 0 Тогда
			...
			Если КакТребуемые Тогда
				...
				ТекстШапки = СтрШаблон(
					НСтр("ru = 'Для %1 ""%2"" требуется использование следующих внешних ресурсов:'; en = '%1 ""%2"" requires following external resources:'"),
			...
			Иначе
				...
				ТекстШапки = СтрШаблон(
					НСтр("ru = 'Для %1 ""%2"" будут предоставлены следующие разрешения на использование внешних ресурсов:'; en = 'Following permissions for the use of external resources will be granted to %1 ""%2"":'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
		...
		Если Удаляемые.Количество() > 0 Тогда
			...
			Если КакТребуемые Тогда
				ВызватьИсключение НСтр("ru = 'Некорректный запрос разрешений'; en = 'Incorrect permissions request'");
			КонецЕсли;
			
			ТекстШапки = СтрШаблон(
					НСтр("ru = 'Будут удалены следующие ранее предоставленные для %1 ""%2"" разрешения на использование внешних ресурсов:'; en = 'You are about to delete the following previously granted for %1 ""%2"" permissions to use external resources:'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
...
Процедура СформироватьПредставлениеОпераций(ТабличныйДокумент, Знач Макет, Знач ОперацииАдминистрирования)
	...
	Для Каждого Описание Из ОперацииАдминистрирования Цикл
		...
		Если Описание.Операция = Перечисления.ОперацииАдминистрированияПрофилейБезопасности.Удаление Тогда
			...
			ТекстШапки = СтрШаблон(
					НСтр("ru = 'Будет удален профиль безопасности для %1 ""%2"".'; en = 'Security profile for %1 ""%2"" will be removed.'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
					...
					Иначе
						...
						Если ТипРазрешения = "FileSystemAccess" Тогда
							...
							Если Разрешение.Path = "/temp" Тогда
								ДополнениеРазрешения.Вставить("Path", НСтр("ru = 'Каталог временных файлов'; en = 'Temporary file directory'"));
							КонецЕсли;
							
							Если Разрешение.Path = "/bin" Тогда
								ДополнениеРазрешения.Вставить("Path", НСтр("ru = 'Каталог, в который установлен сервер 1С:Предприятия'; en = 'The directory in which you installed the 1С:Enterprise server'"));
							КонецЕсли;
