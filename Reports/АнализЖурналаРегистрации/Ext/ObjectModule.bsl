...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВариантОтчета = "ДиаграммаГанта" Тогда
		...
		Если Не ЗначениеЗаполнено(ПериодДень.Дата) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Не заполнено значение поля День!'; en = 'Field Day value is not specified.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		И КонецВыборки.Использование Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Значение начала периода не может быть больше значения конца!'; en = 'Period start value cannot be greater than end value.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ВариантОтчета = "АктивностьПользователя" Тогда
		...
		Если Не ЗначениеЗаполнено(Пользователь) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Не заполнено значение поля Пользователь!'; en = 'Field User value is not specified.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		Если Отчеты.АнализЖурналаРегистрации.ИмяПользователяИБ(Пользователь) = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Формирование отчета возможно только для пользователя, которому указано имя для входа в программу.'; en = 'Report generation is possible only for a user who has logon name in the application.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ВариантОтчета = "АнализАктивностиПользователей" Тогда
		...
		Если Не ЗначениеЗаполнено(ПользователиИГруппы) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Не заполнено значение поля Пользователи!'; en = 'Field Users value is not specified.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
