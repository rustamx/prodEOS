	...
	Если ТипЗнч(НастройкиОтчета.РазрешеноМенятьВарианты) <> Тип("Булево") Тогда
		...
		Если НастройкиОтчета.РазрешеноМенятьВарианты // Команды не отключены параметрами открытия формы.
			...
			Если Отключенные.Найти(НастройкиОтчета.ПредопределенныйСсылка) <> Неопределено Тогда // Вариант отключен.
				Текст = НСтр("ru = 'Контекстный вариант ""%1"" отчета ""%2"" открыт без контекста.'; en = 'Context option ""%1"" of report ""%2"" has been opened without a context.'");
	...
	Для Каждого ПользовательскаяНастройкаКД Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		Если Тип = "ЗначениеПараметраНастроек"
			...
			ИначеЕсли Значение.ДатаНачала > Значение.ДатаОкончания Тогда
				ТекстОшибки = НСтр("ru = 'Конец периода должен быть больше начала'; en = 'The end of the period must be greater than start.'");
...
Процедура ОтправитьПоЭлектроннойПочте(Команда)
	...
	Если ОтображениеСостояния.Видимость = Истина
		...
		ТекстВопроса = НСтр("ru = 'Отчет не сформирован. Сформировать?'; en = 'Report is not created. Create?'");
...
Процедура РедактироватьСхему(Команда)
	...
	#Иначе
		...
		ПоказатьПредупреждение(,(НСтр("ru='Для того чтобы редактировать схему компоновки, необходимо запустить приложение в режиме толстого клиента.'; en = 'To edit data composition schema you must start the application in thick client mode.'")));
...
Процедура Подключаемый_ЗагрузитьВариантОтчета(Команда)
	...
	Если Найденные.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Вариант отчета не найден.'; en = 'Report option not found.'"));
		Возврат;
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Иначе
		...
		Если ПеременныеКлиента.УспешноСформирован Тогда
			ПоказатьОповещениеПользователя(НСтр("ru = 'Отчет сформирован'; en = 'Report has been created'"), , Заголовок);
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ru = 'Отчет не сформирован'; en = 'Report has not been created'"), , Заголовок);
		КонецЕсли;
...
Процедура ПоказатьДиалогОтправкиПоЭлектроннойПочте()
	...
	ЗаголовокСохранения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отправка отчета ""%1"" по почте'; en = 'Send report ""%1"" via email'"), ОтчетНаименованиеТекущегоВарианта);
...
Процедура ВидимостьДоступностьЕслиТребуется()
	Если ВариантыПанелиКлючТекущегоВарианта <> " - " Тогда 
		Возврат;
	КонецЕсли;
	ВидимостьДоступностьКорректность("");
КонецПроцедуры
...
Процедура ВидимостьДоступностьКорректность(Изменения = "")
	...
	КонецЕсли;
	Если РежимРасшифровки Тогда
		Заголовок = Заголовок + " (" + НСтр("ru = 'Расшифровка'; en = 'Details'") + ")";
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗагрузитьВариант(КлючВарианта)
	...
	ОтображениеСостояния.Текст     = НСтр("ru = 'Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.'; en = 'Diffrerent report option has been selected. Press ""Create"" button to get a report.'");
КонецПроцедуры
...
Функция ФоновоеЗаданиеЗапустить(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	Иначе
		...
		ПараметрыЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Варианты отчетов: Формирование отчета'; en = 'Report options: Report creation'");
...
Процедура ВывестиЭлементыНастройки(Форма, Элементы, СвойстваНастройки, ГруппаВывода, Прочее) Экспорт
	...
	Если ВыводитьУсловие Тогда
		...
		КомандаВидСравнения.Заголовок   = НСтр("ru = 'Изменить условие отбора...'; en = 'Change filter...'");
	...
	Если СвойстваНастройки.ТипЭлементов <> "" Тогда
		...
		ИначеЕсли СвойстваНастройки.ТипЭлементов = "СтандартныйПериод" Тогда
			...
			КомандаВыбора.Заголовок   = НСтр("ru = 'Выбрать период...'; en = 'Select period...'");
			...
			Если Не СвойстваНастройки.ВыводитьФлажок Или Не СвойстваНастройки.ОграничиватьВыборУказаннымиЗначениями Тогда
				...
				Если Не СвойстваНастройки.ОграничиватьВыборУказаннымиЗначениями Тогда
					Если ИнформацияОТипах.СодержитСсылочныеТипы Тогда
						...
						КомандаПодбор.Заголовок = НСтр("ru = 'Подбор'; en = 'Fill'");
					Иначе
						КомандаПодбор = Форма.Команды.Добавить(КнопкаПодборИмя);
						КомандаПодбор.Действие  = "Подключаемый_СписокСПодбором_Добавить";
						КомандаПодбор.Заголовок = НСтр("ru = 'Добавить'; en = 'Add'");
					...
					Если СвойстваНастройки.ИнформацияОТипах.СодержитСсылочныеТипы И Прочее.ЕстьЗагрузкаДанныхИзФайла Тогда
						...
						КомандаВставить.Заголовок = НСтр("ru = 'Вставить из буфера обмена...'; en = 'Paste from the clipboard...'");
...
Процедура ПослеИзмененияКлючевыхСостояний(ПараметрыЗаполнения)
	Если ПараметрыЗаполнения.ИмяСобытия <> "ПослеФормирования" Тогда
		...
		ИначеЕсли ПараметрыЗаполнения.ВариантМодифицирован
			...
			ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
			ОтображениеСостояния.Видимость = Истина;
			ОтображениеСостояния.Текст     = НСтр("ru = 'Изменились настройки. Нажмите ""Сформировать"" для получения отчета.'; en = 'Settings have been changed. Press ""Genrate"" button to get a report.'");
