...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ПараметрыЗаписи.Свойство("КогдаНаименованиеУжеИспользуется") Тогда
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			...
			Кнопки.Добавить("ПродолжитьЗапись",            НСтр("ru = 'Продолжить запись'; en = 'Continue writing'"));
			Кнопки.Добавить("ВернутьсяКВводуНаименования", НСтр("ru = 'Вернуться к вводу наименования'; en = 'Back to the description input'"));
...
Процедура КомандаДалее(Команда)
	...
	Если ИндексСтраницы = 0
		...
		ТекстПредупреждения = НСтр("ru = 'Элемент не выбран!'; en = 'Item is not selected!'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСтраницуВыбора()
	...
	Если ЭтоДополнительноеСведение = 1 Тогда
		Элементы.ОбщиеРеквизитыНеВключенныеВНаборы.Заголовок = НСтр("ru ='Только общие дополнительные сведения'; en = 'Only common additional attributes'");
	Иначе
		Элементы.ОбщиеРеквизитыНеВключенныеВНаборы.Заголовок = НСтр("ru ='Только общие дополнительные реквизиты'; en = 'Only common additional attributes'");
	КонецЕсли;
	...
	Если ВариантТекстаВопроса = "ПереходКСпискуЗначений" Тогда
		ТекстВопроса =
			НСтр("ru = 'Переход к работе со списком значений
			           ...
			           |
			           |Данные будут записаны.';
			           |en = 'To work with list of values is possible only after the data is written.
			           |
			           |Data will be recorded.'");
	Иначе
		ТекстВопроса =
			НСтр("ru = 'Данные будут записаны.'; en = 'Data will be saved.'")
	КонецЕсли;
...
Процедура РедактироватьФорматЗначенияЗавершение(Текст, Неопределен) Экспорт
	...
	Если Текст <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Следующие настройки формата автоматически не применяются в большинстве мест:'; en = 'Following format settings will not be used automatically in most places:'");
		...
		Для каждого Подстрока Из Массив Цикл
			Если СтрНайти(Подстрока, "ДП=") > 0 ИЛИ СтрНайти(Подстрока, "DE=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ " - " + НСтр("ru = 'представление пустой даты'; en = 'presentation of blank date'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ЧН=") > 0 ИЛИ СтрНайти(Подстрока, "NZ=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ " - " + НСтр("ru = 'представление пустого числа'; en = 'presentation of blank number'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДФ=") > 0 ИЛИ СтрНайти(Подстрока, "DF=") > 0 Тогда
				Если СтрНайти(Подстрока, "ддд") > 0 ИЛИ СтрНайти(Подстрока, "ddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru = 'кратное название дня недели'; en = 'short name of week day'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "дддд") > 0 ИЛИ СтрНайти(Подстрока, "dddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru = 'полное название дня недели'; en = 'full name of day of the week'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "МММ") > 0 ИЛИ СтрНайти(Подстрока, "MMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru = 'кратное название месяца'; en = 'short name of month'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "ММММ") > 0 ИЛИ СтрНайти(Подстрока, "MMMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru = 'полное название месяца'; en = 'full name of month'");
				КонецЕсли;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДЛФ=") > 0 ИЛИ СтрНайти(Подстрока, "DLF=") > 0 Тогда
				Если СтрНайти(Подстрока, "ДД") > 0 ИЛИ СтрНайти(Подстрока, "DD") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru = 'длинная дата (месяц прописью)'; en = 'long date (month in words)'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	ШаблонЗаголовкаСписка        = НСтр("ru = 'Выберите %1 для включения в набор ""%2""'; en = 'Select %1 for inclusion in set ""%2""'");
	ШаблонЗаголовкаПереключателя = НСтр("ru = 'Выберите вариант добавления дополнительного %1 ""%2"" в набор ""%3""'; en = 'Select option to add an additional %1 ""%2"" in set ""%3""'");
	
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
			Заголовок = НСтр("ru = 'Добавление дополнительного сведения'; en = 'Add additional data'");
		Иначе
			Заголовок = НСтр("ru = 'Добавление дополнительного реквизита'; en = 'Adding additional attribute'");
		КонецЕсли;
		
		Элементы.КоманднаяПанельЛево.Доступность = Ложь;
		Элементы.КомандаДалее.Заголовок = НСтр("ru = 'Далее >'; en = 'Next >'");
		
		Элементы.ДекорацияЗаголовок.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонЗаголовкаСписка,
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ru = 'дополнительное сведение'; en = 'additional data'"), НСтр("ru = 'дополнительный реквизит'; en = 'additional attribute'")),
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Элементы.РежимДобавленияРеквизита.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, НСтр("ru = 'сведения'; en = 'additional data'"), НСтр("ru = 'реквизита'; en = 'attribute'")),
	...
	Иначе
		Элементы.КомандаДалее.Заголовок = НСтр("ru = 'Готово'; en = 'Done'");
		Элементы.КоманднаяПанельЛево.Доступность = Истина;
	КонецЕсли;
...
Процедура ОбновитьСоставЭлементовФормы(ТекстПредупреждения = "")
				   ...
				   И НЕ Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
					...
					ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Недопустимо удалять тип ""%1"",
						           ...
						           |en = 'It is forbidden to remove the type ""%1""
						           |because additional values of the type already exist.
						           |First you need to remove these additional values.
						           |
						           |Removed type is restored.'"),
			...
			Если НЕ СтарыйТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
				...
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недопустимо одновременно использовать типы значения
					           ...
					           |
					           |Второй тип удален.';
					           |en = 'It is unacceptable to use value types ""%1"" and ""%2"".
					           |Second type is removed.'"),
					Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")),
					Строка(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) );
	...
	Иначе
		...
		Если СвойстваВладельца.ЭтоДополнительноеСведение <> Истина Тогда
			ШаблонУточнения = НСтр("ru = 'Список значений общий с реквизитом ""%1""'; en = 'List of values common with attribute ""%1""'");
		Иначе
			ШаблонУточнения = НСтр("ru = 'Список значений общий со сведением ""%1""'; en = 'List of values common with data ""%1""'");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СвойстваВладельца.НаборСвойств) Тогда
			ШаблонУточненияНабор = " " + НСтр("ru = 'набора ""%1""'; en = 'of set ""%1""'");
	...
	Иначе
		...
		Если СписокНаборов.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств)
			   И СписокНаборов.Количество() < 2 Тогда
				...
				Если Объект.ЭтоДополнительноеСведение Тогда
					ШаблонУточнения = НСтр("ru = 'Сведение входит в набор: %1'; en = 'Data is used in the set: %1'");
				Иначе
					ШаблонУточнения = НСтр("ru = 'Реквизит входит в набор: %1'; en = 'Attribute is used in the set: %1'");
				КонецЕсли;
			...
			Иначе
				Если СписокНаборов.Количество() > 1 Тогда
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = НСтр("ru = 'Общее сведение входит в %1 %2'; en = 'Common attribute is used in %1 %2'");
					Иначе
						ШаблонУточнения = НСтр("ru = 'Общий реквизит входит в %1 %2'; en = 'Common attribute is used in %1 %2'");
					КонецЕсли;
					
					СтрокаНаборы = ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(СписокНаборов.Количество(),
						"Л = ru_RU", НСтр("ru = 'набор,набора,наборов,,,,,,0'; en = 'set,set,sets,,,,,,0'"));
				...
				Иначе
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = НСтр("ru = 'Общее сведение входит в набор: %1'; en = 'Common attribute is used in the %1 set'");
					Иначе
						ШаблонУточнения = НСтр("ru = 'Общий реквизит входит в набор: %1'; en = 'Common attribute is used in the %1 set'");
					КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = НСтр("ru = 'Сведение не входит в набор'; en = 'Data is not used in the set'");
				Иначе
					ТекстКомментария = НСтр("ru = 'Реквизит не входит в набор'; en = 'Attribute is not used in the set'");
				КонецЕсли;
			Иначе
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = НСтр("ru = 'Общее сведение не входит в наборы'; en = 'Common attribute is not used in sets'");
				Иначе
					ТекстКомментария = НСтр("ru = 'Общий реквизит не входит в наборы'; en = 'Common attribute is not used in sets'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Элементы.УточнениеНаборовКомментарий.Гиперссылка Тогда
			Элементы.УточнениеНаборовКомментарий.Подсказка = НСтр("ru = 'Переход к набору'; en = 'Go to set'");
		Иначе
...
Процедура ЗаполнитьСписокДействийПриДобавленииРеквизита()
	...
	Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
		ДобавитьОбщий = НСтр("ru = 'Добавить общее сведение в набор (рекомендуется)
			...
			|en = 'Add common data to the set (recommended).
			|
			|The selected data already belongs to serveral sets, so it is recommended to include it to this set ""as is"".
			|In this case it will be possible to filter by it the data of different types in lists and reports.'");
		СделатьОбщим = НСтр("ru = 'Сделать дополнительное сведение общим и добавить в набор
			...
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';
			|en = 'Make the additional data a common one and add to the set. 
			|
			|This option is appropriate in cases when the data should be the same for both sets. 
			|It makes it possible to filter by it data of different types in lists and reports.'");
		СделатьПоОбразцу = НСтр("ru = 'Сделать копию сведения по образцу (с общим списком значений)
			...
			|en = 'Make a copy of the data using template (common list of values)
			| 
			|Value list of the attribute will be the same for both sets.
			|Using this option makes it convenient to configure value lists for several similar data at once.
			|In this case one can edit the description and other properties of the data.'");
		СоздатьКопированием = НСтр("ru = 'Сделать копию сведения
			|
			|Будет создана копия сведения%1';
			|en = 'Make a copy of data. 
			|A copy data is created %1'");
	Иначе
		ДобавитьОбщий = НСтр("ru = 'Добавить общий реквизит в набор (рекомендуется)
			...
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';
			|en = 'Add common attribute to the set (recommended).
			| 
			|The selected attribute already belongs to serveral sets, so it is recommended to include it to this set ""as is"".
			|In this case it will be possible to filter by it the data of different types in lists and reports.'");
		СделатьОбщим = НСтр("ru = 'Сделать реквизит общим и добавить в набор
			...
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.';
			|en = 'Make the attribute a common one and add to the set.
			|This option is appropriate in cases when the attribute should be the same for both sets.
			|It makes it possible to filter by it data of different types in lists and reports.'");
		СделатьПоОбразцу = НСтр("ru = 'Сделать копию реквизита по образцу (с общим списком значений)
			...
			|en = 'Make a copy of the attribute using template (common list of values)
			|
			|Value list of the attribute will be the same for both sets.
			|Using this option makes it convenient to configure value lists for several similar attributes at once.
			|In this case one can edit the description and other properties of the attribute.'");
		СоздатьКопированием = НСтр("ru = 'Сделать копию реквизита
			|
			|Будет создана копия реквизита%1';
			|en = 'Make a copy of attribute. 
			|A copy attribute is created %1'");
	КонецЕсли;
	...
	Если РеквизитСоСпискомДополнительныхЗначений Тогда
		ШаблонВставки = " " + НСтр("ru = 'и всех его значений.'; en = 'and all its values.'");
	Иначе
		ШаблонВставки = ".";
	КонецЕсли;
...
Процедура УстановитьЗаголовкиГиперссылок()
	...
	ШаблонЗависимостьЗадана = НСтр("ru = 'при заданном условии'; en = 'When the specified condition'");
	ШаблонЗависимостьНеЗадана = НСтр("ru = 'всегда'; en = 'always'");
...
Процедура УстановитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru = '(Дополнительное сведение)'; en = '(Additional data)'");
			Иначе
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru = '(Дополнительный реквизит)'; en = '(Additional attribute)'");
			КонецЕсли;
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru = '(Общее дополнительное сведение)'; en = '(Common additional data)'");
			Иначе
				Заголовок = Строка(Объект.Заголовок) + " " + НСтр("ru = '(Общий дополнительный реквизит)'; en = '(Common additional attribute)'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = НСтр("ru = 'Дополнительное сведение (создание)'; en = 'Additional data (create)'");
			Иначе
				Заголовок = НСтр("ru = 'Дополнительный реквизит (создание)'; en = 'Additional attribute (create)'");
			КонецЕсли;
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = НСтр("ru = 'Общее дополнительное сведение (создание)'; en = 'Common additional data (create)'");
			Иначе
				Заголовок = НСтр("ru = 'Общий дополнительный реквизит (создание)'; en = 'Common additional attribute (create)'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
		   ...
		   И УправлениеСвойствамиСлужебный.ТипЗначенияСодержитЗначенияСвойств(Выборка.ТипЗначения) Тогда
			...
			Если ПервыеЗначения.Количество() = 0 Тогда
				Если РезультатыЗапроса[1].Пустой() Тогда
					ПредставлениеЗначений = НСтр("ru = 'Значения еще не введены'; en = 'Values are not input'");
				Иначе
...
Функция НаименованиеУжеИспользуется(Знач Заголовок, Знач ТекущееСвойство, Знач НаборСвойств, НовоеНаименование)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстВопроса + Символы.ПС + Символы.ПС
		                         ...
		                         |en = 'It is recommended that you use a different name,
		                         |or else program may not work correctly.'"),
		НовоеНаименование);
...
Процедура УстановитьЗаголовокКнопкиФормата(Форма)
	...
	Если ПустаяСтрока(Форма.Объект.ФорматСвойства) Тогда
		ТекстЗаголовка = НСтр("ru = 'Формат по умолчанию'; en = 'Default format'");
	Иначе
		ТекстЗаголовка = НСтр("ru = 'Формат установлен'; en = 'Format is set'");
	КонецЕсли;
