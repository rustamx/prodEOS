...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СмешаннаяВажность = НСтр("ru = 'Различная'; en = 'Miscellaneous'");
...
Процедура Разместить(Команда)
	Если ПроверитьКоличествоВариантов() Тогда
		ЗаписатьНаСервере();
		ТекстОповещения = НСтр("ru = 'Изменены настройки вариантов отчетов (%1 шт.).'; en = 'Report options settings have been changed (%1 pcs).'");
...
Функция ПроверитьКоличествоВариантов()
	...
	Если КоличествоВариантов = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Необходимо заполнить список ""Варианты отчетов""'; en = 'You must fill the ""Report options"" list'"),
			,
			"ИзменяемыеВарианты");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПерезаполнитьДерево(ТолькоСнятьФлажки)
	...
	Если КоличествоВариантов = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Необходимо выбрать варианты отчетов'; en = 'You must select report options'");
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из ОтфильтрованныеВарианты Цикл
			Если ТекущаяПричина <> СтрокаТаблицы.Причина Тогда
				...
				Если ТекущаяПричина = 1 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ru = 'Помеченные на удаление:'; en = 'Marked for deletion:'");
				ИначеЕсли ТекущаяПричина = 2 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ru = 'Недостаточно прав для изменения:'; en = 'Insufficient permissinos to modify:'");
				ИначеЕсли ТекущаяПричина = 3 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ru = 'Отчет отключен или недоступен по правам:'; en = 'Report is disabled or not enough permissions:'");
				ИначеЕсли ТекущаяПричина = 4 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ru = 'Вариант отчета отключен по функциональной опции:'; en = 'Report option is disabled by functional option:'");
				КонецЕсли;
			КонецЕсли;
		...
		Если КоличествоВариантов = 0 Тогда
			РезультатВыполнения.Текст = НСтр("ru = 'Недостаточно прав для размещения в разделах выбранных вариантов отчетов.'; en = 'Insufficient permissinos to place into sections selected report options.'");
		...
		Иначе
			РезультатВыполнения.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недостаточно прав для размещения в разделах некоторых вариантов отчетов (%1).'; en = 'Insufficient permissions to place into sections some report options (%1).'"),
