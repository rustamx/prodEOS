		...
		Если МетаданныеРоли = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При проверке поставляемых полномочий ""%1""
				           ...
				           |en = 'When checking the supplied privileges ""%1"" 
				           |role ""%2 "" was not found in metadata.'"),
...
Функция ПоставляемыеПрофили()
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в процедуре ЗаполнитьПоставляемыеПрофилиГруппДоступа
		           ...
		           |
		           |'; en = 'Error in procedure УправлениеДоступомПереопределяемый.ЗаполнитьПоставляемыеПрофилиГруппДоступа.'");
	   ...
	   И НЕ ПараметрыОбновления.ОбновлятьИзмененныеПрофили Тогда
		...
		ВызватьИсключение ЗаголовокОшибки +
			НСтр("ru = 'Когда в переменной ПараметрыОбновления свойство
			           ...
			           |должно быть установлено Ложь.'; en = 'When the ОбновлятьИзмененныеПрофили property of the ПараметрыОбновления variable is False, then the ЗапретитьИзменениеПрофилей property must also be set to false.'");
	КонецЕсли;
	...
	ОписаниеПрофиляАдминистратор.Наименование  = НСтр("ru = 'Администратор'; en = 'Administrator'");
	...
	ОписаниеПрофиляАдминистратор.Описание =
		НСтр("ru = 'Предназначена для:
		           ...
		           |';
		           |en = 'Designed for: 
		           |- configuring settings for operation and maintenance of the information system, 
		           |- setting prrmissions of other users, 
		           |- marked objects deletion 
		           |- in rare cases to make some configuration changes. 
		           |
		           |Not recommend for ""normal"" work in the information system.'");
	...
	Для каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
			...
			Если ВсеРоли.Получить(Роль) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В описании полномочий ""%1""
					           |роль ""%2"" не найдена в метаданных.';
					           |en = 'In the description of the privileges ""%1"" 
					           |role ""%2"" was not found in metadata.'"),
		...
		Если СвойстваПрофилей.Получить(ОписаниеПрофиля.Идентификатор) <> Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Полномочия с идентификатором ""%1"" уже существуют.'; en = 'Privileges with ID ""%1"" already exist.'"),
		...
		Если ЗначениеЗаполнено(ОписаниеПрофиля.Имя) Тогда
			Если СвойстваПрофилей.Получить(ОписаниеПрофиля.Имя) <> Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Полномочия с именем ""%1"" уже существуют.'; en = 'Privileges with name ""%1"" already exist.'"),
					ОписаниеПрофиля.Имя);
			КонецЕсли;
			...
			Если МетаданныеРоли = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При обновлении поставляемых полномочий ""%1""
					           |роль ""%2"" не найдена в метаданных.';
					           |en = 'When updating the supplied privileges ""%1"" 
					           |role ""%2"" was not found in metadata.'"),
					СвойстваПрофиля.Наименование,
					Роль);
			КонецЕсли;
