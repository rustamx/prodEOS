...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Автор) Тогда
			...
			ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru = 'Рассылка отчетов. Открытие формы элемента'; en = 'Report mailing. Opening element form'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|en = 'Error filling the delivery author:
				|the user ""%1"" (%2) could not be the author.'"),
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Объект.Наименование) Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru = 'Не введено наименование'; en = 'Name is not entered'");
	...
	Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.Личная Тогда
		Если Не ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru = 'Не выбран тип получателей'; en = 'Recipient type is not selected'");
	...
	Если Объект.Подготовлена Тогда
		Если Объект.Отчеты.Количество() = 0 Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru = 'Не выбрано ни одного отчета'; en = 'You have not selected any report'");
		...
		Если Не ЗначениеЗаполнено(Объект.ПериодичностьРасписания) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("ru = 'Не выбрана периодичность запуска'; en = 'Start periodicity is not selected'");
		...
		Если Объект.ИспользоватьFTPРесурс Тогда
			Если Не ЗначениеЗаполнено(Объект.FTPСервер)
				...
				Отказ = Истина;
				ТекстСообщения = НСтр("ru = 'Не введен FTP адрес'; en = 'The FTP address is not entered'");
		...
		Если Объект.ИспользоватьСетевойКаталог Тогда
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогWindows) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru = 'Не введен сетевой каталог Windows'; en = 'Networked Windows directory is not entered'");
			...
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогLinux) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru = 'Не введен сетевой каталог Linux'; en = 'Network Linux directory is not entered'");
		...
		Если Объект.ИспользоватьПапку Тогда
			Если Не ЗначениеЗаполнено(Объект.Папка) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru = 'Не выбрана папка'; en = 'You have not selected a folder'");
		...
		Если Объект.ИспользоватьЭлектроннуюПочту Тогда
			Если Объект.Личная Тогда
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("ru = 'Не выбран почтовый адрес'; en = 'Email address is not entered'");
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("ru = 'Не выбран тип почтового адреса получателей'; en = 'You have not selected a type of mail addresses of the recipients'");
			...
			Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru = 'Не выбрана учетная запись для отправки'; en = 'You have not selected account for sending'");
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если СтрокаОтчеты.СКД Тогда
			...
			Если Не ЗаполненыВсеОбязательныеНастройки Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для отчета ''%1'' заполнены не все настройки, обязательные для заполнения. Необходимо заполнить все обязательные настройки или снять флажок ''Подготовлена''.'; en = 'Not all of the settings required are filled for the report ""%1"". You must fill in all the required settings or uncheck the ""Prepared"".'"),
		...
		Если ПроверитьПерсонализированные Тогда
			...
			Если ОтчетПерсонализирован Тогда
				Отказ = Истина;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для отчета ''%1'' задан отбор по получателям рассылки. Необходимо отключить отбор по получателям или изменить вид рассылки на ''Свой отчет для каждого получателя''.'; en = 'For the report ''%1'' filter on recipients is specified. You must disable the filter on recipients or change the kind of the reports delivery to kind ''Personal report for each recipient''.'"),
					Строка(СтрокаОтчеты.Отчет));
				Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
			КонецЕсли;
...
Процедура ТипПолучателейРассылкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.Получатели.Количество() > 0 Тогда
		...
		СтрокаВопроса = НСтр("ru = 'Для продолжения необходимо очистить список получателей.'; en = 'To continue, you need to clear the list of recipients.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Очистить'; en = 'Clear'"));
...
Процедура FTPСерверИКаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыПроизвольнойФормы.Вставить("Заголовок", НСтр("ru = '<Укажите получателя>'; en = '<Select recipient>'"));
...
Процедура ПапкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если Не ПравоИзмененияПапкиИФайлов(ВыбранноеЗначение) Тогда
		СтандартнаяОбработка = Ложь;
		ТекстПредупреждения = НСтр("ru = 'Недостаточно прав для изменения файлов папки ""%1"".'; en = 'Insufficient privileges to modify files in a folder ""%1"".'");
...
Процедура РодительОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ВыбранноеЗначение = Кэш.ГруппаЛичныхРассылок Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("ru = 'Выбранная группа используется только для личных рассылок по электронной почте'; en = 'The selected group is used only for personal email delivery'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтруктураВыбора.СОшибками.Количество > 0 Тогда
		...
		Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
			ТекстСообщения = НСтр("ru = 'Не удалось добавить отчет из-за ошибок'; en = 'The report could not be added due to errors'");
		ИначеЕсли СтруктураВыбора.Успешно.Количество = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Не удалось добавить отчеты из-за ошибок'; en = 'The reports could not be added due to errors'");
		ИначеЕсли СтруктураВыбора.Выбрано.Количество >= 6 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Добавлено отчетов: %1 из %2,
				|Не добавлено из-за ошибок: %3';
				|en = 'The reports added: %1 %2,
				|Not added due to errors: %3'"),
		...
		Иначе
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Успешно добавлены отчеты:
					...
					|%2';
					|en = 'Successfully added: %1  reports
					|
					|The following reports cannot be added due to errors: %2'"),
				ПредставлениеОтчетовУспешно,
				ПредставлениеОтчетовСОшибками);
		КонецЕсли;
		
		ПараметрыПредупреждения = Новый Структура("Заголовок, Текст, Подробно");
		ПараметрыПредупреждения.Заголовок = НСтр("ru = 'Ошибки добавления отчетов в рассылку'; en = 'Errors adding reports to delivery'");
	...
	Иначе
		...
		Если СтруктураВыбора.Успешно.Количество = 0 Тогда
			...
			ТекстОповещения = НСтр("ru = 'Все выбранные отчеты уже включены в рассылку'; en = 'All the selected reports are already included in the delivery'");
		Иначе
			Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
				ЗаголовокОповещения = НСтр("ru = 'Отчет включен в рассылку'; en = 'The report is included in the delivery'");
			Иначе
				ЗаголовокОповещения = НСтр("ru = 'Отчеты включены в рассылку'; en = 'Reports included in the delivery'");
			КонецЕсли;
...
Процедура ПолучателиРассылкиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ТипПолучателейРассылки) Тогда
		ТекстОшибки = НСтр("ru = 'Для ввода получателей необходимо выбрать их тип'; en = 'To enter recipient you must select the type of recipient'");
...
Процедура КомандаВыполнитьСейчас(Команда)
	Если Не Объект.Подготовлена Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Рассылка не подготовлена'; en = 'Report delivery not prepared'"));
		Возврат;
	КонецЕсли;
	ПараметрыЗаписи = Новый Структура;
...
Процедура ИспользоватьПолучателяРассылкиВНастройкеОтчета(Команда)
	...
	Если Не Объект.Персонализирована Тогда
		...
		СообщениеТекст = НСтр("ru = 'Использовать получателя в параметрах возможно только для вида рассылки ""%1""'; en = 'Use the recipient in the parameters is possible only for delivery kind ""%1""'");
	...
	Если КоличествоТипов <> 1 И КоличествоТипов <> 2 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Получатели"" не заполнено'; en = 'Field ""Recipients"" is not filled'"), , "ТипПолучателейРассылки");
		Возврат;
	КонецЕсли;
	...
	Если НайденныеИОМД.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Тип получателей не найден'; en = 'Recipient type is not found'"));
		Возврат;
	КонецЕсли;
	...
	Если СКД Тогда
		...
		Если ИдентификаторНастройки = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Выберите настройку отчета'; en = 'Select report settings'"));
			Возврат;
		КонецЕсли;
		...
		Если СтрокаНастроек = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Настройка отчета не найдена'; en = 'Report settings not found'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Указывать получателя можно только для параметров и отборов отчетов'; en = 'You can specify a recipient only for parameters and report filters'"));
			Возврат;
		КонецЕсли;
		...
		Если ЭтоЭлементОтбора Тогда
			...
			Иначе
				...
				КонецЕсли;
				Если ПолеКД = Неопределено Тогда
					ПоказатьПредупреждение(, НСтр("ru = 'Для настройки отчета не найдено описание доступного поля'; en = 'There is no available field description to customize the report'"));
					Возврат;
				КонецЕсли;
	...
	Если Не ОписаниеДоступныхТипов.СодержитТип(ОсновнойТипПолучателей) Тогда
		...
		|en = 'Type ""%1"" is not suitable for the type of the selected configuration.
		|You must select a different recipient type or other configuration.'");
...
Процедура ЗаполнитьРасписаниеПоШаблону(Команда)
	...
	СписокВариантов.ПоказатьВыборЭлемента(Обработчик, НСтр("ru = 'Выберите шаблон расписания'; en = 'Select schedule template'"));
КонецПроцедуры
...
Процедура ДобавитьШаблонПолучателя(Команда)
	...
	Если НЕ Объект.Персонализирована Тогда
		СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Использование получателя в тексте шаблона возможно только для вида рассылки ""%1""'; en = 'The use of the recipient in the template text is possible only for delivery kind ""%1""'"),
...
Процедура ДобавитьШаблонПоУмолчанию(Команда)
	...
	ИначеЕсли ЗначениеПредмета = ШаблонПоУмолчанию Тогда
		...
		Если ПерезаполняетсяТема Тогда
			ТекстПредупреждения = НСтр("ru = 'Тема письма уже соответствует шаблону по умолчанию.'; en = 'Email subject already matches the default template.'");
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Текст письма уже соответствует шаблону по умолчанию.'; en = 'Text of email already matches the default template.'");
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстПредупреждения);
	...
	Иначе
		...
		Если ПерезаполняетсяТема Тогда
			ЗаголовокВопроса = НСтр("ru = 'Добавить в тему письма шаблон по умолчанию'; en = 'Add in the email subject by default template'");
			ТекстВопроса = НСтр("ru = 'Заменить тему письма на шаблон по умолчанию?'; en = 'Replace the email subject with the default template?'");
		Иначе
			ЗаголовокВопроса = НСтр("ru = 'Добавить в текст письма шаблон по умолчанию'; en = 'Add to the text of a email from the default template'");
			ТекстВопроса = НСтр("ru = 'Заменить текст письма на шаблон по умолчанию?'; en = 'Replace the email text with the default template?'");
		КонецЕсли;
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("ru = 'Заменить'; en = 'Replace'"));
		Кнопки.Добавить(2, НСтр("ru = 'Добавить'; en = 'Add'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ПроверитьРассылкуПослеВыбораПолучателей(РезультатВыбора, ПараметрыДоставки) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.Вставить("ШаблонТекста",  НСтр("ru = 'Это сообщение отправлено системой рассылок 1С:Предприятие.'; en = 'This message is sent by the delivery system of 1С:Enterprise.'") + Символы.ПС + Кэш.ЗаголовокСистемы);
...
Функция ВыбратьФормат(СсылкаНаОтчет, ОбработчикРезультата)
	...
	Если ЭтоФорматПоУмолчанию Тогда
		ЗаголовокДиалога = НСтр("ru = 'Выберите форматы по умолчанию'; en = 'Select the default formats'");
	Иначе
		ЗаголовокДиалога = НСтр("ru = 'Выберите форматы для отчета ""%1""'; en = 'Select formats for report ""%1""'");
...
Процедура ПроверитьРассылку(ПараметрыДоставки)
	...
	Если Не Объект.Подготовлена Или Объект.Ссылка.Пустая() Тогда
		ЗаголовокВопроса = НСтр("ru = 'Проверка способа доставки'; en = 'Check delivery method'");
		Если Не Объект.Подготовлена Тогда
			...
			|en = 'Before checking the delivery should be prepared. 
			|Click ""Continue"" to select ""Prepared"" and save the delivery.'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Перед проверкой рассылка должна быть записана.
			|Нажмите ""Продолжить"", чтобы записать рассылку.';
			|en = 'Before checking the delivery should be saved.
			|Click ""Continue"" to save the delivery.'");
		КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		Если Объект.Личная <> ИспользуетсяГруппаЛичныхРассылок Тогда
			УстановитьМодифицированностьФормы(Форма, "Родитель", , 
				НСтр("ru = 'Группа установлена в соответствии с видом рассылки'; en = 'Group is set in accordance with the distribution kind'"));
	...
	Если Изменения = "" Или Изменения = "ПериодичностьРасписания" Тогда
		...
		ИначеЕсли ИмяПеречисления = "Ежемесячно" Тогда
			...
			Если Форма.Расписание.ДеньВМесяце >= 0 Тогда
				...
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("ru = 'начала'; en = 'beginning'");
			Иначе
				Форма.ДеньВМесяце = -Форма.Расписание.ДеньВМесяце;
...
Функция ПредставлениеПолучателей(Форма)
	...
	Если Представление.Всего = 0 Тогда
		Представление.Краткое = НСтр("ru = '<Укажите получателей>'; en = '<Select recipients>'");
	...
	Если ПредставлениеОтключенных.Всего > 0 Тогда
		ШаблонРазделения = НСтр("ru = 'Кроме'; en = 'Except'")+ ": ";
...
Функция ПолучателиУказаны(Получатели)
	Для Каждого СтрокаТаблицы Из Получатели Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ТекстСообщения = НСтр("ru = 'Не выбрано ни одного получателя'; en = 'You have not selected any recipient'");
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПолучателиРассылки");
	Возврат Ложь;
КонецФункции
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru = 'Рассылка отчетов. Проверка способа доставки'; en = 'Delivery method test has successfully passed.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	Если Выполнена Тогда
		РезультатВыполнения.Текст = НСтр("ru = 'Тест способа доставки успешно пройден.'; en = 'Delivery method test failed.'");
	Иначе
		РезультатВыполнения.Текст = НСтр("ru = 'Тест способа доставки не пройден.'; en = 'Delivery method test failed.'");
...
Процедура ЗаполнитьСведенияТаблицыОтчеты()
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если ИнформацияОбОтчете = Неопределено Тогда
			...
			СтрокаОтчеты.Представление = НСтр("ru = '<Недостаточно прав для работы с отчетом>'; en = '<Insufficient privilege to work with a report>'");
		Иначе
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	...
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru = 'Рассылка отчетов. Инициализация отчета'; en = 'Report mailing. Report initialization'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	Если Не СтрокаОтчеты.Инициализирован Тогда
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|%2';
			|en = 'Report ""%1"" was deleted because of initialization error:
			|%2'"),
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		Если Не ЭтоНовый Тогда
			Если Найденные.Количество() > 0 Тогда
				ШаблонСтрокиСообщения = НСтр("ru = 'Для отчета ""%1"" загружены новые пользовательские настройки.'; en = 'New custom settings loaded for the report ""%1"".'");
			Иначе
				ШаблонСтрокиСообщения = НСтр("ru = 'Добавлен отчет ""%1"".'; en = 'Added report ""%1"".'");
			КонецЕсли;
...
Процедура ЗаписатьНаКлиенте(Результат, ПараметрыЗаписи) Экспорт
	...
	Если ПараметрыЗаписи.Шаг = 3 И ЖелательноОтключитьАрхивацию() Тогда
		...
		ТекстВопроса = НСтр("ru = 'При публикации отчетов в папку рекомендуется отключать архивацию в ZIP.'; en = 'When you publish the reports in a folder, we recommend that you disable the archiving to ZIP.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Отключить архивацию в ZIP'; en = 'Disable archiving to ZIP'"));
