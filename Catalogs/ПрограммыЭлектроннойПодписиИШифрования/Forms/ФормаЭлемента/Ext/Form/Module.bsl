...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЦикла;
	Элементы.Наименование.СписокВыбора.Добавить("", НСтр("ru = '<Другая программа>'; en = '<Another program>'"));
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Программа с указанным именем и типом уже добавлена в список.'; en = 'Program with the specified name and type is already added to the list.'"),
			,
			"Объект.ИмяПрограммы");
	КонецЕсли;
...
Процедура УстановитьПометкуУдаления(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ru = 'Для установки отметки удаления необходимо записать внесенные Вами изменения.
		           |Записать данные?'; en = 'To mark for deletion you should save implemented changes. Do you want to save changes?'"), РежимДиалогаВопрос.ДаНет);
