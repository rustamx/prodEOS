...
Процедура ПапкиПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
		...
		Если ИзменитьПапкуДокументов(ЗначениеПеретаскивания, Строка) = Истина Тогда
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Внутренний документ ""%1"" перенесен в папку ""%2""'; en = 'Internal document ""%1"" moved to folder ""%2""'"), ЗначениеПеретаскивания[0], Строка);
			...
			Иначе
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Внутренние документы (%1 шт.) перенесены в папку ""%2""'; en = 'Internal documents (%1 pcs.) moved to folder ""%2""'"), ЗначениеПеретаскивания.Количество(), Строка);
	...
	Если ТипЗнч(ЗначениеПеретаскивания) = Тип("Массив") 
		...
		Если РаботаСФайламиВызовСервера.СменитьРодителяПапок(ЗначениеПеретаскивания, Строка, НайденоЗацикливание) = Истина Тогда
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Папка ""%1"" перенесена в папку ""%2""'; en = 'Folder ""%1"" moved into folder ""%2""'"), ЗначениеПеретаскивания[0], Строка);
			...
			Иначе
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Папки (%1 шт.) перенесены в папку ""%2""'; en = 'Folders (%1 pcs.) moved to folder ""%2""'"), ЗначениеПеретаскивания.Количество(), Строка);
...
Процедура Подписать(Команда)
	...
	Если Элементы.Список.ТекущиеДанные.ЯвляетсяКомплектомДокументов = Истина Тогда
		ТекстПредупреждения = НСтр("ru = 'Комплекты документов не могут быть подписаны ЭП.'; en = 'Document sets cannot be signed digitally.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьОтчеты(Команда)
	...
	ЗаголовокФормы = НСтр("ru = 'Отчеты по внутренним документам'; en = 'Reports on internal documents'");
...
Процедура ОбработатьПеретаскиваниеДокументов(ПараметрыПеретаскивания, ИмяРеквизита, ЗначениеРеквизита)
	...
	Если ТипЗнч(ЗначениеПеретаскивания) = Тип("Массив") 
		...
		Если ИзменитьРеквизитДокументов(ЗначениеПеретаскивания, ИмяРеквизита, ЗначениеРеквизита) = Истина Тогда
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Внутренний документ ""%1"" перенесен в ""%2""'; en = 'Internal document ""%1"" moved to ""%2""'"), ЗначениеПеретаскивания[0], ЗначениеРеквизита);
			...
			Иначе
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Внутренние документы (%1 шт.) перенесены в ""%2""'; en = 'Internal documents (%1 pcs.) moved to ""%2""'"), ЗначениеПеретаскивания.Количество(), ЗначениеРеквизита);
...
Функция ИзменитьПапкуДокументов(МассивДокументов, НоваяПапка)
	...
	Для Каждого ДокументСсылка Из МассивДокументов Цикл
		...
		Если Не Делопроизводство.ПолеДокументаДоступноПоСостоянию("Папка", ДокументСсылка) Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'В текущем состоянии запрещено изменять папку у документа ""%1"".'; en = 'It is forbidden to change folder of document ""%1"" in its current state.'"),
								Строка(ДокументСсылка));
		КонецЕсли;
