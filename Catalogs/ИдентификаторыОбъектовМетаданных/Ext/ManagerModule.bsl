...
Процедура ПроверкаИспользования(ОбъектыРасширений = Ложь) Экспорт
	...
	Если СтандартныеПодсистемыПовтИсп.ОтключитьИдентификаторыОбъектовМетаданных() Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Справочник ""%1"" не используется.'; en = 'Catalog ""%1"" is not used.'"), НазваниеСправочника(ОбъектыРасширений));
	КонецЕсли;
	   ...
	   И ЗначениеЗаполнено(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени("Константа.ГлавныйУзел").Получить()) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Справочник ""%1"" не может использоваться
			           ...
			           |en = 'Catalog ""%1"" cannot be used
			           |in infobase with unconfirmed cancellation of the master node.
			           |
			           |To restore connection to the master node start 1C:Enterprise and press
			           |Restore button or set the master node programmatically from the value saved
			           |in constant  ГлавныйУзел.
			           |
			           |To confirm the cancellation start 1C:Enterprise and press Disconnect button or
			           |clear the constant ГлавныйУзел programmatically.'"),
				...
				Если Не ЗначениеЗаполнено(СвойстваЗагружаемого.Ссылка) Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка загрузки идентификаторов объектов метаданных.
						           ...
						           |""%1"".';
						           |en = 'Error importing metadata object identifiers.
						           |It is impossible to import a new element if a new object reference is not specified:
						           |""%1"".'"),
						Объект.ПолноеИмя);
				КонецЕсли;
			КонецЕсли;
			...
			Если ПолныеИменаЗагружаемых.Получить(НРег(Объект.ПолноеИмя)) <> Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка загрузки идентификаторов объектов метаданных.
					           ...
					           |""%1"".';
					           |en = 'Error importing metadata object identifiers. 
					           |It is impossible to import two elements with the identical full name: 
					           |""%1"".'"),
					Объект.ПолноеИмя);
			КонецЕсли;
				...
				Если КлючиЗагружаемых.Получить(КлючОбъектаМетаданных) <> Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка загрузки идентификаторов объектов метаданных.
						           ...
						           |""%1"".';
						           |en = 'Error importing metadata object identifiers. 
						           |It is impossible to import two elements with the identical key of metadata object: 
						           |""%1"".'"),
				...
				Если СвойстваЗагружаемого.ОбъектМетаданныхПоКлючу <> СвойстваЗагружаемого.ОбъектМетаданныхПоПолномуИмени Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка загрузки идентификаторов объектов метаданных.
						           ...
						           |en = 'Error importing metadata object identifiers.
						           |It is impossible to import an element with the key of metadata object ""%1""
						           |not matching its full name ""%2"".'"),
	   ...
	   И Не Справочники.ВерсииРасширений.УстановленныеРасширенияДоступны() Тогда
		...
		ТекстОшибки =
			НСтр("ru = 'Невозможно обновить идентификаторы объектов расширений
			           ...
			           |en = 'Unable to update identifiers of extension objects in a non-separated
			           |user session because extensions for separated users are not connected.'");
...
Функция ОбъектыМетаданныхПоИдентификаторам(Идентификаторы) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru = 'Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().'; en = 'An error occurred while executing the function ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().'");
				...
				ИначеЕсли РезультатПроверки.КлючОбъектаМетаданных = Неопределено Тогда
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Идентификатору ""%1""
						           ...
						           |en = 'Identifier ""%1""
						           |found in catalog ""%2"",
						           |corresponds to non-existent metadata object
						           |""%3"".'"),
				...
				Иначе
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Идентификатору ""%1""
						           ...
						           |en = 'Identifier ""%1""
						           |found in catalog ""%2"",
						           |corresponds to a deleted metadata object.'"),
			...
			Иначе
				ОписаниеОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Идентификатору ""%1""
					           ...
					           |en = 'Identifier ""%1""
					           |found in catalog ""%2"",
					           |correpsonds to metadata object ""%3"",
					           |full name of which differs from the one set in Configurator mode.'"),
		...
		Если Свойства.ПометкаУдаления Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Идентификатор ""%1""
				           ...
				           |en = 'Identifier ""%1""
				           |found in catalog ""%2"",
				           |but is ""Marked for deletion"" is True.'"),
				Строка(Свойства.Ссылка), НазваниеСправочника);
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных().
				           ...
				           |""%1"".';
				           |en = 'An error occurred while executing the function ОбщегоНазначения.ИдентификаторОбъектаМетаданных().
				           |Metadata object not found by type:
				           |""%1"".'"),
...
Функция ИдентификаторОбъектаМетаданныхПоПолномуИмени(ПолноеИмяОбъектаМетаданных) Экспорт
	...
	Выгрузка = Запрос.Выполнить().Выгрузить();
	ЗаголовокОшибки = НСтр("ru = 'Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных().'; en = 'An error occurred while executing the function ОбщегоНазначения.ИдентификаторОбъектаМетаданных().'");
	...
	Если Выгрузка.Количество() = 0 Тогда
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Объект метаданных не найден по полному имени:
				           |""%1"".';
				           |en = 'Metadata object is not found by its full name:
				           |""%1"".'"),
		   ...
		   И Не ЭтоПодсистема(ОбъектМетаданных) Тогда
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Объект метаданных не поддерживается:
				           ...
				           |""%1"".
				           |
				           |Only the types enumerated in comments to function can be used.'"),
				ПолноеИмяОбъектаМетаданных);
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + ОписаниеОшибки;
		КонецЕсли;
		...
		ШаблонОшибки = ?(ДоступныИдентификаторыОбъектовРасширений,
			НСтр("ru = 'Для объекта метаданных ""%1""
			           ...
			           |not found identifier in catalog ""Metadata object ids"" and
			           |in information register ""Object identifiers for versions of extensions"".'"),
			НСтр("ru = 'Для объекта метаданных ""%1""
			           |не найден идентификатор в справочнике ""Идентификаторы объектов метаданных"".';
			           |en = 'For metadata object ""%1""
			           |not found identifier in catalog ""Metadata object ids"".'"));
	...
	ИначеЕсли Выгрузка.Количество() > 1 Тогда
		ШаблонОшибки = ?(ДоступныИдентификаторыОбъектовРасширений,
			НСтр("ru = 'Для объекта метаданных ""%1""
			           ...
			           |регистре сведений ""Идентификаторы объектов версий расширений"".';
			           |en = 'For metadata object ""%1""
			           |found several identifiers in catalog ""Metadata object ids"" and
			           |in information register ""Object identifiers for versions of extensions"".'"),
			НСтр("ru = 'Для объекта метаданных ""%1""
			           |найдено несколько идентификаторов в справочнике ""Идентификаторы объектов метаданных"".';
			           |en = 'For metadata object ""%1""
			           |found several identifiers in catalog ""Metadata object ids"".'"));
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПолноеИмяОбъектаМетаданных)
			+ СтандартныеПодсистемыСервер.УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика();
	КонецЕсли;
		...
		Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для объекта метаданных ""%1""
				           ...
				           |en = 'For metadata object ""%1""
				           |found the identifier in catalog ""%2"",
				           |corresponding to a deleted metadata object.'"),
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для объекта метаданных ""%1""
				           ...
				           |en = 'For metadata object ""%1""
				           |found the identifier in catalog ""%2"",
				           |corresponding to another metadata object ""%3"".'"),
...
Процедура ДобавитьПереименование(Итог, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки = "") Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в процедуре ПриДобавленииПереименованийОбъектовМетаданных общего модуля ОбщегоНазначенияПереопределяемый.'; en = 'Error in procedure ПриДобавленииПереименованийОбъектовМетаданных of common module ОбщегоНазначенияПереопределяемый.'");
	...
	Если СтароеИмяКоллекции <> НовоеИмяКоллекции Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не совпадают имена типов переименованного объекта метаданных.
			           |Прежний тип: ""%1"",
			           |новый тип: ""%2"".';
			           |en = 'Names of types of the renamed metadata object do not match.
			           |Previous type: ""%1"",
			           |New type: ""%2"".'"),
			СтароеПолноеИмя,
			НовоеПолноеИмя);
	...
	Если Итог.КоллекцииБезКлюча[СтароеИмяКоллекции] = Неопределено Тогда
		...
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для типа объекта метаданных ""%1"" не требуется описывать переименование,
			           ...
			           |%2.';
			           |en = 'For metadate object type ""%1"" it is not necessary to describe renaming, because
			           |information on these metadata objects are updated automatically.
			           |
			           |Renaming shoud be described for the following types only:
			           |%2.'"),
			СтароеПолноеИмя,
			СписокДопустимыхТипов);
	КонецЕсли;
...
Функция СвойстваКоллекцийОбъектовМетаданных(ОбъектыРасширений = Ложь) Экспорт
	...
	Строка.Синоним         = НСтр("ru = 'Константы'; en = 'Constants'");
	Строка.ИмяВЕдЧисле     = "Константа";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Константа'; en = 'Constant'");
	
	// Подсистемы
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("cdf5ac50-08e8-46af-9a80-4e63fd4a88ff");
	Строка.Имя             = "Подсистемы";
	Строка.Синоним         = НСтр("ru = 'Подсистемы'; en = 'Subsystems'");
	Строка.ИмяВЕдЧисле     = "Подсистема";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Подсистема'; en = 'Subsystem'");
	...
	Строка.Имя             = "Роли";
	Строка.Синоним         = НСтр("ru = 'Роли'; en = 'Roles'");
	Строка.ИмяВЕдЧисле     = "Роль";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Роль'; en = 'Role'");
	Строка.БезДанных       = Истина;
	Строка.БезКлючаОбъектаМетаданных = Истина;
	...
	Строка.Имя             = "Отчеты";
	Строка.Синоним         = НСтр("ru = 'Отчеты'; en = 'Reports'");
	Строка.ИмяВЕдЧисле     = "Отчет";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Отчет'; en = 'Report'");
	Строка.БезДанных       = Истина;
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("ru = 'Обработки'; en = 'Dataprocessors'");
	Строка.ИмяВЕдЧисле     = "Обработка";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Обработка'; en = 'Dataprocessor'");
	Строка.БезДанных       = Истина;
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("ru = 'Планы видов характеристик'; en = 'Charts of characteristic types'");
	Строка.ИмяВЕдЧисле     = "ПланВидовХарактеристик";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'План видов характеристик'; en = 'Chart of characteristic types'");
	
	// ПланыСчетов
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("4295af27-543f-4373-bcfc-c0ace9b7620c");
	Строка.Имя             = "ПланыСчетов";
	Строка.Синоним         = НСтр("ru = 'Планы счетов'; en = 'Charts of accounts'");
	Строка.ИмяВЕдЧисле     = "ПланСчетов";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'План счетов'; en = 'Chart of accounts'");
	
	// ПланыВидовРасчета
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("fca3e7e1-1bf1-49c8-9921-aafb4e787c75");
	Строка.Имя             = "ПланыВидовРасчета";
	Строка.Синоним         = НСтр("ru = 'Планы видов расчета'; en = 'Charts of calculation types'");
	Строка.ИмяВЕдЧисле     = "ПланВидовРасчета";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'План видов расчета'; en = 'Chart of calculation types'");
	
	// РегистрыСведений
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("d7ecc1e9-c068-44dd-83c2-1323ec52dbbb");
	Строка.Имя             = "РегистрыСведений";
	Строка.Синоним         = НСтр("ru = 'Регистры сведений'; en = 'Information registers'");
	Строка.ИмяВЕдЧисле     = "РегистрСведений";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Регистр сведений'; en = 'Information register'");
	
	// РегистрыНакопления
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("74083488-b01e-4441-84a6-c386ce88cdb5");
	Строка.Имя             = "РегистрыНакопления";
	Строка.Синоним         = НСтр("ru = 'Регистры накопления'; en = 'Accumulation registers'");
	Строка.ИмяВЕдЧисле     = "РегистрНакопления";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Регистр накопления'; en = 'Accumulation register'");
	
	// РегистрыБухгалтерии
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("9a0d75ff-0eda-454e-b2b7-d2412ffdff18");
	Строка.Имя             = "РегистрыБухгалтерии";
	Строка.Синоним         = НСтр("ru = 'Регистры бухгалтерии'; en = 'Accounting registers'");
	Строка.ИмяВЕдЧисле     = "РегистрБухгалтерии";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Регистр бухгалтерии'; en = 'Accounting register'");
	
	// РегистрыРасчета
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("f330686a-0acf-4e26-9cda-108f1404687d");
	Строка.Имя             = "РегистрыРасчета";
	Строка.Синоним         = НСтр("ru = 'Регистры расчета'; en = 'Calculation registers'");
	Строка.ИмяВЕдЧисле     = "РегистрРасчета";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Регистр расчета'; en = 'Calculation register'");
	
	// БизнесПроцессы
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("a8cdd0e0-c27f-4bf0-9718-10ec054dc468");
	Строка.Имя             = "БизнесПроцессы";
	Строка.Синоним         = НСтр("ru = 'Бизнес-процессы'; en = 'Business processes'");
	Строка.ИмяВЕдЧисле     = "БизнесПроцесс";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Бизнес-процесс'; en = 'Business process'");
	
	// Задачи
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("8d9153ad-7cea-4e25-9542-a557ee59fd16");
	Строка.Имя             = "Задачи";
	Строка.Синоним         = НСтр("ru = 'Задачи'; en = 'Tasks'");
	Строка.ИмяВЕдЧисле     = "Задача";
	Строка.СинонимВЕдЧисле = НСтр("ru = 'Задача'; en = 'Task'");
	...
	Если Не Объект.ПометкаУдаления Тогда
		ВызватьИсключениеПоОшибке(ОбъектыРасширений,
			НСтр("ru = 'Удаление идентификаторов объектов, у которых значение
			           ...
			           |en = 'Deletion of object identifers with attribute ""Marked
			           |for deletion"" set to False is not allowed.'"));
	КонецЕсли;
...
Процедура ОбновитьДанные(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка,
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СписокПереименованийОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = НСтр("ru = 'Переименование идентификаторов объектов метаданных СтароеПолноеИмя -> НовоеПолноеИмя:'; en = 'Renaming identifiers of metadata objects СтароеПолноеИмя -> НовоеПолноеИмя:'")
		...
		Если ЗначениеЗаполнено(СписокНовыхОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = СписокКритичныхИзменений
				+ НСтр("ru = 'Добавление новых идентификаторов объектов метаданных:'; en = 'Adding new metadata object ids:'")
		...
		Если Не ТолькоПроверка Тогда
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				ЗаписьЖурналаРегистрации(?(ОбъектыРасширений,
						НСтр("ru = 'Идентификаторы объектов расширений.Выполнены критичные изменения'; en = 'Extension objects identifiers. Critical changes made'",
							ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
						НСтр("ru = 'Идентификаторы объектов метаданных.Выполнены критичные изменения'; en = 'Metadata object IDs.Critical changes made'",
							ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
		...
		Если СтрЧислоВхождений(ВРег(ПолноеИмя), ВРег("Подсистема.")) > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при поиске дочерней подсистемы по полному имени (при поиске ""%1"" была найдена ""%2"").
				           ...
				           |en = 'An error occured while searching for subsystem by its full name (when searching for ""%1"" ""%2"" was found).
				           |Subsystems should not have identical names or the platform version should be higher.'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при поиске объекта метаданных по полному имени (при поиске ""%1"" был найден ""%2"").'; en = 'An error occured while searching for metadata object by its full name (when searching for ""%1"" ""%2"" was found).'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		КонецЕсли;
	КонецЕсли;
...
Функция НазваниеСправочника(ОбъектыРасширений)
	...
	Если ОбъектыРасширений Тогда
		НазваниеСправочника = НСтр("ru = 'Идентификаторы объектов расширений'; en = 'Extension objects identifiers'");
	Иначе
		НазваниеСправочника = НСтр("ru = 'Идентификаторы объектов метаданных'; en = 'Metadata object ids'");
	КонецЕсли;
...
Процедура ВызватьИсключениеПоОшибке(ОбъектыРасширений, ТекстОшибки)
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка при работе со справочником ""%1"".'; en = 'An error occured while working with catalog ""%1"".'"),
		НазваниеСправочника(ОбъектыРасширений));
		...
		Если Объект.ЭтоНовый() Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("ru = 'Создание нового идентификатора объекта
				           ...
				           |en = 'New object identifier can only be created automatically
				           |during the catalog data update.'"));
		...
		ИначеЕсли ЗапрещеноИзменятьПолноеИмя(Объект) Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При изменении идентификатора объекта указано
				           ...
				           |en = 'While changing object identifier its full name ""%1""
				           |was specified that can only be set automatically during 
				           |the catalog data update'"),
				Объект.ПолноеИмя));
		Иначе
			...
			Если ЗначениеЗаполнено(НазваниеСправочника) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При изменении идентификатора объекта указано
					           ...
					           |en = 'While changing object identifier its full name ""%1"" 
					           |was specified that is already in use
					           |in catalog ""%2"".'"),
		   ...
		   И Не ЭтоКоллекция(Объект.ПолучитьСсылкуНового(), ЭтоОбъектРасширений(Объект)) Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("ru = 'Добавление новых элементов может быть выполнено
				           |только в главном узле распределенной информационной базы.'; en = 'Adding new elements can only be done in the master node of distributed information base.'"));
		КонецЕсли;
			...
			Если ВРег(Объект.ПолноеИмя) <> ВРег(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ПолноеИмя")) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений,
					НСтр("ru = 'Изменение реквизита ""Полное имя"" может быть выполнено
					           |только в главном узле распределенной информационной базы.'; en = 'Attribute ""Full description"" can only be modified in the master node of distributed information base.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОписаниеОшибкиИдентификаторыОбъектовРасширенийНедоступныВНеразделенномРежиме()
	...
	Возврат
		НСтр("ru = 'Справочник ""Идентификаторы объектов расширений""
		           ...
		           |en = 'Catalog ""Extension objects identifiers"" cannot
		           |be used in the non-separated mode.'");
	...
	Попытка
		Для Каждого СтрокаТаблицы Из ТаблицаСсылок Цикл
			...
			ИначеЕсли Метаданные.РегистрыСведений.Содержит(СтрокаТаблицы.Метаданные) Тогда
				...
				КонецПопытки;
			Иначе
				СообщитьОбОшибке(НСтр("ru = 'Значения не заменяются в данных типа'; en = 'Values are not replaced in data of the type'")
...
Процедура СообщитьОбОшибке(Знач Описание, ОбъектыРасширений)
	...
	ЗаписьЖурналаРегистрации(
		?(ОбъектыРасширений,
			НСтр("ru = 'Идентификаторы объектов расширений.Замена идентификатора'; en = 'Extension objects identifiers.Identifier replacement'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			НСтр("ru = 'Идентификаторы объектов метаданных.Замена идентификатора'; en = 'Metadata object ids.Identifier replacement'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
