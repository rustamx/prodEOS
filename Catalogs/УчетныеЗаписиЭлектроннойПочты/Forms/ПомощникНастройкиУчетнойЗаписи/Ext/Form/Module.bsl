...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НастройкаУчетнойЗаписиЗаголовок.Заголовок = ?(КонтекстныйРежим,
		НСтр("ru = 'Для отправки писем необходимо настроить учетную запись электронной почты'; en = 'To send messages, you must set up an email account'"),
		НСтр("ru = 'Введите параметры учетной записи'; en = 'Enter your account settings'"));
		
	Если Не КонтекстныйРежим Тогда
		Заголовок = НСтр("ru = 'Создание учетной записи электронной почты'; en = 'Creating an email account'");
	Иначе
		Заголовок = НСтр("ru = 'Настройка учетной записи электронной почты'; en = 'Setting up email account'");
	КонецЕсли;
...
Процедура ПоказатьВопросПередЗакрытиемФормы()
	ТекстВопроса = НСтр("ru = 'Введенные данные не записаны. Закрыть форму?'; en = 'The entered data is not saved. Close the form?'");
	...
	Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru = 'Не закрывать'; en = 'Do not close'"));
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Введенные данные не записаны. Закрыть форму?'; en = 'The entered data is not saved. Close the form?'"), Кнопки,,
		КодВозвратаДиалога.Отмена, НСтр("ru = 'Настройка учетной записи'; en = 'Email account setup'"));
КонецПроцедуры
...
Процедура ПроверитьЗаполнениеНаСтраницеНастройкаУчетнойЗаписи(Отказ)
	...
	ИначеЕсли Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(АдресЭлектроннойПочты, Истина) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Адрес электронной почты введен неверно'; en = 'Incorrect email address'"), , "АдресЭлектроннойПочты", , Отказ);
	КонецЕсли;
...
Процедура НастроитьЭлементыТекущейСтраницы()
	...
	Иначе
		Если ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
			И ПроверкаЗавершиласьСОшибками Тогда
				ЗаголовокКнопкиДалее = НСтр("ru = 'Повторить'; en = 'Retry'");
		ИначеЕсли ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи
			И СпособНастройки = "Автоматически" Тогда
			Если КонтекстныйРежим Тогда
				ЗаголовокКнопкиДалее = НСтр("ru = 'Настроить'; en = 'Configure'");
			Иначе
	...
	Если Не КонтекстныйРежим И ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Элементы.КнопкаПерейтиКНастройкам.Заголовок = НСтр("ru = 'Перейти к учетной записи'; en = 'Go to your account'");
	Иначе
	...
	Если ТекущаяСтраница = Элементы.УчетнаяЗаписьУспешноНастроена Тогда
		Элементы.НадписьУчетнаяЗаписьУспешноНастроена.Заголовок = СтрШаблон(
			НСтр("ru = 'Настройка параметров учетной записи
				|%1 завершена.'; en = 'Configuring settings for account %1 completed.'"), АдресЭлектроннойПочты);
	КонецЕсли;
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты()
	...
	Тело = НСтр("ru = 'Это сообщение отправлено подсистемой электронной почты 1С:Документооборот'; en = 'This message is sent by an e-mail subsystem 1C:Enterprise'");
...
Процедура ПроверитьНастройкиУчетнойЗаписи()
	...
	Если Не ПустаяСтрока(СообщениеСервераИсходящейПочты) Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось подключиться к серверу исходящей почты:'; en = 'Unable to the outgoing mail server:'" + Символы.ПС)
			+ СообщениеСервераИсходящейПочты + Символы.ПС;
	КонецЕсли;
	
	Если Не ПустаяСтрока(СообщениеСервераВходящейПочты) Тогда
		ТекстОшибки = ТекстОшибки
			+ НСтр("ru = 'Не удалось подключиться к серверу входящей почты:'; en = 'Unable to the incoming mail server:'" + Символы.ПС)
			+ СообщениеСервераВходящейПочты;
	КонецЕсли;
...
Процедура СоздатьУчетнуюЗапись()
	...
	Если Справочники.УчетныеЗаписиЭлектроннойПочты.ЕстьУчетнаяЗаписьСТакимАдресом(АдресЭлектроннойПочты, ВариантИспользования, Неопределено) Тогда
		
		ПроверкаЗавершиласьСОшибками = Истина;
		
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Адрес электронной почты не уникален'; en = 'The email address is not unique'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			
		Сообщить(НСтр("ru = 'Адрес электронной почты не уникален.'; en = 'The email address is not unique.'"));
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при создании учетной записи электронной почты'; en = 'Error creating email account'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
...
Процедура НастроитьПараметрыПодключенияАвтоматически()
	...
	|en = 'Failed to determine the connection settings. 
...
Процедура ПодобратьНастройкиУчетнойЗаписи()
	...
	НаименованиеЗадания = НСтр("ru = 'Поиск настроек почтового сервера'; en = 'Searching the email server settings'");
