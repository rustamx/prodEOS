...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ОбъектАвторизации) Тогда
		ВызватьИсключение НСтр("ru = 'У внешнего пользователя не задан объект авторизации.'; en = 'External user authorization object is not set.'");
	Иначе
		ТекстОшибки = "";
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СтарыйОбъектАвторизации)
		   И СтарыйОбъектАвторизации <> ОбъектАвторизации Тогда
			...
			ВызватьИсключение НСтр("ru = 'Невозможно изменить ранее указанный объект авторизации.'; en = 'You can not change the specified authorization object.'");
		КонецЕсли;
	КонецЕсли;
