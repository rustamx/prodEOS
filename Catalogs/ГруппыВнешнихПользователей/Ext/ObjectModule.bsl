...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Родитель = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
			"Объект.Родитель",
			НСтр("ru = 'Предопределенная группа ""Все внешние пользователи"" не может быть родителем.'; en = 'Predefined group ""All external users"" cannot be a parent.'"),
			"");
	КонецЕсли;
	...
	Если Назначение.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
			"Объект.Назначение",
			НСтр("ru = 'Не указан вид пользователей.'; en = 'User type is not specifyed'"),
			"");
	КонецЕсли;
	...
	Для каждого ТекущаяСтрока Из Состав Цикл
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.ВнешнийПользователь) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				НСтр("ru = 'Внешний пользователь не выбран.'; en = 'External user is not selected.'"),
				"Объект.Состав",
				НомерСтроки,
				НСтр("ru = 'Внешний пользователь в строке %1 не выбран.'; en = 'External user in line %1 is not selected.'"));
			Продолжить;
		КонецЕсли;
		...
		Если НайденныеЗначения.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				"Объект.Состав[%1].ВнешнийПользователь",
				НСтр("ru = 'Внешний пользователь повторяется.'; en = 'External user is repeated.'"),
				"Объект.Состав",
				НомерСтроки,
				НСтр("ru = 'Внешний пользователь в строке %1 повторяется.'; en = 'External user in line %1 is repeated.'"));
		КонецЕсли;
	КонецЦикла;
...
Процедура ПередЗаписью(Отказ)
	...
	Если Ссылка = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
		...
		Если НЕ Родитель.Пустая() Тогда
			ВызватьИсключение
				НСтр("ru = 'Предопределенная группа ""Все внешние пользователи"" не может быть перемещена.'; en = 'Predefined group ""All external users"" cannot be moved.'");
		КонецЕсли;
		Если Состав.Количество() > 0 Тогда
			ВызватьИсключение
				НСтр("ru = 'Добавление участников в предопределенную группу ""Все внешние пользователи"" запрещено.'; en = 'Add members to a predefined group ""All external users"" are not allowed.'");
		КонецЕсли;
	Иначе
		Если Родитель = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			ВызватьИсключение
				НСтр("ru = 'Невозможно добавить подгруппу к предопределенной группе ""Все внешние пользователи"".'; en = 'You cannot add a subgroup to a fixed group of ""All external users"".'");
			
		ИначеЕсли Родитель.ВсеОбъектыАвторизации Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно добавить подгруппу к группе ""%1"", 
					|так как в число ее участников входят все пользователи.'; en = 'You cannot add a subgroup of the group ""%1"", because the number of participants includes all users.'"), Родитель);
		КонецЕсли;
		
		Если ВсеОбъектыАвторизации И ЗначениеЗаполнено(Родитель) Тогда
			ВызватьИсключение
				НСтр("ru = 'Невозможно переместить группу, в число участников которой входят все пользователи.'; en = 'It is impossible to move the group, which includes all users.'");
		КонецЕсли;
		...
		Если ВсеОбъектыАвторизации Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.СпискиЗначенийИдентичны(НазначениеВсеВнешниеПользователи, МассивНазначения) Тогда
				ВызватьИсключение
					НСтр("ru = 'Невозможно создать группу, совпадающую по назначению с предопределенной группой ""Все внешние пользователи"".'; en = 'Cannot create group matches on purpose with a predefined set of ""All external users"".'");
			КонецЕсли;
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Уже существует группа ""%1"",
					           |в число участников которой входят все пользователи указанных видов.'; en = 'There is already a group ""%1"", which includes all types of users.'"),
			...
			Для Каждого ТипПользователя Из ТипПользователей Цикл
				Если ТипПользователейРодителя.Найти(ТипПользователя) = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						|группы внешних пользователей ""%1"".'; en = 'View group members must be from the higher group of external users ""%1"".'"), Родитель);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Невозможно изменить вид участников группы ""%1"",
							   |так как у нее имеются подгруппы.'; en = 'You cannot change the appearance of the members of the group ""%1"" because it has subgroups.'"),
					Наименование);
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(Ссылка) Тогда
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Невозможно изменить вид участников группы ""%1"",
					           |так как у нее имеется подгруппа ""%2"" с другим назначением участников.'; en = 'You cannot change the appearance of the members of the group ""%1"" because it has a subgroup of ""%2"" with the other parties to the assignment.'"),
					Наименование,
					Выборка.СсылкаПредставление);
			КонецЕсли;
		КонецЕсли;
