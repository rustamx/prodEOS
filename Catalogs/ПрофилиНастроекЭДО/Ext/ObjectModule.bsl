...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
		...
		Если СертификатыПодписейОрганизации.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Список", "Заполнение", , , НСтр("ru = 'Сертификаты организации'; en = 'Company certificates'")),
				ЭтотОбъект,
...
Процедура ПередЗаписью(Отказ)
	...
	Если ПометкаУдаления Тогда
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			ТекстСообщения = НСтр("ru = 'Операция отменена. Текущий профиль настроек ЭДО используется в действующих настройках ЭДО.'; en = 'Operation cancelled. Current EDI settings profile is used in the valid EDI settings.'");
...
Функция ПрофильНастроекЭДОУникален() Экспорт
	...
	Если Не ТекущийПрофильНастроекУникален Тогда
		...
		|Способ обмена - %3;';
		|en = 'There is already a database with the details settings profile: 
		|Company - %1; 
		|Company ID - %2; 
