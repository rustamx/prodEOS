...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезОператораЭДОТакском") Тогда
		...
		Элементы.НадписьИдентификаторУчастникаОбменаЭД.Заголовок = НСтр("ru = 'Получить уникальный идентификатор участника обмена ЭД.'; en = 'Get the unique ID of the participant exchange EDI'");
		ШрифтЭлемента = Элементы.НадписьИдентификаторУчастникаОбменаЭД.Шрифт;
	...
	ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО") Тогда
		...
		Элементы.НадписьИдентификаторУчастникаОбменаЭДЧерезЭДО.Заголовок = НСтр("ru = 'Получить уникальный идентификатор участника обмена ЭД.'; en = 'Get the unique ID of the participant exchange EDI'");
		ШрифтЭлемента = Элементы.НадписьИдентификаторУчастникаОбменаЭДЧерезЭДО.Шрифт;
	...
	Иначе
		Если ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
			...
			ТекстВопроса = НСтр("ru = 'Была изменена организация. Изменить идентификатор обмена организации?'; en = 'Company was changed. Change the company interchange identifier?'");
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
		Иначе
...
Процедура ИспользоватьЭППриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СертификатКриптографии) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Данные по сертификату будут очищены. Продолжить?'; en = 'Certificate data will be cleared. Continue?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
Процедура НадписьИдентификаторУчастникаОбменаЭДНажатие(Элемент)
	...
	Если Не ЗначениеЗаполнено(СертификатКриптографии) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = 'Сертификат ключа электронной подписи'; en = 'Digital signature certificate'")),
			,
...
Процедура ПолучитьИдентификаторУчастникаОбменаЭДЧерез1СЭДО()
	...
	Если Не ЗначениеЗаполнено(КодНалоговогоОргана) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = 'Код налогового органа'; en = 'Tax authority code'")),
			,
			"КодНалоговогоОргана");
		ВозниклиОшибки = Истина;
	Иначе
		Если СтрДлина(КодНалоговогоОргана) <> 4 Тогда
			ТекстСообщения = ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "КОРРЕКТНОСТЬ", НСтр("ru = 'Код налогового органа'; en = 'Tax authority code'"), , ,
				НСтр("ru = 'Длина поля не равна 4.'; en = 'The field length is not equal to 4.'"));
	...
	Если Не ЗначениеЗаполнено(АдресОрганизации) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = 'Юридический адрес'; en = 'Legal address'")),
			,
			"АдресОрганизации");
		ВозниклиОшибки = Истина;
	ИначеЕсли Не ЗначениеЗаполнено(КодРегиона) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = '""КодРегиона"" в юридическом адресе'; en = '""КодРегиона"" in legal address'")),
			,
			"АдресОрганизации");
		ВозниклиОшибки = Истина;
	КонецЕсли;
...
Процедура КаталогОбмена(ПутьККаталогу)
	...
	ДиалогКаталога.Заголовок = НСтр("ru = 'Выберите сетевой каталог для обмена'; en = 'Select network directory to exchange'");
...
Процедура РеквизитыЮрЛицаПоИНН(Знач ИНН)
	...
	Если Прокси <> Неопределено Тогда
		...
		Исключение
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='ИНН %1:'; en = 'TIN %1:'"), ИНН)
...
Функция ТипыОбъектовАдресацииАдресаРФ()
	...
	Результат.Добавить(СтрокаОбъектаАдресации("2030", НСтр("ru = 'оф.'; en = 'off.'"),       3, 7));
	...
	Результат.Добавить(СтрокаОбъектаАдресации("10600000", НСтр("ru = 'Гаражно-строительный кооператив'; en = 'Garage and consruction cooperative'")));
...
Процедура ОбработатьОшибкуСервиса(ОписаниеОшибки, ИНН)
	...
	Если ОписаниеОшибки = "НеУказаныПараметрыАутентификации" Тогда
		...
		ОписаниеОшибки = НСтр("ru='Не указаны логин и пароль для доступа к интернет-поддержке'; en = 'Login and password for online support are not specified'");
		ТекстСобытия   = НСтр("ru='Ошибка доступа'; en = 'Access error'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, """status"":401") > 0 Тогда
		ТекстОшибки  = НСтр("ru='Неверно указаны логин и пароль для доступа к интернет-поддержке'; en = 'Login and password for online support are incorrect'");
		ТекстСобытия = НСтр("ru='Ошибка доступа'; en = 'Access error'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-1") > 0 Тогда
		ТекстОшибки  = НСтр("ru='Не указан ИНН юридического лица'; en = 'TIN of the legal entity is not specified'");
		ТекстСобытия = НСтр("ru='Ошибка получения данных'; en = 'Error retrieving data'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-2") > 0 Тогда
		ТекстОшибки  = НСтр("ru='ИНН юридического лица должен состоять из 10 цифр'; en = 'TIN of a legal entity must consist of 10 digits'");
		ТекстСобытия = НСтр("ru='Ошибка получения данных'; en = 'Error retrieving data'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-3") > 0 Тогда
		ТекстОшибки  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось найти данные для заполнения реквизитов по ИНН %1'; en = 'Could not find the data to fill attributes using TIN %1'"), ИНН);
		ТекстСобытия = НСтр("ru='Ошибка получения данных'; en = 'Error retrieving data'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-4") > 0 Тогда
		ТекстОшибки  = НСтр("ru='Не указан ИНН предпринимателя'; en = 'TIN of the entrepreneur is not specified'");
		ТекстСобытия = НСтр("ru='Ошибка получения данных'; en = 'Error retrieving data'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-5") > 0 Тогда
		ТекстОшибки  = НСтр("ru='ИНН предпринимателя должен состоять из 12 цифр'; en = 'TIN of an individual entrepreneur must consist of 12 digits'");
		ТекстСобытия = НСтр("ru='Ошибка получения данных'; en = 'Error retrieving data'", КодОсновногоЯзыка);
	...
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-7") > 0 Тогда
		ТекстОшибки  = НСтр("ru='Превышен лимит количества вызовов сервиса за один день'; en = 'Exceeded service calls limit for one day'");
		ТекстСобытия = НСтр("ru='Ошибка доступа'; en = 'Access error'", КодОсновногоЯзыка);
		
	ИначеЕсли СтрНайти(ОписаниеОшибки, "SERVER-8") > 0 Тогда
		ТекстОшибки  = НСтр("ru='Отсутствует действующий договор ИТС'; en = 'Effective ITS contract is absent'");
		ТекстСобытия = НСтр("ru='Ошибка доступа'; en = 'Access error'", КодОсновногоЯзыка);
		
	Иначе
		ТекстОшибки  = НСтр("ru='Ошибка при работе с сервисом (подробнее см. Журнал регистрации)'; en = 'Error while working with the service (see details in Events log)'");
		ТекстСобытия = НСтр("ru='Ошибка при работе с сервисом'; en = 'Service error'", КодОсновногоЯзыка);
	КонецЕсли;
	
	ИмяСобытия = НСтр("ru = 'Сервис данных единых гос_реестров.'; en = 'Service for data from united state registires.'", КодОсновногоЯзыка) + " " + ТекстСобытия;
...
Процедура ПодготовитьДанныеДляРегистрационногоПакета1СЭДОНачало()
	...
	Операция = НСтр("ru = 'Подписание регистрационных данных на подключение к сервису 1С-ЭДО'; en = 'Signing the registration data to connect to 1C-EDI service'");
	...
	Представление = НСтр("ru = 'Пользовательское соглашение на подключение к сервису 1С-ЭДО'; en = 'User agreement for connection to 1C-EDI service'");
	...
	Представление = НСтр("ru = 'Данные для регистрации учетной записи у оператора ЭДО'; en = 'Registration data at EDI operator'");
	...
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ru = 'Пользовательское соглашение, Данные для регистрации у оператора ЭДО'; en = 'User agreement details for registration with the EDI operator'"));
...
Процедура ОбработатьПредставлениеДанныхДляОператораЭДО(РезультатВыполнения, Контекст) Экспорт
	...
	ШаблонИмениВременногоФайла = НСтр("ru = 'Данные для регистрации у оператора ЭДО_%1'; en = 'Registration data at EDI operator_%1'");
...
Процедура ПодготовитьДанныеДляРегистрационногоПакета1СЭДОПродолжение2(РезультатВыполнения, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(ОператорЭДОСертификат) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Ошибка получения сертификата оператора ЭДО.
									...
									|en = 'Error obtaining the EDI operator certificate.
									|You should obtain the identifier of EDI participant again.'");
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",            НСтр("ru = 'Шифрование данных для регистрации у оператора ЭДО'; en = 'Data encryption for registration at EDI operator'"));
	...
	Представление.Вставить("Представление", НСтр("ru = 'Данные для регистрации у оператора ЭДО'; en = 'Registration data at EDI operator'"));
...
Процедура СформироватьИОтправитьРегистрационныйПакет1СЭДО(РеквизитыПакета, ЗашифрованныеДанныеОператораЭДО, ПодписанныеДвоичныеДанныеДляОператораЭДО,
	...
	ТекстЗаголовкаСообщения = НСтр("ru = 'Отправка заявления о регистрации на сервисе 1С-ЭДО'; en = 'Sending request for registration at the 1C-EDI service'");
...
Процедура ТестПрофиляНастроекЭДО()
	...
	Если Не ЗначениеЗаполнено(ИдентификаторОрганизации)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = 'Идентификатор организации'; en = 'Company identifier'")),
			,
			"ИдентификаторОрганизации",
			,
			Отказ);
	...
	Если Не ЗначениеЗаполнено(СпособОбменаЭД) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = 'Способ Обмена ЭД'; en = 'ED interchange method'")),
			,
			"СпособОбменаЭД",
			,
			Отказ);
	...
	Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
		Если Не ЗначениеЗаполнено(ЭлектроннаяПочтаОрганизации) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = 'Электронная почта организации'; en = 'Company email'")),
			,
	...
	ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезКаталог") Тогда
		Если Не ЗначениеЗаполнено(КаталогВходящихДокументов) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = 'Каталог входящих документов'; en = 'Incoming documents directory'")),
			,
	...
	ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезFTP") Тогда
		Если Не ЗначениеЗаполнено(АдресСервераFTP) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = 'Адрес сервера'; en = 'Server address'")),
			,
			"АдресСервераFTP",
			,
			Отказ);
	...
	Если Не ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
		...
		Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО") Тогда
			...
			ИначеЕсли ТипРегистрации = 2 И Не ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru = 'Необходимо получить уникальный идентификатор участника обмена ЭД.'; en = 'You must get a unique ID participant of exchange EDI.'"), , , , Отказ);
			КонецЕсли;
		ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезОператораЭДОТакском") Тогда
			...
			Иначе
				...
				Если ДлинаИдентификатора <> 46 Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "КОРРЕКТНОСТЬ", НСтр("ru = 'Идентификатор организации'; en = 'Company identifier'"), , ,
						НСтр("ru = 'Длина поля не равна 46.'; en = 'The field length is not equal to 46.'")),
						,
						"ИдентификаторОрганизации",
						,
						Отказ);
	...
	Если Не ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
		...
		Состояние(НСтр("ru = 'Создание профиля настроек ЭДО.'; en = 'Create a EDI profile settings.'"),
			,
			НСтр("ru = 'Выполняется тестирование связи с оператором. Подождите...'; en = 'Testing connection with operator. please Wait...'"));
	...
	Иначе
		...
		Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
			...
			Состояние(НСтр("ru = 'Создание профиля настроек ЭДО.'; en = 'Create a EDI profile settings.'"),
				,
				НСтр("ru = 'Выполняется тестирование обмена ЭД через электронную почту. Подождите...'; en = 'Testing ED exchange via email. Please wait...'"));
		...
		Иначе
			Если СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезКаталог") Тогда
				...
				Состояние(НСтр("ru = 'Создание профиля настроек ЭДО.'; en = 'Create a EDI profile settings.'"),
					,
					НСтр("ru = 'Выполняется тестирование обмена ЭД через каталог. Подождите...'; en = 'Testing ED exchange via directory. Please wait...'"));
			...
			ИначеЕсли СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезFTP") Тогда
				...
				Состояние(НСтр("ru = 'Создание профиля настроек ЭДО.'; en = 'Create a EDI profile settings.'"),
					,
					НСтр("ru = 'Выполняется тестирование обмена ЭД через FTP. Подождите...'; en = 'Testing ED exchange via FTP. Please wait...'"));
...
Процедура ПроверитьУчетнуюЗаписьЭлектроннойПочтыИСоздатьНовыйПрофиль(ЭлектроннаяПочтаОрганизации)
	...
	Если ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Проверка учетной записи эл. почты'; en = 'Testing email account'"),,
...
Процедура СохранитьПараметры(ДанныеСохранены)
	...
	Попытка
		...
		ШаблонНаименование = НСтр("ru = '%1, %2'; en = '%1, %2'");
...
Процедура ТестСвязиСервисаЭДОЗавершение()
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",            НСтр("ru = 'Тест аутентификации в сервисе ЭДО'; en = 'Test of authentication in the EDI service'"));
...
Функция НастройкиОповещения(РасшифрованныйМаркер, Идентификатор)
	...
	ВидОперации = НСтр("ru = 'Получение информации о свойствах подписки ЭДО'; en = 'Obtaining information about the properties of EDI subscription'");
