...
Функция ЕстьВключенныеСобытия()
	...
	Если Не Результат Тогда
		...
		ТекстСообщения = НСтр("ru = 'Нет событий для уведомления'; en = 'No events for notification'");
...
Функция ТекстКодаОшибки(КодСостояния)
	...
	Если КодСостояния = 405 Тогда
		ТекстОшибки = НСтр("ru = 'Указанный клиентом метод нельзя применить к текущему ресурсу'; en = 'The method specified by the client cannot be applied to the resource'");
	КонецЕсли;
...
Функция СохранитьПараметрыПодписок(РасшифрованныйМаркер, ПараметрыУведомления)
	...
	Исключение
		...
		ВидОперации = НСтр("ru = 'Изменение свойств подписки ЭДО'; en = 'Modify EDI subscription properties'");
	...
	Иначе
		...
		ШаблонСообщения = НСтр("ru = 'Ошибка при сохранении настройки: %1'; en = 'Error while saving settings: %1'");
		ТекстОшибки = ТекстКодаОшибки(Ответ.КодСостояния);
		
		ШаблонСообщения = НСтр("ru = 'Ошибка выполнения команды сервиса. Код ошибки: %1. %2'; en = 'Error executing command of the service. Error code: %1. %2'");
