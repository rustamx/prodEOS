...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ Объект.ПометкаУдаления И Не ПропускатьПроверкуНастроекЭДО И ОтсутствуютПодчиненныеНастройкиЭДО() Тогда
		ТекстВопроса = НСтр("ru = 'Подключить контрагента к обмену электронными документами?'; en = 'Connect the counterparty to electronic document interchange?'");
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
			...
			Если ТипВыбранногоЗначения = Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования") Тогда
				...
				Если МассивСтрок.Количество() > 0 Тогда
					ТекстПредупреждения = НСтр("ru = 'Выбранный сертификат уже зарегистрирован в профиле настроек ЭДО.'; en = 'Selected certificate is already registered in the agreement'");
...
Процедура НадписьЛичныйКабинетУчастникаОбменаЭДНажатие(Элемент)
	...
	Если Объект.СертификатыПодписейОрганизации.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'Для входа в личный кабинет должен быть зарегистрирован хотя бы один сертификат'; en = 'To entry in personal cabinet must be registered at least one certificate'");
		ПоказатьПредупреждение(, ТекстПредупреждения, 30);
		Возврат;
	КонецЕсли;
	...
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ru = 'Выполнить действие можно только в записанном профиле настроек ЭДО.
			|Записать?';
			|en = 'You can perform the action only in a saved EDI settings profile.
			|Save?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	Иначе
...
Процедура ТекстПриглашенийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ru = 'Шаблон текста для приглашений контрагентов'; en = 'Text template for invitations to counterparties'");
...
Процедура ИдентификаторОрганизацииОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Не КонтролироватьСодержимоеНастроекЭД Тогда
		ТекстВопроса = НСтр("ru = 'Изменения ""Профиля настроек ЭДО"" будут применены для связанных ""Настроек ЭДО"" с отключенным расширенным режимом.
								|Продолжить?';
								|en = 'Changes to ""EDI settings profile"" will be applied to the linked ""EDI settings"" where advanced mode was not enabled.
								|Continue?'");
...
Процедура ТестПрофиляНастроек(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru = 'В текущий профиль настроек ЭДО внесены изменения. Продолжить выполнение теста?'; en = 'Current EDI settings profile was changed. Proceed with the test?'");
		СписокКнопок = Новый СписокЗначений();
		СписокКнопок.Добавить("Выполнить", НСтр("ru = 'Сохранить и выполнить тест'; en = 'Save and run test.'"));
		СписокКнопок.Добавить("Отменить", НСтр("ru = 'Отменить тест'; en = 'Cancel test'"));
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок, , "Выполнить", НСтр("ru = 'Тест настроек'; en = 'Test settings'"));
	Иначе
...
Процедура ДобавитьСертификат(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Операция добавления сертификата в профиль настроек ЭДО займет несколько минут.
							|Продолжить?';
							|en = 'Adding the certificate to EDI settings profile will take several minutes.
							|Proceed?'");
...
Процедура НастройкаУведомлений(Команда)
	...
	ПредставлениеОперации = НСтр("ru = 'Настройка уведомлений о событиях ЭДО'; en = 'Configuring EDI notifications'");
...
Процедура ИсходящиеДокументыПриИзменении(Элемент)
	...
	Если Элемент.ТекущиеДанные.ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД")
		...
		Если Элемент.ТекущийЭлемент.Имя = "ИсходящиеДокументыИспользоватьЭП" И НЕ Элемент.ТекущиеДанные.ИспользоватьЭП Тогда
			ШаблонСообщения = НСтр("ru='Отправка документа %1 возможна только с использованием электронной подписи.'; en = 'Sending document %1 is only possible through the operator of EDI.'");
	...
	КонецЕсли;
		...
		ШаблонСообщения = НСтр("ru='Данные документ %1 передаются в составе счет-фактуры(универсального передаточного документа).'; en = 'The %1 doucment data are transferred in a correcting VAT invoice (universal transfer document).'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
			Элемент.ТекущиеДанные.ИсходящийДокумент);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	...
	Если Элемент.ТекущиеДанные.ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель")
		...
		ШаблонСообщения = НСтр("ru='Данные документ %1 передаются в составе корректировочного счет-фактуры(универсального корректировочного документа).'; en = 'The %1 doucment data are transferred in a correcting VAT invoice (universal transfer document).'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
			Элемент.ТекущиеДанные.ИсходящийДокумент);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ТестПрофиляНастроекПродолжить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
		Если Объект.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
			Состояние(НСтр("ru = 'Тест настроек.'; en = 'Test settings.'"),
			,
		...
		Если Объект.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезКаталог") Тогда
			...
			|%1';
			|en = 'Test. The access check to the shared directory for ED interchange.
			|%1'");
			Попытка
		...
		Если Объект.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезFTP") Тогда
			...
			|%1';
			|en = 'Test. The access check to FTP server for ED interchange.
			|%1'");
			Попытка
	...
	Иначе
		...
		Состояние(НСтр("ru = 'Тест настроек.'; en = 'Test settings.'"),
			,
			НСтр("ru = 'Выполняется тестирование связи с сервисом ЭДО. Подождите...'; en = 'Testing connection with EDI service. Please wait...'"));
...
Процедура ПослеПолученияОтпечатковПроверитьСертификаты(Отпечатки, Параметры = Неопределено) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ru = 'Нет доступных сертификатов. Тест не выполнен.'; en = 'There are no available certificates. Test failed.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ПослеПолученияОтпечатковПроверитьСвязьЭДО(Отпечатки, Параметры = Неопределено) Экспорт
	...
	ШаблонОперация = НСтр("ru = 'Тест аутентификации: %1'; en = 'Authentication test: %1'");
...
Процедура ДействияПослеТестаСвязиЭДО(РезультатВыполнения, Контекст = Неопределено) Экспорт
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		Если РезультатВыполнения.Успех = Истина Тогда
			ТекстСообщения = НСтр("ru = 'Тест профиля настроек выполнен успешно.'; en = 'Test of profile settings completed successfully.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПерейтиВЛичныйКабинетОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстСообщения = НСтр("ru='Нет доступных сертификатов, среди зарегистрированных по данному профилю настроек ЭДО.'; en = 'No available certificate is found among the ones registered using this EDI settings profile.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,
			,
...
Процедура ПодготовитьДанныеДляРегистрационногоПакета1СЭДОПродолжение2(РезультатВыполнения, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(ОператорЭДОСертификат) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Ошибка получения сертификата оператора ЭДО.
									...
									|You should add the certificate again.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура СформироватьИОтправитьРегистрационныйПакет1СЭДО(РеквизитыПакета, ЗашифрованныеДанныеОператораЭДО, ПодписанныеДвоичныеДанныеДляОператораЭДО,
	...
	ТекстЗаголовкаСообщения = НСтр("ru = 'Отправка заявления на изменение данных на сервисе 1С-ЭДО'; en = 'Sending request for changing information at the 1C-EDI service'");
...
Процедура ОткрытьФормуВыбораСертификатаЭП()
	...
	Если Объект.ПометкаУдаления Тогда
		ТекстСообщения = НСтр("ru='Для выполнения действия необходимо снять пометку удаления.'; en = 'To perform the action deletion mark must be removed.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
	...
	Если ОбменСКонтрагентамиСлужебныйКлиент.ИспользуетсяИнтернетПоддержкаПользователей() Тогда
		...
		Если ПустаяСтрока(Объект.ИдентификаторОрганизации)
			...
			ТекстПредупреждения = НСтр("ru = 'Перед добавлением новых сертификатов в профиль настроек ЭДО
											|необходимо получить идентификатор участника обмена ЭД'; en = 'Before adding new certificates in the agreement it is necessary to get the ID of the participant exchange of EDI'");
			ПоказатьПредупреждение(, ТекстПредупреждения, 30);
			Возврат;
...
Процедура УстановитьНастройкуУведомлений()
	...
	Если УведомлятьОСобытиях Тогда
		ЗаголовокШаблон = НСтр("ru ='Email для уведомлений: %1'; en = 'Email for notifications: %1'");
	Иначе
		ЗаголовокШаблон = НСтр("ru ='Уведомления не подключены'; en = 'Notifications are not connected'");
	КонецЕсли;
	...
	ИначеЕсли УведомлятьОСобытиях Тогда
		АдресУведомлений = НСтр("ru = '<не указан>'; en = '<not specified>'");
	КонецЕсли;
...
Процедура ЗаполнитьПредставлениеРегламентаЭДО(ТекСтрока)
		...
		Если ТекСтрока.ИспользоватьЭП Тогда
			Подпись = НСтр("ru = 'Подпись'; en = 'Signature'");
		Иначе
			Подпись = НСтр("ru = 'Без подписи'; en = 'Without signature'");
		КонецЕсли;
		
		ОтветнаяПодпись = "";
		Если ТекСтрока.ТребуетсяОтветнаяПодпись Тогда
			ОтветнаяПодпись = НСтр("ru = 'ответная подпись'; en = 'response signature'");
		КонецЕсли;
		
		ИзвещениеОПолучении = "";
		Если ТекСтрока.ТребуетсяИзвещениеОПолучении Тогда
			ИзвещениеОПолучении = НСтр("ru = 'извещение о получении'; en = 'notification on receipt'");
		КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru = 'ФНС 5.01 (с 2016г.)'; en = 'FTS 5.01 (since 2016г.)'");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'ФНС 5.01 (с 01.01.2016)'; en = 'FTS 5.01 (since 01.01.2016)'"));
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ИсходящиеДокументы.ВерсияФормата");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru = 'ФНС 5.01'; en = 'FTS 5.01'");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'ФНС 5.01 (до 31.12.2016)'; en = 'FTS 5.01 (before 31.12.2016)'"));
