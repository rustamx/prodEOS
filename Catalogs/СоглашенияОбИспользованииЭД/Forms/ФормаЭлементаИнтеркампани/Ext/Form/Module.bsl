	...
	Иначе
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				НСтр("ru = 'открытие формы соглашения'; en = 'opening agreement form'"), ТекстОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЕсли;
...
Процедура ПолучательОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ИдентификаторКонтрагента) Тогда
		Если ВыбранноеЗначение <> Объект.Контрагент Тогда
			ТекстВопроса = НСтр("ru = 'Была изменена организация получатель. Изменить идентификатор получателя?'; en = 'Recipient company was changed. Change the identifier of the recipient?'");
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
		КонецЕсли;
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, КодВозвратаДиалога.Да);
...
Процедура ПеречитатьДанныеПоСертификатам(ДокументОбъект)
	...
	Для Каждого ЭлементСтрока Из ДокументОбъект.СертификатыПодписейКонтрагента Цикл
		...
		Если ДвоичныеДанныеСертификата <> Неопределено Тогда
			...
			Исключение
				Операция = НСтр("ru = 'Чтение данных сертификата'; en = 'Reading certificate data'");
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
		...
		Если ТекущийДокументСоглашения.Формировать Тогда
			Для Каждого ДокументВДругихСоглашениях Из ВидыДокументовДругихСоглашений Цикл
				Если ТекущийДокументСоглашения[?(ПроверятьВходящиеДокументы, "ВходящийДокумент", "ИсходящийДокумент")] = ДокументВДругихСоглашениях.ТипДокумента Тогда
					...
					|%5.
					|';
					|en = 'For the type of electronic document %1 %2 
					|already exists an agreement between parties %3 - %4:
					|%5.'");
...
Процедура ПроверитьЗаписатьСоглашение(ОбработчикПродолжения)
	...
	Если НЕ Объект.УдалитьЭтоТиповое Тогда
		...
		|Записать соглашение?';
		|en = 'External certificates are available only in saved agreements.
		|Save the agreement?'");
...
Процедура ИзменитьНадписьНаправлениеДокументов()
	...
	ШаблонСообщения = НСтр("ru='От кого: %1, кому: %2'; en = 'From: %1, to: %2'");
...
Процедура ОбновитьЗаголовокФормы()
	...
	ТекстЗаголовка = НСтр("ru='Настройка ЭДО между организациями (создание)'; en = 'Setting up EDI between companies (create)'");
