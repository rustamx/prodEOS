	...
	Если НЕ ПометкаУдаления Тогда
		...
		Если Результат.Следующий() Тогда
			ШаблонСообщения = НСтр("ru = 'Операция отменена.
				...
				|Current EDI settinfs employ EDI profile that is marked for deletion:
				|%1'");
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			ШаблонСообщения = НСтр("ru = 'В информационной базе уже существует настройка ЭДО со статусом подключения ""Присоединен""
										|между контрагентом %1 и организацией %2'; en = 'There is already an EDI setting in ""Connected"" state for interchange between counterparty %1 and company %2'");
			...
			Если ЗначениеЗаполнено(Выборка.ДоговорКонтрагента) Тогда
				Шаблон = НСтр("ru = '%1 по договору %2'; en = '%1 on a treaty %2'");
