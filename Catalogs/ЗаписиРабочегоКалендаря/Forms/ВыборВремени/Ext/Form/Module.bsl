...
Функция ПолучитьРекомендациюСтрокой(РекомендацияДатаНачала, РекомендацияДатаОкончания,
	...
	Если ДоступныВсеПользователи Тогда
		ДоступныеПользователь = НСтр("ru = 'Свободны все участники'; en = 'All attendees are available'");
	Иначе
		ДоступныеПользователь = НСтр("ru = 'Свободны только обязательные участники'; en = 'Only mandatory attendees are available'");
	КонецЕсли;
	ДобавитьЗначениеКСтрокеЧерезРазделитель(
...
Процедура ОтобразитьСводку()
	...
	Если ВремяНачалаОтображения = Дата(1,1,1) И ВремяОкончанияОтображения = Дата(1,1,2) Тогда
		СводкаВозможноеВремя = НСтр("ru = 'во всем дне'; en = 'throughout the day'");
	Иначе
	...
	ПредставлениеУчастников = НСтр("ru = 'участника'; en = 'attendee'") + "," + НСтр("ru = 'участников'; en = 'attendees'") + "," + НСтр("ru = 'участников'; en = 'attendees'");
	...
	Сводка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ищем %1 %2 для %3.'; en = 'Looking %1 %2 for %3.'"),
		ДлительностьСтр,
...
Процедура ОтобразитьПояснениеРедактированиеКалендаря()
	...
	ТекстПояснения =
		НСтр("ru = 'Выполняется обновление календаря...
			...
			|en = 'The calendar is being refreshed...
			|Please wait.'");
...
Функция ДобавитьУчастниковПодбораВМероприятие()
	...
	ТекстВопроса =НСтр("ru = 'При подборе времени мероприятия были добавлены новые участники.
		...
		|en = 'New attendees have been added during time selection.
		|Add new attendees to the event?'");
	
	Если ЕстьНеучтенныеУчастники Тогда
		Если ЕстьДобавленныеУчастники Тогда
			ДополнительныйТекстВопроса = НСтр("ru = 'Некоторые добавленные участники не отображаются в подборе времени.'; en = 'Some added attendees do not apper in find time table.'");
		Иначе
			ДополнительныйТекстВопроса = НСтр("ru = 'Все добавленные участники не отображаются в подборе времени.'; en = 'All added attendees do not appear in find time table.'");
		КонецЕсли;
		ТекстВопроса = ТекстВопроса + Символы.ПС + Символы.ПС + ДополнительныйТекстВопроса;
	КонецЕсли;
	
	Если ЕстьНеучтенныеУчастники Тогда
		ТекстВариантаВключитьВсех = НСтр("ru = 'Включить всех'; en = 'Include all'");
	Иначе
		ТекстВариантаВключитьВсех = НСтр("ru = 'Включить'; en = 'Include'");
	КонецЕсли;
	ТекстВариантаВключитьОтображаемых = НСтр("ru = 'Включить отображаемых'; en = 'Include displayed'");
	ТекстВариантаНеВключать = НСтр("ru = 'Не включать'; en = 'Do not include'");
