...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Не РазделениеВключено) Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для открытия списка пользователей информационной базы.'; en = 'No permission to open the list of Infobase users.'");
	КонецЕсли;
...
Процедура ОтменитьСопоставление(Команда)
	...
	Кнопки.Добавить("ОтменитьСопоставление", НСтр("ru = 'Отменить сопоставление'; en = 'Clear mapping'"));
	Кнопки.Добавить("ОставитьСопоставление", НСтр("ru = 'Оставить сопоставление'; en = 'Keep mapping'"));
	...
	ПоказатьВопрос(
		...
		НСтр("ru = 'Отмена сопоставления пользователя информационной базы с пользователем в справочнике.
		           ...
		           |en = 'Unmapping a infobase user with a user in the catalog. 
		           |
		           |Unmapping require very rarely, only if the mapping was performed incorrectly, for example, when updating information, so it is not recommended to cancel a comparison for any other reason.'"),
		Кнопки,
		,
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<Нет данных>'; en = '<No data>'"));
...
Процедура УдалитьТекущегоПользователяИБ(УдалитьСтроку = Ложь)
	...
	ПоказатьВопрос(
		...
		НСтр("ru = 'Удалить пользователя информационной базы?'; en = 'Do you want to delete the infobase user?'"),
