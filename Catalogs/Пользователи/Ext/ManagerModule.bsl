...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		...
		Если РольДоступна("ПолныеПрава") 
			...
			Или РольДоступна("ра_ДобавлениеИзменениеПользователей") 
			// ТСК Близнюк С.И.; 27.12.2018; task#1648}
			// ТСК Близнюк С.И.; 27.12.2018; task#2249{
			Или РольДоступна("ра_ДобавлениеИзменениеПользователейПоОрганизации") Тогда
			// ТСК Близнюк С.И.; 27.12.2018; task#2249}
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	ДопУсловие = "";
	Если Параметры.Отбор.Свойство("ра_РуководительКонтролераКО") И Параметры.Отбор.ра_РуководительКонтролераКО Тогда
		ДопУсловие = " ИЛИ ра_РуководительКонтролераКО ";
	ИначеЕсли Параметры.Отбор.Свойство("ра_КонтролерКО") И Параметры.Отбор.ра_КонтролерКО Тогда
		ДопУсловие = " ИЛИ ра_КонтролерКО ";
	КонецЕсли;
	...
	Если Параметры.Отбор.Свойство("Nesootvetstvie") Тогда
		...
		ТекстЗапроса = СтрШаблон(ТекстЗапроса, ТекстЗапросаСоединение, ТекстЗапросаУсловие, ДопУсловие);
	Иначе
		ТекстЗапроса = СтрШаблон(ТекстЗапроса, "", "", ДопУсловие);
	КонецЕсли;
