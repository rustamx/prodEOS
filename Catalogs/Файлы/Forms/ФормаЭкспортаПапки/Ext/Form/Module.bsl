...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'В Веб-клиенте экспорт каталогов не поддерживается.'; en = 'Folder export is not supported in Web client.'"));
		Отказ = Истина;
		Возврат;
	#КонецЕсли
	...
	Если Не ПустаяСтрока(ПапкаДляЭкспорта) Тогда
		...
		Если Не Файл.Существует() Тогда
			ТекстПредупреждения = НСтр("ru = 'Невозможно открыть папку выгрузки.
			                                 ...
			                                 |en = 'Cannot open the export directory.
			                                 |The directory may have not yet created.'");
...
Процедура СохранитьПапку()
	...
	Если Не КаталогВыгрузки.Существует() Тогда
		...
		Исключение
			ТекстОшибки = НСтр("ru = 'Не удалось создать папку выгрузки:'; en = 'Failed to create directory for export:'");
	...
	Если Результат.Успех = Истина Тогда
		...
		Состояние(СтрШаблон(
		             НСтр("ru = 'Успешно завершен экспорт папки ""%1""
		                        |в каталог на диске ""%2"".'; en = 'Successfully completed exporting folder ""%1"" to directory ""%2"".'"),
...
Процедура ОбойтиДеревоФайлов4(ПараметрыВыполнения)
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка создания папки ""%1"":'; en = 'Error creating directory ""%1"":'");
	...
	Если ПараметрыВыполнения.ФайлНаДиске.Существует() И ПараметрыВыполнения.ФайлНаДиске.ЭтоКаталог() Тогда
		ТекстВопроса = СтрШаблон(
			НСтр("ru = 'Вместо файла
			           ...
			           |en = 'Instead of file ""%1"" there is a directory with the same name.
			           |
			           |Repeat the export of that file?'"),
		...
		Иначе
			Если Не ПараметрыВыполнения.ОбщиеПараметры.ДляВсехФайлов Тогда
				...
				ТекстНачало = 	
				 СтрШаблон(
					НСтр("ru = 'В папке ""%1""
					           |существует файл '; en = 'There is a file in the directory ""%1"" '"),
					ПараметрыВыполнения.БазовыйКаталогСохраненияФайла);
				...
				ТекстОкончание = СтрШаблон(
					НСтр("ru = '
					           ...
					           |en = 'the size of the existing file =%1 bytes, modification date %2.
					           |the saved file size = %3 bytes date modified %4.
					           |
...
Процедура ОбойтиДеревоФайлов14(ПараметрыВыполнения)
	...
	СостояниеЗаголовок = СтрШаблон(
		НСтр("ru = 'Экспорт папки ""%1""'; en = 'Export folder ""%1""'"),
		ПараметрыВыполнения.ЗаписьФайла.НаименованиеПапки);
	СостояниеТекст = СтрШаблон(
		НСтр("ru = 'Сохраняется на диск файл
		           |""%1"" (%2 Мб)...'; en = 'Saving to disk file ""%1"" (%2 MB) ...'"),
