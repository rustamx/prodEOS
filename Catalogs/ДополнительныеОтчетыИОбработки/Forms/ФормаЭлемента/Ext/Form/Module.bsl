...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДобавления Тогда
		Если ЭтоНовый Тогда
			ВызватьИсключение НСтр("ru = 'Недостаточно прав доступа для добавления дополнительных отчетов или обработок.'; en = 'Insufficient access rights for adding additional reports or data processors. .'");
		Иначе
	...
	Если ПоказатьДиалогЗагрузкиИзФайлаПриОткрытии И Не Элементы.ЗагрузитьИзФайла.Видимость Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для загрузки дополнительных отчетов и обработок'; en = 'Insufficient rights to import additional report or data processor'");
	КонецЕсли;
...
Процедура ВариантыДополнительногоОтчетаПередУдалением(Элемент, Отказ)
	...
	Если НЕ Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Пометка на удаление предопределенного варианта отчета запрещена.'; en = 'Cannot mark a predefined report option for deletion.'"));
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Пометить ""%1"" на удаление?'; en = 'Mark ""%1"" for deletion?'"), Вариант.Наименование);
...
Процедура ВариантыДополнительногоОтчетаОткрыть(Команда)
	...
	Если Вариант = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите вариант отчета.'; en = 'Select report option.'"));
		Возврат;
	КонецЕсли;
...
Процедура НастроитьВидимость(Команда)
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru = 'Для настройки видимости команд печати обработку необходимо записать. Продолжить?'; en = 'To configure visibility print commands you want to save. Do you want to continue?'");
...
Процедура ПродолжитьЗаписьНаКлиенте(Результат, ЗакрытьПослеЗаписи)  Экспорт
	...
	Если ПараметрыЗаписи.РегистрацияОбработки Тогда
		ОбновитьПовторноИспользуемыеЗначения();
		ТекстОповещения = НСтр("ru = 'Для применения изменений в открытых окнах необходимо их закрыть и открыть заново.'; en = 'To apply the changes to the open windows, close and reopen them.'");
...
Процедура ОбновлениеИзФайлаЗапуск()
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПараметрыДиалога.Заголовок = НСтр("ru = 'Выберите файл внешнего отчета или обработки'; en = 'Select external report or data processor file'");
	ИначеЕсли Объект.Вид = ВидДополнительныйОтчет Или Объект.Вид = ВидОтчет Тогда
		ПараметрыДиалога.ИндексФильтра = 1;
		ПараметрыДиалога.Заголовок = НСтр("ru = 'Выберите файл внешнего отчета'; en = 'Select external report file'");
	Иначе
		ПараметрыДиалога.ИндексФильтра = 2;
		ПараметрыДиалога.Заголовок = НСтр("ru = 'Выберите файл внешней обработки'; en = 'Select external data processor file'");
	КонецЕсли;
...
Процедура ОбновлениеИзФайлаПослеВыбораФайла(ПомещенныеФайлы, ПараметрыРегистрации) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = НСтр("ru = 'Расширение файла не соответствует расширению внешнего отчета (ERF) или обработки (EPF).'; en = 'The file extension does not match external report extension (ERF) or external data processor extension (EPF).'");
...
Процедура ОбновлениеИзФайлаМеханикаНаКлиенте(ПараметрыРегистрации)
	...
	Если ПараметрыРегистрации.Успех Тогда
		ОповещениеЗаголовок = ?(ПараметрыРегистрации.ЭтоОтчет, НСтр("ru = 'Файл внешнего отчета загружен'; en = 'External report file is imported'"), НСтр("ru = 'Файл внешней обработки загружен'; en = 'External data processor file is imported'"));
	...
	Иначе
		...
		Иначе
			...
			Если ПараметрыРегистрации.ЭтоОтчет Тогда
				ЗаголовокОшибки = НСтр("ru = 'Отчет не подключен'; en = 'Report is not connected'");
			Иначе
				ЗаголовокОшибки = НСтр("ru = 'Обработка не подключена'; en = 'Data processor is not connected'");
			КонецЕсли;
...
Процедура ОбновлениеИзФайлаПоказатьКонфликты(ПараметрыРегистрации)
	Если ПараметрыРегистрации.КоличествоКонфликтующих > 1 Тогда
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("ru = 'Конфликты при загрузке внешнего отчета'; en = 'External report import conflict'");
			...
			|en = 'Internal report name ""[Name]"" is already occupied with the existing additional reports ([Count]): [List]. 
			|
			|Select: 
			|1. ""[Continue]"" - download the new report in debug mode. 
			|2. ""[Disable]"" - download the new report by disabling publishing of all the conflicting reports. 
			|3. ""[Open]"" - cancel downloading and show the list of conflicting reports.'");
		Иначе
			ЗаголовокВопроса = НСтр("ru = 'Конфликты при загрузке внешней обработки'; en = 'External data processor import conflict'");
			...
			|en = 'Internal data processor name ""[Name]"" is already occupied with the existing data processors ([Count]):[List]. 
			|
			|Select: 
			|1. ""[Continue]"" - download new data processor in debug mode. 
			|2. ""[Disable]"" - download new data processor by disabling publishing of all the conflicting data processors. 
			|3. ""[Open]"" - cancel downloading and show the list of conflicting data processors.'");
		КонецЕсли;
		ПредставлениеКнопкиОтключить = НСтр("ru = 'Отключить конфликтующие'; en = 'Disable conflicting'");
		ПредставлениеКнопкиОткрыть = НСтр("ru = 'Отменить и показать список'; en = 'Cancel and show list'");
	Иначе
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("ru = 'Конфликт при загрузке внешнего отчета'; en = 'External report import conflict'");
			ТекстВопроса = НСтр("ru = 'Внутреннее имя отчета ""[Name]""
			...
			|en = 'Internal report name ""[Name]"" is already occupied with the existing additional report [List]. 
			|
			|Select: 
			|1. ""[Continue]"" - download the new report in debug mode. 
			|2. ""[Disable]"" - download the new report by disabling publishing of the conflicting report.
			|3. ""[Open]"" - open the form of conflicting report.'");
			ПредставлениеКнопкиОтключить = НСтр("ru = 'Отключить другой отчет'; en = 'Disable another report'");
		Иначе
			ЗаголовокВопроса = НСтр("ru = 'Конфликт при загрузке внешней обработки'; en = 'External data processor import conflict'");
			ТекстВопроса = НСтр("ru = 'Внутреннее имя обработки ""[Name]""
			...
			|en = 'Internal data processor name ""[Name]"" is already occupied with the existing data processor [List]. 
			|
			|Select: 
			|1. ""[Continue]"" - download new data processor in debug mode. 
			|2. ""[Disable]"" - download new data processor by disabling publishing of the conflicting data processor. 
			|3. ""[Open]"" - open the form of conflicting data processor.'");
			ПредставлениеКнопкиОтключить = НСтр("ru = 'Отключить другую обработку'; en = 'Disable another data processor'");
		КонецЕсли;
		ПредставлениеКнопкиОткрыть = НСтр("ru = 'Отменить и открыть'; en = 'Cancel and open'");
	КонецЕсли;
	ПредставлениеКнопкиПродолжить = НСтр("ru = 'В режиме отладки'; en = 'In debug mode'");
...
Процедура ОбновлениеИзФайлаРазрешениеКонфликтов(Ответ, ПараметрыРегистрации) Экспорт
	...
	ИначеЕсли Ответ = "ОтменитьИОткрыть" Тогда
		...
		Если ПоказатьСписок Тогда // Форма списка с отбором по конфликтующим.
			...
			ФормаЗаголовок = НСтр("ru = 'Дополнительные отчеты и обработки с внутреннем именем ""%1""'; en = 'Additional reports and data processors with the internal name ""%1""'");
...
Процедура ОткрытьВариант()
	...
	Если НЕ ЗначениеЗаполнено(Вариант.Ссылка) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вариант отчета ""%1"" не зарегистрирован.'; en = 'Report option ""%1"" is not registered.'"), Вариант.Наименование);
...
Процедура ИзменитьРегламентноеЗадание(РежимВыбора = Ложь, ИзмененФлажок = Ложь)
	...
	Если ЭлементКоманда.ВариантЗапуска <> ПредопределенноеЗначение("Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода")
		...
		|не может использоваться в регламентных заданиях.'; en = 'Command with running option ""%1"" cannot be used in the scheduled jobs.'");
...
Функция ПодготовитьПараметрыФормыВыборОбъектовМетаданных()
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Назначение дополнительной обработки'; en = 'Additional data processor purpose'"));
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если Объект.Вид = ВидДополнительныйОтчет И Объект.ИспользуетХранилищеВариантов Тогда
		...
		Если Регистрация ИЛИ КоличествоВариантов = 0 Тогда
			...
			Элементы.СтраницаВарианты.Заголовок = НСтр("ru = 'Варианты отчета'; en = 'Report options'");
		Иначе
			...
			Элементы.СтраницаВарианты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Варианты отчета (%1)'; en = 'Report options (%1)'"),
	...
	Иначе
		...
		Элементы.СтраницаРазрешения.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Разрешения (%1)'; en = 'Permissions (%1)'"),
		...
		ИначеЕсли ТипЗнч(БезопасныйРежим) = Тип("Строка") Тогда
			...
			Элементы.ДекорацияПерсональныйПрофильБезопасностиНадпись.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Дополнительный отчет или обработка будет подключаться к программе с использованием ""персонального""
					|профиля безопасности %1, в котором будут разрешены только следующие операции:'; en = 'An additional report or data processor will connect to a program by using the ""personal"" security profile %1, which will only allow the following operations:'"),
				БезопасныйРежим);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 не является корректным режимом подключения для дополнительных отчетов и обработок, требующих разрешений на использование профилей безопасности!'; en = 'invalid attachment mode for additional reports or data processors that require permissions to use security profiles: %1.'"),
				БезопасныйРежим);
		КонецЕсли;
	...
	Если ЭтоГлобальнаяОбработка Тогда
		ПредставлениеНазначения = НСтр("ru = '<Не задано>'; en = '<Not set>'");
	...
	Иначе
		Если Объект.Назначение.Количество() = 0 Тогда
			ПредставлениеНазначения = НСтр("ru = 'Не определено'; en = 'Not defined'");
		Иначе
			ПредставлениеНазначения = "";
	...
	Если ЭтоНовый Тогда
		Заголовок = ?(ЭтоОтчет, НСтр("ru = 'Дополнительный отчет (создание)'; en = 'Additional report (create)'"), НСтр("ru = 'Дополнительная обработка (создание)'; en = 'Additional data processor (create)'"));
	Иначе
		Заголовок = Объект.Наименование + " " + ?(ЭтоОтчет, НСтр("ru = '(Дополнительный отчет)'; en = '(Additional report)'"), НСтр("ru = '(Дополнительная обработка)'; en = '(Additional data processor)'"));
	КонецЕсли;
	...
	Для Каждого ЭлементКоманда Из Объект.Команды Цикл
			...
			Если Не ПустаяСтрока(ЭлементКоманда.ЗаменяемыеКоманды) Тогда
				...
				Если КоличествоЗаменяемых > 0 Тогда
					Если КоличествоЗаменяемых = 1 Тогда
						ШаблонКомментария = НСтр("ru = 'Заменяет стандартную команду печати %1'; en = 'Replaces the standard print command %1'");
					Иначе
						ШаблонКомментария = НСтр("ru = 'Заменяет стандартные команды печати: %1'; en = 'Replaces the standard printing commands: %1'");
					КонецЕсли;
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не применимо для команд с вариантом запуска ""%1""'; en = 'Not applicable for commands with running option ""%1""'"),
...
Процедура ВариантыДополнительногоОтчетаЗаполнить()
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Не удалось получить список вариантов отчета из-за ошибки, возникшей при подключении этого отчета:'; en = 'Cannot get the list of report options due to report attachment error:'");
...
Функция НазваниеСтраницыКоманд()
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма Тогда
		Возврат НСтр("ru = 'Команды печати'; en = 'Print commands'");
	Иначе
		Возврат НСтр("ru = 'Команды'; en = 'Commands'");
	КонецЕсли;
КонецФункции
