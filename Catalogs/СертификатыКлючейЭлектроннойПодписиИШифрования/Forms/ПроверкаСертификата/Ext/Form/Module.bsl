...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПроверкаПриВыборе Тогда
		Элементы.ФормаПроверить.Заголовок = НСтр("ru = 'Проверить и продолжить'; en = 'Validate and continue'");
		...
		Если ДополнительныеПроверки.Количество() = 0 Тогда
			ВызватьИсключение
				НСтр("ru = 'Для проверки сертификата отключены стандартные проверки,
				           ...
				           |en = 'Standard tests for certificate validation are disabled, and
				           |no additional checks are specified.'");
		КонецЕсли;
...
Процедура ПоказатьПредупреждениеОНевозможностиПродолжения()
	...
	ПоказатьПредупреждение(,
		НСтр("ru = 'Не удалось продолжить, т.к. пройдены не все требуемые проверки.'; en = 'Unable to continue, because not all required validation checks were passed.'"));
...
Процедура ПроверитьНаСторонеКлиентаПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	Если Контекст.СертификатКриптографии.Субъект.Свойство("SN") Тогда
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ru = 'В описании субъекта сертификата не найдено поле ""SN"".'; en = 'Field ""SN"" is not found in the certificate subject details.'");
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		ОписаниеОшибки = Результат.ОписаниеОшибки + Символы.ПС + Символы.ПС
			+ НСтр("ru = 'Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.'; en = 'Validation of the signing, of the signature generated and of decryption cannot be performed.'");
	Иначе
		ОписаниеОшибки = "";
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификатаВМоделиСервиса(Результат, Контекст) Экспорт
	...
	ИначеЕсли Результат.Сертификат = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru = 'Сертификат не найден в хранилище сертификатов.'; en = 'The certificate is not found in a certificate store.'") + Символы.ПС
			+ НСтр("ru = 'Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.'; en = 'Validation of the signing, of the signature generated and of decryption cannot be performed.'");
	Иначе
		ОписаниеОшибки = "";
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru = 'Программа для использования закрытого ключа не указана в сертификате.'; en = 'Program to use the private key is not specified in the certificate.'");
	...
	Иначе
		ОписаниеОшибки = Результат + Символы.ПС + Символы.ПС
			+ НСтр("ru = 'Проверка подписания, созданной подписи, шифрования и
			             |расшифровки не могут быть выполнены.'; en = 'Validation of the signing, of the signature generated, of encryption, and decryption cannot be performed.'");
	КонецЕсли;
...
Процедура УстановитьЭлемент(Форма, НачалоЭлемента, НаСервере, ОписаниеОшибки = Неопределено, ЭтоОшибка = Ложь)
	...
	Если ОписаниеОшибки = Неопределено Тогда
		...
		ЭлементКартинка.Подсказка   = НСтр("ru = 'Проверка не выполнялась.'; en = 'No validation was done.'");
	...
	Иначе
		...
		ЭлементКартинка.Подсказка   = НСтр("ru = 'Проверка выполнена успешно.'; en = 'Validation successful.'");;
