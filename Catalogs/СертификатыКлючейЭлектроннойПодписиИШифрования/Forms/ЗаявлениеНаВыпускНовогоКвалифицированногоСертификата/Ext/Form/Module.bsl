...
Процедура ЮридическийАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеАдресаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"ЮридическийАдрес", НСтр("ru = 'Юридический адрес организации'; en = 'Company legal address'"));
...
Процедура ФактическийАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеАдресаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"ФактическийАдрес", НСтр("ru = 'Фактический адрес организации'; en = 'Company actual address'"));
...
Процедура ТелефонНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеТелефонаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"Телефон", НСтр("ru = 'Телефон организации'; en = 'Company phone'"));
	...
	Если Не ЗначениеЗаполнено(ГражданствоОКСМКодАльфа3) Тогда
		Если ЗначениеЗаполнено(Гражданство) Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У страны ""%1"" не указан трехзначный буквенный код альфа-3 по ОКСМ.'; en = 'Country ""%1"" has no three-digit alphabetic code specified (Alpha-3 for OKSM).'"),
				Гражданство));
		КонецЕсли;
		...
		Если Не ЭтоПолноправныйПользователь Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Программа %1 еще не добавлена в список используемых программ.
					           ...
					           |en = 'Program %1 is not yet added to the list of used programs.
		...
		ИначеЕсли ЭтоПодчиненныйУзелРИБ Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Программа %1 еще не добавлена в список используемых программ.
					           ...
					           |en = 'Program %1 is not yet added to the list of used programs.
					           |Add it at the master node of infobase.'"),
					Представление));
		Иначе
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("Добавить",    НСтр("ru = 'Добавить'; en = 'Add'"));
			Кнопки.Добавить("НеДобавлять", НСтр("ru = 'Не добавлять'; en = 'Do not add'"));
			ПоказатьВопрос(
	...
	Если НавигационнаяСсылка = "КлючЭлектроннойПодписи" Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Ключ электронной подписи - это секретная информация, которая сохраняется
			           ...
			           |en = 'Digital signature key is a secret information which is stored onto computer,
			           |USB flash drive, disk or other medium and is used in future to create 
			           |digital signatures.'"));
	...
	ИначеЕсли НавигационнаяСсылка = "ЗапросНаСертификат" Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Запрос на сертификат - это не секретная информация, которая создается
			           ...
			           |en = 'The certificate request is not secret information that is based on digital signature 
			           |key, is sent together with the application for the issue of the certificate, and 
			           |is required for the issuance of the certificate.'"));
	КонецЕсли;
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		ТекстПредупреждения =
			НСтр("ru = 'Комплект документов:
			           ...
			           |en = 'Document set:
			           |1. Application for issuance of certificate (prepared at the previous step).
			           |2. Copy* of certificate of registration at the tax authority (TIN).
			           |3. Copy* of certificate of state registration of the individual entrepreneur (PSRN).
			           |4. Copy* of the identity document of the certificate owner.
			           |5. Copy* of the mandatory retirement insurance certificate of the certificate owner.'");
	Иначе
		ТекстПредупреждения =
			НСтр("ru = 'Комплект документов:
			           |1. Заявление на выпуск сертификата (подготовленное на предыдущем шаге).
			           ...
			           |en = 'Document set: 
			           |1. Application for issue of certificate (prepared at the previous step). 
			           |2. Copy* of certificate of registration at the tax authority (TIN). 
			           |3. Copy* of certificate of state registration of the legal entity (PSRN). 
			           |4. Copy* of the identity document of the representative of the legal entity, specified for the certificate. 
			           |5. Copy* of the mandatory retirement insurance certificate of the representative of the legal entity, specified for the certificate. 
			           |6. Copy* of the document that confirms the authority of the manager who signed the appication (charter, etc) or a valid statement from the United Registry of Legal Entities certified by a tax authority. 
			           |
			           |* Copies are certified by the seal of the Entiry and by the signature of its head.'");
	КонецЕсли;
...
Процедура ВыгрузитьКорневойСертификат(Команда)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СохранитьСертификат(Неопределено,
		АдресКорневогоСертификата, НСтр("ru = 'Корневой сертификат ООО НПЦ 1С'; en = 'Root certificate of 1C LLC'"));
...
Процедура НапечататьДокументы(Команда)
	...
	НазваниеПечатнойФормы = НСтр("ru = 'Заявление на выпуск сертификата'; en = 'Application for issue of a certificate'");
...
Процедура ПерейтиНаСтраницуСоглашение()
	...
	Элементы.ФормаЗакрыть.Заголовок = НСтр("ru = 'Отмена'; en = 'Cancel'");
...
Процедура ПерейтиНаСтраницуОтправкаЗаявления()
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Программа электронной подписи"" не заполнено'; en = 'Field ""Digital signature program"" is not filled in'"), , "Программа");
...
Процедура ПерейтиНаСтраницуОтправкаЗаявленияПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru = 'Создание ключа электронной подписи'; en = 'Creating digital signature key'"), "", Результат, Новый Структура);
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуОжиданиеОбработкиЗаявления()
	...
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Отправить",   НСтр("ru = 'Отправить'; en = 'Send'"));
	Кнопки.Добавить("НеОтправить", НСтр("ru = 'Не отправлять'; en = 'Do not send'"));
	...
	Если Не ДокументыПечатались Тогда
		Текст =
			НСтр("ru = 'Документы еще не печатались.
			           ...
			           |en = 'The documents have not been printed yet.
			           |The request will not be accepted until the printed documents are received.'");
	КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ДокументыПартнерИНН) Тогда
			Текст = Текст + Символы.ПС  + Символы.ПС
				+ НСтр("ru = 'Не указан ИНН обслуживающей организации.
				             ...
				             |en = 'TIN of the supporting partner is not specified. 
				             |Processing of the application might take longer than usual.'");
		КонецЕсли;
	...
	ИначеЕсли Не ЗначениеЗаполнено(ДокументыПартнерИНН)
		...
		Текст = Текст + Символы.ПС  + Символы.ПС
			+ НСтр("ru = 'Не указан ИНН или КПП обслуживающей организации.
			             |Заявление может обрабатываться дольше обычного.';
			             |en = 'TIN or TRRC of the supporting partner are not specified.
			             |Processing of the application might take longer than usual.'");
	КонецЕсли;
	...
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Невозможно подключиться к порталу интернет-поддержки по причине:
			           ...
			           |en = 'Unable to connect to online customer support because: 
			           |Online support library is not included into the configuration.'"));
	КонецЕсли;
...
Функция ПерейтиНаСтраницуОжиданиеОбработкиЗаявленияНаСервере(ОписаниеОшибки)
	...
	СостояниеОбработкиЗаявления = НСтр("ru = 'Заявление принято для обработки.'; en = 'Application accepted for processing.'");
...
Процедура ОбновитьДатаОбновленияСостоянияВЗаголовке(Форма)
	...
	Форма.Элементы.СостояниеОбработкиЗаявления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Состояние обработки заявления (на %1)'; en = 'Application processing state (at %1)'"),
...
Процедура ПерейтиНаСтраницуУстановкаСертификата()
	...
	Если Результат <> Истина Тогда
		ПослеЗаписи(НСтр("ru = 'Заявление отклонено.'; en = 'Application rejected.'"));
		Закрыть();
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуУстановкаСертификатаПослеУстановки(Установлен, Неопределен) Экспорт
	...
	Если Установлен Тогда
		...
		ПослеЗаписи(НСтр("ru = 'Заявление исполнено.'; en = 'Application is executed.'"));
		Закрыть();
	Иначе
		ПослеЗаписи();
		ЗаписатьЗаявление();
...
Процедура ОбновитьДатаУстановкиСертификатаВЗаголовке(Форма)
	...
	Форма.Элементы.ОшибкаУстановкиСертификата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка установки сертификата (на %1)'; en = 'Certificate installation error (%1)'"),
...
Процедура ЗакрытьПомощникПослеУстановкиСертификатаЗавершение(Установлен, Неопределен) Экспорт
	...
	ПослеЗаписи(НСтр("ru = 'Сертификат установлен.'; en = 'Certificate is installed.'"));
	Закрыть();
...
Процедура ПриИзмененииОрганизацииНаСервере(Загрузка = Ложь)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		...
		Элементы.ОГРН.Заголовок = НСтр("ru = 'ОГРНИП'; en = 'PSRNIE'");
	Иначе
		Элементы.КПП.Видимость = Истина;
		Элементы.ОГРН.Заголовок = НСтр("ru = 'ОГРН'; en = 'PSRN'");
	КонецЕсли;
	...
	Попытка
		...
		Если ПодробныйИтог.Результат <> "Корректный" Тогда
			...
			Если Не ЗначениеЗаполнено(Сообщение) Тогда
				Сообщение = НСтр("ru = 'Адрес не заполнен'; en = 'Address is not filled in'");
			КонецЕсли;
		КонецЕсли;
	Исключение
...
Функция РасчетныйСчетСоответствуетТребованиям(РасчетныйСчет, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ru = 'Расчетный счет должен состоять только из цифр.'; en = 'Current account must consist only of digits.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(Значение) <> 20 Тогда
		ТекстСообщения = НСтр("ru = 'Расчетный счет должен состоять из 20 цифр.'; en = 'Current account should consist of 20 digits.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСКонтрагентами") Тогда
		...
		Если Не МодульРегламентированныеДанныеКлиентСервер.КонтрольныйКлючЛицевогоСчетаСоответствуетТребованиям(Значение, БИК, ЭтоБанк) Тогда
			ТекстСообщения = НСтр("ru = 'Контрольное число счета не совпадает с рассчитанным с учетом БИК'; en = 'Control digit of the number does not match the one calculated considering the BIN'");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция БИКСоответствуетТребованиям(БИК, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ru = 'БИК должен состоять только из цифр.'; en = 'BIN must contain numbers only.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(Значение) <> 9 Тогда
		ТекстСообщения = НСтр("ru = 'БИК должен состоять из 9 цифр.'; en = 'BIN must consist of 9 digits.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция КорреспондентскийСчетСоответствуетТребованиям(КорреспондентскийСчет, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("ru = 'Корреспондентский счет должен состоять только из цифр.'; en = 'Correspondent account must contain only numbers.'");
		Возврат Ложь;
	КонецЕсли;

	Если СтрДлина(Значение) <> 20 Тогда
		ТекстСообщения = НСтр("ru = 'Корреспондентский счет должен состоять из 20 цифр.'; en = 'Correspondent account should consist of 20 digits.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция АдресСоответствуетТребованиям(Знач АдресXML, ТекстСообщения)
	...
	Если Не АдресСтруктура.Свойство("КодРегиона") Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Это не российский адрес'; en = 'The address is not a Russian one'");
		Возврат Ложь;
	КонецЕсли;
	
	// Проверка, что указан регион.
	Если Не АдресСтруктура.Свойство("Регион") Или Не ЗначениеЗаполнено(АдресСтруктура.Регион) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Не указан регион'; en = 'Region is not specified'");
		Возврат Ложь;
	КонецЕсли;
	
	// Проверка, что регион указан правильно - код региона определен.
	Если Не ЗначениеЗаполнено(АдресСтруктура.КодРегиона) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Некорректный регион (код региона не определен)'; en = 'Incorrect region (region code not determined)'");
		Возврат Ложь; 
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НаселенныйПунктПолностью) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Не указан населенный пункт'; en = 'Settlement is not specified'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ТелефонСоответствуетТребованиям(Знач ТелефонXML, ТекстСообщения)
	...
	Если СтрЗаменить(ТелефонСтруктура.КодСтраны, "+", "") <> "7" Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Код страны не российский (должен быть ""7"")'; en = 'Country code is not Russian (must be ""7"")'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НомерТелефонаБезКодаСтраны) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Не заполнен номер телефона'; en = 'Phone number is not filled in'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(ТолькоЦифры(НомерТелефонаБезКодаСтраны)) <> 10 Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Номер телефона с кодом города должен состоять из 10-и цифр'; en = 'Phone number with area code should consist of 10 digits'");
		Возврат Ложь;
	КонецЕсли;
		...
		Если Не ЭтоИндивидуальныйПредприниматель Тогда
			Если ЗначениеЗаполнено(Реквизиты.Директор) Тогда
				Элементы.Сотрудник.СписокВыбора.Добавить(Реквизиты.Директор, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Директор: %1'; en = 'Top manager: %1'"), Реквизиты.Директор));
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(Реквизиты.ГлавныйБухгалтер) Тогда
				Элементы.Сотрудник.СписокВыбора.Добавить(Реквизиты.ГлавныйБухгалтер, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Главный бухгалтер: %1'; en = 'Chief accountant: %1'"), Реквизиты.ГлавныйБухгалтер));
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоИндивидуальныйПредприниматель Тогда
			Элементы.Сотрудник.Заголовок = НСтр("ru = 'Индивидуальный предприниматель'; en = 'Individual entrepreneur'");
		Иначе
			Элементы.Сотрудник.Заголовок = НСтр("ru = 'Сотрудник'; en = 'Employee'");
		КонецЕсли;
...
Процедура ПриИзмененииВидаДокументаНаСервере()
	...
	Если ДокументВид = "21" Тогда
		Элементы.ДокументНомер.Заголовок = НСтр("ru = 'Серия и номер'; en = 'Series and number'");
		Элементы.ДокументНомер.Маска = "99 99 999999";
	Иначе
		Элементы.ДокументНомер.Заголовок = НСтр("ru = 'Номер'; en = 'Number'");
...
Функция НомерПаспортаРФСоответствуетТребованиям(Знач НомерПаспортаРФ, ТекстСообщения)
	...
	Если ПустаяСтрока(СтрокаЦифр) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Номер паспорта не заполнен'; en = 'Passport number is not filled in'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(СтрокаЦифр) < 10 Тогда
		ТекстСообщения  =  ТекстСообщения + НСтр("ru = 'Номер паспорта задан неполностью'; en = 'Passport number is not complete'");
		Возврат Ложь;
	КонецЕсли;
	
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаЦифр) Тогда
		Результат = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Номер паспорта должен состоять только из цифр.'; en = 'Passport number must consist only of digits.'");
	КонецЕсли;
...
Функция ПроверитьРеквизит(Отказ, ИмяРеквизита)
	...
	Если Не ЗначениеЗаполнено(ЭтотОбъект[ИмяРеквизита]) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Поле ""%1"" не заполнено'; en = 'Field ""%1"" is not filled in'"), ЗаголовокПоля), , ИмяРеквизита, , Отказ);
...
Функция ПодготовленныйДокумент()
	...
	Макет.Параметры.УдостоверениеЛичности = ДокументВидПредставление + " " + ДокументНомерПредставление + " "
		+ НСтр("ru = 'от'; en = 'of'") + " " + ДокументДатаВыдачиПредставление + " "
		+ НСтр("ru = 'выданный'; en = 'issued'") + " " + ДокументКемВыдан;
...
Функция ОтправитьЗаявление(ОписаниеОшибки)
	...
	ШаблонОписанияОшибки =
		НСтр("ru = 'Не удалось отправить заявление по причине:
		           |%1';
		           |en = 'Failed to send the application because:
		           |%1'");
	...
	Если ЗначениеЗаполнено(Результат.КодОшибки) Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось подключиться к порталу интернет-поддержки по причине:
			           |%1'; en = 'Unable to connect to online customres support because:%1'"),
	...
	Если Не ЗначениеЗаполнено(Ответ) Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки,
			НСтр("ru = 'Сервер вернул пустой ответ.'; en = 'Server returned an empty response.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если КодРезультата = "202" Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось проверить договор на ИТС (its.1c.ru) по причине:
			           ...
			           |en = 'Could not verify the ITS contract (its.1c.ru) because: 
			           |the ticket obtained for user %1 is invalid or outdated. 
			           |
			           |Try to send the application again.'"),
	...
	ИначеЕсли КодРезультата = "206" Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Заявление не принято по причине:
			           ...
			           |en = 'Application is not accepted on the grounds of: 
			           |User %1 does not have a valid contract for ITS (its.1c.ru). 
			           |For the conclusion of the contract, contact the supporting partner. 
			           |
			           |After the conclusion of the contract, send the statement again.'"),
			ПредставлениеПользователяПорталаИнтернетПоддержки());
	Иначе
...
Функция ТекстФайлаЗапросаНаСертификат()
	...
	ТекстОшибки =
		НСтр("ru = 'Не удалось прочитать ранее созданный запрос на сертификат.
		           ...
		           |en = 'Could not read the certificate request created earlier.
...
Функция ПолучитьСертификат()
	...
	ШаблонОписанияОшибки =
		НСтр("ru = 'Не удалось обновить состояние по причине:
		           |%1';
		           |en = 'Failed to update the state because:
		           |%1'");
	...
	Если КодРезультата = "0" Тогда
		...
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки,
				НСтр("ru = 'Сервер вернул пустые данные.'; en = 'Server returned empty data.'"));
		КонецЕсли;
		
	ИначеЕсли КодРезультата = "1" Тогда
		ОписаниеОшибки = НСтр("ru = 'Заявление еще не обработано, попробуйте позже.'; en = 'The application is not yet processed, try again later.'");
	Иначе
		УзелDOM = ПостроительDOM.ПолучитьЭлементыПоИмени("errorMessage");
		Если УзелDOM.Количество() > 0 Тогда
			ОписаниеОшибки = УзелDOM[0].ТекстовоеСодержимое;
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сервер вернул код ошибки: %1'; en = 'Server returned error code: %1'"),
	...
	Попытка
		...
		Если Не ФайлНайден Тогда
			ВызватьИсключение НСтр("ru = 'Неверный формат данных.'; en = 'Invalid data format.'");
		КонецЕсли;
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось разобрать ответ сервера по причине:
			           |%1';
			           |en = 'Could not parse the server response because:
			           |%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
...
Процедура УстановитьСертификатПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru = 'Установка сертификата на компьютер'; en = 'Installing certificate onto computer'"), "", Менеджер, Новый Структура);
	...
	Иначе
		СоздатьОбъектКриптографии(Новый ОписаниеОповещения(
			...
			НСтр("ru = 'Для установки сертификата на компьютер требуется
			           ...
			           |en = 'To install the certificate onto the computer the extension 
			           |for 1C:Enterprise web client must be installed.'"),
			НСтр("ru = 'Для установки сертификата на компьютер требуется
			           |установить дополнительную внешнюю компоненту.';
			           |en = 'To install the certificate onto the computer the additional
			           |external component must be installed.'"));
	КонецЕсли;
...
Процедура УстановитьСертификатПриОшибкеСозданияОбъектаКриптографииПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ОшибкаУстановкиСертификата = НСтр("ru = 'Не установлено расширение для работы с файлами.'; en = 'The extension for working with files is not installed.'");
	Иначе
...
Процедура УстановитьСертификатПриОшибкеСозданияОбъектаКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ОшибкаУстановкиСертификата = НСтр("ru = 'Не установлено расширение для работы с криптографией.'; en = 'The extension for working with cryptography is not installed.'");
	Иначе
		ОшибкаУстановкиСертификата = НСтр("ru = 'Не установлена дополнительная внешняя компонента.'; en = 'The additional external component is not installed.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияРазрешений(РазрешенияПолучены, Контекст) Экспорт
	...
	Если Не РазрешенияПолучены Тогда
		ОшибкаУстановкиСертификата =
			НСтр("ru = 'Сохранение сертификатов во временную папку отменено пользователем.'; en = 'Saving certificates to a temporary folder was canceled by the user.'");
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ПолученныеФайлы = Неопределено
	 Или ПолученныеФайлы.Количество() <> 2 Тогда
		...
		ОшибкаУстановкиСертификата = НСтр("ru = 'Сертификаты не были сохранены во временную папку.'; en = 'Certificates were not saved to the temporary folder.'");
	...
	Если Не Существует Тогда
		ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось найти контейнер ключа на компьютере.
			           |Имя контейнера: ""%1"".
			           |Путь к контейнеру: ""%2"".';
			           |en = 'Could not find the key container on the computer.
			           |Container name: ""%1"".
			           |Container path: ""%2"".'"),
			КонтейнерКлючаИмя,
			КонтейнерКлючаПуть);
...
Процедура УстановитьСертификатПослеВызоваУстановитьСертификатВКонтейнерИХранилище(Результат, ПараметрыВызова, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("ru = 'Не удалось установить личный сертификат.'; en = 'Failed to install the personal certificate.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("ru = 'Не удалось найти личный сертификат, установленный на компьютер.'; en = 'Could not find the personal certificate installed on the computer.'");
		Иначе
			ОшибкаПоиска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти личный сертификат, установленный на компьютер по причине:
				           |%1';
				           |en = 'Could not find personal certificate installed on the computer because:
				           |%1'"),
...
Процедура УстановитьСертификатПослеВызоваИмпортироватьСертификат(Результат, ПараметрыВызова, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("ru = 'Не удалось установить корневой сертификат.'; en = 'Failed to install the root certificate.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПоискаКорневогоСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("ru = 'Не удалось найти корневой сертификат, установленный на компьютер.'; en = 'Could not find the root certificate installed on the computer.'");
		Иначе
			ОшибкаПоиска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось найти корневой сертификат, установленный на компьютер по причине:
				           |%1';
				           |en = 'Could not find the root certificate installed on the computer because:
				           |%1'"),
				Результат.ОписаниеОшибки);
		КонецЕсли;
		ОшибкаУстановкиСертификата = СокрЛ(ОшибкаУстановкиСертификата + Символы.ПС + Символы.ПС) + ОшибкаПоиска;
...
Функция ИмяРегионаРФПоРекомендациямДляСКПЭП(КодРегиона)
	...
	Если Не СтрНачинаетсяС(Строка, КодРегиона + " ") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Регион РФ с кодом ""%1"" не существует.'; en = 'Region of Russian Federation with the code ""%1"" does not exist.'"),
			КодРегиона);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Строка) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для региона РФ с кодом ""%1"" имя, рекомендованное для СКПЭП, еще не назначено.'; en = 'For the region of the Russian Federation with the code ""%1"" the name recommended for certificates is not yet assigned.'"),
			КодРегиона);
	КонецЕсли;
...
Процедура ЗагрузитьЗаявление()
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отклонено Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru = 'Заявление, по которому не удалось получить сертификат'; en = 'Application for which a certificate was not obtained'");
		Элементы.ФормаДалее.Видимость = Ложь;
		Элементы.ФормаЗакрыть.Заголовок = НСтр("ru = 'Закрыть'; en = 'Close'");
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Исполнено Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru = 'Заявление, по которому был получен сертификат'; en = 'Application for which the certificate was obtained'");
...
Процедура ЗаполнитьРеквизитыЗаявления(Все = Ложь)
	...
	Если Все Тогда
		...
		Строка.Реквизит = НСтр("ru = 'Программа электронной подписи'; en = 'Digital signature program'");
		Строка.Значение = ПредставлениеПрограммы;
	КонецЕсли;
	
	// Заполнение сведений об организации.
	ЗаполнитьРеквизит(НСтр("ru = 'Сведения об организации'; en = 'Company information'"),, Истина);
	...
	ЗаполнитьРеквизит(НСтр("ru = 'Сведения о владельце сертификата'; en = 'Certificate owner information'"),, Истина);
	...
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ru = 'Документ, удостоверяющий личность'; en = 'Identity document'");
	Строка.Значение = Элементы.ДокументВид.СписокВыбора.НайтиПоЗначению(ДокументВид).Представление;
	
	Строка = РеквизитыЗаявления.Добавить();
	Если ДокументВид = "21" Тогда
		Строка.Реквизит = НСтр("ru = 'Серия и номер'; en = 'Series and number'");
		Строка.Значение = ПредставлениеРеквизитаНомерПаспортаРФ(ДокументНомер);
	Иначе
		Строка.Реквизит = НСтр("ru = 'Номер'; en = 'Number'");
	...
	Если Не ЭтоИндивидуальныйПредприниматель Тогда
		...
		ЗаполнитьРеквизит(НСтр("ru = 'Сведения о руководителе'; en = 'Manager information'"),, Истина);
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ru = 'ФИО руководителя'; en = 'Full name of manager'");
		Строка.Значение = ДокументыРуководитель;
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ru = 'Должность руководителя'; en = 'Title of manager'");
		Строка.Значение = ДокументыРуководительДолжность;
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ru = 'Основание действий руководителя'; en = 'Manager authorized by'");
		Строка.Значение = ДокументыРуководительОснование;
	КонецЕсли;
	
	// Заполнение сведений о печатных документах.
	ЗаполнитьРеквизит(НСтр("ru = 'Сведения об обслуживающей организации'; en = 'Supporing partner information'"),, Истина);
	
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ru = 'Наименование партнера'; en = 'Name of partner'");
	Строка.Значение = ДокументыПартнер;
	
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("ru = 'ИНН партнера'; en = 'Partner TIN'");
	Строка.Значение = ДокументыПартнерИНН;
	
	Если Не ДокументыПартнерЭтоИП Тогда
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("ru = 'КПП партнера'; en = 'Partner TRRC'");
...
Функция ПредставлениеРеквизитаПол(Пол)
	...
	Если Пол = "Мужской" Тогда
		Возврат НСтр("ru = 'Мужской'; en = 'Male'");
		
	ИначеЕсли Пол = "Женский" Тогда
		Возврат НСтр("ru = 'Женский'; en = 'Female'");
	КонецЕсли;
...
Процедура УстановитьСоглашение()
	...
	ТекстHTML = СтрЗаменить(ТекстHTML, "%1",
		...
		НСтр("ru = 'ООО «Научно-производственный центр ""1С""»'; en = 'ООО «Научно-производственный центр ""1С""»'"));
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%2",
		// Адрес регламента удостоверяющего центра.
		НСтр("ru = 'http://ca.1c.ru/reglament.pdf'; en = 'http://ca.1c.ru/reglament.pdf'"));
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%3",
		// Сеть доверенных удостоверяющих центров.
		НСтр("ru = 'уполномоченного федерального органа в области использования электронной подписи'; en = 'of the federal authority in the area of digital signature use'"));
...
Процедура ПослеВыбораПрограммыПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		...
		Если Контекст.ПоказатьОшибку Тогда
			ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
				НСтр("ru = 'Выбор программы электронной подписи'; en = 'Selecting digital signatures program'"), "", Менеджер, Новый Структура);
		КонецЕсли;
...
Процедура ЗаписатьЗаявление()
	...
	Если ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.НеПодготовлено Тогда
		...
		Имя = СокрЛП(Имя);
		Отчество = СокрЛП(Отчество);
		ТекущийОбъект.Наименование = Фамилия + " " + Имя + " " + Отчество + " (" + НСтр("ru = 'Заявление на новый сертификат'; en = 'Application for new certificate'") + ")";
		// Организация.
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отклонено Тогда
		Если ЭтоНовоеСостояние Тогда
			ОбновитьЗначение(Содержание, "ДатаОбновленияСостояния");
			ОбновитьЗначение(Содержание, "СостояниеОбработкиЗаявления");
			НовоеНаименование = Фамилия + " " + Имя + " " + Отчество + " (" + НСтр("ru = 'Удалить'; en = 'Delete'") + ")";
...
Процедура ПослеЗаписи(ТекстОповещения = "")
	...
	Если Не ЗначениеЗаполнено(ТекстОповещения) Тогда
		ТекстОповещения = НСтр("ru = 'Заявление сохранено.'; en = 'Application saved.'");
	КонецЕсли;
	...
	Иначе
		СоздатьОбъектКриптографии(Новый ОписаниеОповещения(
			...
			НСтр("ru = 'Для создания ключа электронной подписи и запроса на сертификат
			           ...
			           |en = 'To create digital signature key and to request a certificate the extension 
			           |for 1C:Enterprise web client must be installed.'"),
			НСтр("ru = 'Для создания ключа электронной подписи и запроса на сертификат
			           |требуется установить дополнительную внешнюю компоненту.';
			           |en = 'To create digital signatures and certificate requests the additional external 
...
Процедура СоздатьКлючИЗапросНаСертификатПослеПроверкиСуществованияКонтейнераКлючей(Существует, Контекст) Экспорт
	...
	Если Существует Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Используется ранее созданный контейнер ключа:'; en = 'Using previously created container key:'"),,
			КонтейнерКлючаПуть);
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваСоздатьКонтейнер(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Ошибка.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось выполнить операцию по причине:
		           |%1';
		           |en = 'The operation failed because:
		           |%1'"),
		КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
	
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ru = 'Создание ключа электронной подписи'; en = 'Creating digital signature key'"), "", Ошибка, Новый Структура);
...
Процедура СоздатьКлючИЗапросНаСертификатПослеВызоваСоздатьКонтейнер(Путь, ПараметрыВызова, Контекст) Экспорт
		...
		Если Строки[0].Идентификатор = "CryptoPro" Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ключ электронной подписи не создан.
				           ...
				           |en = 'Digtal signature key is not created.
				           |
				           |It should be borne in mind that in order to create a key using the program %1,
				           |administrator permissions of the operating system are required.'"),
				Строки[0].Представление));
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ru = 'Ключ электронной подписи не создан.'; en = 'Digtal signature key is not created.'"));
		КонецЕсли;
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Создан новый контейнер ключа:'; en = 'New key container created:'"),, КонтейнерКлючаПуть);
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваСоздатьЗапросНаСертификат(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ru = 'Не удалось выполнить операцию по причине:'; en = 'The operation failed because:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваПолучитьОткрытыйКлюч(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ru = 'Не удалось получить открытую часть ключа по причине:'; en = 'Failed to get the public part of the key because:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваВычислитьИдентификаторКлючаСубъекта(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("ru = 'Не удалось вычислить идентификатор ключа субъекта по причине:'; en = 'Failed to evaluate the subject key identifier because:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке = Неопределено, ЗаголовокОшибки = "")
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru = 'Создание запроса на сертификат'; en = 'Creating certificate request'"), "", Ошибка, Новый Структура);
		
	ИначеЕсли Не Контекст.ЗапросСоздан Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Запрос на сертификат не создан.'; en = 'Certificate request is not created.'"));
	КонецЕсли;
...
Процедура ПолучитьИмяНовогоКонтейнераКлюча(Оповещение)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		...
		Субъект = ПодготовитьСтрокуДляИмениКонтейнера(Фамилия) + " "
			...
			+ НСтр("ru = 'Индивидуальный предприниматель'; en = 'Individual entrepreneur'");
	Иначе
...
Процедура СоздатьОбъектКриптографииПослеПодключенияВнешнейКомпоненты(Подключена, Контекст) Экспорт
	...
	Иначе
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Установить",      НСтр("ru = 'Установить'; en = 'Install'"));
		Кнопки.Добавить("НеУстанавливать", НСтр("ru = 'Не устанавливать'; en = 'Do not install'"));
	...
	ПоказатьПредупреждение(Новый ОписаниеОповещения(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось установить дополнительную внешнюю компоненту по причине:
			           |%1';
			           |en = 'Unable to install the additional external component because:
			           |%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
	...
	Если Не Подключена Тогда
		ПоказатьПредупреждение(
			Новый ОписаниеОповещения("СоздатьОбъектКриптографииПослеПредупрежденияОбОшибке",
				ЭтотОбъект, Контекст),
			НСтр("ru = 'Не удалось подключить дополнительную внешнюю компоненту.'; en = 'Unable to connect the additional external component.'"));
	Иначе
		СоздатьОбъектКриптографииПродолжение(Контекст);
	КонецЕсли;
	...
	Исключение
		...
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось создать объект внешней компоненты для работы с криптографией по причине:
			           |%1';
			           |en = 'Failed to create the object of the external component for working with cryptography because:
			           |%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
		ВыполнитьОбработкуОповещения(Контекст.ОбработкаПродолжения, Ложь);
		Возврат;
		...
		Исключение
			...
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать объект внешней компоненты для работы с двоичными данными по причине:
				           |%1';
				           |en = 'Failed to create the object of the external component to work with binary data because:
				           |%1'"),
				КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
			ВыполнитьОбработкуОповещения(Контекст.ОбработкаПродолжения, Ложь);
			Возврат;
	...
	ПоказатьПредупреждение(Новый ОписаниеОповещения(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Программа электронной подписи не доступна через внешнюю компоненту по причине:
			           |%1';
			           |en = 'Digital signature program is not available via the external component because:
			           |%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке)));
