...
Процедура Далее(Команда)
	...
	Если Элементы.Сертификаты.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выделите сертификаты, которые требуется добавить.'; en = 'Select certificates to add.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДалееПослеПоискаСертификата(Результат, Неопределен) Экспорт
	...
	Если Результат.Свойство("СертификатНеНайден") Тогда
		Контекст.Вставить("ОписаниеОшибки", НСтр("ru = 'Сертификат не найден на компьютере (возможно удален).'; en = 'Certificate is not found on the computer (perhaps deleted).'"));
	Иначе
...
Процедура ДалееПослеПоискаСертификатаВОблачномСервисе(Результат, Неопределен) Экспорт
	...
	Если Не Результат.Выполнено Тогда
		Контекст.Вставить("ОписаниеОшибки", НСтр("ru = 'Сертификат не найден в облачном сервисе (возможно удален).'; en = 'No certificate found in cloud service (possibly deleted).'"));
	Иначе
		Контекст.Вставить("ОписаниеОшибки", Результат.ОписаниеОшибки);
	КонецЕсли;
...
Процедура ПоказатьСтраницуУточнениеСвойствСертификата(Форма, СертификатКриптографии, ДанныеСертификата)
	...
	Строка = ?(ЗначениеЗаполнено(Форма.Сертификат), НСтр("ru = 'Обновить'; en = 'Refresh'"), НСтр("ru = 'Добавить'; en = 'Add'"));
