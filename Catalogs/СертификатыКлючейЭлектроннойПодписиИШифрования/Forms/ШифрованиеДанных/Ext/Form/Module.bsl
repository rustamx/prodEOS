...
Процедура СертификатыШифрованияПередУдалением(Элемент, Отказ)
	...
	Если Элемент.ТекущиеДанные <> Неопределено
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Нельзя удалить обязательный для шифрования сертификат.'; en = 'It is not possible to delete a certificate that is necessary for encryption.'"));
		Отказ = Истина;
	КонецЕсли;
		...
		Для каждого Строка Из Строки Цикл
			...
			Если ТипЗнч(ДанныеСертификата) <> Тип("ДвоичныеДанные") Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Данные сертификата ""%1"" не найдены в справочнике'; en = 'The data of certificate""%1"" is not found in the catalog'"), Выборка.Представление);
			КонецЕсли;
			Попытка
				СертификатКриптографии = Новый СертификатКриптографии(ДанныеСертификата);
			Исключение
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Данные сертификата ""%1"" в справочнике не корректны по причине:
					           |%2';
					           |en = 'The data of certificate""%1"" in the catalog is invalid because: 
					           |%2'"),
...
Процедура ЗашифроватьДанные(Оповещение)
	...
	Если ЗначениеЗаполнено(Сертификат) Тогда
		...
		Если Не ЗначениеЗаполнено(СертификатПрограмма) Тогда
			...
			|Выберите другой сертификат.';
			|en = 'The selected personal certificate has no program assigned for use with the private key.
			|Select another certificate.'"));
			ПоказатьОшибку(Контекст.ОшибкаНаКлиенте, Контекст.ОшибкаНаСервере);
			ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
			Возврат;
...
Функция СвойстваСертификатов(Знач Ссылки, Знач ИдентификаторФормы)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(ДанныеСертификата) <> Тип("ДвоичныеДанные") Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Данные сертификата ""%1"" не найдены в справочнике'; en = 'The data of certificate""%1"" is not found in the catalog'"), Выборка.Наименование);
		КонецЕсли;
...
Процедура ЗаписатьСертификатыШифрованияНаСервере(ОписаниеОбъектов, АдресСертификатов, ИдентификаторФормы, Ошибка)
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОтменитьТранзакцию();
		Ошибка.Вставить("ОписаниеОшибки", НСтр("ru = 'При записи сертификатов шифрования возникла ошибка:'; en = 'When saving the certificates an error occurred:'")
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("ru = 'Не удалось зашифровать данные'; en = 'Failed to encrypt data'"), "",
