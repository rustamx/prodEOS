...
Функция ПолучитьСообщения(НастройкаПриемаОтправки, Транспорт) Экспорт
    ...
    Если Сообщения <> Неопределено Тогда
		...
		Если Не Почта.УдалитьСообщенияВПочтовомЯщике(
			...
			ЗаписьЖурналаРегистрации(
				...
				НастройкаПриемаОтправки.Метаданные(),
				НастройкаПриемаОтправки, 
				НСтр("ru = 'Невозможно удалить полученные сообщения. '; en = 'Unable to delete the received messages. '", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + СообщениеОбОшибке);
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации(
			...
			НастройкаПриемаОтправки.Метаданные(),
			НастройкаПриемаОтправки,
			НСтр("ru = 'Невозможно загрузить сообщения. '; en = 'Unable to download the messages. '", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + СообщениеОбОшибке);
