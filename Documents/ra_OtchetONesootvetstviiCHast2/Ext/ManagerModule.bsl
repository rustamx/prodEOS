...
Процедура ЗаполнитьАктуальнымиДанными(Объект, ЗаполнятьСогласующих = Истина) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	KorrektiruyushcheeDejstvie.Ссылка КАК KorrektiruyushcheeDejstvie
	|ИЗ
	|	Документ.ra_KorrektiruyushcheeDejstvie КАК KorrektiruyushcheeDejstvie
	|ГДЕ
	|	KorrektiruyushcheeDejstvie.Nesootvetstvie = &Несоответствие
	|	И KorrektiruyushcheeDejstvie.ПометкаУдаления = ЛОЖЬ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Объект.DejstviyaKD.Загрузить(Результат[0].Выгрузить());
...
Процедура СформироватьМассивДанныхGetList(Результат, ПолноеИмя, ПараметрыЗапросаHTTP) Экспорт
	...
	ОбъектМетаданных = Метаданные.Документы.ra_OtchetONesootvetstviiCHast2;
...
Функция ПолучитьМассивКнопок(ДокументОбъект) Экспорт
	...
	ПараметрыПроверки = Новый Структура("ra_KorrektiruyushcheeDejstvie");
	ПараметрыПроверки.Вставить("ra_OtchetONesootvetstviiCHast2", Перечисления.СостоянияДокументов.ra_Утвержден);
	...
	ОписаниеКнопки = НСтр("ru = 'Сформировать отчет, часть 2'; en = 'Generate report, part 2'");
	КнопкаСформироватьОтчетЧ2 = ра_ОбменДанными.ПолучитьСтруктуруНастроекКнопки(ИмяКнопки, ОписаниеКнопки);
	КнопкаСформироватьОтчетЧ2.Availability = Полномочия.ПервыйЛидер И 
		РезультатыПроверки.Свойство("ra_KorrektiruyushcheeDejstvie") И
		Не РезультатыПроверки.Свойство("ra_OtchetONesootvetstviiCHast2");
	КнопкаСформироватьОтчетЧ2.Visibility = КнопкаСформироватьОтчетЧ2.Availability;
	...
	Если ВидФормы = "ФормаСписка" Тогда
		...
		|			ГДЕ
		|				ra_OtchetONesootvetstviiCHast2.Nesootvetstvie = &Nesootvetstvie)";
		
		Запрос.УстановитьПараметр("Nesootvetstvie", Несоответствие);
		РезультатЗапроса = Запрос.Выполнить();
		ЕстьНеЗавершенныеБизнесПроцессы = НЕ РезультатЗапроса.Пустой();
		
		Отчет2Утвержден = РезультатыПроверки.Свойство("ra_OtchetONesootvetstviiCHast2") И РезультатыПроверки.ra_OtchetONesootvetstviiCHast2;
		
		КнопкаОтменитьДокумент.Availability = Полномочия.ПервыйЛидер И
			Не Отчет2Утвержден И
			РезультатыПроверки.Свойство("ra_OtchetONesootvetstviiCHast2") И
	...
	МассивКнопок.Добавить(КнопкаСформироватьОтчетЧ2);
