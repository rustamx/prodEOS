...
Процедура СформироватьМассивДанныхGetList(Результат, ПолноеИмя, ПараметрыЗапросаHTTP) Экспорт
	...
	ОбъектМетаданных = Метаданные.Документы.ra_KorrektiruyushcheeDejstvie;
...
Функция ПолучитьМассивКнопок(ДокументОбъект) Экспорт
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		...
		КнопкаСохранить.Availability = Полномочия.ПервыйЛидер И Не РезультатыПроверки.Свойство("ra_OtchetONesootvetstviiCHast2");
		...
		Если Не ДокументОбъект.Ссылка.Пустая() Тогда
			...
			КнопкаЗагрузить.Availability = (Полномочия.ПервыйЛидер Или Полномочия.ОтветственныйЗаКачество)
				И Статус <> Перечисления.СостоянияДокументов.Исполнен;
	...
	ИначеЕсли ВидФормы = "ФормаСписка" Тогда
		...
		ИмяКнопки = "AddRow";
		ОписаниеКнопки = НСтр("ru = 'Добавить корректирующее действие'; en = 'Add corrective action'");
		...
		КнопкаДобавитьСтроку.Availability = Полномочия.ПервыйЛидер И Не РезультатыПроверки.Свойство("ra_OtchetONesootvetstviiCHast2") И НЕ ЭтоD3;
		...
		КнопкаОтчетПоНерезМероприятиям.Availability = РегистрыСведений.ra_PrichinyNesootvetstvij.ДоступностьКнопкиОтчетПоНерезультативнымМероприятиям(Несоответствие);
		КнопкаОтчетПоНерезМероприятиям.Visibility = КнопкаОтчетПоНерезМероприятиям.Availability;
		
		ИмяКнопки = "UploadOtchetONesootvetstviiCHast2";
		ОписаниеКнопки = НСтр("ru = 'Загрузить Отчет (часть 2)'; en = 'Upload Report (part 2)'");
		КнопкаЗагрузитьОтчет2 = ра_ОбменДанными.ПолучитьСтруктуруНастроекКнопки(ИмяКнопки, ОписаниеКнопки);
		КнопкаЗагрузитьОтчет2.Availability = Полномочия.ПервыйЛидер;
		КнопкаЗагрузитьОтчет2.Visibility = КнопкаЗагрузитьОтчет2.Availability;
		
		МассивКнопок.Добавить(КнопкаДобавитьСтроку);
		МассивКнопок.Добавить(КнопкаОтчетПоНерезМероприятиям);
		МассивКнопок.Добавить(КнопкаЗагрузитьОтчет2);
...
Функция ПолучитьПолучитьМассивКолонокСписка() Экспорт
	...
	МетаданныеДокумента = Метаданные.Документы.ra_KorrektiruyushcheeDejstvie;
...
Процедура ДополнитьОписаниеМетаданных(ОбработкаОбъект, Данные, ПараметрыФормирования) Экспорт
	...
	ОбработкаОбъект.УстановитьДоступность(
		...
		|Opisanie,
		|Organizaciya,
		|Otvetstvennyj", Полномочия.ПервыйЛидер И Не РезультатыПроверки.Свойство("ra_OtchetONesootvetstviiCHast2"));
