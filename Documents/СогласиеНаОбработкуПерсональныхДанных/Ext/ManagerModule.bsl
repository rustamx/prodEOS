...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Согласие на обработку ПДн (табличный документ 1С:Предприятия)'; en = 'Consent on PD processing (1C:Enterprise spreadsheet document)'");
	
	// В формате Microsoft Word.
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "СогласиеНаОбработкуПерсональныхДанных(MSWord)";
	КомандаПечати.Представление = НСтр("ru = 'Согласие на обработку ПДн (документ Microsoft Word)'; en = 'Consent on PD processing (Microsoft Word document)'");
	...
	КомандаПечати.Представление = НСтр("ru = 'Согласие на обработку ПДн (документ OpenOffice.org Writer)'; en = 'Consent on PD processing (OpenOffice.org Writer document)'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		Шаблон = НСтр("ru = 'Согласие на обработку ПДн [Субъект] - [Организация] (№[Номер] получено [ДатаПолучения])'; en = 'Consent on PD processing [Субъект] - [Организация] (№[Номер] granted [ДатаПолучения])'");
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ru = 'Согласие на обработку ПДн'; en = 'Consent on PD processing'");
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru = 'Согласие на обработку ПДн (документ Microsoft Word)'; en = 'Consent on PD processing (Microsoft Word document)'"),
	...
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru = 'Согласие на обработку ПДн (документ OpenOffice.org Writer)'; en = 'Consent on PD processing (OpenOffice.org Writer document)'"),
			ПечатьСогласияНаОбработкуПДн(МассивОбъектов, ОбъектыПечати),
			,
...
Функция ДанныеДляПечатиСогласий(Согласия)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.СрокДействия) Тогда
			Описание.ПериодДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'с %1 по %2.'; en = 'from %1 to %2.'"), Формат(Выборка.ДатаСогласия, "ДЛФ=D"), Формат(Выборка.СрокДействия, "ДЛФ=D"));
		Иначе
			Описание.ПериодДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'с %1 бессрочно.'; en = 'from %1 indefinitely.'"), Формат(Выборка.ДатаСогласия, "ДЛФ=D"));
		КонецЕсли;
...
Функция ОписаниеСогласия()
	...
	ОписаниеСогласия.ДатаСогласия = НСтр("ru = '«____»_______________20___г.'; en = '«____»_______________20___.'");
	ОписаниеСогласия.Организация = НСтр("ru = '<Организация>'; en = '<Company>'");
	ОписаниеСогласия.АдресОрганизации = НСтр("ru = '<Адрес организации>'; en = '<Company address>'");
	ОписаниеСогласия.ОтветственныйЗаОбработкуПерсональныхДанных = НСтр("ru = '<ФИО ответственного лица>'; en = '<Responsible person name>'");
