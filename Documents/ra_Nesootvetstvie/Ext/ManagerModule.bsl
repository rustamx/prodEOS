...
Процедура ДобавитьУчастниковРабочейГруппыВНабор(ТаблицаНабора, Объект) Экспорт
	...
	Запрос.УстановитьПараметр("Предметы", ПроизводныеДокументыНесоответствия(Запрос.Параметры.Nesootvetstvie));
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ПерезаписатьРабочиеГруппыОбъектов(Ссылка, ОбновитьПраваДоступа) Экспорт
	...
	ДобавитьУчастниковРабочейГруппыВНабор(РабочаяГруппа, Ссылка);
	
	Для каждого Документ Из ПроизводныеДокументыНесоответствия(Ссылка) Цикл
...
Функция ПроизводныеДокументыНесоответствия(Ссылка, Подробно = Ложь) Экспорт
...
Функция ЭтоВыявившийНесоответствие(Несоответствие, УчитыватьАннулирование = Истина) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ra_Nesootvetstvie.VyyavivsheeLico КАК VyyavivsheeLico
	|ИЗ
	|	Документ.ra_Nesootvetstvie КАК ra_Nesootvetstvie
	|ГДЕ
	|	ra_Nesootvetstvie.Ссылка = &Несоответствие
...
Функция СписокВладельцевФайлов(Ссылка) Экспорт
	...
	ПроизводныеДокументы = ПроизводныеДокументыНесоответствия(Ссылка);
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		...
		Если Не Несоответствие.Пустая() Тогда
			...
			КнопкаАннулировать.Availability =
				ЭтоВыявившийНС И Не РезультатыПроверки.Свойство("ra_Uvedomlenie")
					Или (Полномочия.ОтветственныйЗаКачествоВыявившей Или Полномочия.ПервыйЛидер) И Не РезультатыПроверки.Свойство("ra_AktObUstraneniiNesootvetstviya");
		Иначе
...
Процедура ЗаполнитьГлавноеМенюОбъекта(ОбъектБД, МассивПунктовМеню) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ДокументыПоНесоответствию = ПроизводныеДокументыНесоответствия(ОбъектБД.Ссылка, Истина);
