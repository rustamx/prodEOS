...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если VidVremennyhSderzhivayushchihDejstvijIKorrekcij = 
		Справочники.ra_VidyPoruchenij.ПринятьКакЕсть Тогда
		
		Если НЕ ЗначениеЗаполнено(Otvetstvennyj) Тогда
			Otvetstvennyj = ПользователиКлиентСервер.ТекущийПользователь();
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(CelevojSrok) Тогда
			CelevojSrok = ТекущаяДатаСеанса();
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(FakticheskijSrok) Тогда
			FakticheskijSrok = ТекущаяДатаСеанса();
		КонецЕсли;		
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Документы.ra_VremennyeSderzhivayushchieDejstviyaIKorrekciya.АктуализироватьМассивОбязательныхРеквизитов(ПроверяемыеРеквизиты, ЭтотОбъект);
...
Процедура ПриЗаписи(Отказ)
	...
	Если VidVremennyhSderzhivayushchihDejstvijIKorrekcij = 
		...
		СостояниеДокумента = РегистрыСведений.ИсторияСостоянийДокументов.ПолучитьСостояниеДокумента(Ссылка);
		
		Если СостояниеДокумента <> Перечисления.СостоянияДокументов.ra_Принято Тогда
			Делопроизводство.ЗаписатьСостояниеДокумента(
				Ссылка,
				ТекущаяДата(),
				Перечисления.СостоянияДокументов.ra_Принято,
				ПараметрыСеанса.ТекущийПользователь);
		КонецЕсли;
