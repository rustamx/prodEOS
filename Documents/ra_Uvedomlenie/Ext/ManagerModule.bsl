...
Функция ПолучитьМассивКнопок(ДокументОбъект) Экспорт
	...
	Если ВидФормы = "ФормаСписка" Тогда
		...
		Если ДопустившаяОрганизация = Неопределено Тогда
			...
			КнопкаСоздатьУведомлениеСвояОрг.Availability = ЭтоВыявившийНС И Не РезультатыПроверки.Свойство("ra_Uvedomlenie") И НесоответствиеПроведен;
			КнопкаСоздатьУведомлениеСторОрг.Availability = ЭтоВыявившийНС И Не РезультатыПроверки.Свойство("ra_Uvedomlenie") И НесоответствиеПроведен;
			КнопкаСоздатьУведомлениеЗаказчику.Availability = ЭтоВыявившийНС И Не ЕстьУведомлениеПоставщику(Несоответствие) И НесоответствиеПроведен;
		Иначе
