...
Функция ПолучитьМассивКнопок(ДокументОбъект) Экспорт
	...
	Если ВидФормы = "ФормаСписка" Тогда
		...
		Если ДопустившаяОрганизация = Неопределено Тогда
			...
			КнопкаСоздатьУведомлениеСвояОрг.Availability = ЭтоВыявившийНС И Не РезультатыПроверки.Свойство("ra_Uvedomlenie");
			КнопкаСоздатьУведомлениеСторОрг.Availability = ЭтоВыявившийНС И Не РезультатыПроверки.Свойство("ra_Uvedomlenie");
			КнопкаСоздатьУведомлениеЗаказчику.Availability = ЭтоВыявившийНС И Не ЕстьУведомлениеПоставщику(Несоответствие);
		Иначе
			...
			КнопкаСоздатьУведомлениеСторОрг.Availability = Полномочия.Лидер;
			КнопкаСоздатьУведомлениеЗаказчику.Availability = Полномочия.Лидер И Не ЕстьУведомлениеПоставщику(Несоответствие);
		КонецЕсли;
	...
	ИначеЕсли ВидФормы = "ФормаОбъекта" Тогда
		...
		КонецЕсли;
		Кнопка.Availability = ЭтоОзК И Не ДокументАннулирован;
		Кнопка.Visibility = Кнопка.Availability;
		МассивКнопок.Добавить(Кнопка);
