...
Процедура ПриЗаписи(Отказ)
	...
	// ТСК Близнюк С.И.; 11.12.2018; task#2117{
		Если БизнесПроцессы.Ознакомление.ЕстьНеЗавершенныеБизнесПроцессыПоДокументуИсполнителюВидуСобытия(Ссылка, OtvetstvenniyZaKachestvo, Справочники.ВидыБизнесСобытий.ра_ОзнакомлениеССигналом) Тогда
			ВызватьИсключение(НСтр("ru = 'Уже есть задача на ознакомление получателю.'; en = 'There is already a task to familiarize the recipient.'"));  
		КонецЕсли;
		// ТСК Близнюк С.И.; 11.12.2018; task#2117}
