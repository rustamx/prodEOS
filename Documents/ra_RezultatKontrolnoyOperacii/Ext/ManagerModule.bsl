...
Функция ПоследнийРезультатКО(ЗаявкаКО, СоСтатусом = Истина) Экспорт
	
	СтруктураДанных = Новый Структура("Ссылка", Документы.ra_RezultatKontrolnoyOperacii.ПустаяСсылка());
	
	Если СоСтатусом Тогда
		СтруктураДанных.Вставить("Состояние", Перечисления.СостоянияДокументов.ПустаяСсылка());
	КонецЕсли;
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ra_RezultatKontrolnoyOperacii.Дата УБЫВ";
	
	Если СоСтатусом Тогда
		ТекстЗапроса = СтрШаблон(ТекстЗапроса,
			", ИсторияСостоянийДокументов.Состояние КАК Состояние",
		    "ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияСостоянийДокументов.СрезПоследних(, ВЫРАЗИТЬ(Документ КАК Документ.ra_RezultatKontrolnoyOperacii).ZayavkaNaKontrolnuyuOperaciyu = &ZayavkaNaKontrolnuyuOperaciyu) КАК ИсторияСостоянийДокументов
			|ПО ra_RezultatKontrolnoyOperacii.Ссылка = ИсторияСостоянийДокументов.Документ");
	Иначе
		ТекстЗапроса = СтрШаблон(ТекстЗапроса, "", "");
	КонецЕсли;
...
Функция ПолучитьМассивКнопок(ДокументОбъект) Экспорт
	...
	Если ВидФормы = "ФормаСписка" Тогда
		...
		РезультатСоСтатусом = ПоследнийРезультатКО(ЗаявкаКО);
		КнопкаЗарегистрироватьРезультат.Availability = РезультатСоСтатусом.Ссылка.Пустая()
			Или РезультатСоСтатусом.Состояние = Перечисления.СостоянияДокументов.НеСогласован;
		...
		Если НЕ РезультатСоСтатусом.Ссылка.Пустая() Тогда
		...
		Если РезультатСоСтатусом.Ссылка.Пустая() Тогда
