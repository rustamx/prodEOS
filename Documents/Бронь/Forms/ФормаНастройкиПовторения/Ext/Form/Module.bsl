...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Бронь.ТипЗаписи = Перечисления.ТипЗаписиКалендаря.ЭлементПовторяющегосяСобытия Тогда
		ТекстОшибки = НСтр("ru = 'Невозможно настроить повторение для исключения повторяющегося события.'; en = 'It is impossible to set up recurrence for recurrence exception.'");
...
Процедура ОбновитьОтображение()
	...
	Если Бронь.КоличествоПовторов = 1 Тогда
		КоличествоПовторовЕдиницаИзмерения = НСтр("ru = 'повтора'; en = 'occurence'");
	Иначе
		КоличествоПовторовЕдиницаИзмерения = НСтр("ru = 'повторов'; en = 'occurences'");
	КонецЕсли;
...
Функция ПроверитьКорректностьНастройкиПовторения()
	...
	Если Бронь.ЧастотаПовторения = ПредопределенноеЗначение("Перечисление.ЧастотаПовторения.Еженедельно")
		...
		ТекстПредупреждения = НСтр("ru = 'Не указаны дни повторения.'; en = 'The recurrence days are not specified.'");
