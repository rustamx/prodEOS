...
Процедура ДобавитьИсключениеПовторения(ДатаИсключения, ЗаписьИсключения = Неопределено) Экспорт
	...
	Если ТипЗаписи <> Перечисления.ТипЗаписиКалендаря.ПовторяющеесяСобытие Тогда
		ВызватьИсключение НСтр("ru = 'Некорректный тип повторяющейся брони.'; en = 'Invalid recurring reservation type.'");
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("СправочникСсылка.Мероприятия") Тогда
		...
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Бронь для мероприятия %1'; en = 'Reservation for the event %1'"), ДанныеЗаполнения);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ВместимостьПомещения) Тогда
		Если ЗначениеЗаполнено(КоличествоЧеловек) И КоличествоЧеловек > ВместимостьПомещения Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Количество человек (%1) превышает вместимость  помещения (%2).'; en = 'Number of persons (%1) exceeds the capacity of premises (%2).'"),
		...
		ИначеЕсли Не ЗначениеЗаполнено(КоличествоЧеловек) Тогда
			ТекстОшибки = НСтр("ru = 'Не указано количество человек.'; en = 'Unspecified number of people.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ПроверитьКорректностьДат()
	...
	Если ДатаНачала >= ДатаОкончания Тогда
		ТекстОшибки = НСтр("ru = 'Дата начала  не может быть меньше даты окончания.'; en = 'Start date cannot be less than end date.'");
...
Процедура ПроверитьПересечение()
	...
	Если Не ДоступноБронирование Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Помещение %1 недоступно для бронирования.'; en = 'Premises %1 may not be reserved.'"), Помещение);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если БронированиеПомещений.ЕстьПересекающиесяБрони(ЭтотОбъект, БроньИсключение, ДатаИсключения) Тогда
		ТекстОшибки = НСтр("ru = 'Невозможно забронировать, так как на данное время уже введена бронь.'; en = 'It is impossible to reserve as a reservation has already been specified on this time.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ЗаполнитьПовторение()
	...
	Если ДополнительныеСвойства.Свойство("НастройкиПовторения") Тогда
		...
		Если ПометкаУдаления = Истина Тогда
			Отказ = Истина;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Невозможно настроить повторение для брони, помеченной на удаление.'; en = 'It is impossible to set up recurrence for marked for deletion reservation.'"));
			Возврат;
		КонецЕсли;
		...
		Если ТипЗаписи = Перечисления.ТипЗаписиКалендаря.ЭлементПовторяющегосяСобытия Тогда
			Отказ = Истина;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Невозможно настроить повторение для брони, являющейся исключением повторения.'; en = 'It is impossible to set up recurrence for recurrence exception.'"));
			Возврат;
		КонецЕсли;
...
Процедура ПроверитьВводБрониОтветственным()
	...
	Если Не БронированиеПомещений.ДоступноБронированиеПомещения(Помещение) Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для ввода брони. Обратитесь к ответственному за помещение.'; en = 'Not enough permissions to enter the reservation. Please contact the responsible for the premises.'");
	КонецЕсли;
