...
Функция ПолучитьСтруктуруДанныхGetForm(Результат, ЗапросHTTP)
	...
	Если ПредставлениеОбъектаМетаданных = "Document_ra_RezultatKontrolnoyOperacii"
		...
		РезультатСсылка = Документы.ra_RezultatKontrolnoyOperacii.ДокументРезультатКОПоЗаявке(СтруктураОтбора.ZayavkaNaKontrolnuyuOperaciyu);
		
		Если НЕ РезультатСсылка.Пустая() Тогда
			ГУИДОбъекта = Строка(РезультатСсылка.УникальныйИдентификатор());
		КонецЕсли;
	КонецЕсли;
...
Функция ДобавитьПредставлениеДанныхВыбора(ПолноеИмяОбъекта, СписокРезультатов)
	...
	Для каждого ЭлементСписка из СписокРезультатов Цикл
		...
		МассивДанных.Добавить(Новый Структура("Ref_Key, FormattedView, Presentation", 
			...
			Строка(ЭлементСписка.Представление), 
	...
	Если ЧтениеXML.Имя = "p" Тогда
		...
		Пока ЧтениеXML.Прочитать() Цикл
			...
			Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				МассивСтрок.Добавить(СтрШаблон("<span style=""display: inline; %1"">", ЧтениеXML.ПолучитьАтрибут("style"))); 
			ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
				МассивСтрок.Добавить(XMLСтрока(ЧтениеXML.Значение));
			ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
				МассивСтрок.Добавить("</span>");
			КонецЕсли;
		КонецЦикла;
...
Функция ЗаполнитьИЗаписатьОбъект(ОбъектДанных, СтруктураСвойств, ИмяОбъектаМетаданных)
	...
	Если ЭтоДокумент И ОбъектМетаданных.Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить Тогда
		Если СтруктураСвойств.Свойство("Parameters") И ТипЗнч(СтруктураСвойств.Parameters) = Тип("Структура")
			И СтруктураСвойств.Parameters.Свойство("DocumentWriteMode") И СтруктураСвойств.Parameters.DocumentWriteMode = "Posting" Тогда
			РежимЗаписи = РежимЗаписиДокумента.Проведение;
			ПроверкаЗаполненияПройдена = ОбъектДанных.ПроверитьЗаполнение();
		Иначе
			РежимЗаписи = РежимЗаписиДокумента.Запись;
			ПроверкаЗаполненияПройдена = Истина;
		КонецЕсли;
	Иначе
		РежимЗаписи = РежимЗаписиДокумента.Запись;
		ПроверкаЗаполненияПройдена = ОбъектДанных.ПроверитьЗаполнение();
