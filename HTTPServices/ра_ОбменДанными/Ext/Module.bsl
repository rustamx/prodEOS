...
Функция ПолучитьМассивДанныхПолнотекстовогоПоиска(Результат, ЗапросHTTP)
	...
	Если Параметры <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыВыбора) Тогда
			...
			ИначеЕсли ПараметрыВыбора.Свойство("VidPredmetaNesootvetstviya") Тогда
				Если ЗначениеЗаполнено(ПараметрыВыбора.VidPredmetaNesootvetstviya) Тогда
					ПараметрыВыбора.Вставить("VidPredmetaNesootvetstviya", Перечисления.ra_VidyPredmetovNesootvetstviya[ПараметрыВыбора.VidPredmetaNesootvetstviya]);	
				Иначе
					ПараметрыВыбора.Вставить("VidPredmetaNesootvetstviya", Перечисления.ra_VidyPredmetovNesootvetstviya.ПустаяСсылка());
				КонецЕсли;
			КонецЕсли;
...
Функция СформироватьВариантОтчета(Результат, ЗапросHTTP)
	...
	Если НастройкиВарианта <> Неопределено Тогда
		...
		Если НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НастройкаВариантаОтчета,"ра_НеПередаватьГруппировки") Тогда
		// ТСК Близнюк С.И.; 15.01.2019; task#2087}
...
Процедура ЗаполнитьОбъектДанных(ОбъектДанных, СтруктураСвойств)
	...
	Для Каждого ЭлементСтруктуры ИЗ СтруктураСвойств Цикл
		...
		//{Муллазанов 15.01.19
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ОбъектДанных,ИмяРеквизита) Тогда
			Продолжить;
		КонецЕсли;
		//}Муллазанов 15.01.19
...
Функция ПолучитьЗначениеРеквизитаПоСтроковомуПредставлению(ОбъектМетаданных, ИмяРеквизита, СтроковоеПредставление)
	...
	Иначе //здесь будут исключения для гуидов, которые не являются реквизитами объекта
		...
		ИначеЕсли ИмяРеквизита = "ZayavkaNaKontrolnuyuOperaciyu" Тогда
			УникальныйИдентификатор = Новый УникальныйИдентификатор(СтроковоеПредставление);
			ЗначениеРеквизита = Документы.ra_ZayavkaNaKontrolnuyuOperaciyu.ПолучитьСсылку(УникальныйИдентификатор);
		КонецЕсли;
	КонецЕсли;
