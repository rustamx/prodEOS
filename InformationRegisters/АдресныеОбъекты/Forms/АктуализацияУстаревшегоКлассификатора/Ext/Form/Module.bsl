...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ru = 'Отказаться от обновления данных классификатора?'; en = 'Refuse to update the classifier data?'");
...
Процедура Отмена(Команда)
	...
	Если ЗначениеЗаполнено(ИдентификаторФоновогоЗадания) Тогда
		...
		ПоказатьОповещениеПользователя(,, НСтр("ru = 'Обновление адресного классификатора не завершено.'; en = 'Address classifier update is not completed.'"));
	КонецЕсли;
...
Процедура АктуализироватьАдресныйКлассификаторКлиент()
	...
	Иначе 
		Если Результат = "Выполнено" Тогда
			...
			ПоказатьОповещениеПользователя(,, НСтр("ru = 'Обновление адресного классификатора успешно завершено.'; en = 'Address classifier update completed successfully.'"));
		Иначе
			ПоказатьОповещениеПользователя(,, НСтр("ru = 'Обновление адресного классификатора не завершено.'; en = 'Address classifier update is not completed.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура Подключаемый_ПроверитьВыполнениеЗадания()
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ru = 'Обновление адресного классификатора'; en = 'Address classifier update'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			"Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), , Истина);
		ТекстОбОшибке = НСтр("ru = 'Обновление адресного классификатора прервана.|Подробности см. в журнале регистрации.'; en = 'Address classifier update was terminated.|For details see Events log.'");
	...
	Если ЗаданиеВыполнено Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ru = 'Обновление адресного классификатора'; en = 'Address classifier update'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			"Информация", НСтр("ru = 'Обновление адресного классификатора успешно завершено.'; en = 'Address classifier update completed successfully.'"), , Истина);
		ПоказатьОповещениеПользователя(,, НСтр("ru = 'Обновление адресного классификатора успешно завершено.'; en = 'Address classifier update completed successfully.'"));
		ОбновитьПовторноИспользуемыеЗначения(); 
		Оповестить("АдресныйКлассификаторАктуализирован", , ЭтотОбъект);
		ПодтверждениеЗакрытияФормы = Истина;
...
Процедура АктуализироватьАдресныйКлассификатор(Результат, ФоновоеЗадание = Ложь)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Подсистема Адресный классификатор: актуализация данных'; en = 'Address classifier subsystem: updating data'");
