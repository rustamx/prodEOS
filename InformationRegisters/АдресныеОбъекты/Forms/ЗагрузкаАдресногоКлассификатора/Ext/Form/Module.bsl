...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если КоличествоОбновлений > 0 Тогда
			...
			ИнформацияОбОбновление = НСтр("ru = 'Доступно обновление от'; en = 'Available update of'") + " " + ДатаОбновления;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("ru = 'Прервать загрузку адресного классификатора?'; en = 'Terminate importing address classifier?'");
...
Процедура АдресЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	АдресныйКлассификаторКлиент.ВыбратьКаталог(ЭтотОбъект, "АдресЗагрузки", 
		НСтр("ru = 'Каталог с файлами адресного классификатора'; en = 'Directory that contains address classifier files'"),
...
Процедура Загрузить(Команда)
	...
	Если КодИсточникаЗагрузки = "КАТАЛОГ" Тогда
		Текст = НСтр("ru = 'Для загрузки адресного классификатора из папки
		                   ...
		                   |en = 'To import address classifier from a folder it is necessary
		                   |to install the extension for working with files.'");
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю( НСтр("ru = 'Не указан вариант загрузки классификатора.'; en = 'Import option is not specified.'") );
	КонецЕсли;
...
Процедура ПослеПроверкиДоступностиОбновленияКлиент(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Даты.ДатаОбновления > Даты.ДатаСамойСтаройЗагрузки Тогда
			...
			ИнформацияОбОбновление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Доступно обновление от %1'; en = 'Available update of %1'"), ДатаОбновленияСтрокой);
			Элементы.ИнформацияОбОбновление.Видимость = НЕ Элементы.АвторизацияНаСайтеПоддержкиПользователей.Видимость;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗапуститьПроверкуДоступностиОбновления(УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Проверка обновления для адресного классификатора'; en = 'Chech for address classifier update'");
...
Процедура ОбновитьИнтерфейсПоКоличествуЗагружаемых()
	...
	// Страница загрузки
	ТекстОписанияЗагрузки = СтрШаблон(НСтр("ru = 'Загружаются данные по выбранным регионам (%1)'; en = 'Import data for selected regions (%1)'"), ВыбраноРегионовДляЗагрузки);
...
Процедура ЗагрузитьКлассификаторССайтаАутентификация(Знач Аутентификация, Знач КодыРегионов)
	...
	Элементы.ШагиЗагрузки.ТекущаяСтраница = Элементы.ОжиданиеЗагрузки;
	ТекстСостоянияЗагрузки = НСтр("ru = 'Загрузка файлов с сайта поддержки пользователей...'; en = 'Downloading files from customer support site...'");
...
Процедура ПослеФоновойЗагрузкиССайта(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Ошибка" Тогда
		Если СтрНайти(Задание.КраткоеПредставлениеОшибки, "404 Not Found") > 0 ИЛИ СтрНайти(Задание.КраткоеПредставлениеОшибки, "401 Unauthorized") > 0 Тогда
			ТекстОшибки = НСтр("ru = 'Не удается загрузить адресные сведения.'; en = 'Unable to import address information.'");
			ТекстОшибки = ТекстОшибки + НСтр("ru = 'Возможные причины:'; en = 'Possible causes:'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru = '• Некорректно введен или не введен логин и пароль;'; en = '• Login and password entered incorrectly or not entered;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru = '• Нет подключения к Интернету;'; en = '• No Internet connection;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru = '• На сайте ведутся технические работы. Попробуйте повторить загрузку позднее.'; en = '• Site is shut down for maintenance. Try to import later.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru = '• Брандмауэр или другое промежуточное ПО (антивирусы и т.п.) блокируют попытки программы подключиться к Интернету;'; en = '• Firewall or other middleware (anti-virus etc.) is blocking the program attempts to connect to the Internet;'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru = '• Подключение к Интернету выполняется через прокси-сервер, но его параметры не заданы в программе.'; en = '• Connection to the Internet is via a proxy server, but its parameters are not specified in the program.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru = 'Техническая информация:'; en = 'Technical information:'") + " " + Символы.ПС;
			ТекстОшибки = ТекстОшибки + СтрПолучитьСтроку(Задание.КраткоеПредставлениеОшибки, 1);
		Иначе
			ТекстОшибки = НСтр("ru = 'Вероятно в данный момент на сайте ведутся технические работы. Попробуйте повторить загрузку позднее.'; en = 'The site is probably under maintenance. Try the attempt later.'") + Символы.ПС;
			ТекстОшибки = ТекстОшибки + НСтр("ru = 'Техническая информация:'; en = 'Technical information:'") + " " + Символы.ПС;
...
Процедура ЗавершениеЗагрузки()
	...
	// Для сброса признака АдресныйКлассификаторУстарел в параметрах работы клиента.
		...
		ТекстОписанияЗагрузки = НСтр("ru = 'Адресный классификатор успешно загружен.'; en = 'Address classifier imported successfully.'");
		...
		Если ЕстьУстаревшиеАдресныеСведенияКЛАДРДляУдаления() Тогда
			...
			ТекстВопроса = НСтр("ru = 'Найдены устаревшие сведения о регионах в формате КЛАДР, которые не используются в программе.'; en = 'Found outdated kladr format region data that ARE not used in the program.'");
			ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ru = 'Если загруженных регионов много, то процесс очистки может занять продолжительное время.'; en = 'If a lot of regions were imported, the clearing process may take a long time.'");
			ТекстВопроса = ТекстВопроса + Символы.ПС + НСтр("ru = 'Очистить?'; en = 'Clear?'");
...
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиента(Знач ПараметрыЗагрузки)
	...
	Если ПараметрыЗагрузки = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекстСостоянияЗагрузки = НСтр("ru = 'Передача файлов на сервер приложения...'; en = 'Transferring files to the application server...'");
...
Процедура ПослеФоновойЗагрузкиИзКаталога(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Ошибка" Тогда
		ТекстОшибки = НСтр("ru = 'Не удается загрузить адресные сведения из файлов.'; en = 'Unable to import address information from files.'");
		ТекстОшибки = ТекстОшибки + НСтр("ru = 'Необходимо сохранить файлы с сайта «1С» http://its.1c.ru/download/fias на диск, а затем загрузить в программу.'; en = 'It is necessary to save files from 1C site http://its.1c.ru/download/fias to disk, and then import them into the program.'") + Символы.ПС;
		ТекстОшибки = ТекстОшибки + НСтр("ru = 'Техническая информация:'; en = 'Technical information:'") + Символы.ПС + Задание.КраткоеПредставлениеОшибки;
		ВывестиСообщениеОбОшибке(ТекстОшибки);
...
Функция ЗапуститьФоновуюЗагрузкуНаСервере(Знач КодыРегионов, Знач ОписаниеФайловЗагрузки, Знач Режим, Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Загрузка адресного классификатора'; en = 'Address classifier import'");
...
Процедура ОчиститьУстаревшиеАдресныеСведенияКЛАДР()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Очистка адресного классификатора в формате КЛАДР'; en = 'Clearing addres classifier in KLADR format'");
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Очистка адресного классификатора в формате КЛАДР'; en = 'Clearing addres classifier in KLADR format'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,
