...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.СпособУстановкиВремениНапоминания = ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноТекущегоВремени") Тогда
		СпособУстановкиВремениНапоминания = НСтр("ru = 'через'; en = 'in'") + " " + НапоминанияПользователяКлиентСервер.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания);
	Иначе
...
Процедура СпособУстановкиВремениНапоминанияПриИзменении(Элемент)
	...
	Если ИнтервалВремени > 0 Тогда
		...
		СпособУстановкиВремениНапоминания = НСтр("ru = 'через'; en = 'in'") + " " + ИнтервалВремениСтрокой;
	Иначе
		Если Элементы.СпособУстановкиВремениНапоминания.СписокВыбора.НайтиПоЗначению(СпособУстановкиВремениНапоминания) = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Интервал времени не определен.'; en = 'Time interval is not specified.'"), , "СпособУстановкиВремениНапоминания");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииИнтервалаВремени(Элемент)
	...
	Если Объект.ИнтервалВремениНапоминания > 0 Тогда
		ИнтервалВремениСтрокой = НапоминанияПользователяКлиентСервер.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания);
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Интервал времени не определен'; en = 'Time interval is not specified'"), , "ИнтервалВремениСтрокой");
	КонецЕсли;
КонецПроцедуры
...
Процедура Прекратить(Команда)
	...
	КнопкиДиалога.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Прекратить'; en = 'Stop'"));
	КнопкиДиалога.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru = 'Не прекращать'; en = 'Do not stop'"));
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПрекратитьНапоминание", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Прекратить напоминание?'; en = 'Stop the reminder?'"), КнопкиДиалога);
...
Функция ПолучитьПредопределенныеСпособыОповещения()
	...
	Результат.Вставить(НСтр("ru = 'относительно предмета'; en = 'Relative to time of subject'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноВремениПредмета"));
	Результат.Вставить(НСтр("ru = 'в указанное время'; en = 'at specified time'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ВУказанноеВремя"));
	Результат.Вставить(НСтр("ru = 'периодически'; en = 'periodically'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.Периодически"));
	Возврат Результат;
КонецФункции
...
Процедура ЗаполнитьСпособыОповещения()
	...
	Для Каждого Интервал Из ИнтервалыВремени Цикл
		СпособыОповещения.Добавить(НСтр("ru = 'через'; en = 'in'") + " " + Интервал);
...
Процедура ЗаполнитьВариантыПериодичности()
	...
	КонецЦикла;
	Элементы.ВариантПериодичности.СписокВыбора.Добавить("", НСтр("ru = 'по заданному расписанию...'; en = 'According to the specified schedule...'"));	
КонецПроцедуры
...
Процедура ОткрытьДиалогНастройкиРасписанияЗавершение(ВыбранноеРасписание, ДополнительныеПараметры) Экспорт
	...
	Если Не РасписаниеСоответствуетТребованиям(Расписание) Тогда 
		ПоказатьПредупреждение(, НСтр("ru = 'Периодичность в течение дня не поддерживается, соответствующие настройки очищены.'; en = 'Frequency during the day is not supported, the corresponding settings has been cleared.'"));
	КонецЕсли;
