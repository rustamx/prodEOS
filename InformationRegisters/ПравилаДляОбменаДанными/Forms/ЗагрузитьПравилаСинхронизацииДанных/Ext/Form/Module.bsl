...
Процедура ПриОткрытии(Отказ)
	...
	ШаблонПодсказки = НСтр("ru = 'Комплект правил можно скачать с %1
		|или найти в %2';
		|en = 'Rules set can be downloaded from %1
		|or found on %2'");
	
	ШаблонКаталогОбновлений = НСтр("ru = 'каталоге поставки программы ""%1""'; en = 'directory of program distributive ""%1""'");
	ШаблонКаталогОбновлений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКаталогОбновлений, ИмяПрограммы);
	
	ШаблонПользовательскийСайт = НСтр("ru = 'сайта поддержки пользователей системы ""1С:Предприятие 8""'; en = 'support site for customers of ""1C:Enterprise 8""'");
	...
	Если РежимОтладки Тогда
		...
		Если РежимОтладкиВыгрузки Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
				...
				СтрокаСообщения = НСтр("ru = 'Не задано имя файла внешней обработки.'; en = 'External data processor file name is not specified.'");
...
Процедура ВнешняяОбработкаДляОтладкиВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Фильтр", НСтр("ru = 'Внешняя обработка(*.epf)'; en = 'External data processor (*.epf)'") + "|*.epf" );
...
Процедура ФайлПротоколаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИзмененыНастройкиРежимаОтладки = Истина;
	НастройкиДиалога = Новый Структура;
	НастройкиДиалога.Вставить("Фильтр", НСтр("ru = 'Текстовый документ(*.txt)'; en = 'Text document (*.txt)'")+ "|*.txt" );
...
Процедура ВыгрузитьПравила(Команда)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ru = 'Укажите в какой файл выгрузить правила'; en = 'Specify file to export rules'") );
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Иначе
		Если ИсточникПравилКонвертации = ПредопределенноеЗначение("Перечисление.ИсточникиПравилДляОбменаДанными.МакетКонфигурации") Тогда
			...
			|en = 'Rules from file are not imported. Closing the form will result in use of the model rules.
			|Use the model conversion rules?'");
...
Процедура ВыполнитьЗагрузкуПравил(Знач АдресПомещенногоФайла, Знач ИмяФайла, ОписаниеОшибки = Неопределено)
	...
	Состояние(НСтр("ru = 'Выполняется загрузка правил в информационную базу...'; en = 'Importing rules into the infobase.'"));
	...
	Если ТипЗнч(ОписаниеОшибки) <> Тип("Булево") И ОписаниеОшибки <> Неопределено Тогда
		...
		ПараметрыФормы.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = НСтр("ru = 'Правила не могут быть загружены'; en = 'Cannot import rules'");
		Иначе
			ПараметрыФормы.Заголовок = НСтр("ru = 'Синхронизация данных может работать некорректно'; en = 'Data synchronization might be performed incorrectly'");
		КонецЕсли;
	...
	ИначеЕсли Отказ Тогда
		ТекстОшибки = НСтр("ru = 'В процессе загрузки правил были обнаружены ошибки.
			...
			|en = 'Errors occurred while importing rules.
			|Show event log?'");
	...
	Иначе
		ПоказатьОповещениеПользователя(,, НСтр("ru = 'Правила успешно загружены в информационную базу.'; en = 'Rules successfully imported into the infobase.'"));
		Закрыть();
	КонецЕсли;
...
Функция ПолучитьАдресВременногоХранилищаАрхиваПравилНаСервере()
	...
	Если Результат.Пустой() Тогда
		...
		НСтрока = НСтр("ru = 'Не удалось получить правила обмена.'; en = 'Cannot read exchange rules.'");
...
Процедура ОбновитьИнформациюОПравилах()
	...
	Если ИсточникПравил = "ЗагруженныеИзФайла" Тогда
		ИнформацияОбИспользовании = НСтр("ru = 'Используются правила загруженные из файла.'; en = 'Exchange rules imported from a file are used.'");
	Иначе
		ИнформацияОбИспользовании = НСтр("ru = 'Используются типовые правила из состава конфигурации.'; en = 'Default configuration exchange rules are used.'");
	КонецЕсли;
