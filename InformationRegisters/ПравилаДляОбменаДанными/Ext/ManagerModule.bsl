...
Процедура ЗагрузитьПоставляемыеПравила(ИмяПланаОбмена, ИмяФайлаПравил) Экспорт
	...
	Если ОбменДаннымиСервер.РаспаковатьZipФайл(ИмяФайлаПравил, ИмяВременнойПапки) Тогда
		...
		Если СписокРаспакованныхФайлов.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ru = 'При распаковке архива не найден файл с правилами.'; en = 'Rules file not found upon unpacking the archive.'");
		КонецЕсли;
		...
		Если СписокРаспакованныхФайлов.Количество() <> 3 Тогда
			...
			|en = 'Incorrect format of rules set. Expected number of files in the archive - 3. Expected files:
			|ExchangeRules.xml - conversion rules for the current program;
			|CorrespondentExchangeRules.xml - conversion rules for another program;
		...
		Для Каждого ПолученныйФайл Из СписокРаспакованныхФайлов Цикл
			...
			Иначе
				...
				|CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента;
				|RegistrationRules.xml - правила регистрации для текущей программы.';
				|en = 'Names of files in the archive are not expected. Expected files:
				|ExchangeRules.xml - conversion rules for the current program;
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru = 'Не удалось распаковать архив с правилами.'; en = 'Unable to decompress the archive with the rules.'");
	КонецЕсли;
	...
	Если ЗагрузкаПравилРегистрации.ФлагОшибки Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка при загрузке правил регистрации.'; en = 'Error importing registration rules'");
	КонецЕсли;
...
Процедура УдалитьПоставляемыеПравила(ИмяПланаОбмена) Экспорт
	...
	Для Каждого ВидПравил Из Перечисления.ВидыПравилДляОбменаДанными Цикл
		...
		Если ЕстьОшибки Тогда
			ВызватьИсключение НСтр("ru = 'Ошибка при загрузке правил из конфигурации.'; en = 'Error importing rules from configuration.'");
		Иначе
...
Процедура ЗагрузитьПравила(Отказ, Запись, АдресВременногоХранилища = "", ИмяФайлаПравил = "", ЭтоАрхив = Ложь) Экспорт
	...
	Если ЭтоАрхив Тогда
		...
		Если ОбменДаннымиСервер.РаспаковатьZipФайл(ИмяВременногоАрхива, ИмяВременнойПапки) Тогда
			...
			// В архиве не было ни одного файла - отказываемся от загрузки.
			Если СписокРаспакованныхФайлов.Количество() = 0 Тогда
				НСтрока = НСтр("ru = 'При распаковке архива не найден файл с правилами.'; en = 'Rules file not found upon unpacking the archive.'");
			...
			Если ЭтоПравилаКонвертации Тогда
				...
				ИначеЕсли СписокРаспакованныхФайлов.Количество() = 1 Тогда
					НСтрока = НСтр("ru = 'В архиве найден один файл правил конвертации. Ожидаемое количество файлов в архиве - два. Ожидаются файлы:
						|ExchangeRules.xml - правила конвертации для текущей программы;
						|CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента.';
						|en = 'One conversion rules file is found in the archive. The expected number of files in the archive is two. Expected are files:
						|ExchangeRules.xml - conversion rules for the current program;
						|CorrespondentExchangeRules.xml - conversion rules for the correspondent program.'");
					ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
				// В архиве оказалось несколько файлов, хотя должен быть один - отказываемся от загрузки.
				ИначеЕсли СписокРаспакованныхФайлов.Количество() > 1 Тогда
					НСтрока = НСтр("ru = 'При распаковке архива найдено несколько файлов. Должен быть только один файл с правилами.'; en = 'Multiple files found upon uncompressing the archive. There should be only one file with rules.'");
					ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
				КонецЕсли;
		...
		Иначе // Если не удалось распаковать файл - отказываемся от загрузки.
			НСтрока = НСтр("ru = 'Не удалось распаковать архив с правилами.'; en = 'Unable to decompress the archive with the rules.'");
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтрока, Отказ);
		КонецЕсли;
...
Функция ПолучитьДвоичныеДанныеИзМакетаКонфигурации(Отказ, ИмяПланаОбмена, ИмяМакета)
	...
	Исключение
		...
		СтрокаСообщения = НСтр("ru = 'Ошибка получения макета конфигурации %1 для плана обмена %2'; en = 'Error getting configuration template %1 for exchange plan %2'");
...
Процедура ВыполнитьПроверкуЗаполненияПолей(Отказ, Запись)
	...
	ИначеЕсли Запись.ИсточникПравил = Перечисления.ИсточникиПравилДляОбменаДанными.МакетКонфигурации
		...
		НСтрока = НСтр("ru = 'Укажите типовые правила.'; en = 'Specify the model rules.'");
...
Функция ИнформацияОПравилахИзФайла(ИмяФайлаПравил)
	...
	Если Не ((ПравилаОбмена.ЛокальноеИмя = "ПравилаОбмена") И (ПравилаОбмена.ТипУзла = ТипУзлаXML.НачалоЭлемента)) Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка формата правил обмена'; en = 'Exchange rule format error'");
	КонецЕсли;
...
Функция ПравилаКонвертацииСовместимыСТекущейВерсией(ИмяПланаОбмена, ОписаниеОшибки, ДанныеОПравилах)
	...
	Если РезультатСравнения <> 0 Тогда
		...
		Если РезультатСравнения < 0 Тогда
			...
			ТекстОшибки = НСтр("ru = 'Синхронизация данных может работать некорректно, так как загружаемые правила предназначены для предыдущей версии программы ""%1"" (%2). Рекомендуется использовать правила из конфигурации или загрузить комплект правил, предназначенный для текущей версии программы (%3).'; en = 'Data synchronization might be performed incorrectly because the rules being imported were designed for the previous version of program ""%1"" (%2). It is recommended to use the rules from the program or import the set of rules designed for the current version of the program (%3).'");
			ВидОшибки = "УстаревшаяВерсияКонфигурации";
			
		Иначе
			
			ТекстОшибки = НСтр("ru = 'Синхронизация данных может работать некорректно, так как загружаемые правила предназначены для более новой версии программы ""%1"" (%2). Рекомендуется обновить версию программы или использовать комплект правил, предназначенный для текущей версии программы (%3).'; en = 'Data synchronization might be performed incorrectly because the rules being imported were designed for the newer version of program ""%1"" (%2). It is recommended to update the program version or use the set of rules designed for the current version of the program (%3).'");
