...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И ИспользуетсяВерсионирование Тогда
		...
		Элементы.КоллизииАвторДругойВерсии.Заголовок = НСтр("ru = 'Версия получена из приложения'; en = 'Object version obtained from the application'");
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элемент.ТекущиеДанные.ДругаяВерсияПринята Тогда
			...
			ПричинаКонфликта = НСтр("ru = 'Конфликт был разрешен автоматически в пользу программы ""%1"".
				...
				|en = 'The conflict was resolved automatically in favour of program ""%1"". 
				|The version in this program replaced with the version from another program.'");
		...
		Иначе
			...
			ПричинаКонфликта =НСтр("ru = 'Конфликт был разрешен автоматически в пользу этой программы.
				...
				|en = 'The conflict was resolved automatically in favour of this program.
...
Процедура ПринятьВерсиюНепринятые(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Принять версию, несмотря на запрет загрузки?'; en = 'Do you want to accept the version even though import is prohibited?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ИзменитьРезультатКонфликта(Команда)
	...
	Если Элементы.Коллизии.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элементы.Коллизии.ТекущиеДанные.ДругаяВерсияПринята Тогда
			...
			ТекстВопроса = НСтр("ru = 'Заменить версию, полученную из другой программы, на версию из этой программы?'; en = 'Do you want to replace the object version from another application with the object version from this application?'");
			
		Иначе
			
			ТекстВопроса = НСтр("ru = 'Заменить версию этой программы на версию, полученную из другой программы?'; en = 'Do you want to replace the object version from this application with the object version from another application?'");
...
Процедура ЗаполнитьСписокУзлов()
	...
	Если КонтекстноеОткрытие И ОтсутствуетОбменПоПравилам Тогда
		Заголовок = НСтр("ru = 'Конфликты при синхронизации данных'; en = 'Data synchronization conflicts'");
...
Процедура ПоказатьОтличия(Элемент)
	...
	Если СравниваемыеВерсии.Количество() <> 2 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Нет версии для сравнения.'; en = 'No object version to compare.'"));
		Возврат;
...
Процедура ОбновитьЗаголовкиСтраниц()
	...
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНепроведенныеДокументы, НСтр("ru= 'Непроведенные документы'; en = 'Unposted documents'"), КоличествоНепроведенныхДокументов());
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНезаполненныеРеквизиты, НСтр("ru= 'Незаполненные реквизиты'; en = 'Blank attributes'"), КоличествоНезаполненныхРеквизитов());
	
	Если ИспользуетсяВерсионирование Тогда
		УстановитьЗаголовокСтраницы(Элементы.СтраницаКоллизии, НСтр("ru= 'Конфликты'; en = 'Conflicts'"), КоличествоКоллизий());
		УстановитьЗаголовокСтраницы(Элементы.СтраницаНепринятыеПоДатеЗапрета, НСтр("ru= 'Непринятые по дате запрета'; en = 'Declined due to date'"), КоличествоНепринятых());
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Удалена'; en = 'Deleted'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Отсутствуют'; en = 'Missing'"));
