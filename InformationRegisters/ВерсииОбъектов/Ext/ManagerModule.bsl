	...
	Для Каждого текСтрока Из ТаблицаИзмененийТабличныхДокументов Цикл
		...
		Для Индекс = 0 По ВГраница Цикл
			...
			Если ТекСтрока[ИмяКолонки] = "И" Тогда
				...
				ШаблонТекста = НСтр("ru='сравнить версию №%1 с версией №%2'; en = 'compare version №%1 with version №%2'");
			...
			ИначеЕсли текСтрока[ИмяКолонки] = "У" Тогда
				...
				Область.Текст = НСтр("ru='Сохранение изменений табличного документа отключено'; en = 'Saving changes to the table document is disabled'");
				
			ИначеЕсли текСтрока[ИмяКолонки] = "Д" Тогда
				Область = ТЧОтчета.Присоединить(ШаблонЯчейкаДобавлена);
				Область.Текст = НСтр("ru='открыть'; en = 'Open'");
...
Функция ВывестиШапку(ТЧОтчета, СписокВерсий, ЧислоВерсий, ОбщийШаблон, СсылкаНаОбъект)
	...
	ОбластьШапка.Параметры.НаименованиеОтчета = НСтр("ru = 'Отчет по изменениям версий объекта'; en = 'Report on object versions change'");
...
Функция ПолучитьОписаниеПоВерсии(СсылкаНаОбъект, НомерВерсии)
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '№ %1 / (%2) / %3'; en = '№ %1 / (%2) / %3'"), НомерВерсии.Представление, 
...
Процедура ДобавитьНомераСтрокВТабличныеЧасти(ТабличныеЧасти)
	...
	Для Каждого Соответствие Из ТабличныеЧасти Цикл
		...
		Если Таблица.Колонки.Найти("НомерСтроки") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Таблица.Колонки.Вставить(0, "НомерСтроки",,НСтр("ru = '№ строки'; en = 'row №'"));
