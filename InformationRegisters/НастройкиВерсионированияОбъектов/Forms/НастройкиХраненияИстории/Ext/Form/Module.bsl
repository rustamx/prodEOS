...
Процедура УстановитьВариантВерсионированияПриПроведении(Команда)
	...
	Если ВыбранТипДокументовБезВозможностиПроведения() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Документам, которые не могут быть проведены, установлен режим версионирования ""Версионировать при записи"".'; en = 'For documents that cannot be posted versioning mode ""Version when saving"" is set.'"));
	КонецЕсли;
	...
	Для Каждого Тип Из МассивТипов Цикл
		Если ВсеСправочники.СодержитТип(Тип) Тогда
			Если УзелСправочники = НеОпределено Тогда
				...
				УзелСправочники.СинонимНаименованияОбъекта = НСтр("ru = 'Справочники'; en = 'Catalogs'");
		...
		ИначеЕсли ВсеДокументы.СодержитТип(Тип) Тогда
			Если УзелДокументы = НеОпределено Тогда
				...
				УзелДокументы.СинонимНаименованияОбъекта = НСтр("ru = 'Документы'; en = 'Documents'");
		...
		ИначеЕсли БизнесПроцессы.ТипВсеСсылки().СодержитТип(Тип) Тогда
			Если УзелБизнесПроцессы = Неопределено Тогда
				...
				УзелБизнесПроцессы.СинонимНаименованияОбъекта = НСтр("ru = 'Бизнес-процессы'; en = 'Business processes'");
...
Процедура ЗапуститьРегламентноеЗадание()
	...
	Иначе
		...
		ПараметрыЗадания.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Запуск вручную: %1'; en = 'Manual start: %1'"), РегламентноеЗаданиеМетаданные.Синоним);
...
Процедура ПриЗавершенииОперацииПоискаУстаревшихВерсий(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("ru = 'Поиск устаревших версий объектов'; en = 'Search outdated object versions'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ТекстСостоянияПодсчет()
	Возврат НСтр("ru = 'Поиск устаревших версий...'; en = 'Search for outdated versions...'");
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТекстСостоянияОчистка()
	Возврат НСтр("ru = 'Выполняется очистка устаревших версий...'; en = 'Purging outdated versions...'");
КонецФункции
...
Функция ВыполнитьПоискУстаревшихВерсий()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Поиск устаревших версий объектов'; en = 'Search outdated object versions'");
...
Процедура ВывестиИнформациюОбУстаревшихВерсиях()
	...
	Если ИнформацияОбУстаревшихВерсиях.РазмерДанных > 0 Тогда
		Элементы.ИнформацияОбУстаревшихВерсиях.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Всего устаревших версий: %1 (%2)'; en = 'Total outdated versions: %1 (%2)'"),
			ИнформацияОбУстаревшихВерсиях.КоличествоВерсий,
			ИнформацияОбУстаревшихВерсиях.РазмерДанныхСтрокой);
	Иначе
		Элементы.ИнформацияОбУстаревшихВерсиях.Заголовок = НСтр("ru = 'Всего устаревших версий: нет'; en = 'Total outdated versions: no'");
	КонецЕсли;
