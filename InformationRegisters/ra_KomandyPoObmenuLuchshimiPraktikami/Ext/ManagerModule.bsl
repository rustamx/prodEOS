...
Функция СоздатьСтартоватьПроцессНаОзнакомление(Исполнитель, Предемет,  ЭтоКуратор) Экспорт
	
	СтруктураПараметров = Справочники.ШаблоныОзнакомления.ПолучитьСтруктуруШаблонаОзнакомление();
	
	ЗаполнитьЗначенияСвойств(СтруктураПараметров, Справочники.ШаблоныОзнакомления.ОзнакомлениеКуратораОбменаЛучшимиПрактиками);
	
	СтруктураПараметров.Вставить("СрокИсполненияПроцесса", ТекущаяДатаСеанса() + (60 * 60 * 24) * 2);
	
	СтруктураПараметров.Автор = ПараметрыСеанса.АвторизованныйПользователь;
	
	Предметы = Новый ТаблицаЗначений;
	Предметы.Колонки.Добавить("Предмет");
	НоваяСтрока = Предметы.Добавить();
	НоваяСтрока.Предмет = Предемет;
	
	СтруктураПараметров.Вставить("Предметы", Предметы);
	...
	НоваяСтрока.СрокИсполнения = ТекущаяДатаСеанса() + (60 * 60 * 24) * 2;
	
	СтруктураПараметров.Вставить("Исполнители", Исполнители);
	
	Если ЭтоКуратор Тогда 
	    ТекстНаименования = НСтр("ru = 'Вы назначены куратором по документу '; en = 'You are designated by the curator of the document '");
		СтруктураПараметров.Вставить("Шаблон", Справочники.ШаблоныОзнакомления.ОзнакомлениеКуратораОбменаЛучшимиПрактиками);
	Иначе
		ТекстНаименования = НСтр("ru = 'Вы назначены получателем итогового отчета по документу '; en = 'You are designated as the recipient of the final report of the document '");	
		СтруктураПараметров.Вставить("Шаблон", Справочники.ШаблоныОзнакомления.ОзнакомлениеКуратораОбменаЛучшимиПрактиками);
	КонецЕсли;
	
	СтруктураПараметров.Вставить("Наименование", ТекстНаименования + Предемет);
	
	БизнесПроцессы.Ознакомление.СоздатьПроцесс(СтруктураПараметров , Истина);
...
Функция ПолучитьМассивКнопок(МенеджерЗаписи) Экспорт
	...
	Если ВидФормы = "ФормаСписка" Тогда
		...
		КнопкаДобавитьКуратора.Hint = НСтр("ru = 'Пользователь ответственный за распространение итогового отчета'; en = 'User responsible for distributing the final report'");
		
		ИмяКнопки = "AddResponsible";
		ОписаниеКнопки = НСтр("ru = 'Добавить получателя итогового отчета'; en = 'Add recipient of the final report'");
