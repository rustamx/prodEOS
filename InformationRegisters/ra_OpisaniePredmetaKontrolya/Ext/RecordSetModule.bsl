...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	РегистрыСведений.ra_OpisaniePredmetaKontrolya.АктуализироватьМассивОбязательныхРеквизитов(ПроверяемыеРеквизиты, ЭтотОбъект[0]);
...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	//Перед записью заполняется KlassifikatorMTR из NaimenovanieOborudovaniya
	МассивСсылокДо = ЭтотОбъект.ВыгрузитьКолонку("NaimenovanieOborudovaniya");
	МассивСсылокПосле = Новый Массив;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивСсылокПосле, МассивСсылокДо, Истина);
	ИндексЭлемента = МассивСсылокПосле.Найти(Справочники.ra_MTRiO.ПустаяСсылка());
	Если ИндексЭлемента <> Неопределено Тогда
		МассивСсылокПосле.Удалить(ИндексЭлемента);
	КонецЕсли;
	
	Если МассивСсылокПосле.Количество() Тогда
		
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
			МассивСсылокПосле, "KlassifikatorMTR");
			
		Для Каждого Запись Из ЭтотОбъект Цикл
			Если Не ЗначениеЗаполнено(Запись.KlassifikatorMTRiO) И ЗначениеЗаполнено(Запись.NaimenovanieOborudovaniya) Тогда
				Запись.KlassifikatorMTRiO = ЗначенияРеквизитов.Получить(Запись.NaimenovanieOborudovaniya);
			КонецЕсли;
		КонецЦикла;
