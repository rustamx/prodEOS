	 ...
	 Или Параметры.ВерсияКонфигурации <> Метаданные.Версия Тогда
		ТекстОшибки =
			НСтр("ru = 'Не удалось обновить все параметры работы расширений, так как
			           ...
			           |en = 'Could not update all extension parameters as configuration name of version
			           |were changed. One should restart the session.'");
	КонецЕсли;
	...
	Если Параметры.УстановленныеРасширения <> ПараметрыСеанса.УстановленныеРасширения Тогда
		ТекстОшибки =
			НСтр("ru = 'Не удалось обновить все параметры работы расширений,
			           ...
			           |en = 'Could not update all extension parameters as the specified content 
			           |of extensions differs from the current one.'");
	КонецЕсли;
