...
Процедура ПодготовитьДанныеДляЗаписи() Экспорт
	...
	Если Не ДополнительныеСвойства.Свойство("ПараметрыПолучения", ПараметрыПолучения) Тогда
		ВызватьИсключение НСтр("ru = 'Не определены параметры получения данных'; en = 'Data retrieval options are not defined'");
	КонецЕсли;
...
Процедура ОшибкаКонтроля()
	...
	ВызватьИсключение НСтр("ru = 'Недопустимое изменение ресурса Данные записи регистра сведений КэшПрограммныхИнтерфейсов
                            |внутри транзакции записи из сеанса с включенным разделением!'; en = 'Invalid modification of resource Данные of the record of information register КэшПрограммныхИнтерфейсов within a transaction started in a session with separation enabled!'");
