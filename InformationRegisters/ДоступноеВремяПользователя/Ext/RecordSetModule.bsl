...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Если Не ЗначениеЗаполнено(Пользователь) Тогда
		ВызватьИсключение НСтр("ru = 'Запрещено записывать набор регистра сведений ""Доступное время пользователя"" без установки отбора по пользователю.'; en = 'It is forbidden to write informaton regirster ""User''s free time"" record set without filter by user.'");
	КонецЕсли;
		...
		Если Запись.ВремяОкончания < Запись.ВремяНачала Тогда
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Время окончания не может быть меньше времени окончания доступности:
					|%1';
					|en = 'End time cannot be less than availability end time:
					|%1'"),
