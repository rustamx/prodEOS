...
Функция ЕстьНеЗавершенныеБизнесПроцессыПоДокументуИсполнителюВидуСобытия(Объект, Исполнитель, ВидСобытия) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ОзнакомлениеИсполнители.Ссылка КАК Ссылка
	|ИЗ
	|	БизнесПроцесс.Ознакомление.Исполнители КАК ОзнакомлениеИсполнители
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ БизнесПроцесс.Ознакомление.Предметы КАК ОзнакомлениеПредметы
	|		ПО ОзнакомлениеИсполнители.Ссылка = ОзнакомлениеПредметы.Ссылка
	|ГДЕ
	|	ОзнакомлениеИсполнители.Исполнитель = &Исполнитель
	|	И ОзнакомлениеПредметы.Предмет = &Предмет
	|	И НЕ ОзнакомлениеИсполнители.Ссылка.Завершен
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
