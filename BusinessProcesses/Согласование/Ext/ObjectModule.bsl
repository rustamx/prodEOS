...
Процедура ОбработкаРезультатаОбработка(ТочкаМаршрутаБизнесПроцесса)
	...
	Для Каждого Предмет Из ОбрабатываемыеДокументы Цикл
		...
		// после согласования документа "Заявка на контрольную операцию" необходимо отправить задачу
		// Руководителю контролера о назначении Контролера и Длительности
		Если СостояниеДокумента = Перечисления.СостоянияДокументов.ra_Подтвержден И
				ТипЗнч(Предмет) = Тип("ДокументСсылка.ra_ZayavkaNaKontrolnuyuOperaciyu") Тогда
				
			Kontroler = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Предмет, "Kontroler");
			
			Если Не ЗначениеЗаполнено(Kontroler) Тогда
				БизнесСобытияВызовСервера.ЗарегистрироватьСобытие(
					Ссылка, 
					Справочники.ВидыБизнесСобытий.ра_НазначениеКонтролера);
			КонецЕсли;
		КонецЕсли;
		// ТСК Шепелев М.Н. 17.12.2018 }
