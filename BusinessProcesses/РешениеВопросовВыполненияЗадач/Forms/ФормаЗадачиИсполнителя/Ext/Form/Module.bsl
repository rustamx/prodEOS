...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.БизнесПроцесс.ВидВопроса = Перечисления.ВидыВопросовВыполненияЗадач.ПереносСрока Тогда
		...
		Если КоличествоПереносов = 0 Тогда
			РезультатРассмотрения = НСтр("ru = 'Перенос срока не согласован.'; en = 'Postponement is not approved.'");
			Элементы.РезультатРассмотрения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
		Иначе
			РезультатРассмотрения = НСтр("ru = 'Автор задачи перенес срок.'; en = 'Task author postponed the task.'");
		КонецЕсли;
	Иначе
