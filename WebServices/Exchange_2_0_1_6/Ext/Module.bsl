...
Функция СоздатьОбменДанными(ИмяПланаОбмена, СтрокаПараметров, НастройкаОтборовXDTO, ЗначенияПоУмолчаниюXDTO)
	...
	Если Отказ Тогда
		Сообщение = НСтр("ru = 'При создании настройки обмена во второй информационной базе возникли ошибки: %1'; en = 'Errors occurred in the second infobase during the data exchange setup: %1'");
...
Функция GetFilePart(TransferId, PartNumber, PartData)
	...
	Если ИменаФайлов.Количество() = 0 Тогда
		...
		ШаблонСообщения = НСтр("ru = 'Не найден фрагмент %1 сессии передачи с идентификатором %2'; en = 'Volume %1 is not found in the transfer session with the following ID: %2'");
	...
	ИначеЕсли ИменаФайлов.Количество() > 1 Тогда
		...
		ШаблонСообщения = НСтр("ru = 'Найдено несколько фрагментов %1 сессии передачи с идентификатором %2'; en = 'Multiple instances of volume %1 are found in the transfer session with the following ID: %2'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Строка(PartNumber), Строка(TransferId));
		ВызватьИсключение(ТекстСообщения);
	...
	Для НомерЧасти = 1 По PartQuantity Цикл
		...
		Если НайтиФайлы(ИмяФайла).Количество() = 0 Тогда
			ШаблонСообщения = НСтр("ru = 'Не найден фрагмент %1 сессии передачи с идентификатором %2.
					...
					|en = 'Volume %1 is not found in the transfer session with the following ID: %2. 
					|You must ensure that parameters ""Temporary files directory for Linux"" and 
					|""Temporary files directory for Windows"" are specified in the program settings.'");
...
Функция TestConnection(ИмяПланаОбмена, КодУзла, Результат)
	...
	Если ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(КодУзла).Пустая() Тогда
		Результат = НСтр("ru = 'Возможность синхронизации данных отключена администратором приложения в Интернете.'; en = 'Data synchronization is disabled by the administrator.'");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления()
	...
	Если ЗначениеЗаполнено(ОбновлениеИнформационнойБазыСлужебный.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
		...
		ВызватьИсключение НСтр("ru = 'Синхронизация данных временно недоступна в связи с обновлением приложения в Интернете.'; en = 'Data synchronization is unavailable for the duration of Internet-based update.'");
...
Процедура ВыполнитьВыгрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	Если ФоновыеЗадания.ПолучитьФоновыеЗадания (Отбор).Количество() = 1 Тогда
		ВызватьИсключение НСтр("ru = 'Синхронизация данных уже выполняется.'; en = 'Synchronization is already in progress.'");
	КонецЕсли;
	...
	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ОбменДаннымиСервер.ВыполнитьВыгрузкуДляУзлаИнформационнойБазыВСервисПередачиФайлов",
										Параметры,
										КлючФоновогоЗадания,
										НСтр("ru = 'Выполнение обмена данными через веб-сервис.'; en = 'Data exchange through a web service.'"));
	...
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Завершено Тогда
		...
		ВызватьИсключение НСтр("ru = 'Ошибка при выгрузке данных через веб-сервис.'; en = 'An error occurred during the data export through the web service.'");
	КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Завершено Тогда
		...
		ВызватьИсключение НСтр("ru = 'Ошибка при загрузке данных через веб-сервис.'; en = 'An error occurred during the data import through the web service.'");
	КонецЕсли;
...
Функция КлючФоновогоЗаданияВыгрузкиЗагрузкиДанных(ПланОбмена, КодУзла)
	...
	Ключ = НСтр("ru = 'ПланОбмена:[ПланОбмена] КодУзла:[КодУзла]'; en = 'ПланОбмена:[ПланОбмена] КодУзла:[КодУзла]'");
...
Функция ЗарегистрироватьДанныеДляНачальнойВыгрузки(Знач ИмяПланаОбмена, Знач КодУзла, ДлительнаяОперация, ИдентификаторОперации, ТолькоСправочники)
	...
	Иначе
		...
		ФоновоеЗадание = ФоновыеЗадания.Выполнить(ИмяМетода, Параметры,, НСтр("ru = 'Создание обмена данными.'; en = 'Data exchange setup.'"));
