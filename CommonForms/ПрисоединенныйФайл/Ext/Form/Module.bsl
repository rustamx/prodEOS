		...
		Если ЭтотОбъект.Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр(
				"ru = 'Для выполнения действия требуется записать изменения файла.
				      ...
				      |""%1""?';
				      |en = 'To perform the action one should save the file changes.
				      |Save changes and remove the deletion mark from ""%1"" file?'");
		Иначе
			ТекстВопроса = НСтр(
				"ru = 'Для выполнения действия требуется записать изменения файла.
				      ...
				      |""%1""?';
				      |en = 'To perform the action one should save the file changes.
				      |Save changes and mark the ""%1"" file for deletion?'");
		КонецЕсли;
	Иначе
		Если ЭтотОбъект.Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("ru = 'Снять пометку на удаление с файла
			                          |""%1""?';
			                          |en = 'Remove deletion mark from file
			                          |""%1""?'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Пометить на удаление файл
			                          |""%1""?'; en = 'Mark to delete file ""%1""?'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СтандартнаяПеречитать(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Данные изменены. Перечитать данные?'; en = 'Data was changed. Reread the data?'");
...
Процедура УдалитьЭП(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Удалить выделенные подписи?'; en = 'Delete the selected signatures?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 (Присоединенный файл)'; en = '%1 (Attached file)'"), Строка(ЭтотОбъект.Объект.Ссылка));
	Иначе
		Заголовок = НСтр("ru = 'Присоединенный файл (Создание)'; en = 'Attached file (Create)'")
	КонецЕсли;
...
Процедура ЗаполнитьСписокШифрования()
	...
	ТекстЗаголовка = НСтр("ru = 'Разрешено расшифровывать'; en = 'Allowed to decrypt'");
...
Процедура ЗаполнитьСписокПодписей()
	...
	ТекстЗаголовка = НСтр("ru = 'Электронные подписи'; en = 'Digital signatures'");
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Если ПустаяСтрока(ЭтотОбъект.Объект.Наименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Для продолжения укажите имя файла.'; en = 'To continue, specify a file name.'"), , "Наименование", "Объект");
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Исключение
		Если ТранзакцияАктивна Тогда
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Ошибка записи присоединенного файла'; en = 'Файлы.Ошибка записи присоединенного файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
