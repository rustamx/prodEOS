	...
	Если Не РазделениеВключено И Не ИзменилосьРазделение Тогда
		...
		ЗаголовокНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Размещение информационной базы изменилось с
				...
				|
				|%3';
				|en = 'Infobase location changed from 
				|<b>%1</b> 
				|to 
				|<b>%2</b>
				|
				|<a href = ""ЖурналРегистрации"">Technical information about the reason for the lock</a>
				|* If you moved the infobase, click <b>Infobase moved</b>. 
				|* If this is a copy of the information base, then <b>This is a copy of information base</b>. 
				|In this case work with all external resources (data synchronization, mail sending, etc.) that is performed on a schedule will be locked to prevent conflicts with initial base. 
				|
	...
	ИначеЕсли Не РазделениеВключено И ИзменилосьРазделение Тогда
		ЗаголовокНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Информационная база была загружена из приложения в Интернете
				...
				|
				|%1';
				|en = 'Infobase was loaded from an application on the Internet 
				|
				|• If the information will be used to keep a record, click <b>Infobase moved</b>. 
				|• If you select <b>This is a copy of the information base</b> work with all external resources
				|(data synchronization, sending mail, etc.) executed on schedule,
				|It will be blocked to prevent conflicts with an application on the Internet.
				|
	...
	ИначеЕсли РазделениеВключено И Не ИзменилосьРазделение Тогда
		ЗаголовокНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр(" ru = 'Приложение было перемещено
				...
				|
				|%1';
				|en = 'The application was moved 
				|
				|• If application will be used for accounting, click <b>Application moved</b>.
				|• When choosing a version <b>Is a copy of the application</b> work with all external resources (data synchronization, sending mail, etc.) that is performed on a schedule will be locked to prevent conflicts with an application on the Internet.
				|
				|%1'"), ТекстСнятияБлокировки);
		Элементы.ИнформационнаяБазаПеремещена.Заголовок = НСтр("ru = 'Приложение перемещено'; en = 'Application moved'");
		Элементы.ЭтоКопияИнформационнойБазы.Заголовок = НСтр("ru = 'Это копия приложения'; en = 'This is a copy of the application'");
		Заголовок = НСтр("ru = 'Приложение было перемещено или восстановлено из резервной копии'; en = 'Application was moved or restored from backup'");
	Иначе // Если РазделениеВключено И ИзменилосьРазделение
		ЗаголовокНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр(" ru = 'Приложение было загружено из локальной версии
				...
				|
				|%1';
				|en = 'The application was loaded from a local version
				|
				|• If the information will be used to keep a record, click <b>Infobase moved</b>. 
				|• If you select <b>This is a copy of the information base</b> work with all external resources 
				|(data synchronization, sending mail, etc.) executed on schedule, 
				|It will be blocked to prevent conflicts with local version.
				|
				|%1'"), ТекстСнятияБлокировки);
		Элементы.ИнформационнаяБазаПеремещена.Заголовок = НСтр("ru = 'Приложение перемещено'; en = 'Application moved'");
		Элементы.ЭтоКопияИнформационнойБазы.Заголовок = НСтр("ru = 'Это копия приложения'; en = 'This is a copy of the application'");
...
Процедура НадписьПредупреждениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("СобытиеЖурналаРегистрации",
		НСтр("ru = 'Работа с внешними ресурсами заблокирована'; en = 'Access to external resources is locked'",
