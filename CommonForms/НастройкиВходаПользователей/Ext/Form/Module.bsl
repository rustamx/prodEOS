...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПоказатьНастройкиВнешнихПользователей Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru = 'Настройки входа внешних пользователей'; en = 'External user login settings'");
	...
	Если ПолучитьПроверкуСложностиПаролейПользователей() Тогда
		ТекстВопросаОбОчисткеНастроекКонфигуратора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Установлена минимальная длина паролей %1 и требование сложности паролей в конфигураторе
			           ...
			           |en = 'Minimum password length %1 and the requirement of password complexity set in the configurator in ""Administration"" - ""Infobase parameters ..."".
			           |Requires to clear the minimum length and the requirement of password complexity specified in the configurator to correctly use the login settings.'"),
	...
	ИначеЕсли ПолучитьМинимальнуюДлинуПаролейПользователей() > 0 Тогда
		ТекстВопросаОбОчисткеНастроекКонфигуратора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Установлена минимальная длина паролей %1 в конфигураторе
			           ...
			           |чтобы корректно использовать настройки входа.';
			           |en = 'Minimum password length %1 is set in the configurator in ""Administration"" - ""Infobase parameters ..."".
			           |
			           |Requires to clear the minimum length specified in the configurator to correctly use the login settings.'"),
			ПолучитьМинимальнуюДлинуПаролейПользователей());
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ТекстВопросаОбОчисткеНастроекКонфигуратора) Тогда
		...
		Кнопки.Добавить("Очистить", НСтр("ru = 'Очистить'; en = 'Clear'"));
