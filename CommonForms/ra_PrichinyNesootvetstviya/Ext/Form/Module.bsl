...
Процедура ЗаполнитьДеревоПричинНаСервере()
	...
	ВременноеДерево = РегистрыСведений.ra_PrichinyNesootvetstvij.ПрочитатьДеревоПричинНаСервере(Несоответствие, 0);
	
	КодПричины = 0;
	
	ДеревоПричин.ПолучитьЭлементы().Очистить();
	
	ДобавитьСтрокиВДеревоПричинНаСервере(ВременноеДерево, ДеревоПричин, 0, Ложь);
