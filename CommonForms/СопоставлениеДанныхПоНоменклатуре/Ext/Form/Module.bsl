...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
		ТекстСообщения = НСтр("ru = 'Не выбран электронный документ'; en = 'Electronic document is not selected'");
...
Процедура ПрочитатьНоменклатуруКонтрагента(Команда)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОбработчикОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,, НСтр("ru = 'Заполнение списка номенклатуры'; en = 'Filling in list of products and services'"));
...
Процедура СохранитьДанныеКаталога(СсылкаНаЭД)
	...
	Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
		...
		Если ИмяФайла = Неопределено Тогда
			ТекстОшибки = НСтр("ru = 'Не удалось просмотреть электронный документ. Проверьте настройку рабочего каталога'; en = 'Unable to view the electronic document. Check the configuration of the working directory'");
				...
				Если ДопДанныеЭД.Свойство("СсылкаНаДвоичныеДанныеФайла", СсылкаНаДДДопДанныхЭД)
					...
					Если ИмяФайлаДопДанных = Неопределено Тогда
						ТекстОшибки = НСтр("ru = 'Не удалось получить доп. данные электронного документа.
												...
												|en = 'Could not retrieve additional data of the electronic document.
												|Check settings of the working directory'");
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
						Возврат;
					КонецЕсли;
		...
		Если СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
			...
			Исключение
				...
				КонецЕсли;
				Операция = НСтр("ru = 'Распаковка ЭД'; en = 'Unpacking ED'");
...
Процедура ПрочитатьНоменклатуруКонтрагентаСервер()
	...
	Если НЕ ЗначениеЗаполнено(Запрос.Текст) Тогда
		...
		|en = 'No mapping is defined for products and services of the company and of the vendors.
		|Text of ОбменСКонтрагентамиПереопределяемый.ТекстЗапросаСопоставленияНоменклатуры procedure must be filled in.'");
