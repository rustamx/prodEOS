...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("НастройкиОтчета", НастройкиОтчета) Тогда
		ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""НастройкиОтчета"".'; en = 'Service parameter ""НастройкиОтчета"" is not specified.'");
	КонецЕсли;
	Если Не Параметры.Свойство("ВариантНаименование", ВариантНаименование) Тогда
		ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""ВариантНаименование"".'; en = 'Service parameter ""ВариантНаименование"" is not specified.'");
	КонецЕсли;
	...
	Если ИдентификаторТекущегоУзлаКД <> Неопределено Тогда
		...
		Если Не Параметры.Свойство("Заголовок", Заголовок) Тогда
			ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""Заголовок"".'; en = 'Service parameter ""Заголовок"" is not specified.'");
		КонецЕсли;
		Если Не Параметры.Свойство("ТипТекущегоУзлаКД", ТипТекущегоУзлаКД) Тогда
			ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""ТипТекущегоУзлаКД"".'; en = 'Service parameter ""ТипТекущегоУзлаКД"" is not specified.'");
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(ВариантНаименование) Тогда
			ВариантНаименование = НастройкиОтчета.Наименование;
		КонецЕсли;
		Заголовок = НСтр("ru = 'Настройки отчета'; en = 'Report settings'") + " """ + ВариантНаименование + """";
	КонецЕсли;
	...
	Иначе
		Для Каждого ЭлементСпискаДубльВФорме Из СписокЗначенийВФорме Цикл
			Если ЭлементСпискаДубльВФорме.Значение = Значение И ЭлементСпискаДубльВФорме <> ЭлементСпискаВФорме Тогда
				Состояние(НСтр("ru = 'Обнаружены дублирующиеся записи. Редактирование отменено.'; en = 'Found duplicate entries. Modification has been canceled.'"));
...
Процедура Подключаемый_СписокСПодбором_ОбработкаВыбора(Элемент, РезультатВыбора, СтандартнаяОбработка)
	...
	Если Дополнение.Всего > 0 Тогда
		Если Дополнение.Всего = 1 Тогда
			ЗаголовокОповещения = НСтр("ru = 'Элемент добавлен в список'; en = 'Item has been added to the list'");
		Иначе
			ЗаголовокОповещения = НСтр("ru = 'Элементы добавлены в список'; en = 'Items have been added to the list'");
		КонецЕсли;
...
Процедура СтруктураВариантаДобавитьГруппировкуПослеВыбораПоля(ДоступноеПолеКД, ПараметрыВыполнения) Экспорт
	...
	Если ДоступноеПолеКД = "<>" Тогда
		...
		Представление = НСтр("ru = '<Детальные записи>'; en = '<Detailed records>'");
	Иначе
...
Процедура ДобавитьТаблицуИлиДиаграмму(ТипЭлемента)
	...
	Если СтрокаТаблицы.Тип = "Диаграмма" Тогда
		...
		СтрокаТаблицы.Представление = НСтр("ru = 'Диаграмма'; en = 'Chart'");
		...
		ВложеннаяСтрока.Представление = НСтр("ru = 'Точки'; en = 'Dots'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru = 'Серии'; en = 'Series'");
		
	ИначеЕсли СтрокаТаблицы.Тип = "Таблица" Тогда
		
		СтрокаТаблицы.Представление = НСтр("ru = 'Таблица'; en = 'Table'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru = 'Строки'; en = 'Rows'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = НСтр("ru = 'Колонки'; en = 'Columns'");
...
Процедура СтруктураВарианта_СохранитьВФайл(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("ru = 'Настройки.xml'; en = 'Settings.xml'"), Истина);
КонецПроцедуры
...
Функция ВыполнитьШаг(ВыборПользователя, Сценарий)
	...
	Иначе
		Сценарий.ПричинаОтказа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка в сценарии: Неизвестный шаг ""%1"".'; en = 'Error in script: Unknown step ""%1"".'"), Сценарий.ТекущийШаг);
		Возврат Ложь;
...
Функция ШагОпределитьВыделенныеСтроки(ВыборПользователя, Сценарий)
	...
	Если Сценарий.СтрокиДерева.Количество() = 0 Тогда
		Сценарий.ПричинаОтказа = НСтр("ru = 'Выберите элементы.'; en = 'Select items.'");
		Возврат Ложь; // Завершить сценарий.
	КонецЕсли;
	Если Сценарий.ТекущаяСтрока = Неопределено Тогда
		Если Сценарий.Действие = "ИзменитьГруппу" Тогда
			Сценарий.ПричинаОтказа = НСтр("ru = 'Выберите группу.'; en = 'Select group.'");
			Возврат Ложь; // Завершить сценарий.
		КонецЕсли;
	КонецЕсли;
...
Функция ЗарегистрироватьВыделеннуюСтроку(Сценарий, СтрокаДерева, Особенности)
	...
	Если Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр Тогда
		Если Сценарий.Действие = "Переместить" Тогда
			Сценарий.ПричинаОтказа = НСтр("ru = 'Параметры не могут быть перемещены.'; en = 'Parameters cannot be moved.'");
		ИначеЕсли Сценарий.Действие = "Сгруппировать" Тогда
			Сценарий.ПричинаОтказа = НСтр("ru = 'Параметры не могут быть участниками групп.'; en = 'Parameters cannot be in groups.'");
		ИначеЕсли Сценарий.Действие = "Удалить" Тогда
			Сценарий.ПричинаОтказа = НСтр("ru = 'Параметры не могут быть удалены.'; en = 'Parameters cannot be deleted.'");
		КонецЕсли;
		Возврат Ложь; // Завершить сценарий.
	КонецЕсли;
	Если Особенности.ТребоватьОдногоРодителя Тогда
		...
		ИначеЕсли Сценарий.ТекущийРодитель <> Родитель Тогда
			Если Сценарий.Действие = "Переместить" Тогда
				Сценарий.ПричинаОтказа = НСтр("ru = 'Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.'; en = 'The selected items cannot be moved because they belong to different parents.'");
			ИначеЕсли Сценарий.Действие = "Сгруппировать" Тогда
				Сценарий.ПричинаОтказа = НСтр("ru = 'Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.'; en = 'The selected items cannot be grouped because they belong to different parents.'");
			КонецЕсли;
			Возврат Ложь; // Завершить сценарий.
		КонецЕсли;
	КонецЕсли;
...
Процедура СтруктураВариантаИзменитьУзел(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы, ИмяСтраницы)
	...
	ШаблонЗаголовка = НСтр("ru = 'Настройка %1 отчета ""%2""'; en = 'Setiings %1 of the report ""%2""'");
	Если СтрокаТаблицы.Тип = "Диаграмма" Тогда
		ПредставлениеУзла = НСтр("ru = 'диаграммы'; en = 'charts'");
	Иначе
		ПредставлениеУзла = НСтр("ru = 'группировки'; en = 'groups'");
	КонецЕсли;
...
Процедура ТаблицыПолейИзменитьУровеньДоступа(ТаблицаИмя, СтрокаИдентификатор, ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков)
	...
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить(2, НСтр("ru = 'В шапке отчета'; en = 'In report header'"), , БиблиотекаКартинок.БыстрыйДоступ);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить(1, НСтр("ru = 'Только флажок в шапке отчета'; en = 'Only checkbox in report header'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить(4, НСтр("ru = 'В настройках отчета'; en = 'In report settings'"), , БиблиотекаКартинок.Реквизит);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить(3, НСтр("ru = 'Только флажок в настройках отчета'; en = 'Only checkbox in report settings'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
	Список.Добавить(5, НСтр("ru = 'Не показывать'; en = 'Do not show'"), , БиблиотекаКартинок.СкрытаяНастройкаОтчета);
...
Процедура ОформлениеИзменитьЭлемент(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы)
	...
	КонецЕсли;
	ПараметрыФормы.Вставить("Заголовок", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Элемент условного оформления отчета ""%1""'; en = 'Report ""%1"" conditional appearance item'"), ВариантНаименование));
...
Процедура ТаблицыПолейРазгруппировать(ТаблицаИмя)
	...
	Если ВыделенныеСтроки.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите одну группу.'; en = 'Select one group.'"));
		Возврат;
	КонецЕсли;
	...
	Если ГруппаДерева = Неопределено Или Не ГруппаДерева.ЭтоГруппа Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите группу.'; en = 'Select group.'"));
		Возврат;
	КонецЕсли;
...
Функция ТаблицыПолейНайтиУзел(ЭтотОбъект, ТаблицаИмя, СтрокаТаблицы = Неопределено, ИдентификаторУзлаКД = Неопределено)
	Если ЭтотОбъект.РасширенныйРежим = 1 Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Изменение узлов таблицы ""%1"" не поддерживается.'; en = 'Table ""%1"" nodes modification is not supported.'"), ТаблицаИмя);
		КонецЕсли;
	Иначе
...
Процедура ВидимостьДоступностьКорректность(ИмяСобытия = "")
	Если ИмяСобытия = "ПриСозданииНаСервере" Тогда
		Если РежимИзмененияУзлаВарианта Тогда
			...
			Элементы.ЗакрытьИСформировать.Заголовок = НСтр("ru = 'Завершить редактирование'; en = 'Finish editing'");
...
Процедура РасширенныеНастройкиЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	Если ВидимостьВыбранныхПолей И ВидимостьСортировок Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ru = 'Поля и сортировки'; en = 'Fields and order'");
	ИначеЕсли ВидимостьВыбранныхПолей Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ru = 'Поля'; en = 'Fields'");
	ИначеЕсли ВидимостьСортировок Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("ru = 'Сортировки'; en = 'Order'");
	КонецЕсли;
...
Функция ЗарегистрироватьЭлементСтруктурыОтчета(СтрокаДерева, СтрокиРодителя)
	...
	Если СтрокаФормы.Тип = "Настройки" Тогда
		...
		СтрокаФормы.Представление = НСтр("ru = 'Отчет'; en = 'Report'");
...
Функция ЗарегистрироватьЭлементыСортировки()
	...
	СтрокаРаздел.Представление = НСтр("ru = 'Сортировки'; en = 'Order'");
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоЭлементПорядка" Тогда
			СтрокаТаблицы.Представление = НСтр("ru = 'Авто (сортировки родителя)'; en = 'Auto (parent order)'");
...
Процедура ЗарегистрироватьЭлементыСоставаГруппировки(УзелКД = Неопределено, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено)
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоПолеГруппировки" Тогда
			СтрокаТаблицы.Представление  = НСтр("ru = 'Авто (по всем полям)'; en = 'Auto (all fields)'");
...
Функция ЗарегистрироватьЭлементыВыбранныхПолей(УзелКД, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено, ДоступныеПоля = Неопределено)
	...
	Если НаборСтрок = Неопределено Тогда
		...
		СтрокаРаздел.ЭтоРаздел = Истина;
		СтрокаРаздел.Представление = НСтр("ru = 'Поля'; en = 'Fields'");
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоВыбранноеПоле" Тогда
			Представление = НСтр("ru = 'Авто (поля родителя)'; en = 'Auto (parent fields)'");
			ИндексКартинки = 6;
		Иначе
...
Процедура ЗарегистрироватьПараметры(Информация)
	...
	СтрокаРаздел.Представление = НСтр("ru = 'Параметры'; en = 'Parameters'");
...
Процедура ЗарегистрироватьЭлементыОтборов(Информация, НаборСтрокВарианта = Неопределено, НаборСтрокФормы = Неопределено)
	...
	Если НаборСтрокФормы = Неопределено Тогда
		...
		СтрокаРаздел.ЭтоРаздел = Истина;
		СтрокаРаздел.Представление = НСтр("ru = 'Отборы'; en = 'Filters'");
