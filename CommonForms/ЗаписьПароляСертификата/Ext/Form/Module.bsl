	...
	Если УсиленнаяЗащитаЗакрытогоКлюча Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'У сертификата установлена усиленная защита закрытого ключа.
			           ...
			           |en = 'The certificate requires secure protection of the private key.
			           |In this case the password is requested by the digital signature and encryption program,
			           |and 1C:Enterprise must pass an empty password to avoid errors.
			           |
			           |It is not possible to save the password.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Не (ЗначениеЗаполнено(Программа) ИЛИ ЗначениеЗаполнено(ПрограммаБанка)) Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'У сертификата не указана программа для закрытого ключа.
			           ...
			           |en = 'The certificate has no program assigned for use with the private key.
			           |Impossible to check the password before saving.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(СохраненныйПароль) Тогда
				...
				ВызватьИсключение НСтр("ru = 'Введите пароль повторно.'; en = 'Enter the password again.'");
			КонецЕсли;
		КонецЕсли;
...
Функция ОписаниеОшибкиПраваДоступа()
	...
	Возврат
		НСтр("ru = 'Недостаточно прав для записи пароля.
		           ...
		           |en = 'Insufficient permissions to save the password.
		           |
		           |Only the users specified in the certificate fields ""User"" and ""Added""
		           |may save the password.'");
...
Процедура УстановитьЗаголовокНадписиИПометкуКомандыДоступенВсем()
	...
	Если ЗначениеЗаполнено(Пользователь) Тогда
		Элементы.НадписьДоступенПользователю.Заголовок = НСтр("ru = 'Пароль доступен пользователю:'; en = 'Password is available to user:'")
			+ " " + Пользователь;
	Иначе
		Элементы.НадписьДоступенПользователю.Заголовок = НСтр("ru = 'Пароль доступен всем пользователям.'; en = 'Password is available to all users.'")
	КонецЕсли;
