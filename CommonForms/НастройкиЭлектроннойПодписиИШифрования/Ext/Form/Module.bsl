...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("СохранениеДанныхПользователя", Метаданные) Тогда
		ТекстОшибки = НСтр("ru = 'Отсутствует право сохранения данных. Обратитесь к администратору.'; en = 'Insufficient permissions to save data. Contact the administrator.'");
	...
	Иначе
		...
		Элементы.ПрограммыПояснение.Заголовок =
			НСтр("ru = 'Список программ, предусмотренных администратором, которые можно использовать на компьютере.'; en = 'List of programs provided by the administrator that you can use on the computer.'");
...
Процедура ПрограммыУстановитьПометкуУдаления(Команда)
	...
	Если ТекущиеДанные.ПометкаУдаления Тогда
		ТекстВопроса = НСтр("ru = 'Снять с ""%1"" пометку на удаление?'; en = 'Remove deletion mark from ""%1""?'");
	Иначе
		ТекстВопроса = НСтр("ru = 'Пометить ""%1"" на удаление?'; en = 'Mark ""%1"" for deletion?'");
	КонецЕсли;
...
Процедура ОбновитьТекущуюВидимостьЭлементов()
	...
	Если Константы.ИспользоватьШифрование.Получить()
	 Или ЭлектроннаяПодпись.ОбщиеНастройки().ЗаявлениеНаВыпускСертификатаДоступно Тогда
		...
		Элементы.СертификатыСоздать.Заголовок = НСтр("ru = 'Добавить...'; en = 'Add...'");
		Элементы.СертификатыКонтекстноеМенюСоздать.Заголовок = НСтр("ru = 'Добавить...'; en = 'Add...'");
	...
	Если Константы.ИспользоватьШифрование.Получить() Тогда
		Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
			НСтр("ru = 'Из установленных на компьютере...'; en = 'From the installed on the computer...'");
	Иначе
		Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
			НСтр("ru = 'Из установленных на компьютере'; en = 'From installed on computer'");
	КонецЕсли;
	
	Если Константы.ИспользоватьЭлектронныеПодписи.Получить() Тогда
		ЗаголовокФлажка = НСтр("ru = 'Проверять подписи и сертификаты на сервере'; en = 'Validate signatures and certificates on the server'");
		ПодсказкаФлажка =
			НСтр("ru = 'Позволяет не устанавливать программу на компьютер пользователя
			           ...
			           |en = 'Allows to do not install the program and certificates onto the user computers.
			           |
			           |Important: the program and the certificate with the private key must be installed onto
			           |each server that runs 1C:Enterprise server and onto each Web server that uses
			           |infobase operating in file mode.'");
	...
	Иначе
		ЗаголовокФлажка = НСтр("ru = 'Проверять сертификаты на сервере'; en = 'Validate certificates on the server'");
		ПодсказкаФлажка =
			НСтр("ru = 'Позволяет не устанавливать программу на компьютер пользователя
	...
	Если Не Константы.ИспользоватьЭлектронныеПодписи.Получить() Тогда
		ЗаголовокФлажка = НСтр("ru = 'Шифровать и расшифровывать на сервере'; en = 'Encrypt and decrypt on server side'");
		ПодсказкаФлажка =
	...
	ИначеЕсли Не Константы.ИспользоватьШифрование.Получить() Тогда
		ЗаголовокФлажка = НСтр("ru = 'Подписывать на сервере'; en = 'Sign on server'");
		ПодсказкаФлажка =
			НСтр("ru = 'Позволяет не устанавливать программу и сертификат
			           ...
			           |infobase operating in file mode.'");
	Иначе
		ЗаголовокФлажка = НСтр("ru = 'Подписывать и шифровать на сервере'; en = 'Sign and encrypt on server side'");
		ПодсказкаФлажка =
			НСтр("ru = 'Позволяет не устанавливать программу и сертификат
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Если Менеджер <> Неопределено Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("ru = 'Установлена на компьютере.'; en = 'Installed on the computer.'"));
	...
	Если Ошибка.НеУказанПуть Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("ru = 'Не указан путь к программе.'; en = 'Program path is not specified.'"));
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, "");
	Иначе
		ТекстОшибки = НСтр("ru = 'Не установлена на компьютере.'; en = 'Not installed on the computer.'") + " " + Ошибка.Описание;
		Если Ошибка.КАдминистратору И Не ЭтоПолноправныйПользователь Тогда
			ТекстОшибки = ТекстОшибки + " " + НСтр("ru = 'Обратитесь к администратору.'; en = 'Contact the administrator.'");
		КонецЕсли;
...
Процедура ЗаполнитьПрограммыИНастройки(ОбновитьПовтИсп = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтрокаСертификатДляШифрования = "" Тогда
			ОбновитьЗначение(Строка.СертификатДляШифрования, НСтр("ru = 'Отсутствует. Кликните, чтобы добавить.'; en = 'Missing. Click to add.'"));
