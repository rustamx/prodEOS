...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		ТекстИсключения = НСтр("ru='Заполнение реквизитов в данной конфигурации невозможно.'; en = 'Filling in attributes in this configuration impossible.'");
	...
	Элементы.Регион.СписокВыбора.Вставить(0, "", НСтр("ru='Все регионы'; en = 'All regions'"));
	Регион = "";
...
Процедура ВыполнитьПоискКонтрагентов() Экспорт
	...
	ИначеЕсли КоличествоНайденных > 20 Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Найдено слишком много похожих контрагентов (%1). Уточните реквизиты для поиска.'; en = 'Too many similar counterparties (%1). Specify details for search.'"),
			КоличествоНайденных);
		ПоказатьПредупреждение(, ТекстПредупреждения);
	ИначеЕсли НЕ ПустаяСтрока(СтрокаПоиска) И КоличествоНайденных = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Ничего не найдено. Уточните реквизиты для поиска.'; en = 'Nothing found. Specify details for search.'"));
	КонецЕсли;
...
Процедура ВыполнитьВыборКонтрагента()
	...
	Если ТекДанные = Неопределено Тогда
		Если Контрагенты.Количество() = 0 Тогда
			ТекстПредупреждения = НСтр("ru='Ничего не найдено. Уточните реквизиты и нажмите ""Найти"".'; en = 'Nothing found. Refine details and click ""Find"".'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ТекДанные.ИНН) Тогда
		ТекстПредупреждения = НСтр("ru='Выберите контрагента, у которого указан ИНН.'; en = 'Select a counterparty, which listed TIN.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура НайтиРеквизитыНаСервере(ОписаниеОшибки)
	...
	КоличествоНайденных = 0;
		...
		Информация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Показаны первые 20 контрагентов из %1 найденных. Уточните реквизиты для поиска.'; en = 'Shows first 20 counterparties from %1 found. Specify details for search.'"), 
			12999);
		// Отладка конец
