...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СписокПлановОбмена.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Возможность настройки синхронизации данных не предусмотрена.'; en = 'Data synchronization setup is not available'");
...
Процедура СписокСостоянияУзловВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Выполнить синхронизацию данных?'; en = 'Executa data synchronization?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
Процедура ОбновитьКоличествоСинхронизаций(КоличествоНастроенныхОбменов)
	...
	Если КоличествоСинхронизаций = 0 Тогда
		...
		Заголовок = НСтр("ru = 'Синхронизация данных'; en = 'Data synchronization'");
	...
	Иначе
		...
		Заголовок = НСтр("ru = 'Список настроенных синхронизаций данных'; en = 'List of configured data synchronizations'");
...
Процедура УстановитьЭлементыОднойСинхронизации()
	...
	Если НастроеннаяСинхронизация.УзелИнформационнойБазы = Неопределено Тогда
		...
		ВызватьИсключение НСтр("ru = 'Работа монитора синхронизации данных в неразделенном сеансе не поддерживается'; en = 'Data synchronization monitor is not supported in unseparated sessions'");
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойВыгрузки <> НастроеннаяСинхронизация.ДатаПоследнейВыгрузки
		...
		ПодсказкаВыгрузки = НСтр("ru = 'Данные отправлены: %ДатаОтправки%
										...
										|en = 'Data sent: %ДатаОтправки% 
										|Last attempt: %ДатаПопытки%'");
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойЗагрузки <> НастроеннаяСинхронизация.ДатаПоследнейЗагрузки
		...
		ПодсказкаЗагрузки = НСтр("ru = 'Данные получены: %ДатаПолучения%
										|Последняя попытка: %ДатаПопытки%';
										|en = 'Data received: %ДатаПолучения%
										|Last attempt: %ДатаПопытки%'");
	...
	Иначе
		...
		РасписаниеСинхронизацииДанных = НСтр("ru = 'Расписание синхронизации не настроено'; en = 'Data synchronization schedule is not set up'");;
			...
			Если СписокВозможныхНастроекОбмена.Количество() > 0 Тогда
				...
				Если МенеджерПланаОбмена.КорреспондентВМоделиСервиса() Тогда
					...
					Для Каждого ПредопределеннаяНастройка Из СписокВозможныхНастроекОбмена Цикл
						...
						СоздатьКомандуИЭлементФормы(НаименованиеКоманды, ЗаголовокКоманды + " " + НСтр("ru = '(в сервисе)'; en = '(in service)'"), СуммирующаяГруппа);
			...
			Иначе
				...
				Если МенеджерПланаОбмена.КорреспондентВМоделиСервиса() Тогда
					СоздатьКомандуИЭлементФормы(НаименованиеКоманды + "КорреспондентВМоделиСервиса", 
					                            ЗаголовокКоманды + " " + НСтр("ru = '(в сервисе)'; en = '(in service)'"),
					                            РодительскаяГруппа);
				КонецЕсли;
...
Процедура ПроверитьСовместимостьПравилКонвертации(Знач ИмяПланаОбмена, ОбработкаПродолжения)
	...
	Иначе
		...
		Кнопки.Добавить("ПерейтиКЗагрузкеПравил", НСтр("ru = 'Загрузить правила'; en = 'Import rules'"));
		...
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = НСтр("ru = 'Синхронизация данных не может быть выполнена'; en = 'Data synchronization cannot be performed'");
		Иначе
			ПараметрыФормы.Заголовок = НСтр("ru = 'Синхронизация данных может быть выполнена некорректно'; en = 'Data synchronization may work incorrectly'");
		КонецЕсли;
...
|en = 'Data synchronization cannot be performed, because rules intended for ""%1"" are used.
|You should use the rules from the configuration or import the correct set of rules from a file.'"));
		...
		Если РезультатСравнения <> 0 Тогда
			...
			Если РезультатСравнения < 0 Тогда
				...
				ТекстОшибки = НСтр("ru = 'Синхронизация данных может быть выполнена некорректно, так как используются правила, предназначенные для предыдущей версии программы ""%1"" (%2). Рекомендуется использовать правила из конфигурации или загрузить комплект правил, предназначенный для текущей версии программы (%3).'; en = 'Data synchronization may work incorrectly, because the rules designed for the previous version of the application ""%1"" (%2) are used. It is recommended that you use the rules from the configuration or import the set of rules designed for the current version of the program (%3).'");
				ВидОшибки = "УстаревшаяВерсияКонфигурации";
				
			Иначе
				
				ТекстОшибки = НСтр("ru = 'Синхронизация данных может быть выполнена некорректно, так как используются правила, предназначенные для более новой версии программы ""%1"" (%2). Рекомендуется обновить версию программы или использовать комплект правил, предназначенный для текущей версии программы (%3).'; en = 'Data synchronization may work incorrectly, because the rules designed for the newer version of the application ""%1"" (%2) are used. It is recommended that you update the program version or use the set of rules designed for the current version of the program (%3).'");
