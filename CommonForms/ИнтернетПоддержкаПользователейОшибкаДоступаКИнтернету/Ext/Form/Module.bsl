	...
	Элементы.НадписьСообщенияВТехПоддержку.Заголовок =
		ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
			ИнтернетПоддержкаПользователейКлиентСервер.ПодставитьДомен(
				НСтр("ru = '<body>При возникновении проблем отправьте письмо по адресу <a href=""SendMail"">webits-info@1c.ru</a></body>'; en = '<body>If you have any problems send an email to <a href=""SendMail"">webits-info@1c.ru</a></body>'")));
...
Процедура НадписьСообщенияВТехПоддержкуОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "SendMail" Тогда
		...
		Если МестоЗапуска = "handStartNew"
			...
			Тема = НСтр("ru = 'Интернет-поддержка. Обращение к монитору Интернет-поддержки.'; en = 'Online customer support. Accessing online customer support monitor.'");
			Тело = НСтр("ru = 'Возникает ошибка обращения к монитору Интернет-поддержки.'; en = 'Unable to open online support monitor:'");
			
		ИначеЕсли МестоЗапуска = "taxcomGetID" Тогда
			
			Тема = НСтр("ru = 'Интернет-поддержка. Обращение к 1С-Такском'; en = 'Online customer support. Accessing 1С-Taxcom'");
			Тело = НСтр("ru = 'Возникает ошибка при обращении к сервису получения идентификатора 1С-Такском.'; en = 'An error occurs when accessing a service to retrieve ID 1c-taxcom'");
		...
		Иначе
			...
			Тема = НСтр("ru = 'Интернет-поддержка. Ошибка обращения к сервису'; en = 'Online customer support. Error connecting to the service'");
			Тело = НСтр("ru = 'Возникает ошибка при обращении к сервису Интернет-поддержки.'; en = 'An error occurs when accessing a service online support.'");
			
		КонецЕсли;
		
		
		Если Не ПустаяСтрока(ПодробноеОписаниеОшибки) Тогда
			Тело = Тело + Символы.ПС + НСтр("ru = 'Описание ошибки:'; en = 'Error description:'")
