...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ru = 'Использование Интернет-поддержки недоступно при работе в модели сервиса.'; en = 'Using online customer support is not available when using the software as a service.'");
	КонецЕсли;
...
Процедура НадписьПоясненияПодключенияАвторизацияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "action:openPortal" Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
			...
			НСтр("ru = 'Портал 1С:ИТС'; en = '1C:ITS portal'"));
		
	Иначе
		
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru = 'Интернет-поддержка. Подключение Интернет-поддержки.'; en = 'Online customer support. Connecting to online customer support.'"),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не получается подключить Интернет-поддержку пользователей.
					|Для подключения указывается логин %1.';
					|en = 'Unable to connect to Online customer support.
					|Attempts were made using login %1.'"),
...
Процедура НадписьВосстановленияПароляАвторизацияНажатие(Элемент)
	...
	ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
		...
		НСтр("ru = 'Восстановление пароля'; en = 'Password recovery'"));
...
Процедура НадписьНетЛогинаИПароляАвторизацияНажатие(Элемент)
	...
	ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
		...
		НСтр("ru = 'Регистрация'; en = 'Registration'"));
...
Процедура КомандаОК(Команда)
	...
	Состояние(, , НСтр("ru = 'Подключение Интернет-поддержки...'; en = 'Connecting Online support...'"));
	...
	Иначе
		...
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Логин и пароль сохранены в программе, но проверка корректности
				...
				|%1';
				|en = 'Login and password are saved in the program,
				|but validation failed because:
				|%1'"),
...
Функция ЗаполнениеЛогинаИПароляКорректно()
	...
	Если ПустаяСтрока(Логин) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Логин"" не заполнено.'; en = 'Field ""Login"" is not filled in'"),
			,
			"Логин");
		Возврат Ложь;
	...
	ИначеЕсли ПустаяСтрока(Пароль) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Пароль"" не заполнено.'; en = 'Field ""Password"" is not filled in'"),
			,
			"Пароль");
		Возврат Ложь;
...
Процедура СохранитьДанныеАутентификации(Знач ДанныеАутентификации)
	...
	Если Не ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП() Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для записи данных аутентификации Интернет-поддержки.'; en = 'Insufficient permissions to save authentication parameters for Online support.'");
	КонецЕсли;
...
Процедура ВывестиДатуНастройкиНеНапоминатьОбАвторизацииДо()
	...
	ОбщийЗаголовокФлажка = НСтр("ru = 'Не напоминать о подключении семь дней'; en = 'Do not remind for seven days'");
	...
	СтрокаФлажка = ОбщийЗаголовокФлажка
		+ ?(ЗначениеНастройки = '00010101',
			"",
			" " + НСтр("ru = '(до'; en = '(before'") + " " + Формат(ЗначениеНастройки, "ДЛФ=D") + ")");
