...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.НастройкаПроксиНаКлиенте
		...
		ВызватьИсключение НСтр("ru = 'Недостаточно прав доступа.
			...
			|
			|Proxy configuration is performed by the administrator.'");
	КонецЕсли;
	...
	Иначе
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru = 'Параметры прокси-сервера на сервере 1С:Предприятия'; en = 'Proxy server parameters for access from 1С:Enterprise server'");
