...
Функция СоздатьАдминистратора(ПользовательИБ = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ru = 'Справочник Пользователи недоступен в неразделенном режиме.'; en = 'Catalog Users is unavailabale in undivided mode.'");
	КонецЕсли;
	...
	Если ПользовательИБ = Неопределено Тогда
		...
		Если ПользователиИБ.Количество() = 0 Тогда
			Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
				ВызватьИсключение
					НСтр("ru = 'Невозможно автоматически создать первого администратора области данных.'; en = 'You cannot automatically create the first administrator of data region.'");
			КонецЕсли;
			ПользовательИБ = ПользователиИнформационнойБазы.СоздатьПользователя();
			ПользовательИБ.Имя       = НСтр("ru = 'Администратор'; en = 'Administrator'");
			...
			Если Метаданные.Роли.ПолныеПрава = РольАдминистратораСистемы(Истина) Тогда
				ТекстОшибки =
					НСтр("ru = 'Список пользователей информационной базы не пустой, однако
					           ...
					           |en = 'List of infobase users is not empty, but failed to find any user with Full access role. 
					           |Probably users has been created in designer. 
					           |Need to assign the Full access role to at least one user.'");
			Иначе
				ТекстОшибки =
					НСтр("ru = 'Список пользователей информационной базы не пустой, однако не удалось
					           ...
					           |en = 'List of infobase users is not empty, but failed to find any user with Full access and System administrator roles. 
					           |
					           |Probably users has been created in designer. 
					           |Need to assign the Full access and System administrator roles to at least one user.'");
			КонецЕсли;
			...
			Если Метаданные.Роли.ПолныеПрава = РольАдминистратораСистемы(Истина) Тогда
				ТекстОшибки =
					НСтр("ru = 'Невозможно создать пользователя в справочнике для пользователя
					           ...
					           |en = 'You cannot create user in the catalog for the 
					           |infobase user ""%1"", 
					           |because he does not have Full access role. 
					           |Probably the user was created in the designer. 
					           |To automatically create a new user in the catalog you want to assign him Full access role.'");
			Иначе
				ТекстОшибки =
					НСтр("ru = 'Невозможно создать пользователя в справочнике для пользователя
					           |информационной базы ""%1"",
					           ...
					           |en = 'You cannot create user in the catalog for the 
					           |infobase user ""%1"", 
					           |because he does not have Full access and System administrator roles. 
					           |
					           |Probably the user was created in the designer. 
					           |To automatically create a new user in the catalog you want to assign him Full access and System administrator roles.'");
			КонецЕсли;
	...
	Пользователь.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
		?(ПользовательИБ = Неопределено,
		  НСтр("ru = 'Выполнено создание первого администратора.'; en = 'Successfully created the first administrator.'"),
		  ?(ПользовательСоздан,
		    НСтр("ru = 'Администратор сопоставлен с новым пользователем справочника.'; en = 'Administrator is associated with the new user.'"),
		    НСтр("ru = 'Администратор сопоставлен с существующим пользователем справочника.'; en = 'Administrator is associated with an existing user.'")) ) );
...
Функция НовоеОписаниеПользователяИБ() Экспорт
	...
	// Корнюшенков А.Ю. 25.09.2018 {
	// основной интерфейс для нового пользователя - русский
	Свойства.Вставить("Язык",
		?(Метаданные.ОсновнойЯзык = Неопределено, "", Метаданные.Языки.Русский.Имя));
	// было	
	//Свойства.Вставить("Язык",
	//	?(Метаданные.ОсновнойЯзык = Неопределено, "", Метаданные.ОсновнойЯзык.Имя));	
	// Корнюшенков А.Ю. Искать в тексте "ДоработкиМП" 25.09.2018 } 	
...
Функция ПрочитатьПользователяИБ(Знач Идентификатор,
	...
	Если ПользовательИБ = Неопределено Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователь информационной базы ""%1"" не найден.'; en = 'infobase user ""%1"" not found.'"),
			Идентификатор);
		Возврат Ложь;
	КонецЕсли;
	...
	ИначеЕсли СоздатьНового = Истина Тогда
		ОписаниеОшибки = ОписаниеОшибкиПриЗаписиПользователяИБ(
			НСтр("ru = 'Невозможно создать пользователя информационной базы
			           |%1,
			           |так как он уже существует.'; en = 'Cannot create infobase user %1, because it already exists.'"),
			СтарыеСвойства.Имя,
			...
			Если НЕ СтарыйПарольСовпадает Тогда
				ОписаниеОшибки = ОписаниеОшибкиПриЗаписиПользователяИБ(
					НСтр("ru = 'При записи пользователя информационной базы
					           ...
					           |en = 'When saving the infobase user 
					           |%1 
		...
		Исключение
			ОписаниеОшибки = ОписаниеОшибкиПриЗаписиПользователяИБ(
				НСтр("ru = 'При удалении пользователя информационной базы
				           ...
				           |
				           |""%2"".';
				           |en = 'While deleting infobase user 
				           |%1 error occurred: 
				           |
				           |""%2"".'"),
				ПользовательИБ.Имя,
						...
						Если СтрДлина(ЗначениеСвойства) > 64 Тогда
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Ошибка записи пользователя информационной базы
								           ...
								           |en = 'Failed to write the infobase user
								           |Name (login): ""%1"" 
								           |is greater than 64 symbols.'"),
								ЗначениеСвойства);
...
Процедура НайтиНеоднозначныхПользователейИБ(Знач Пользователь,
	...
	|					Справочник.Пользователи КАК Пользователи
	|				ГДЕ
	|					Пользователи.ра_ИдентификаторПользователяИБ <> &ПустойУникальныйИдентификатор
	|					И НЕ(&Пользователь <> НЕОПРЕДЕЛЕНО
	|							И Пользователи.Ссылка <> &Пользователь)
	|					И НЕ(&УникальныйИдентификатор <> &ПустойУникальныйИдентификатор
	|							И Пользователи.ра_ИдентификаторПользователяИБ <> &УникальныйИдентификатор)
	// ТСК Близнюк С.И.; 26.09.2018; task#657}
	|		
	|				ОБЪЕДИНИТЬ ВСЕ
	|		
	...
	ОписаниеОшибки = НСтр("ru = 'Ошибка в базе данных:'; en = 'Database error:'") + Символы.ПС;
		...
		Если Строка.НеоднозначныйИдентификатор <> ТекущийНеоднозначныйИдентификатор Тогда
			...
			Иначе
				...
				Если ТекущийПользовательИБ = Неопределено Тогда
					ИмяДляВхода = НСтр("ru = '<не найден>'; en = '<not found>'");
				Иначе
				...
				Если ИдентификаторПользователяСервиса Тогда
					ОписаниеОшибки = ОписаниеОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Пользователю сервиса с идентификатором ""%1""
						           |соответствует более одного элемента в справочнике Пользователи:'; en = 'Service user with ID ""%1""matches more than one items in the Users catalog:'"),
						ТекущийНеоднозначныйИдентификатор);
				Иначе
					ОписаниеОшибки = ОписаниеОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Пользователю ИБ ""%1"" с идентификатором ""%2""
						           |соответствует более одного элемента в справочнике Пользователи:'; en = 'Infobase user ""%1"" with ID ""%2"" matches more than one items in the Users catalog:'"),
						ИмяДляВхода,
						ТекущийНеоднозначныйИдентификатор);
				КонецЕсли;
			КонецЕсли;
		...
		Иначе
			...
			ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + "	"
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '""%1"" с идентификатором ссылки %2'; en = '""%1"" with link ID %2.'"),
					Строка.Пользователь,
...
Функция ОписаниеОшибкиПриЗаписиПользователяИБ(ШаблонОшибки,
	...
	Если ЗаписатьВЖурнал Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Пользователи.Ошибка записи пользователя ИБ'; en = 'New'",
