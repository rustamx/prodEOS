...
Процедура УстановитьУсловноеОформлениеПоляДата(Форма, ПолноеИмяРеквизита = "Список.Дата", ИмяЭлемента = "Дата") Экспорт
	...
	ЭлементОформления.Представление = НСтр("ru = 'Представление даты: ""10.06.2012""'; en = 'Date submission: "" 10.06.2012 ""'");
	...
	ЭлементОформления = УсловноеОформлениеКД.Элементы.Добавить();
	ЭлементОформления.Использование = Истина;
	ЭлементОформления.Представление = НСтр("ru = 'Представление даты сегодня: ""09:46""'; en = 'Date submission today: "" 9:46 ""'");
	...
	Если ВключитьВСостав.Количество() <> 0
		...
		Если ВключитьВСостав.Количество() <> 0 Тогда
			...
			ОписаниеИсключения1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В состав плана обмена %1 должны входить следующие объекты метаданных: %2'; en = 'Composition of the exchange plan %1 should include the following metadata objects: %2'"),
				ИмяПланаОбмена,
		...
		Если ИсключитьИзСостава.Количество() <> 0 Тогда
			...
			ОписаниеИсключения2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В состав плана обмена %1 НЕ должны входить следующие объекты метаданных: %2'; en = 'Composition of the exchange plan %1 shall not include the following metadata objects: %2'"),
				ИмяПланаОбмена,
		...
		Если ОтключитьАвторегистрацию.Количество() <> 0 Тогда
			...
			ОписаниеИсключения3 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В составе плана обмена %1 не должно быть объектов с установленным признаком авторегистрации.
					...
					|en = 'Composition of exchange plan %1 should not include objects with the established flag of autoregistration. 
					|Requires to prevent the autoregistration for the following metadata: %2'"),
				ИмяПланаОбмена,
...
Функция ПараметрыКлиентаНаСервере() Экспорт
	...
	Если ПараметрыКлиента.Количество() = 0
	   И ТекущийРежимЗапуска() <> Неопределено Тогда
		...
		ВызватьИсключение НСтр("ru = 'Не заполнены параметры клиента на сервере.'; en = 'Not filled client settings on the server.'");
	КонецЕсли;
	...
	Если ИменаПараметров <> "" Тогда
		...
		Если ТребуетсяОбновление Тогда
			...
			Если ТекущийРежимЗапуска() = Неопределено Тогда
				ВызватьИсключение
					НСтр("ru = 'Вход в программу временно невозможен в связи с обновлением на новую версию.'; en = 'Signing on is temporarily disabled due to update.'");
			Иначе
				ВызватьИсключение
					НСтр("ru = 'Недопустимое обращение к необновленным параметрам работы программы
					           ...
					           |en = 'An invalid address to unrecoverable parameters of the program 
					           |(for example, some session parameters): 
					           |- if this appeal runs from the form on the home page (the desktop), then you must make sure that there is a procedure call CommonUse.OnCreateAtServer; 
					           |- in other cases, you must transfer the call application code after updating application settings.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
							...
							Если ОшибкаУстановкиМонопольногоРежима <> Неопределено Тогда
								ОшибкаУстановкиМонопольногоРежима =
									НСтр("ru = 'Невозможно выполнить обновление информационной базы:
									           ...
									           |en = 'Unable to update the infobase:
									           |- Can not install the exclusive mode
									           |- Configuration version does not provide update without exclusive mode.'");
							КонецЕсли;
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				...
				ИмяСобытия = НСтр("ru = 'Идентификаторы объектов метаданных.Требуется загрузить критичные изменения'; en = 'Metadata object IDs.Critical changes must be imported'",
				...
				ТекстОшибки =
					НСтр("ru = 'Информационная база не может быть обновлена из-за проблемы в главном узле:
					           ...
					           |%1';
					           |en = 'Infobase could not be updated because of a problem in the main node: 
					           |- the main node has been updated incorrectly (perhaps the configuration version number was not 
					           |incremented and so identifiers of metadata objects were not updated); 
					           |- or priority data were unregistered for export (elements of catalog 
					           |Metadata object identifiers). 
					           |
					           |
					           |It is necessary to update the main node, register priority data for export, and 
					           |repeat data synchronization of data: 
					           |- in the main node run the program with parameter /C ЗапуститьОбновлениеИнформационнойБазы; 
					           |%1'");
				...
				Если ВыполнитьЗагрузку Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
						НСтр("ru = '- затем повторите синхронизацию данных с этой информационной базой
						           ...
						           |en = '- then repeat data synchronization with the infobase
						           |(first in the main node, then in this infobase after restart).'"));
				Иначе
					// Настройка подчиненного узла РИБ при первом запуске.
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
						НСтр("ru = '- затем повторите создание подчиненного узла.'; en = '- then try to create a slave node.'"));
				КонецЕсли;
...
Процедура ЗарегистрироватьОбъектНаВсехУзлах(Знач Объект, Знач ИмяПланаОбмена) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
			ВызватьИсключение НСтр("ru = 'Регистрация изменений неразделенных данных в разделенном режиме.'; en = 'Registration of changes of unseparated data in separated mode'");
		КонецЕсли;
		...
		Если Не ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных("ПланОбмена." + ИмяПланаОбмена,
			ОбщегоНазначенияПовтИсп.РазделительОсновныхДанных()) Тогда
				ВызватьИсключение НСтр("ru = 'Регистрация изменений для неразделенных планов обмена не поддерживается.'; en = 'Registration of changes for unseparated exchange plans is not supported.'");
		КонецЕсли;
		
		Если ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(Объект.Метаданные().ПолноеИмя(),
			ОбщегоНазначенияПовтИсп.РазделительОсновныхДанных()) Тогда
				ВызватьИсключение НСтр("ru = 'Регистрация изменений для разделенных объектов не поддерживается.'; en = 'Registration of changes for separated objects is not supported.'");
		КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаВременныхФайлов(Истина, Истина,
		НСтр("ru = 'Для возможности работы программы.'; en = 'For running program.'")));
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Важное        = Истина;
	Дело.Представление = НСтр("ru = 'Скорость работы снижена'; en = 'Operating speed reduced'");
	Дело.Форма         = "Обработка.РекомендацияПоПовышениюСкоростиРаботы.Форма.Форма";
	Дело.Владелец      = НСтр("ru = 'Скорость работы программы'; en = 'Speed of program operation'");
...
Процедура ОчиститьКаталогВременныхФайлов(ПутьККаталогу) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Стандартные подсистемы'; en = 'Standard subsystems'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка очистки каталога временных файлов ""%1"":%2'; en = 'Error cleaning temporary files directory ""%1"":%2'"),
				ПутьККаталогу,
...
Функция УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика() Экспорт
	...
	Возврат Символы.ПС + Символы.ПС 
		+ НСтр("ru = 'Для разработчика: возможно требуется обновить вспомогательные данные,
					...
					|en = 'For developer: perhaps you need to update the supporting data
					|that affect the operation of the program. To perform the upgrade, you can:
					|- use external dataprocessor ""Developer tools: Updated auxiliary data""
					|- run this program from the command line with option ""/C StartInfoBaseUpdate""
					|- increase the version number of the configuration to the next time you start the update procedure performed infobase data.'");
...
Функция ДобавитьЭлементУсловногоОформления(Форма, Инструкция) Экспорт
	...
	Для Каждого Поле Из Поля Цикл
		...
		Если РежимОтладки И Форма.Элементы.Найти(ИмяЭлемента) = Неопределено Тогда
			Шаблон = НСтр("ru = 'Ошибка добавления условного оформления: Не найден элемент ""%1"".'; en = 'Error adding conditional appearance: element not found ""%1"".'");
...
Функция ОбработчикиСобытий() Экспорт
	...
	Если ОбязательныеСобытияБезОбработчиков.Количество() > 0 Тогда
		ИмяСобытия  = НСтр("ru = 'Обработчики событий'; en = 'Events handlers'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		
		Комментарий = НСтр("ru = 'Для следующих обязательных событий не определены обработчики:'; en = 'Handlers for the following mandatory events are not defined:'")
		...
		ВызватьИсключение НСтр("ru = 'Для обязательных событий не определены обработчики.
		                             ...
		                             |en = 'Handlers for mandatory events are not defined. 
		                             |see log for details.'");
	КонецЕсли;
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Тогда
		ВызватьИсключение
			НСтр("ru = 'Недопустимо создавать предопределенный элемент помеченный на удаление.'; en = 'It is unacceptable to create predefined item marked for deletion.'");
	Иначе
		   ...
		   И СтарыеСвойства.ПометкаУдаления <> Истина Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недопустимо помечать на удаление предопределенный элемент:
				           |""%1"".';
				           |en = 'It is unacceptable to mark to delete a predefined item:
				           |""%1"".'"),
		...
		ИначеЕсли СтарыеСвойства.ИмяПредопределенныхДанных = ""
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недопустимо связывать с именем предопределенного элемент, помеченный на удаление:
				           |""%1"".';
				           |en = 'It is unacceptable to associate with the name of a predefined item marked for deletion: 
				           |""%1"".'"),
				Строка(Источник.Ссылка));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗапретитьУдалениеПредопределенныхЭлементовПередУдалением(Источник, Отказ) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Недопустимо удалять предопределенный элемент
		           |""%1"".';
		           |en = 'It is unacceptable to delete predefined element
		           |""%1"".'"),
		Строка(Источник.Ссылка));
...
Процедура ПередЗапускомПрограммы()
	...
	Если Метаданные.ВариантВстроенногоЯзыка <> Метаданные.СвойстваОбъектов.ВариантВстроенногоЯзыка.Русский Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Вариант встроенного языка конфигурации ""%1"" не поддерживается.
			           ...
			           |en = 'Built-in configuration language variant ""%1"" is not supported.
			           |You must use variant ""%2"".'"),
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы) < 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для запуска необходима версия платформы 1С:Предприятие %1 или выше.'; en = 'Required version of 1C:Enterprise platform %1 or higher.'"), МинимальнаяВерсияПлатформы);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НедоступныйРежим) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Режим совместимости конфигурации с 1С:Предприятием версии %1 не поддерживается.
			           ...
			           |en = 'Configuration compatibility mode with 1C:Enterprise version %1 is not supported. 
			           |To run it set compatibility mode with 1C:Enterprise of version 8.3.6 or ""Do not use"".'"),
			НедоступныйРежим);
	КонецЕсли;
	
	// Проверка заполнения версии конфигурации.
	Если ПустаяСтрока(Метаданные.Версия) Тогда
		ВызватьИсключение НСтр("ru = 'Не заполнено свойство конфигурации Версия.'; en = 'Not full configuration property Version.'");
	Иначе
		Попытка
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не правильно заполнено свойство конфигурации Версия: ""%1"".
				           ...
				           |en = 'Not correctly filled the configuration property Version: ""%1"".
	 ...
	 Или Не Метаданные.ОсновныеРоли.Содержит(Метаданные.Роли.ПолныеПрава) Тогда
		ВызватьИсключение
			НСтр("ru = 'В конфигурации в свойстве ОсновныеРоли не указаны стандартные роли
			           ...
			           |en = 'In the configuration DefaultRoles property not specified predefined roles
			           |FullAdministrator and FullAccess or specified unecessary roles.'");
	КонецЕсли;
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при подготовке списка событий.
				           ...
				           |Событие ""%1""
				           |уже добавлено.';
				           |en = 'Error occurred while preparing the list of events.
				           |
				           |Event ""%1""is already added.'"),
				Событие);
		КонецЕсли;
				 ...
				 ИЛИ НЕ ЗначениеЗаполнено(Обработчик.Модуль) Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка при подготовке обработчиков события
						           ...
						           |en = 'Error occurred while preparing the event handlers
						           |""%1"".
						           |
						           |Error in module name ""%2"".'"),
						Событие,
						Обработчик.Модуль);
				КонецЕсли;
				...
				Иначе
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка при подготовке обработчиков события
						           ...
						           |""%1"".
						           |
						           |Module ""%2"" has already been added.'"),
						Событие,
						Обработчик.Модуль);
				КонецЕсли;
				...
				Иначе
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Ошибка при подготовке обработчиков события
						           ...
						           |""%1"".
						           |
						           |Handler ""%2"" has already been added for event
						           |""%3"".'"),
						Событие,
						ИмяОбработчика,
			...
			Исключение
				ТекстОшибки =
					НСтр("ru = 'Невозможно выполнить обновление информационной базы:
					           ...
					           |- Версия конфигурации не предусматривает обновление без установки монопольного режима.';
					           |en = 'Unable to update the infobase:
					           |- Can not set the exclusive mode
					           |- Configuration version does not provide update without the exclusive mode.'");
