...
Функция ДокументооборотПриЗаписиПравообразующихРегистров(Источник, Отказ) Экспорт 
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("РегистрСведенийНаборЗаписей.ra_KomandyNesootvetstvij")
		Или ТипЗнч(Источник) = Тип("РегистрСведенийНаборЗаписей.ra_KomandyPoObmenuLuchshimiPraktikami") Тогда
		...
		ра_ОбщегоНазначения.ПерезаписатьРабочиеГруппыОбъектов(ОтборПоНесоответствию.Значение, Истина);
		
		Возврат Истина;
		
	ИначеЕсли ТипЗнч(Источник) = Тип("РегистрСведенийНаборЗаписей.ra_UchastnikiKontrolnyhOperaciy") Тогда
		
		ОтборПоЗаявке = Источник.Отбор.ZayavkaNaKontrolnuyuOperaciyu;
		
		Если Не ОтборПоЗаявке.Использование Тогда
			Возврат Истина;
		КонецЕсли;
		
		ЗначениеСсылки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтборПоЗаявке.Значение, "Ссылка");
		Если ЗначениеСсылки = Неопределено Тогда
			Если Не Источник.ОбменДанными.Загрузка Или Источник.Количество() = 0 Тогда
				// Это удаление элемента, ставить его в очередь не нужно.
				Возврат Истина;
		...
		ра_ОбщегоНазначения.ПерезаписатьРабочиеГруппыОбъектов(ОтборПоЗаявке.Значение, Истина);
