		...
		Если ИменаПредметовДляФункций <> Неопределено Тогда
			Если ИменаПредметовДляФункций.Количество() > 0 Тогда
				Для Каждого ИмяПредмета Из ИменаПредметовДляФункций Цикл
					...
					// Для документов "Заявка на контрольную операцию"
					...
					ДоступныеФункции.Добавить("ШаблоныБизнесПроцессовПереопределяемый.СогласовантыРезультатаКО(Объект, ИмяПредмета)",
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.Согласованты результата (КО)", Строка(ИмяПредмета)));
						
					ДоступныеФункции.Добавить("ШаблоныБизнесПроцессовПереопределяемый.ПодписантыРезультатаКО(Объект, ИмяПредмета)",
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.Подписанты результата (КО)", Строка(ИмяПредмета)));
						
					// Для документов "Несоответствие"
					
					ДоступныеФункции.Добавить("ШаблоныБизнесПроцессовПереопределяемый.ВыявившийНесоответствие(Объект, ИмяПредмета)",
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.Выявивший Несоответствие", Строка(ИмяПредмета)));
...
Функция ВыявившийНесоответствие(БизнесПроцессОбъект, ИмяПредмета) Экспорт 
	...
	Если СтрокаПредмета <> Неопределено Тогда
		...
		Участник = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПредмета.Предмет, "VyyavivsheeLico");
				
		Если Не ЗначениеЗаполнено(Участник) Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не заполнен Выявивший в документе ""%1"".'; en = 'Detected person is not filled out ""%1""'"),
				Строка(СтрокаПредмета.Предмет));
		КонецЕсли;
...
Функция КонтролерКО(БизнесПроцессОбъект, ИмяПредмета) Экспорт 
	...
	Если СтрокаПредмета <> Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(Участник) Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не заполнен Контролер в документе ""%1"".'; en = 'Controller is not filled out ""%1""'"),
				Строка(СтрокаПредмета.Предмет));
		КонецЕсли;
...
Функция СогласовантыРезультатаКО(БизнесПроцессОбъект, ИмяПредмета) Экспорт
	...
	Если СтрокаПредмета <> Неопределено Тогда
		...
		Если ТипЗнч(СтрокаПредмета.Предмет) <> Тип("ДокументСсылка.ra_RezultatKontrolnoyOperacii") Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Автоподстановка не может быть выполнена, так как указан предмет с типом, отличным от ""%1"".'; en = 'Auto-substitution cannot be executed because selected subject not ""%1"".'"),
				Строка(ИмяПредмета));
		КонецЕсли;
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияСостоянийДокументов.СрезПоследних( , ВЫРАЗИТЬ(Документ КАК Документ.ra_RezultatKontrolnoyOperacii) = &RezultatKontrolnoyOperacii) КАК ИсторияСостоянийДокументов
...
Функция ПодписантыРезультатаКО(БизнесПроцессОбъект, ИмяПредмета) Экспорт
