		...
		Если ИменаПредметовДляФункций <> Неопределено Тогда
			Если ИменаПредметовДляФункций.Количество() > 0 Тогда
				Для Каждого ИмяПредмета Из ИменаПредметовДляФункций Цикл
					...
					ДоступныеФункции.Добавить("ШаблоныБизнесПроцессовПереопределяемый.ЗаявительКО(Объект, ИмяПредмета)",
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.Заявитель (КО)", Строка(ИмяПредмета)));
					...
					ДоступныеФункции.Добавить("ШаблоныБизнесПроцессовПереопределяемый.УчастникиСогласующиеКО(Объект, ИмяПредмета)",
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.Участники / согласующие (КО)", Строка(ИмяПредмета)));
...
Функция ЗаявительКО(БизнесПроцессОбъект, ИмяПредмета) Экспорт 
	...
	Если СтрокаПредмета <> Неопределено Тогда
		...
		Участник = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПредмета.Предмет, "Zayavitel");
				
		Если Не ЗначениеЗаполнено(Участник) Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не заполнен заявитель в документе ""%1"".'; en = 'Applicant is not filled out ""%1""'"),
				Строка(СтрокаПредмета.Предмет));
		КонецЕсли;
...
Функция УчастникиСогласующиеКО(БизнесПроцессОбъект, ИмяПредмета) Экспорт 
	...
	Если СтрокаПредмета <> Неопределено Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
			|	UchastnikiSoglasuyushchie.Otvetstvennyj КАК Otvetstvennyj
			|ИЗ
			|	Документ.ra_ZayavkaNaKontrolnuyuOperaciyu.UchastnikiSoglasuyushchie КАК UchastnikiSoglasuyushchie
			|ГДЕ
			|	UchastnikiSoglasuyushchie.Ссылка = &ZayavkaNaKontrolnuyuOperaciyu";
		...
		Если МассивУчастников.Количество() = 0 Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не заполнен состав участников для ""%1"".'; en = 'Approvers list for ""%1"" is empty'"),
				Строка(СтрокаПредмета.Предмет));
		КонецЕсли;
