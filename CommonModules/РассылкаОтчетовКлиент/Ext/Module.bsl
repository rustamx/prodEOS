...
Процедура ВыбратьПолучателя(ОбработчикРезультата, Объект, МножественныйВыбор, ВозвращатьСоответствие) Экспорт
	...
	Если МножественныйВыбор Тогда
		ВозможныеПолучатели.Вставить(0, Неопределено, НСтр("ru = 'Всем получателям'; en = 'All recipients'"));
	КонецЕсли;
	...
	ВозможныеПолучатели.ПоказатьВыборЭлемента(Обработчик, НСтр("ru = 'Выбор получателя'; en = 'Recipient selection'"));
КонецПроцедуры
...
Процедура ЗапускФоновогоЗадания(Получатели, Параметры) Экспорт
	...
	Если Параметры.ЭтоФормаЭлемента Тогда
		...
		КонецЕсли;
		ТекстСостояния = НСтр("ru = 'Выполняется рассылка отчетов'; en = 'Report delivery in progress'");
	Иначе
		ТекстСостояния = НСтр("ru = 'Выполняются рассылки отчетов'; en = 'Report deliveries in progress'");
	КонецЕсли;
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, Параметры) Экспорт
	...
	Иначе
		...
		Предупреждение.Текст = НСтр("ru = 'Не удалось выполнить рассылки отчетов.'; en = 'Failed to send reports.'")
			+ Символы.ПС
