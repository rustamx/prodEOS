...
Процедура ПередатьФайлВоВнешнийОбъект(Знач КлючСессии, ВнешнийОбъект, Знач АдресДвоичныхДанных, Знач ИмяМетода, Знач Параметры) Экспорт
	...
	Исключение
		ДополнительныеОтчетыИОбработки.ЗаписатьПредупреждение(
			...
			|%2'; en = 'Could not delete a temporary file ""%1"":%2'"),
			ВременныйФайл,
...
Функция ПолучитьФайлИзИнтернета(Знач КлючСессии, Знач URL, Знач Порт = 0, Знач ИмяПользователя = "", Знач Пароль = "", Знач Таймаут = 20,Знач ЗащищенноеСоединение = Неопределено, Знач ПассивноеСоединение = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
		ВызватьИсключение НСтр("ru = 'В данной конфигурации не поддерживается вызов функции ДополнительныеОтчетыИОбработкиВБезопасномРежиме.ПолучитьФайлИзИнтернета!'; en = 'The application does not support ДополнительныеОтчетыИОбработкиВБезопасномРежиме.ПолучитьФайлИзИнтернета method.'");
	КонецЕсли;
	...
	Если ВРег(Протокол) = "FTP" Тогда
		...
		Исключение
			...
			СообщениеОбОшибке = НСтр("ru = 'Ошибка при создании FTP-соединения с сервером %1:'; en = 'Error creating ftp connection to server %1:'") + Символы.ПС + "%2";
	...
	Иначе
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			СообщениеОбОшибке = НСтр("ru = 'Ошибка при создании HTTP-соединения с сервером %1:'; en = 'Error creating http connection to server %1:'") + Символы.ПС + "%2";
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		СообщениеОбОшибке = НСтр("ru = 'Ошибка при получении файла с сервера %1:'; en = 'Failure on attempt to get a file from server %1:'") + Символы.ПС + "%2";
...
Функция ПередатьФайлВИнтернет(Знач КлючСессии, Знач АдресДвоичныхДанных, Знач URL, Знач Порт = 0, Знач ИмяПользователя = "", Знач Пароль = "", Знач Таймаут = 20,Знач ЗащищенноеСоединение = Неопределено, Знач ПассивноеСоединение = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
		ВызватьИсключение НСтр("ru = 'В данной конфигурации не поддерживается вызов функции ДополнительныеОтчетыИОбработкиВБезопасномРежиме.ПередатьФайлВИнтернет!'; en = 'The application does not support ДополнительныеОтчетыИОбработкиВБезопасномРежиме.ПередатьФайлВИнтернет method.'");
	КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		СообщениеОбОшибке = НСтр("ru = 'Ошибка при передаче файла на сервер %1:'; en = 'Error uploading file to server %1:'") + Символы.ПС + "%2";
...
Процедура ПроверитьКорректностьИмениМетодаВнешнегоОбъекта(Знач ИмяМетода)
	...
	Для Каждого ЗапрещенныйСимвол Из ЗапрещенныеСимволы Цикл
		...
		Если СтрНайти(ИмяМетода, ЗапрещенныйСимвол) > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 не является корректным именем метода для COM-объекта или объекта внешней компоненты!'; en = 'Invalid COM object or add-in object method name: %1.'"), ИмяМетода);
...
Процедура ПроверитьКорректностьВызоваПоОкружению()
	...
	Если Не ДополнительныеОтчетыИОбработкиВБезопасномРежимеСлужебный.ПроверитьКорректностьВызоваПоОкружению() Тогда
		...
		ВызватьИсключение НСтр("ru = 'Некорректный вызов функции общего модуля ДополнительныеОтчетыИОбработкиВБезопасномРежиме!
                                ...
                                |из сценария!';
                                |en = 'Incorrect function call common module ДополнительныеОтчетыИОбработкиВБезопасномРежиме!
                                |Exported functions of this module for use in safe mode should only be called from a script!'");
