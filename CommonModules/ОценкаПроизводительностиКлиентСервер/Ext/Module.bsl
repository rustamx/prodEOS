...
Функция НачатьЗамерВремениСлужебный(Параметры)
	...
	Если ТипЗнч(УИДЗамера) <> Тип("УникальныйИдентификатор") Тогда
		ВызватьИсключение НСтр("ru = 'Не верный тип УИДа замера.'; en = 'Type of measurement UID is incorrect.'");
	КонецЕсли;
	...
	Если ОценкаПроизводительностиВызовСервераПовтИсп.ВыполнятьЗамерыПроизводительности() Тогда
		...
		#Если Клиент Тогда
			Если Не ЗначениеЗаполнено(КлючеваяОперация) Тогда
				ВызватьИсключение НСтр("ru = 'Не указана ключевая операция.'; en = 'The key operation is not specified.'");
			КонецЕсли;
...
Процедура ЗаписатьВЖурналРегистрации(ИмяСобытия, Уровень, ТекстСообщения) Экспорт
	...
	ЗаписьЖурналаРегистрации(ИмяСобытия,
		Уровень,
		,
		НСтр("ru = 'Оценка производительности'; en = 'Performance monitor'"),
		ТекстСообщения);
