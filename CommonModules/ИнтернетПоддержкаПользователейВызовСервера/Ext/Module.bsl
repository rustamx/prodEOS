...
Процедура ДобавитьКомандыСервиса(
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ОтобразитьФормуОтсутствуетПодключение";
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
				НСтр("ru = 'Ошибка при получении данных с сервера Интернет-поддержки.'; en = 'Error retrieving data from online support site.'");
		КонецЕсли;
...
Процедура ЗаписатьОбщиеПараметрыИПП(Знач ОбщиеПараметры) Экспорт
	...
	Если Не ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП() Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для записи параметров Интернет-поддержки.'; en = 'Insufficient permissions to save Online support parameters.'");
	КонецЕсли;
...
Функция ДопПараметрыОтправкиСообщения(Знач ДопПараметры) Экспорт
	...
	Вложения.Добавить(
		Новый Структура(
			"Представление, Текст",
			НСтр("ru = 'Техническая информация.txt'; en = 'Technical information.txt'"),
...
Функция ШаблонТекстаСообщения(ДопПараметры)
	...
	Возврат НСтр("ru = 'Здравствуйте!
			...
			|%msgtxt 
			|
			|Software product registration number: <Specify reg. number>; 
...
Функция ТекстВложенияТехническаяИнформация(ВидПриложения, ДопПараметры)
	...
	Если ИмяПрограммы = "Unknown" Тогда
		ИмяПрограммы = НСтр("ru = '<Не заполнено>'; en = '<Is not filled>'");
	КонецЕсли;
	...
	Результат = НСтр("ru = 'Техническая информация о программе:
		...
		|Program desctiption: %1;
		|Program name: %2;
		|Program version: %3;
		|Vendor: %4;
		|1C:Enteprise platform version: %5;
		|Online support library version: %6;
		|Standard subsystems library version: %7;
		|Application type: %8;
		|Mode: %9;'");
	...
	Результат =
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ВидПриложения,
			?(ЭтоФайловаяИБ, НСтр("ru = 'Файловый'; en = 'File'"), НСтр("ru = 'Серверный'; en = 'Server'")));
	...
	Результат = Результат
		...
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Полные права: %1;
				...
				|en = 'Full permissions: %1;
				|Administrator permissions: %2;
				|Permissions for connection to Online support: %3;'"),
			?(Пользователи.ЭтоПолноправныйПользователь(, Ложь, Ложь), НСтр("ru = 'есть'; en = 'has'"), НСтр("ru = 'нет'; en = 'no'")),
			?(Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь), НСтр("ru = 'есть'; en = 'has'"), НСтр("ru = 'нет'; en = 'no'")),
			?(ИнтернетПоддержкаПользователей.ПравоЗаписиПараметровИПП(), НСтр("ru = 'есть'; en = 'has'"), НСтр("ru = 'нет'; en = 'no'")));
	...
	Результат = Результат
		...
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Логин для подключения Интернет-поддержки: %1;
				...
				|en = 'Login for Online support: %1;
				|Password for Online support: %2;
				|Software product registration number: %3;'"),
			ДопПараметры.Логин,
			?(ДопПараметры.ПарольЗаполнен, НСтр("ru = 'заполнен'; en = 'filled'"), НСтр("ru = 'не заполнен'; en = 'not filled'")),
	...
	Результат = Результат
		...
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Настройки соединения с серверами Интернет-поддержки:
				...
				|	Доменная зона: %3';
				|en = 'Configure connection to Online support servers:
				|Timeout: %1;
				|Establish connection from 1С:Enterprise server: %2;
				|Domain zone: %3'"),
	...
	Если ДоступнаРаботаСНастройкамиКлиентаЛицензирования() Тогда
		...
		Если Не ПустаяСтрока(ИДКонфигурации) Тогда
			Результат = Результат + Символы.ПС
				+ НСтр("ru = 'Имя клиента лицензирования:'; en = 'Licensing client name:'") + " "
				+ КлиентЛицензирования.ИмяКлиентаЛицензирования()
				+ Символы.ПС + НСтр("ru = 'Идентификатор конфигурации:'; en = 'Configuration ID:'")
...
Функция СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейОшибка()
	...
	Возврат НСтр("ru = 'Интернет-поддержка пользователей.Ошибка'; en = 'Online customer support.Error'",
