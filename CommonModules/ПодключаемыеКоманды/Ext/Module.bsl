	...
	Исключение
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|%2';
				|en = 'When determining commands attached to form ""%1"", an error occurred:
				|%2'"),
				ИмяФормы,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Подключаемые команды'; en = 'Attachable commands'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ЗначениеЗаполнено(ОписаниеКоманды.Обработчик) И Не ПолучатьФорму Тогда
		...
		Если Вид = "ОБЩИЙМОДУЛЬ" Тогда
			...
			Если ОбъектМетаданныхОбщийМодуль = Неопределено Тогда
				ВызватьИсключение СтрШаблон(НСтр("ru = 'Общий модуль ""%1"" не найден.'; en = 'Common module ""%1"" not found.'"), МассивПодстрок[1]);
			КонецЕсли;
