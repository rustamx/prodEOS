...
Процедура ОбменСКонтрагентамиВладелецПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Не Источник.ДополнительныеСвойства.Свойство("ЭтоНовыйОбъект") Тогда
		...
		Если НЕ РедактированиеРазрешено Тогда
			ТекстСообщения = НСтр("ru='Существует электронный документ. Изменение ключевых реквизитов документа запрещено.'; en = 'Electronic document already exists. Key attributes of the document are now read-only.'");
...
Процедура ЗаполнитьЗначениеРеквизитаОрганизацияПриОдноФирменномУчете(Источник, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если НЕ ИспользуетсяНесколькоОрганизаций И ОбщегоНазначения.ЕстьРеквизитОбъекта("Организация", Источник.Метаданные())
		...
		Если Не ЗначениеЗаполнено(Источник.Организация) Тогда
			ВызватьИсключение НСтр("ru = 'Не удалось определить организацию.'; en = 'Unable to determine the company.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура РегламентноеПолучениеНовыхЭД() Экспорт
	...
	Текст = НСтр("ru = 'Начато регламентное задание по получению новых эл.документов.'; en = 'Started scheduled job of obtaining new e-documents.'");
	...
	Попытка
		...
		ШаблонСообщения = НСтр("ru = 'Закончено регламентное задание.
									...
									|en = 'Finished scheduled job.
									|Packets received: %1. Unpacked: %2.'");
	...
	Исключение
		...
		Операция = НСтр("ru = 'Регламентное получение электронных документов'; en = 'Routine receipt of electronic documents'");
		
		ШаблонСообщения = НСтр("ru = 'При процессе выполнении произошла ошибка: %1'; en = 'An error occurred while executing the process: %1'");
...
Процедура РегламентнаяОтправкаОформленныхЭД() Экспорт
	...
	Текст = НСтр("ru = 'Начато регламентное задание по отправке оформленных эл.документов.'; en = 'Started scheduled job by sending a decorated e-documents.'");
	...
	Попытка
		...
		Текст = НСтр("ru = 'Закончено регламентное задание. Отправлено пакетов: %КоличествоПакетов%.'; en = 'Finished scheduled job. Packets sent: %КоличествоПакетов%.'");
	...
	Исключение
		...
		|en = 'During a routine sending of generated documents an error occurred. 
...
Процедура ПроверитьНаличиеНовыхЭД() Экспорт
	...
	ВидОперации = НСтр("ru = 'Получение информации о новых электронных документах'; en = 'Getting information about new electronic documents'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Ошибка выполнения команды сервиса.'; en = 'Error executing the service command.'");
		...
		Если Не Ответ.КодСостояния = 200 Тогда
			...
			ШаблонСообщения = НСтр("ru = 'Код ответа сервиса %1'; en = 'Service response code %1'");
...
Функция ПолучитьFTPСоединение(ПрофильНастроекЭДО, ЭтоТест = Ложь) Экспорт
	...
	Если ЭтоТест Тогда
		РезультатТеста = НСтр("ru = 'Пройден успешно.'; en = 'Completed successfully.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, РезультатТеста);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ТестСвязиОбменаЧерезFTP(ПрофильНастроекЭДО, КаталогВходящихДокументов, КаталогИсходящихДокументов) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Проверка каталога исходящих документов.
							|%1';
							|en = 'Checking directory of outgoing documents. 
							|%1'");
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, РезультатТеста);
	ТекстСообщения = НСтр("ru = 'Тест обмена по профилю %1.'; en = 'Exchange test using profile %1.'") + " " + ТекстСообщения;
	...
	Если НЕ ЗначениеЗаполнено(ТекстОшибки) Тогда
		ШаблонСообщения = НСтр("ru = 'Проверка записи и чтения файлов в каталоге исходящих документов.
									|%1';
									|en = 'Check reading and writing files in the directory to outgoing documents. 
									|%1'");
	...
	ШаблонСообщения = НСтр("ru = 'Проверка каталога входящих документов.
		|%1';
		|en = 'Checking directory of incoming documents. 
		|%1'");
	Попытка
	...
	Если НЕ ЗначениеЗаполнено(ТекстОшибки) Тогда
		ШаблонСообщения = НСтр("ru = 'Проверка записи и чтения файлов в каталоге входящих документов.
									|%1';
									|en = 'Check reading and writing files in the directory of incoming documents. 
									|%1'");
		ПроверитьФайл(ШаблонСообщения, FTPСоединение, ТекстОшибки);
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			РезультатТеста = ТекстОшибки;
...
Функция СформироватьНовыйПакетЭД(СтруктураПараметров) Экспорт
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'создание нового ПакетаЭД'; en = 'creating ED package'"),
																					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
																					ТекстСообщения);
	КонецПопытки;
...
Процедура ОбновитьСтатусыДокументовПакетаЭД(ПакетЭД, НовыйСтатусПакетаЭД, ДатаИзменения) Экспорт
	...
	Если ПакетЭД.ЭлектронныеДокументы.Количество() = 0 Тогда
		...
		ШаблонОшибки = НСтр("ru = 'Документ %1 не был отправлен. Подробнее см. Журнал регистрации'; en = 'Document %1 has not been sent. For details, see event log'");
		...
		ШаблонОшибки = НСтр("ru = 'Не корректно заполнен документ %1.
			...
			|en = 'Document %1 is not filled in correctly.
			|Tabular section ""ОбменСКонтрагентами"" is not filled in'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
					ПакетЭД );
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ru = 'Отправка пакета ЭД и обновление статусов ЭД'; en = 'Sending ED package and updating ED statuses'"),
			ТекстОшибки,
			ТекстСообщения);
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			НСтр("ru = 'смена статуса пакета ЭД и обновление статусов ЭД'; en = 'change of status of package ED and update statuses ED'"), ТекстОшибки, ТекстСообщения);
	КонецПопытки;
...
Функция ПолучитьПредставлениеЭД(СсылкаНаФайлЭД) Экспорт
	...
	Иначе
		...
		ИначеЕсли РеквизитыЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.СЧФДОПУПД
			...
			Представление = НСтр("ru = 'Информация продавца №'; en = 'Seller information №'") + " " + РеквизитыЭД.НомерДокументаОтправителя + ТекстДаты;
		Иначе
			Представление = "" + РеквизитыЭД.ВидЭД + " № " + РеквизитыЭД.НомерДокументаОтправителя + ТекстДаты
			+ ТекстВерсии;
		КонецЕсли;
...
Функция ОпределитьПредставлениеЭД(ВидЭД, СтруктураПараметров) Экспорт
	...
	Если ВидЭД <> Перечисления.ВидыЭД.СчетФактура И ВидЭД <> Перечисления.ВидыЭД.ИзвещениеОПолучении
		...
		ШаблонТекстаВерсии = НСтр("ru = '(версия %1)'; en = '(version %1)'");
	...
	Если СтруктураПараметров.Свойство("ТипЭД", ТипЭД)
		...
		Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
			...
			ТекстВидЭД = НСтр("ru = 'Информация покупателя'; en = 'Buyer information'");
		...
		ИначеЕсли ЭтоИзвещениеОПолучении(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ru = 'Извещение о получении'; en = 'notification on receipt'");
		ИначеЕсли ЭтоПодтверждение(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ru = 'Подтверждение'; en = 'Confirmation'");
			
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ПДПЭСФ
				ИЛИ ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ПДП Тогда
				ТекстВидЭД = ТекстВидЭД + " " + НСтр("ru = 'даты получения'; en = 'receiving dates'");
			Иначе
				ТекстВидЭД = ТекстВидЭД + " " + НСтр("ru = 'даты отправки'; en = 'sending dates'");
			КонецЕсли;
			
		ИначеЕсли ЭтоУведомлениеОбУточнении(ТипЭД) Тогда
			ТекстВидЭД = НСтр("ru = 'Уведомление об уточнении'; en = 'Notification on clarification'");
		ИначеЕсли ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ПОА Тогда
			ТекстВидЭД = НСтр("ru = 'Предложение об аннулировании'; en = 'Cancellation proposal'");
		КонецЕсли;
	Иначе
		
		Если ВидЭД = Перечисления.ВидыЭД.СчетФактура Тогда
			ТекстВидЭД = НСтр("ru = 'Счет-фактура'; en = 'VAT invoice'");
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.СЧФДОПУПД Тогда
				ТекстВидЭД = НСтр("ru = 'Информация продавца'; en = 'Seller information'");
			КонецЕсли;
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.КорректировочныйСчетФактура Тогда
			ТекстВидЭД = НСтр("ru = 'Корректировочный счет-фактура'; en = 'Corrective VAT invoice'");
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.КСЧФДИСУКД Тогда
				ТекстВидЭД = НСтр("ru = 'Информация продавца'; en = 'Seller information'");
			КонецЕсли;
			
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ТОРГ12 Тогда
			ТекстВидЭД = НСтр("ru = 'Товарная накладная (ТОРГ-12)'; en = 'Packing slip (ТОРГ-12)'");
		...
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.АктВыполненныхРабот Тогда
			ТекстВидЭД = НСтр("ru = 'Акт'; en = 'Statement'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.АктНаПередачуПрав Тогда
			ТекстВидЭД = НСтр("ru = 'Акт на передачу прав'; en = 'Assignment and transfer agreement'");
			Если ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ДОПУПД Тогда
				ТекстВидЭД = НСтр("ru = 'Информация продавца'; en = 'Seller information'");
			КонецЕсли;
		...
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.СчетНаОплату Тогда
			ТекстВидЭД = НСтр("ru = 'Счет'; en = 'Account'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ЗаказТовара Тогда
			ТекстВидЭД = НСтр("ru = 'Заказ товара'; en = 'Purchase order'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтветНаЗаказ Тогда
			ТекстВидЭД = НСтр("ru = 'Ответ на заказ'; en = 'Reply to order'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ПрайсЛист Тогда
			ТекстВидЭД = НСтр("ru = 'Прайс-лист'; en = 'Price list'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
			ТекстВидЭД = НСтр("ru = 'Каталог товаров'; en = 'Products catalog'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтчетОПродажахКомиссионногоТовара Тогда
			ТекстВидЭД = НСтр("ru = 'Отчет о продажах комиссионного товара'; en = 'Commissionaire report'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ОтчетОСписанииКомиссионногоТовара Тогда
			ТекстВидЭД = НСтр("ru = 'Отчет о списании комиссионного товара'; en = 'Commissionaire write-off report'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ПередачаТоваровМеждуОрганизациями Тогда
			ТекстВидЭД = НСтр("ru = 'Передача Интеркампани'; en = 'Intercompany transfer'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ВозвратТоваровМеждуОрганизациями Тогда
			ТекстВидЭД = НСтр("ru = 'Возврат Интеркампани'; en = 'Return of intercompany payments'");
		ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
			ТекстВидЭД = НСтр("ru = 'Произвольный электронный документ'; en = 'Arbitrary electronic document'");
		КонецЕсли;
	КонецЕсли;
...
Функция УстановленныеПодписиВалидны(ЭД) Экспорт
	...
	Если Результат.Количество() > 0 Тогда
		...
		|en = 'Processing electronic document %1. 
		|Document was not processed because it contains invalid signature.'");
...
Функция ПолучитьНовыеЭД(СоотвСоглашенийИСтруктурСертификатов = Неопределено, ПовторноеПолучение = Ложь) Экспорт
	...
	Для Каждого СтрокаРесурсовЭП Из МассивРесурсовЭП Цикл
		...
		Исключение
			...
			|%1';
			|en = 'Error occurred while retrieving messages from mail server. 
			|%1'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ТекстОшибки);
			
			ТекстСообщения = НСтр("ru = 'Ошибка при получении новых эл.документов.'; en = 'Error occured while obtaining new electronic documents.'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				НСтр("ru = 'Получение новых эл.документов'; en = 'Receiving new electronic documents'"), ТекстОшибки, ТекстСообщения);
			Продолжить;
		КонецПопытки;
		...
		Для Каждого Сообщение Из НаборСообщений Цикл
			...
			Если СтрНайти(Сообщение.Тема, НСтр("ru = 'Подтверждение получения пакета электронных документов.'; en = 'Acknowledgement of receipt of the package of electronic documents.'")) Тогда
	...
	Попытка
		...
		Если ПоверятьОбязательныеРеквизиты Тогда
			...
			Для Каждого КлючЗначение Из СтруктураОбязательныхПолей Цикл
				...
				Если СтруктураЭД.Свойство(КлючЗначение.Ключ, Значение) Тогда
					Если НЕ ЗначениеЗаполнено(Значение) Тогда
						Текст = НСтр("ru = 'Значение параметра <%1> не заполнено.'; en = 'Value of parameter <%1> is not filled in.'");
					...
					КонецЕсли;
				Иначе
					Текст = НСтр("ru = 'Не передан обязательный параметр <%1>.'; en = 'The required parameter was not passed: <%1>.'");
					Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, КлючЗначение.Ключ);
					ТекстОшибки = ?(ЗначениеЗаполнено(ТекстОшибки), ТекстОшибки + Символы.ПС + Текст, Текст);
				КонецЕсли;
			КонецЦикла;
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				...
				|%1';
				|en = 'Error while filling in additional properties of ED.
				|%1'");
		...
		Для Каждого НовыеПриглашения Из ТЗПоследнихИзменений Цикл
			...
			Если Контрагент = Неопределено Тогда
				...
				Если Не Ошибка Тогда
					...
					|КПП: %4
					|Статус: %5.';
					|en = 'Exchange according to profile %1.
					|No counterparty in the infobase:
					|Description: %2
					|TIN: %3
					|TRRC: %4
					|State: %5.'");
				...
				Иначе
					ОписаниеОшибки = ?(ЗначениеЗаполнено(НовыеПриглашения.ОписаниеОшибки),
						НовыеПриглашения.ОписаниеОшибки, НСтр("ru = 'Ошибка'; en = 'Error'"));
					...
					|Статус: %7.';
					|en = 'Exchange according to profile %1.
					|%2 in invitation for counterparty:
					|Description: %3
					|Email: %4
					|TIN: %5
					|TRRC: %6
			...
			Если НовыеПриглашения.СтатусУчастника = Перечисления.СтатусыУчастниковОбменаЭД.Присоединен Тогда
				...
				Если Выборка.Следующий() Тогда
					...
					НастройкаЭДО.Комментарий           = НСтр("ru = '##Автоматически снят статус ""присоединен"". Уже существует настройка ЭДО по выбранному контрагенту.'; en = '##State ""attached"" was removed automatically. EDI configuration already exists for the selected counterparty.'");
				КонецЕсли;
			...
			ТекстОписанияОшибки = "";
			Если Ошибка Тогда
				ШаблонОписанияОшибки = НСтр("ru = '%1. Повторите отправку.'; en = '%1. Send again.'");
		...
		Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
			...
			Если СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					...
					КонецЕсли;
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Распаковка пакета ЭД'; en = 'Unpacking ED package'"),
...
Функция КорректноеИмяФайла(Знач СтрИмяФайла, УдалятьНекорректные = Ложь) Экспорт
	...
	Если Не Результат Тогда
		...
		|Имя файла: %2';
		|en = 'File names must not contain any of the following characters: %1 
		|File name: %2'");
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		Если СтруктураОбмена.Свойство("МассивФайлов") И СтруктураОбмена.МассивФайлов.Количество() > 0 Тогда
			...
			Если Не АдресАрхива = Неопределено Тогда
				...
				ПараметрыФайла.Вставить("Автор", Пользователи.АвторизованныйПользователь());
				ПараметрыФайла.Вставить("ВладелецФайлов", НовыйЭлектронныйДокумент); 
				ПараметрыФайла.Вставить("ИмяБезРасширения", НСтр("ru = 'Дополнительные файлы'; en = 'Additional files'"));
...
Функция ЗагрузитьСтрокуXMLВДеревоЗначений(СтрокаXML, ОписаниеОшибки = Неопределено, ЧтениеXML = Неопределено, Знач ТекущийУзел = Неопределено) Экспорт
	...
	Если ПерваяИтерация Тогда
		...
		Исключение
			ОписаниеОшибки = НСтр("ru = 'Ошибка разбора XML:'; en = 'Error while parsing XML:'") + " " + ИнформацияОбОшибке().Описание + ".";
			Возврат Неопределено;
		КонецПопытки;
	КонецЕсли;
...
Функция БилетНаСайтПоддержки(Аутентификация, ПоказыватьОшибки = Истина) Экспорт
	...
	Исключение
		...
		Если СтрНайти(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке), "IncorrectLoginOrPasswordApiException") > 0 Тогда
			КраткоеПредставлениеОшибки = НСтр("ru = 'Некорректный логин или пароль пользователя.'; en = 'Wrong login or password.'");
		КонецЕсли;
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось подключиться к пользовательскому сайту по причине:
					   ...
					   |en = 'Failed to connect to user site due to:
					   |%1
					   |You should:
					   |Check login/password;
					   |Check the existence of ITS by reg. number;
					   |Check the validity of ITS contract at the site;
					   |Check the version of the platform;
					   |Check the authorization at the Online support site;
					   |Add 1C:Enterprise platform to the list of exclusions of antivirus 
					   |software/firewall.'"),
...
Процедура ОтправитьРегистрационныйПакет1СЭДО(Знач Файл, ПараметрыАутентификации, ЕстьОшибки) Экспорт
	...
	Исключение
		...
		ШаблонЗаголовкаСообщения = НСтр("ru = 'При отправке заявления о регистрации: %1 на сервисе 1С-ЭДО возникла ошибка:'; en = 'When submitting the request for registration: %1 an error occurred at the 1C-EDI service:'");
		...
		Если ЗначениеЗаполнено(Результат) Тогда
			...
			Если МассивПараметровОшибки.Количество() >= 7 Тогда
				...
				ШаблонОшибки = НСтр("ru = '%1: %2.'; en = '%1: %2.'");
		...
		ШаблонСообщения = НСтр("ru = '%1
			...
			|Check the existence of ITS by the reg. number; 
			|Check the validity of ITS contract at the site; 
			|Check that the platform is up to date (access to ports 80 and 443); 
			|Check the authorization at the online support site; 
			|Add the platform to the list of exclusions of your antivirus software and your firewall.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
			ТекстСообщения);
...
Функция ПолучитьФайлОтветаОтСервераЭДО(АдресРесурса, ПараметрЗапросаОтветаОтСервера = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстЗаголовкаСообщения = НСтр("ru = 'При работе с сервисом 1С-ЭДО возникла ошибка:'; en = 'There was an error while working with 1C-EDI service:'");
...
Процедура ПроверитьКонтрагентаФоновоеЗадание(Параметры) Экспорт
	...
	Исключение
		...
		Операция = НСтр("ru = 'Проверка подключения контрагента к 1С-ЭДО из формы контрагента.'; en = 'Checking connection of counterparty to 1C-EDI from the counterparty form.'");
...
Процедура ПолучитьРезультатПроверкиВебСервисом(ДанныеКонтрагентов, Отбор)
	...
	Для НомерПорции = 1 По КоличествоЗапросов Цикл 
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru = 'При проверке состояния контрагентов на сервисе 1С-ЭДО возникла ошибка:'; en = 'An error occurred when checking the status of counterparties at 1С-EDI:'");
...
Процедура СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента)
	...
	Исключение
		...
		Операция = НСтр("ru = 'Запись результатов проверки подключения контрагентов к 1С-ЭДО в регистр.'; en = 'Saving to register results of checking connections of counterparties to 1C-EDI'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(Операция, ПодробныйТекстОшибки);
	КонецПопытки;
...
Функция УведомленияПодключены(ИдентификаторОрганизации, Соединение = Неопределено) Экспорт
	...
	ВидОперации = НСтр("ru = 'Получение информации о состоянии уведомлений'; en = 'Getting information on the status of the notifications'");
...
Процедура ОбработатьПодтверждениеПолучения(Сообщение, ЭтоФайл = Ложь)
	...
	Иначе
		Если СтрНайти(Сообщение.Тема, НСтр("ru = 'Подтверждение получения пакета электронных документов'; en = 'Confirmation of receipt of the package of electronic documents'")) > 0 Тогда
...
Процедура ОтправитьПодтверждениеПоПакету(ПакетЭД, РесурсОтправителя, АдресОтправителя, ТипОтправки)
	...
	Если ЗначениеЗаполнено(ВыборкаПрисоединенныхФайлов) И ВыборкаПрисоединенныхФайлов.Следующий() Тогда
		Если ТипОтправки = Перечисления.СпособыОбменаЭД.ЧерезЭлектроннуюПочту Тогда
			...
			ПараметрыОтправки.Вставить("Тема",     НСтр("ru = 'Подтверждение получения пакета электронных документов:'; en = 'Confirmation of receipt of the package of electronic documents'") + " " + ИмяПакетаЭД);
			...
			Исключение
				...
				|%1';
				|en = 'Error occurred while sending message to mail server. 
				|%1'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ТекстОшибки);
				
				ТекстСообщения = НСтр("ru = 'Ошибка при отправке эл.документов.'; en = 'Error occured while sending electronic documents.'");
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Отправка эл.документов'; en = 'Sending e-documents'"),
				ТекстОшибки,
				ТекстСообщения);
			КонецПопытки;
...
Функция НеобходимоПолучить(Сообщение, СтруктураПараметров)
	...
	Если Сообщение.Вложения.Количество() = 1 Тогда 
		...
		Если СтрНайти(Сообщение.Тема, НСтр("ru = 'Подтверждение'; en = 'Confirmation'")) Тогда
			Возврат Истина;
		КонецЕсли;
		
		Если СтрНайти(Сообщение.Тема, НСтр("ru = 'Обмен эл.документами:'; en = 'ED exchange:'")) Тогда
...
Функция НеобходимоПолучитьДвоичныеДанные(ДвоичныеДанные, ИмяФайла, СтруктураПараметров)
	...
	Если НЕ ФайлКарточки = Неопределено Тогда
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru = 'Возникла ошибка при чтении данных из файла %1: %2.'; en = 'An error occurred while reading data from the file %1: %2.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				ФайлКарточки.ПолноеИмя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Чтение ЭД.'; en = 'Reading ED.'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				ТекстСообщения);
		КонецПопытки;
...
Процедура ОбновитьСтатусыЭД(МассивДокументов, ПризнакПодписи)
	...
	Для Каждого ЭлектронныйДокумент Из МассивДокументов Цикл
		...
		Исключение
			...
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'подготовка к отправке ЭД'; en = 'preparing to send ED'"),
																						ТекстОшибки,
																						ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
...
Процедура ЗаписатьДатуПолучения(ЭД, ДатаИзменения)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'запись даты получения ЭД'; en = 'record the date of receipt of the ED'"),
																					ТекстОшибки,
																					ТекстСообщения);
	КонецПопытки;
...
Процедура ЗаписатьДатуОтправки(ЭД, ДатаИзменения)
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'запись даты отправки ЭД'; en = 'record the date of sending ED'"),
																					ТекстОшибки,
																					ТекстСообщения);
	КонецПопытки;
...
Процедура ПроверитьФайл(ШаблонСообщения, FTPСоединение, ТекстОшибки)
	...
	ТестоваяСтрока = НСтр("ru = 'Тестовая строка 1С: Предприятие'; en = 'Test string 1C: Enterprise'");
	...
	Если НЕ СтрокаРезультата = ТестоваяСтрока Тогда
		ШаблонСообщения = НСтр("ru = '%1 %2.'; en = '%1 %2.'");
...
Функция СоздатьПакетЭД(РеквизитыДокумента, МассивЭД, ТекстСообщения = "")
	...
	Исключение
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Операция = НСтр("ru = 'формирование пакета ЭД'; en = 'Generating of ED package'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция АрхивДополнительныхФайлов(МассивФайлов) Экспорт
	...
	Исключение
		...
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()) + Символы.ПС
			...
			|и наличие доступа к каталогу временных файлов.'; en = 'Check the support of Russian language in regional settings of the OS for non-Unicode programs and access to the directory for temporary files.'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'запись файла архива на диск'; en = 'saving archive file to disk'"),
																					ТекстОшибки,
																					ТекстСообщения);
...
Процедура СообщитьОбОтсутствииСоглашенияЭД(ПараметрыЭД, Источник)
	...
	ШаблонСообщения = НСтр("ru = 'Обработка %1.
								 ...
								 |One should create ""EDI settings"" first with attributes:'");
		...
		Если Порядок > 0 Тогда
			...
			Если ЭтоСчетФактураВыданный Тогда
				Если НРег(ТекПараметр.Ключ) = НРег("ЭтоСчетФактураВыданный") Тогда
					СтрокаПараметров.Ключ = НСтр("ru = 'Способ обмена'; en = 'Exchange method'");
					СтрокаПараметров.Значение = НСтр("ru = 'через оператора ЭДО'; en = 'Via EDI operator'");
				ИначеЕсли НРег(ТекПараметр.Ключ) = НРег("Контрагент") Тогда
	...
	Для Каждого СтрокаПараметров Из ТаблицаПараметров Цикл
		Если ЗначениеЗаполнено(СтрокаПараметров.Значение) Тогда
			Текст = Текст + Символы.ПС + НСтр("ru = '<%1>: %2'; en = '<%1>: %2'");
...
Функция ЕстьДоступКВебСервису1CЭДО(ВыводитьСообщения = Ложь) Экспорт
	...
	Исключение
		...
		|en = '%1
		|1С-EDI service is unavailable. Please try later.'");
...
Функция ПрисоединенныйФайл(ВладелецФайла, ТипЭлементаВерсииЭД = Неопределено,СообщатьОбОшибке = Ложь) Экспорт
	...
	ИначеЕсли СообщатьОбОшибке Тогда
		Операция = НСтр("ru = 'Получение присоединенного файла'; en = 'Getting attached file'");
		ТекстОшибки = НСтр("ru = 'Не обнаружен присоединенный файл документа %1'; en = 'Not found attached file for document %1'");
...
Функция СписокОперацийВидаЭД(ВидЭД, ДобавлятьПредопределенныеСпособы = Ложь, Префикс = "") Экспорт 
	...
	Если ДобавлятьПредопределенныеСпособы Тогда
		Если СписокОпераций.Количество() = 0 Тогда
			СписокОпераций.Добавить("Автоматически", НСтр("ru = 'Автоматически'; en = 'Automatically'"),Истина);
		КонецЕсли;
		
		СписокОпераций.Добавить("Вручную", НСтр("ru = 'Вручную в форме электронного документа'; en = 'Manually in the form of electronic document'"));
	КонецЕсли;
...
Функция ПолучитьОжидаемоеДействиеКонтрагента(Строка,ВладелецФайла)
	...
	Если Строка.ВидЭД = Перечисления.ВидыЭД.ИзвещениеОПолучении
		...
		Если Строка.ВидЭД = Перечисления.ВидыЭД.Подтверждение И НЕ ЗначениеЗаполнено(Строка.Ссылка)
			...
			ВозвращаемоеЗначение = НСтр("ru = 'Подтверждения оператора ЭДО'; en = 'Confirmations of EDI operator'");
		ИначеЕсли (Строка.ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ИПУУЭСФ
			...
			ВозвращаемоеЗначение = НСтр("ru = 'Извещения о получении'; en = 'Notice of receipt'");
		КонецЕсли;
		
	Иначе
		Если Строка.СтатусЭД = Перечисления.СтатусыЭД.ПереданОператору Тогда
			
			ВозвращаемоеЗначение = НСтр("ru = 'Подтверждения отправки'; en = 'Send confirmation'");
		...
		ИначеЕсли Строка.СтатусЭД = Перечисления.СтатусыЭД.Отправлен Тогда
			...
			ВозвращаемоеЗначение = НСтр("ru = 'Подтверждения о доставке'; en = 'Confirmation of delivery'");
		КонецЕсли;
	КонецЕсли;
...
Функция ВерсияСхемыФНС_2016() Экспорт
	Возврат НСтр("ru = 'ФНС 5.01 (с 2016г.)'; en = 'FTS 5.01 (since 2016г.)'");
КонецФункции
