...
Функция Распознать(ТекущаяВерсия, РасширениеФайлаРезультата, ОписаниеОшибки, РаспознанныйТекст, ПутьКФайлу) Экспорт
	...
	Исключение
		ОписаниеОшибки = НСтр("ru = 'Не удалось загрузить компоненту загрузки CuneiForm'; en = 'Could not load component for loading CuneiForm'");
		Возврат "Ошибка";
	КонецПопытки;
	...
	ОписаниеОшибки = НСтр("ru = 'Не удалось распознать картинку. Возможно она не содержит текста для распознавания. '; en = 'Could not recognize picture. Perhaps it contains no text for recognition. '");
