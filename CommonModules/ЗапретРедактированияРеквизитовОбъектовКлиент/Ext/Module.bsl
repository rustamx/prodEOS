...
Процедура ПроверитьСсылкиНаОбъект(Знач ОбработкаПродолжения, Знач МассивСсылок, Знач СинонимыРеквизитов) Экспорт
	...
	ЗаголовокДиалога = НСтр("ru = 'Разрешение редактирования реквизитов'; en = 'Editing permission details'");
	...
	Если СинонимыРеквизитов.Количество() > 1 Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для того чтобы не допустить рассогласования данных в программе,
			           ...
			           |en = 'In order to prevent misalignment of data, attributes are not available for editing:
			           |%1.
			           |
			           |Before allowing them to edit, it is recommended that you assess the impact by checking all sites use this item in the program.
			           |Search places use can take a long time.'"),
			РеквизитыПредставление);
	Иначе
		Если МассивСсылок.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для того чтобы не допустить рассогласования данных в программе,
				           ...
				           |Поиск мест использования может занять длительное время.';
				           |en = 'In order to prevent misalignment of data in your program, attribute %1 is not available for editing.
				           |
				           |Before allowing its editing, we recommend that you evaluate the consequences by checking all the sites use ""%2"" in the program.
				           |Search places use can take a long time.'"),
		...
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для того чтобы не допустить рассогласования данных в программе,
				           ...
				           |Поиск мест использования может занять длительное время.';
				           |en = 'In order to prevent misalignment of data in your program, attribute %1 is not available for editing.
				           |Before allowing its editing, we recommend that you evaluate the consequences by checking all the sites use selected items (%2) in the program.
				           |Search places use can take a long time.'"),
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Проверить и разрешить'; en = 'Check and resolve'"));
...
Процедура ПоказатьПредупреждениеВсеВидимыеРеквизитыРазблокированы(ОбработкаПродолжения = Неопределено) Экспорт
	...
	ПоказатьПредупреждение(ОбработкаПродолжения,
		НСтр("ru = 'Редактирование всех видимых реквизитов объекта уже разрешено.'; en = 'Edit all visible details of the object is already permitted.'"));
