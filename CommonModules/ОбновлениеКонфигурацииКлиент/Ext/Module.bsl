...
Функция ЗаголовокСозданияРезервнойКопии(Параметры) Экспорт
	...
	Если Параметры.СоздаватьРезервнуюКопию = 0 Тогда
		Результат = НСтр("ru = 'Не создавать резервную копию ИБ'; en = 'Do not create a backup'");
	ИначеЕсли Параметры.СоздаватьРезервнуюКопию = 1 Тогда
		Результат = НСтр("ru = 'Создавать временную резервную копию ИБ'; en = 'Create a temporary backup'");
	ИначеЕсли Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		Результат = НСтр("ru = 'Создавать резервную копию ИБ'; en = 'Create a backup'");
	КонецЕсли;
	
	Если Параметры.ВосстанавливатьИнформационнуюБазу Тогда
		Результат = Результат + " " + НСтр("ru = 'и выполнять откат при нештатной ситуации'; en = 'and roll back when under stress conditions'");
	Иначе
		Результат = Результат + " " + НСтр("ru = 'и не выполнять откат при нештатной ситуации'; en = 'and do not roll back when under stress conditions'");
	КонецЕсли;
...
Функция ВозможнаУстановкаОбновления(Параметры, ПараметрыАдминистрирования)
	...
	Если ЭтоФайловаяБаза И Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		...
		Если Не Файл.Существует() Или Не Файл.ЭтоКаталог() Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Укажите существующий каталог для сохранения резервной копии ИБ.'; en = 'Specify an existing directory for saving backup.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.РежимОбновления = 0 Тогда // Обновить сейчас
		...
		Если ЭтоФайловаяБаза И ОбновлениеКонфигурацииВызовСервера.НаличиеАктивныхСоединений(ПараметрыПриложения[ИмяПараметра]) Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Невозможно продолжить обновление конфигурации, так как не завершены все соединения с информационной базой.'; en = 'Configuration update cannot proceed because not all the connections to the infobase are terminated.'"));
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли Параметры.РежимОбновления = 2 Тогда
		...
		Если Параметры.ВыслатьОтчетНаПочту
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Укажите допустимый адрес электронной почты.'; en = 'Specify a valid e-mail address.'"));
			Возврат Ложь;
		КонецЕсли;
		...
		Если Не ЗапланироватьОбновлениеКонфигурации(Параметры, ПараметрыАдминистрирования) Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Не удалось запланировать обновление конфигурации. Сведения об ошибке сохранены в журнал регистрации.'; en = 'Failed to schedule the configuration update. Error information stored in the log.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗапуститьСкриптОбновления(Параметры, ПараметрыАдминистрирования)
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Информация",
		НСтр("ru = 'Выполняется процедура обновления конфигурации:'; en = 'Configuration update procedure is executed:'") + " " + ИмяГлавногоФайлаСкрипта);
...
Функция ДатаОбновленияУказанаВерно(Параметры)
	...
	Если Параметры.ДатаВремяОбновления < ТекущаяДата Тогда
		ТекстСообщения = НСтр("ru = 'Обновление конфигурации может быть запланировано только на будущую дату и время.'; en = 'Configuration update can be scheduled only at a future date and time.'");
	ИначеЕсли Параметры.ДатаВремяОбновления > ДобавитьМесяц(ТекущаяДата, 1) Тогда
		ТекстСообщения = НСтр("ru = 'Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.'; en = 'Configuration update can be scheduled not later than one month from the current date.'");
	КонецЕсли;
...
Функция УдалитьЗадачуПланировщикаScheduledJob(КодЗадачи)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Ошибка при удалении задачи планировщика: %1'; en = 'Error deleting scheduler task: %1'");
	...
	Если Не Результат Тогда // Коды ошибок: http<<ВНЕШНЕЕСОЕДИНЕНИЕ>>msdn2.microsoft.com/en-us/library/aa389957(VS.85).aspx.
		ТекстСообщения = НСтр("ru = 'Ошибка при удалении задачи планировщика: %1'; en = 'Error deleting scheduler task: %1'");
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КодОшибки));
		Возврат Результат;
	КонецЕсли;
	ТекстСообщения = НСтр("ru = 'Задача планировщика успешно удалена (код задачи: %1).'; en = 'Task of scheduler has successfully removed (task ID: %1).'");
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Информация",
...
Функция СоздатьЗадачуПланировщикаScheduledJob(Знач ИмяФайлаПрограммы, Знач ДатаВремя)
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при создании задачи планировщика: %1'; en = 'Error creating scheduler task: %1'"), ОписаниеОшибки());
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ТекстСообщения);
		Возврат 0;
	КонецПопытки;
	
	Если КодОшибки <> 0 Тогда // Коды ошибок: http<<ВНЕШНЕЕСОЕДИНЕНИЕ>>msdn2.microsoft.com/en-us/library/aa389389(VS.85).aspx.
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при создании задачи планировщика: %1'; en = 'Error creating scheduler task: %1'"), КодОшибки);
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ТекстСообщения);
		Возврат 0;
	КонецЕсли;
	
	ТекстСообщения = НСтр("ru = 'Задача планировщика успешно запланирована (команда: %1; дата: %2; код задачи: %3).'; en = 'Task of scheduler has successfully planned (command: %1; date: %2; task ID: %3).'");
...
Функция СоздатьЗадачуПланировщикаScheduleService(Знач ИмяФайлаПрограммы, Знач ДатаВремя)
	...
	Попытка
		...
		ОписаниеЗадачи.RegistrationInfo.Description = НСтр("ru = 'Обновление конфигурации 1С:Предприятие'; en = 'Updating the configuration of 1C:Enterprise'");
...
Функция ИмяЗадачиScheduleService(Знач КодЗадачи)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновление конфигурации (%1)'; en = 'Update configuration (%1)'"), Формат(КодЗадачи, "ЧГ=0"));
...
#Если НЕ ВебКлиент Тогда
	...
	Если НастройкиОбновления.КонфигурацияИзменена Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Обновление конфигурации'; en = 'Configuration update'"),
			"e1cib/app/Обработка.УстановкаОбновлений",
			НСтр("ru = 'Конфигурация отличается от основной конфигурации информационной базы.'; en = 'Configuration differs from the basic information base configuration.'"), 
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	Если ПараметрыПриложения["СтандартныеПодсистемы.ПредлагатьОбновлениеИнформационнойБазыПриЗавершенииСеанса"] = Истина Тогда
		...
		ПараметрыПредупреждения.ТекстФлажка  = НСтр("ru = 'Установить обновление конфигурации'; en = 'Install configuration update'");
		ПараметрыПредупреждения.ТекстПредупреждения  = НСтр("ru = 'Запланирована установка обновления'; en = 'Software update installation was scheduled'");
