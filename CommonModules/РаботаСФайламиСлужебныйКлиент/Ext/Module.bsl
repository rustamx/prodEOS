...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	Если Не ПараметрыВыполнения.Свойство("ПолноеИмяФайла") Тогда
		...
		ДиалогВыбораФайла.Фильтр = НСтр("ru = 'Все файлы (*.*)|*.*'; en = 'All files (*. *) | *. *'");
	...
	ИначеЕсли Не ЗначениеЗаполнено(Редактирует) Тогда
		СтрокаОшибки = СтрШаблон(
			НСтр("ru = 'Невозможно освободить файл ""%1"",
			           |т.к. он никем не занят.'; en = 'It is impossible to unlock the file ""%1"" because it is not locked.'"),
			Строка(ОбъектСсылка));
		Возврат Ложь;
	Иначе
		...
		СтрокаОшибки = СтрШаблон(
			НСтр("ru = 'Невозможно освободить файл ""%1"",
			           |т.к. он занят пользователем ""%2"".'; en = 'It is impossible to unlock file ""%1"" because it is locked by user ""%2"".'"),
			Строка(ОбъектСсылка),
			Строка(Редактирует));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОсвободитьФайлБезВопроса(ДанныеФайла, УникальныйИдентификатор = Неопределено)
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Файл освобожден'; en = 'The file is unlocked'"),
...
Процедура ЗакончитьРедактированиеСРасширением(ПараметрыВыполнения)
			...
			Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
				СтрокаПредупреждения = СтрШаблон(
					НСтр("ru = 'Не удалось поместить файл
					           ...
					           |
					           |Освободить файл?';
					           |en = 'Unable to put the file ""%1"" (%2), in the file storage, because it is not found in the working directory. 
					           |
					           |Unlock file?'"),
			...
			Иначе
				СтрокаПредупреждения = СтрШаблон(
					НСтр("ru = 'Не удалось поместить файл ""%1"",
					           ...
					           |
					           |Освободить файл?';
					           |en = 'Unable to put the file ""%1"" in the file storage, because it was not found in the working directory. 
					           |
					           |Unlock file?'"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка));
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Не удалось поместить файл ""%1"" в хранилище файлов,
			|возможно он заблокирован другой программой.'; en = 'Unable to put the file ""%1"" in the file storage, perhaps it is locked by another program.'");
	...
	Иначе
		...
		Исключение
			...
			ТекстВопроса = СтрШаблон(
				...
				|
				|Повторить операцию?';
				|en = 'Unable to put file in the file storage because: ""%1"". 
				|
				|retry?'"),
	...
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		Если ПараметрыВыполнения.ВерсияОбновлена Тогда
			ШаблонПояснения = НСтр("ru = 'Файл ""%1""
			                             |обновлен и освобожден.'; en = 'File ""%1"" has been updated and unlocked.'");
		Иначе
			ШаблонПояснения = НСтр("ru = 'Файл ""%1""
			                             |не изменился и освобожден.'; en = 'File ""%1"" has not changed and was unlocked.'");
		КонецЕсли;
...
Процедура ЗакончитьРедактированиеСРасширениемОбработкаИсключения(ИнформацияОбОшибке, ПараметрыВыполнения)
	...
	ТекстВопроса = СтрШаблон(
		НСтр("ru = 'Не удалось поместить файл ""%1""
		           ...
		           |
		           |Повторить операцию?';
		           |en = 'Unable to put the file ""%1"" in the file storage because ""%2"". 
		           |
		           |Retry?'"),
		Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
		КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ЗакончитьРедактированиеБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	ОписаниеДанных.Вставить("Операция",            НСтр("ru = 'Шифрование файла'; en = 'Encrypting a file'"));
...
Процедура ОсвободитьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьВопрос(
		...
		НСтр("ru = 'Отмена редактирования файлов может
		           ...
		           |
		           |Продолжить?';
		           |en = 'Cancel editing of files can cause you to lose your changes. 
		           |
		...
		Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
			СтрокаПредупреждения = СтрШаблон(
				НСтр("ru = 'Не удалось поместить файл ""%1"" в хранилище файлов:
				           ...
				           |
				           |Освободить файл?';
				           |en = 'Unable to put the file ""%1"" in the file storage: file in the working directory not found: %2. 
				           |
				           |Unlock file?'"),
				Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
				ПараметрыВыполнения.ПолныйПутьКФайлу);
		Иначе
			СтрокаПредупреждения = СтрШаблон(
				НСтр("ru = 'Не удалось поместить файл ""%1"" в хранилище файлов:
				           ...
				           |
				           |Освободить файл?';
				           |en = 'Unable to put the file ""%1"" in the file storage, file not found in the working directory. 
				           |
				           |Unlock file?'"),
				Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка));
		КонецЕсли;
	...
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		...
		Иначе
			ПоказатьОповещениеПользователя(
				...
				НСтр("ru = 'Файл не изменился'; en = 'File has not changed'"),
				БиблиотекаКартинок.Информация32);
	...
	Если ЗадаватьВопрос = Истина Тогда
		ТекстВопроса =
			НСтр("ru = 'Удалить файл ""%1""
			           |из рабочего каталога?'; en = 'Delete the file ""%1"" from the working directory?'");
	...
	Если Результат.ВозниклаОшибка Тогда
		...
		ТекстПредупреждения = СтрШаблон(
			НСтр("ru = 'Не удалось скопировать обратно содержимое каталога
			           |""%1""
			           |в каталог
			           |""%2"".'; en = 'Failed to copy back the contents of the directory ""%1"" to directory ""%2"".'"),
			ПараметрыВыполнения.КаталогПриемника,
			ПараметрыВыполнения.КаталогИсточника);
...
Процедура ИмпортФайловПослеПроверкиРазмеров(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.КоличествоСуммарное = 0 Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, 
			НСтр("ru = 'Нет файлов для добавления'; en = 'No files to add'"), Неопределено);
		Возврат;
	КонецЕсли;
...
Процедура Зашифровать(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор) Экспорт
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		ТекстПредупреждения = НСтр("ru = 'Файл ""%1"" уже зашифрован.'; en = 'The file ""%1"" is already encrypted.'");
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ДанныеФайла.Редактирует) Тогда
		ТекстПредупреждения = НСтр("ru = 'Нельзя зашифровать занятый файл.'; en = 'You cannot encrypt a locked file.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстПредупреждения, ПараметрыВыполнения);
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий > 1 Тогда
		ПредставлениеФайла = ПредставлениеФайла + " (" + СтрШаблон(
			НСтр("ru = 'Версий: %1'; en = 'Versions: %1'"), ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий) + ")";
	КонецЕсли;
	...
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ru = 'Файлы (%1)'; en = 'Files (%1)'"));
...
Процедура ДобавитьИзФайловойСистемыБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	Если ПустаяСтрока(СтруктураПути.Расширение) Тогда
		ТекстВопроса = НСтр("ru = 'Необходимо указать файл с расширением.'; en = 'You must specify a file with the extension.'");
		Кнопки = Новый СписокЗначений;
...
Процедура ИнформироватьОШифровании(МассивФайловВРабочемКаталогеДляУдаления,
	...
	МодульЭлектроннаяПодписьКлиент.ИнформироватьОШифрованииОбъекта(
		СтрШаблон(
			НСтр("ru = 'Файл: %1'; en = 'File: %1'"), ФайлСсылка));
...
Процедура ПодписатьФайл(МассивФайлов, ИдентификаторФормы, ОбработчикЗавершения) Экспорт
	...
	ОписаниеДанных.Вставить("Операция",            НСтр("ru = 'Подписание файла'; en = 'Signing a file'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",     НСтр("ru = 'Файл'; en = 'File'"));
	ОписаниеДанных.Вставить("НаборДанных",         НаборДанных);
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("ru = 'Файлы (%1)'; en = 'Files (%1)'"));
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеЗаголовок",   НСтр("ru = 'Следующие файлы заняты вами для редактирования:'; en = 'The following files are locked for editing:'"));
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПредупреждениеПользователю.ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Открыть список редактируемых файлов (%1)'; en = 'Open a list of files being edited (%1)'"),
		Ответ.КоличествоЗанятыхФайлов);
	ПредупреждениеПользователю.ТекстПредупреждения = НСтр("ru = 'Имеются занятые для редактирования файлы'; en = 'You have files locked for editing'");
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Неверный путь или отсутствуют права на запись в каталог
				           |""%1"".'; en = 'Invalid path or you do not have write access to the directory ""%1"".'"),
				ИмяКаталога);
			...
			Если МассивФайловВКаталоге.Количество() <> 0 Тогда
				ТекстОшибки = СтрШаблон(
					НСтр("ru = 'В выбранном рабочем каталоге
					           ...
					           |уже есть файлы.
					           |
					           |Выберите другой каталог.'; en = 'Selected working directory (""%1"") already has files. Select another directory.'"),
					ИмяКаталога);
				ПоказатьПредупреждение(, ТекстОшибки);
				Возврат Ложь;
			КонецЕсли;
