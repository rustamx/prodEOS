...
Функция СоседнийЭлемент(ПеремещаемыйЭлемент, Список, Знач Направление)
	...
	Если Не ПоляОтбораВСпискеДоступных(КомпоновщикНастроекКомпоновкиДанных.ПолучитьНастройки().Отбор.Элементы, КомпоновщикНастроекКомпоновкиДанных.ПолучитьНастройки().Отбор.ДоступныеПоляОтбора.Элементы) Тогда
		ВызватьИсключение НСтр("ru = 'Для перемещения элемента необходимо отключить все отборы в списке.'; en = 'To move an item, you must disable all selections in the list.'");
	КонецЕсли;
...
Функция ПроверитьВозможностьПеремещения(Ссылка, Список, ОтображениеСписком)
	...
	Если Не ПараметрыДоступа.Доступность Тогда
		Возврат НСтр("ru = 'Недостаточно прав для изменения порядка элементов.'; en = 'Not enough permissions to change item order.'");
	КонецЕсли;
	...
	Для Каждого ЭлементГруппировки Из Список.КомпоновщикНастроек.ПолучитьНастройки().Структура Цикл
		Если ЭлементГруппировки.Использование Тогда
			Возврат НСтр("ru = 'Для изменения порядка элементов необходимо отключить все группировки.'; en = 'To change item order, you must disable all groups.'");
		КонецЕсли;
	КонецЦикла;
	...
	Если Информация.ЕстьРодитель И ОтображениеСписком И Не СписокСодержитОтборПоРодителю(Список) Тогда
		Возврат НСтр("ru = 'Для изменения порядка элементов необходимо установить режим просмотра ""Дерево"" или ""Иерархический список"".'; en = 'To change item order you must set view mode  to ""Tree"" or ""Hierarchical list""'");
	КонецЕсли;
	...
	Если Информация.ЕстьВладелец И Не СписокСодержитОтборПоВладельцу(Список) Тогда
		Возврат НСтр("ru = 'Для изменения порядка элементов необходимо установить отбор по полю ""Владелец"".'; en = 'To cahange item order, you must set filter by field ""Owner""'");
	КонецЕсли;
	...
	Если Информация.ЕстьГруппы Тогда
		...
		Если ЭтоГруппа И Не Информация.ДляГрупп Или Не ЭтоГруппа И Не Информация.ДляЭлементов Тогда
			Возврат НСтр("ru = 'Выбранный элемент нельзя перемещать.'; en = 'Selected item cannot be moved.'");
		КонецЕсли;
	КонецЕсли;
