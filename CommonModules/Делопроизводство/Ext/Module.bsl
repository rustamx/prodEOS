...
Процедура ЗаписатьСостояниеДокумента(Документ, Знач Период, Знач Состояние, Знач Установил) Экспорт
	...
	// ТСК Шепелев М.Н. Для документов блока КО статусы меняем самостоятельно 17.12.2018 {
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.ra_ZayavkaNaKontrolnuyuOperaciyu") И
			ТипЗнч(Установил) = Тип("БизнесПроцессСсылка.Согласование") Тогда
		Возврат;
	КонецЕсли;
	// ТСК Корнюшенков А.Ю. Искать текст "МаршрутыСогласованияЕОСК" 17.12.2018 }
