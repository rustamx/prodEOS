...
Функция ПолучитьМассивПунктовМеню(ОбъектБД) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектБД) = Тип("ДокументОбъект.ra_ZayavkaNaKontrolnuyuOperaciyu") Или
		ТипЗнч(ОбъектБД) = Тип("ДокументСсылка.ra_ZayavkaNaKontrolnuyuOperaciyu") Тогда
		
		Документы.ra_ZayavkaNaKontrolnuyuOperaciyu.ЗаполнитьГлавноеМенюОбъекта(ОбъектБД,МассивПунктовМеню);
	ИначеЕсли ра_ОбщегоНазначенияКлиентСервер.ЭтоДокументКачества(ОбъектБД, "ОценкаСоответствия") Тогда
		Документы.ra_ZayavkaNaOcenkuSootvetstviya3.ЗаполнитьГлавноеМенюОбъекта(ОбъектБД,МассивПунктовМеню);
	КонецЕсли;
...
Функция ПолучитьЗапрос(ТаблицаРеквизитов, ПараметрыЗапросаHTTP, ТекстЗапросаИЗ, ТекстЗапросаВложенныеТаблицы, ТекстЗапросаСоединений) Экспорт
	...
	УсловиеПометкиУдаления = СтрНайти(ТекстЗапросаИЗ, "Справочник") <> 0 ИЛИ СтрНайти(ТекстЗапросаИЗ, "Документ") <> 0;
	Если ЗначениеЗаполнено(ТекстЗапросаГДЕ) Тогда
		ПараметрыЗапроса = ОбработатьКонструкциюГДЕ(ТаблицаРеквизитов, ТекстЗапросаГДЕ);
		ТекстЗапросаГДЕ = " ГДЕ " + ТекстЗапросаГДЕ + ?(УсловиеПометкиУдаления, " И НЕ ОсновнаяТаблица.ПометкаУдаления", "");	
	ИначеЕсли УсловиеПометкиУдаления Тогда
		ТекстЗапросаГДЕ = " ГДЕ НЕ ОсновнаяТаблица.ПометкаУдаления";
	КонецЕсли;
	...
	Для Каждого СтрокаТЗ ИЗ ТаблицаРеквизитов Цикл
		...
		Если ПозицияСимвола > 0 Тогда
			...
			Если СтрНайти(СтрокаТЗ.Тип, ".") Тогда //поле ссылочного типа
				...
				// ТСК Шепелев М.Н.; 09.01.2019; task#2361 {
...
Процедура СформироватьМассивДанныхВариантовОтчетов(СтруктураОтвета,ЗапросHTTP) Экспорт
	...
	// ТСК Близнюк С.И.; 15.01.2019; task#2087{
	|	НастройкиВариантовОтчетовДокументооборотКатегории.Ссылка.ра_НеПередаватьГруппировки КАК ра_НеПередаватьГруппировки,
	// ТСК Близнюк С.И.; 15.01.2019; task#2087}
	...
	Если ЗначениеЗаполнено(ГУИДОбъекта) Тогда
		НастройкаСсылка = Справочники.НастройкиВариантовОтчетовДокументооборот.ПолучитьСсылку(Новый УникальныйИдентификатор(ГУИДОбъекта));
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НастройкиВарианта <> Неопределено Тогда
			...
			//Группировки
			// ТСК Близнюк С.И.; 15.01.2019; task#2087{
			Если НЕ Выборка.ра_НеПередаватьГруппировки Тогда
			// ТСК Близнюк С.И.; 15.01.2019; task#2087}
			Попытка
				Счетчик = 1;
				...
				Пока СтруктураКомпоновки.Количество() Цикл
					Для каждого СтрокаГруппировка Из СтруктураКомпоновки Цикл
						Если СтрокаГруппировка.ПоляГруппировки.Элементы.Количество() Тогда
							...
							//	Счетчик = Счетчик + 1;
							//КонецЕсли;
							Для каждого ПолеГруппировки Из СтрокаГруппировка.ПоляГруппировки.Элементы Цикл
								КешГруппировки[ПолеГруппировки.Поле].Order = Счетчик;
							КонецЦикла;
							Счетчик = Счетчик + 1;
						    // ТСК Близнюк С.И.; 15.01.2019; task#2087}
			...
			// ТСК Близнюк С.И.; 15.01.2019; task#2087{
			Иначе
				СтруктураДанных.Вставить("Groupings", Новый Массив);
			КонецЕсли; 
			// ТСК Близнюк С.И.; 15.01.2019; task#2087}
...
#Область ИнтеграцияСМобильнымПриложением

Процедура ЗарегистрироватьДанныеДляПервогоОбмена(УзелОбмена) Экспорт
	
	Если ЗначениеЗаполнено(УзелОбмена) Тогда 
		
		ПланыОбмена.ЗарегистрироватьИзменения(УзелОбмена, Метаданные.Справочники.ra_VidyNesootvetstvij)
