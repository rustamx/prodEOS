...
Процедура ПриСозданииКонтекстаВзаимодействия(Контекст) Экспорт
	...
	Если МестоЗапуска = "taxcomGetID" Тогда
		Контекст.Вставить("СообщениеОНедоступностиДействия",
			НСтр("ru = 'Получение уникального идентификатора абонента Такском недоступно для этой конфигурации.'; en = 'Getting unique caller ID Taxcom is not available for this configuration.'"));
	ИначеЕсли МестоЗапуска = "taxcomPrivat" Тогда
		Контекст.Вставить("СообщениеОНедоступностиДействия",
			НСтр("ru = 'Работа с личным кабинетом абонента Такском недоступна для этой конфигурации.'; en = 'Working with personal cabinet subscriber''s Taxcom is not available for this configuration.'"));
	КонецЕсли;
...
Процедура ПодготовитьСертификатЭПКОтправке(КСКонтекст, КонтекстОбработчика)
	...
	Если СертификатЭП = Неопределено Тогда
		...
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("ru = 'Не удалось получить двоичные данные сертификата. Сертификат не обнаружен в списке параметров.'; en = 'Failed to retrieve the binary data of the certificate. Certificate is not found in the parameter list.'");
...
|en = 'Error retrieving binary data of the certificate.
|Binary data certificate missing or certificate has been removed.'");
...
|en = 'Error retrieving certificate data.
...
Процедура ПодготовитьДанныеОбОрганизации(КСКонтекст, КонтекстОбработчика)
	...
	Исключение
		...
		КонтекстОбработчика.ПолноеОписаниеОшибки =
			НСтр("ru = 'Не удалось получить регистрационные данные организации.'; en = 'Failed to retrieve registration data on your company.'")
		...
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("ru = 'Ошибка при получении данных организации.
				 ...
				 |en = 'Error retrieving company data.
