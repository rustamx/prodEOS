...
Функция НайтиПеречисление(Знач ИмяПеречисления, ПредставлениеПеречисления) Экспорт
	...
	Если ИмяПрикладногоПеречисления = Неопределено Тогда // не задано соответствие
		ШаблонСообщения = НСтр("ru = 'В коде прикладного решения необходимо указать соответствие для перечисления %1.'; en = 'In code of applied solution one must specify the correspondence for enumeration %1.'");
...
Функция СтавкаНДСИзПредставления(ПредставлениеСтавкиНДС) Экспорт
	...
	Если СтрСтавкаНДС = Неопределено ИЛИ СтрНайти(НСтр("ru ='БЕЗ НДС'; en = 'without VAT'"), ВРег(СтрСтавкаНДС)) > 0 Тогда
		ЗначениеНДС = НСтр("ru ='без НДС'; en = 'without VAT'");
	Иначе
...
Функция ПредставлениеОснованияДляВидаЭД(ВидЭД) Экспорт
	...
	СоответствиеВидовЭДДокументамИБ.Вставить(Перечисления.ВидыЭД.ПроизвольныйЭД, НСтр("ru = 'Исходящий произвольный документ'; en = 'Arbitrary outgoing document'"));
	...
	Если ПредставлениеОснования = Неопределено Тогда // не задано соответствие
		ШаблонСообщения = НСтр("ru = 'В переопределяемом модуле прикладного решения необходимо указать представление документаИБ(основания) и хоз. операции для вида ЭД %1.'; en = 'In the redefinable module of the applied solution, you must specify the presentation of the dokument and of economic operation for %1 ED.'");
...
Функция ТЗСопоставленияПолейАдресаФНС_CML()
	...
	НоваяСтрока.ПредставлениеCML = НСтр("ru = 'Почтовый индекс'; en = 'Zip code'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "КодРегион";
	НоваяСтрока.ПредставлениеCML = НСтр("ru = 'Регион'; en = 'State'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "НаселПункт";
	НоваяСтрока.ПредставлениеCML = НСтр("ru = 'Населенный пункт'; en = 'Settlement'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "Город";
	НоваяСтрока.ПредставлениеCML = НСтр("ru = 'Город'; en = 'City'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "Улица";
	НоваяСтрока.ПредставлениеCML = НСтр("ru = 'Улица'; en = 'Street'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "Дом";
	НоваяСтрока.ПредставлениеCML = НСтр("ru = 'Дом'; en = 'Building'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "Корпус";
	НоваяСтрока.ПредставлениеCML = НСтр("ru = 'Корпус'; en = 'Housing'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "Кварт";
	НоваяСтрока.ПредставлениеCML = НСтр("ru = 'Квартира'; en = 'Apartment'");
	
	НоваяСтрока = ТЗ.Добавить();
	НоваяСтрока.ПредставлениеФНС = "КодСтр";
	НоваяСтрока.ПредставлениеCML = НСтр("ru = 'Страна'; en = 'Country'");
