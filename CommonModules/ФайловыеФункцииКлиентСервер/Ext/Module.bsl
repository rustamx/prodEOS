...
Функция ФайлМожноЗагружать(ВыбранныйФайл,
	...
	Если ВыбранныйФайл.Размер() > МаксРазмерФайла Тогда
		...
		Запись.Вставить("Ошибка", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Размер файла (%1 Мб) превышает максимальный размер файла: (%2 Мб)'; en = 'File size (%1 MB) exceeds the maximum file size: (%2 MB)'"),
...
Функция ПолучитьСтрокуСРазмеромФайлаДляКарточкиПапки(Знач РазмерБайт) Экспорт
	...
	Если РазмерБайт >= 1024*1024*1024 Тогда // больше 1 Гб
		СтрокаРазмера = Формат(РазмерБайт / (1024*1024*1024), "ЧДЦ=0") + НСтр("ru=' Гб'; en = ' GB'");
	ИначеЕсли РазмерБайт < 1024*1024*1024  И РазмерБайт >= 1024*1024 Тогда // от 1 Мб до 1 Гб
		СтрокаРазмера = Формат(РазмерБайт / (1024*1024), "ЧДЦ=0; ЧН=0") + НСтр("ru=' Мб'; en = ' MB'");
	ИначеЕсли РазмерБайт < 1024*1024  И РазмерБайт >= 1024 Тогда // от 1 кб до 1 Мб
		СтрокаРазмера = Формат(РазмерБайт / 1024, "ЧДЦ=0; ЧН=0") + НСтр("ru=' Кб'; en = ' KB'");
	Иначе // размер меньше 1 кб
		СтрокаРазмера = Формат(РазмерБайт, "ЧДЦ=0; ЧН=0") + НСтр("ru=' байт'; en = ' byte'");
	КонецЕсли;
