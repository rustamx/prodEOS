...
Процедура ПодключитьИнтернетПоддержкуПользователей(
	...
	Если Не ИнтернетПоддержкаПользователейКлиентСервер.ДоступноПодключениеИнтернетПоддержки() Тогда
		...
		ПоказатьПредупреждение(
			ОповещениеАвторизацияНедоступна,
			НСтр("ru = 'Недостаточно прав для подключения Интернет-поддержки.'; en = 'Insufficient permissions to connect to Online support.'"));
		Возврат;
	...
	Исключение
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось подключить обработчик запроса настроек клиента лицензирования. %1'; en = 'Unable to connect settings request handler for the licensing client. %1'"),
...
Процедура СтартоватьМеханизмИзМеню(СтартовыеПараметры = Неопределено) Экспорт
	...
	Если НЕ ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.МониторИнтернетПоддержки") Тогда
		...
		ВызватьИсключение НСтр("ru = 'Ошибка встраивания. Отсутствует подсистема ""Монитор Интернет-поддержки"".'; en = 'Embedding error. There is no ""Online support monitor"" subsystem.'");
...
Процедура СтартоватьМеханизмРаботыСОператоромЭДО(
	...
	Если НЕ ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Подключение1СТакском") Тогда
		...
		ВызватьИсключение НСтр("ru = 'Ошибка встраивания. Отсутствует подсистема ""Подключение 1С-Такском"".'; en = 'Embedding error. There is no ""Connection to 1C-Taxcom"" subsystem.'");
...
Процедура ОткрытьВебСтраницу(Знач АдресСтраницы, ЗаголовокОкна, Логин = Неопределено, Пароль = Неопределено) Экспорт
	...
	Если НеобходимаАвторизации И ПараметрыКлиента.ЭтоПолноправныйПользователь Тогда
		...
		Если ЗначениеЗаполнено(Логин) И ЗначениеЗаполнено(Пароль) Тогда
			...
			Состояние(, , НСтр("ru = 'Переход на Портал 1С:ИТС'; en = 'Go to 1C:ITS portal'"));
...
Процедура ОтправитьСообщениеВТехПоддержку(
	...
	Если Получатель <> "webIts" И Получатель <> "taxcom" И Получатель <> "backup" Тогда
		ВызватьИсключение НСтр("ru = 'Неизвестный получатель сообщения.'; en = 'Unknown message recipient.'");
	КонецЕсли;
	...
	Состояние(, , НСтр("ru = 'Подготовка сообщения в службу технической поддержки'; en = 'Preparing message to technical support service'"));
...
Процедура ВыполнитьСценарий(
	...
	Исключение
		...
		ПараметрыФормы.Вставить("ОписаниеОшибки",
			НСтр("ru = 'Неизвестная ошибка. Подробнее см. в журнале регистрации.'; en = 'Unknown error. See details in events log.'"));
...
Процедура ОбработкаКомандСервиса(
	...
	Исключение
		...
		КонтекстОбработчика.ПолноеОписаниеОшибки = НСтр("ru = 'Произошло необрабатываемое исключение:'; en = 'Unhandled exception occurred:'")
			+ " " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	ИначеЕсли КонтекстОбработчика.ВыполнитьОстановку Тогда
		...
		Если КонтекстОбработчика.Свойство("ОписаниеПричиныОстановки") Тогда
			ПоказатьОповещениеПользователя(НСтр("ru = 'Интернет-поддержка пользователей'; en = 'Online customer support'"),
				,
	...
	Иначе
		...
		ОбщегоНазначенияКлиент.ПроверитьРасширениеРаботыСФайламиПодключено(
			Новый ОписаниеОповещения(
				...
				ЭтотОбъект,
				ПараметрыСообщения),
			НСтр("ru = 'Для продолжения необходимо установить расширение для работы с файлами.'; en = 'To proceed, you must install the extension for working with files.'"));
...
Процедура ПриОтправкеДанныхСообщенияВТехПоддержку(РезультатОтправки, ПараметрыСообщения)
	...
	Если Результат Тогда
		...
		ОткрытьВебСтраницу(
			...
			НСтр("ru = 'Отправка сообщения в службу технической поддержки'; en = 'Sending message to technical support service'"));
...
Процедура ДобавитьКомандыСервиса(
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
				НСтр("ru = 'Ошибка при подключении к сервису Интернет-поддержки.'; en = 'Error connecting to Online support service'");
		КонецЕсли;
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ДействияПриОшибкеДляСервера.Добавить("СоздатьЗаписьВЖурналеРегистрации");
			КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ОтобразитьФормуОтсутствуетПодключение";
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки = НСтр("ru = 'Ошибка при получении данных с сервера Интернет-поддержки.'; en = 'Error retrieving data from online support site.'");
		КонецЕсли;
...
Функция СформироватьПараметрыОткрытияФормы(КСКонтекст, ИмяОткрываемойФормы)
	...
	Если ИмяОткрываемойФормы = "ОбщаяФорма.ПодключениеИнтернетПоддержки" Тогда
		...
		ПараметрыНовойФормы.Вставить("ЗаголовокКнопкиОК", НСтр("ru = 'Войти'; en = 'Sign in'"));
...
Процедура ОтобразитьСостояниеОбращениеКСервису()
	...
	Состояние(
		,
		,
		НСтр("ru = 'Ожидание ответа от сервера Интернет-поддержки пользователей'; en = 'Waiting for response from the Online customer support server'"),
		БиблиотекаКартинок.ИнтернетПоддержкаПользователей);
...
Функция ВопросПриВыходеАвторизованногоПользователя(КонтекстВзаимодействия)
	...
	ТекстВопроса = НСтр("ru = 'Подключение пользователя с логином %1 к Интернет-поддержке будет
					...
					|Продолжить?';
					|en = 'Connection of user with login %1 to Online support will be terminated.
					|It will be necessary to re-enter username and password later.
