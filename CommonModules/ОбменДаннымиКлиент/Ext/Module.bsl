...
Процедура ФормаНастройкиПередЗакрытием(Отказ, Форма, ЗавершениеРаботы) Экспорт
	...
	ТекстВопроса = НСтр("ru = 'Данные были изменены. Закрыть форму без сохранения изменений?'; en = 'Data were changed. Close the form without saving the changes?'");
...
Функция ЗаголовкиГиперссылокЗагрузкиДанных() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Неопределено",               НСтр("ru = 'Получение данных не выполнялось'; en = 'Data import was not executed'"));
	Структура.Вставить("Ошибка",                     НСтр("ru = 'Не удалось получить данные'; en = 'Failed to import data'"));
	Структура.Вставить("ВыполненоСПредупреждениями", НСтр("ru = 'Данные получены с предупреждениями'; en = 'Data received with warnings'"));
	Структура.Вставить("Успех",                      НСтр("ru = 'Данные успешно получены'; en = 'Data received successfully'"));
	Структура.Вставить("Выполнение",                 НСтр("ru = 'Выполняется получение данных...'; en = 'Retrieving data...'"));
	
	Структура.Вставить("Предупреждение_СообщениеОбменаБылоРанееПринято", НСтр("ru = 'Нет новых данных для получения'; en = 'No new data to obtain'"));
	Структура.Вставить("Ошибка_ТранспортСообщения",                      НСтр("ru = 'Не удалось получить данные'; en = 'Failed to import data'"));
...
Функция ЗаголовкиГиперссылокВыгрузкиДанных() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Неопределено", НСтр("ru = 'Отправка данных не выполнялась'; en = 'Data export was not executed'"));
	Структура.Вставить("Ошибка",       НСтр("ru = 'Не удалось отправить данные'; en = 'Failed to export data'"));
	Структура.Вставить("Успех",        НСтр("ru = 'Данные успешно отправлены'; en = 'Data sent successfully'"));
	Структура.Вставить("Выполнение",   НСтр("ru = 'Выполняется отправка данных...'; en = 'Sending data...'"));
	
	Структура.Вставить("Предупреждение_СообщениеОбменаБылоРанееПринято", НСтр("ru = 'Данные отправлены с предупреждениями'; en = 'Data sent with warnings'"));
	Структура.Вставить("ВыполненоСПредупреждениями",                     НСтр("ru = 'Данные отправлены с предупреждениями'; en = 'Data sent with warnings'"));
	Структура.Вставить("Ошибка_ТранспортСообщения",                      НСтр("ru = 'Не удалось отправить данные'; en = 'Failed to export data'"));
...
Процедура ОбработчикВыбораФайловогоКаталога(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	УмолчанияДиалога.Вставить("Заголовок", НСтр("ru = 'Укажите каталог'; en = 'Specify the directory'") );
	
	УстановитьЗначенияСтруктурыПоУмолчанию(ПараметрыДиалога, УмолчанияДиалога);
	
	ТекстПредупреждения = НСтр("ru = 'Для данной операции необходимо установить расширение для веб-клиента 1С:Предприятие.'; en = 'For this procedure one must install the Web client extension for 1C:Enterprise.'");
...
Процедура ПередатьФайлыНаСервер(ОповещениеЗавершения, Знач ИменаФайлов, Знач ИдентификаторФормы = Неопределено, Знач ТекстПредупреждения = Неопределено) Экспорт
	...
	Для Каждого ИмяФайла Из ИменаФайлов Цикл
		...
		Если ПустаяСтрока(ИмяФайла) Тогда
			ЕстьПустые = Истина;
			ОписаниеФайла.ОписаниеОшибки = НСтр("ru = 'Файл не выбран.'; en = 'File not selected.'");
		КонецЕсли;
	...
	Если ТекстПредупреждения = Неопределено Тогда
		Если ИменаФайлов.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("ru = 'Для передачи файла на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.'; en = 'To transfer file to the server one must install the Web client extension for 1C:Enterprise.'");
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Для передачи файлов на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.'; en = 'To transfer files to the server one must install the Web client extension for 1C:Enterprise.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередатьФайлНаСерверЗавершениеПодключенияРасширенияПослеПроверкиСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		...
		Элемент.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Файл ""%1"" не существует или к нему нет доступа'; en = 'File ""%1"" does not exist or cannot be accessed'"), Элемент.Имя);
...
Процедура ВыбратьИСохранитьФайлНаКлиентеПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	УмолчанияДиалога = Новый Структура;
	УмолчанияДиалога.Вставить("Заголовок",               НСтр("ru = 'Выберите файл для сохранения'; en = 'Select the file to save'"));
	УмолчанияДиалога.Вставить("МножественныйВыбор",      Ложь);
	УмолчанияДиалога.Вставить("ПредварительныйПросмотр", Ложь);
...
Процедура УдалитьНастройкуСинхронизации(Знач УзелИнформационнойБазы) Экспорт
	...
	Если ОбменДаннымиВызовСервера.УзелЯвляетсяГлавным(УзелИнформационнойБазы) Тогда
		ТекстПредупреждения = НСтр("ru = 'Отключение информационной базы от главного узла можно выполнить
			...
			|en = 'One can disconnect the infobase from the main node running the
			|Configurator with parameter /ResetMasterNode.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	Иначе
		ТекстВопроса = НСтр("ru = 'Удалить настройку синхронизации данных?'; en = 'Delete the data synchronization settings?'");
...
Процедура ПриОткрытииИнструкцииКакИзменитьПарольСинхронизацииДанных(Знач АдресДляВосстановленияПароляУчетнойЗаписи) Экспорт
	...
	Если ПустаяСтрока(АдресДляВосстановленияПароляУчетнойЗаписи) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Адрес для восстановления пароля учетной записи не задан.'; en = 'Address to recover account password is not specified.'"));
...
Функция ОткрытьФормуДополненияВыгрузкиВсеДокументы(Знач ДополнениеВыгрузки, Знач Владелец=Неопределено, Знач Уникальность=Неопределено, Знач Окно=Неопределено) Экспорт
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Добавление документов для отправки'; en = 'Adding documents to send'") );
