...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление сведений о записанных версиях объектов. После выполнения обновления увеличится скорость записи версионируемых объектов.'; en = 'Updated information about the recorded object versions. After the update the recording speed of versioned objects will increase.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.2.4.13";
	Обработчик.Процедура = "ВерсионированиеОбъектов.ПеренестиНастройкиВерсионирования";
	Обработчик.Комментарий = НСтр("ru = 'Обновление настроек версионирования.'; en = 'Versioning settings updating.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Версионирование'; en = 'Versioning'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось обновить сведения о версии №%1 объекта ""%2"" по причине:
					|%3';
					|en = 'Unable to update information about version №%1 of object ""%2"" because:
	...
	Если Выборка.Количество() > 0 Тогда
		Если ОбработаноЗаписей = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедуре ОбновитьСведенияОВерсияхОбъектов не удалось обработать некоторые записи регистра сведений ВерсииОбъектов (пропущены): %1'; en = 'Procedure ОбновитьСведенияОВерсияхОбъектов could not process some records of the register information ВерсииОбъектов (ignored): %1'"), 
					Выборка.Количество());
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Подсказка = НСтр("ru = 'Устаревших версий: %1 (%2)'; en = 'Oudated versions: %1 (%2)'");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ru = 'Устаревшие версии объектов'; en = 'Outdated versions of objects'");
		...
		Если Не Объект.ЭтоНовый() И (ИзмененаПроведенность И СведенияОВерсииОбъекта.НомерВерсии > 1 Или ВерсияОтличаетсяОтРанееЗаписанной(Объект)) Тогда
			...
			Если СведенияОВерсииОбъекта.НомерВерсии = 1 Тогда
				Если ОбъектВерсионируется(Объект.Ссылка, Ложь) Тогда
					...
					ПараметрыВерсии.Вставить("Комментарий", НСтр("ru = 'Версия создана по уже имеющемуся объекту'; en = 'Version created on an existing object'"));
...
Процедура ПроверитьНаличиеПравНаИзменениеОбъекта(ОбъектМетаданных)
	...
	Если Не ПривилегированныйРежим() И Не ПравоДоступа("Изменение", ОбъектМетаданных)Тогда
		ТекстСообщения = НСтр("ru = 'Недостаточно прав на изменение ""%1"".'; en = 'Not enough permissions to modify ""%1"".'");
...
Функция ОтчетПоВерсииОбъекта(СсылкаНаОбъект, Знач ВерсияОбъекта = Неопределено, ПользовательскийНомерВерсии = Неопределено) Экспорт
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '№ %1 / (%2) / %3'; en = '№ %1 / (%2) / %3'"), ПользовательскийНомерВерсии,
...
Функция ВосстановитьОбъектПоXML(ДанныеОбъекта, ТекстСообщенияОбОшибке = "")
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Версионирование'; en = 'Versioning'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		ТекстСообщенияОбОшибке = НСтр("ru = 'Не удалось перейти на выбранную версию.
											...
											|en = 'Unable to move to the selected version. 
											|Possible cause: the object version was saved in a different version of the program. 
...
Функция СведенияОВерсииОбъекта(Знач Ссылка, Знач НомерВерсии) Экспорт
	СообщениеНеУдалосьПолучитьВерсию = НСтр("ru = 'Не удалось получить предыдущую версию объекта.'; en = 'Unable to get the previous version of the object.'");
	...
	Если Результат.ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Выбранная версия объекта отсутствует в программе.'; en = 'The selected object version is absent in the program.'");
	КонецЕсли;
...
Функция РазмерДанныхСтрокой(Знач РазмерДанных)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 %2'; en = '%1 %2'"), РазмерДанных, ЕдиницаИзмерения);
...
Функция ПолучитьПредставлениеРеквизитаНаЯзыке(Знач ИмяРеквизита)
	...
	Если      ИмяРеквизита = "Number" Тогда
		Возврат НСтр("ru = 'Номер'; en='Number'; en = 'Number'");
	ИначеЕсли ИмяРеквизита = "Name" Тогда
		Возврат НСтр("ru = 'Наименование'; en='Name'; en = 'Description'");
	ИначеЕсли ИмяРеквизита = "Code" Тогда
		Возврат НСтр("ru = 'Код'; en='Code'; en = 'Code'");
	ИначеЕсли ИмяРеквизита = "IsFolder" Тогда
		Возврат НСтр("ru = 'ЭтоГруппа'; en='Is folder'; en = 'IsFolder'");
	ИначеЕсли ИмяРеквизита = "Description" Тогда
		Возврат НСтр("ru = 'Наименование'; en='Description'; en = 'Description'");
	ИначеЕсли ИмяРеквизита = "Date" Тогда
		Возврат НСтр("ru = 'Дата'; en='Date'; en = 'Date'");
	ИначеЕсли ИмяРеквизита = "Posted" Тогда
		Возврат НСтр("ru = 'Проведен'; en='Posted'; en = 'Posted'");
	ИначеЕсли ИмяРеквизита = "DeletionMark" Тогда
		Возврат НСтр("ru = 'ПометкаУдаления'; en='Deletion mark'; en = 'DeletionMark'");
	ИначеЕсли ИмяРеквизита = "Ref" Тогда
		Возврат НСтр("ru = 'Ссылка'; en='Ref'; en = 'Reference'");
	ИначеЕсли ИмяРеквизита = "Parent" Тогда
		Возврат НСтр("ru = 'Родитель'; en='Parent'; en = 'Parent'");
	ИначеЕсли ИмяРеквизита = "Owner" Тогда
		Возврат НСтр("ru = 'Владелец'; en='Owner'; en = 'Owner'");
	Иначе
...
Процедура ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	Если ТипЗнч(ЭлементДанных) = Тип("РегистрСведенийНаборЗаписей.ВерсииОбъектов") И ЭлементДанных.Количество() > 0 Тогда
		...
		Если НомерВерсииСКоллизией = Неопределено Тогда
			...
			Запись.Комментарий = НСтр("ru = 'Отклоненная версия (автоматическое разрешение конфликта).'; en = 'Rejected version (automatic conflict resolution).'");
			НаборЗаписей.Записать();
...
Функция ПерейтиНаВерсиюСервер(Ссылка, НомерВерсии, ТекстСообщенияОбОшибке, ОтменаПроведения = Ложь) Экспорт
	...
	Объект.ДополнительныеСвойства.Вставить("ВерсионированиеОбъектовКомментарийКВерсии",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выполнен переход к версии №%1 от %2'; en = 'Convertion to version %1 dated %2 complete'"),
