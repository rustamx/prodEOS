...
Процедура ПроверитьВозможностьРаботыСВнешнимиРесурсами(РегламентноеЗадание, ЗависимостиЗаданий = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Приложение было перемещено.
			...
			|en = 'Application was moved. 
			|Scheduled job ""%1"", working with external resources, is disabled.'"), РегламентноеЗадание.Синоним);
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Изменилась строка соединения информационной базы.
			...
			|en = 'Information base connection string has changed.
			|Perhaps Infobase has been moved.
			|Scheduled job ""%1"" is disabled.'"), РегламентноеЗадание.Синоним);
...
Функция НастройкиПоУмолчанию()
	...
	НастройкиПодсистемы.Вставить("РасположениеКомандыСнятияБлокировки",
		НСтр("ru = 'Блокировку также можно снять позднее в разделе <b>Администрирование - Поддержка и обслуживание</b>.'; en = 'Lock also can be removed later in the section <b>Administration - Support and maintenance</b>.'"));
...
Функция ВыполнитьРегламентноеЗаданиеВручную(Знач Задание) Экспорт
	...
	Если СвойстваПоследнегоФоновогоЗадания <> Неопределено
	   И СвойстваПоследнегоФоновогоЗадания.Состояние = СостояниеФоновогоЗадания.Активно Тогда
		...
		КонецЕсли;
	Иначе
		НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Запуск вручную: %1'; en = '%1 started manually'"), ПредставлениеРегламентногоЗадания(Задание));
...
Функция ТекстНеОпределено() Экспорт
	...
	Возврат НСтр("ru = '<не определено>'; en = '<not defined>'");
...
Процедура ОтменитьФоновоеЗадание(Идентификатор) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Фоновое задание не найдено на сервере.'; en = 'The background job is not found on the server.'");
	КонецЕсли;
	
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Активно Тогда
		ВызватьИсключение НСтр("ru = 'Задание не выполняется, его нельзя отменить.'; en = 'The job is not running, it cannot be cancelled.'");
	КонецЕсли;
...
Функция ИдентификаторФайловойБазы(ЭтоФайловаяБаза)
	...
	Если ЭтоФайловаяБаза Тогда
		...
		|en = '%1 
		|
		|The file is created automatically by ""%2"".
		|It contains an identifier database and allows you to specify that the information database has been copied.
		|
		|When copying infobase files, including the creation of a backup copy, do not copy the file.
		|Simultaneous use of two copies of the information base with the same identifier can lead to conflicts
		|when you synchronize data, sending e-mail and other work with external resources.
		|
		|If the file is not in the database catalog with the information, the program will ask for the administrator as to whether this
		|information base to work with external resources.'"), ИдентификаторБазы, Метаданные.Синоним);
...
Процедура УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Работа с внешними ресурсами заблокирована'; en = 'Access to external resources is locked'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция УстановленаБлокировкаРаботыСВнешнимиРесурсами(ЭтоВызовПередНачаломРаботыСистемы = Ложь, ПоказатьФорму = Ложь) Экспорт
	...
	Если РазделениеВключено <> ПараметрыБлокировки.РазделениеВключено Тогда
		...
		ТекстСообщения = НСтр("ru = 'Информационная база была перемещена из приложения в Интернете.'; en = 'Information base was moved from Internet application.'");
	...
	Если ЭтоФайловаяБаза <> ПараметрыБлокировки.ЭтоФайловаяБаза Тогда
		ТекстСообщения = ?(ЭтоФайловаяБаза, НСтр("ru = 'Информационная база была перемещена из клиент-серверного режима работы в файловый.'; en = 'Information base was moved from file mode work to client-server one.'"),
			НСтр("ru = 'Информационная база была перемещена из файловый режима работы в клиент-серверный.'; en = 'The information base was moved from file mode to the client-server one.'"));
		УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
		Возврат Истина;
	КонецЕсли;
	...
	Если ЭтоФайловаяБаза Тогда
		...
		Если НайденныеФайлы.Количество() = 0 Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В каталоге информационной базы отсутствует файл проверки %1.'; en = 'No validation file %1 in the infobase directory.'"), ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
		...
		Если ИдентификаторБазы <> ПараметрыБлокировки.ИдентификаторБазы Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Идентификатор информационной базы в файле проверки %1 не соответствует идентификатору в текущей базе.'; en = 'ID of the information base in validation file %1 does not match the identifier in the current database.'"), ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
		...
		Если ИмяБазы <> СохраненноеИмяБазы
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Различаются имя информационной базы или имя компьютера.
					...
					|en = 'The distinguished name of the infobase or computer name.
					|Saved infobase name: %1, current infobase name: %2.
					|Saved computer name: %3, current computer name: %4'"),
