		...
		Если ТекущиеДанные <> Неопределено Тогда
			Если Поле.Имя = ПроверкаКонтрагентовКлиентСервер.ИмяПоляКартинки(Элемент) Тогда
				Если ПроверкаКонтрагентовКлиентСерверПовтИсп.ЭтоНекорректноеСостояниеКонтрагента(ТекущиеДанные.Состояние) Тогда
					...
					ПоказатьПредупреждение(,ПодсказкаВДокументе.Текст, , НСтр("ru = 'Проверка контрагентов'; en = 'Counterparties verification'"));
				КонецЕсли;
...
Процедура УведомитьОбОтсутствииДоступа()
	...
	ПоказатьПредупреждение(, НСтр("ru = 'Проверка контрагентов сервисом ФНС не выполнена из-за ошибки подключения к сервису'; en = 'Verification of counterparties using the FTS data failed because of connection error.'"));
