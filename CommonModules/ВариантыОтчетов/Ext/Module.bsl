...
Функция ИспользуемыеРазделы() Экспорт
	...
	Для Каждого ЭлементСписка Из СписокРазделов Цикл
		...
		Иначе
			ШаблонЗаголовка = НСтр("ru = 'Отчеты раздела ""%1""'; en = 'Reports of section ""%1""'");
		КонецЕсли;
		...
		Если РазделМетаданные = ИдентификаторНачальнойСтраницы Тогда
			...
			Строка.Представление = НСтр("ru = 'Начальная страница'; en = 'Startpage'");
		Иначе
...
Функция ОписаниеОтчета(Настройки, Отчет) Экспорт
	...
	Если СтрокаОтчет = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Error retrieving report description ""%1"": it is not connected to the subsystem ""%2"".
			|Please check ""Options storage"" property in report properties.'"),
			Строка(Отчет),
...
Функция ОписаниеВарианта(Настройки, Отчет, КлючВарианта) Экспорт
	...
	Если СтрокаВариант = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Вариант ""%1"" не найден для отчета ""%2"".'; en = 'Option ""%1"" is not found in report ""%2"".'"),
			КлючВарианта,
...
Процедура ДобавитьОбработчикиПолногоОбновления(Обработчики, Версия) Экспорт
	...
	Если РазрешеноИндексированиеНеразделенныхДанных() Тогда
		...
		Обработчик.Комментарий   = НСтр("ru = 'Полное обновление индекса поиска отчетов, предусмотренных в программе.
			...
			|en = 'Report search index is being updated.
			|Report search is temporarily unavailable.'");
	КонецЕсли;
	...
	Обработчик.Комментарий = НСтр("ru = 'Полное обновление индекса поиска отчетов, сохраненных пользователями.
		|Поиск отчетов временно недоступен.';
		|en = 'Custom report search index is being updated.
		|Report search is temporarily unavailable.'");
...
Процедура СброситьПользовательскиеНастройки(Ключ, ТипыНастроек = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректный тип параметра ""Отчет""'; en = 'Parameter ""Report"" has incorrect value'");
	КонецЕсли;
	...
	Для Каждого ПолноеИмяВарианта Из КлючиВариантов Цикл
		...
		Пока Истина Цикл
			...
			Исключение
				...
				ОшибкаПоВарианту(
					Неопределено,
					НСтр("ru = 'В процессе выборки пользовательских настроек отчетов из системного хранилища возникла ошибка:'; en = 'Error loading custom settings from system storage:'")
					+ Символы.ПС
...
Процедура ПеренестиПользовательскиеИзСтандартногоХранилища(ИменаОтчетов = "") Экспорт
	ПредставлениеПроцедуры = НСтр("ru = 'Прямая конвертация вариантов отчетов'; en = 'Direct report options conversion'");
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ОшибкаПоВарианту(
				Неопределено,
				НСтр("ru = 'В процессе выборки вариантов отчетов из стандартного хранилища возникла ошибка:'; en = 'Error loading report options from standard storage:'")
				+ Символы.ПС
				+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ЗагрузитьПользовательские(ТаблицаВариантов = Неопределено) Экспорт
	...
	ПредставлениеПроцедуры = НСтр("ru = 'Завершить конвертацию вариантов отчетов'; en = 'Finish report options conversion'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ОпределенныеВарианты Цикл
		...
		Иначе
			ОшибкаПоВарианту(
				...
				НСтр("ru = 'Вариант ""%1"" отчета ""%2"": не найден автор ""%3""'; en = 'Option ""%1"" of report ""%2"": author is not found ""%3""'"),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Уменьшает количество быстрых настроек
		|в пользовательских отчетах до 2 шт.'; en = 'Reduces the number of quick settings to 2 pcs. in custom reports.'");
...
Функция ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
		...
		Если ТипЗнч(Результат.СсылкаВарианта) <> Тип("СправочникСсылка.ВариантыОтчетов")
			...
			Если Результат.СсылкаВарианта = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В методе ""%1"" не указаны параметры ""%2"".'; en = 'Parameters ""%2"" are not specified in the method ""%1"".'"),
	...
	Если НадоПрочитатьНастройки Тогда
		...
		Если ВариантНастроекКД = Неопределено Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Вариант ""%1"" (ключ ""%2"") не найден в схеме отчета ""%3"".'; en = 'Option ""%1"" (key ""%2"") is not found in report ""%3"" schema.'"),
...
Функция ПодключитьОтчетОбъект(СсылкаОтчета, ПолучатьМетаданные)
	...
	Если СсылкаОтчета = Неопределено Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В методе ""%1"" не указан параметр ""%2"".'; en = 'Parameter ""%2"" is not specified in the method ""%1"".'"),
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отчет ""%1"" записан как внешний и не может быть подключен из программы'; en = 'Report ""%1"" is saved as external and cannot be connected from the application.'"),
			Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
		...
		Если ПолучатьМетаданные Тогда
			...
			Если Результат.Метаданные = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отчет ""%1"" не найден в программе'; en = 'Report ""%1"" not found in application'"),
					Результат.Имя);
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриЗаписиДополнительногоОтчета(ТекущийОбъект, Отказ, ВнешнийОбъект) Экспорт
	...
	Если Не ВариантыОтчетовПовтИсп.ПравоДобавления() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недостаточно прав доступа для записи вариантов дополнительного отчета ""%1"".'; en = 'Not enough permission to save the additional report ""%1"" options.'"),
...
Функция ПараметрыОткрытия(ВариантСсылка) Экспорт
	...
	Иначе
		...
		Если Не Выборка.Следующий() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Недостаточно прав для открытия варианта ""%1"".'; en = 'Not enough permissions to open option ""%1"".'"), Строка(ВариантСсылка));
		КонецЕсли;
		...
		Если Метаданные.Отчеты.Найти(ПараметрыОткрытия.ИмяОтчета) = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось открыть отчет ""%1"".
					...
					|en = 'Report ""%1"" is not found in applicaton. 
					|Maybe extensions with the report has been disabled.'"),
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	Если Не Подключение.Успех Тогда
		Результат.ТекстОшибки = НСтр("ru = 'Не удалось сформировать отчет:'; en = 'Failed to create report:'") + Символы.ПС + Подключение.ТекстОшибки;
		Возврат Результат;
	КонецЕсли;
	...
	Если ПроверятьЗаполнение Тогда
		...
		Если Не ПроверкаПройдена Тогда
			Результат.ТекстОшибки = НСтр("ru = 'Отчет не прошел проверку заполнения:'; en = 'Report fill check failed:'");
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Не удалось определить используемые таблицы:'; en = 'Failed to determine used tables:'");
	...
	Если СсылкиВариантов.Количество() > 0 Тогда
		...
		Для Каждого Ссылка Из СсылкиВариантов Цикл
			...
			Если ЗначенияРеквизитов = Неопределено Тогда
				СтрокаТаблицы.Представление = НСтр("ru = '<Недостаточно прав для работы с вариантом отчета>'; en = '<Insufficient permissions to work with the report option>'");
			Иначе
	...
	Для Каждого Отчет Из СсылкиОтчетов Цикл
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			...
			ИначеЕсли Не ДоступенПоПравам Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Недостаточно прав для работы с отчетом ""%1"">'; en = '<Insufficient permissions to work with the report ""%1"">'"),
					ИмяОтчета);
			ИначеЕсли Не ДоступенПоОпциям Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Отчет ""%1"" отключен в настройках программы>'; en = '<Report ""%1"" is disabled in application settings>'"),
					ИмяОтчета);
			Иначе
...
Функция ДеревоПредопределенных(ФильтрПоТипуОтчетов = "Внутренний") Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		Если СтрокаОтчет.ИспользуетСКД Тогда
			...
			Исключение
				ТекстОшибки = НСтр("ru = 'Не удалось прочитать схему отчета ""%1"":'; en = 'Failed to read report ""%1"" schema:'");
			...
			Если СхемаКД <> Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ru = 'Не удалось прочитать список вариантов отчета ""%1"" из схемы:'; en = 'Failed to read report ""%1"" option list from schema:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ПредупреждениеПоВарианту(Неопределено, ТекстОшибки);
				КонецПопытки;
			...
			Если ВариантыНастроек = Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ru = 'Не удалось прочитать список вариантов отчета ""%1"" из модуля менеджера:'; en = 'Failed to read report ""%1"" option list from manager module:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			...
			Если ВариантыНастроек <> Неопределено Тогда
				Для Каждого ВариантНастроекКД Из ВариантыНастроек Цикл
					...
					Если РазрешеноИндексирование И ТипЗнч(ВариантНастроекКД) = Тип("ВариантНастроекКомпоновкиДанных") Тогда
						...
						Исключение
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Не удалось прочитать настройки варианта ""%1"" отчета ""%2"":'; en = 'Failed to read option settings ""%1"" of report ""%2"":'"),
...
Процедура УстановитьРежимВыводаОтчетаВПанеляхОтчетов(ДеревоВариантов, Отчет, ВыводитьОтчетыВместоВариантов)
	...
	Иначе
		...
		Если СтрокаОтчет = Неопределено Тогда
			ПредупреждениеПоВарианту(Неопределено, НСтр("ru = 'Отчет ""%1"" не подключен к подсистеме.'; en = 'Report ""%1"" is not connected to sub-system.'"), Отчет.Имя);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	СтрокаОтчет.ГруппироватьПоОтчету = ВыводитьОтчетыВместоВариантов;
	...
	Для Каждого Изменение Из Изменения Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|en = 'Error registering name change of report option ""%1"":
				|Actual option name ""%2"" (old name ""%3"") 
				|is also listed as the old name ""%4"" (actual name ""%5"").'"),
		...
		Если Найденные.Количество() > 2 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|en = 'Error registering report option ""%1"" name changes: 
				|Old option name ""%2"" (actual name ""%3"") 
				|is also listed as the old report option name ""%4"" (actual name ""%5"").'"),
				Строка(Изменение.Отчет),
				Изменение.СтароеИмяВарианта,
				Изменение.АктуальноеИмяВарианта,
...
Функция СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя) Экспорт
	...
	Если ВРег(Префикс) = "ОТЧЕТ" Тогда
		...
		Если Результат.ОтчетМетаданные = Неопределено Тогда
			...
			ПредупреждениеПоВарианту(
				Неопределено,
				НСтр("ru = 'Отчет ""%1"" не найден в программе, он будет значиться как внешний.'; en = 'Report ""%1"" not found in application. It will appear as an external.'"),
				ОтчетПолноеИмя);
		ИначеЕсли Не ПравоДоступа("Просмотр", Результат.ОтчетМетаданные) Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Недостаточно прав доступа к отчету ""%1"".'; en = 'Not enough permission to access the report ""%1"".'"),
				ОтчетПолноеИмя);
		КонецЕсли;
	...
	Иначе
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для отчета ""%1"" невозможно определить тип (не установлен префикс).'; en = 'It is impossible to determine report ""%1"" type (prefix is not set)'"),
			ОтчетПолноеИмя);
		Возврат Результат;
	КонецЕсли;
...
Функция ГлобальныеНастройки() Экспорт
	...
	Результат.Поиск.Вставить("ПодсказкаВвода", НСтр("ru = 'Наименование, поле или автор отчета'; en = 'Description, field and report author'"));
...
Процедура ЗапускПроцедуры(ПредставлениеПроцедуры)
	ИнформацияПоВарианту(Неопределено, НСтр("ru = 'Запуск процедуры ""%1"".'; en = 'Procedure ""%1"" has been started.'"), ПредставлениеПроцедуры);
КонецПроцедуры
...
Процедура ЗавершениеПроцедуры(ПредставлениеПроцедуры, Изменено = Неопределено)
	Текст = НСтр("ru = 'Завершение процедуры ""%1"".'; en = 'Procedure ""%1"" has ended.'");
	Если Изменено <> Неопределено Тогда
		Текст = Текст + " " + НСтр("ru = 'Изменено %2 объектов.'; en = '%2 objects have been changed.'");
	КонецЕсли;
...
Процедура ПерейтиКРедакции21() Экспорт
	ПредставлениеПроцедуры = НСтр("ru = 'Перейти к редакции 2.1'; en = 'Go to edition 2.1'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗаполнитьСсылкиПредопределенных() Экспорт
	ПредставлениеПроцедуры = НСтр("ru = 'Заполнить ссылки предопределенных вариантов отчетов'; en = 'Fill predefined report options references'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура СократитьКоличествоБыстрыхНастроек(ВходящиеПараметры = Неопределено) Экспорт
	ПредставлениеПроцедуры = НСтр("ru = 'Сокращение количества быстрых настроек в отчетах'; en = 'Reducing the number of quick settings in reports'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОтчетов Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда // Возникла проблема.
			ТекстОшибки = НСтр("ru = 'Вариант ""%1"" отчета ""%2"":'; en = 'Option ""%1"" of report ""%2"":'")
			+ Символы.ПС + НСтр("ru = 'При уменьшении количества быстрых настроек пользовательского возникла ошибка:'; en = 'Error reducing number of quick settings:'")
...
|en = 'Failed to reduce the number of quick setting %1 of reports.
|Не удалось уменьшить количество быстрых настроек %1 отчетов.'");
...
Функция ОперативноеОбновлениеОбщихДанных(Режим, РазделенныеОбработчики)
	...
	Кэш.Вставить("Уточнение", ?(Кэш.Режим = "ОбщиеДанныеКонфигурации", НСтр("ru = 'метаданные конфигурации'; en = 'Configuration metadata'"), НСтр("ru = 'метаданные расширений'; en = 'Extensions metadata'")));
...
Функция ОтложенноеОбновлениеДанных(Режим, Полное)
	...
	Кэш.Вставить("Уточнение", НРег(ОтчетыКлиентСервер.ИмяВПредставление(Режим)));
	Кэш.Уточнение = Кэш.Уточнение + ", " + ?(Полное, НСтр("ru = 'полное'; en = 'full'"), НСтр("ru = 'по изменениям'; en = 'by changes'"));
...
Процедура ОбновитьИндексПоиска(Кэш)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновление индекса поиска (%1)'; en = 'Updating search index (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ПримечаниеПоВарианту(Выборка.Ссылка, НСтр("ru = 'Индексация схемы'; en = 'Schema index'"));
		...
		Если Кэш.ОбщиеДанные Тогда
			...
			Если Найденные.Количество() = 0 Тогда
				ТекстОшибки = НСтр("ru = 'Вариант ""%1"" не найден для отчета ""%2""'; en = 'Option ""%1"" is not found in report ""%2""'");
				ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
...
Процедура АктуализироватьКлючиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновление ключей вариантов отчетов (%1)'; en = 'Updating report option keys (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ОбновитьНастройкиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновление настроек предопределенных (%1)'; en = 'Updating predefined settings (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Функция ИзменилисьКлючевыеНастройкиПредопределенного(ОписаниеВарианта, ВариантИзБазы)
	Если ВариантИзБазы.ПометкаУдаления = Истина 
...
Процедура АктуализироватьСоставОбласти(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновление вариантов отчетов (%1)'; en = 'Updating report options (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Удаление вариантов удаленных отчетов (%1)'; en = 'Deleting options of deleted reports (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗарегистрироватьЗамерыВариантаКОбновлению(Кэш, Знач СтарыйКлюч, Знач АктуальныйКлюч, Знач АктуальноеНаименование)
	...
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отчет ""%1"" (открытие)'; en = 'Report ""%1"" (opening)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Формирование";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Формирование";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отчет ""%1"" (формирование)'; en = 'Report ""%1"" (generation)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Настройки";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Настройки";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отчет ""%1"" (настройки)'; en = 'Report ""%1"" (settings)'"), АктуальноеНаименование);
КонецПроцедуры
...
Функция СписокГлобальныхКлючевыхОпераций()
	...
	СписокОпераций.Добавить("ПанельОтчетов.Открытие",       НСтр("ru = 'Панель отчетов (открытие)'; en = 'Reports panel (opening)'"));
	СписокОпераций.Добавить("ПанельОтчетов.Поиск",          НСтр("ru = 'Панель отчетов (поиск в режиме просмотра)'; en = 'Reports panel (search in view mode)'"));
	СписокОпераций.Добавить("ПанельОтчетов.РежимНастройки", НСтр("ru = 'Панель отчетов (режим настройки)'; en = 'Reports panel (settings mode)'"));
	СписокОпераций.Добавить("СписокОтчетов.Открытие", НСтр("ru = 'Список отчетов (открытие)'; en = 'Reports list (opening)'"));
	СписокОпераций.Добавить("СписокОтчетов.Поиск",    НСтр("ru = 'Список отчетов (поиск)'; en = 'Reports list (search)'"));
...
Процедура ЗаписатьПараметрыВариантовОтчетов(Кэш)
	...
	КонецЕсли;
	ПредставлениеПроцедуры = НСтр("ru = 'Запись неразделенного кэша в константу'; en = 'Save unseparated cache to constant'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗафиксироватьТекущуюВерсиюРасширений(Кэш)
	...
	ПредставлениеПроцедуры = НСтр("ru = 'Запись регистра версий расширений'; en = 'Saving extensions versions register entry'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Функция ПроиндексироватьСодержимоеСхемы(ВариантОбъект) Экспорт
	...
	Если СхемаКД = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Search settings are not filled for option ""%1"" of report ""%2"": 
			|Field and or Parameters and filters names'"),
			ВариантОбъект.КлючВарианта,
			ВариантОбъект.Отчет);
		Если ЭтоПредопределенный Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС
				+ НСтр("ru = 'Подробнее - см. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".'; en = 'For details - see procedure ""НастроитьВариантыОтчетов"" in module ""ВариантыОтчетовПереопределяемый""'");
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Если ТипЗнч(ВариантОбъект) = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов")
			...
			Текст = НСтр("ru = 'Не удалось прочитать настройки предопределенного варианта отчета ""%1"".'; en = 'Failed to read predefined report option settings ""%1"".'");
...
Функция УменьшитьКоличествоБыстрыхНастроек(ВариантОбъект, ОтчетОбъект)
	...
	НастройкиКД = ВариантОбъект.Настройки.Получить();
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		ТекстОшибки = НСтр("ru = 'Обнаружены пустые настройки пользовательского варианта ""%1"" отчета ""%2"".'; en = 'Found empty custom options ""%1"" settings of report ""%2"".'");
		ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
	...
	Если ОтчетыСНастройками.Найти(ОтчетСсылка) <> Неопределено Тогда
		Если ОтчетОбъект = Неопределено Тогда
			...
			Если Подключение.Успех Тогда
				ОтчетОбъект = Подключение.Объект;
			Иначе
				Текст = НСтр("ru = 'Не удалось получить настройки отчета ""%1"":'; en = 'Failed to retrieve report settings ""%1"":'") + Символы.ПС + Подключение.ТекстОшибки;
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		Если Не ЕстьСтрокаПоиска И Не ПолучатьИтоговуюТаблицу Тогда
			...
			ТекстЗапроса = СтрЗаменить(
				...
				"НЕОПРЕДЕЛЕНО");
		Иначе
			Запрос.УстановитьПараметр("НаименованиеРабочегоСтола", НСтр("ru = 'Начальная страница'; en = 'Startpage'"));
		КонецЕсли;
...
Функция СообщениеДанныеЕщеОбновляются() Экспорт
	Возврат НСтр("ru = 'Отчет может содержать некорректные данные, так как не завершен переход на новую версию программы. Если отчет долгое время недоступен, необходимо обратиться к администратору.'; en = 'Report can contain incorrect data because update to new application version has not been completed. If the report has been unavailable for a long time please contact the administrator.'");
КонецФункции
