...
Функция ПКОПоИмени(КомпонентыОбмена, Имя) Экспорт
	...
	Если ПравилоКонвертации = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПКО с именем %1'; en = 'Object conversion rule with name %1 is not found'"), Имя);
...
Функция ОбъектXDTOИзДанныхXDTO(
	...
	Для Каждого Свойство Из ТипXDTO.Свойства Цикл
		...
		Иначе
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Неизвестный тип свойства <%1>. Тип объекта: %2'; en = 'Unknown property type <%1>. Object type: %2'"), Свойство.Имя, Строка(ТипXDTO));
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка формирования объекта XDTO: Тип свойства <%1>. Имя свойства: <%2>.'; en = 'Error generating XDTO object: Property type <%1>. Property name: <%2>.'"), ТипСвойства, Свойство.Имя)
	...
	Если ПравилоКонвертации.ЭтоКонстанта Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|Тип источника: %1
				|Тип приемника: %2';
				|en = 'XML schema error. Receiver must be a single property.
				|Source type: %1
				|Receiver type: %2'"),
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если ТранзакцияАктивна() Тогда
			ВызватьИсключение НСтр("ru = 'Блокировка на обмен данными не может быть установлена в активной транзакции.'; en = 'Lock on data exchange cannot be set in an active transaction.'");
		КонецЕсли;
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока НомерИтерации = 0 Цикл
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = НСтр("ru = 'Ошибка при загрузке данных: %1'; en = 'Error occurred when importing data: %1'");
		...
		Если (ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента
			...
			Если СообщениеОтНеобновленнойНастройки(ЧтениеXML) Тогда
				СтрокаСообщенияОбОшибке = НСтр("ru = 'Получение данных от источника, в котором не выполнено
					...
					|en = 'Importing data from a source where data synchronization settings
					|were not updated. It is necessary to:'")
					+ Символы.ПС + НСтр("ru = '1) Выполнить повторную синхронизацию данных через некоторое время.'; en = '1) Repeatedly synchronize data after some time.'")
					...
					|en = '2) Synchronize data on side of the source infobase, then repeatedly
					|synchronize data in this infobase.'")
					+ Символы.ПС + НСтр("ru = '(1 - для вида транспорта Через интернет, 2 - для вида транспорта Другое)'; en = '(1 - for transport mode Via the Internet, 2 - for transport mode of Other)'");
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрокаСообщенияОбОшибке);
			Иначе
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Если МенеджерПланаОбмена.ФорматОбмена() <> ФорматОбмена.БазовыйФормат Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Формат сообщения обмена <%1> не соответствует формату плана обмена <%2>.'; en = 'Format of exchange message <%1 > does not match exchange plan <%2 >.'"),
			...
			Если ВерсииФорматаОбменаМассив(КомпонентыОбмена.УзелКорреспондента).Найти(ФорматОбмена.Версия) = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Версия формата сообщения обмена <%1> не поддерживается.'; en = 'Message exchange format version <%1> is not supported.'"),
					ФорматОбмена.Версия);
			КонецЕсли;
			...
			Если МаксимальнаяОбщаяВерсия <> НомерВерсииКорреспондента Тогда
				...
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, 
				                            НСтр("ru = 'Изменен номер версии формата обмена.'; en = 'Changed version number of exchange format.'"),,Ложь,,, Истина);
			КонецЕсли;
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		СтрокаСообщенияОбОшибке = НСтр("ru = 'Попытка изменение неразделенных данных (%1) в разделенном режиме.'; en = 'Attempting to change unseparated data (%1) in separated mode.'");
...
Функция МенеджерОбменаВерсииФормата(Знач УзелИнформационнойБазы, Знач ВерсияФормата) Экспорт
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не определен Менеджер конвертации для версии формата обмена <%1>.'; en = 'Conversion manager for exchange format version <%1> was not found.'"),
			ВерсияФормата);
	КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
	...
	Если Версия1.Количество() < 2 ИЛИ Версия1.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неправильный формат параметра СтрокаВерсии1: %1'; en = 'Invalid format of parameter СтрокаВерсии1: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() < 2 ИЛИ Версия2.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неправильный формат параметра СтрокаВерсии2: %1'; en = 'Invalid format of parameter СтрокаВерсии2: %1'"), СтрокаВерсии2);
	КонецЕсли;
	...
	Если ТипXDTO = Неопределено Тогда
		...
		ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|en = 'Error in conversion rules for predefined data.
				|More than one conversion rule provided for source type <%1>.'"),
				Строка(Тип));
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Error in conversion rules for predefined data.
			|No conversion rule provided for source type <%1>.'"),
			Строка(Тип));
	...
	Иначе
		...
		ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|en = 'Error in conversion rules for predefined data.
				|More than one conversion rule provided for source type <%1> and destination type <%2>.'"),
				Строка(Тип),
				Строка(ТипXDTO));
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Error in conversion rules for predefined data.
			|No conversion rule provided for source type <%1> and destination type <%2>.'"),
			Строка(Тип),
			Строка(ТипXDTO));
...
Функция ЗначениеПеречисленияXDTO(Знач КонвертацииЗначений, Знач Значение)
	...
	Если ЗначениеXDTO = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Conversion rule was not found for predefined data values.
			|Source value type: <%1>
			|Source value: <%2>'"),
			ТипЗнч(Значение),
			Строка(Значение));
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник, ИмяДляСвойстваСоставногоТипа = "")
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка чтения объекта XDTO, имя свойства: <%1>.'; en = 'Error reading XDTO object, property name: <%1>.'"), ИмяСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка конвертации свойства объекта XDTO, имя свойства: <%2>.'; en = 'Error converting XDTO object properties, property name: <%2>.'"), ЗначениеСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если ЭлементыФормата.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое имя формата обмена <%1>'; en = 'Noncanonical exchange format name, <%1>'"), ФорматОбмена);
	КонецЕсли;
...
Функция НайтиСсылкуПоПубличномуИдентификатору(УИДОбъектаXDTO, КомпонентыОбмена, ТипЗначенияОбъектаИБ)
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Если  КоличествоСсылокПодходящегоТипа > 1 Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для уникального идентификатора <%1> и узла <%2> назначено несколько ссылок.'; en = 'For unique identifier <%1> and node <%2> several references are assigned.'"),
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найдено ПОД для типа ссылки XDTO.
					|Тип ссылки XDTO: %1
					|Описание ошибки: %2';
					|en = 'Data processing rule for XTDO reference type is not found.
					|XTDO reference type: %1
					|Error description: %2'"),
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найдено ПОД для типа объекта XDTO.
					|Тип объекта XDTO: %1
					|Описание ошибки: %2';
					|en = 'Data processing rule for XTDO object is not found.
					|XDTO object type: %1
					|Error description: %2'"),
...
Функция ПОДПоОбъектуМетаданных(КомпонентыОбмена, ОбъектМетаданных)
	...
	Если ПравилоОбработки = Неопределено Тогда
		ТекстОшибки = Символы.ПС + СтрШаблон(НСтр("ru = 'Не найдено ПОД для объекта метаданных.
				|Объект метаданных: %3.';
				|en = 'Data processing rule not found for the metadata object.
				|Metadata object: %3.'"),
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не найдено ПОД с именем %1'; en = 'Data processing rule with name %1 is not found'"), Имя);
...
Функция ПредставлениеОбъектаДляПротокола(Объект)
		...
		Если ТипОбъекта = Тип("ОбъектXDTO") Тогда
			...
			Если КоллекцияСвойств.Количество() > 0 И КоллекцияСвойств.Получить("КлючевыеСвойства") <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Номер) Тогда
					ПредставлениеОбъекта = СтрШаблон(НСтр("ru = '%1 №%2 от %3'; en = '%1 №%2 of %3'"), Наименование, Номер, Дата);
				ИначеЕсли ЗначениеЗаполнено(Код) Тогда
					ПредставлениеОбъекта = СтрШаблон(НСтр("ru = '%1, %2'; en = '%1, %2'"), Наименование, Код);
				КонецЕсли;
			КонецЕсли;
	...
	ШаблонПредставления = НСтр("ru = '%1, %2%3'; en = '%1, %2%3'");
...
Процедура ПроверитьВерсию(Знач Версия)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неканоническое представление версии формата обмена: <%1>.'; en = 'Noncanonical presentation of exchange format version, <%1>.'"), Версия);
	КонецЕсли;
