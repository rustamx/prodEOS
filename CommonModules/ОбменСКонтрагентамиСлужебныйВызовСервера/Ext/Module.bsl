			...
			Если Не Выборка.Следующий() Тогда
				...
				Если НЕ Выборка.Следующий() И СообщатьОбОшибке Тогда
					ШаблонСообщения = НСтр("ru = 'Не удалось определить настройку ЭДО для параметров: 
						...
						|контрагент %2, 
						|договор %3.';
						|en = 'Unable to determine the configuration of EDI for parameters:
						|%1 company,
						|%2 counterparty,
						|%3 contract.'");
	...
	Пока Выборка.Следующий() Цикл
		Если ТипЗнч(Выборка.ДокументПотомок) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий") Тогда
			...
			Исключение
				...
				ВидОперации = НСтр("ru = 'запись документа-основания ""%1"" в документ ""%2""'; en = 'saving source document (""%1"") to ""%2"" document'");
...
Процедура РаспаковатьПакетыЭДНаСервере(Знач МассивПакетовЭД, КолРаспакованныхЭД) Экспорт
	...
	Для ОбратныйИндекс = 1 По КоличествоЭлементовКоллекции Цикл
		...
		Для каждого ЭлектронныйДокумент Из СтруктураПакетаЭД.МассивЭД Цикл
			...
			Если ЭлектронныйДокумент.Зашифрован Тогда
				...
				Если ПарольДоступаКЗакрытомуКлючу = Неопределено Тогда
					ШаблонСообщения = НСтр("ru = 'Не удалось получить пароль к сертификату: %1'; en = 'Unable to retrieve the password for certificate: %1'");
				...
				Исключение
					ШаблонСообщения = НСтр("ru = 'Расшифровка пакета ЭД: %1'; en = 'Decrypting ED package: %1'");
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
		...
		Если Не ПустаяСтрока(ОписаниеОшибки) Тогда
			...
			|%2.';
			|en = 'When unpacking the package %1 an error occurred:
			|%2.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СтруктураПакетаЭД.ПакетЭДСсылка,
				ОписаниеОшибки);
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Распаковка пакета ЭД'; en = 'Unpacking ED package'"), ТекстСообщения,
				ТекстСообщения);
		...
		Для каждого Подпись Из СтруктураПакетаЭД.МассивЭП Цикл
			...
			Исключение
				ШаблонСообщения = НСтр("ru = 'Получение сертификатов из подписи: %1'; en = 'Obtaining certificates of signature: %1'");
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Прервать;
			КонецПопытки;
			...
			Если НЕ ЗначениеЗаполнено(Подпись.АдресЭД) Тогда
				ОписаниеОшибки = НСтр("ru = 'Не удалось найти исходные данные для проверки ответной подписи.'; en = 'Could not find the source data to verify the signature of the respondent.'");
				Прервать;
			КонецЕсли;
	...
	Для каждого ПакетЭД Из МассивПакетовЭД Цикл
		...
		Попытка
			...
			Исключение
				...
				КонецЕсли;
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Распаковка архива пакета ЭД'; en = 'Unpacking ED archived package'"), ТекстОшибки,
					ТекстСообщения);
		...
		Исключение
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Распаковка пакета ЭД'; en = 'Unpacking ED package'"), ТекстОшибки,
				ТекстСообщения);
...
Процедура ЗаписатьДанныеПакетовЭД(ДанныеПакетовЭД, МассивСлужебныхЭД, КолРаспакованныхЭД) Экспорт
	...
	Для каждого СтруктураПакетаЭД Из ДанныеПакетовЭД Цикл
		...
		Попытка
			...
			Для каждого ЭлектронныйДокумент Из СтруктураПакетаЭД.МассивЭД Цикл
								...
								Иначе
									...
									Если ОбменСКонтрагентамиСлужебный.ЭтоОтветныйТитул(СтруктураФайлаЭД.ТипЭлементаВерсииЭД) Тогда
										...
										Если ВладелецЭД = Неопределено Тогда
											...
											|en = 'For the response title (%1) source ED is not found.
											|Possible reasons:
											|- primary title was deleted from the infobase;
											|- the package was not intended for the infobase (the primary title was created in another infobase or copy of this infobase).'");
				...
				Если ЭлектронныйДокумент.ТипДанных = "СлужебныйЭД" Тогда
					...
					Если Не ПервичныйЭДНайден Тогда
						ТекстСообщения = НСтр("ru = 'Не удалось найти владельца для служебного электронного документа.'; en = 'No owner could not be found for the service electronic document.'");
					...
					Если СтруктураФайлаЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.Ошибка Тогда
						...
						ШаблонСообщения = НСтр("ru = 'Возникла ошибка при передаче электронного документа %1, получатель %2.'; en = 'There was an error while transferring %1 electronic document, recipient: %2.'");
						...
						ПодробныйШаблонОшибки = НСтр("ru = 'При передаче электронного документа %1, возникла ошибка: %2.
							...
							|en = 'When sending electronic document %1, an error occurred: %2. 
							|Infobase document (electronic document owner) - %3; 
							|EDI agreement - %4.'");
						...
						ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Передача электронного документа'; en = 'Electronic document transmission'"),
			...
			Для каждого Подпись Из СтруктураПакетаЭД.МассивЭП Цикл
				...
				Если НЕ ЗначениеЗаполнено(Подпись.СсылкаНаЭДПрисоединенныйФайл) Тогда
					ТекстСообщения = НСтр("ru = 'Не удалось найти электронный документ для сохранения подписи.'; en = 'Electronic document could not be found to save the signature.'");
					ВызватьИсключение ТекстСообщения;
				КонецЕсли;
		...
		Исключение
			...
			|%2';
			|en = 'When unpacking ED package: %1
			|an error occurred:
			|%2'");
...
Процедура ДобавитьПодпись(ПрисоединенныйФайл, ДанныеПодписи) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru = 'Ошибка подписи электронного документа %1'; en = 'Error signing electronic document %1'");
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
		ВидОперации = НСтр("ru = 'установка подписи ЭП'; en = 'installing DS signature'");
...
Процедура ДействияПослеПодписанияЭДНаСервере(МассивЭДКОбновлениюСтатуса) Экспорт
	...
	Для Каждого ПрисоединенныйФайл Из МассивЭДКОбновлениюСтатуса Цикл
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru = 'Ошибка подписи электронного документа %1'; en = 'Error signing electronic document %1'");
			ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
			Операция = НСтр("ru = 'установка подписи ЭП'; en = 'installing DS signature'");
	...
	Для Каждого ПакетЭД Из МассивПакетовЭД Цикл
		...
		Попытка
			...
			ИначеЕсли ТипОтправки =  Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
				...
				Если КоличествоОтправленных <> КоличествоКОтправке Тогда
					ШаблонСообщения = НСтр("ru = 'Не удалось отправить оператору ЭДО ""%1"".'; en = 'Could not be sent to EDI operator ""%1"".'");
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru = 'Ошибка отправки пакета по настройке: %1, способ обмена: %2
									|%3';
									|en = 'Error while sending package according to settings: %1, exchange method: %2
									|%3'");
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				РеквизитыПакетаЭД.НастройкаЭДО, РеквизитыПакетаЭД.СпособОбменаЭД, КраткоеПредставлениеОшибки);
			ШаблонВидаОперации = НСтр("ru = 'отправка пакета ЭД по настройке: %1, способ обмена %2'; en = 'sending ED package according to configuration: %1, exhange method %2'");
	...
	Если ЗначениеЗаполнено(СсылкаНаВладельца) Тогда
		...
		Если НастройкаЭДСуществует(СсылкаНаВладельца, ПараметрыЭД) Тогда
			Если Не ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЕстьПравоЧтенияЭД(Ложь) Тогда
				ШаблонСостоянияЭД = НСтр("ru = '%1 «Недостаточно прав для чтения ЭДО»'; en = '%1 Not enough permissions to read EDI'");
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Результат) Тогда
					Гиперссылка = Ложь;
					Результат = НСтр("ru = 'ЭДО не начат'; en = 'EDI was not started'");
				КонецЕсли;
				...
				Если СостояниеСоглашения = Перечисления.СостоянияСоглашенийЭД.ПроверкаТехническойСовместимости Тогда
					ШаблонСостоянияЭД = НСтр("ru = '%1 (проверка технической совместимости)'; en = '%1 (verifying technical compatibility)'");
					Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостоянияЭД, Результат);
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(Результат) Тогда
				ШаблонСостоянияЭД = НСтр("ru = '%1 (настройка ЭДО не подключена)'; en = '%1 (EDI settings are not connected)'");
				Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостоянияЭД, Результат);
				Гиперссылка = Истина;
			Иначе
...
Процедура ЗаполнитьТекстПриглашенияКЭДО(ТекстПриглашения, ПараметрыЭД, СсылкаНаВладельца, Гиперссылка) Экспорт
	...
	Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ПередачаТоваровМеждуОрганизациями
		...
		ТекстПриглашения = НСтр("ru = 'Нет действующей настройки ЭДО с организацией-получателем'; en = 'No current EDI settings with the recipient company'");
	ИначеЕсли ЗначениеЗаполнено(ПараметрыЭД.Организация) И ЗначениеЗаполнено(ПараметрыЭД.Контрагент)
		...
		ШаблонТекстаНавигационнойСсылки = НСтр("ru = 'Пригласить %1 к обмену электронными документами в 1С-ЭДО'; en = 'Invite %1 to electronic documents interchange in 1С-EDI'");
		...
		Если Запрос.Выполнить().Пустой() Тогда
			...
			ШаблонТекстаНавигационнойСсылки = НСтр("ru = 'Подключить ""%1"" к ЭДО'; en = 'Connect ""%1"" to EDI'");
			...
			Если Выборка.Следующий()
				...
				ШаблонТекстаНавигационнойСсылки = НСтр("ru = 'С ""%1"" возможен обмен электронными документами'; en = 'With ""%1"" EDI is possible'");
				...
				Если ИмяРеализацияТоваровУслуг <> Неопределено
					...
					ШаблонТекстаНавигационнойСсылки = НСтр("ru = 'Обмен электронными документами с ""%1"" можно начать прямо сейчас'; en = 'Exchange of electronic documents with ""%1"" can start right now'");
				ИначеЕсли ИмяПоступлениеТоваровУслуг <> Неопределено
					...
					ШаблонТекстаНавигационнойСсылки = НСтр("ru = 'Получать электронные документы от ""%1"", а не вводить руками, можно прямо сейчас'; en = 'Right now you can receive electronic documents from ""%1"", instead of typing them manually.'");
				КонецЕсли;
...
Процедура ОпределитьСтатусыПодписей(ЭД, ВыводитьСообщения = Ложь) Экспорт
	...
	Для каждого СтрокаЭП Из ОбъектЭД.ЭлектронныеПодписи Цикл
		...
		Исключение
			...
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = НСтр("ru = 'проверка электронной подписи'; en = 'validating digital signature'");
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(СертификатПодписи) Тогда
		...
		Если ПолучаемДанные Тогда 
			...
			ИначеЕсли СтрНайти(ПараметрыФайлаЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					...
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
						НСтр("ru = 'Извлечение ЭД CML из архива при подписи'; en = 'Extract from the archive the CML ED when signing'"), ТекстОшибки, ТекстСообщения);
				КонецПопытки;
				АрхивЧтение.Закрыть();
			...
			Если СтрНайти(ПараметрыФайлаЭД.Расширение, "zip") > 0 Тогда
				...
				Исключение
					ТекстОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
					ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Формирование архива ЭД при подписи'; en = 'Forming ED archive when signing'"),
			...
			Если ТекСтрока.СтатусЭД = Перечисления.СтатусыЭД.Сформирован 
				...
				Исключение
					...
					Если ЗначениеЗаполнено(ТекстСообщения) Тогда
						Операция = НСтр("ru = 'Утверждение ЭД'; en = 'Confirmation of ED'");
		...
		Если ТЗ_Сертификатов.Количество() = 0 И ЗначениеЗаполнено(ЭД) Тогда
			...
			Если ДоступныеСертификаты[0].Количество > 0 Тогда
				...
				Если Не ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
					ШаблонСообщения = НСтр("ru = 'В профиле настроек ЭДО: %1
						...
						|en = 'In EDI settings profile : %1
						|none of the available DS certificates is found.'");
			...
			ИначеЕсли ДопПараметры.Свойство("ОшибкаНастройкиКриптографии", ОшибкаНастройкиКриптографии)
				...
				ТекстСообщения = НСтр("ru = 'Ошибка подписания электронного документа.
											...
											|en = 'Error signing electronic document. 
											|Check the cryptography settings.'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
...
Функция НастройкиЭДОИСертификатыАвторизации(Знач ПрофилиНастроекЭДО, Знач ЭлектронныеДокументы = Неопределено,
	...
	Если ТипЗнч(СтМассивовСтруктурСертификатов) = Тип("Структура") Тогда
		...
		Если ВыполнятьКриптооперацииНаСервере И НЕ ЗначениеЗаполнено(МассивОтпечатковСервер) И ИспользоватьЭП Тогда
			...
			Исключение
				ВидОперации = НСтр("ru = 'Получение данных сертификатов'; en = 'Obtaining certificate data'");
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если ТипЗнч(СтруктураСертификата) = Тип("Структура") Тогда
		...
		ИначеЕсли СтруктураСертификата.Свойство("МаркерЗашифрованный", МаркерЗашифрованный)
			...
			Попытка
				...
				КонецЕсли;
			Исключение
				Операция = НСтр("ru = 'Расшифровка маркера'; en = 'Decrypting the token'");
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
...
Функция СвойстваСертификата(АдресДанныхСертификата) Экспорт
	...
	Если ВыбранныйСертификат=Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Сертификат не найден'; en = 'Certificate not found'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция СостояниеВерсииЭД(СсылкаНаВладельца)
	...
	Если СтруктураВерсииЭД.Свойство("СостояниеВерсииЭД") Тогда
		...
		ИначеЕсли СтруктураВерсииЭД.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.НеСформирован 
			...
			СостояниеВерсии = НСтр("ru='ЭДО не начат'; en = 'EDI was not started'");
		Иначе
		...
		Если ДобавитьПричинуЗакрытия Тогда
			...
			Причина = СтрЗаменить(НСтр("ru = ', причина: %1'; en = ', reason: %1'"), "%1", ?(ЗначениеЗаполнено(Комментарий), Комментарий, НСтр("ru ='не указана'; en = 'not specified'")));
...
Функция ПолучитьТекстСводногоСостоянияЭД(СсылкаНаВладельца)
	...
	Если ТаблицаДанных.Количество() > 0 Тогда
		...
		Если Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ЗакрытПринудительно Тогда
			ТекстСводногоСостояния = НСтр("ru = 'ЭДО закрыт принудительно'; en = 'EDI closed forcefully'");
		Иначе
			Если ЗначениеЗаполнено(Строка.ДействияСНашейСтороны)
				...
				ТекстСНашейСтороны = НСтр("ru = 'с нашей стороны'; en = 'on our part'");
			КонецЕсли;
			Если ЗначениеЗаполнено(Строка.ДействияСоСтороныДругогоУчастника)
				...
				ТекстСоСтороныДругогоУчастника = НСтр("ru = 'со стороны других участников'; en = 'by othes parties'");
			КонецЕсли;
			Если ЗначениеЗаполнено(ТекстСНашейСтороны) 
				Или ЗначениеЗаполнено(ТекстСоСтороныДругогоУчастника) Тогда
				
				ТекстСводногоСостояния = НСтр("ru = 'Требуются действия'; en = 'Actions necessary'")+ " " + ТекстСНашейСтороны
			...
			ИначеЕсли ЗначениеЗаполнено(Строка.ДействияСНашейСтороны)
				...
				Если Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершен Тогда
					ТекстСводногоСостояния = НСтр("ru = 'ЭДО завершен'; en = 'EDI completed'");
				ИначеЕсли Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершенСИсправлением Тогда
					ТекстСводногоСостояния = НСтр("ru = 'ЭДО завершен с исправлением'; en = 'EDI completed with correction'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ПередатьПисьмоСВложениями(ПакетЭД, СоответствиеВложений)
	...
	Исключение
		...
		Текст = НСтр("ru = 'Ошибка при отправке сообщения на сервер электронной почты по профилю настроек ЭДО %1.
						|%2';
						|en = 'Error occurred while sending message to mail server using EDI profile settings %1. 
						|%2'");
		...
		ШаблонСообщения = НСтр("ru = 'Ошибка при отправке эл.документов по профилю настроек ЭДО: %1, способ обмена: %2.'; en = 'Error occured while sending electronic documents according to profile: %1, exchange method: %2.'"); 
		...
		ВидОперации = НСтр("ru = 'Отправка эл.документов'; en = 'Sending e-documents'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
							ВидОперации,
							ТекстОшибки,
							ТекстСообщения);
...
Функция СформироватьТемуПисьма(Отправитель, Получатель)
	...
	Возврат НСтр("ru = 'Обмен эл.документами:'; en = 'ED exchange:'")+ " " + Отправитель + ?(ЗначениеЗаполнено(Получатель), " -> " + Получатель, "");
...
Процедура УдалитьНедоступныеДляФормированияЭДОбъекты(МассивСсылок)
	...
	Пока Результат.Следующий() Цикл
		...
		ШаблонСообщения = НСтр("ru='Для документа %1 уже есть актуальный электронный документ.'; en = 'Document %1 already have current electronic document.'");
...
Процедура ОпределитьНеОбрабатываемыеОбъекты(НастройкиОбъектов, МассивНеОбрабатываемыхОбъектов)
	...
	Пока Результат.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ru = 'Обработка %1.
				...
				|en = 'Processing %1. 
				|No actions are necessary for the electronic document.'");
...
Процедура СохранитьРезультатыПроверкиПодписей(ЭД, МассивРезультатов) Экспорт
	...
	Исключение
		...
		ВидОперации = НСтр("ru = 'Сохранение результатов проверки подписей'; en = 'Save the results of signature verification'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Функция ПодписатьЭДОпределеннымСертификатом(ДобавленныеФайлы,
	...
	Если КриптоСертификат <> Неопределено Тогда
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'проверка сертификата на корректность'; en = 'certificate verification for correctness'"),
											  ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
											  ТекстСообщения);
			Для Каждого НеПодписанныйЭД Из ФайлыНаПодпись Цикл
				МассивНеПодписанныхЭД.Добавить(НеПодписанныйЭД);
		...
		Для Каждого ТекДокумент Из ФайлыНаПодпись Цикл
			...
			СтруктураДанныхПодписи = Новый Структура("Сертификат, ПарольПользователя, Комментарий", КриптоСертификат, ПараметрыСертификата.ПарольПользователя,
				НСтр("ru = 'Подписание электронного документа'; en = 'Electronic document signing'"));
			Попытка
			...
			Исключение
				...
				|%2';
				|en = 'Error signing on the server. It is necessary to test cryptography certificate for: %1. 
				|%2'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СертификатПодписи,
					КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
						НСтр("ru = 'установка подписи ЭП'; en = 'installing DS signature'"),
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
						ТекстСообщения);
	...
	Если ЗначениеЗаполнено(ПутьККаталогу) Тогда
		...
		Если НЕ КаталогиДоступны Тогда
			ТекстСообщения = НСтр("ru = 'Указанный каталог %1 не может использоваться для обмена, так как он не доступен с сервера.
				...
				|en = 'Specified directory %1 cannot be used for interchange, because it is not available from the server. 
				|You must specify a network directory for interchange.'");
...
Процедура УстановитьОтветственногоЭД(Знач СписокОбъектов, Знач НовыйОтветственный, КоличествоОбработанныхЭД,
	...
	Исключение
		ОтменитьТранзакцию();
		ТекстОшибки = СтрШаблон(НСтр("ru='Не удалось установить ответственного электронного документа (%1). %2'; en = 'Unable to set the responsible for electronic document (%1). %2'"), 
			Выборка.Ссылка,
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Для Каждого СсылкаНаЭД Из МассивЭД Цикл
		...
		Если ЭтоИзвещение Тогда
			...
			Если НЕ (ПараметрыЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ПДОЭСФ
				...
				ШаблонСообщения = НСтр("ru = 'Для типа ЭД %1 не предусмотрено формирование Извещения о получении.'; en = 'For ED of %1 type notification of receipt is not supported.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
		...
		ИначеЕсли СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
			...
			Исключение
				...
				Если НЕ ЭлектронноеВзаимодействиеСлужебный.ВозможноИзвлечьФайлы(АрхивЧтение, ПапкаДляРаспаковки) Тогда
					ТекстСообщения = ЭлектронноеВзаимодействиеСлужебныйПовтИсп.ПолучитьСообщениеОбОшибке("006");
				КонецЕсли;
				ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Распаковка пакета ЭД'; en = 'Unpacking ED package'"), ТекстОшибки, ТекстСообщения);
				ЭлектронноеВзаимодействиеСлужебный.УдалитьВременныеФайлы(ПапкаДляРаспаковки);
				Возврат "";
			КонецПопытки;
