...
Процедура Записать_ra_Signal(ДанныеОбъекта, МобильныйКлиент, ПараметрыСинхронизации)
	...
	Письмо.PodrobnoeOpisanie = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "Description", "Строка");
	...
	СсылкаНаВыявившего = ПолучитьЗначениеРеквизита(ДанныеОбъекта, "Viyavivshiy", "УникальныйИдентификатор", Ложь);
	Если ЗначениеЗаполнено(СсылкаНаВыявившего) Тогда
		Письмо.Viyavivshiy = Справочники.Пользователи.ПолучитьСсылку(СсылкаНаВыявившего);
	КонецЕсли;
	// ТСК Близнюк С.И.; 29.10.2018; task#1602}
