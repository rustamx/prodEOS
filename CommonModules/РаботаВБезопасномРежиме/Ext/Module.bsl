...
Процедура ВыполнитьМетодОбъекта(Знач Объект, Знач ИмяМетода, Знач Параметры = Неопределено) Экспорт
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Некорректное значение параметра ИмяМетода (%1) в РаботаВБезопасномРежиме.ВыполнитьМетодОбъекта'; en = 'Incorrect value of parameter ИмяМетода (%1) in РаботаВБезопасномРежиме.ВыполнитьМетодОбъекта'"), ИмяМетода);
	КонецПопытки;
...
Процедура ПроверитьИмяМетодаКонфигурации(Знач ИмяМетода) Экспорт
	...
	Если ЧастиИмени.Количество() <> 2 И ЧастиИмени.Количество() <> 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации'; en = 'Incorrect value of parameter ИмяМетода (value passe: ""%1"") in РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации'"), ИмяМетода);
	КонецЕсли;
	...
	Если ЧастиИмени.Количество() = 2 И Метаданные.ОбщиеМодули.Найти(ИмяОбъекта) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:
				...
				|en = 'Incorrect format of parameter ИмяМетода (value passed: ""%1"") in РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:
				|Common module ""%2"" not found.'"),
			ИмяМетода,
			ИмяОбъекта);
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Безопасное выполнение метода'; en = 'Safe execution of method'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неправильный формат параметра ИмяМетода (передано значение: ""%1"") в РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:
			           |Имя метода ""%2"" не соответствует требованиям образования имен процедур и функций.';
			           |en = 'Incorrect format of parameter ИмяМетода (value passed: ""%1"") in РаботаВБезопасномРежиме.ПроверитьИмяМетодаКонфигурации:
			           |Method name ""%2"" does not match requirements for procedures and functions.'"),
			ИмяМетода,
			ИмяМетодаОбъекта);
	КонецПопытки;
			...
			Иначе
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Невозможно выполнение обработчиков установки параметров сеанса по причине: профиль безопасности %1 отсутствует в кластере серверов 1С:Предприятия, или для него запрещено использование в качестве профиля безопасности безопасного режима.
						...
						|en = 'Cannot perform installation handlers session settings for a reason: security profile %1 does not exist in a server cluster, 1 c: Enterprise, or it is forbidden to use as safe mode security profile. 
						|
						|Recovery program to disable the use of the security profile of the cluster console and reconfigure the security profiles by using the configuration interface (commands are in the program settings).'"),
		...
		Если Не ДоступенПривилегированныйРежим Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно выполнение обработчиков установки параметров сеанса по причине: профиль безопасности %1 не содержит разрешения на установку привилегированного режима. Возможно, он был отредактирован через консоль кластера.
					|
					|Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';
					|en = 'Cannot perform installation handlers session settings for a reason: security profile %1 does not include permission to install kernel-mode. Perhaps it was edited through the cluster console.
					|
					|Recovery program to disable the use of the security profile of the cluster console and reconfigure the security profiles by using the configuration interface (commands are in the program settings).'"),
				ПрофильИнформационнойБазы);
		...
		Исключение
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно выполнение обработчиков установки параметров сеанса по причине: %1.
					...
					|en = 'You cannot execute the handlers set the parameters of a session reason: %1.
					|
					|Perhaps, for the information base through the cluster console was installed a security profile that does not allow execution of plug-ins without installing safe mode. In this case, the recovery program you need to disable use of the security profile of the cluster console and reconfigure the security profiles by using the configuration interface (the corresponding commands can be found under program options). The program will be automatically configured correctly to use in conjunction with security profiles enabled.'"),
...
Функция ВозможноВыполнениеОбработчиковУстановкиПараметровСеансаБезУстановкиБезопасногоРежима() Экспорт
	...
	Исключение
		...
		ШаблонЗаписиЖР = НСтр("ru = 'При установке параметров сеанса произошла ошибка:
			...
			|en = 'When setting the session parameters an error occurred: 
			|-------------------------------------------------------------------------------------------- 
			|%1 
			|-------------------------------------------------------------------------------------------- 
			|Program start will be impossible.'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Установка параметров сеанса'; en = 'Setting session parameters'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция МенеджерОбъектаПоИмени(Имя)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось получить менеджер для объекта ""%1""'; en = 'Failed to get object manager ""%1""'"), Имя);
