...
Процедура СформироватьКомандыЭДО(ПолноеИмя, КомандыЭДО, НаправлениеЭД = Неопределено, ТолькоВМенюЕще = Ложь) Экспорт
	...
	Если СоставКоманд.Исходящие.Найти(ПолноеИмя) <> Неопределено 
		...
		Если ЗначениеФОИспользоватьОбменЭД Тогда
			...
			КомандаЭДО.Представление = НСтр("ru = 'Просмотреть электронный документ'; en = 'Vew electronic document'");
			...
			КомандаЭДО.Представление = НСтр("ru = 'Создать электронный документ'; en = 'Generate electronic document'");
			...
			КомандаЭДО.Представление = НСтр("ru = 'Отправить электронный документ'; en = 'Send electronic document'");
			...
			КомандаЭДО.Представление = НСтр("ru = 'Открыть электронные документы'; en = 'Open electronic documents'");
			...
			КомандаЭДО.Представление = НСтр("ru = 'Текущие дела ЭДО'; en = 'Current EDI tasks'");
			...
			КомандаЭДО.Представление = НСтр("ru = 'Создать произвольный документ'; en = 'Create an arbitrary document'");
		...
		КомандаЭДО.Представление = НСтр("ru = 'Выгрузить документ в файл'; en = 'Export document to file'");
		КомандаЭДО.Порядок       = 50;
		...
		Если ЕстьПодсистемаБизнесСеть И СоставКоманд.БезПодписи.Найти(ПолноеИмя) <> Неопределено Тогда
			...
			КомандаЭДО.Представление = НСтр("ru = 'Отправить документ без электронной подписи'; en = 'Send the document without a signature'");
	...
	Если СоставКоманд.Входящие.Найти(ПолноеИмя) <> Неопределено 
		...
		Если ЗначениеФОИспользоватьОбменЭД Тогда
			...
			КомандаЭДО.Представление = НСтр("ru = 'Перезаполнить по электронному документу'; en = 'Refill by electronic document'");
			КомандаЭДО.Порядок       = 30;
		...
		КомандаЭДО.Представление = НСтр("ru = 'Загрузить документ из файла'; en = 'Import document from file'");
		КомандаЭДО.Порядок       = 50;
		...
		Если ЗначениеФОИспользоватьОбменБизнесСеть И СоставКоманд.БезПодписи.Найти(ПолноеИмя) <> Неопределено Тогда
			...
			КомандаЭДО.Представление = НСтр("ru = 'Загрузить документ без электронной подписи'; en = 'Import the document without digital signature'");
	...
	Если ЗначениеФОИспользоватьИнтеркампани И СоставКоманд.Интеркампани.Найти(ПолноеИмя) <> Неопределено Тогда
		...
		КомандаЭДО.СписокФорм    = "";
		КомандаЭДО.Идентификатор = "СформироватьПодписатьОтправитьЭД";
		КомандаЭДО.Представление = НСтр("ru = 'Подписать электронный документ'; en = 'Sign electronic document'");
...
Процедура ПроверитьКонтрагентаВ1СЭДОПриСозданииНаСервере(СсылкаНаКонтрагента) Экспорт
	...
	Попытка
		...
		ФоновыеЗадания.Выполнить("ОбменСКонтрагентамиСлужебный.ПроверитьКонтрагентаФоновоеЗадание", 
			Параметры, ПараметрыЗапуска.ИНН + " " + ПараметрыЗапуска.КПП, НСтр("ru = 'Проверка подключения'; en = 'Connection test'"));
	...
	Исключение
		...
		Операция = НСтр("ru = 'Проверка подключения контрагента к 1С-ЭДО в фоновом задании.'; en = 'Checking connection of counterparty to 1C-EDI in background job.'");
...
Процедура ЗаполнитьДанныеОПрофиляхНастроекЭДО() Экспорт
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			НастройкаЭДО.Комментарий = НСтр("ru = '##Настройка ЭДО помечена на удаление автоматически при обновлении.'; en = '##EDI configuration was marked for deletion automatically during upgrade.'");
				...
				Для Каждого ЗначениеПеречисления Из АктуальныеВидыЭД Цикл
					Если ЗначениеПеречисления <> Перечисления.ВидыЭД.Подтверждение
						...
						ИначеЕсли ЗначениеПеречисления = Перечисления.ВидыЭД.АктЗаказчик
							...
							ВерсияФормата = НСтр("ru = 'ФНС 5.01'; en = 'FTS 5.01'");
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				Строка.ВерсияФормата            = НСтр("ru = 'ФНС 5.01'; en = 'FTS 5.01'");
		...
		Пока ВыборкаУчастников.Следующий() Цикл
			...
			Если Не ВыборкаУчастников.СоглашениеОбИспользованииЭД.ПометкаУдаления Тогда
				...
				НастройкаЭДО.УстановитьПометкуУдаления(Истина);
				НастройкаЭДО.Наименование = НСтр("ru = 'Не использовать'; en = 'Do not use'") + " - " + НастройкаЭДО.Наименование;
				НастройкаЭДО.Комментарий = НСтр("ru = '##Настройка ЭДО помечена на удаление автоматически при обновлении.'; en = '##EDI configuration was marked for deletion automatically during upgrade.'");
				ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НастройкаЭДО);
			КонецЕсли;
...
Процедура НастроитьАвтоПереходНаНовыеВерсииФорматовЭД() Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		Операция = НСтр("ru = 'Обновление подсистемы обмена с контрагентами'; en = 'Updating subsystem on exchange with counterparties'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = НСтр("ru = 'При обновлении подсистемы обмена с контрагентами произошла ошибка'; en = 'There was an error while updating subsystem of exchange with banks'");
...
Процедура УдалитьНовыеВидыЭДУПД_УКД() Экспорт
	...
	Пока Результат.Следующий() Цикл
		...
		Для каждого Строка Из ИскомыйПрофильНастроек.ИсходящиеДокументы Цикл
			...
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.СчетФактура Тогда
				Строка.ВерсияФормата = ?(ИскомыйПрофильНастроек.ИспользоватьУПД, НСтр("ru = 'ФНС 5.01 (УПД:Счет-фактура и первичный документ)'; en = 'FTS 5.01 (УПД:VAT invoice and the primary document)'"), НСтр("ru = 'ФНС 5.02'; en = 'FTS 5.02'"));
				Строка.Приоритет = 4;
			КонецЕсли;
			...
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.КорректировочныйСчетФактура Тогда
				Строка.ВерсияФормата = ?(ИскомыйПрофильНастроек.ИспользоватьУКД, НСтр("ru = 'ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)'; en = 'FTS 5.01 (УКД:Correcting VAT invoice and the primary document)'"), НСтр("ru = 'ФНС 5.02'; en = 'FTS 5.02'"));
				Строка.Приоритет = 6;
			КонецЕсли;
	...
	Пока Результат.Следующий() Цикл
		...
		Для каждого Строка Из ИскомаяНастройка.ИсходящиеДокументы Цикл
			...
			КонецЕсли;
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.СчетФактура Тогда
				Строка.ВерсияФормата = ?(ИскомаяНастройка.ИспользоватьУПД, НСтр("ru = 'ФНС 5.01 (УПД:Счет-фактура и первичный документ)'; en = 'FTS 5.01 (УПД:VAT invoice and the primary document)'"), НСтр("ru = 'ФНС 5.02'; en = 'FTS 5.02'"));
				Строка.Приоритет = 4;
			КонецЕсли;
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель Тогда
			...
			КонецЕсли;
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.КорректировочныйСчетФактура Тогда
				Строка.ВерсияФормата = ?(ИскомаяНастройка.ИспользоватьУКД, НСтр("ru = 'ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)'; en = 'FTS 5.01 (УКД:Correcting VAT invoice and the primary document)'"), НСтр("ru = 'ФНС 5.02'; en = 'FTS 5.02'"));
				Строка.Приоритет = 6;
			КонецЕсли;
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
				Строка.Приоритет = 7;
	...
	Попытка
		...
		ЗафиксироватьТранзакцию();
	Исключение
		ВидОперации = НСтр("ru = 'Обновление информационной базы'; en = 'Infobase update'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		...
		Пока Результат.Следующий() Цикл
			...
			Если СтрокаВидЭД = Неопределено Тогда
				...
				ИначеЕсли ВидЭД = Перечисления.ВидыЭД.УдалитьУПД_ИнформацияПродавца Тогда
					...
					ВерсияФормата = НСтр("ru = 'ФНС 5.01 (СЧФ+ДОП)'; en = 'FTS 5.01 (СЧФ+ДОП)'");
				ИначеЕсли ВидЭД = Перечисления.ВидыЭД.УдалитьУКД_ИнформацияПродавца Тогда
					
					ВерсияФормата = НСтр("ru = 'ФНС 5.01 (КСЧФ+ДИС)'; en = 'FTS 5.01 (КСЧФ+ДИС)'");
				КонецЕсли;
				НоваяСтрока.ВерсияФормата = ВерсияФормата;
