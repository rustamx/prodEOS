...
Процедура ПроверитьКонтрагентовПослеВключенияПроверкиФоновоеЗадание() Экспорт
	...
	Попытка
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Проверка контрагентов'; en = 'Counterparties verification'");
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка контрагентов.Фоновое задание по обработке непроверенных контрагентов после включения'; en = 'Counterparty verification.Background job for processing of unverified counterparties after enabling'", КодОсновногоЯзыка),
...
Процедура ВыполнитьПроверкуКонтрагентов(ПроверяемыеКонтрагенты, Знач ВидПроверки = "Смешанный", Знач ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ВидПроверки <> "ИзКэша" И ВидПроверки <> "АктуальныеДанные" И ВидПроверки <> "Смешанный" Тогда
		ВызватьИсключение(НСтр("ru = 'Недопустимое значение параметра (параметр номер ''2'')'; en = 'Invalid parameter value (parameter number ''2'')'"));
	КонецЕсли;
...
Процедура УстановитьВидимостьИЗаголовокПредупрежденияПроТестовыйРежим(Элемент) Экспорт
	...
	Иначе
		...
		Элемент.Заголовок = 
			НСтр("ru = 'В настоящее время веб-сервис ФНС работает в тестовом режиме.'; en = 'Currently the FTS Web service works in test mode.'");
	КонецЕсли;
...
Процедура УстановитьНадписиВПанелиОтчетаПриСозданииНаСервере(Форма) Экспорт
	...
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Проверка контрагентов по данным ФНС выполнена успешно'; en = 'Verification of counterparties using the FTS data completed successfully.'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Есть некорректные контрагенты.
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриНекорректныхКонтрагентах");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Обнаружены недействующие контрагенты по данным ФНС'; en = 'Found discontinued counterparties according to the FTS data'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Выполняется проверка
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриПроверкеВПроцессеВыполнения");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Выполняется проверка контрагентов согласно данным ФНС'; en = 'Counterparties are being verified using FTS data'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Нет доступа к сервису
	ЭлементФормы = Форма.Элементы.Найти("ТекстНетДоступаКСервису");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Не удалось произвести проверку контрагентов: сервис ФНС временно недоступен'; en = 'Could not verify counterparties: FTS service is temporarily inaccessible'"), СсылкаНаИнструкцию);
	КонецЕсли;
	...
	Если ДанныеКонтрагентов.Количество() > 0 Тогда
		...
		Попытка
		 	ФоновоеЗадание = ФоновыеЗадания.Выполнить("ПроверкаКонтрагентов.ПроверитьКонтрагентовВДокументе", 
				Параметры, , НСтр("ru = 'Проверка контрагентов в документе'; en = 'Verification of counterparties in document'"));
		...
		Исключение
			...
			КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка контрагентов.Фоновое задание в документе'; en = 'Counterparty verification.Background job in document'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ПроверитьКонтрагентовВОтчете(Форма, СтруктураПараметров) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Проверка контрагентов в отчете'; en = 'Verification of counterparties in report'");
...
Процедура УдалитьРезультатПроверкиКонтрагентов(Контрагенты = Неопределено) Экспорт 
	...
	Исключение
		...
		НСтр("ru = 'Проверка контрагентов.Очистка состояний контрагентов'; en = 'Counterparty verification.Clearing counterparty states'", КодОсновногоЯзыка), 
...
Процедура УдалитьРезультатПроверкиКонтрагентовПоДокументам(Документы = Неопределено) Экспорт 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Проверка контрагентов.Очистка состояний документов'; en = 'Counterparty verification.Clearing document states'", КодОсновногоЯзыка), 
		УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Проверяет существование контрагентов по базе ФНС.
		...
		|en = 'Verifies existence of counterparties using the FTS data.
		|While in test mode the verification might not be successful for some counterparties'");
...
Процедура ПроверитьКонтрагентаФоновоеЗадание(Параметры) Экспорт 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка контрагентов.Ошибка при проверке из карточки контрагента'; en = 'Counterparty verification.Error when verifying from counterparty form'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ПроксиСервиса(Таймаут = 120)
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка контрагентов.Ошибка доступа к веб-сервису'; en = 'Counterparty verification.Error accessing Web service'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ОтветСервисаФНС(Прокси, WSЗапрос)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка контрагентов.NdsRequest2'; en = 'Counterparty verification.NdsRequest2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ДобавитьКонтрагентаВЗапросКСервису(WSЗапрос, Прокси, ДанныеКонтрагента)
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка контрагентов.Формирование данных для обращения к сервису ФНС'; en = 'Counterparty verification.Generating data for requests to Web service'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки; 
	...
	Для ИндексТекущегоКонтрагента = МинимальныйНомерКонтрагента По МаксимальныйНомерКонтрагента Цикл 
		...
		Если ДанныеКонтрагента.Состояние <> Перечисления.СостоянияСуществованияКонтрагента.КонтрагентСодержитОшибкиВДанных Тогда
			...
			Исключение
				...
				КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
				ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Проверка контрагентов.Обработка ответа от веб-сервиса'; en = 'Counterparty verification.Processing Web service response'", КодОсновногоЯзыка), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура СохранитьРезультатыПроверкиКонтрагентаВРегистр(ДанныеКонтрагента)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Проверка контрагентов.Запись результатов проверки контрагентов в регистр'; en = 'Counterparty verification.Writing counterparty verification results to register'", КодОсновногоЯзыка), 
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура СохранитьРезультатыПроверкиКонтрагентовВДокументеВРегистр(СохраняемыйРезультат)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Проверка контрагентов.Запись результатов проверки документов в регистр'; en = 'Counterparty verification.Writing document verification results to register'", КодОсновногоЯзыка), 
		УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция НовоеПолеКартинки(Форма, ТаблицаФормы, Родитель, ПолеСКонтрагентом)
	...
	НовоеПоле.Заголовок					= НСтр("ru = 'Есть ошибка по данным ФНС'; en = 'Has error according to FTS'");
