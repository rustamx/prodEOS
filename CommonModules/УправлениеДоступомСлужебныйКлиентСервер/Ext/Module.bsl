...
Процедура ЗаполнитьВсеРазрешеныПредставление(Форма, ОписаниеВидаДоступа, ДобавитьКоличествоЗначений = Истина) Экспорт
		...
		Иначе
			ЧислоИПредмет = Формат(КоличествоЗначений, "ЧГ=") + " "
				+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоЗначений,
					"Л = ru_RU", НСтр("ru = 'значение,значения,значений,,,,,,0'; en = 'value, value, values,,,,,,0'"));
		КонецЕсли;
	...
	Иначе
		ЧислоИПредмет = Формат(КоличествоЗначений, "ЧГ=")
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоЗначений,
				"Л = ru_RU", НСтр("ru = '-го значения,-х значений,-и значений,,,,,,0'; en = 'values, values, values,,,,,,0'"));
		
		Представление = СтрШаблон(
			?(ОписаниеВидаДоступа.ВсеРазрешены,
				НСтр("ru = 'Все разрешены, кроме %1'; en = 'All permitted, except for %1'"),
				НСтр("ru = 'Все запрещены, кроме %1'; en = 'All are forbidden, except for %1'")),
			ЧислоИПредмет);
	КонецЕсли;
...
Процедура ПриИзмененииТекущегоВидаДоступа(Форма) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущиеДанные.ВидДоступа = Форма.ВидДоступаПользователи Тогда
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				НСтр("ru = 'Запрещенные значения (%1) - текущий пользователь всегда разрешен'; en = 'Forbidden values (%1) - current user is always allowed'"),
		...
		ИначеЕсли ТекущиеДанные.ВидДоступа = Форма.ВидДоступаВнешниеПользователи Тогда
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				...
				НСтр("ru = 'Разрешенные значения (%1) - текущий внешний пользователь всегда разрешен'; en = 'Allowed values (%1) - current external user is always allowed'") );
		Иначе
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				НСтр("ru = 'Запрещенные значения (%1)'; en = 'Forbidden values (%1)'"),
	...
	Если Форма.ТекущиеТипыВыбираемыхЗначений.Количество() = 0 Тогда
		Форма.ТекущиеТипыВыбираемыхЗначений.Добавить(Неопределено, НСтр("ru = 'Неопределено'; en = 'Undefined'"));
	КонецЕсли;
	...
	Пока НЕ Отказ И ИндексВидаДоступа >= 0 Цикл
		...
		Если СтрокаВидаДоступа.ВидДоступа = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				НСтр("ru = 'Вид доступа не выбран.'; en = 'Access type is not selected.'"),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				НСтр("ru = 'Вид доступа в строке %1 не выбран.'; en = 'Access type on row %1 is not selected.'"),
		...
		Если Параметры.Назначение <> Неопределено
		  И Не ВидДоступаСоответствуетНазначениюПрофиля(СтрокаВидаДоступа.ВидДоступа, НазначениеПрофиля) Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				Параметры.ПутьКТаблицам + "ВидыДоступа[%1].ВидДоступа",
				СтрШаблон(НСтр("ru = 'Вид доступа ""%1"" не соответствует назначению профиля.'; en = 'Access type ""%1"" does not match the profile destination.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				СтрШаблон(НСтр("ru = 'Вид доступа ""%1"" в строке %%1 не соответствует назначению профиля.'; en = 'Access type ""%1"" in the row %%1 does not match the profile destination.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				Параметры.ВидыДоступа.Индекс(СтрокаВидаДоступа));
			Отказ = Истина;
			Прервать;
		...
		Если НайденныеВидыДоступа.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				Параметры.ПутьКТаблицам + "ВидыДоступа[%1].ВидДоступа",
				СтрШаблон(НСтр("ru = 'Вид доступа ""%1"" повторяется.'; en = 'Access type ""%1"" is repeated.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				СтрШаблон(НСтр("ru = 'Вид доступа ""%1"" в строке %%1 повторяется.'; en = 'Access type ""%1"" in the row %%1 is repeated.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				Параметры.ВидыДоступа.Индекс(СтрокаВидаДоступа));
			Отказ = Истина;
			Прервать;
		...
		Пока НЕ Отказ И ИндексЗначенияДоступа >= 0 Цикл
			...
			Если СтрокаЗначенияДоступа.ЗначениеДоступа = Неопределено Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					НСтр("ru = 'Значение не выбрано.'; en = 'Value is not selected.'"),
					"ЗначенияДоступа",
					ЗначенияДоступа.Найти(СтрокаЗначенияДоступа),
					НСтр("ru = 'Значение в строке %1 не выбрано.'; en = 'Value at row %1 is not selected.'"),
			...
			Если НайденныеЗначения.Количество() > 1 Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					Параметры.ПутьКТаблицам + "ЗначенияДоступа[%1].ЗначениеДоступа",
					НСтр("ru = 'Значение повторяется.'; en = 'The value is repeated.'"),
					"ЗначенияДоступа",
					ЗначенияДоступа.Найти(СтрокаЗначенияДоступа),
					НСтр("ru = 'Значение в строке %1 повторяется.'; en = 'Value at row %1 is repeated.'"),
					Параметры.ЗначенияДоступа.Индекс(СтрокаЗначенияДоступа));
				Отказ = Истина;
				Прервать;
			КонецЕсли;
