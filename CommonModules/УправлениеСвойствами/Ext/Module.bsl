		...
		Если Строка.ЗаполнятьОбязательно И НЕ Строка.Удалено Тогда
			...
			Если НЕ ЗначениеЗаполнено(Форма[Строка.ИмяРеквизитаЗначение]) Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Поле ""%1"" не заполнено.'; en = 'Field ""%1"" is not filled in.'"), Строка.Наименование),
					"");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьСписокСвойств(ВладелецСвойств, ПолучатьДопРеквизиты = Истина, ПолучатьДопСведения = Истина) Экспорт
	...
	// ТСК Мусабеков Роман {
	//ххх#25.05.2018#ххх
	Если ра_ОбщегоНазначенияКлиентСервер.ЭтоДокументКачества(ВладелецСвойств) тогда
		Возврат Новый Массив;
	КонецЕсли;
	//} ТСК Мусабеков Роман
...
Процедура СоздатьОсновныеОбъектыФормы(Форма, Контекст, СоздатьОписаниеДополнительныхРеквизитов)
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
			...
			Если Пользователи.РолиДоступны("ДобавлениеИзменениеДополнительныхРеквизитовИСведений") Тогда
				...
				Команда.Заголовок = НСтр("ru = 'Изменить состав дополнительных реквизитов'; en = 'Change additional attribute content'");
				Команда.Действие = "Подключаемый_РедактироватьСоставСвойств";
				Команда.Подсказка = НСтр("ru = 'Изменить состав дополнительных реквизитов'; en = 'Change additional attribute content'");
	...
	Если ОпцияИспользоватьСвойства
		...
		Если Родитель <> Неопределено Тогда
			...
			ПредупреждениеОРазмещении = НСтр("ru = 'Для отображения дополнительных реквизитов необходимо разместить группу ""%1"" не первым элементом (после любой другой группы) в группе ""%2"" (меню Еще - Изменить форму).'; en = 'To display additional attributes one should place group ""%1"" not as the first element (after any other group) in group ""%2"" (menu More - Change form).'");
