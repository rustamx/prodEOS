...
Процедура ДополнитьПолучателей(Объект, Узлы) Экспорт
	...
	Для Каждого Элемент Из Узлы Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru = 'Для состава плана обмена [ИмяПланаОбмена] не указана регистрация объекта [ПолноеИмя]'; en = 'Content of exchange plan [ИмяПланаОбмена] does not include reigstration for object [ПолноеИмя]'");
	...
	Иначе
		...
		Если РежимВыгрузкиОбъекта = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости Тогда
			...
			Если ЭтоНовыйОбъект Тогда
				ВызватьИсключение НСтр("ru = 'Регистрация незаписанных объектов, выгружаемых по ссылке, не поддерживается.'; en = 'Registration of unsaved objects, exported by reference, is not supported'");
			КонецЕсли;
...
Процедура ПроверитьВозможностьИзмененияНастроекОбменаДанными(Источник, Отказ) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		ВызватьИсключение НСтр("ru = 'Изменение наименования и кода синхронизации данных недопустимо.'; en = 'It is forbidden to change descriptions and codes of data synchronizations.'");
...
Процедура ЗарегистрироватьИзменениеОбъекта(ИмяПланаОбмена, Объект, Отказ, ДополнительныеПараметры = Неопределено)
		...
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			Иначе
				...
				Если РазделенныеДанные(ОбъектМетаданных) Тогда
					ВызватьИсключение НСтр("ru = 'Регистрация изменений разделенных данных в неразделенном режиме.'; en = 'Registration of changes of separated data in unseparated mode'");
				КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными.Правила регистрации объектов'; en = 'Data exchange.Object registration rules'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОбъектМодифицированДляПланаОбмена(Источник, ОбъектМетаданных, ИмяПланаОбмена, РежимЗаписи, ЗарегистрироватьОбъектКВыгрузке)
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка определения модифицированности объекта: %1'; en = 'Error determining object modification state: %1'"),
...
Процедура ВыполнитьПравилаРегистрацииОбъектовДляПланаОбмена(МассивУзловРезультат, Объект, ИмяПланаОбмена, ДополнительныеПараметры)
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|Описание ошибки:
			|%2';
			|en = 'Error executing object registration rules for exchange plan %1.
			|Error description:
			|%2'"),
			ИмяПланаОбмена,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПолучитьЗначенияСвойствДляСсылки(Ссылка, СвойстваОбъекта, Знач СвойстваОбъектаСтрокой, Знач ОбъектМетаданныхИмя) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("ru = 'Ошибка при получении свойств ссылки. Ошибка выполнения запроса: [ОписаниеОшибки]'; en = 'Error retrieving properties of reference. Query execution error: [ОписаниеОшибки]'");
...
Функция МассивУзловПоЗначениямСвойств(ЗначенияСвойств, Знач ТекстЗапроса, Знач ИмяПланаОбмена, Знач ИмяРеквизитаФлага, Знач Выгрузка = Ложь) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("ru = 'Ошибка при получении списка узлов получателей. Ошибка выполнения запроса: [ОписаниеОшибки]'; en = 'Error retrieving list of recipient nodes. Error executing query: [ОписаниеОшибки]'");
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОписаниеОшибки]", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СтрокаСообщения;
	КонецПопытки;
...
Функция НаборЗаписейПоТипу(ОбъектМетаданных)
	...
	Иначе
		...
		СтрокаСообщения = НСтр("ru = 'Для объекта метаданных %1 не предусмотрено набора записей.'; en = 'Metadata object %1 does not support recordset property.'");
			...
			Если СтрокаДерева.ВидЭлементаОтбора = ОбменДаннымиСервер.ЭлементОтбораСвойствоАлгоритмЗначения() Тогда
				...
				Исключение
					...
					СтрокаСообщения = НСтр("ru = 'Ошибка алгоритма вычисления значения константы:
												...
												|// {Окончание алгоритма}
												|';
												|en = 'Error in algorithm for constant calculation:
	...
	Если ПРО.ЕстьОбработчикПередОбработкой Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru = 'Ошибка при выполнении обработчика: ""[ИмяОбработчика]"";
				...
				|Объект метаданных: [ОбъектМетаданныхИмя]
				|Описание ошибки: [Описание]';
				|en = 'Error occurred while executing handler: ""[ИмяОбработчика]"";
				|Exchange plan: [ИмяПланаОбмена];
				|Metadata object: [ОбъектМетаданныхИмя]
				|Error description: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ru = 'Перед обработкой'; en = 'Before processing'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
	...
	Если ПРО.ЕстьОбработчикПриОбработке Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru = 'Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]
				|Описание ошибки: [Описание]';
				|en = 'Error occurred while executing handler: ""[ИмяОбработчика]""; Exchange plan: [ИмяПланаОбмена]; Metadata object: [ОбъектМетаданныхИмя]
				|Error description: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ru = 'При обработке'; en = 'When processing'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
	...
	Если ПРО.ЕстьОбработчикПриОбработкеДополнительный Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru = 'Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]
				...
				|en = 'Error occurred while executing handler: ""[ИмяОбработчика]""; Exchange plan: [ИмяПланаОбмена]; Metadata object: [ОбъектМетаданныхИмя]
				|Error description: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ru = 'При обработке (дополнительный)'; en = 'When processing (additional)'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
	...
	Если ПРО.ЕстьОбработчикПослеОбработки Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru = 'Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]
				...
				|en = 'Error occurred while executing handler: ""[ИмяОбработчика]""; Exchange plan: [ИмяПланаОбмена]; Metadata object: [ОбъектМетаданныхИмя]
				|Error description: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("ru = 'После обработки'; en = 'After processing'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
...
Функция ДанныеРазличаются(Данные1, Данные2, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru = 'Задано недопустимое значение параметра [1] метода ОбщегоНазначения.ЗначенияСвойствИзменены.'; en = 'Incorrect value of parameter [1] of method of ОбщегоНазначения.ЗначенияСвойствИзменены.'");
		...
		Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель) Тогда
			Если СсылкаСуществует Тогда
				...
				Если ЗаписатьОбъект Тогда
					Комментарий = НСтр("ru = 'Предыдущая версия (автоматическое разрешение конфликта).'; en = 'Previous version (automatic conflict resolution).'");
				Иначе
					Комментарий = НСтр("ru = 'Текущая версия (автоматическое разрешение конфликта).'; en = 'Current version (automatic conflict resolution).'");
				КонецЕсли;
			...
			Если ЗаписатьОбъект Тогда
				...
				СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("ru = 'Текущая версия (автоматическое разрешение конфликта).'; en = 'Current version (automatic conflict resolution).'"));
			Иначе
				СведенияОВерсииОбъекта = Новый Структура;
				СведенияОВерсииОбъекта.Вставить("АвторВерсии", Отправитель);
				СведенияОВерсииОбъекта.Вставить("ТипВерсииОбъекта", "НепринятыеДанныеПоКоллизии");
				СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("ru = 'Отклоненная версия (автоматическое разрешение конфликта).'; en = 'Rejected version (automatic conflict resolution).'"));
			КонецЕсли;
...
Процедура ЗарегистрироватьЗапретЗагрузкиДанныхПоДате(ЭлементДанных, Отправитель, СообщениеОбОшибке)
	...
	Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель.Ссылка) И ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ЭлементДанных.Метаданные()) Тогда
		...
		Если СсылкаСуществует Тогда
			...
			Комментарий = НСтр("ru = 'Версия создана при синхронизации данных.'; en = 'Version created during data synchronization.'");
		...
		Иначе
			...
			СтрокаСообщенияОбОшибке = НСтр("ru = '%1 запрещено загружать в запрещенный период.%2%2%3'; en = '%1 forbidden to import beyond the prohibition date.%2%2%3'");
...
Функция ПолучитьДанныеОбъектаСтрокойДоИзменения(Объект, МетаданныеОбъекта, ЭтоСсылочныйТип, СсылкаСуществует, ОбъектВБазе)
	...
	Если ЭтоСсылочныйТип Тогда
		...
		Иначе
			...
			СтрокаОбъекта = НСтр("ru = 'Объект удален'; en = 'Object deleted'");
