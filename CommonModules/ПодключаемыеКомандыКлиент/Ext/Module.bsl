...
Процедура ПродолжитьВыполнениеКоманды(Контекст)
	...
	Если Контекст.ТребуетсяРаботаСФайлами Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для продолжения необходимо установить расширение для веб-клиента 1С:Предприятие.'; en = 'To proceed, you must install the Web client extension for 1C:Enterprise.'");
		...
		Если ИнформацияОДокументах.КоличествоНеПроведенных > 0 Тогда
			Если ИнформацияОДокументах.ЕстьПравоПроведенияХотяБыОдногоНепроведенного Тогда
				Если ИнформацияОДокументах.КоличествоНеПроведенных = 1 Тогда
					ТекстВопроса = НСтр("ru = 'Для выполнения команды необходимо предварительно провести документ. Выполнить проведение документа и продолжить?'; en = 'To execute the command, you must first post the document. Post the document and continue?'");
				Иначе
					ТекстВопроса = НСтр("ru = 'Для выполнения команды необходимо предварительно провести документы. Выполнить проведение документов и продолжить?'; en = 'To execute the command, you must first post the documents.  Post the documents and continue?'");
				КонецЕсли;
			...
			Иначе
				Если ИнформацияОДокументах.КоличествоНеПроведенных = 1 Тогда
					ТекстПредупреждения = НСтр("ru = 'Для выполнения команды необходимо предварительно провести документ. Недостаточно прав для проведения документа.'; en = 'To execute the command, you must first post the document. Insufficient permissions to post the document.'");
				Иначе
					ТекстПредупреждения = НСтр("ru = 'Для выполнения команды необходимо предварительно провести документы. Недостаточно прав для проведения документов.'; en = 'To execute the command, you must first post the documents. Insufficient permissions to post the documents.'");
				КонецЕсли;
...
Процедура ПродолжитьВыполнениеКомандыПослеПодтверждениеПроведения(Ответ, Контекст) Экспорт
	...
	Если НепроведенныеДокументы.Количество() > 0 Тогда
		...
		Если Контекст.МассивСсылок.Количество() = 0 Тогда
			КнопкиДиалога.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru = 'ОК'; en = 'ОК'"));
		Иначе
			ТекстДиалога = ТекстДиалога + " " + НСтр("ru = 'Продолжить?'; en = 'Continue?'");
