...
Функция ЗапросДанныхВыбораПоТексту(Текст, Параметры, ПоляВводаПоСтроке) Экспорт
	...
	Если Параметры.Свойство("СтрокаПоиска") Тогда
		...
		Оператор.Отбор.Добавить(ПолучитьУсловиеПоиска(ПоляВводаПоСтроке, СпособПоискаСтроки, Параметры.СтрокаПоиска));
...
Функция ПолучитьУсловиеПоиска(ПоляВводаПоСтроке, СпособПоискаСтроки, Знач СтрокаПоиска) Экспорт
	
	СтрокаПоиска = СтрЗаменить(СтрокаПоиска, """", """""");
	СтрокаШаблон = ?(СпособПоискаСтроки = СпособПоискаСтрокиПриВводеПоСтроке.Начало, "%1 ПОДОБНО ""%%%2""", "%1 ПОДОБНО ""%%%2%%""");
...
Процедура ПереопределитьДескрипторыЗависимыхОбъектов(Ссылка) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ra_Nesootvetstvie") Тогда
		Для каждого Документ Из Документы.ra_Nesootvetstvie.ПроизводныеДокументы(Ссылка) Цикл
			ДокументооборотПраваДоступа.ОпределитьДескрипторыОбъекта(Документ);
		КонецЦикла;
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ra_ZayavkaNaKontrolnuyuOperaciyu") Тогда
		Для каждого Документ Из Документы.ra_ZayavkaNaKontrolnuyuOperaciyu.ПроизводныеДокументы(Ссылка) Цикл
			ДокументооборотПраваДоступа.ОпределитьДескрипторыОбъекта(Документ);
		КонецЦикла;
	КонецЕсли;
...
Функция ТекущийПользовательИсполнительИлиДелегат(Исполнитель) Экспорт
