...
Процедура ПолучитьXMLИзОбъекта(ЗаписьXML, ВерсияСервиса, ПараметрыСинхронизации, ЭлементДанных,
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("ДокументСсылка.ra_Nesootvetstvie") Тогда

		ВыгрузитьВXMLra_Nesootvetstvie(ЗаписьXML, ВерсияСервиса, ПараметрыСинхронизации, Объект);
	// Ткаченко И.Л. Искать в тексте "ДоработкиМП" 19.12.2018 }
	Иначе
...
Процедура ВыгрузитьВXMLra_Signal(ЗаписьXML, ВерсияКлиента, ПараметрыСинхронизации, ЭлементДанных)
	...
	ЗаписьXMLОбъекта.ЗаписатьНачалоЭлемента("Proekt");
	ЗаписатьИдентификаторЭлементаДанных(ЗаписьXMLОбъекта, "ID", Реквизиты.Proekt);
    ЗаписатьЭлементОбъектаДанных(ЗаписьXMLОбъекта, "Name", Строка(Реквизиты.Proekt));
	ЗаписьXMLОбъекта.ЗаписатьКонецЭлемента();
	...
	//ВыгрузитьВXMLФайлы(ЗаписьXMLОбъекта, "206", ПараметрыСинхронизации, ЭлементДанных);
...
Процедура ВыгрузитьВXMLra_Nesootvetstvie(ЗаписьXML, ВерсияКлиента, ПараметрыСинхронизации, ЭлементДанных)

	Если ПараметрыСинхронизации.ВыгруженныеОбъекты[ЭлементДанных.Ссылка] = Истина Тогда
		Возврат;
	КонецЕсли;

	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЭлементДанных, "Ссылка, Проведен, Дата, DataVyyavleniya, NarushennyeTrebovaniya, NomerVnutrennij, PodrobnoeOpisanie, VidNesootvetstviya");

	// Запись данных
	ЗаписьXMLОбъекта = ЗаписатьЗаголовокОбъекта("Nesootvetstvie", "206");
