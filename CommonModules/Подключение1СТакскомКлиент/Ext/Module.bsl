...
Процедура ПолучитьУникальныйИдентификаторАбонента(
	...
	Если ТипЗнч(СертификатЭП) <> Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования")
		...
		ВызватьИсключение НСтр("ru = 'Некорректное значение параметра ""СертификатЭП"".'; en = 'Incorrect value of parameter ""СертификатЭП"".'");
	КонецЕсли;
...
Процедура ЗарегистрироватьСертификатАбонента(
	...
	ИначеЕсли ТипЗнч(ИдентификаторАбонента) <> Тип("Строка") Или ПустаяСтрока(ИдентификаторАбонента) Тогда
		ВызватьИсключение НСтр("ru = 'Некорректное значение параметра ""ИдентификаторАбонента"".'; en = 'Incorrect value of parameter ""ИдентификаторАбонента"".'");
	КонецЕсли;
...
Процедура СтартоватьМеханизмРаботыСОператоромЭДО(
	...
	Если Не ЗначениеЗаполнено(СертификатЭП) Тогда
		...
		ТекстОшибки = НСтр("ru = '""Сертификат абонента""'; en = '""Certificate Subscriber""'");
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		...
		Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
			ТекстОшибки = ТекстОшибки + " " + НСтр("ru = 'и'; en = 'and'") + " ";
		КонецЕсли;
		
		ТекстОшибки = ТекстОшибки + НСтр("ru = '""Организация""'; en = '""Company""'");
		КоличествоОшибок = КоличествоОшибок + 1;
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Если КоличествоОшибок = 1 Тогда
			ДопТекст = НСтр("ru = 'Пожалуйста, заполните поле'; en = 'Please, fill field'") + " ";
		Иначе
			ДопТекст = НСтр("ru = 'Пожалуйста, заполните поля'; en = 'Please, fill fields'") + " ";
		КонецЕсли;
		...
		Если ВариантБизнесПроцесса = "taxcomGetID" Тогда
			ТекстСообщения = ТекстСообщения
				+ " " + НСтр("ru = 'до получения уникального идентификатора участника обмена ЭД'; en = 'to obtain a unique identifier attendee ED exchange'");
		ИначеЕсли ВариантБизнесПроцесса = "taxcomPrivat" Тогда
			ТекстСообщения = ТекстСообщения + " " + НСтр("ru = 'до перехода в личный кабинет'; en = 'prior to joining personal cabinet'");
		КонецЕсли;
	...
	Если ВариантБизнесПроцесса = "taxcomGetID" Тогда
		...
		Если ПустаяСтрока(ИдентификаторОрганизации) Тогда
			...
			НСтр("ru = 'Получение уникального идентификатора участника обмена ЭД будет доступно
					   ...
					   |Продолжить?';
					   |en = 'Receive a unique ID attendee ED exchange will be available after connecting to service online customer support and the owner of the specified authentication certificate subscriber agreement.
					   |Continue?'");
		Иначе
			ТекстОповещенияПользователя =
			НСтр("ru = 'Добавление нового сертификата в соглашение будет выполнено
					   ...
					   |подлинности владельца указанного в соглашении сертификата абонента.
					   |Продолжить?';
					   |en = 'The certificate will be added to the agreement after connecting to online customers support
					   |service and validating the certificate owner that is specified in subscriber''s agreement. 
					   |Continue?'");
		КонецЕсли;
	...
	ИначеЕсли ВариантБизнесПроцесса = "taxcomPrivat" Тогда
		...
		НСтр("ru = 'Вход в личный кабинет участника обмена ЭД будет доступен
				   ...
				   |подлинности владельца указанного в соглашении сертификата абонента.
				   |Продолжить?';
				   |en = 'Entrance to personal office attendee ED exchange will be available after connecting to service online customer support and the owner of the specified authentication certificate subscriber agreement.
				   |Continue?'");
	...
	Иначе
		...
		ТекстПредупреждения = СтрЗаменить(
			НСтр("ru = 'Ошибка встраивания механизма Интернет-поддержки пользователей.
						...
						|en = 'Error embedding mechanism online customer support.
						|An incorrect version of the business process (%1).'"),
			"%1",
			ВариантБизнесПроцесса);
...
Процедура РасшифроватьКонтрольныйМаркерЭПСервера(
	...
	СообщениеОбОшибкеДляПользователя = НСтр("ru = 'Ошибка при проверке пароля сертификата.
		...
		|en = 'Error validating certificate password.
	...
	Если ПараметрыДляРасшифрования.markerED = Неопределено Тогда
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Ошибка при проверке подлинности владельца сертификата. Отсутствует маркер аутентификации (markerED)'; en = 'An error occurred while authenticating the certificate owner. Authentication token is missing (markerED)'"));
	...
	Исключение
		...
		СообщениеДляЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при проверке подлинности владельца сертификата. Не удалось получить двоичные данные маркера (markerED).
				|%1';
				|en = 'An error occurred while authenticating the certificate owner. Failed to retrieve binary data marker (markerED).
				|%1'"),
	...
	Иначе
		...
		ИнтернетПоддержкаПользователейКлиент.ЗавершитьБизнесПроцесс(КонтекстВзаимодействия);
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Ошибка при проверке подлинности владельца сертификата. Не указан сертификат (IDCertificateED, IDCertificateED_Dop)'; en = 'An error occurred while authenticating the certificate owner. Certificate is not specified (IDCertificateED, IDCertificateED_Dop)'"));
		ПоказатьПредупреждение(, СообщениеОбОшибкеДляПользователя);
		Возврат;
	...
	ОписаниеДанных.Вставить("Операция", НСтр("ru = 'Проверка пароля сертификата'; en = 'Check certificate password'"));
...
Процедура ПоказатьПричинуОтклоненияЗаявкиЭДО(КонтекстВзаимодействия) Экспорт
	...
	Если НЕ ПустаяСтрока(ОписаниеПричины.КодОшибки) Тогда
		ТекстСообщения = НСтр("ru = 'Код ошибки: %1'; en = 'Error code: %1'");
	...
	Если ПустаяСтрока(ОписаниеПричины.ТекстОшибки) Тогда
		ТекстСообщения = НСтр("ru = 'Неизвестная ошибка. Обратитесь в службу техподдержки.'; en = 'Unknown error. Contact tech support.'");
	КонецЕсли;
...
Функция ТекстТехническихПараметровЭДО(КонтекстВзаимодействия, Знач Сертификат = Неопределено) Экспорт
	...
	ТехническиеПараметры = НСтр("ru = 'Параметры участника обмена ЭД:
		...
		|en = 'Attendee ED exchange options:
