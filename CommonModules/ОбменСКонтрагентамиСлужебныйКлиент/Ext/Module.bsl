...
Процедура ТестНастроекСПроверкойСертификата(Сертификат, ВладелецФормы, ПроверитьАвторизацию,
	...
	ЗаголовокФормы = НСтр("ru = 'Проверка сертификата %1'; en = 'Certificate validation %1'");
		...
		Для Каждого Элемент Из СтруктСтруктурМассивовЭДИСертификатов Цикл
			...
			Если Структура.Свойство("СоответствиеЭдИДд", СоответствиеЭдИДд)
				...
				Если СоответствиеЭдИДд.Количество() = 1 Тогда
					Операция = НСтр("ru = 'Подписание электронного документа'; en = 'Electronic document signing'");
				Иначе
					Операция = НСтр("ru = 'Подписание электронных документов'; en = 'Electronic documents signing'");
				КонецЕсли;
				...
				Иначе
					ПредставлениеЭД = НСтр("ru = 'Электронные документы (%1)'; en = 'Electronic documents (%1)'");
...
Процедура ОтправитьПолучитьЭДВыполнить(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Выполняется получение информации о приглашениях. Подождите...'; en = 'Retrieves information on the invitations. Please wait...'");
	Состояние(НСтр("ru = 'Получение.'; en = 'Receiving.'"), , ТекстСообщения);
	...
	ТекстСообщения = НСтр("ru = 'Выполняется отправка и получение пакетов электронных документов. Подождите...'; en = 'Sends and receive packets of electronic documents. Please wait...'");
	Состояние(НСтр("ru = 'Отправка и получение.'; en = 'Sending and receiving.'"), , ТекстСообщения);
	...
	ШаблонОповещения = НСтр("ru = 'Отправлено пакетов: (%1), получено пакетов: (%2).'; en = 'Packets sent: (%1), packets received: (%2).'");
	...
	ТекстСообщения = НСтр("ru = 'Выполняется распаковка пакетов электронных документов. Подождите...'; en = 'Running unpacking packages of electronic documents. Please wait...'");
	Состояние(НСтр("ru = 'Распаковка.'; en = 'Unpacking.'"), , ТекстСообщения);
...
Процедура ПолучитьМаркерПродолжить(Отпечатки, ДополнительныеПараметры) Экспорт
	...
	Если СвойстваСертификата.Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Нет доступных сертификатов. Выполнение операции %1 прервано.'; en = 'No available certificates. Operation %1 was terminated.'");
	...
	Если ЗашифрованныеДанные = Неопределено Тогда
		...
		ПредставлениеОперации = ДополнительныеПараметры.ПредставлениеОперации;
		ТекстСообщения = НСтр("ru = 'Ошибка получения данных от сервиса ЭДО. Выполнение операции %1 прервано.'; en = 'Error while receiving data from EDI service. Operation %1 was terminated.'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, ПредставлениеОперации);
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",   НСтр("ru = 'Аутентификация в сервисе ЭДО'; en = 'EDI service authentication'"));
	ОписаниеДанных.Вставить("ОтборСертификатов",   МассивСертификатов);
...
Процедура РасшифроватьЭлектронныеДокументыВнутреннийЦиклНачало(Контекст)
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",            НСтр("ru = 'Расшифровка электронного документа'; en = 'Decryption of electronic document'"));
...
Процедура РасшифроватьЭлектронныеДокументыВнутреннийЦиклПослеРасшифровкиЭД(РезультатВыполнения, Контекст) Экспорт
	...
	Если Не РезультатВыполнения.Успех Тогда
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", НСтр("ru = 'Не удалось расшифровать электронный документ из пакета.'; en = 'Failed to decrypt electronic document from the package.'"));
		ОбработатьПакетыЭДЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклПослеОшибкиВыгрузкиСертификата(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Выгрузка сертификата: %1'; en = 'Exporting certificate: %1'");
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ИнформацияОбОшибке);
...
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклПослеВыгрузкиСертификата(ДанныеСертификата, Контекст) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Контекст.Подпись.АдресЭД) Тогда
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", НСтр("ru = 'Не удалось найти исходные данные для проверки ответной подписи.'; en = 'Could not find the source data to verify the signature of the respondent.'"));
		ОбработатьПакетыЭДЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьПакетыЭДПослеЦикла(Контекст)
	...
	Для ОбратныйИндекс = 1 По КоличествоЭлементовКоллекции Цикл
		...
		Если ПакетЭД.Свойство("ОписаниеОшибки") Тогда
			...
			|%2.';
			|en = 'When unpacking ED package: %1 an error occurred: 
			|%2'");
	...
	ШаблонОповещения = НСтр("ru = 'Распаковано пакетов: %1.'; en = 'Unpacked packages: %1.'");
...
Процедура ОпределитьСтатусОчереднойПодписи(Результат, Параметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Результат) = Тип("Строка") Тогда
		ВидОперации = НСтр("ru = 'Проверка подписи'; en = 'Signature verification'");
		ТекстСообщения = НСтр("ru = 'При проверке подписи электронного документа:
									|%1
									|произошла ошибка: %2';
									|en = 'When verifying a signature of electronic document:
									|%1
									|an error occurred: %2'");
...
Процедура ОткрытьЭДДляПросмотраПослеПолученияОтпечатков(Отпечатки, Параметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИмяФормыЭлектронногоДокумента) Тогда
		ВызватьИсключение НСтр("ru='Электронный документ не предназначен для просмотра.'; en = 'Electronic document is not intended for browsing.'");
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(СоответствиеДанныхПакетов) = Тип("Соответствие") Тогда
		Если СоответствиеДанныхПакетов.Количество() > Параметры.ИндексПеребораПакетов Тогда
			...
			Если Параметры.ИндексПеребораДанных + 1 < МассивДанныхЭД.Количество() Тогда
				...
				Исключение
					ВидОперации = НСтр("ru = 'Шифрование данных'; en = 'Data encryption'");
					ТекстСообщения = НСтр("ru = 'При шифровании данных произошла ошибка: %1'; en = 'An error occurred while encrypting data: %1'");
...
Процедура СформироватьЭД(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	Если ПараметрКоманды.Количество() > 1 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Создать электронные документы для выделенных элементов?'; en = 'Create electronic documents for selected items?'");
...
Процедура СформироватьПодписатьОтправитьЭД(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	Если ПараметрКоманды.Количество() > 1 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Отправить электронные документы выделенных элементов?
			...
			|en = 'Send the electronic documents of the marked items?
			|If the electronic documents are missing, they will be created automatically.'");
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
