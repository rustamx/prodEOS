...
Функция ИнициализироватьПечатнуюФормуOOWriter(Знач Шаблон = Неопределено) Экспорт
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			НСтр("ru = 'Ошибка при связи с сервис менеджером (com.sun.star.ServiceManager).'; en = 'Error occurred while communicating with the service manager (com.sun. star. ServiceManager).'")
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			НСтр("ru = 'Ошибка при запуске сервиса Desktop (com.sun.star.frame.Desktop).'; en = 'Error starting service Desktop (com.sun. star frame. Desktop).'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),,Истина);
		НеУдалосьСформироватьПечатнуюФорму(ИнформацияОбОшибке());
	КонецПопытки;
...
Функция ПолучитьПозициюНачалаОбласти(Знач xDocument, Знач ИмяОбласти)
	...
	Если xFound = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не найдено начало области макета:'; en = 'Start the template not found:'") + " " + ИмяОбласти;	
	КонецЕсли;
	Возврат xFound.End;
...
Функция ПолучитьПозициюОкончанияОбласти(Знач xDocument, Знач ИмяОбласти)
	...
	xFound = xDocument.findFirst(xSearchDescr);
	Если xFound = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не найден конец области макета:'; en = 'Not found the end of template:'") + " " + ИмяОбласти;	
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации()
	Возврат НСтр("ru = 'Печать'; en = 'Print'");
КонецФункции

Процедура НеУдалосьСформироватьПечатнуюФорму(ИнформацияОбОшибке)
#Если ВебКлиент Тогда
	ТекстУточнения = Символы.ПС + НСтр("ru = 'При работе через веб, требуется браузер Internet Explorer под управлением операционной системы Windows.'; en = 'When working via Web, requires Internet Explorer running Windows operating system.'");
#Иначе		
	ТекстУточнения = "";	
#КонецЕсли
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		|en = 'To create print form in OpenOffice.org Writer format it is necessary to install the package OpenOffice.org onto the computer. %1
		|Technical details: %2'"),
