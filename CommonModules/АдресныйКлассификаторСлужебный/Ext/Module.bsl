...
Процедура ЗаполнитьВерсиюПоставщикаДанныхВнутр(Результат) Экспорт
	...
	Результат.Данные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'БСП %1'; en = 'SSL %1'"), СтандартныеПодсистемыСервер.ВерсияБиблиотеки());
...
Функция ОписаниеПоследнейЗагрузки(ИдентификаторРегиона = Неопределено) Экспорт
	...
	Иначе
		...
		Результат.Представление        = НСтр("ru = 'Адресный классификатор еще не загружался.'; en = 'Address classifier was not yet imported.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ДнейРазницы = 0 Тогда
		Представление = НСтр("ru = 'Адресный классификатор был загружен сегодня.'; en = 'Address classifier was imported today.'");
		
	ИначеЕсли ДнейРазницы = 1 Тогда
		Представление = НСтр("ru = 'Адресный классификатор был загружен вчера.'; en = 'Address classifier was imported yesterday.'");
			
	ИначеЕсли ДнейРазницы = 2 Тогда
		Представление = НСтр("ru = 'Адресный классификатор был загружен позавчера.'; en = 'Address classifier was imported the day before yesterday.'");
			
	ИначеЕсли ДнейРазницы > 2 Тогда
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Адресный классификатор был загружен %1 назад.'; en = 'Address classifier was imported %1 ago.'"),
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ДополнительныеПараметры) Экспорт
	...
	Если Уровень = 1 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите регион'; en = 'Select region'");
	ИначеЕсли Уровень = 2 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите округ'; en = 'Select county'");
	ИначеЕсли Уровень = 3 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите район'; en = 'Select district'");
	ИначеЕсли Уровень = 4 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите город'; en = 'Select city'");
	ИначеЕсли Уровень = 5 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите внутригородской район'; en = 'Select urban district'");
	ИначеЕсли Уровень = 6 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите населенный пункт'; en = 'Select settlement'");
	ИначеЕсли Уровень = 7 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите улицу'; en = 'Select street'");
	ИначеЕсли Уровень = 90 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите дополнительный элемент'; en = 'Select additional element'");
	ИначеЕсли Уровень = 91 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите подчиненный элемент'; en = 'Select subordinate element'");
	ИначеЕсли Уровень = -1 Тогда
		Результат.Заголовок = НСтр("ru = 'Выберите ориентир'; en = 'Select landmark'");
	КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли Уровень = 91 Тогда
			...
			ТекстЗапроса = ТекстЗапроса + "
				...
				|";
		Иначе
			ВызватьИсключение НСтр("ru = 'Запрос некорректного уровня адресного объекта'; en = 'Incorrect address object level query'");
		КонецЕсли;
...
Функция ЕстьСписокДомов(ИдентификаторАдресногоОбъекта) Экспорт
	...
	ИначеЕсли Источник = "Сервис1С" Тогда
		...
		Порция = Сервис.Autocomplete(ИдентификаторАдресногоОбъекта, 0, НСтр("ru = 'ДОМ='; en = 'BUILDING='"), 1, КодЯзыка, Метаданные.Имя);
...
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина) Экспорт
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		Если ОповещатьОПрогрессе Тогда
			ТекстОсталосьЗагрузить = ?(ВсегоСубъектов - НомерПоПорядку > 0, " (" + НСтр("ru = 'осталось'; en = 'left'")
				+ " " + Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=") + ")", "");
			ИнформацияОЗагрузке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка региона ""%1 - %2""'; en = 'Importing region ""%1 - %2""'"),
...
Процедура УдалитьВременныйФайл(ПолноеИмяФайла) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
			...
			|%1 по причине: %2';
			|en = 'Failed to delete temporary file
			|%1 because: %2'"), ПолноеИмяФайла, КраткоеПредставлениеОшибки(ИнформацияОбОшибке())));
	КонецПопытки
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	Если ТипЗнч(Адрес) <> Тип("ОбъектXDTO") Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Адрес", НСтр("ru = 'Невозможно проверить адрес из-за некорректных сведений. Проверьте правильность заполнения адреса.'; en = 'It was not possible to verify the address due to incorrect information. Check the correctness of addresses.'"));
		Возврат Результат;
	КонецЕсли;
	...
	Если ПустаяСтрока(СубъектРФ.Наименование) Тогда
		// Нет субъекта
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ru = 'Не указан субъект РФ адреса'; en = 'Subject of the Russian Federation is not specified in the address'") );
		Возврат Результат;
	ИначеЕсли Уровни.Найти(1) = Неопределено Тогда
		// В уровнях должно быть указано всегда.
		ВызватьИсключение НСтр("ru = 'В уровнях для проверки адреса не указан уровень субъекта РФ.'; en = 'In levels for address verification the level of subject of the Russian Federation is not specified.'");
	КонецЕсли;
	...
	Если РегионЗагруженВАдресныеСведения = Неопределено Тогда 
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Регион ""%1"" не существует'; en = 'Region ""%1"" does not exist'"), Адрес.СубъектРФ));
		Результат.АдресПроверен = Истина;
		Возврат Результат;
	ИначеЕсли НЕ РегионЗагруженВАдресныеСведения Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ru = 'Отсутствуют адресные сведения по региону'; en = 'Missing address information on the region'") + ": " + Адрес.СубъектРФ + ".");
	...
	Если РезультатЗапроса.Пустой() Тогда
		// Нет региона
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Субъект РФ (регион) ""%1"" не найден в адресном классификаторе'; en = 'Subject of the Russian Federation (region) ""%1"" is not found in the address classifier'"), СубъектРФ.Наименование + " " + СубъектРФ.Сокращение));
		Возврат Результат;
	...
	Для Каждого Запись Из ЗаписиДомов Цикл;
		...
		Если ПроверятьОкруг И Не Запись.ОкругНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Округ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Округ ""%1"" отсутствует в адресном классификаторе'; en = 'County ""%1"" does not exist in the address classifier'"), Округ.Наименование + " " + Округ.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьРайон И Не Запись.РайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Район ""%1"" отсутствует в адресном классификаторе'; en = 'District ""%1"" does not exist in the address classifier'"), Район.Наименование + " " + Район.Сокращение));
		КонецЕсли;
		
		Если ПроверятьГород И Не Запись.ГородНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Город", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Город ""%1"" отсутствует в адресном классификаторе'; en = 'City ""%1"" does not exist in the address classifier'"), Город.Наименование + " " + Город.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьВнутригородскойРайон И Не Запись.ВнутригородскойРайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "ВнутригРайон", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Внутригородской район ""%1"" отсутствует в адресном классификаторе'; en = 'Urban district ""%1"" does not exist in the address classifier'"),
		...
		Если ПроверятьНаселенныйПункт И Не Запись.НаселенныйПунктНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "НаселПункт", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Населенный пункт ""%1"" отсутствует в адресном классификаторе'; en = 'Settlement ""%1"" does not exist in the address classifier'"),
		...
		Если ПроверятьДополнительный И Не Запись.ДополнительныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Дополнительный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Дополнительная территория ""%1""отсутствует в адресном классификаторе'; en = 'Additional territory ""%1""does not exist in the address classifier'"),
		...
		Если ПроверятьПодчиненный И Не Запись.ПодчиненныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Элемент дополнительной территории ""%1"" отсутствует в адресном классификаторе'; en = 'Additional land element ""%1"" does not exist in the target classifier'"),
		...
		Если ИсторияОбъекта.Количество() > 0 Тогда 
			Для каждого СтрокаОбъект Из ИсторияОбъекта Цикл
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Улица ""%1"" была переименована в ""%2""'; en = 'Street ""%1"" was renamed to ""%2""'"), СтрокаОбъект.Значение, СтрокаОбъект.Представление));
			КонецЦикла;
		Иначе
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Улица ""%1"" отсутствует в адресном классификаторе'; en = 'Street ""%1"" does not exist in the address classifier'"), Улица.Наименование + " " + Улица.Сокращение));
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьЗаписиДомов И Не ЕстьВарианты Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеЗданияПоСоответствиюФИАС) Тогда
			...
			Если НеточныеСовпадения.Количество() = 1 Тогда
				Подсказка = ", " + НСтр("ru = 'но есть'; en = 'but there are'") + """" + НеточныеСовпадения[0]+ """";
			ИначеЕсли НеточныеСовпадения.Количество() > 1 Тогда
				...
				Если НеточныеСовпадения.Количество() = 2 Тогда
					Подсказка = НСтр("ru = 'но есть ""%1"" или ""%2""'; en = 'but there is ""%1"" or ""%2""'");
					ВтороеЗначение =  НеточныеСовпадения[1];
				Иначе
					Подсказка = НСтр("ru = 'но есть ""%1"" и другие варианты(%2)'; en = 'but there is ""%1"" and other options (%2)'");
			...
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1"" отсутствует в адресном классификаторе'; en = '""%1"" does not exist in the address classifier'"), ОписаниеЗданияПоСоответствиюФИАС(ЗданияИПомещения)) + Подсказка);
		КонецЕсли;
	...
	Если НЕ ИндексКорректный И ЗначениеЗаполнено(Индекс) И ОшибкиПроверки.Количество() = 0 Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Индекс", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Индекс ""%1"" в адресном классификаторе не соответствует введенному адресу'; en = 'Zip code ""%1"" in the address classifier does not match the address entered'"), Индекс));
	КонецЕсли;
...
Функция СформироватьОписаниеЗдания(НазваниеВладения, НомерВладения, НомерКорпуса, НазваниеСтроения, НомерСтроения, ДополнительныеПараметры = Неопределено)
	...
	Если ЗначениеЗаполнено(НомерКорпуса) Тогда
		КорпусИСтроение = ", "+ НСтр("ru = 'корпус'; en = 'housing'") + " " + НомерКорпуса;
	КонецЕсли;
...
Функция ТипыОбъектовАдресацииАдресаРФ()
	...
	Результат.Добавить(СтрокаОбъектаАдресации("1010", НСтр("ru = 'Дом'; en = 'Building'"),          1, 1, 2));
	Результат.Добавить(СтрокаОбъектаАдресации("1020", НСтр("ru = 'Владение'; en = 'Estate'"),     1, 2, 1));
	Результат.Добавить(СтрокаОбъектаАдресации("1030", НСтр("ru = 'Домовладение'; en = 'House and grounds'"), 1, 3, 3));
	
	Результат.Добавить(СтрокаОбъектаАдресации("1050", НСтр("ru = 'Корпус'; en = 'Housing'"),     2, 1));
	Результат.Добавить(СтрокаОбъектаАдресации("1060", НСтр("ru = 'Строение'; en = 'Building'"),   2, 2, 1));
	...
	Результат.Добавить(СтрокаОбъектаАдресации("1040", НСтр("ru = 'Участок'; en = 'Land site'"),    2, 5));
	...
	Результат.Добавить(СтрокаОбъектаАдресации("2040", НСтр("ru = 'Бокс'; en = 'Box'"),      3, 3));
	Результат.Добавить(СтрокаОбъектаАдресации("2020", НСтр("ru = 'Помещение'; en = 'Premises'"), 3, 4));
	...
	Результат.Добавить(СтрокаОбъектаАдресации("2010", НСтр("ru = 'кв.'; en = 'apart.'"),       3, 6));
	...
	Результат.Добавить(СтрокаОбъектаАдресации("10400000", НСтр("ru = 'Элемент улично-дорожной сети, планировочной структуры дополнительного адресного элемента'; en = 'Element of the highway-road network, planning structure of the secondary address element'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10500000", НСтр("ru = 'Промышленная зона'; en = 'Industrial zone'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10600000", НСтр("ru = 'Гаражно-строительный кооператив'; en = 'Garage construction cooperative'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10700000", НСтр("ru = 'Территория'; en = 'Site'")));
...
Функция ПараметрыАутентификацииНаСайте() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Сервис интернет-поддержки пользователей не подключен.'; en = 'Online customers support service is not connected.'");
	КонецЕсли;
...
Функция СтруктураОписанияОшибкиПоставщика(Описание = Неопределено, ИнформацияОбОшибке = Неопределено)
	...
	Если ИнформацияОбОшибке = Неопределено Тогда
		Описание.ПодробноеПредставлениеОшибки = НСтр("ru = 'Информация об ошибке отсутствует'; en = 'No information on the error'");
		Возврат Описание;
	КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Описание = НСтр("ru = 'Загрузка обновлений адресного классификатора.'; en = 'Downloading address classifier updates.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
			...
			Дело.Представление  = НСтр("ru = 'Адресный классификатор устарел'; en = 'Address classifier is outdated'");
			Дело.Количество     = 0;
			Дело.Подсказка      = НСтр("ru = 'Автоподбор и проверка корректности адресов временно недоступны.'; en = 'Autofill and validating are temporarily unavailable.'");
	...
	Подсказка = Подсказка + Символы.ПС + НСтр("ru = 'Необходимо проверить наличие обновлений.'; en = 'Need to check for updates.'");
	Для Каждого Раздел Из Разделы Цикл
		Дело = ТекущиеДела.Добавить();
...
Процедура ПеренестиДанныеУстаревшегоКлассификатор(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Актуализация сведений о регионе ""%1 - %2"" (осталось %3) ...'; en = 'Updating information about the region ""%1 - %2"" (%3 left) ...'"),
...
Функция ОписаниеДиапазонаКЛАДР(Описание, ТаблицыДанных)
	...
	Если ВариантыТипов.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка поиска типа диапазона для %1'; en = 'Error searching range type %1'"), Описание);
	КонецЕсли;
...
Функция ПредставлениеРегиона(Наименование, Сокращение)
	Если Сокращение = НСтр("ru = 'Респ'; en = 'Rep'") Тогда
		Возврат НСтр("ru = 'Республика'; en = 'Republic'") + " " + Наименование;
	ИначеЕсли Сокращение = НСтр("ru = 'обл'; en = 'rgn'") Тогда
		Возврат Наименование + " " + НСтр("ru = 'область'; en = 'region'");
	ИначеЕсли Сокращение = НСтр("ru = 'г'; en = 'city'") Тогда
		Возврат НСтр("ru = 'г.'; en = 'city.'") + " " + Наименование;
	КонецЕсли;
...
Функция ИдентификаторДома(СтрокаСоСпискомДомов, СведенияОДоме)
	...
	Пока Позиция <= ЧислоСтрок Цикл
		...
		Если Строка = "H" Тогда
			...
			ИначеЕсли НЕ ЗначениеЗаполнено(ОписаниеДома.Дом) ИЛИ ОписаниеДома.Дом = НСтр("ru = 'НЕТ'; en = 'NO'") Тогда
...
Функция ИзвлечьСписокДомовСервис1С(ИдентификаторРодитель, СтрокаПоиска, ВариантыДомов)
	...
	Порция = Сервис.Autocomplete(ИдентификаторРодитель, 0, НСтр("ru = 'ДОМ='; en = 'BUILDING='") + СтрокаПоиска, 2000, КодЯзыка, Метаданные.Имя);
	...
	Для Каждого Строка Из Список Цикл
		...
		Для каждого Дом Из МассивДомов Цикл
			...
			Если ТипАдресногоОбъекта = НСтр("ru = 'ДОМ'; en = 'BUILDING'")
				ИЛИ ТипАдресногоОбъекта = НСтр("ru = 'ВЛАДЕНИЕ'; en = 'ESTATE'")
				ИЛИ ТипАдресногоОбъекта = НСтр("ru = 'ДОМОВЛАДЕНИЕ'; en = 'HOUSE AND GROUNDS'") Тогда
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Корпус) Тогда
		ТекстПроСтроения = НСтр("ru = 'корпус'; en = 'housing'") + " " + ОписаниеДома.Корпус;
...
Функция НаименованиеВладенийИСтроений()
	...
	Результат.Владения.Вставить(0, НСтр("ru = 'Дом'; en = 'Building'"));
	Результат.Строения.Вставить(0, НСтр("ru = 'Строение'; en = 'Building'"));
		...
		Если Не ЗагруженныйФайл.Статус Тогда
			...
			ВызватьИсключение ЗагруженныйФайл.СообщениеОбОшибке + Символы.ПС
				+ НСтр("ru = 'Возможные причины:
					...
					|• Login and password are incorrect or not specified;
					|• No connection to the Internet;
					|• Web site has problems;
					|• Firewill or other software (such as antivirus) are blocking attempts to connect to the Internet;
					|• Internet is connected via proxy server but its parameters are not configured in the program.'");
		КонецЕсли;
...
Процедура ОчиститьКлассификаторАдресов(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Очистка региона ""%1 - %2"" (осталось %3) ...'; en = 'Cleaning region ""%1 - %2"" (%3 left).'"),
			СубъектРФ, АдресныйКлассификатор.НаименованиеРегионаПоКоду(СубъектРФ),
			Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=")));
...
Функция РазрешенияБезопасностиОбновления() Экспорт
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, НСтр("ru = 'Проверка обновления адресного классификатора.'; en = 'Address classifier update check.'")));
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, НСтр("ru = 'Загрузка данных адресного классификатора.'; en = 'Importing address classifier data.'")));
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	Если ТипXDTOЗаписи = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В файле ""%1"" не найден список записей в типе  %2'; en = 'No record list in type %2 in file ""%1""'"), Результат.ПолноеИмя, Результат.ТипXDTO);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В файле ""%1"" не найден корневой узел %2'; en = 'No root node %2 in file ""%1""'"), Результат.ПолноеИмя, ИмяОписывающегоУзла);
	КонецЕсли;
