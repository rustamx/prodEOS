...
Процедура РазместитьНаФормеКомандыЭДО(Форма, МестоРазмещенияКомандПоУмолчанию, МодульПодсистемы, НаправлениеЭД = Неопределено, ТолькоВМенюЕще = Ложь) Экспорт
	...
	Если МестоРазмещенияКомандПоУмолчанию <> Неопределено Тогда
		...
		Если МодульПодсистемы = "ОбменСБанками" Тогда
			МестоРазмещенияКомандПоУмолчанию.Заголовок = НСтр("ru = '1С:ДиректБанк'; en = '1C:DirectBank'");
		КонецЕсли;
	КонецЕсли;
	...
	Если МестоРазмещенияКомандПоУмолчанию = Неопределено Тогда
		...
		Если МодульПодсистемы = "ОбменСБанками" Тогда
			ПодменюЭДО.Заголовок = НСтр("ru = '1С:ДиректБанк'; en = '1C:DirectBank'");
		Иначе
			ПодменюЭДО.Заголовок = НСтр("ru = 'ЭДО'; en = 'EDI'");
		КонецЕсли;
...
Процедура ВыполнитьЗаписьСобытияПоЭДВЖурналРегистрации(ОписаниеСобытия, КодСобытия, УровеньВажности = Неопределено,
	...
	Если КодСобытия = 0 Тогда Подсистема = НСтр("ru = 'Общая подсистема'; en = 'Common subsystem'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 1 Тогда Подсистема = НСтр("ru = 'Обмен с банками'; en = 'Exchange with banks'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 2 Тогда Подсистема = НСтр("ru = 'Обмен с контрагентами'; en = 'Interchange with counterparties'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 3 Тогда Подсистема = НСтр("ru = 'Обмен с сайтами'; en = 'Exchange with sites'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 4 Тогда Подсистема = НСтр("ru = 'Регламентные задания'; en = 'Scheduled jobs'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 5 Тогда Подсистема = НСтр("ru = 'Бизнес-сеть'; en = 'Business network'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИначеЕсли КодСобытия = 6 Тогда Подсистема = НСтр("ru = 'Интеграция с Яндекс Кассой'; en = 'Integration with Yandex.Checkout'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Подсистема = НСтр("ru = 'Общая подсистема'; en = 'Common subsystem'",
		ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
	...
	ИмяСобытия = СтрЗаменить(НСтр("ru = 'Электронное взаимодействие.%1'; en = 'Electronic interaction.%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		"%1", Подсистема);
...
Процедура СообщитьПользователюОНарушенииПравДоступа() Экспорт
	...
	ТекстСообщения = НСтр("ru = 'Нарушение прав доступа.'; en = 'Access violation.'");
...
Функция РасшифровкаКодаСостоянияHTTP(КодСостояния) Экспорт
	...
	СоответствиеКодов.Вставить(400, НСтр("ru = 'Сервер обнаружил в запросе клиента синтаксическую ошибку.'; en = 'Server found in the client query syntax error.'"));
	СоответствиеКодов.Вставить(401, НСтр("ru = 'Неверные данные аутентификации.'; en = 'Wrong authentication data.'"));
	СоответствиеКодов.Вставить(403, НСтр("ru = 'У клиента нет доступа к ресурсу.'; en = 'The client does has no access to the resource.'"));
	СоответствиеКодов.Вставить(404, НСтр("ru = 'На сервере нет ресурса по указанному URI.'; en = 'No resource with the specified URI was found on server.'"));
	СоответствиеКодов.Вставить(405, НСтр("ru = 'Указанный клиентом метод нельзя применить к текущему ресурсу.'; en = 'The method specified by the client cannot be applied to the resource.'"));
	СоответствиеКодов.Вставить(406, НСтр("ru = 'Запрошенный URI не может удовлетворить переданным в заголовке характеристикам.'; en = 'The requested URI does not match the parameters passed in the header.'"));
	СоответствиеКодов.Вставить(407, НСтр("ru = 'Для доступа к серверу требуется аутентификация для прокси-сервера.'; en = 'To access the server authentication is required for the proxy server.'"));
	СоответствиеКодов.Вставить(408, НСтр("ru = 'Время ожидания сервером передачи от клиента истекло.'; en = 'Timeout exceeded while waiting for response by server from the client.'"));
	СоответствиеКодов.Вставить(409, НСтр("ru = 'Запрос не может быть выполнен из-за конфликтного обращения к ресурсу.'; en = 'The request cannot be performed due to a conflict at the resource.'"));
	СоответствиеКодов.Вставить(410, НСтр("ru = 'Ресурс был удален и теперь недоступен.'; en = 'The resource has been deleted and is no longer available.'"));
	СоответствиеКодов.Вставить(411, НСтр("ru = 'Не указан объем передаваемых данных в заголовке.'; en = 'The amount of data transmitted in the header is not specified.'"));
	СоответствиеКодов.Вставить(412, НСтр("ru = 'Ни одно из условных полей заголовка запроса не было выполнено.'; en = 'Not a single condition in the request header was met.'"));
	СоответствиеКодов.Вставить(413, НСтр("ru = 'Сервер отказался обработать запрос по причине слишком большого размера тела запроса.'; en = 'The server refused to process the request because the request body is too large.'"));
	СоответствиеКодов.Вставить(414, НСтр("ru = 'Сервер не может обработать запрос из-за слишком длинного указанного URL.'; en = 'The server cannot process the request because the specified URL is too long.'"));
	СоответствиеКодов.Вставить(415, НСтр("ru = 'Сервер отказывается работать с указанным типом данных при данном методе.'; en = 'The server refuses to work with a specified type of data with this method.'"));
	СоответствиеКодов.Вставить(422, НСтр("ru = 'Имеется логическая ошибка, из-за которой невозможно произвести операцию.'; en = 'There is a logical error due to which it is impossible to perform the operation.'"));
	СоответствиеКодов.Вставить(423, НСтр("ru = 'Целевой ресурс из запроса заблокирован от применения к нему указанного метода.'; en = 'The target resource of the request is blocked from the use of the specified method.'"));
	СоответствиеКодов.Вставить(426, НСтр("ru = 'Клиенту на необходимость обновить протокол.'; en = 'For client to update protocol.'"));
	СоответствиеКодов.Вставить(429, НСтр("ru = 'Клиент попытался отправить слишком много запросов за короткое время.'; en = 'The client attempted to send too many requests in a short period of time.'"));
	СоответствиеКодов.Вставить(431, НСтр("ru = 'Превышена допустимая длина заголовков.'; en = 'Exceeded allowable header length .'"));
	СоответствиеКодов.Вставить(434, НСтр("ru = 'Запрашиваемый адрес недоступен.'; en = 'The address requested is inaccessible.'"));
	СоответствиеКодов.Вставить(449, НСтр("ru = 'Поступило недостаточно информации.'; en = 'Submitted information is not sufficient.'"));
	СоответствиеКодов.Вставить(451, НСтр("ru = 'Доступ к ресурсу закрыт по юридическим причинам.'; en = 'Access to the resource is forbidden for legal reasons.'"));
	
	СоответствиеКодов.Вставить(500, НСтр("ru = 'Внутренняя ошибка сервера.'; en = 'Internal server error.'"));
	СоответствиеКодов.Вставить(501, НСтр("ru = 'Сервер не поддерживает возможностей, необходимых для обработки запроса.'; en = 'The server does not support the functionality needed to handle the request.'"));
	СоответствиеКодов.Вставить(502, НСтр("ru = 'Сервер, выступая в роли шлюза или прокси-сервера, получил недействительное ответное сообщение от вышестоящего сервера.'; en = 'Your server, acting as a gateway or proxy, received an invalid response from the upstream server message.'"));
	СоответствиеКодов.Вставить(503, НСтр("ru = 'Сервер временно не имеет возможности обрабатывать запросы по техническим причинам.'; en = 'The server temporarily is unable to process requests for technical reasons.'"));
	СоответствиеКодов.Вставить(504, НСтр("ru = 'Сервер в роли шлюза или прокси-сервера не дождался ответа от вышестоящего сервера для завершения текущего запроса.'; en = 'Server as a gateway or proxy server failed to wait for a response from the upstream server to complete the current request.'"));
	СоответствиеКодов.Вставить(505, НСтр("ru = 'Сервер не поддерживает указанную в запросе версию протокола HTTP.'; en = 'The server does not support the specified in the query version of the HTTP protocol.'"));
	СоответствиеКодов.Вставить(507, НСтр("ru = 'Не хватает места для выполнения текущего запроса.'; en = 'Not enough space for the execution of the request.'"));
	СоответствиеКодов.Вставить(510, НСтр("ru = 'На сервере отсутствует расширение, которое желает использовать клиент.'; en = 'Extension required by the cliend is missing from the server.'"));
	СоответствиеКодов.Вставить(511, НСтр("ru = 'Необходимо авторизоваться в сети провайдера.'; en = 'It is necessary to authenticate in the provider network.'"));
...
Процедура УдалитьВременныеФайлы(Путь, Маска = Неопределено) Экспорт
	...
	Исключение
		ВидОперации = НСтр("ru = 'Удаление временного файла.'; en = 'Deleting temporary file.'");
...
Функция ЗначениеРеквизитаВДереве(ДеревоДанных, ПолныйПуть, СообщатьОбОшибке = Истина) Экспорт
	...
	ИначеЕсли СообщатьОбОшибке Тогда
		ТекстСообщения = НСтр("ru = 'Не найдено поле в дереве данных по полному пути <%1>'; en = 'Field is not found in the data tree paths <%1 >'");
	...
	Для Каждого Строка Из ДеревоДанных.Строки Цикл
		...
		Если Строка.Обязательное = "Да" И Не ЗначениеЗаполнено(Строка.Значение)
			...
			Если ЭтоТаблица Тогда
				ШаблонОшибки = НСтр("ru = 'Не заполнено обязательное поле таблицы: ""%1."" в строке %2'; en = 'Required field not filled in the table: ""%1."" at line %2'");
				ТекстОшибки = ТекстОшибки
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПрикладноеПредставление, НомерСтроки)
							+ Символы.ПС;
			Иначе
				ШаблонОшибки = НСтр("ru = 'Не заполнено обязательное поле: ""%1.""'; en = 'Required field not filled in: ""%1.""'");
				ТекстОшибки = ТекстОшибки
...
Функция ТаблицыРеквизитовОбъектовОдинаковые(Таблица1, Таблица2, РеквизитыОбъекта, ДопПараметры = Неопределено)
	...
	Если НЕ ПризнакСовпадения И ЗначениеЗаполнено(ДопПараметры) Тогда
		...
		Если ИмяТабличнойЧасти = "Шапка" Тогда
			...
			НовСтрокаДереваМесто.Место = НСтр("ru ='Реквизиты шапки'; en = 'Header attributes'");
		...
		Иначе
			НовСтрокаДереваМесто = СтрокаДереваСравнения.Строки.Добавить();
			НовСтрокаДереваМесто.Место = СтрШаблон(НСтр("ru ='Табличная часть <%1>'; en = 'Tabular section <%1>'"), ИмяТабличнойЧасти);
			НовСтрокаДереваРеквизита = НовСтрокаДереваМесто.Строки.Добавить();
