...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКомандПоУмолчанию = Неопределено, ОбъектыПечати = Неопределено) Экспорт
	...
	Если ОбъектыПечати = Неопределено Тогда
		// ТСК Шепелев М.Н. 03.09.2018 {
		//КомандыПечати = УправлениеПечатьюПовтИсп.КомандыПечатиФормы(ИмяФормы, ОбъектыПечати, ВидДокумента).Скопировать();
		ра_СтруктураПараметров = Новый Структура;
		Если Лев(Форма.ИмяФормы,8) = "Документ" Тогда
			РеквизитыФормы = Форма.ПолучитьРеквизиты(); 
			РеквизитОбъектНайден = Ложь; 
			Для Каждого Реквизит Из РеквизитыФормы Цикл
				Если Реквизит.Имя = "Объект" тогда
					РеквизитОбъектНайден = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если РеквизитОбъектНайден Тогда
				Если ра_ОбщегоНазначенияКлиентСервер.ЭтоДокументКачества(Форма.Объект.Ссылка) Тогда
					ра_СтруктураПараметров = ра_ОбщегоНазначения.СтруктураПараметровДляПечатныхФормЕОС(Форма.Объект.Ссылка);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		КомандыПечати = КомандыПечатиФормы(ИмяФормы, ОбъектыПечати, ра_СтруктураПараметров).Скопировать();
		// ТСК Шепелев М.Н. 03.09.2018 }
...
Функция КомандыПечатиФормы(Форма, СписокОбъектов = Неопределено, 
	// ТСК Мусабеков Роман 03.05.2018 {
	ра_СтруктураПараметров = Неопределено
	// ТСК Мусабеков Роман 03.05.2018 }
	) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		Иначе
			...
			Иначе
				...
				// ТСК Мусабеков Роман 03.05.2018 {
				Если ЗначениеЗаполнено(ра_СтруктураПараметров) Тогда
					Попытка
						МенеджерПечати.ДобавитьКомандыПечати(ДобавляемыеКомандыПечати, ра_СтруктураПараметров);
					Исключение
...
Функция СоздатьКоллекциюКомандПечати() Экспорт
	...
	// ТСК Мусабеков Роман 03.05.2018 {
	Результат.Колонки.Добавить("ра_ШаблонФайла", Новый ОписаниеТипов("СправочникСсылка.Файлы"));
	// ТСК Мусабеков Роман 03.05.2018 }
...
Функция МакетПечатнойФормы(ПолныйПутьКМакету) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Макет ""%1"" не найден. Операция прервана.'; en = 'Template ""%1"" is not found. Operation aborted.'"), ПолныйПутьКМакету);
	КонецЕсли;
...
Функция ДанныеQRКода(QRСтрока, УровеньКоррекции, Размер) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Формирование QR-кода'; en = 'QR-code generation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление = НСтр("ru = 'Макеты печатных форм'; en = 'Print form templates'");
...
Функция Разрешения()
	...
	Разрешения.Добавить(РаботаВБезопасномРежиме.РазрешениеНаИспользованиеВнешнейКомпоненты(
		"ОбщийМакет.КомпонентаПечатиQRКода", НСтр("ru = 'Печать QR кодов.'; en = 'Print QR codes.'")));
	...
	Для Каждого ИмяМакета Из ИменаМакетов Цикл
		...
		Для Каждого ОписаниеПечатнойФормы Из ВременнаяКоллекцияДляОднойПечатнойФормы Цикл
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не задано количество экземпляров для печатной формы ""%1"".'; en = 'Do not set the number of copies for printing form ""%1"".'"),
		...
		Если Отказ Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'При формировании печатной формы ""%1"" возникла ошибка. Обратитесь к администратору.'; en = 'When generating printed form ""%1"" error occurred. Please contact your administrator.'"), ИмяМакета);
...
Процедура ДобавитьКомандыПечати(Форма, КомандыПечати, Знач МестоРазмещенияКоманд = Неопределено)
	...
	Иначе
		Если Не ОднаКомандаПечати Тогда
			...
			ПодменюПечать.Заголовок = НСтр("ru = 'Печать'; en = 'Print'");
	...
	КомандаФормы.Заголовок = НСтр("ru = '(нет печатных форм)'; en = '(no print forms)'");
	КомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
	КомандаФормы.Отображение = ОтображениеКнопки.КартинкаИТекст;
...
Функция СтрокаОбязательныхРеквизитов(ДанныеДокумента, ТекстСообщения)
	...
	Для Каждого Элемент Из ОбязательныеРеквизиты Цикл
		Если Не ЗначениеЗаполнено(ДанныеДокумента[Элемент.Ключ]) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнен обязательный реквизит: %1'; en = 'Not full mandatory requisite: %1'");
	...
	Если СтрДлина(ОбязательныеДанные) > 300 Тогда
		Шаблон = НСтр("ru = 'Невозможно создать QR-код для документа %1
			...
			|""%2""';
			|en = 'You cannot create a QR code for the document %1
			|String obligatory requisites must be less than 300 characters:""%2""'");
...
Функция КомпонентаФормированияQRКода(Отказ)
	...
	ТекстОшибки = НСтр("ru = 'Не удалось подключить внешнюю компоненту для генерации QR-кода'; en = 'Unable to connect an external component to generate QR-code'");
...
Процедура СообщитьПечатнаяФормаНедоступна(Объект)
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Печать %1 не выполнена: выбранная печатная форма недоступна.'; en = 'Printing %1 failed: your printed form is not available.'"), Объект);
