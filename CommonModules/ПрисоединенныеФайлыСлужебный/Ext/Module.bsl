			...
			Иначе // Перечисления.ТипыХраненияФайлов.ВИнформационнойБаза
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						НСтр("ru = 'Файлы.Не удалось отправить файл при обмене данными'; en = 'Файлы.Не удалось отправить файл при обмене данными'",
...
Процедура ВыполнитьОчисткуДанныхОФайле(ФайлСсылка) Экспорт
	...
	Попытка
		...
		ТекстОписания = НСтр("ru = 'Файл удален при очистке ненужных файлов.'; en = 'File was deleted while purging unnecessary files.'")
	...
	Исключение
		...
		Если Не ПустаяСтрока(ИмяФайлаСПутем) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Очистка ненужных файлов'; en = 'Purging unnecessary files'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбновитьДвоичныеДанныеФайлаНаСервере(Знач ПрисоединенныйФайл,
	...
	Попытка
		...
		Иначе
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Файлы.Запись файла на диск'; en = 'Файлы.Запись файла на диск'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
	...
	Исключение
		...
		КонецЕсли;
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Файлы.Обновление данных присоединенного файла в хранилище файлов'; en = 'Файлы.Обновление данных присоединенного файла в хранилище файлов'",
			     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
Функция ИзвлечьТекст(Знач ДвоичныеДанные, Знач Расширение, ИзвлеченныйТекст) Экспорт
	...
	Если ФайловыеФункцииСлужебный.ЭтоПлатформаWindows()
	   И ФайловыеФункцииСлужебный.ИзвлекатьТекстыФайловНаСервере() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Файлы.Извлечение текста'; en = 'Файлы.Извлечение текста'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
		...
		Если Метаданные.Справочники.Найти(КлючИЗначение.Ключ) = Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при определении имен справочников для хранения файлов.
				           ...
				           |en = 'Error while determining catalog names for file storage.
				           |Owner of files of type ""%1""
				           |is linked to a nonexisting catalog ""%2"".'"),
		...
		ИначеЕсли Не СтрЗаканчиваетсяНа(КлючИЗначение.Ключ, "ПрисоединенныеФайлы") Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при определении имен справочников для хранения файлов.
				           ...
				           |en = 'Error while determining catalog names for file storage.
				           |Owner of files of type ""%1""
				           |is linked to catalog ""%2""
				           |whose name does not end with ""ПрисоединенныеФайлы"".'"),
				Строка(ТипВладельцаФайлов),
				Строка(КлючИЗначение.Ключ));
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при определении имен справочников для хранения файлов.
			           ...
			           |en = 'Error while determining catalog names for file storage.
			           |Owner of files of type ""%1""
			           |has no catalogs for file storage.'"),
			Строка(ТипВладельцаФайлов));
	КонецЕсли;
...
Функция ИмяСправочникаХраненияФайлов(ВладелецФайлов, ИмяСправочника = "",
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			+ НСтр("ru = 'У владельца файлов ""%1"" типа ""%2""
			             ...
			             |en = 'To the owner of file ""%1"" of type ""%2""
			             |no catalogs for file storage were assigned.'"),
	...
	Если ЗначениеЗаполнено(ИмяСправочника) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			+ НСтр("ru = 'У владельца файлов ""%1"" типа ""%2""
			             ...
			             |en = 'Owner of files ""%1"" of type ""%2""
			             |has no catalog ""%3"" to store files.'"),
			Строка(ВладелецФайлов),
			Строка(ТипЗнч(ВладелецФайлов)),
	...
	ШаблонПричиныОшибки = 
		НСтр("ru = 'У владельца файлов ""%1"" типа ""%2""
			...
			|en = 'To the owner of file ""%1"" of type ""%2"" 
			|was not assinged the main catalog for file storage.'") + Символы.ПС;
...
Функция ТекстОшибкиПриСохраненииФайлаВТоме(Знач СообщениеОбОшибке, Знач Файл)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка, при сохранении файла в томе:
		           ...
		           |
		           |Ссылка на файл: ""%2"".';
		           |en = 'Error while saving file into volume:
		           |""%1"".
		           |
		           |Link to file: ""%2"".'"),
		СообщениеОбОшибке,
