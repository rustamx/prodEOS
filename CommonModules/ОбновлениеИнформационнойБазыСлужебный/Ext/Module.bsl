...
Функция ВыполнитьОбновлениеИнформационнойБазы(ПараметрыОбновления) Экспорт
	...
	Если НЕ ЕстьПраваНаОбновлениеИнформационнойБазы() Тогда
		Сообщение = НСтр("ru = 'Недостаточно прав для обновления версии программы.'; en = 'Not enough permissions for infobase update.'");
	...
	Если РежимОбновленияДанных = "ПереходСДругойПрограммы" Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Изменилось имя конфигурации на ""%1"".
			           ...
			           |en = 'Configuration name has been changed to ""%1"".
			           |Update from another program will be executed.'"),
			Метаданные.Имя);
	ИначеЕсли РежимОбновленияДанных = "ОбновлениеВерсии" Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Изменился номер версии конфигурации: с ""%1"" на ""%2"".
			           ...
			           |en = 'Configuration version number has been changed: from ""%1"" to ""%2"". 
			           |Infobase update will be executed.'"),
			ВерсияДанных, ВерсияМетаданных);
	Иначе
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Выполняется начальное заполнение данных до версии ""%1"".'; en = 'Primary data filling up to version ""%1"" is running.'"),
			ВерсияМетаданных);
	КонецЕсли;
	...
	Сообщение = НСтр("ru = 'Для обновления программы на новую версию будут выполнены обработчики: %1'; en = 'To update program to the new version the following handers are to be executed: %1'");
	...
	Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Обновление информационной базы на версию ""%1"" выполнено успешно.'; en = 'Infobase update to version ""%1"" executed successfully.'"), ВерсияМетаданных);
	ЗаписатьИнформацию(Сообщение);
...
Функция РежимОбновленияДанных() Экспорт
	...
	Если НЕ СтандартнаяОбработка Тогда
		...
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Недопустимое значение параметра %1 в %2. 
			...
			НСтр("ru = 'НачальноеЗаполнение, ОбновлениеВерсии или ПереходСДругойПрограммы'; en = 'НачальноеЗаполнение, ОбновлениеВерсии or ПереходСДругойПрограммы'"), 
...
Функция ИнформационнаяБазаЗаблокированаДляОбновления(УчитыватьПривилегированныйРежим = Истина) Экспорт
	...
	СообщениеАдминистраторуСистемы =
		НСтр("ru = 'Вход в программу временно невозможен в связи с обновлением на новую версию.
		           ...
		           |Administrative permissions are required to complete the update (roles ""System administrator"" and ""Full access"").'");
	...
	Если НеобходимоОбновлениеНеразделенныхДанныхИнформационнойБазы() Тогда
		...
		СообщениеАдминистраторуОбластиДанных =
			НСтр("ru = 'Вход в приложение временно невозможен в связи с обновлением на новую версию.
			           ...
			           |en = 'Signing on to the application is temporarily disabled due to update to the new version. 
			           |Contact the service administrator for further details.'");
...
Процедура ЗаписатьВремяВыполненияОбновления(ВремяНачалаОбновления, ВремяОкончанияОбновления) Экспорт
	...
	ПродолжительностьЧасы = ?(Часы = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 час'; en = '%1 hour'"), Часы));
	ПродолжительностьМинуты = ?(Минуты = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 мин'; en = '%1 min'"), Минуты));
	ПродолжительностьСекунды = ?(Секунды = 0, "", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 сек'; en = '%1 sec'"), Секунды));
...
Функция ИтерацииОбновления()
	...
	Если ИтерацияОбновлениеОсновнойПодсистемы = Неопределено И ИмяОсновнойКонфигурации = "БиблиотекаСтандартныхПодсистем" Тогда
		ТекстСообщения = НСтр("ru = 'Файл поставки 1С:Библиотека стандартных подсистем не предназначен для создания
			...
			|en = 'Distribution file of 1C:Standard systems library is not intended to create
			|infobases using templates. Refer to ITS manual (http://its.1c.ru/db/bspdoc)
			|before using.'");
...
Функция ВыполнитьИтерациюОбновления(Знач ИтерацияОбновления, Знач Параметры) Экспорт
	...
	Если ТекущаяВерсияИБ <> "0.0.0.0"
		...
		Если ВыполняемыеОбработчики = Неопределено Тогда
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				ШаблонСообщения = НСтр("ru = 'Не найден план обновления конфигурации %1 с версии %2 на версию %3'; en = 'Update plan not found for configuration %1 from version %2 to version %3'");
			Иначе
				ШаблонСообщения = НСтр("ru = 'Не найден план обновления библиотеки %1 с версии %2 на версию %3'; en = 'Update plan not found for library %1 from version %2 to version %3'");
			КонецЕсли;
	...
	Для Каждого Версия Из ВыполняемыеОбработчики.Строки Цикл
		...
		Если Версия.Версия = "*" Тогда
			Сообщение = НСтр("ru = 'Выполняются обязательные процедуры обновления информационной базы.'; en = 'Running mandatory infobase update procedures'");
		Иначе
			НоваяВерсияИБ = Версия.Версия;
			Если ТекущаяВерсияИБ = "0.0.0.0" Тогда
				Сообщение = НСтр("ru = 'Выполняется начальное заполнение данных.'; en = 'Primary data filling is running.'");
			ИначеЕсли ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выполняется обновление информационной базы с версии %1 на версию %2.'; en = 'Updating infobase from version %1 to version %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выполняется обновление данных библиотеки %3 с версии %1 на версию %2.'; en = 'Updating data of library %3 from version %1 to version %2.'"), 
		...
		Если Версия.Версия = "*" Тогда
			Сообщение = НСтр("ru = 'Выполнены обязательные процедуры обновления информационной базы.'; en = 'Mandatory infobase update procedures are completed.'");
		Иначе
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выполнено обновление информационной базы с версии %1 на версию %2.'; en = 'Completed infobase update from version %1 to version %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выполнено обновление данных библиотеки %3 с версии %1 на версию %2.'; en = 'Completed updating data of library %3 from version %1 to version %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ, ИдентификаторБиблиотеки);
			КонецЕсли;
		КонецЕсли;
		ЗаписатьИнформацию(Сообщение);
...
Функция ОбновитьИнформационнуюБазуВФоне(УникальныйИдентификаторФормы, БлокировкаИБ) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Фоновое обновление информационной базы'; en = 'Background infobase update'");
...
Функция ЗаблокироватьИБ(БлокировкаИБ, ИсключениеПриНевозможностиБлокировкиИБ)
	...
	Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Невозможно выполнить обновление информационной базы:
			...
			|Подробности ошибки:
			|%1';
			|en = 'Infobase update is not possible:
			|- Impossible to set the exclusive mode
			|- The configuration version does not allow update without setting the exclusive mode
			|
			|Error details:
			|%1'"),
...
Процедура ПерейтиСДругойПрограммы()
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("ru = 'При работе в модели сервиса переход с другой программы не предусмотрен.'; en = 'When using software as a service update from another program is not provided.'");
	КонецЕсли;
		...
		Исключение
			...
			ЗаписатьОшибку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При вызове обработчика перехода с другой программы
				           ...
				           |произошла ошибка:
				           |""%2"".';
				           |en = 'Upon calling a handler for updating from another program
				           |""%1""
				           |occurred an error:
				           |""%2"".'"),
				ИмяОбработчика,
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Обновление информационной базы'; en = 'Infobase update'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииПротокол() Экспорт
	...
	Возврат СобытиеЖурналаРегистрации() + "." + НСтр("ru = 'Протокол выполнения'; en = 'Execution protocol'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПроверитьСвойстваОтложенногоОбработчика(Знач Обработчик, Знач РежимВыполненияОтложенныхОбработчиков, ТекстОшибок)
	...
	Если РежимВыполненияОтложенныхОбработчиков = "Параллельно"
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не указана процедура заполнения данных
					   ...
					   |""%1"".';
					   |en = 'Data filling procedure is not found 
	...
	Если Обработчик.МонопольныйРежим = Истина Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У отложенного обработчика ""%1""
			...
			|must not have the property ""монопольныйрежим"" enabled.'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
	...
	Если РежимВыполненияОтложенныхОбработчиков = "Параллельно" И Обработчик.ЗапускатьТолькоВГлавномУзле
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У отложенного обработчика ""%1""
			...
			|en = 'Deferred handler ""%1"" 
			|have invalid values of properties: 
			|- ""ЗапускатьТолькоВГлавномУзле"" 
			|- ""ЗапускатьИВПодчиненномУзлеРИБСФильтрами"". 
			|
			|These properties cannot be both set to ""Истина"".'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
	...
	Если Обработчик.ОбщиеДанные = Истина Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У отложенного обработчика ""%1""
			...
			|en = 'Deferred handler ""%1"" 
			|has invalid value of property ""ОбщиеДанные"". 
			|The property cannot be set to ""Истина"" for deferred handler.'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
...
Процедура ПроверитьУникальностьИдентификаторовОтложенныхОбработчиков(ИтерацииОбновления)
	...
	ТекстСообщения = НСтр("ru = 'У следующих отложенных обработчиков неуникальные идентификаторы:
		|'; en = 'Found non-unique identifiers of deferred update handlers:'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("ru = 'Обновление программы не завершено'; en = 'Porgram update is not completed'");
	...
	Для Каждого ОтключаемыйОбработчик Из ОтключаемыеОбработчики Цикл
		...
		ИначеЕсли ВыполняемыйОбработчик <> Неопределено И ВыполняемыйОбработчик.Версия <> "*"
			...
			ТекстИсключения = НСтр("ru='Обработчик обновления %1 не может быть отключен, 
										...
										|en = 'Update handler %1 cannot be disabled, 
		...
		ИначеЕсли ВыполняемыйОбработчик = Неопределено Тогда
			ТекстИсключения = НСтр("ru='Отключаемый обработчик обновления %1 не существует'; en = 'Optional update hanlder %1 does not exist'");
	...
	Исключение
		...
		ЗаписатьОшибку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При вызове обработчика обновления:
					   ...
					   |произошла ошибка:
					   |""%2"".';
					   |en = 'Upon calling an update handler: 
					   |""%1"" 
					   |occured an error: 
					   |""%2"".'"),
			ИмяОбработчика,
...
Процедура ПроверитьВложеннуюТранзакцию(ТранзакцияАктивнаНаНачалоВыполнения, НазваниеОбработчика)
	...
	ИмяСобытия = СобытиеЖурналаРегистрации() + "." + НСтр("ru = 'Выполнение обработчиков'; en = 'Execution of handlers'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Если ТранзакцияАктивнаНаНачалоВыполнения Тогда
		...
		Если ТранзакцияАктивна() Тогда
			...
			Исключение
				...
				|en = 'Execution error of update handler %1:
				|The handler swallowed an exception with an external transaction active.
				|Having active external transactions that were opened higher at the call stack all exceptions should be thrown higher on the stack.'");
		...
		Иначе
			...
			|en = 'Execution error of update handler %1:
			|The handler closed an extra transaction that was opened earlier (higher at the stack).'");
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария, НазваниеОбработчика);
	...
	Иначе
		Если ТранзакцияАктивна() Тогда
			...
			|en = 'Execution error of update handler %1:
			|A transaction opened inside the handler was still active (was not closed nor cancelled).'");
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКомментария, НазваниеОбработчика);
	...
	Для каждого Обработчик Из ИтерацияОбновления.Обработчики Цикл
		...
		Если ПустаяСтрока(Обработчик.Версия) Тогда
			...
			Если Обработчик.НачальноеЗаполнение <> Истина Тогда
				ОписаниеОшибки = НСтр("ru = 'У обработчика не заполнено свойство Версия или свойство НачальноеЗаполнение.'; en = 'Handler properties Версия or НачальноеЗаполнение are not specified.'");
			КонецЕсли;
			...
			Исключение
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'У обработчика не правильно заполнено свойство Версия: ""%1"".
					           ...
					           |en = 'Property Версия of the handler contains an invalid value: ""%1"".
					           |The correct format is, for example, ""2.1.3.70"".'"),
					Метаданные.Версия);
			КонецПопытки;
			...
			Если НЕ ЗначениеЗаполнено(ОписаниеОшибки)
			   ...
			   И Обработчик.Приоритет <> 0 Тогда
				...
				ОписаниеОшибки = НСтр("ru = 'У обработчика не правильно заполнено свойство Приоритет или
				                            |свойство ВыполнятьВГруппеОбязательных.'; en = 'Invalid values of handler properties Приоритет or ВыполнятьВГруппеОбязательных.'");
			КонецЕсли;
		КонецЕсли;
		...
		Если Обработчик.РежимВыполнения <> ""
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У обработчика ""%1"" не правильно заполнено свойство РежимВыполнения.
				           ...
				           |en = 'Handler ""%1"" has invalid value of property РежимВыполнения.
				           |Allowed values are: ""Монопольно"", ""Отложенно"", ""Оперативно"".'"),
				Обработчик.Процедура);
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ОписаниеОшибки)
		   ...
		   И Обработчик.НачальноеЗаполнение = Истина Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'У обработчика не правильно заполнено свойство Опциональный или
			                            |свойство НачальноеЗаполнение.'; en = 'Invalid values of handler properties Опциональный or НачальноеЗаполнение.'");
		КонецЕсли;
		...
		Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда
			ЗаголовокОшибки = НСтр("ru = 'Ошибка в свойстве обработчика обновления конфигурации'; en = 'Error in properties of update handler for configuration'");
		Иначе
			ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка в свойстве обработчика обновления библиотеки %1 версии %2'; en = 'Error in properties of update handler for library %1 of version %2'"),
...
Процедура ЗаписатьИнформациюОХодеОбновления(Обработчик, ХодВыполненияОбработчиков, ВФоне)
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		Сообщение = НСтр("ru = 'Выполняется обработчик обновления %1 (%2 из %3).'; en = 'Running update handler %1 (%2 of %3).'");
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ЗапуститьПроцедуруОбработкиДанных(ОбработчикОбновления, ПараметрыОбновления)
	...
	Попытка
		СообщениеОЗапущенномОбработчике = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выполняется процедура обновления ""%1"".'; en = 'Running update procedure ""%1"".'"), ИмяОбработчика);
		...
		ИначеЕсли КоличествоЗапусковПроцедурыОбновления > 10000 Тогда // Защита от зацикливания.
			...
			ТекстОшибки = НСтр("ru = 'Превышено допустимое количество запусков процедуры обновления.
				...
				|en = 'Update procedure was launched too many times.
				|Execution was terminated to avoid cycling of data processing mechanism.'");
		...
		Если Параметры.ОбработкаЗавершена = Неопределено Тогда
			...
			|en = 'Parameter ОбработкаЗавершена is not initialized by update handler.
			|Run aborted due to an apparent error in the handler code.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
		...
		ИначеЕсли ПараметрыОбновления.ПараллельныйРежим Тогда
			...
			Если Не ЕстьОбработанныеОбъекты
				...
				Если ЧислоПопыток >= 2 Тогда
					ТекстИсключения = НСтр("ru = 'Произошло зацикливание процедуры обработки данных. Выполнение прервано.'; en = 'Data processing loop has occurred. Execution aborted.'");
					ВызватьИсключение ТекстИсключения;
				Иначе
	...
	Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(БлокируемыеОбъекты) И Не ЗначениеЗаполнено(ПроцедураПроверки) Тогда
			ТекстИсключения = НСтр("ru = 'У отложенного обработчика обновления ""%1""
				...
				|en = 'Deferred handler ""%1""
				|has the list of locked objects but the property ""ПроцедураПроверки"" is not specified.'");
			...
			Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
				...
				Попытка
					Сообщение = НСтр("ru = 'Выполняется процедура заполнения данных
						                   ...
						                   |отложенного обработчика обновления
						                   |""%2"".';
						                   |en = 'Executing data filling procedure 
						                   |""%1"" 
						                   |of a deferred update handler 
						                   |""%2""'");
				...
				Исключение
					ЗаписатьОшибку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'При вызове процедуры заполнения данных
								   ...
								   |произошла ошибка:
								   |""%3"".';
								   |en = 'Upon calling a data filling procedure 
								   |""%1"" 
								   |of a deferred update handler 
								   |""%2"" 
								   |an error occured: 
