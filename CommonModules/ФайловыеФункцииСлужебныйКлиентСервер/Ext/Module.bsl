			...
			Исключение
				ВызватьИсключение СтрШаблон(
					НСтр("ru = 'Ошибка при создании каталога ""%1"":
					           |""%2"".'; en = 'Error creating directory ""%1"":""%2"".'"),
					ПолныйПодКаталог,
	...
	Если Файл.Размер() > ОбщиеНастройки.МаксимальныйРазмерФайла Тогда
		...
		ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Размер файла ""%1"" (%2 Мб)
			           |превышает максимально допустимый размер файла (%3 Мб).'; en = 'The file size ""%1"" (%2 MB) exceeds the maximum allowed file size (%3 MB).'"),
			Файл.Имя,
	...
	Если Не ПроверитьРасширениеФайлаДляЗагрузки(Файл.Расширение, Ложь) Тогда
		...
		ОписаниеОшибки = СтрШаблон(
			НСтр("ru = 'Загрузка файлов с расширением ""%1"" запрещена.
			           ...
			           |en = 'Loading files with the extension ""%1"" is not allowed.
...
Функция СтрокаСообщенияОНедопустимостиПодписанияЗанятогоФайла(ФайлСсылка = Неопределено) Экспорт
	...
	Если ФайлСсылка = Неопределено Тогда
		Возврат НСтр("ru = 'Нельзя подписать занятый файл.'; en = 'You cannot sign locked file.'");
	Иначе
		Возврат СтрШаблон(
			НСтр("ru = 'Нельзя подписать занятый файл: %1.'; en = 'You cannot sign locked file: %1.'"),
			Строка(ФайлСсылка) );
	КонецЕсли;
...
Функция СтрокаСообщенияОНедопустимостиПодписанияЗашифрованногоФайла(ФайлСсылка = Неопределено) Экспорт
	...
	Если ФайлСсылка = Неопределено Тогда
		Возврат НСтр("ru = 'Нельзя подписать зашифрованный файл.'; en = 'You cannot sign the encrypted file.'");
	Иначе
		Возврат СтрШаблон(
						НСтр("ru = 'Нельзя подписать зашифрованный файл: %1.'; en = 'You cannot sign the encrypted file: %1.'"),
						Строка(ФайлСсылка) );
	КонецЕсли;
	...
	Если ПоискВТоме Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Ошибка открытия файла:
			           ...
			           |Perhaps the file is deleted by antivirus software.
			           |Please contact your administrator.'"),
			ИмяФайла,
			ВладелецФайла);
	Иначе
...
Процедура ЗаполнитьСтатусПодписи(СтрокаПодписи, ТекстОшибки = "") Экспорт
	...
	Если СтрокаПодписи.ПодписьВерна Тогда
		СтрокаПодписи.Статус = НСтр("ru = 'Верна'; en = 'Valid'");
		
	ИначеЕсли ЗначениеЗаполнено(ТекстОшибки) Тогда
		СтрокаПодписи.Статус = СтрШаблон(НСтр("ru = 'Неверна. %1'; en = 'Invalid. %1'"), ТекстОшибки);
		Если Прав(СтрокаПодписи.Статус, 1) = "." Тогда
			СтрокаПодписи.Статус = Лев(СтрокаПодписи.Статус, СтрДлина(СтрокаПодписи.Статус)-1);
		КонецЕсли;
	Иначе
		СтрокаПодписи.Статус = НСтр("ru = 'Неверна'; en = 'Invalid'");
	КонецЕсли;
