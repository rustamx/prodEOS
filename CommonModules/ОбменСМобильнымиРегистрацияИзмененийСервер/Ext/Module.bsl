...
Функция ПолучитьУзлыОбмена(ЭлементДанных) Экспорт
	...
	Если ТипЭлемента = Тип("РегистрСведенийНаборЗаписей.ИзмененныеНастройкиСинхронизацииСМобильнымКлиентом") Тогда
     ...
     // ТСК Ткаченко И.Л. 27.12.2018 Закомментиировано{
	//ИначеЕсли ТипЭлемента = Тип("СправочникСсылка.Пользователи")
	 // ТСК Ткаченко И.Л. 27.12.2018 Закомментиировано{
	ИначеЕсли 
		ТипЭлемента = Тип("СправочникСсылка.РолиИсполнителей")
		...
		Или ТипЭлемента = Тип("СправочникСсылка.ra_VidyNesootvetstvij")
		// Корнюшенков А.Ю. Искать в тексте "ДоработкиМП" 18.10.2018 } 
		Тогда
	...
	ИначеЕсли ТипЭлемента = Тип("ДокументСсылка.ra_Signal") Тогда
		Возврат ra_ПолучитьУзлыПоСигналу(ЭлементДанных)
		
	ИначеЕсли ТипЭлемента = Тип("ДокументСсылка.ra_Nesootvetstvie") Тогда
		Возврат ra_ПолучитьУзлыПоНесоответствию(ЭлементДанных)
...
// Ткаченко И.Л. Искать в тексте "ДоработкиМП" 19.12.2018 {
Функция ra_ПолучитьУзлыПоСигналу(ЭлементДанных)
	
	ЗапросПолучениеУзлов = Новый Запрос;
	ЗапросПолучениеУзлов.УстановитьПараметр("Ссылка", ЭлементДанных);
	ЗапросПолучениеУзлов.Текст = 
		"ВЫБРАТЬ
		|	Мобильный.Ссылка КАК Ссылка
		|ИЗ
		|	ПланОбмена.Мобильный КАК Мобильный
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ra_Signal КАК ra_Signal
		|		ПО Мобильный.Пользователь = ra_Signal.VyyavivsheeLico
		|ГДЕ
...
Функция ra_ПолучитьУзлыПоНесоответствию(ЭлементДанных)
	...
	ЗапросПолучениеУзлов.Текст = 
		...
		|ИЗ
		|	ПланОбмена.Мобильный КАК Мобильный
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ra_Nesootvetstvie КАК ra_Nesootvetstvie
		|		ПО Мобильный.Пользователь = ra_Nesootvetstvie.VyyavivsheeLico
		|ГДЕ
