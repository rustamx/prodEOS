	...
	ИначеЕсли ТипЗнч(Контекст.ДанныеСертификата) <> Тип("Строка") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при вызове процедуры ОткрытьСертификат общего модуля ЭлектроннаяПодписьКлиент:
			           ...
			           |en = 'Error when calling procedure ОткрытьСертификат of common module ЭлектроннаяПодписьКлиент: 
...
Процедура СохранитьСертификатПродолжение(Контекст)
	...
	Диалог.Заголовок = НСтр("ru = 'Выберите файл для сохранения сертификата'; en = 'Select a file to save the certificate'");
	Диалог.Фильтр    = НСтр("ru = 'Файлы сертификатов (*.cer)|*.cer|Все файлы (*.*)|*.*'; en = 'Certificate files (*.cer)|*.cer|All files (*.*)|*.*'");
...
Процедура СохранитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Сертификат сохранен в файл:'; en = 'Certificate saved to file:'"),,
...
Процедура СохранитьЗапросНаСертификатПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	Диалог.Заголовок = НСтр("ru = 'Выберите файл для сохранения запроса на сертификат'; en = 'Select a file to save the certificate request'");
	Диалог.Фильтр    = НСтр("ru = 'Файлы сертификатов (*.p10)|*.p10|Все файлы (*.*)|*.*'; en = 'Certificate files (*.p10)|*.p10|All files (*.*)|*.*'");
	Диалог.МножественныйВыбор = Ложь;
...
Процедура СохранитьЗапросНаСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		ЕстьПолученныеФайлы = Истина;
		ПоказатьОповещениеПользователя(НСтр("ru = 'Запрос на сертификат сохранен в файл:'; en = 'Certificate request is saved to file:'"),,
			ПолученныеФайлы[0].Имя);
	КонецЕсли;
...
Процедура СохранитьПодписьПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Файлы электронных подписей (*.%1)|*.%1|Все файлы (*.*)|*.*'; en = 'Digital signature files (*.%1)|*.%1|All files (*.*)|*.*'"),
		РасширениеДляФайловПодписи);
	
	ДиалогОткрытияФайла.Фильтр = Фильтр;
	ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
	ДиалогОткрытияФайла.Заголовок = НСтр("ru = 'Выберите файл для сохранения подписи'; en = 'Select a file to save the signature'");
...
Процедура СохранитьПодписьПослеПолученияФайла(ПолученныеФайлы, Контекст) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Электронная подпись сохранена в файл:'; en = 'Digital signature saved to file:'"),,
		ПолученныеФайлы[0].Имя);
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Сертификат не найден на компьютере по причине:
			           |%1';
			           |en = 'Certificate is not found on the computer because:
			           |%1'"),
			ПредставлениеОшибки);
	Иначе
		ТекстОшибки = НСтр("ru = 'Сертификат не найден на компьютере.'; en = 'Certificate is not found on the computer.'");
	КонецЕсли;
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	...
	Если Сертификат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение,
			НСтр("ru = 'Сертификат не найден в данных подписи.'; en = 'Certificate is not found in the signature data.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьСертификатНаКлиентеПослеОшибкиПроверки(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Если Контекст.ОписаниеОшибкиНаСервере <> Неопределено Тогда
		ОписаниеОшибки = Контекст.ОписаниеОшибкиНаСервере + " " + НСтр("ru = '(на сервере)'; en = '(on the server)'") + Символы.ПС
			+ ОписаниеОшибки + " " + НСтр("ru = '(на клиенте)'; en = '(on the client)'");
	КонецЕсли;
...
Процедура СоздатьМенеджерКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	ЗаголовокФормы = НСтр("ru = 'Требуется программа электронной подписи и шифрования'; en = 'Digital signature and encryption program is required'");
	Операция       = Контекст.Операция;
	
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось подписать данные по причине:'; en = 'Unable to sign data because:'");
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось проверить подпись по причине:'; en = 'Signature could not be validated because:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось зашифровать данные по причине:'; en = 'Failed to encrypt data because:'");
		
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось расшифровать данные по причине:'; en = 'Failed to decrypt data because:'");
		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось проверить сертификат по причине:'; en = 'Certificate could not be validated because:'");
		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("ru = 'Не удалось получить сертификаты по причине:'; en = 'Failed to obtain certificates because:'");
	...
	ИначеЕсли Операция <> "" Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в функции МенеджерКриптографии.
			           ...
			           |en = 'Error in function МенеджерКриптографии.
			           |Invalid vaule of parameter Операция ""%1"".'"), Операция);
	Иначе
		ЗаголовокОшибки = НСтр("ru = 'Не удалось выполнить операцию по причине:'; en = 'The operation failed because:'");
	КонецЕсли;
	...
	Если Не Подключено Тогда
		ТекстОшибки =
			НСтр("ru = 'В обозреватель интернет требуется установить расширение
			           ...
			           |en = 'Extension necessary to work with digital signature and
			           |encryption must be installed in the browser.'");
...
Процедура СоздатьМенеджерКриптографииПослеЦикла(Контекст)
	...
	Если СвойстваОшибки.Ошибки.Количество() = 0 Тогда
		ТекстОшибки = НСтр("ru = 'Не предусмотрено использование ни одной программы.'; en = 'Not a single program is avaliable to use.'");
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
		...
		Если Не СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ЭтоПолноправныйПользователь Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("ru = 'Обратитесь к администратору.'; en = 'Contact the administrator.'");
		КонецЕсли;
	...
	Иначе
		Если ЗначениеЗаполнено(Контекст.Программа) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Программа %1 не доступна или не установлена на компьютере.'; en = 'Program %1 is not available or not installed on the computer.'"),
				Контекст.Программа);
		Иначе
			ТекстОшибки = НСтр("ru = 'Ни одна из программ не доступна или не установлена на компьютере.'; en = 'Not a single program is installed and available on the computer.'");
		КонецЕсли;
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
	КонецЕсли;
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПодключенияРасширения(Подключено, Контекст) Экспорт
	...
	Диалог.Заголовок = НСтр("ru = 'Выберите файл сертификата (только для шифрования)'; en = 'Select the certificate file (for encryption only)'");
	Диалог.Фильтр = НСтр("ru = 'Сертификат X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы(*.*)|*.*'; en = 'Certificate X.509 (*.cer;*.crt)|*.cer;*.crt|All files(*.*)|*.*'");
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПомещенияФайла(Адрес, Контекст)
	...
	Если Форма = Неопределено Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Файл сертификата должен быть в формате DER X.509, операция прервана.'; en = 'Certificate file should be formatted as DER X.509, operation aborted.'"));
		Возврат;
	КонецЕсли;
	
	Если Не Форма.Открыта() Тогда
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Открыть", НСтр("ru = 'Открыть'; en = 'Open'"));
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Сертификат уже добавлен.'; en = 'Certificate is already added.'"), Кнопки);
	КонецЕсли;
	...
	Если ТипЗнч(ОшибкаНаКлиенте) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ru = 'Для процедуры ПоказатьОшибкуОбращенияКПрограмме
			           ...
			           |en = 'Wrong type of parameter ОшибкаНаКлиенте is
			           |specified for procedure ПоказатьОшибкуОбращенияКПрограмме.'");
	КонецЕсли;
	...
	Если ТипЗнч(ОшибкаНаСервере) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("ru = 'Для процедуры ПоказатьОшибкуОбращенияКПрограмме
			           ...
			           |en = 'Wrong type of parameter ОшибкаНаСервере is
			           |specified for procedure ПоказатьОшибкуОбращенияКПрограмме.'");
	КонецЕсли;
...
Процедура НачалоВыбораСертификатаПриУстановленномОтборе(Форма) Экспорт
	...
	Текст = НСтр("ru = 'Сертификаты, которые могут быть использованы для этой операции ограничены.'; en = 'Certificates that can be used for this operation are restricted.'");
	...
	Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
		Заголовок = НСтр("ru = 'Следующие разрешенные сертификаты доступны для выбора:'; en = 'The following allowed certificates are available for selection:'");
	...
	Если ЗначениеЗаполнено(НедоступныеСертификаты) Тогда
		Если ЭлектроннаяПодписьКлиентСервер.ОбщиеНастройки().СоздаватьЭлектронныеПодписиНаСервере Тогда
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ru = 'Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:'; en = 'The following allowed certificates are not found neither on the computer nor on the server:'");
			Иначе
				Заголовок = НСтр("ru = 'Ни один из следующих разрешенных сертификатов не найден ни на компьютере, ни на сервере:'; en = 'Not a single of the following allowed certificates is found on the computer or on the server:'");
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("ru = 'Следующие разрешенные сертификаты не найдены на компьютере:'; en = 'The following allowed certificates are not found on the computer:'");
			Иначе
				Заголовок = НСтр("ru = 'Ни один из следующих разрешенных сертификатов не найден на компьютере:'; en = 'Not a single of the following allowed certificates is found on the computer:'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьПодписьИзФайлаПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если Контекст.ПроверитьМенеджерКриптографииНаКлиенте
	   И ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru = 'Требуется программа электронной подписи и шифрования'; en = 'Digital signature and encryption program is required'"),
...
Процедура СохранитьДанныеВместеСПодписью(ОписаниеДанных, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("ru = 'Данные'; en = 'Data'"));
...
Процедура СохранитьДанныеВместеСПодписьюПослеСохраненияФайлаДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'При записи файла возникла ошибка:'; en = 'When saving the file an error occurred:'") + Символы.ПС + Результат.ОписаниеОшибки);
		
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("ru = 'Не удалось сохранить подписи вместе с файлом'; en = 'Unable to save the signature along with the file'"), "", Ошибка, Новый Структура);
		Возврат;
...
Процедура СохранитьДанныеВместеСПодписьюПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Текст = НСтр("ru = 'Папка с файлами:'; en = 'File folder:'") + Символы.ПС;
...
Процедура УстановитьРасширениеПослеПроверкиПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	ОбщегоНазначенияКлиент.ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(
		...
		НСтр("ru = 'Перед установкой расширения для работы с электронной подписью и шифрованием
		           ...
		           |en = 'Before installing the extension for working with digital signatures and cryptography 
		           |the extension for working with files must be installed.'"),
		Ложь);
		...
		Если Контекст.НаСторонеКлиента Тогда
			...
			Исключение
				...
				ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
					НСтр("ru = 'При записи подписи возникла ошибка:'; en = 'When saving the signature an error occurred:'")
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиЗашифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'При записи зашифрованных данных возникла ошибка:'; en = 'When saving encrypted data an error occurred:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиРасшифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'При записи расшифрованных данных возникла ошибка:'; en = 'When saving the decrypted data an error occurred:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
	 ...
	 Или ТипЗнч(Результат.Данные) <> Тип("ДвоичныеДанные")
	   И ТипЗнч(Результат.Данные) <> Тип("Строка") Тогда
		...
		Если ТипЗнч(Результат) <> Тип("Структура") Или Не Результат.Свойство("ОписаниеОшибки") Тогда
			Ошибка = Новый Структура("ОписаниеОшибки", НСтр("ru = 'Некорректный тип данных.'; en = 'Incorrect data type.'"));
		Иначе
...
Процедура ПолучитьДанныеИзОписанияДанныхЗавершение(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Или ПомещенныеФайлы.Количество() = 0 Тогда
		Результат = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'Передача данных отменена пользователем.'; en = 'Data transfer was cancelled by the user.'"));
	Иначе
