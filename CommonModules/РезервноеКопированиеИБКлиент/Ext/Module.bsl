...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	Если ФиксированныеПараметрыРезервногоКопированияИБ.ПроведеноВосстановление Тогда
		ТекстОповещения = НСтр("ru = 'Восстановление данных проведено успешно.'; en = 'Data recovery was successful.'");
		ПоказатьОповещениеПользователя(НСтр("ru = 'Данные восстановлены.'; en = 'Data restored.'"), , ТекстОповещения);
	КонецЕсли;
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПараметрыПредупреждения.ТекстФлажка = НСтр("ru = 'Выполнить резервное копирование'; en = 'Perform a backup'");
	ПараметрыПредупреждения.Приоритет = 50;
	ПараметрыПредупреждения.ТекстПредупреждения = НСтр("ru = 'Не выполнено резервное копирование при завершении работы.'; en = 'Backup at shutdown was not performed.'");
...
Процедура ПровестиРезервноеКопирование()
	...
	Кнопки.Добавить("Нет", НСтр("ru = 'Нет'; en = 'None'"));
	Кнопки.Добавить("Отложить", НСтр("ru = 'Отложить на 15 минут'; en = 'Defer to 15 minutes'"));
	...
	ПоказатьВопрос(ОписаниеОповещение, НСтр("ru = 'Все готово для выполнения резервного копирования по расписанию.
		...
		|en = 'You are ready to perform a scheduled backup.
		|Perform a backup now?'"),
		Кнопки, 30, "Да", НСтр("ru = 'Резервное копирование по расписанию'; en = 'Backup schedule'"), "Да");
	...
	Если ПараметрыУдаления.ТипОграничения <> "ХранитьВсе" И КаталогХранения <> Неопределено Тогда
		...
		Исключение
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
				НСтр("ru = 'Не удалось провести очистку каталога с резервными копиями.'; en = 'Failed to cleanup the backup directory.'") + Символы.ПС 
...
Процедура ПроверитьРезервноеКопированиеИБ(Параметры)
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Резервное копирование'; en = 'Backup'"),
			"e1cib/command/ОбщаяКоманда.ПоказатьРезультатРезервногоКопирования",
			НСтр("ru = 'Резервное копирование проведено успешно'; en = 'Backup was successful'"), БиблиотекаКартинок.Информация32);
...
Процедура ОповеститьПользователяОРезервномКопировании(ВариантОповещения)
	...
	Если ВариантОповещения = "Просрочено" Тогда
		...
		ТекстПояснения = НСтр("ru = 'Автоматическое резервное копирование не было выполнено.'; en = 'Automatic backup has not been met.'"); 
		ПоказатьОповещениеПользователя(НСтр("ru = 'Резервное копирование'; en = 'Backup'"),
	...
	ИначеЕсли ВариантОповещения = "ЕщеНеНастроено" Тогда
		...
		ТекстПояснения = НСтр("ru = 'Рекомендуется настроить резервное копирование информационной базы.'; en = 'It is recommended to configure infobase backup.'"); 
		ПоказатьОповещениеПользователя(НСтр("ru = 'Резервное копирование'; en = 'Backup'"),
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Резервное копирование информационной базы'; en = 'Infobase backup'",
...
Функция КлиентскиеПараметрыРезервногоКопирования() Экспорт
	#Если НЕ ВебКлиент Тогда
		...
		СтруктураПараметров.Вставить("СобытиеЖурналаРегистрации", НСтр("ru = 'Резервное копирование ИБ'; en = 'Infobase backup'"));
