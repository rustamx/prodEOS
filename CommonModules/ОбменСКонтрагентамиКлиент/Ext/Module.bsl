...
Процедура ОповеститьОНовыхДокументахЭДО() Экспорт
	...
	НавигационнаяСсылка = НавигационнаяСсылкаТекущиеДелаЭДО();
		ЗаголовокОповещения = НСтр("ru ='Сервис ЭДО'; en = 'EDI service'");
		ТекстОповещения = НСтр("ru = 'Получить новые документы'; en = 'Get new documents'");
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Если РезультатПроверки = Неопределено Тогда
			...
			Если Форма.Модифицированность ИЛИ НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
				...
				|Записать и провести?';
				|en = 'save and post.
				|Save and post?'"), НСтр("ru = 'записать.
				|Записать?';
				|en = 'save.
				|Save?'"));
				
				ШаблонСообщения = НСтр("ru = 'Документ изменен. Для формирования электронного документа его необходимо %1'; en = 'Document has been changed. To generate an electronic document, it is necessary to %1'");
				...
				ПоказатьВопрос( Обработчик, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.Отмена,
					НСтр("ru = 'Документ изменен.'; en = 'The document has been changed.'"));
...
Процедура ЗаписатьВФорме(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		...
		Если Проведен Тогда
			...
			Исключение
				ПоказатьПредупреждение(, НСтр("ru = 'Операция не выполнена.'; en = 'Operation not executed.'"));
				Отказ = Истина;
			КонецПопытки;
		Иначе
...
Процедура ОткрытьАктуальныйЭД(ПараметрКоманды, Источник = Неопределено, ПараметрыОткрытия = Неопределено) Экспорт
	...
	Для Каждого ТекЭл Из МассивСсылок Цикл
		...
		Иначе
			ТекстШаблона = НСтр("ru = '%1. Электронный документ не найден.'; en = '%1. Electronic document could not be found.'");
...
Процедура ЗакрытьПринудительноЭДО(МассивВладельцевЭД) Экспорт
	...
	ПричинаЗакрытия = "";
	ПоказатьВводСтроки(Обработчик, ПричинаЗакрытия, НСтр("ru = 'Укажите причину закрытия документооборота'; en = 'Specify reason for finishing interchange'"),,Истина);
...
Процедура ПерезаполнитьДокумент(ПараметрКоманды, Источник = Неопределено, СопоставлениеУжеВыполнено = Ложь, ЭД = Неопределено, СпособОбработки = "") Экспорт
	...
	Шаблон = НСтр("ru = 'Перезаполнение документа %1.
						...
						|en = 'Refilling document %1.
						|The operation is possible only for unposted documents.'");
	...
	Если СоответствиеВладельцевИЭД.Количество() = 0 Тогда
		Для Каждого ТекущийДокумент Из МассивСсылок Цикл
			Шаблон = НСтр("ru = 'Электронный документ для %1 не найден'; en = 'Electronic document for %1 is not found'");
	...
	Если МассивИзмененныхВладельцев.Количество() > 0 Тогда
		...
		Если МассивИзмененныхВладельцев.Количество() = 1 Тогда
			ТекстСостоянияВывод = НСтр("ru = 'Документ перезаполнен.'; en = 'Document refilled.'");
		Иначе
			ТекстСостоянияВывод = НСтр("ru = 'Документы перезаполнены (%1).'; en = 'Documents were refilled (%1).'");
...
Процедура ОбработатьРезультатПомещенияФайла(ВыборВыполнен, АдресФайла, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если ВРег(Расширение) <> ВРег("zip") Тогда
		ТекстСообщения = НСтр("ru = 'Не корректный формат файла.
									...
									|Select the file with the ""zip"" extension.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ЗакрытьПринудительноРезультатВводаСтроки(ПричинаЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПричинаЗакрытия) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для закрытия документооборота по выбранным ЭД необходимо указать причину закрытия.'; en = 'To finish the interchange for the selected ED, you must specify the reason for finishing.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	...
	ТекстОповещения = НСтр("ru = 'Изменено состояние ЭД документов на ""Закрыт принудительно"": (%1)'; en = 'Changed status of ED documents to ""Forced closing"": (%1)'");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", КоличествоОбработанныхЭД);
	ПоказатьОповещениеПользователя(НСтр("ru = 'Обработка документов'; en = 'Processing documents'"), , ТекстОповещения);
...
Процедура ОбработкаНавигационнойСсылкиВФормеОбъектаИБ(СсылкаНаОбъект)
	...
	Если Не ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЕстьПравоНастройкиЭДО(Ложь) Тогда
		ТекстСообщения = НСтр("ru = 'Недостаточно прав для настройки ЭДО с контрагентом.
									...
									|en = 'Not enough permissions to set up EDI with a counterparty.
...
Процедура ОбновитьКартинкуЭДО(Форма, КартинкаЭДО, МестоРазмещенияКоманд)
	...
	Если Не КартинкаЭДО = БиблиотекаКартинок.ЭмблемаСервиса1СЭДО Тогда
		ПодменюЭДО.Подсказка = НСтр("ru = 'В сервисе 1С-ЭДО есть новые документы'; en = 'New documents are present in 1C-EDI service'");
	Иначе
		ПодменюЭДО.Подсказка = НСтр("ru = 'Команды ЭДО'; en = 'EDI commands'");
	КонецЕсли;
