...
Процедура ОткрытьОтчетИлиФорму(ТекущийЭлемент, Пользователь, ТекущийПользователь, ИмяФормыПерсональныхНастроек) Экспорт
	...
	Если Пользователь <> ТекущийПользователь Тогда
		ТекстПредупреждения = НСтр("ru = 'Для просмотра настроек другого пользователя необходимо запустить программу от его имени 
			|и открыть нужный отчет или форму.'; en = 'To view another user''s settings, you must run the program from its name and open the desired report or form.'");
			...
			Иначе
				...
				Если ЭлементДереваЗначений.ТекущиеДанные.ТипСтроки = "НастройкиРабочегоСтола" Тогда
					ПоказатьПредупреждение(,НСтр("ru = 'Для просмотра настроек рабочего стола перейдите к разделу ""Рабочий стол""
						|в командном интерфейсе программы.'; en = 'To view the desktop settings, go to ""Desktop"" in the command interface.'"));
					Возврат;
				КонецЕсли;
				...
				Если ЭлементДереваЗначений.ТекущиеДанные.ТипСтроки = "НастройкиКомандногоИнтерфейса" Тогда
					ПоказатьПредупреждение(,НСтр("ru = 'Для просмотра настроек командного интерфейса выберите нужный раздел
						|командного интерфейса программы.'; en = 'To view the settings command interface select the partition command interface program.'"));
					Возврат;
				КонецЕсли;
				
				Если РодительЭлемента <> Неопределено Тогда
					ТекстПредупреждения = НСтр("ru = 'Для просмотра данной настройки необходимо открыть ""%1"" 
						|и затем перейти к форме ""%2"".'; en = 'To view this setting, you must open ""%1"" and then go to the form ""%2"".'");
...
Функция ФормированиеСтрокиКоличестваНастроек(КоличествоНастроек) Экспорт
	...
	ПрописьЧислаИПредмета = ЧислоПрописью(
		КоличествоНастроек,
		"Л = ru_RU",
		НСтр("ru = 'настройка,настройки,настроек,,,,,,0'; en = 'setting,settings,settings,,,,,,0'"));
...
Функция ФормированиеПоясненияПриКопировании(ПредставлениеНастройки, КоличествоНастроек, ПояснениеКомуСкопированыНастройки) Экспорт
	...
	Если КоличествоНастроек = 1 Тогда
		...
		ТекстПояснения = НСтр("ru = '""%1"" скопирована %2'; en = '""%1"" copied %2'");
	...
	Иначе
		...
		ТекстПояснения = НСтр("ru = 'Скопировано %1 %2'; en = 'Copied %1 %2'");
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ПояснениеПользователи(КоличествоПользователей, Пользователь) Экспорт
	...
	Если КоличествоПользователей = 1 Тогда
		ПояснениеКомуСкопированыНастройки = НСтр("ru = 'пользователю ""%1""'; en = 'user ""%1""'");
	...
	Иначе
		ПояснениеКомуСкопированыНастройки = НСтр("ru = '%1 пользователям'; en = '%1 users'");
		ПояснениеКомуСкопированыНастройки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
