...
Процедура СлужебнаяУстановитьПрефиксИБИПерепрефиксоватьОбъектыСозданныеВЭтойИБ(
	...
	Если ТранзакцияАктивна() Тогда
		...
		ВызватьИсключение НСтр("ru = 'Изменение префикса информационной базы не может выполняться в транзакции.'; en = 'Changing the infobase prefix cannot be executed within a transaction.'");
		
	ИначеЕсли Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для изменения префикса информационной базы.'; en = 'Insufficient privilege to modify the prefix information base.'");
...
Функция НовыйКодОбъекта(Знач ФорматНовогоКода, Знач Код, Знач ЭтоДокумент, Объект)
	...
	Если КоличествоЛидирующихНулей < 0 Тогда
		...
		СтрокаСообщения = НСтр("ru = 'Преобразование %1 объекта %2 не может быть выполнено.
			...
			|en = 'Convert %1 object %2 cannot be performed.
			|Insufficient length %1 . Minimum length %1 object should be %3 characters.'");
		СтрокаСообщения = СтрШаблон(СтрокаСообщения, ?(ЭтоДокумент, НСтр("ru = 'номера'; en = 'number'"), НСтр("ru = 'кода'; en = 'code'")),
...
Процедура ПроверитьВозможностьУстановкиПрефикса()
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		...
		ВызватьИсключение НСтр("ru='Перепрефиксация объектов недоступна.'; en = 'Change prefix objects are not available.'");
...
Функция СобытиеЖурналаРегистрацииПерепрефиксацияОбъектов()
	...
	Возврат НСтр("ru = 'Префиксация объектов.Изменение префикса информационной базы'; en = 'Object prefixation.Changed infobase prefix'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
