	...
	Если CLSID = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Разрешение на использование COM-класса ""%1"" не может быть предоставлено дополнительной обработке,
				...
				|en = 'Permission on using COM class ""%1"" cannot be granted to an additional data processor that works
				|in reverse compartibility mode with extensions mechanism of SSL v. 2.1.3.
				|To use the COM class it is necessary to reconfigure the data processor to work without using the reverse
				|compartibility mode.'"),
			ProgId);
