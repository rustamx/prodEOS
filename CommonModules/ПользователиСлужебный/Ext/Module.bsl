	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось установить параметр сеанса ТекущийПользователь по причине:
			           ...
			           |en = 'Failed to set session parameter ТекущийПользователь because:
			           |""%1"".
			           |
...
Функция АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь, РегистрироватьВЖурнале = Ложь) Экспорт
	...
	Если Не РолиАдминистратораДоступны() Тогда
		Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(
			НСтр("ru = 'Запуск от имени пользователя с правом Администрирование невозможен,
			           ...
			           |en = 'Starting with user account that has System administrator role is not 
			           |possible because such user is not registered in the list of users. 
			           |
			           |For adding and configuring users one should use the users list,
			           |do not use the 1C:Enterprise designer mode for this.'"),
			, РегистрироватьВЖурнале);
	КонецЕсли;
	...
	Исключение
		...
		Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке,
			НСтр("ru = 'Не удалось выполнить автоматическую регистрацию администратора в списке по причине:
			           ...
			           |Для ведения списка и настройки прав пользователей предназначен список Пользователи,
			           |режим конфигурирования 1С:Предприятия для этого использовать не следует.';
			           |en = 'Unable to register the administrator automatically in the list because: 
			           |""%1"". 
			           |
			           |For adding and configuring users one should use the users list,
			           |do not use the 1C:Enterprise designer mode for this.'"),
	...
	Комментарий =
		НСтр("ru = 'Выполнен запуск от имени пользователя с ролью ""Полные права"",
		           ...
		           |Для ведения списка и настройки прав пользователей предназначен список Пользователи,
		           |режим конфигурирования 1С:Предприятия для этого использовать не следует.';
		           |en = 'The program was started on behalf of a user with role ""Full permissions"", 
		           |who does not exist in the list of users. Registered the user automatically. 
		           |
		           |For adding and configuring users one should use the users list,
		           |do not use the 1C:Enterprise designer mode for this.'");
...
|en = 'Run user name with the Administration right unavailabale 
|because user is not registered in the list of users. 
|
|Users list is destined for add and configure users. Do not use 1C:Enterprise designer mode for it.'",
...
Функция ОшибкаНедостаточноПравДляВходаВПрограмму(РегистрироватьВЖурнале = Истина) Экспорт
	...
	Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(
		НСтр("ru = 'Недостаточно прав для входа в программу.
		           |
		           |Обратитесь к администратору.';
		           |en = 'Insufficient permissions for sign in.
		           |
		           |Contact the administrator.'"),
		, РегистрироватьВЖурнале);
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре ПользователиСлужебный.ОбработатьИнтерфейсРолей()
			           ...
			           |en = 'Error in procedure ПользователиСлужебный.ОбработатьИнтерфейсРолей()
			           |Invalid parameter Action value: ""%1"".'"),
			Действие);
	КонецЕсли;
...
Функция СвойстваНеуказанногоПользователя() Экспорт
	...
	Свойства.Вставить("ПолноеИмяДляПоиска", НСтр("ru = '<Не указан>'; en = '<Not specified>'"));
...
Функция ПользовательПоИдентификаторуСуществует(УникальныйИдентификатор,
	...
	|	(Пользователи.ИдентификаторПользователяИБ = &УникальныйИдентификатор
	|	ИЛИ Пользователи.ра_ИдентификаторПользователяИБ = &УникальныйИдентификатор)
	// ТСК Близнюк С.И.; 26.09.2018; task#657}
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление          = НСтр("ru = 'Пользователи'; en = 'Users'");
	...
	ВидДоступа.Представление          = НСтр("ru = 'Внешние пользователи'; en = 'External users'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru = 'Некорректные сведения о пользователях'; en = 'Incorrect user information'");
...
Функция ВозможноИзменитьПароль(Пользователь, ДополнительныеПараметры = Неопределено) Экспорт
	   ...
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ПользователиВМоделиСервиса") Тогда
		...
		Если Не ДействияСПользователемСервиса.ИзменениеПароля Тогда
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сервис: Недостаточно прав для изменения пароля пользователя ""%1"".'; en = 'Service: do not have permission to change the password for user ""%1"".'"), Пользователь));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьДействительностьПользователя")
	   И РеквизитыПользователя.Недействителен <> Ложь Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователь ""%1"" недействителен.'; en = 'User ""%1"" is notvalid.'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьНаличиеПользователяИБ")
	   И ПользовательИБ = Неопределено Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не найдена учетная запись пользователя ""%1"".'; en = 'Not found user account ""%1"".'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   И Не УровеньДоступа.НастройкиДляВхода Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недостаточно прав для изменения пароля пользователя ""%1"".'; en = 'Insufficient permissions to modify password for the user ""%1"".'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПользовательИБ <> Неопределено И ПользовательИБ.ЗапрещеноИзменятьПароль Тогда
		Если УровеньДоступа.НастройкиДляВхода Тогда
			Если ДополнительныеПараметры.Свойство("УчитыватьСвойствоЗапрещеноИзменятьПароль") Тогда
				ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Установлен запрет изменения пароля пользователя ""%1"".'; en = 'Prohibits changes the password of the user ""%1"".'"), Пользователь));
				Возврат Ложь;
			КонецЕсли;
		Иначе
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Установлен запрет изменения пароля пользователя ""%1"".
				           |Обратитесь к администратору.';
				           |en = 'Prohibits changes the password of the user ""%1"".
				           |Please contact your administrator.'"), Пользователь));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	ЧислоИПредмет = Формат(КоличествоДней, "ЧГ=") + " "
		+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоДней,
			"Л = ru_RU", НСтр("ru = 'день,дня,дней,,,,,,0'; en = 'day, day, days,,,,,,0'"));
	
	ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Пароль можно будет сменить только через %1.'; en = 'Password can be reset only through %1.'"), ЧислоИПредмет));
...
Функция ОбработатьНовыйПароль(Параметры) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   ...
	   И (ВызовИзФормыСменаПароля Или СтарыйПароль <> Неопределено) Тогда
		...
		Если Не Параметры.СтарыйПарольСовпадает Тогда
			Возврат НСтр("ru = 'Старый пароль указан неверно.'; en = 'Old password is incorrect.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1
			           ...
			           |en = '%1 
			           |
			           |Limit is set in the Configurator in the menu ""Tools"", click ""Infobase parameters..."". 
			           |Clear the minimum length and complexity requirement in the configurator and set up users log in settings in the program settings.'"),
			ТекстОшибки);
	КонецЕсли;
	...
	Попытка
		...
		Если ПользователиСлужебныйПовтИсп.Настройки().ОбщиеНастройкиВхода
		   ...
		   И ИспользованныеПароли.Найти(СохраняемоеЗначениеПароля) <> Неопределено Тогда
			...
			ТекстОшибки = НСтр("ru = 'Новый пароль использовался ранее.'; en = 'The new password has been used previously.'");
	...
	Исключение
		...
		Если ВызовИзФормыСменаПароля Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Пользователи.Ошибка смены пароля'; en = 'Users.Password change error'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось сменить пароль пользователя ""%1"" по причине:
					           |%2';
					           |en = 'Failed to change password for user ""%1"" because: 
					           |%2'"),
...
Функция ПодсказкаДляНовогоПароля() Экспорт
	...
	Возврат
		НСтр("ru = 'Надежный пароль:
		           ...
		           |- has no less than 7 characters;
		           |- contains at least 3 of 4 types of characters: uppercase letters, lowercase letters, numbers, special characters;
		           |- does not match the name (login).'");
...
Функция СохраняемоеЗначениеСтрокиПароля(Знач Пароль,
	...
	Иначе
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(ИмяПользователя())
			   И ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0 Тогда
				...
				ВременныйАдминистраторИБ.Имя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Временный первый администратор (%1)'; en = 'Temporary first administrator (%1)'"), Новый УникальныйИдентификатор);
			...
			ВременныйПользовательИБ.Имя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Временный пользователь (%1)'; en = 'Temporary user (%1)'"),
...
Функция УровеньДоступаКСвойствамПользователя(ОписаниеОбъекта, ПараметрыОбработки = Неопределено) Экспорт
	...
	Если ТипЗнч(ОписаниеОбъекта.Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
		...
		УровеньДоступа.Вставить("ра_ДобавлениеИзменениеПользователей", 				РольДоступна("ра_ДобавлениеИзменениеПользователей")); 
		// ТСК Близнюк С.И.; 27.12.2018; task#1648}
		// ТСК Близнюк С.И.; 27.12.2018; task#2249{
		УровеньДоступа.Вставить("ра_ДобавлениеИзменениеПользователейПоОрганизации", РольДоступна("ра_ДобавлениеИзменениеПользователейПоОрганизации")); 
		// ТСК Близнюк С.И.; 27.12.2018; task#2249}
...
Процедура НачатьОбработкуПользователяИБ(ПользовательОбъект,
	...
	ПараметрыОбработки.Вставить("ТекстСообщенияНедостаточноПрав",
		НСтр("ru = 'Недостаточно прав для изменения пользователя информационной базы.'; en = 'Insufficient permissions to modify the infobase user.'"));
	...
	АвтоРеквизиты.Вставить("ра_ИдентификаторПользователяИБ");
	АвтоРеквизиты.Вставить("ра_СвойстваПользователяИБ");
	// ТСК Близнюк С.И.; 12.09.2018; task#465}
	...
	// ТСК Близнюк С.И.; 12.09.2018; task#465{
	Если ДополнительныеСвойства.Свойство("ра_ОписаниеПользователяИБ") Тогда
		ра_ОписаниеПользователяИБ = ДополнительныеСвойства.ра_ОписаниеПользователяИБ;
	КонецЕсли;
	// ТСК Близнюк С.И.; 12.09.2018; task#465}
	...
	Если НЕ ОписаниеПользователяИБ.Свойство("Действие") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |en = 'Error writing user ""%1"".
			           |In ОписаниеПользователяИБ parameter is not specified the action Property.'"),
	   ...
	   И ОписаниеПользователяИБ.Действие <> "Удалить" Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |en = 'Error writing user ""%1"".
			           |In ОписаниеПользователяИБ parameter
			           |is incorrect value ""%2"" of property Action.'"),
			ПользовательОбъект.Ссылка,
		...
		Если ПараметрыОбработки.СтарыйПользовательИБСуществует Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при записи пользователя ""%1"".
				           ...
				           |en = 'Error writing user ""%1"". 
				           |Infobase user cannot be mapped to user in catalog , which is already associated with another infobase user.'"),
		...
		Если ПользовательПоИдентификаторуСуществует(
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при записи пользователя ""%1"".
				           ...
				           |""%2"".';
				           |en = 'Error writing user ""%1"".
				           |You cannot map the infobase user with the user in the catalog,
				           |because it is already mapped to another user in the catalog
				           |""%2"".'"),
	...
	КонецЕсли;
	// ТСК Близнюк С.И.; 12.09.2018; task#465{
	Если ДополнительныеСвойства.Свойство("ра_ОписаниеПользователяИБ")
	   И ра_ОписаниеПользователяИБ.Действие = "Записать"
	   И ра_ОписаниеПользователяИБ.Свойство("УникальныйИдентификатор")
	   И ЗначениеЗаполнено(ра_ОписаниеПользователяИБ.УникальныйИдентификатор)
	   И ра_ОписаниеПользователяИБ.УникальныйИдентификатор
	     <> ПараметрыОбработки.СтарыйПользователь.ра_ИдентификаторПользователяИБ Тогда
		
		ПараметрыОбработки.Вставить("УстановкаПользователяИБ");
		
		Если ПараметрыОбработки.ра_СтарыйПользовательИБСуществует Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при записи пользователя ""%1"".
				           |Нельзя сопоставить пользователя ИБ с пользователем в справочнике,
				           |с которым уже сопоставлен другой пользователем ИБ.';
		...
		Если ПользовательПоИдентификаторуСуществует(
			ра_ОписаниеПользователяИБ.УникальныйИдентификатор,
			ПользовательОбъект.Ссылка,
			НайденныйПользователь) Тогда
	...
	// ТСК Близнюк С.И.; 12.09.2018; task#465{
	Если ДополнительныеСвойства.Свойство("ра_ОписаниеПользователяИБ") Тогда 
		Если ра_ОписаниеПользователяИБ.Действие = "Записать" Тогда
			
			// Проверка прав на изменение пользователей с широким доступом.
			Если ПараметрыОбработки.ра_СтарыйПользовательИБСуществует
			   И УровеньДоступаПользователяВыше(ПараметрыОбработки.ра_ОписаниеСтарогоПользователяИБ, УровеньДоступа) Тогда
			...
			ЗаписатьПользователяИБ(ПользовательОбъект, ПараметрыОбработки, Истина);
		Иначе
			УдалитьПользователяИБ(ПользовательОбъект, ПараметрыОбработки);
		КонецЕсли;
	КонецЕсли;
	...
	// Обновление значения реквизита контролируемого при записи.
	ПараметрыОбработки.АвтоРеквизиты.ра_ИдентификаторПользователяИБ =
		ПользовательОбъект.ра_ИдентификаторПользователяИБ;
	
	ра_ОписаниеНовогоПользователяИБ = Неопределено;
	Если Пользователи.ПрочитатьПользователяИБ(
	         ПользовательОбъект.ра_ИдентификаторПользователяИБ,
	         ра_ОписаниеНовогоПользователяИБ) Тогда
		
		ПараметрыОбработки.Вставить("ра_НовыйПользовательИБСуществует", Истина);
		ПараметрыОбработки.Вставить("ра_ОписаниеНовогоПользователяИБ", ра_ОписаниеНовогоПользователяИБ);
		
		// Проверка прав на изменение пользователей с широким доступом.
		Если ПараметрыОбработки.ра_СтарыйПользовательИБСуществует
		   И УровеньДоступаПользователяВыше(ПараметрыОбработки.ра_ОписаниеНовогоПользователяИБ, УровеньДоступа) Тогда
			
			ВызватьИсключение ПараметрыОбработки.ТекстСообщенияНедостаточноПрав;
		КонецЕсли;
	Иначе
		ПараметрыОбработки.Вставить("ра_НовыйПользовательИБСуществует", Ложь);
	КонецЕсли;
	// ТСК Близнюк С.И.; 12.09.2018; task#465}
	   ...
	   И ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0 Тогда
			...
			Если ЗапретРедактированияРолей()
				...
				Если ВРег(Пользователи.РольАдминистратораСистемы().Имя) = ВРег("ПолныеПрава") Тогда
					Текст = НСтр("ru = 'В список пользователей программы добавляется первый пользователь,
					                   ...
					                   |Продолжить?';
					                   |en = 'The first user is added in the list, so it will automatically be assigned the ""Full accsess"" roles.
				...
				Иначе
					Текст = НСтр("ru = 'В список пользователей программы добавляется первый пользователь, поэтому ему
					                   ...
					                   |Продолжить?';
					                   |en = 'The first user is added in the list, so it will automatically be assigned the ""System administrator"" and ""Full accsess"" roles.
					                   |Do you want to continue?'");
				КонецЕсли;
		...
		Иначе
			...
			Текст = НСтр("ru = 'Первый пользователь информационной базы должен иметь полные права.
			                   ...
			                   |An external user may not be full.
			                   |First, create administrator in Users catalog.'");
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОписаниеПользователяИБ.Свойство("Имя") Тогда
		...
		Если ПустаяСтрока(Имя) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Не заполнено Имя (для входа).'; en = 'Name is empty (for logging).'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрДлина(Имя) > 64 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Имя (для входа) превышает 64 символа.'; en = 'Name (login) exceeds 64 characters.'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрНайти(Имя, ":") > 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Имя (для входа) содержит запрещенный символ "":"".'; en = 'Name (login) contains an forbidden character "":"".'"),
				,
				"Имя",
				,
				Отказ);
			   ...
			   И ПользовательИБ.УникальныйИдентификатор
				...
				Если НайденныйПользователь = Неопределено
				 ИЛИ НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
					...
					ТекстОшибки = НСтр("ru = 'Имя (для входа) уже занято.'; en = 'Name (login) is already taken.'");
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Имя (для входа) уже занято для пользователя ""%1"".'; en = 'Name (login) is already taken for the user ""%1"".'"),
			...
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru = 'Пользователь ОС должен быть в формате
					           |""\\ИмяДомена\ИмяПользователя"".'; en = 'OS user must be in the format ""\\DomainName\UserName"".'"),
					,
					"ПользовательОС",
					,
					Отказ);
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru= 'При обновлении ролей внешнего пользователя
				          ...
				          |en = 'When you update roles for the external user
				          |""%1""
				          |role ""%2""
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Пользователи.Роль не найдена в метаданных'; en = 'Users.Role not found in metadata'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Функция ОбъектАвторизацииИспользуется(Знач СсылкаНаОбъектАвторизации,
	...
	Если Результат Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Уже существует внешний пользователь, связанный с объектом ""%1"".'; en = 'There is already an external user, associated with the ""%1"".'"),
...
Функция ПеремещениеПользователяВНовуюГруппу(МассивПользователей, ГруппаИсточник,
	...
	Если МассивПеремещенныхПользователей.Количество() = 0 И МассивНеПеремещенныхПользователей.Количество() = 0 Тогда
		Если МассивПользователей.Количество() = 1 Тогда
			ТекстСообщения = НСтр("ru = 'Пользователь ""%1"" уже включен в группу ""%2"".'; en = 'User ""%1"" is already included in the group ""%2"".'");
			ИмяПеремещаемогоПользователя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МассивПользователей[0], "Наименование");
		Иначе
			ТекстСообщения = НСтр("ru = 'Все выбранные пользователи уже включены в группу ""%2"".'; en = 'All selected users are already included in the group ""%2"".'");
			...
			Если ТипПользователяСовпадаетСГруппой Тогда
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Пользователь ""%1"" не может быть включен в группу ""%2"",
					           ...
					           |en = 'User ""%1"" could not be included in the group ""%2"",
					           |because the group have a sign ""All users of a given type"".'"),
			...
			Иначе
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Пользователь ""%1"" не может быть включен в группу ""%2"",
					           ...
					           |en = 'User ""%1"" could not be included in the group ""%2"",
					           |because its participants are only %3.'"),
		...
		Иначе
			...
			СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не все пользователи могут быть включены в группу ""%1"",
				           ...
				           |en = 'Not all users may be included in the group ""%1"",
				           |because its participants are only %2
				           |or group stands a sign ""All users of a given type"".'"),
				НаименованиеГруппы,
	...
	Если КоличествоПользователей = 1 Тогда
		...
		Если ГруппаПриемник = Справочники.РабочиеГруппы.ВсеПользователи
		 Или ГруппаПриемник = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			...
			СтрокаДействие = НСтр("ru = 'исключен из группы'; en = 'excluded from the group'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СтрокаДействие = НСтр("ru = 'перемещен в группу'; en = 'moved to the group'");
		Иначе
			СтрокаДействие = НСтр("ru = 'включен в группу'; en = 'included in the group'");
		КонецЕсли;
		
		СообщениеПользователю = НСтр("ru = '""%1"" %2 ""%3""'; en = '""%1"" %2 ""%3""'");
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		...
		СтрокаОбъект = Формат(КоличествоПользователей, "ЧДЦ=0") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПользователей,
				"Л = ru_RU", НСтр("ru = 'пользователь,пользователя,пользователей,,,,,,0'; en = 'user, user, users,,,,,,0'"));
		
		Если ГруппаПриемник = Справочники.РабочиеГруппы.ВсеПользователи Тогда
			СтрокаДействие = НСтр("ru = 'исключены из группы'; en = 'excluded from the group'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СтрокаДействие = НСтр("ru = 'перемещены в группу'; en = 'moved to the group'");
		Иначе
			СтрокаДействие = НСтр("ru = 'включены в группу'; en = 'included in the group'");
		КонецЕсли;
...
Функция ЗаголовокСообщенияАвторизацияНеВыполненаСПереводомСтроки()
	...
	Возврат НСтр("ru = 'Авторизация не выполнена. Работа системы будет завершена.'; en = 'Authorization is not performed. The system will be shut down.'")
...
Функция ЗначенияПараметровСеансаТекущийПользователь()
	...
	ЗаголовокОшибки = НСтр("ru = 'Не удалось установить параметр сеанса ТекущийПользователь.'; en = 'Failed to set session parameter ТекущийПользователь.'") + Символы.ПС;
	...
	Если СведенияОПользователе.ТекущийПользователь        = Неопределено
	 Или СведенияОПользователе.ТекущийВнешнийПользователь = Неопределено Тогда
		...
		Возврат ЗаголовокОшибки + ТекстСообщенияПользовательНеНайденВСправочнике(
			...
			+ НСтр("ru = 'Возникла внутренняя ошибка при поиске пользователя.'; en = 'Error occurred while searching for the user.'");
	КонецЕсли;
...
Функция ТекстСообщенияТекущийПользовательНедоступенВСеансеБезРазделителей()
	...
	Возврат
		НСтр("ru = 'Недопустимое получение параметра сеанса ТекущийПользователь
		           ...
		           |en = 'Invalid attempt to get the value of session parameter ТекущийПользователь
		           |in a separated infobase without specifying all the separators.'");
...
Функция НайтиТекущегоПользователяВСправочнике()
	...
	Если Выборка.Следующий() Тогда
		...
		Если Не ВнешниеПользователи.ИспользоватьВнешнихПользователей() Тогда
			Возврат НСтр("ru = 'Внешние пользователи отключены.'; en = 'External users are disabled.'");
		КонецЕсли;
	...
	|	ИЛИ Пользователи.ра_ИдентификаторПользователяИБ = &ИдентификаторПользователяИБ";
	// ТСК Близнюк С.И.; 26.09.2018; task#657}
	...
	Если ВнешниеПользователи.ИспользоватьВнешнихПользователей() Тогда
		ШаблонСообщенияОбОшибке =
			НСтр("ru = 'Пользователь ""%1"" не найден в справочниках
			           ...
			           |
			           |Обратитесь к администратору.';
			           |en = 'User ""%1"" is not found in catalogs ""Users"" and ""External users"".
			           |
			           |Contact the administrator.'");
	Иначе
		ШаблонСообщенияОбОшибке =
			НСтр("ru = 'Пользователь ""%1"" не найден в справочнике ""Пользователи"".
			           |
			           |Обратитесь к администратору.';
			           |en = 'User ""%1"" is not found in catalog ""Users"".
			           |
			           |Contact the administrator.'");
	КонецЕсли;
...
Функция КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке, ШаблонОшибки = "", РегистрироватьВЖурнале = Истина)
	...
	Если РегистрироватьВЖурнале Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Пользователи.Ошибка входа в программу'; en = 'Users.Sign in error'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПроверитьПраваПользователя(ПользовательИБ, РежимПроверки, ЭтоВнешнийПользователь)
	...
	Для Каждого ОписаниеРоли Из РолиДляПроверки Цикл
		...
		Если НазначениеРолей.ТолькоДляАдминистраторовСистемы.Получить(ИмяРоли) <> Неопределено Тогда
			ТекстШаблона = НСтр("ru = '""%1"" (предназначена только для администраторов системы)'; en = '""%1"" (intended only for system administrators)'");
		
		ИначеЕсли РазделениеВключено
		        И НазначениеРолей.ТолькоДляПользователейСистемы.Получить(ИмяРоли) <> Неопределено Тогда
			
			ТекстШаблона = НСтр("ru = '""%1"" (предназначена только для пользователей системы)'; en = '""%1"" (intended only for users of the system)'");
		Иначе
			...
			Если НазначениеРолей.ТолькоДляВнешнихПользователей.Получить(ИмяРоли) <> Неопределено Тогда
				ТекстШаблона = НСтр("ru = '""%1"" (предназначена только для внешних пользователей)'; en = '""%1"" (intended only for external users)'");
				
			Иначе // Это внешний пользователь.
				ТекстШаблона = НСтр("ru = '""%1"" (предназначена только для пользователей)'; en = '""%1"" (users only)'");
			КонецЕсли;
		КонецЕсли;
	...
	ИмяСобытия = НСтр("ru = 'Пользователи.Ошибка при установке ролей пользователю ИБ'; en = 'Users.Error while assigning roles to infobase user'",
	     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если РежимПроверки = "ПриЗапуске" Тогда
		Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 Тогда
			ТекстРегистрацииВхода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Попытка входа пользователя %1 с недоступной ролью:
				           |%2.';
				           |en = 'User logon attempt %1 with inaccessible role: 
				           |%2.'"),
		...
		Иначе
			ТекстРегистрацииВхода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Попытка входа пользователя %1 с недоступными ролями:
				           |%2.';
				           |en = 'Logon attempt of user %1 with inaccessible roles: 
				           |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
		КонецЕсли;
		...
		ТекстСообщенияВхода =
			НСтр("ru = 'Невозможно выполнить вход из-за наличия недоступных ролей.
			           |Обратитесь к администратору.';
			           |en = 'Unable to sign in because inaccessible roles are present.
			           |Contact the administrator.'");
	...
	Если СтрЧислоСтрок(УдаляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(УдаляемыеНедоступныеРоли) Тогда
		ТекстРегистрацииУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка удалить пользователю %1 недоступную роль
			           |""%2"".';
			           |en = 'Attempting to deprive user %1 of inaccessible role: 
			           |""%2"".'"),
	...
	ИначеЕсли СтрЧислоСтрок(УдаляемыеНедоступныеРоли) > 1 Тогда
		ТекстРегистрацииУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка удалить пользователю %1 недоступные роли:
			           |%2.';
			           |en = 'Attempting to deprive user %1 of inaccessible roles: 
			           |%2.'"),
			ПользовательИБ.ПолноеИмя, УдаляемыеНедоступныеРоли);
	Иначе
	...
	Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(ДобавляемыеНедоступныеРоли) Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка назначить пользователю %1 недоступную роль:
			           |%2.';
			           |en = 'Attempting to assign to user %1 inaccessible role: 
			           |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	...
	ИначеЕсли СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) > 1 Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Попытка назначить пользователю %1 недоступные роли:
			           |%2.';
			           |en = 'Attempting to assign to user %1 inaccessible roles: 
			           |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
	...
	Если СтрЧислоСтрок(УдаляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(УдаляемыеНедоступныеРоли) Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователю ""%1"" невозможно удалить недоступную роль.'; en = 'User ""%1"" cannot be deprived of inaccessible role.'"),
			ПользовательИБ.ПолноеИмя);
	ИначеЕсли СтрЧислоСтрок(УдаляемыеНедоступныеРоли) > 1 Тогда
		ТекстСообщенияУдаления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователю ""%1"" невозможно удалить недоступные роли.'; en = 'User ""%1"" cannot be deprived of inaccessible roles.'"),
			ПользовательИБ.ПолноеИмя);
	Иначе
	...
	Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(ДобавляемыеНедоступныеРоли) Тогда
		ТекстСообщенияДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователю ""%1"" невозможно назначить недоступную роль
			           |%2.';
			           |en = 'User ""%1"" cannot acquire inaccessible role: 
			           |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	...
	ИначеЕсли СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) > 1 Тогда
		ТекстСообщенияДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Пользователю ""%1"" невозможно назначить недоступные роли:
			           |%2.';
			           |en = 'User ""%1"" cannot acquire inaccessible roles: 
			           |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
	...
	Исключение
		...
		ШаблонТекстаОшибки =
			НСтр("ru = 'Не удалось записать сведения о текущем пользователе по причине:
			           ...
			           |
			           |Обратитесь к администратору.';
			           |en = 'Failed to write information about the current user because:
			           |%1
			           |
		...
		Если ПриЗапуске Тогда
			...
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Пользователи.Ошибка входа в программу'; en = 'Users.Sign in error'",
					     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
		...
		Иначе
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Пользователи.Ошибка обновления даты последней активности'; en = 'Users.Error while updating last activity data'",
					     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Метаданные.НайтиПоТипу(ТипЗнч(ТекущийПользователь)),
					ТекущийПользователь,
...
Функция ОшибкаДлиныИлиСложностиПароля(Пароль, МинимальнаяДлинаПароля, СложныйПароль)
	...
	Если СтрДлина(Пароль) < МинимальнаяДлинаПароля Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Количество символов нового пароля должно быть не менее %1.'; en = 'Number of characters of new password must be at least %1.'"),
	...
	Если СложныйПароль И Не ПарольОтвечаетТребованиямСложности(Пароль) Тогда
		Возврат НСтр("ru = 'Пароль не отвечает требованиям сложности.'; en = 'Password does not meet complexity requirements.'")
			+ Символы.ПС + Символы.ПС
...
Функция ПарольОтвечаетТребованиямСложности(Пароль)
	...
	Попытка
		...
		ВременныйПользовательИБ.Имя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Временный пользователь (%1)'; en = 'Temporary user (%1)'"),
			Новый УникальныйИдентификатор);
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось проверить отвечает ли пароль требованиям сложности по причине:
			           |%1';
			           |en = 'Could not verify whether the password meets the complexity requirements because: 
			           |%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
...
Процедура ЗапомнитьСвойстваПользователя(ПользовательОбъект, ПараметрыОбработки)
	...
	|ПометкаУдаления,
	// ТСК Близнюк С.И.; 12.09.2018; task#465{
	|ра_ИдентификаторПользователяИБ,
	|ра_СвойстваПользователяИБ,
	// ТСК Близнюк С.И.; 12.09.2018; task#465}
	|Недействителен";
	...
	Если ПользовательОбъект.ЭтоНовый() Или ПользовательОбъект.Ссылка <> СтарыйПользователь.Ссылка Тогда
		...
		// ТСК Близнюк С.И.; 12.09.2018; task#465{
		СтарыйПользователь.ра_ИдентификаторПользователяИБ =
			Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000");
		СтарыйПользователь.ра_СвойстваПользователяИБ = Новый ХранилищеЗначения(Неопределено);
		// ТСК Близнюк С.И.; 12.09.2018; task#465}
	КонецЕсли;
	...
	КонецЕсли;
	// ТСК Близнюк С.И.; 12.09.2018; task#465{
	ра_ОписаниеСтарогоПользователяИБ = Неопределено;
	ПараметрыОбработки.Вставить("ра_СтарыйПользовательИБСуществует", Пользователи.ПрочитатьПользователяИБ(
		СтарыйПользователь.ра_ИдентификаторПользователяИБ, ра_ОписаниеСтарогоПользователяИБ));
	
	ПараметрыОбработки.Вставить("ра_СтарыйПользовательИБТекущий", Ложь);
	
	Если ПараметрыОбработки.ра_СтарыйПользовательИБСуществует Тогда
		ПараметрыОбработки.Вставить("ра_ОписаниеСтарогоПользователяИБ", ра_ОписаниеСтарогоПользователяИБ);
		
		Если ра_ОписаниеСтарогоПользователяИБ.УникальныйИдентификатор =
				ПользователиИнформационнойБазы.ТекущийПользователь().УникальныйИдентификатор Тогда
		
			ПараметрыОбработки.Вставить("ра_СтарыйПользовательИБТекущий", Истина);
		КонецЕсли;
	КонецЕсли;
	// ТСК Близнюк С.И.; 12.09.2018; task#465}
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура ЗаписатьПользователяИБ(ПользовательОбъект, ПараметрыОбработки, ЭтоДополнительныйПользователь = Ложь)
// ТСК Близнюк С.И.; 12.09.2018; task#465}
	...
	// ТСК Близнюк С.И.; 12.09.2018; task#465{
	Если ЭтоДополнительныйПользователь Тогда
		ОписаниеПользователяИБ = ДополнительныеСвойства.ра_ОписаниеПользователяИБ;
	КонецЕсли;
	// ТСК Близнюк С.И.; 12.09.2018; task#465}
	...
	//	
	//ИначеЕсли ЗначениеЗаполнено(ПараметрыОбработки.СтарыйПользователь.ИдентификаторПользователяИБ) Тогда
	...
	Если ЭтоДополнительныйПользователь Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ПараметрыОбработки.СтарыйПользователь.ра_ИдентификаторПользователяИБ) Тогда
			ИдентификаторПользователяИБ = СтарыйПользователь.ра_ИдентификаторПользователяИБ;
			СоздатьНовогоПользователяИБ = НЕ ПараметрыОбработки.ра_СтарыйПользовательИБСуществует;
		Иначе
			ИдентификаторПользователяИБ = Неопределено;
			СоздатьНовогоПользователяИБ = Истина;
		КонецЕсли;
	...
	Если ?(ЭтоДополнительныйПользователь, ПараметрыОбработки.ра_СтарыйПользовательИБСуществует, ПараметрыОбработки.СтарыйПользовательИБСуществует) Тогда
		СтараяАутентификация = ?(ЭтоДополнительныйПользователь, ПараметрыОбработки.ра_ОписаниеСтарогоПользователяИБ, ПараметрыОбработки.ОписаниеСтарогоПользователяИБ);
	// ТСК Близнюк С.И.; 12.09.2018; task#465}
		Если Пользователи.ВходВПрограммуРазрешен(СтараяАутентификация) Тогда
			...
			// ТСК Близнюк С.И.; 12.09.2018; task#465{
			Если ЭтоДополнительныйПользователь Тогда
				ПользовательОбъект.ра_СвойстваПользователяИБ = Новый ХранилищеЗначения(ХранимыеСвойства);
				АвтоРеквизиты.ра_СвойстваПользователяИБ = ПользовательОбъект.ра_СвойстваПользователяИБ;
			Иначе
			// ТСК Близнюк С.И.; 12.09.2018; task#465}
	...
	Если ЭтоДополнительныйПользователь Тогда
		ПользовательОбъект.ра_ИдентификаторПользователяИБ = ИдентификаторПользователяИБ;
	Иначе
		ПользовательОбъект.ИдентификаторПользователяИБ = ИдентификаторПользователяИБ;
	КонецЕсли;
	// ТСК Близнюк С.И.; 12.09.2018; task#465}
	   ...
	   И БлокируемыеРеквизиты.Служебный <> ПользовательОбъект.Служебный Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |en = 'Error writing user ""%1"". 
			           |Attribute Prepared cannot modify in subscriptions to events.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если АвтоРеквизиты.ИдентификаторПользователяИБ <> ПользовательОбъект.ИдентификаторПользователяИБ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |en = 'Error writing user ""%1"".
			           |Attribute InfoBaseUserID cannot modify.
			           |Atribute is automatically updated.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если Не ОбщегоНазначения.ДанныеСовпадают(АвтоРеквизиты.СвойстваПользователяИБ,
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |Обновление реквизита выполняется автоматически.';
			           |en = 'Error writing user ""%1"". 
			           |AttributeInfoBaseUserProperties cannot modify. 
			           |Atribute is automatically updated.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |en = 'Error writing user ""%1"".
			           |You cannot tag to delete a user who is allowed entry into the program.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |en = 'Error writing user ""%1"".
			           |Cannot be marked invalid user who is allowed entry into the program.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |en = 'Error writing user ""%1"". 
			           |You cannot mark a prepared user who is allowed entry into the program.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
			...
			Если Выборка.СрокДействияИстек Тогда
				ШаблонКомментария =
					НСтр("ru = 'Не удалось снять пользователю ""%1"" признак
					           ...
					           |%2';
					           |en = 'Unable to remove sign ""Connection is permitted""
					           |for the user ""%1"" due to expiry for reason: 
					           |%2'");
			Иначе
				ШаблонКомментария =
					НСтр("ru = 'Не удалось снять пользователю ""%1"" признак
					           ...
					           |%2';
					           |en = 'Unable to remove sign ""Connection is permitted""
					           |for the user ""%1"" due to inactivity over the deadline for reason:
					           |%2'");
			КонецЕсли;
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Пользователи.Ошибка автоматического запрещения входа в программу'; en = 'Users.Error of automatical ban on user sign in'",
				     ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.НайтиПоТипу(ТипЗнч(Пользователь)),
				Пользователь,
