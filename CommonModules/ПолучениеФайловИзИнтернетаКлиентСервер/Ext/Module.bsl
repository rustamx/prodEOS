...
Функция ПолучитьФайлИзИнтернет(Знач URL, Знач НастройкаСохранения, Знач НастройкаСоединения = Неопределено,
	...
	Если ИспользуетсяFTPПротокол Тогда
		...
		Исключение
			...
			СообщениеОбОшибке = НСтр("ru = 'Не удалось установить FTP-соединение с сервером %1:'; en = 'Unable to establish an FTP connection to the server %1:'") + Символы.ПС + "%2";
	...
	Иначе
		...
		Исключение
			...
			|%2';
			|en = 'Unable to establish an HTTP connection to the server %1:
			|%2'");
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		СообщениеОбОшибке = НСтр("ru = 'Не удалось получить файл с сервера %1:'; en = 'Failed to get file from server %1:'") + Символы.ПС + "%2";
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Получение файлов из Интернета'; en = 'Get files from Internet'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
