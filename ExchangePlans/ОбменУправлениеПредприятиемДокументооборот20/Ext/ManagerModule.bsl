...
Процедура ОпределитьНастройки(Настройки, ИдентификаторНастройки) Экспорт
	...
	Если ИдентификаторНастройки = "КА" Тогда
		Синоним = НСтр("ru = '1С:Комплексная автоматизация, ред. 2.1'; en = '1C:Integrated automation, v.2.1'");
	ИначеЕсли ИдентификаторНастройки = "УП" Тогда
		Синоним = НСтр("ru = '1С:ERP Управление предприятием, ред. 2'; en = '1C:Enterprise Resource Planning 2'");
	Иначе
		Синоним = НСтр("ru = '1С:ERP Управление предприятием 2 / 1С:Комплексная автоматизация 2'; en = '1C:Enterprise Resource Planning 2 / 1C:Integrated automation 2.1'");
	КонецЕсли;
	Настройки.ЗаголовокКомандыДляСозданияНовогоОбменаДанными = Синоним;
	Настройки.ЗаголовокПомощникаСозданияОбмена = СтрШаблон(НСтр("ru = 'Синхронизация данных с конфигурацией %1 (настройка)'; en = 'Data synchronization with configuration %1 (setup)'"), Синоним);
...
Функция ИмяФайлаНастроекДляПриемника() Экспорт
	...
	Возврат НСтр("ru = 'Настройки обмена УП-ДО'; en = 'Exchange settings ERP-DM'");
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = '1С:Управление предприятием, ред. 2'; en = '1C:Enterprise resource planning 2'");
...
Функция КраткаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	...
	ПоясняющийТекст = СтрШаблон(
		НСтр("ru = 'Позволяет синхронизировать данные между конфигурациями ""1С:Документооборот"" и ""%1"".'; en = 'Provides data synchronizaion between ""1C:Document management"" and ""%1"".'"),
		Синоним);
