...
Функция УзелПоОчереди(Очередь) Экспорт
	...
	Если ТипЗнч(Очередь) <> Тип("Число")  
		Или Очередь = 0 Тогда
		ТекстИсключения = НСтр("ru = 'Не возможно получить узел плана обмена ОбновлениеИнформационнойБазы, т.к. не передан номер очереди.'; en = 'Unable to get node of exchange plan ОбновлениеИнформационнойБазы, because queue number was not specified.'");
