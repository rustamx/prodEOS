...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если НастройкаОтборовНаУзле.ВыгружатьЗаявкиНаОплату Тогда
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Заявки на оплату выгружаются'; en = 'Requests for payment are exported'");
		...
		ТекстОписания = ТекстОписания + ".";
	Иначе
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Заявки на оплату не выгружаются.'; en = 'Requests for payment are not exported'");
	КонецЕсли;
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = '1С:Бухгалтерия предприятия КОРП, ред. 3.0'; en = '1C:Accounting Corp 3.0'");
...
Функция КраткаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	...
	|en = 'Provides data synchronizaion between ""1C:Document management"" and ""1C:Accounting"".
	|Synchronization is two-way and deals with reference information, contracts and payment requests.'");
