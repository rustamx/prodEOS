...
Процедура ПриОткрытии(Отказ)
	...
	Иначе
		...
		ТекстВопроса = НСтр("ru = 'Для работы с ЭП необходимо установить
								|расширение работы с криптографией.'; en = 'To work with DS you should install the extension for working with cryptography.'");
...
Функция ЕстьДокументыНаПодпись() 
	...
	Если ПроверочныеДанные.КоличествоДокументов = 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'По данному сертификату нет документов на подпись'; en = 'No documents to sign for the certificate'");
...
Процедура ПерейтиНаСтраницу(КДетализации)
	...
	Если КДетализации Тогда
		...
		Заголовок = НСтр("ru = 'Документы на подпись по сертификату'; en = 'Documents to sign by certificate'")+ ": " + СертификатПодписи;
	Иначе
		Элементы.Страницы.ТекущаяСтраница = Элементы.Страницы.ПодчиненныеЭлементы.СтраницаСводки;
		Заголовок = НСтр("ru = 'Документы на подпись'; en = 'Documents to sign'");
	КонецЕсли;
	
	Элементы.Подписать.Заголовок = НСтр("ru = 'Подписать и отправить отмеченные'; en = 'Sign and send the marked'");
...
Процедура ПослеПолученияОтпечатковВыполнитьДействия(Отпечатки, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТаблицаСертификатов.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'Нет сертификатов подписи для пользователя
										...
										|en = 'No digital certificates for the user of rules of
