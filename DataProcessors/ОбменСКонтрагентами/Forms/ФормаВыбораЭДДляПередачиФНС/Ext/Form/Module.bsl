...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЕстьНевыгруженныеДокументы И ТаблицаВыбранныхДокументов.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru = 'В списке выбора есть невыгруженные документы.
			...
			|en = 'Unexported documents are present in the choice list.
			|Do you really wish to close the form?'");
		Отказ = Истина;
...
Процедура БыстрыеОтборыЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли ВРег(Элементы.БыстрыеОтборы.ТекущиеДанные.Параметр) = ВРег("СостояниеВерсииЭД") Тогда
		...
		СостоянияЭД.ПоказатьОтметкуЭлементов(ВыборЭлементаЗавершить,НСтр("ru = 'Состояния ЭД'; en = 'ED states'"));
...
Процедура СформироватьТаблицуБыстрогоОтбора()
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru = 'Сумма с:'; en = 'Amount from:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru = 'Состояние ЭД:'; en = 'ED state:'");
	...
	НоваяСтрока.ПредставлениеПараметра = НСтр("ru = 'Только отраженные в учете:'; en = 'Only reflected in accounting:'");
...
Процедура ДобавитьВВыбранныеДокументы(МассивСтрок)
	...
	ШаблонСообщения = НСтр("ru = 'Для документа ""%1"" не завершен электронный документооборот.'; en = 'For ""%1"" document EDI is not finished.'");
	ТекстСообщения = "";
	...
	Если ВерсияВызова <> 1 И МножественныйВыбор = Ложь И ТаблицаВыбранныхДокументов.Количество() > 0 Тогда
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			ТекстВопроса = ТекстСообщения + Символы.ПС + НСтр("ru = 'Продолжить выгрузку?'; en = 'Proceed with the export?'");
...
Функция СформироватьФайлВыгрузки(Знач ТаблицаДокументов)
	...
	Пока Результат.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Если Файлы.Количество() = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Не удалось выгрузить документы по Организации ""%1"".'; en = 'Failed to export documents of company ""%1"".'");
...
Функция ФайлОписанияВыгрузки(Организация, ТЗОписи, АдресКаталога)
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Формирование выгрузки ЭД в 1с-Отчетность'; en = 'Generation of ED export to 1C-Reports'"),
...
#Если НЕ ВебКлиент Тогда
				...
				ДиалогВыбора.Заголовок = НСтр("ru = 'Выберите каталог для сохранения файла (файлов) выгрузки'; en = 'Select directory to save exported files'");
...
Процедура Выгрузить()
	...
	Если ТаблицаВыбранныхДокументов.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Для формирования выгрузки необходимо выбрать хотя бы один документ.'; en = 'In order to export you must select at least one document.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	Иначе
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			ТекстВопроса = НСтр("ru = 'В списке выбранных документов, присутствуют документы вида ""%1"",
				...
				|Продолжить выгрузку?';
				|en = 'Documents of ""%1"" type are present in the list,
				|which have blank source document attributes (number, date).
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<не требуется>'; en = '<not necessary>'"));
