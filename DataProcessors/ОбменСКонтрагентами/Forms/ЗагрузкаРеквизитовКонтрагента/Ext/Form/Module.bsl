...
Процедура Загрузить(Команда)
	...
	Если ЗаполненыОбязательныеРеквизиты() Тогда
		Если ЗначениеЗаполнено(Контрагент) Тогда
			ТекстВопроса = НСтр("ru = 'Контрагент существует. Перезаполнить реквизиты?'; en = 'The counterparty already exists. Refill the attributes?'");
...
Функция ЗаполненыОбязательныеРеквизиты()
	...
	Если НЕ ЗначениеЗаполнено(Наименование) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не заполнено <Наименование>, загрузка не возможна.'; en = '<Наименование> is not filled in, import is impossible.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ТекстСообщения = НСтр("ru = 'Проверьте правильность указания <Файла> с реквизитами контрагента.'; en = 'Check the correctness of <Файла> with attributes of the counterparty'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
