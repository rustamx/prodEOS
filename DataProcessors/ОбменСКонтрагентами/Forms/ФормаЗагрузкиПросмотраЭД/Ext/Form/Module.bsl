...
Процедура ИзменитьВидимостьДоступностьПриСозданииНаСервере()
	...
	Если ЗагрузкаЭД Тогда
		...
		Если ВРег(ВидЭД) = ВРег("РеквизитыОрганизации") Тогда
			...
			Заголовок = НСтр("ru = 'Загрузка данных из файла'; en = 'Import data from file'");
			
			Элементы.ГруппаНастроекСправочники.Видимость = Истина;
			Элементы.ГруппаНастроекДокументы.Видимость = Ложь;
		Иначе
			
			Заголовок = НСтр("ru = 'Загрузка документа из файла'; en = 'Document import from file'");
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ВидЭД) Тогда
			Текст = Строка(ВидЭД);
		Иначе	
			Текст = НСтр("ru = 'Электронный документ'; en = 'Electronic document'");
		КонецЕсли; 
		Заголовок = Текст;
		Элементы.ГруппаНастроекСправочники.Видимость = Ложь;
		Элементы.ГруппаНастроекДокументы.Видимость = Ложь;
	...
	Если ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
		...
		Элементы.ТипОбъекта.Заголовок = НСтр("ru = 'Загрузить'; en = 'Import'");
		ТипОбъекта = НСтр("ru = 'Каталог товаров'; en = 'Products catalog'");
	КонецЕсли;
...
Процедура ВыполнитьПросмотрЭДСервер(СтруктураЭД, Отказ)
	...
	Если СтруктураЭД.ФайлАрхива Тогда
		...
		Исключение
			...
			КонецЕсли;
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(НСтр("ru = 'Распаковка архива ЭД'; en = 'Unpacking ED archive'"),
		...
		Если ФайлКарточки = Неопределено Или ФайлИнформации = Неопределено Тогда
			...
			ШаблонСообщения = НСтр("ru = 'Возникла ошибка при чтении данных из файла ""%1№"".'; en = 'There was an error while reading data from ""%1№"" file.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СтруктураЭД.ИмяФайла);
			
			ШаблонСообщения = НСтр("ru = 'Файл ""%1"" не содержит электронных документов.'; en = 'File ""%1"" does not contain electronic documents.'");
...
Процедура СформироватьТекстСлужебногоСообщения()
	...
	Если ЗначениеЗаполнено(ИмяФайлаХМЛ) Тогда
		...
		ЗаголовокЭлемента = НСтр("ru = 'Не удалось прочитать файл """+ ИмяФайлаХМЛ+".""'; en = 'Failed to read file """ + ИмяФайлаХМЛ+ ".""'");
		
	Иначе
		
		ЗаголовокЭлемента = НСтр("ru = 'Не найден файл электронного документа ""* .xml.""'; en = 'Electronic document file not found (""*.xml"")'");
...
Процедура ЗагрузитьРеквизитыОрганизации()
	...
	Если СпособЗагрузкиДокумента = 1 И Не ЗначениеЗаполнено(ДокументИБ) Тогда
		ТекстСообщения = НСтр("ru = 'Не указан элемент справочника для перезаполнения.'; en = 'Catalog element for refilling  is not specified.'");
...
Процедура ЗагрузитьДокументЭДО(ЗакрытьФорму)
	...
	Если НЕ МожноЗагрузитьЭДВида(ВидЭД) Тогда
		ТекстСообщения = НСтр("ru = 'Не поддерживается загрузка электронных документов вида ""%1"".'; en = 'Not supported download electronic documents of the type ""%1"".'");
	...
	Если НЕ ЗначениеЗаполнено(Контрагент) Тогда
		ТекстСообщения = НСтр("ru = 'Не указан контрагент.'; en = 'Counterparty is not specified.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
	КонецЕсли;
	...
	Если СпособЗагрузкиДокумента = 1 Тогда
		...
		Если Не ЗначениеЗаполнено(ДокументИБ) Тогда
			ТекстСообщения = НСтр("ru = 'Не указан документ для перезаполнения.'; en = 'Document to refill not specified.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Отказ = Истина;
		КонецЕсли;
...
Процедура ЗагрузитьДокументВИБ(СопоставлятьНоменклатуруПередЗаполнениемДокумента, ОбновитьСтруктуруРазбора = Ложь)
	...
	Иначе
		...
		ТекстОповещения	= НСтр("ru = 'Документ загружен.'; en = 'The document is imported.'");
