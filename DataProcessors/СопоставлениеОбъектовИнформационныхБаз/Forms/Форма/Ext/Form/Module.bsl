...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("ВсеОбъекты", НСтр("ru='Все данные'; en = 'All data'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, Новый ФиксированнаяСтруктура);
	
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъектыНеутвержденные", НСтр("ru='Изменения'; en = 'Changes'"));
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъекты", НСтр("ru='Сопоставленные данные'; en = 'Mapped data'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 0));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъекты", НСтр("ru='Несопоставленные данные'; en = 'Unmapped data'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 1));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъектыПриемника", НСтр("ru='Несопоставленные данные этой базы'; en = 'Unmapped data of this infobase'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставления",  1));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъектыИсточника", НСтр("ru='Несопоставленные данные второй базы'; en = 'Unmapped data of the second infobase'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставления", -1));
	...
	КонецЕсли;
	Заголовок = НСтр("ru = 'Сопоставление данных ""[ПредставлениеДанных]""'; en = 'Data mapping ""[ПредставлениеДанных]""'");
	...
	НаименованиеВторойПрограммы = ?(ПустаяСтрока(НаименованиеВторойПрограммы), НСтр("ru = 'Вторая программа'; en = 'Second program'"), НаименованиеВторойПрограммы);
	...
	Элементы.Пояснение.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		|en = 'For mapping of the data of ""%1"" 
		|to the data of ""%2"" use command ""Map automatically..."".
		|The data that remain unmapped can be matched manually later.'"),
...
Процедура ВыполнитьЗагрузкуДанных(Команда)
	НСтрока = НСтр("ru = 'Получить данные в информационную базу?'; en = 'Import data into infobase?'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Нет соответствия, объект будет скопирован'; en = 'There is no match, the object will be copied'"));
...
Процедура ВыполнитьЗагрузкуДанныхПослеВопросаОПодтвержденииПолученияДанных(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если Объект.ДанныеУспешноЗагружены Тогда
		НСтрока = НСтр("ru = 'Данные уже были получены. Выполнить получение данных повторно?'; en = 'Data has already been received. Receive the data again?'");
...
Процедура ВыполнитьЗагрузкуДанныхПослеПодтвержденияПолученияДанных()
	...
	Состояние(НСтр("ru = 'Выполняется получение данных. Пожалуйста, подождите...'; en = 'Retrieving data. Please wait...'"));
...
Процедура ВыполнитьПроверкуЗаданияПользовательскихПолей(Отказ, ПользовательскиеПоля)
	...
	Если ПользовательскиеПоля.Количество() = 0 Тогда
		...
		НСтрока = НСтр("ru = 'Следует указать хотя бы одно поле для отображения'; en = 'One should specify at least one field to display'");
	...
	ИначеЕсли ПользовательскиеПоля.Количество() > МаксимальноеКоличествоПользовательскихПолей() Тогда
		...
		СтрокаСообщения = НСтр("ru = 'Уменьшите количество полей (можно выбирать не более %1 полей)'; en = 'Reduce the number of fields (you can select no more than %1 fields)'");
...
Процедура УстановитьСвязьИнтерактивно()
	...
	Если Не (ТекущиеДанные.СтатусСопоставления=-1 Или ТекущиеДанные.СтатусСопоставления=+1) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Объекты уже сопоставлены'; en = 'Objects already mapped'"), 2);
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да,     НСтр("ru='Установить'; en = 'Map'"));
	...
	ТекстВопроса = НСтр("ru='Установить соответствие между выбранными объектами?'; en = 'Set the match between the selected objects?'");
...
Функция ПолучитьОбъектДляСопоставления(Данные)
	...
	Если Результат.Количество() = 0 Тогда
		...
		Результат.Добавить(НСтр("ru = '<не задан>'; en = '<not specified>'"));
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(Отказ)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Сопоставление объектов'; en = 'Object mapping'");
...
Процедура ЗафиксироватьОшибку(ПодробноеПредставлениеОшибки)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Помощник сопоставления объектов.Анализ данных'; en = 'Object mapping assistant.Data analysis'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
