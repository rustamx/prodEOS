...
Процедура Применить(Команда)
	...
	Если МассивОтмеченныхЭлементовСписка.Количество() = 0 Тогда
		...
		НСтрока = НСтр("ru = 'Следует указать хотя бы одно поле'; en = 'Select one or more fields'");
	...
	ИначеЕсли МассивОтмеченныхЭлементовСписка.Количество() > МаксимальноеКоличествоПользовательскихПолей() Тогда
		...
		СтрокаСообщения = НСтр("ru = 'Уменьшите количество полей (можно выбирать не более [КоличествоПолей] полей)'; en = 'Reduce the number of fields (you can select no more than [КоличествоПолей] fields)'");
