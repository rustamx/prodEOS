...
Процедура ЗавершитьСеанс(Команда)
	...
	Если КоличествоВыделенныхСтрок = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Не выбраны пользователи для завершения сеансов.'; en = 'Users to complete the sessions are not selected.'"));
		Возврат;
	ИначеЕсли КоличествоВыделенныхСтрок = 1 Тогда
		Если Элементы.СписокПользователей.ТекущиеДанные.Сеанс = НомерСеансаИнформационнойБазы Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Невозможно завершить текущий сеанс. Для выхода из программы можно закрыть главное окно программы.'; en = 'Cannot terminate the current session. To exit the program, you can close the main application window.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		Если ЗапрашиватьПараметрыАдминистрированияИБ Тогда
			...
			ЗаголовокФормы = НСтр("ru = 'Завершение сеанса'; en = 'Session terminating'");
			ПоясняющаяНадпись = НСтр("ru = 'Для завершения сеанса необходимо ввести параметры
				|администрирования кластера серверов'; en = 'To end a session, you must enter the server cluster administration options'");
...
Процедура ОткрытьЖурналРегистрации()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Выберите пользователей для просмотра журнала регистрации.'; en = 'Select users for the log view.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПослеЗавершенияСеанса(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если ДополнительныеПараметры.Свойство("НомераСеансов")
			...
			ТекстОповещения = НСтр("ru = 'Сеансы %1 завершены.'; en = 'Sessions %1 terminated.'");
			...
			ПоказатьОповещениеПользователя(НСтр("ru = 'Завершение сеансов'; en = 'Sessions terminating'"),, ТекстОповещения);
		...
		Иначе
			...
			ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сеанс %1 завершен.'; en = 'Session %1 terminated.'"), НомерСеанса);
			ПоказатьОповещениеПользователя(НСтр("ru = 'Завершение сеанса'; en = 'Session terminating'"),, ТекстОповещения);
