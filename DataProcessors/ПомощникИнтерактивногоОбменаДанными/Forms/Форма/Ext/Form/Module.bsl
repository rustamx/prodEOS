...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.Свойство("КодУзлаИнформационнойБазы", КодУзлаИнформационнойБазы) Тогда
		...
		Если Объект.УзелИнформационнойБазы.Пустая() Тогда
			...
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru = 'Настройка обмена данными не найдена.'; en = 'Data exchange settings item not found.'"), Отказ);
			Возврат;
	...
	Иначе
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru = 'Непосредственное открытие помощника не предусмотрено.'; en = 'Direct opening of the assistant is not supported.'"), Отказ);
		Возврат;
	...
	Если Не ОбменДаннымиПовтИсп.ЭтоУзелУниверсальногоОбменаДанными(Объект.УзелИнформационнойБазы) Тогда
		...
		ОбменДаннымиСервер.СообщитьОбОшибке(
			НСтр("ru = 'Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.'; en = 'For the selected node it is impossible to synchronize data with additional settings.'"), Отказ);
		Возврат;
	...
	Если МассивУзлов.Найти(Объект.УзелИнформационнойБазы) = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Для выбранного узла сопоставление данных не предусмотрено.'; en = 'For the selected node it is impossible to map data.'"),,,, Отказ);
		Возврат;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ИзменитьЗаголовок Тогда
		Элементы.ЗаголовокСтраницы.Заголовок = НСтр("ru='Параметры подключения'; en = 'Connection parameters'");
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ТекущаяСтраница = Элементы.СтраницаНачала Тогда
		ТекстПодтверждения = НСтр("ru='Отменить синхронизацию данных?'; en = 'Cancel data synchronization?'");
	Иначе
		ТекстПодтверждения = НСтр("ru='Прервать синхронизацию данных?'; en = 'Terminate data synchronization?'");
	КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхДляСтроки(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		НСтрока = НСтр("ru = 'Выберите имя таблицы в поле статистической информации.'; en = 'Select the name of the table in the field of statistical information.'");
	...
	Если ЕстьНесопоставленныеОбъекты Тогда
		НСтрока = НСтр("ru = 'Имеются несопоставленные объекты.
		                     ...
		                     |en = 'There are unmapped objects.
		                     |After data import, duplicates of unmapped objects will be created. Continue?'");
...
Процедура ОткрытьФормуСопоставления(Команда)
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для типа данных нельзя выполнить сопоставление объектов.'; en = 'Cannot perform data mapping for this object type.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДополнениеВыгрузкиОчисткаОбщегоОтбора(Команда)
	...
	ТекстЗаголовка = НСтр("ru='Подтверждение'; en = 'Confirmation'");
	ТекстВопроса   = НСтр("ru='Очистить общий отбор?'; en = 'Clear common filter?'");
...
Процедура ДополнениеВыгрузкиОчисткаДетальногоОтбора(Команда)
	ТекстЗаголовка = НСтр("ru='Подтверждение'; en = 'Confirmation'");
	ТекстВопроса   = НСтр("ru='Очистить детальный отбор?'; en = 'Clear detailed filter?'");
...
Процедура ДополнениеВыгрузкиИсторияОтборов(Команда)
	...
	Текст = НСтр("ru='Сохранить текущую настройку...'; en = 'Save current settings...'");
...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		НСтр("ru='Восстановить настройки ""%1""?'; en = 'Restore settings ""%1""?'"), ПредставлениеНастройки);
...
Процедура ВыполнитьЗагрузкуДанныхДляСтрокиПродолжение(Знач ВыделенныеСтроки) 
	...
	Состояние(НСтр("ru = 'Выполняется загрузка данных...'; en = 'Importing data...'"));
	...
	Если Отказ Тогда
		НСтрока = НСтр("ru = 'При загрузке данных возникли ошибки.
		                     ...
		                     |en = 'Errors occurred while importing data.
		                     |Show event log?'");
	...
	Состояние(НСтр("ru = 'Загрузка данных завершена.'; en = 'Data import completed.'"));
КонецПроцедуры
...
Процедура ОткрытьКаталогОбменаИнформациейУзла()
	...
	Если ПустаяСтрока(ИмяКаталога) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не задан каталог обмена информацией.'; en = 'Information exchange directory is not specified.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = НСтр("ru = 'Расширение для работы с файлами не установлено, открытие каталога не возможно.'; en = 'Extension for work with files is not installed, opening the directory is not possible.'");
...
Процедура ПослеОпределенияСуществованияФайла(Существует, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Каталог ""%1"" не существует или к нему нет доступа.'; en = 'Directory ""%1"" does not exist or cannot be accessed.'"),
...
Процедура ОпределитьЭтоКаталог(ЭтоКаталог, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '""%1"" является файлом, а не каталогом.'; en = '""%1"" is a file, not a directory.'"),
			ДополнительныеПараметры.ИмяКаталога);
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура ВидТранспортаСообщенийОбменаПриИзмененииНаСервере()
	...
	Если ОбменЧерезПодключениеККорреспонденту Тогда
		...
		Иначе
			...
			ВызватьИсключение НСтр("ru = 'Заданный сценарий синхронизации данных не поддерживается.'; en = 'The specified data synchronization scenario is not supported.'");
...
Процедура ОбновитьСтраницыНастроекТранспорта()
	...
	Иначе
		...
		СписокВыбораТранспорта.Добавить(Неопределено, НСтр("ru='подключение не настроено'; en = 'connection is not configured'") );
...
Процедура УстановитьОписаниеДополненияОбщегоОтбора()
	...
	Если НетОтбора Тогда
		Текст = НСтр("ru='Все документы'; en = 'All documents'");
	КонецЕсли;
...
Процедура УстановитьОписаниеДополненияДетально()
	...
	НетОтбора = ПустаяСтрока(Текст);
	Если НетОтбора Тогда
		Текст = НСтр("ru='Дополнительные данные не выбраны'; en = 'Additional data is not selected'");
	КонецЕсли;
...
Процедура ОбновитьОтображениеЭлементовСтатусаОбменаДанными()
	...
	Если Элементы.СтраницыСтатусаЗагрузкиДанных.ТекущаяСтраница=Элементы.СтатусЗагрузкиНеопределено Тогда
		Элементы.ПерейтиВЖурналРегистрацииСобытийЗагрузкиДанных.Заголовок = НСтр("ru='Загрузка данных не произведена'; en = 'Data import not performed'");
	Иначе
	...
	Если Элементы.СтраницыСтатусаВыгрузкиДанных.ТекущаяСтраница=Элементы.СтатусВыгрузкиНеопределено Тогда
		Элементы.ПерейтиВЖурналРегистрацииСобытийВыгрузкиДанных.Заголовок = НСтр("ru='Выгрузка данных не произведена'; en = 'Data export was not performed'");
	Иначе
...
Функция Подключаемый_СтраницаНачала_ПриПереходеДалее(Отказ)
	...
	Если Объект.УзелИнформационнойБазы.Пустая() Тогда
		...
		НСтрока = НСтр("ru = 'Укажите узел информационной базы'; en = 'Specify the infobase node'");
	...
	ИначеЕсли Объект.ВидТранспортаСообщенийОбмена.Пустая() Тогда
		...
		НСтрока = НСтр("ru = 'Укажите вариант подключения'; en = 'Specify a connection method'");
	...
	ИначеЕсли ОбменЧерезВебСервис И ПустаяСтрока(WSПароль) Тогда
		...
		НСтрока = НСтр("ru = 'Не указан пароль.'; en = 'No password specified.'");
...
Функция Подключаемый_СтраницаОжиданияПроверкиПодключения_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если ОбменЧерезВебСервис Тогда
		...
		Если Отказ Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Не удалось выполнить операцию.'; en = 'Failed to perform operation.'"));
...
Функция Подключаемый_СтраницаОжиданияАнализаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru = 'Получение сообщения обмена во временный каталог'; en = 'Receiving exchange message to a temporary directory'");
	...
	Если ДлительнаяОперацияЗавершена Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Объект.ИмяФайлаСообщенияОбмена) Тогда
				...
				ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
				ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru = 'Получение файла с данными сообщения обмена во временный каталог'; en = 'Receiving the file with exchange message data to a temporary directory'");
...
Процедура ЗавершениеПолученияДанныхВоВременныйКаталогНаСервере();
	...
	ШаблонСообщенияОбОшибке = НСтр("ru = 'Не удалось выполнить загрузку данных. Подробности см. в журнале регистрации'; en = 'Failed to import data. For details, see event log'");
...
Функция Подключаемый_АнализДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru = 'Анализ данных сообщения обмена'; en = 'Analysis of data exchange message'");
...
Процедура ЗавершениеАнализаДанныхНаСервере()
	...
	Если ЗафиксироватьОшибку Тогда
		...
		ТекстСообщения = ?(ЗначениеЗаполнено(СообщениеОбОшибке), СообщениеОбОшибке,
			СтрШаблон(НСтр("ru = 'Не удалось загрузить данные из ""%1"" (этап анализа данных).'; en = 'Failed to import data from ""%1"" (stage of data analysis).'"), Объект.УзелИнформационнойБазы));
...
Функция Подключаемый_СтраницаИнформацииСтатистики_ПриПереходеДалее(Отказ)
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да,  НСтр("ru = 'Продолжить'; en = 'Continue'"));
...
Функция Подключаемый_ЗагрузкаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru = 'Загрузка данных из сообщения обмена'; en = 'Data import from exchange message'");
...
Функция Подключаемый_СтраницаОжиданияВыгрузкиДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru = 'Выгрузка данных из сообщения обмена'; en = 'Exporting data from exchange message'");
...
Процедура ЗавершениеВыгрузкиНаСервере();
	...
	Если РезультатВыполненияМетода.Отказ
		И Не ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
		СообщениеОбОшибке = НСтр("ru = 'Не удалось выполнить отправку данных. Подробности см. в журнале регистрации'; en = 'Failed to export data. For details, see events log'");
	КонецЕсли;
