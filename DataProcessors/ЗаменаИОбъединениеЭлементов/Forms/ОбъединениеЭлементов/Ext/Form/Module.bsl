...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ru = 'Прервать объединение элементов и закрыть форму?'; en = 'Interrupt merging items and close the form?'");
...
Процедура МестаИспользованияПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Удалить из списка для объединения элемент ""%1""?'; en = 'Remove from list for merge item ""%1""?'"),
...
Функция ПроверитьОбъединяемыеСсылки(Знач НаборСсылок)
	...
	Если КоличествоСсылок < 2 Тогда
		Результат.Ошибка = НСтр("ru = 'Для объединения необходимо указать несколько элементов.'; en = 'Select more then one item to be merged'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Контроль.ЕстьГруппы Тогда
		Результат.Ошибка = НСтр("ru = 'Один из объединяемых элементов является группой.
		                              |Группы не могут быть объединены.'; en = 'One of the merged items is a group. Groups cannot be merged.'");
	ИначеЕсли Контроль.КоличествоВладельцев > 1 Тогда 
		Результат.Ошибка = НСтр("ru = 'У объединяемых элементов различные владельцы.
		                              |Такие элементы не могут быть объединены.'; en = 'The merged items have a different owners. Such items cannot be merged.'");
	ИначеЕсли Контроль.КоличествоСсылок <> КоличествоСсылок Тогда
		Результат.Ошибка = НСтр("ru = 'Все объединяемые элементы должны быть одного типа.'; en = 'All merged items must be of the same type.'");
	Иначе 
		// Все нормально
	...
	Если ЕстьПравоБезвозвратногоУдаления Тогда
		Если ТекущийВариантУдаления = "Пометка" Тогда
			ТекстПодсказки = НСтр("ru = 'Элементы (%1) будут <a href = ""ПереключениеРежимаУдаления"">помечены на удаление</a> и заменены во всех местах
				|использования на ""%2"" (отмечен стрелкой).'; en = 'Items (%1) will be <a href = ""DeletionModeSwitching"">marked for deletion</a> and replaced in all places the use of the ""%2"" (indicated by arrow).'");
		Иначе
			ТекстПодсказки = НСтр("ru = 'Элементы (%1) будут <a href = ""ПереключениеРежимаУдаления"">удалены безвозвратно</a> и заменены во всех местах
				|использования на ""%2"" (отмечен стрелкой).'; en = 'Items (%1) will be <a href = ""DeletionModeSwitching"">deleted permanently</a> and replaced by ""%2"" (indicated by arrow).'");
		КонецЕсли;
	Иначе
		ТекстПодсказки = НСтр("ru = 'Элементы (%1) будут помечены на удаление и заменены во всех местах
			|использования на ""%2"" (отмечен стрелкой).'; en = 'Items (%1) will be marked for deletion and replaced by ""%2"" (indicated by arrow).'");
	КонецЕсли;
...
Функция СообщениеОЗавершении()
	Возврат СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(
		...
		НСтр("ru = 'элемент объединен в:'; en = 'item merged into:'") + "," + НСтр("ru = 'элемента объединено в:'; en = 'elements merged in:'") + "," + НСтр("ru = 'элементов объединено в:'; en = 'elements merged in:'"));
КонецФункции
...
Процедура СформироватьНадписьНеудачныеЗамены()
	...
	Элементы.РезультатНеудачныеЗамены.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Объединение элементов не выполнено. В некоторых местах использования не может быть произведена
		           |автоматическая замена на ""%1""'; en = 'Merging items failed. In some places cannot be made automatic replacement on ""%1""'"),
		ОсновнойЭлемент);
...
Процедура ДобавитьСтрокиМестИспользования(Знач МассивСсылок)
	...
	Для Каждого Ссылка Из МассивСсылок Цикл
		...
		Если СуществующиеСтроки.Количество() = 0 Тогда
			...
			Строка.НеИспользуется    = НСтр("ru = 'Не рассчитано'; en = 'Not calculated'");
		Иначе
...
Процедура ИнициализироватьСценарийМастера()
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ru = 'Отказаться от объединения элементов'; en = 'Cancel item merging'");
	...
	КнопкиМастера.Далее.Заголовок = НСтр("ru = 'Объединить >'; en = 'Merge >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru = 'Начать объединение элементов'; en = 'Start item merging'");
	...
	КнопкиМастера.Далее.Видимость = Ложь;
	КнопкиМастера.Отмена.Заголовок = НСтр("ru = 'Прервать'; en = 'Terminate'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru = 'Прервать объединение элементов'; en = 'Abort item merging'");
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ru = 'Закрыть результаты объединения'; en = 'Close merging results'");
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Заголовок = НСтр("ru = '< В начало'; en = '< To the beginning'");
	КнопкиМастера.Назад.Подсказка = НСтр("ru = 'Вернутся к выбору основного элемента'; en = 'Return to the main item selection'");
	КнопкиМастера.Далее.КнопкаПоУмолчанию = Истина;
	КнопкиМастера.Далее.Заголовок = НСтр("ru = 'Повторить'; en = 'Retry'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru = 'Повторить объединение'; en = 'Retry merging'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ru = 'Отмена'; en = 'Cancel'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru = 'Закрыть результаты объединения'; en = 'Close merging results'");
...
Процедура ШагВыборОсновногоЭлементаПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	НастройкиДиалога.Вставить("Заголовок", НСтр("ru = 'Невозможно объединить элементы'; en = 'Cannot merge items'"));
...
Процедура ДобавитьШагМастера(Знач Страница, Знач Действия, Знач Кнопки)
	...
	ОписаниеШага.КнопкаДалее.Вставить("Заголовок", НСтр("ru='Далее >'; en = 'Next >'"));
	
	ОписаниеШага.Вставить("КнопкаНазад", Новый Структура(ПоляКнопки, Истина, Истина, Ложь));
	ОписаниеШага.КнопкаНазад.Вставить("Заголовок", НСтр("ru='< Назад'; en = '< Back'"));
	
	ОписаниеШага.Вставить("КнопкаОтмена",Новый Структура(ПоляКнопки, Истина, Истина, Ложь));
	ОписаниеШага.КнопкаОтмена.Вставить("Заголовок", НСтр("ru='Отмена'; en = 'Cancel'"));
...
Функция ЗапуститьФоновоеЗадание()
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ОпределитьМестаИспользования" Тогда
		...
		НаименованиеМетода = НСтр("ru = 'Поиск и удаление дублей: Определение мест использования'; en = 'Search and deletion of duplicates: Definition of usage instances'");
	...
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ЗаменитьСсылки" Тогда
		...
		НаименованиеМетода = НСтр("ru = 'Поиск и удаление дублей: Объединение элементов'; en = 'Search and deletion of duplicates: Merging items'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Кратко = НСтр("ru = 'При замене элементов возникла ошибка:'; en = 'Error occurred when item replacement:'") + Символы.ПС + Задание.КраткоеПредставлениеОшибки;
...
Процедура ЗаполнитьМестаИспользования(Знач АдресРезультата)
	...
	Для Каждого Строка Из ТаблицаИспользования Цикл
		...
		СтрокаИспользования.НеИспользуется = ?(Мест = 0, НСтр("ru = 'Не используется'; en = 'Not used'"), "");
	КонецЦикла;
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Объединение элементов %1 в один'; en = 'Merging %1 items into single one'"), Представление);
КонецПроцедуры
