...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Заголовок = НСтр("ru = 'Групповое изменение реквизитов'; en = 'Batch modification of attributes'");
	КонецЕсли;
...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		...
		Элементы.ФормаИзменить.Заголовок = НСтр("ru = 'Выполнить'; en = 'Execute'");
	...
	Иначе
		...
		Элементы.ФормаИзменить.Заголовок = НСтр("ru = 'Изменить реквизиты'; en = 'Change attributes'");
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Изменить" Тогда
		Если КоличествоОбъектовДляОбработки = 0 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Не указаны элементы для изменения'; en = 'You have not specified the elements to change'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			ТекстВопроса = НСтр("ru = 'Отбор не задан. Изменить все элементы?'; en = 'The selection is not specified. Change all items?'");
			ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьИзменениеПроверкаОтбораВыполнена", ЭтотОбъект);
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , НСтр("ru = 'Изменение элементов'; en = 'Changing items'"));
		КонецЕсли;
...
Процедура ВыполнитьИзменениеПроверкаОтбораВыполнена(РезультатВопроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЕстьНастроенныеИзменения() И Объект.ТипОперации = "ВвестиЗначения" Тогда
		ТекстВопроса = НСтр("ru = 'Изменения не настроены. Выполнить перезапись элементов без изменений?'; en = 'Changes are not configured. Overwrite elements unchanged?'");
...
Процедура ВыполнитьДействияПриКонтекстномОткрытии()
	...
	ШаблонЗаголовка = НСтр("ru = 'Изменение выделенных элементов ""%1"" (%2)'; en = 'Changing selected items ""%1"" (%2)'");
	...
	Если ТипыОбъектов.Количество() > 1 Тогда
		ПредставлениеТипа = "";
		ШаблонЗаголовка = НСтр("ru = 'Изменение выделенных элементов (%2)'; en = 'Changing selected items (%2)'");
	КонецЕсли;
...
Процедура ВопросПерейтиКРазблокированиюРеквизитов()
	...
	ТекстВопроса = НСтр("ru = 'Реквизит заблокирован, перейти к разблокированию реквизитов?'; en = 'Attribute is locked, go to unlocking the attributes?'");
...
Процедура УстановитьКнопкиНаВремяИзменения(НачалоИзменения)
	...
	Если НачалоИзменения Тогда
		Элементы.ФормаИзменить.Заголовок = НСтр("ru = 'Прервать'; en = 'Terminate'");
	Иначе
		Если ОбъектыКоторыеНеУдалосьИзменить.Количество() > 0 Тогда
			Элементы.ФормаИзменить.Заголовок = НСтр("ru = 'Повторить изменение'; en = 'Redo change'");
		Иначе
...
Процедура ИзменитьОбъекты()
	...
	Если Объект.ИзменятьВТранзакции Тогда
		...
		Если ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
			...
			ПоказатьОповещениеПользователя(НСтр("ru = 'Изменение выделенных элементов'; en = 'Changing selected items'"), ,НСтр("ru = 'Пожалуйста подождите, обработка может занять некоторое время...'; en = 'Please wait, processing may take some time...'"));
	...
	Иначе
		...
		Состояние(НСтр("ru = 'Обрабатываются элементы...'; en = 'Processing items...'"), 0, НСтр("ru = 'Изменение выделенных элементов'; en = 'Changing selected items'"));
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	Пока ПродолжитьОбработку Цикл
		...
		Если Объект.ИзменятьВТранзакции Тогда
			ТекстПредупреждения = НСтр("ru = 'При изменении элементов обнаружены ошибки - изменения отменены.'; en = 'Errors were detected when changing items - changes cancelled.'");
		...
		ТекстВопроса = НСтр("ru = 'При изменении элементов (группы элементов) возникли ошибки.
			...
			|';
			|en = 'There were some errors when changing items (items group).
			|Abort changing items and go to errors overview?'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Прервать, НСтр("ru = 'Прервать'; en = 'Terminate'"));
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Прервать, НСтр("ru = 'Ошибки при изменении элементов'; en = 'Error when changing items'"));
		Возврат;
	КонецЦикла;
	...
	Если ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных Тогда
		...
		Состояние(НСтр("ru = 'Обрабатываются элементы...'; en = 'Processing items...'"), ТекущийПроцент, НСтр("ru = 'Изменение выделенных элементов'; en = 'Changing selected items'"));
	КонецЕсли;
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Изменение реквизитов элементов'; en = 'Changing attributes of items'"), , 
			ПодставитьПараметрыВСтроку(НСтр("ru = 'Изменены элементы (%1).'; en = 'Changed items (%1).'"), ТекущееСостояниеИзменения.КоличествоИзмененных));
	...
	Если ОбработкаЗавершена Тогда
		ШаблонСообщения = НСтр("ru = 'Изменения выполнены во всех выбранных элементах (%2).'; en = 'The changes complete on all selected items (%2).'");
	Иначе
		Если Объект.ИзменятьВТранзакции Или ТекущееСостояниеИзменения.КоличествоИзмененных = 0 Тогда
			ШаблонСообщения = НСтр("ru = 'Изменения не выполнены.'; en = 'The changes are not implemented.'");
		Иначе
			ШаблонСообщения = НСтр("ru = 'Изменения выполнены частично.
										|Изменено: %1; Не удалось изменить: %3';
										|en = 'The changes are partly complete.
										|Modified: %1; Cannot change: %3'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Объект.ИзменятьВТранзакции И Не ОбработкаЗавершена Тогда
		...
		Если КоличествоПропущенных > 0 И Не ТекущееСостояниеИзменения.ПрерватьИзменение Тогда
			...
			СтрокаТаблицы.Объект = ПодставитьПараметрыВСтроку(НСтр("ru = '... и другие элементы (%1)'; en = '... and other elements (%1)'"), КоличествоПропущенных);
			СтрокаТаблицы.Причина = НСтр("ru = 'Пропущены, так как не были изменены один или более элементов.'; en = 'Skipped because it does not have changed one or more elements.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура НазадСервер()
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		Элементы.ФормаИзменить.Заголовок = НСтр("ru = 'Выполнить'; en = 'Execute'");
		Элементы.ФормаИзменить.РасширеннаяПодсказка.Заголовок = НСтр("ru = 'Выполнить алгоритм'; en = 'Run the algorithm'");
	Иначе
		Элементы.ФормаИзменить.Заголовок = НСтр("ru = 'Изменить реквизиты'; en = 'Change attributes'");
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуВсеГотово()
	...
	Элементы.НадписьУспешноЗавершено.Заголовок = ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Реквизиты выбранных элементов были изменены.
			...
			|Total modified elements: %1'"), ТекущееСостояниеИзменения.КоличествоИзмененных);
...
Функция ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	Иначе
		НаименованиеЗадания = НСтр("ru = 'Групповое изменение элементов'; en = 'Batch modification of items'");
...
Процедура СформироватьПояснениеПоАвтонумерации()
	...
	ИначеЕсли Автонумерация Тогда
		ПояснениеПоАвтонумерации = НСтр("ru = '<Установить автоматически>'; en = '<Assign automatically>'");
	Иначе
...
Функция ОписанияКолонокТаблицыРеквизитов()
	...
	ОписаниеКолонки.Тип = Новый ОписаниеТипов("Строка");
	ОписаниеКолонки.Представление = НСтр("ru = 'Реквизит'; en = 'Attribute'");
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Если Не ПустаяСтрока(Пояснение) Тогда
		Пояснение = Пояснение + " " + НСтр("ru = 'в выбранных элементах'; en = 'in selected items'");
	КонецЕсли;
	...
	Для Каждого ТабличнаяЧасть Из ИзменяемыеТабличныеЧасти Цикл
		...
		КонецЕсли;
		Пояснение = Пояснение + " " 
			+ ПодставитьПараметрыВСтроку(НСтр("ru = 'в табличной части ""%1""'; en = 'in the tabular section of the ""%1""'"), ТабличнаяЧасть.Ключ);
	КонецЦикла;
	
	Если Не ПустаяСтрока(Пояснение) Тогда
		Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
			Если ЕстьОтборПоСтрокам Тогда 
				Пояснение = Пояснение + " " + НСтр("ru = 'в тех строках выбранных элементов, которые удовлетворяют условиям отбора'; en = 'on the lines of the selected items that meet the selection criteria'")
			Иначе
				Пояснение = Пояснение + " " + НСтр("ru = '<b>во всех строках</b> выбранных элементов'; en = '<b>in all the rows</b> of the selected items'")
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьВыбранныеОбъекты Тогда
		...
		Иначе
			Пояснение = НСтр("ru = 'Выполнить <b>перезапись</b> выбранных элементов.'; en = 'Execute <b>overwrite</b> the selected items.'");
		КонецЕсли;
	Иначе
		Пояснение = НСтр("ru = 'Не выбраны элементы, реквизиты которых необходимо изменить.'; en = 'You have not selected any items for which you want to change the attributes.'");
	КонецЕсли;
...
Процедура ОбновитьНадписьКоличествоВыбранных()
	...
	Если ЕстьНастроенныеОтборы() Тогда
		...
		ТекстНадписи = ЧислоЦифрамиПредметИсчисленияПрописью(КоличествоВыбранныхОбъектов, НСтр("ru = 'элемент,элемента,элементов'; en = 'item,items,items'"));
	Иначе
...
Процедура УстановитьНастройкуИзменения(Знач Настройка)
	...
	Если ЕстьЗаблокированные Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Некоторые реквизиты заблокированы для изменения, изменения не установлены.'; en = 'Some attributes are locked for changes, changes are not assigned.'"));
	КонецЕсли;
...
Функция ВидОбъектаПоТипу(Тип)
	...
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru='Неверный тип значения параметра (%1)'; en = 'Invalid parameter type (%1)'"), Строка(Тип));
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты)
	...
	ИначеЕсли ТипЗнч(Реквизиты) = Тип("Массив") Или ТипЗнч(Реквизиты) = Тип("ФиксированныйМассив") Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru = 'Неверный тип второго параметра Реквизиты: %1'; en = 'Wrong type of the second parameter Реквизиты: %1'"), Строка(ТипЗнч(Реквизиты)));
	КонецЕсли;
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru = 'Общий модуль ""%1"" не найден.'; en = 'Common module ""%1"" not found.'"), Имя);
	КонецЕсли;
#Иначе
	Модуль = Вычислить(Имя);
...
Функция ПроверитьВерсиюИРежимСовместимостиПлатформы()
	...
	Если Не (Лев(Информация.ВерсияПриложения, 3) = "8.3"
		...
		ВызватьИсключение НСтр("ru = 'Обработка предназначена для запуска на версии платформы
			|1С:Предприятие 8.3 с отключенным режимом совместимости или выше'; en = 'Data processor is designed to run on the version of the Platform 1C:Enterprise 8.3 with compatibility mode off or higher'");
