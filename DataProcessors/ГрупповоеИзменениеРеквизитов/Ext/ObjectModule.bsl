...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		НоваяКоманда.Представление = НСтр("ru = 'Групповое изменение реквизитов'; en = 'Batch modification of attributes'");
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя)
	...
	ИначеЕсли ВРег(КлассОМ) = "РЕГИСТРРАСЧЕТА" Тогда
		...
		Иначе
			...
			Иначе
				ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru = 'Неизвестный тип объекта метаданных ""%1""'; en = 'Unknown metadata object type for ""%1""'"), ПолноеИмя);
			КонецЕсли;
		КонецЕсли;
