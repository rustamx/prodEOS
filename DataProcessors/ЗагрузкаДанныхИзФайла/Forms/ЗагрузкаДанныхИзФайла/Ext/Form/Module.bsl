...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение(НСтр("ru = 'Недостаточно прав для открытия загрузки данных из файла'; en = 'Insufficient privileges to open the data loading from a file'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = НСтр("ru = 'Введенные данные не будут записаны.'; en = 'The entered data will not be saved. Close the form?'");
		Возврат;
	КонецЕсли;
		
	Если Отказ Тогда
		Оповещение = Новый ОписаниеОповещения("ЗакрытиеФормыЗавершение", ЭтотОбъект);
		ТекстВопроса = НСтр("ru = 'Введенные данные не будут записаны. Закрыть форму?'; en = 'The entered data will not be saved. Close the form?'");
...
Процедура ОтменитьСопоставление(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Отменить сопоставление?'; en = 'Cancel mapping?'"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура Назад(Команда)
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ЗаполнениеТаблицыДанными Тогда
		...
		ЭтотОбъект.Заголовок = НСтр("ru = 'Загрузка данных в справочник'; en = 'Loading data into catalog'");
		ОчиститьТаблицу();
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных ИЛИ Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.НеНайдено Тогда
		...
		Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда 
			Элементы.Далее.Заголовок = НСтр("ru = 'Вставить в список'; en = 'Insert into a list'");
		Иначе
...
Процедура ПереходКСледующемуШагуЗагрузкиДанных()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.РезультатыСопоставления Тогда
		...
		Элементы.Далее.Заголовок = НСтр("ru = 'Вставить в список'; en = 'Insert into a list'");
		Элементы.Далее.КнопкаПоУмолчанию = Истина;
		Элементы.Назад.Заголовок = НСтр("ru = '< В начало'; en = '< To the beginning'");
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных Тогда
		...
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			Если Строки.Количество() > 0 Тогда
				...
				ПоказатьВопрос(Оповещение, НСтр("ru = 'Строки, в которых не заполнены обязательные колонки, будут пропущены.'; en = 'Rows that are not filled in the required column, will be ignored.'")
					+ Символы.ПС + НСтр("ru = 'Продолжить?'; en = 'Do you want to continue?'"), РежимДиалогаВопрос.ДаНет);
				Возврат;
			КонецЕсли;
			...
			Закрыть(АдресЗагруженныхДанных);
		ИначеЕсли ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
			Элементы.Назад.Заголовок = НСтр("ru = '< В начало_'; en = '< To the beginning_'");
			ЗакрытьФормуИВернутьМассивСсылок();
		Иначе
...
Процедура ВыполнитьСопоставление()
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Выполнено сопоставление'; en = 'Mapped completed'"),, НСтр("ru = 'Сопоставлено элементов:'; en = 'Mapped elements:'") + " " + Строка(КоличествоСопоставленныхПоКолонкам));
	ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
КонецПроцедуры
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	ТекстВсе = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Все (%1)'; en = 'All (%1)'"), Статистика.Всего);
	
	Элементы.СоздаватьЕслиНеСопоставлено.Заголовок = НСтр("ru = 'Несопоставленные ('; en = 'Unmapped ('") + Статистика.НеСопоставленных + ")";
	Элементы.ОбновлятьСуществующие.Заголовок = НСтр("ru = 'Сопоставленные элементы ('; en = 'Mapped elements ('") + Строка(Статистика.Сопоставленных) + ")";
	
	СписокВыбора = Элементы.ФильтрТаблицаСопоставления.СписокВыбора;
	СписокВыбора.Очистить();
	СписокВыбора.Добавить("Все", ТекстВсе, Истина);
	СписокВыбора.Добавить("Несопоставленные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Несопоставленные (%1 из %2)'; en = 'Unmapped (%1 of %2)'"),
		Статистика.НеСопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Сопоставленные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Сопоставленные (%1 из %2)'; en = 'Mapped (%1 of %2)'"),
		Статистика.Сопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Неоднозначные", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Неоднозначные (%1 из %2)'; en = 'Ambiguous (%1 of %2)'"),
	...
	Если Статистика.Неоднозначных > 0 Тогда 
		...
		Элементы.ОписаниеНеоднозначность.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(неоднозначностей: %1)'; en = '(ambiguities: %1)'"),
...
Процедура ВыгрузитьШаблонВФайлЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогВыбораФайла.Фильтр                      = НСтр("ru='Книга Excel 97 (*.xls)|*.xls|Книга Excel 2007 (*.xlsx)|*.xlsx|Электронная таблица OpenDocument (*.ods)|*.ods|Текстовый документ c разделителями (*.csv)|*.csv|Табличный документ (*.mxl)|*.mxl'; en = 'Excel 97 workbook (*.xls)| *.xls| Excel 2007 workbook (*.xlsx) | *.xlsx | OpenDocument spreadsheet (*.ods) | *.ods | Text document delimited (*.csv)| *.csv | Table document (*.mxl)| *.mxl'");
...
Процедура ИнициализацияВставкиИзБуфераОбмена()
	...
	Если Параметры.Свойство("ПредставлениеПоля") Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Вставка из буфера обмена (%1)'; en = 'Paste from the Clipboard (%1)'"), Параметры.ПредставлениеПоля);
	Иначе
		Заголовок = НСтр("ru = 'Вставка из буфера обмена'; en = 'Paste from the Clipboard'");
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовФормы()
	ЭтотОбъект.Заголовок = ?(ПустаяСтрока(Параметры.Заголовок), НСтр("ru = 'Загрузка данных в справочник'; en = 'Loading data into catalog'"), Параметры.Заголовок);
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		...
		| (например: Microsoft Office Excel, OpenOffice Calc и др.).'; en = 'The choice of catalog for loading data from spreadsheets, located in external files (for example, Microsoft Office Excel, OpenOffice Calc, etc.).'");
		ВидЗагрузкиДанных = 1;
	КонецЕсли;
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Элементы.Закрыть.Заголовок = НСтр("ru = 'Отмена'; en = 'Cancel'");
	ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
		Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ЗаполнениеТаблицыДанными;
		Элементы.ГруппаНастройкиСопоставления.Видимость = Ложь;
		Элементы.СписокКолонокСопоставления.Видимость = Ложь;
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка данных в справочник ""%1""'; en = 'Load data into catalog ""%1""'"), ПредставлениеСправочника(ИмяОбъектаСопоставления));
...
Процедура СопоставитьЗагружаемыеДанные()
	...
	Если ТаблицаСДаннымиПустая() Тогда
		Если ВариантЗагрузки = 0 Тогда
			ПоказатьПредупреждение(, (НСтр("ru ='Для сопоставления и загрузки данных, необходимо заполнить таблицу.'; en = 'For mapping and data loading, you must fill in the table.'")));
		Иначе
			...
			|en = 'You cannot perform a comparison of data, because the data has not been loaded into the table. 
			|Perhaps the names of the columns in the file do not match the columns in the design grid.'")));
		КонецЕсли;
		ВыполнитьШагЗаполнениеТаблицыДаннымиНаКлиенте();
	...
	Если СписокНезаполненныхКолонок.Количество() > 0 Тогда
		Если СписокНезаполненныхКолонок.Количество() = 1 Тогда
			ТекстПроКолонки = НСтр("ru = 'Обязательная колонка""'; en = 'Required column ""'") + " " + СписокНезаполненныхКолонок[0]
				+ НСтр("ru = '"" содержит незаполненные строки, эти строки будут пропущены при загрузке.'; en = '"" contains blank rows, those rows will be skipped when loading.'");
		Иначе
			ТекстПроКолонки = НСтр("ru = 'Обязательные колонки""'; en = 'Required columns ""'") + " " + СтрСоединить(СписокНезаполненныхКолонок,", ")
				+ НСтр("ru = '"" содержат незаполненные строки, эти строки будут пропущены при загрузке.'; en = '"" contain empty rows, those rows will be skipped when loading.'");
		КонецЕсли;
		ТекстПроКолонки = ТекстПроКолонки + Символы.ПС + НСтр("ru = 'Продолжить?'; en = 'Continue?'");
...
Процедура ПоказатьОтчет(АдресРезультата)
	...
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("ВсеЭлементы", НСтр("ru = 'Все ('; en = 'All ('") + Отчет.Всего + ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Новые", НСтр("ru = 'Новые ('; en = 'New ('") + Отчет.Создано+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Обновленные", НСтр("ru = 'Обновленные ('; en = 'Updated ('") + Отчет.Обновлено+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Пропущенные", НСтр("ru = 'Пропущенные ('; en = 'Missed ('") + Отчет.Пропущено+ ")");
...
Процедура ВывестиСообщениеОбОшибке(ТекстОшибкиДляПользователя, ТехническаяИнформация)
	ТекстСообщенияОбОшибке = ТекстОшибкиДляПользователя + Символы.ПС
		+ НСтр("ru = 'Возможная причина: Загружаемые данные некорректные.
					...
					|en = 'Possible cause: the data being loaded is incorrect.
					|Technical data: %1'");
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	...
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Тогда
			ТекстНайдено = НСтр("ru = 'Из %1 введенных строк в список будут вставлены: %2.'; en = 'From %1 entered strings will be inserted to list: %2.'");
			...
			Если Статистика.Неоднозначных > 0 И Статистика.НеНайдено > 0 Тогда 
				ТекстНеНайдено = НСтр("ru = '%3 строк будут пропущены:'; en = '%3 rows will be skipped:'") + Символы.ПС + "  - " + НСтр("ru = 'Нет данных в программе: %1'; en = 'No data: %1'") 
					+ Символы.ПС + "  - " +НСтр("ru = 'Несколько вариантов для вставки: %2'; en = 'Several options to insert: %2'");
				ТекстНеНайдено = НСтр("ru = 'Будет пропущено строк: %3
				|  - Нет данных в программе: %1
				|  - Несколько вариантов для вставки: %2';
				|en = 'Strings will be ignored : %3
				|-no data: %1
				|-several options to insert: %2'");
			...
			ИначеЕсли Статистика.Неоднозначных > 0 Тогда
				ТекстНеНайдено = НСтр("ru = 'Строки, для которых в программе имеется несколько вариантов, будут пропущены: %1'; en = 'Rows for which the program has several options will be ignored: %1'");
			...
			ИначеЕсли Статистика.НеНайдено > 0 Тогда
				ТекстНеНайдено = НСтр("ru = 'Строки, для которых в программе нет соответствующих данных, будут пропущены: %1'; en = 'Rows for which relevant data are not available in the program, will be ignored: %1'");
				ТекстНеНайдено = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстНеНайдено, Статистика.НеНайдено);
			КонецЕсли;
			ТекстНеНайдено = ТекстНеНайдено + Символы.ПС + НСтр("ru = 'Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".'; en = 'For viewing the skipped rows and data selection to insert, click the ""Next"" button.'");
			...
			Элементы.Далее.Видимость = Истина;
			Элементы.Назад.Заголовок = НСтр("ru = '< Назад'; en = '< Back'");
			Элементы.Далее.Заголовок = НСтр("ru = 'Далее >'; en = 'Next >'");
			...
			ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПоискаСсылок, Ложь, НСтр("ru = 'Далее >'; en = 'Next >'"));
		Иначе
			Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.НеНайдено;
			Элементы.Закрыть.Заголовок = НСтр("ru = 'Закрыть'; en = 'Close'");
			Элементы.Назад.Видимость = Истина;
			Элементы.ВставитьВСписок.Видимость = Ложь;
	...
	Иначе 
		...
		Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Истина, Элементы.ПояснениеДляСопоставленияДанных, Истина, НСтр("ru = 'Загрузить данные >'; en = 'Load data >'"));
		ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
			Отбор = Новый Структура("РезультатСопоставленияСтроки", "НеСопоставлен");
			...
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляТабличнойЧасти, Истина, НСтр("ru = 'Загрузить данные'; en = 'Load data'"));
			УстановитьОформлениеДляПолейСНеоднозначностью(Новый Структура("РезультатСопоставленияСтроки", "Неоднозначность"));
		ИначеЕсли ТипЗагрузки = "ВнешняяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, НСтр("ru = 'Загрузить данные >'; en = 'Load data >'"));
		Иначе
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, НСтр("ru = 'Загрузить данные >'; en = 'Load data >'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьОформлениеДанных(СписокКолонок = Неопределено)
	...
	Если ЗначениеЗаполнено(СписокКолонок) Тогда
		Для каждого ИмяКолонки Из СписокКолонок Цикл
			...
			ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<неоднозначность>'; en = '<ambiguity>'"));
		КонецЦикла;
	Иначе
		ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
		ПолеОформления = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
...
Функция МенеджерОбъекта(ИмяОбъектаСопоставления)
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Объект ""%1"" не найден'; en = 'Object ""%1"" was not found'"), ИмяОбъектаСопоставления);
		КонецЕсли;
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	...
	Если Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница = Элементы.СтраницаВариантЗагрузкаИзФайла Тогда
		...
		|en = 'To load the data, it is necessary to keep the form in the file to fill in another program. 
		...
		|• delimited text (.csv) 
		|• a tabular document (.mxl)'") + Символы.ПС;
	Иначе
		ТекстПодсказки = НСтр("ru = 'Для заполнения таблицы необходимо скопировать данные в таблицу из внешнего файла через буфер обмена.'; en = 'To populate a table you need to copy the data into a table from an external file via Clipboard.'") + Символы.ПС;
	КонецЕсли;
	...
	Если ОбязательныеКолонки.Количество() > 0 Тогда 
		...
		Если ОбязательныеКолонки.Количество() = 1 Тогда
			ТекстПодсказки = ТекстПодсказки + НСтр("ru = 'Колонка, обязательная для заполнения:'; en = 'The column required for filling:'") + " " + СписокКолонок;
		Иначе
			ТекстПодсказки = ТекстПодсказки + НСтр("ru = 'Колонки, обязательные для заполнения:'; en = 'Columns are required for filling:'") + " " + СписокКолонок;
		КонецЕсли;
...
Процедура ДобавитьСтандартныеКолонкиВТаблицуСопоставления(ВременнаяТЗ, СтруктураОбъектаСопоставления, ДобавитьИдентификатор,
	...
	Если ДобавитьИдентификатор Тогда 
		ВременнаяТЗ.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("Число"), НСтр("ru = 'п/п'; en = '№'"));
	КонецЕсли;
	...
	Если ДобавитьРезультатСопоставленияСтроки Тогда 
		ВременнаяТЗ.Колонки.Добавить("РезультатСопоставленияСтроки", Новый ОписаниеТипов("Строка"), НСтр("ru = 'Результат'; en = 'Result'"));
	КонецЕсли;
	Если ДобавитьОписаниеОшибки Тогда
		ВременнаяТЗ.Колонки.Добавить("ОписаниеОшибки", Новый ОписаниеТипов("Строка"), НСтр("ru = 'Причина'; en = 'Reason'"));
	КонецЕсли;
		...
		Если СтрНачинаетсяС(Колонка.Имя, "ТЧ_") Тогда
			...
			Если КолонкиГруппировки.Количество() = 1 Тогда
				...
				Если КолонкаУровень2 <> Неопределено Тогда 
					...
					Если ТипКолонки <> Неопределено И СтрНайти(ТипКолонки.ПолноеИмя(), "Справочник") > 0 Тогда
						НовыйЭлемент.Заголовок = НСтр("ru = 'Данные из файла'; en = 'the data from the file'");
					Иначе
			...
			ИначеЕсли КолонкиГруппировки.Количество() > 1 Тогда
				...
				Префикс = НСтр("ru = 'Данные из файла:'; en = 'The data from the file:'");
	...
	Если Результат <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВыбранныйФайл.Имя) Тогда
			...
			Иначе
				...
				Иначе
					ПоказатьПредупреждение(, НСтр("ru = 'Шаблон файла не был сохранен.'; en = 'File template was not saved.'"));
					Возврат;
				КонецЕсли;
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru = 'Подсистема ЗагрузкаДанныхИзФайла: Выполнение серверного метода загрузка данных из файла'; en = 'ЗагрузкаДанныхИзФайла subsystem: performing server method to load data from a file'");
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru = 'Заполнение таблицы сопоставления загруженными данными из файла.'; en = 'Populating a table comparing the loaded data from a file.'");
...
Процедура ПослеСопоставленияЗагружаемыхДанных(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ru = 'Не удалось произвести сопоставление данных.'; en = 'Data mapping failed.'"),
...
Функция ЗаписатьЗагружаемыеДанныеОтчетУниверсальнаяЗагрузка()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Запись загруженных данных из файла'; en = 'Recording data loaded from a file'");
...
Процедура ПослеЗаписиЗагружаемыхДанныхОтчет(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке(НСтр("ru = 'Не удалось произвести запись данных.'; en = 'Failed to write data.'"), ФоновоеЗадание.КраткоеПредставлениеОшибки);
...
Функция СформироватьОтчетОЗагрузке(ТипОтчета = "ВсеЭлементы",  РассчитыватьПроцентПрогресса = Ложь)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Создание отчета о загрузке данных из файла'; en = 'Creating a report about loading data from a file'");
