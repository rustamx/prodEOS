...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДаннымиЧерезВебСервисВДвухБазах(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ru = 'При создании настройки обмена во второй информационной базе возникли ошибки.'; en = 'There were errors while creating exchange settings in the second infobase.'");
		КонецЕсли;
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ru = 'Возникла ошибка при создании настройки синхронизации данных.'; en = 'An error occurred when creating the data synchronization settings.'"));
		КонецЕсли;
...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДаннымиЧерезВнешнееСоединение(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			Сообщение = НСтр("ru = 'При создании настройки обмена во второй информационной базе возникли ошибки: %1'; en = 'There were errors while creating exchange settings in the second infobase: %1'");
...
Процедура ОбновитьНастройкиОбменаДанными(Отказ,
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ru = 'Возникла ошибка при обновлении настройки синхронизации данных.'; en = 'An error occurred while updating data synchronization settings.'"));
		КонецЕсли;
...
Процедура ВнешнееСоединениеВыполнитьДействияПоНастройкеНовогоОбменаДанными(Отказ, 
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ru = 'При создании настройки синхронизации данных возникли ошибки.'; en = 'There were errors while creating data exchange settings.'"));
		КонецЕсли;
	...
	Если ЭтоНастройкаРаспределеннойИнформационнойБазы
		...
		Если ГлавныйУзел = Неопределено Тогда
			...
			ВызватьИсключение НСтр("ru = 'Главный узел для текущей информационной базы не определен.
							...
							|en = 'Main node for the current infobase is not defined.
							|Perhaps the infobase is not a subordinate DIB node.'");
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуПараметровМастера(Отказ, СтрокаXML) Экспорт
	...
	Если СтруктураНастроек.Свойство("ИмяПланаОбмена")
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ru = 'Файл содержит настройки обмена для другой информационной базы.'; en = 'The file contains exchange settings for another infobase.'");
	...
	Если Не ПустаяСтрока(ПрефиксИнформационнойБазы)
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ru = 'На первом этапе настройки обмена был неправильно указан префикс второй информационной базы.
				...
				|en = 'Prefix of the second infobase was specified incorrectly on the first step of exchange setup.
				|It is necessary to repeat the setup.'");
		//
		ОбменДаннымиСервер.СообщитьОбОшибке(СтрокаСообщенияОбОшибке(), Отказ);
...
Функция СчитатьДанныеВСтруктуру(ЧтениеXML)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		...
		ВызватьИсключение НСтр("ru = 'Ошибка чтения XML'; en = 'XMLread error'");
