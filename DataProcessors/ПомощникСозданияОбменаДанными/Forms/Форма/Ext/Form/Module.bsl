...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
		...
		Элементы.ГруппаПереключателяВыбораРежимаРаботыПомощника.Заголовок = НСтр("ru = 'Продолжение настройки синхронизации данных с главным узлом'; en = 'Continuing to configure data synchronization with main node'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиент.ПредлагатьСоздаватьРезервныеКопии() Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Перед настройкой синхронизации рекомендуется сделать <a href = %1 >резервную копию данных</a>.'; en = 'It is recommended to create <a href = %1 >backup copy of data</a> before configuring synchronization.'"),
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		...
		Если ДлительнаяОперация Тогда
			ТекстПредупреждения = НСтр("ru = 'Настройка синхронизации данных будет прервана.'; en = 'Data synchronization setup will be terminated.'");
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		
		Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
			ТекстПредупреждения = НСтр("ru = 'Настройка подчиненного узла распределенной информационной базы не выполнена.'; en = 'Subordinate node setup is not completed.'");
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	Если ДлительнаяОперация Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выполняется настройка синхронизации данных.
		                                    ...
		                                    |en = 'Setting up data synchronization.
		                                    |The assistant cannot be finished right now.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ЭтоПродолжениеНастройкиВПодчиненномУзлеРИБ Тогда
		ТекстПредупреждения = НСтр("ru = 'Выполняется настройка подчиненного узла распределенной информационной базы.
		                                 ...
		                                 |en = 'Setting up subordinate node of distributed infobase. 
		                                 |Cancel setup and use default values?'");
	...
	ТекстПредупреждения = НСтр("ru = 'Отменить настройку синхронизации и выйти из помощника?'; en = 'Cancel synchronization setup and exit the assistant?'");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗакрытиеФормыСопоставленияОбъектов" Тогда
		...
		Состояние(НСтр("ru = 'Выполняется сбор информации сопоставления...'; en = 'Mapping information is being collected...'"));
		
		ОбновитьДанныеСтатистикиСопоставленияНаСервере(Отказ, Параметр);
		
		Если Отказ Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'При получении информации статистики возникли ошибки.'; en = 'Errors occurred while obtaining statistical information.'"));
		Иначе
			
			РазвернутьДеревоИнформацииСтатистики(Параметр.КлючУникальности);
			
			Состояние(НСтр("ru = 'Сбор информации завершен'; en = 'Data gathering completed'"));
		КонецЕсли;
...
Процедура ИмяФайлаНастроекОбменаДаннымиДляЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Фильтр", НСтр("ru = 'Файл настроек синхронизации данных (*.xml)'; en = 'Data synchronization settings file (*.xml)'") + "|*.xml" );
...
Процедура СохранитьФайлНастроекОбменаДанными(Команда)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'При сохранении файла настроек синхронизации данных возникли ошибки.'; en = 'There were errors while saving exchange settings to file.'"));
	...
	Иначе
		...
		#Иначе
			...
			ТекстПредложения = НСтр("ru = 'Для открытия каталога необходимо необходимо установить расширение работы с файлами.'; en = 'To open the directory it is necessary to install the extension for working with files.'");
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Диалог.Заголовок      = НСтр("ru = 'Укажите имя файла настроек синхронизации данных'; en = 'Specify the name of the file with data synchronization settings'");
...
Процедура ПорядковыйНомерПереходаПриИзменении(Знач ЭтоПереходДалее)
	...
	Если СтрокиПереходаТекущие.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Не определена страница для отображения.'; en = 'Page to be displayed is not specified.'");
	КонецЕсли;
...
Процедура ПроверитьПодключениеCOMЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если ПустаяСтрока(ТекстСообщения) Тогда
			ТекстСообщения = НСтр("ru = 'Проверка подключения успешно завершена.'; en = 'Connection test completed successfully.'");
		...
		Иначе
			...
			Если ЭтоОшибкаРегистрацииКомпоненты Тогда
				...
				ТекстРегистрацияКомпоненты = Символы.ПС
					+ НСтр("ru = 'Возможно, необходимо зарегистрировать COM-компоненту comcntr.dll'; en = 'You may need to register the COM component comcntr.dll'")
					+ ?(ЭтоФайловаяИБ, ".", НСтр("ru = ', после чего выполнить перезапуск сервера 1С:Предприятие.'; en = ', then restart 1C:Enterprise server.'"))
			КонецЕсли;
...
Процедура ПроверитьПодключениеWSЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если Не Отказ Тогда
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Подключение успешно установлено.'; en = 'Connection established successfully.'"));
...
Процедура ОбработчикОжиданияДлительнойОперации()
	...
	Иначе // Failed, Canceled
		...
		ТекстВопроса = НСтр("ru = 'При создании синхронизации данных возникли ошибки.
							...
							|en = 'There were errors while creating data exchange settings.
							|Show event log?'");
...
Процедура УстановитьВидимостьНаСервере()
	...
	Если Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы Тогда
		...
		Элементы.ГруппаПереключателяВыбораРежимаРаботыПомощника.Заголовок = НСтр("ru = 'Создание начального образа подчиненного узла РИБ'; en = 'Creating initial image of subordinate DIB node.'");
...
Функция ПредставлениеРезультатаТранспортСообщений()
	...
	Если НастройкаОбменаССервисом Тогда
		...
		|%1';
		|en = 'Connection parameters for application as a service:
		|%1'");
	...
	Иначе
		...
		Результат = Строка(Объект.ВидТранспортаСообщенийОбмена)
			+ НСтр("ru = ', параметры:'; en = ', parameters:'") + Символы.ПС
...
Функция ПолучитьОписаниеНастроекТранспортаОбмена()
	...
	Для Каждого Элемент Из ПредставленияНастроек Цикл
		...
		Если РежимРаботыМастера = "ОбменЧерезВнешнееСоединение" Тогда
			...
			Если Элемент.Ключ = "COMВариантРаботыИнформационнойБазы" Тогда
				...
				ЗначениеНастройки = ?(Объект[Элемент.Ключ] = 0, НСтр("ru = 'Файловый'; en = 'File mode'"), НСтр("ru = 'Клиент-серверный'; en = 'Client-server'"));
		...
		ИначеЕсли  Не ТипЗначения(ЗначениеНастройки, "Число")
			...
			ЗначениеНастройки = НСтр("ru = '<пусто>'; en = '<empty>'");
	...
	Если ПустаяСтрока(Результат) Тогда
		...
		Результат = НСтр("ru = 'Настройки подключения не заданы.'; en = 'Connection settings are not specified.'");
...
Процедура ПроверитьПодключение(ВидТранспорта)
	...
	Если Не Отказ Тогда
		...
		Если ВидТранспорта = ПредопределенноеЗначение("Перечисление.ВидыТранспортаСообщенийОбмена.FILE") Тогда
			ТекстПредупреждения = НСтр("ru = 'Запись данных в указанный каталог разрешена.'; en = 'Writing data to the specified directory is permitted.'");
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Подключение успешно установлено.'; en = 'Connection established successfully.'");
		КонецЕсли;
...
Процедура ПроверитьКорректностьПараметровПодключенияWS(Отказ)
	...
	Если ПустаяСтрока(Объект.WSURLВебСервиса) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите адрес приложения в Интернете.'; en = 'Specify the address of the application on the Web.'");
	...
	ИначеЕсли ПустаяСтрока(Объект.WSИмяПользователя) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите имя пользователя.'; en = 'Specify the user name.'");
	...
	ИначеЕсли ПустаяСтрока(Объект.WSПароль) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите пароль пользователя.'; en = 'Specify user password.'");
...
Процедура ПроверитьПодключениеWSНаКлиенте(Отказ, РасширеннаяПроверка = Ложь)
	...
	Если Отказ И ПредлагатьПерейтиВЖурналРегистрации Тогда
		...
		ТекстВопроса = НСтр("ru = 'Ошибка установки подключения.
			|Перейти в журнал регистрации?';
			|en = 'Error establishing connection. 
			|Show event log?'");
...
Процедура ПроверитьПодключениеWSНаСервере(Отказ, РасширеннаяПроверка, ПредлагатьПерейтиВЖурналРегистрации)
	...
	Если РасширеннаяПроверка Тогда
		...
		Если Не ПараметрыПриемника.ПланОбменаСуществует Тогда
			...
			Сообщение = НСтр("ru = 'Другая программа не предназначена для синхронизации с текущей.'; en = 'Another program is not designed to synchronize with this one.'");
...
Процедура ПроверитьЗаполнениеРеквизитовВФорме(Отказ, ИмяФормыДляПроверки, ПараметрыФормы, ИмяРеквизитаФормы)
	...
	Если Не ФормаНастройки.ПроверитьЗаполнение() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Необходимо задать обязательные настройки.'; en = 'Specify the mandatory settings.'"),,, ИмяРеквизитаФормы, Отказ);
...
Процедура ЗавершениеПервогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ)
	...
	Если Не Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы
		...
		НСтрока = НСтр("ru = 'Сохраните файл с настройками для другой программы'; en = 'Saved file with settings for another program'");
	...
	Иначе
		...
		Если ВыполнитьОбменДаннымиСейчас Тогда
			...
			Состояние(НСтр("ru = 'Выполняется отправка данных...'; en = 'Sending data...'"));
			...
			ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru = 'Выгрузка данных'; en = 'Data export'");
...
Процедура ЗавершениеВыгрузкиДанных()
	...
	ИначеЕсли РезультатВыполненияФоновогоЗадания.Статус = "Отменено" Тогда
		СообщениеОбОшибке = НСтр("ru = 'Действие отменено пользователем.'; en = 'Action cancelled by user.'");
	Иначе
		
		РезультатВыполнения = ПолучитьРезультатВыполненияФоновогоЗаданияНаСервере();
		
		Если РезультатВыполнения.Отказ Тогда
			СообщениеОбОшибке = НСтр("ru = 'Во время отправки данных возникли ошибки(см. журнал регистрации).'; en = 'Errors occurred while exporting data (see event log).'");
		КонецЕсли;
	...
	ДлительнаяОперация      = Ложь;
	Состояние(НСтр("ru = 'Отправка данных завершена'; en = 'Data export completed'"));
...
Процедура ЗавершениеВторогоЭтапаНастройкиОбменаЧерезОбычныеКаналыСвязи(Отказ, Знач ОткрыватьПослеЗакрытияТекущей)
	...
	Состояние(НСтр("ru = 'Выполняется создание настройки синхронизации данных'; en = 'Creating data synchronization settings.'"));
	
	НастроитьНовыйОбменДаннымиНаСервере(Отказ, НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле);
	
	Если Отказ Тогда
		
		ПоказатьПредупреждение(, НСтр("ru = 'При создании настройки синхронизации данных возникли ошибки.'; en = 'There were errors while creating data exchange settings.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуСопоставления()
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для этих данных нельзя выполнить сопоставление.'; en = 'Cannot perform mapping for the selected data.'"));
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При выполнении обработчика ПриПодключенииККорреспонденту произошла ошибка:%1%2'; en = 'When executing handler ПриПодключенииККорреспонденту error occurred:%1%2'"),
				Символы.ПС,
...
Функция НадписьДалееFTP()
	...
	Возврат НСтр("ru = 'Нажмите кнопку ""Далее"" для настройки подключения через FTP-ресурс.'; en = 'Click ""Next"" button to configure connection to an FTP resource.'");
...
Функция НадписьДалееEMAIL()
	...
	Возврат НСтр("ru = 'Нажмите кнопку ""Далее"" для настройки подключения по почте.'; en = 'Click ""Next"" button to configure connection to email server.'");
...
Функция НадписьДалееНастройки()
	...
	Возврат НСтр("ru = 'Нажмите кнопку ""Далее"" для настройки дополнительных параметров синхронизации данных.'; en = 'Click ""Next"" button to configure advanced data synchronization parameters.'");
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите наименование этой программы.'; en = 'Specify the name of the program.'");
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите наименование другой программы.'; en = 'Specify the name of another program.'");
	...
	Если ПустаяСтрока(Объект.ПрефиксИнформационнойБазыПриемника) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите существующий или желаемый префикс второй информационной базы.'; en = 'Specify the existing or desired prefix of the second infobase.'");
	...
	Если СокрЛП(Объект.ПрефиксИнформационнойБазыИсточника) = СокрЛП(Объект.ПрефиксИнформационнойБазыПриемника) Тогда
		...
		НСтрока = НСтр("ru = 'Префиксы информационных баз должны быть различными.'; en = 'Infobase prefixes must differ.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.ПрефиксИнформационнойБазыПриемника",, Отказ);
...
Функция Подключаемый_СтраницаМастераНастройкаПараметровДляВнешнегоСоединенияПервойБазы_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите наименование информационной базы.'; en = 'Specify the name of infobase.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеЭтойБазы",, Отказ);
		Возврат Неопределено;
	КонецЕсли;
...
Функция Подключаемый_СтраницаМастераНачало_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.ИмяФайлаНастроекСинхронизацииДанныхДляЗагрузки) Тогда
		ТекстОшибки = НСтр("ru = 'Выберите файл с настройками синхронизации данных'; en = 'Select data synchronization settings file'");
	...
	ТекстПредупреждения = НСтр("ru = 'Для передачи файла настроек синхронизации данных необходимо установить расширение для веб-клиента 1С:Предприятие.'; en = 'To transfer the data synchronization settings file, it is necessary to install the extension for working with files.'");
...
Процедура ЗавершениеВыбораФайлаНастроекОбменаДаннымиДляЗагрузки(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) Тогда
		...
		Если ОшибкаЗагрузкиПараметровМастера Тогда
			ТекстОшибки = НСтр("ru = 'Указан неправильный файл настроек синхронизации данных. Укажите корректный файл.'; en = 'Invalid data synchronization settings file. Specify the correct file.'");
		Иначе
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если НЕ    (Объект.ИспользоватьПараметрыТранспортаEMAIL
		...
		НСтрока = НСтр("ru = 'Не указаны параметры подключения для синхронизации данных.
						...
						|en = 'Connection parameters for data synchronization are not specified.
						|You must configure at least one connection option.'");
		//
...
Функция Подключаемый_СтраницаМастераИтогНастройкиОбмена_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если ВариантРаботыМастера = "НастроитьНовыйОбменДанными" Тогда
		...
		|en = '%1%2%3This infobase prefix: %4
		|Second infobase prefix: %5'");
	...
	Иначе
		...
		// Представление результата настройки обмена.
		СтрокаСообщения = НСтр("ru = '%1%2%3Префикс этой информационной базы: %4'; en = '%1%2%This infobase prefix: %4'");
...
Функция Подключаемый_СтраницаМастераИтогНастройкиОбмена_ПриОткрытии_ВнешнееСоединение(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если НастройкаОбменаССервисом Тогда
		...
		|en = '%1
		|Settings for this infobase:
		|========================================================
		|%2%3Infobase prefix: %4
		|
		|Settings for application used as a service:
		|========================================================
		|%5%Application prefix: %7'");
	...
	Иначе
		...
		|en = '%1 
		|Data synchronization settings for this program: 
		|======================================================== 
		|%2%3Infobase prefix: %4 
		|
		|Data synchronization settings for another program: 
		|======================================================== 
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхСозданиеНастройкиОбмена_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Возникли ошибки на этапе создания настройки синхронизации данных.
					...
					|en = 'There were errors on stage of creationg of data synchronization settings.
					|To solve them use events log.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхПолучениеСообщения_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Возникли ошибки на этапе анализа данных.
					|Для решения проблем воспользуйтесь журналом регистрации.';
					|en = 'There were errors on data analysis stage.
					|To solve them use event log.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхАвтоматическоеСопоставление_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Возникли ошибки на этапе анализа данных.'; en = 'There were errors on data analysis stage.'"));
...
Функция Подключаемый_СтраницаМастераСопоставлениеДанных_ПриПереходеДалее(Отказ)
	...
	Если Не ВсеДанныеСопоставлены Тогда
		...
		Сообщение = НСтр("ru = 'Не все данные сопоставлены. Наличие несопоставленных данных
							   ...
							   |Продолжить?';
							   |en = 'Not all the data are mapped. Unmapped data can lead to duplication
							   |of elements in catalogs.
							   |Continue?'");
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииСправочниковЗагрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru = 'Загрузка справочников из сообщения обмена'; en = 'Importing catalogs from the exchange message'");
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Возникли ошибки на этапе синхронизации справочников.
					|Для решения проблем воспользуйтесь журналом регистрации.';
					|en = 'There were errors on catalog synchronization stage.
					|To solve them use events log.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеСохраненияНастроек_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Возникли ошибки на этапе сохранения настроек.
					|Для решения проблем воспользуйтесь журналом регистрации.';
					|en = 'There were errors on stage of saving of data synchronization settings.
					|To solve them use events log.'"));
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Возникли ошибки на этапе синхронизации данных.
					|Для решения проблем воспользуйтесь журналом регистрации.';
					|en = 'There were errors on data synchronization stage.
					|To solve them use events log.'"));
...
Процедура ПроверитьНастройкиПараметровУчетаНаСервере(
	...
	Если Не НастройкиПараметровУчетаЗаданы Тогда
		...
		Если ПустаяСтрока(СообщениеОбОшибке) Тогда
			СообщениеОбОшибке = НСтр("ru = 'Не заданы параметры учета в этой программе.'; en = 'Accounting options for this program are not specified.'");
		КонецЕсли;
	...
	Если Не НастройкиПараметровУчетаКорреспондентаЗаданы Тогда
		...
		Если ПустаяСтрока(СообщениеОбОшибкеКорреспондента) Тогда
			СообщениеОбОшибкеКорреспондента = НСтр("ru = 'Не заданы параметры учета в приложении, расположенном в Интернете.'; en = 'Accounting options for the application on the Internet are not specified.'");
		КонецЕсли;
...
Процедура ПроверитьЗаданиеНастроекДляПервойБазы(Отказ, ТипСоединения = "ВебСервис")
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите наименование приложения в Интернете.'; en = 'Specify the name of the application on Web.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеВторойБазы",, Отказ);
...
Процедура ПроверитьЗаданиеНастроекДляВторойБазы(Отказ, ТипСоединения)
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите наименование программы.'; en = 'Specify program name.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.НаименованиеВторойБазы",, Отказ);
...
Процедура СтраницаМастераНастройкаПараметров_ПриПереходеДалееНаСервере(Отказ)
	...
	Если Не ПланыОбмена[Объект.ИмяПланаОбмена].НайтиПоКоду(ОбменДаннымиСервер.КодУзлаПланаОбменаСтрокой(Объект.ПрефиксИнформационнойБазыПриемника)).Пустая() Тогда
		...
		НСтрока = НСтр("ru = 'Значение префикса второй информационной базы не уникально.
			...
			|en = 'Prefix value of the second infobase is not unique.
			|Data synchronization settings for infobase with the same prefix already exist.
			|Change the value of the prefix or use the existing synchronization settings.'");
		//
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.ПрефиксИнформационнойБазыПриемника",, Отказ);
...
Процедура СтраницаМастераОжиданиеПроверкиПодключенияВнешнегоСоединения_ОбработкаДлительнойОперацииНаСервере(Отказ)
	...
	Если Объект.COMВариантРаботыИнформационнойБазы = 0 Тогда
		...
		Если ПустаяСтрока(Объект.COMКаталогИнформационнойБазы) Тогда
			...
			НСтрока = НСтр("ru = 'Укажите каталог информационной базы.'; en = 'Specify the infobase directory'");
	...
	Иначе
		...
		Если ПустаяСтрока(Объект.COMИмяСервера1СПредприятия) Тогда
			...
			НСтрока = НСтр("ru = 'Укажите имя кластера серверов.'; en = 'Specify the name of the server cluster.'");
		...
		ИначеЕсли ПустаяСтрока(Объект.COMИмяИнформационнойБазыНаСервере1СПредприятия) Тогда
			...
			НСтрока = НСтр("ru = 'Укажите имя информационной базы.'; en = 'Specify the name of the infobase.'");
	...
	Если Не ПланОбменаСуществует Тогда
		...
		Сообщение = НСтр("ru = 'Синхронизация данных с указанной программой не предусмотрена.'; en = 'Data synchronization with the specified program is not supported.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
		Возврат;
	...
	Если НРег(СтрокаСоединенияИнформационнойБазы()) = НРег(ВнешнееСоединение.СтрокаСоединенияИнформационнойБазы()) Тогда
		...
		Сообщение = НСтр("ru = 'Заданы настройки подключения к этой информационной базе.'; en = 'Connection settings for this infobase are specified.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
		Возврат;
...
Процедура ПроверитьНаличиеСуществующегоОбменаДаннымиСВторойБазой(Отказ)
	...
	Если Не ПустаяСтрока(КодУзла)
		...
		Сообщение = НСтр("ru = 'Синхронизация данных между программами уже была настроена ранее.'; en = 'Data synchronization between the programs has already been configured.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
