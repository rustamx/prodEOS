...
Процедура ДобавитьЗаписьВЖурналОбмена(Объект, Фабрика = Неопределено, ВидИнтеграции, Описание, URL = Неопределено, ЭтоОшибка, ЭтоОбъект1С, ЭтоЗагрузка = Истина, ИД = Неопределено, Request=Ложь, ОбъектXDTO=Неопределено) Экспорт	
	...
	Запись.Объект           = ?(ЭтоОбъект1С И Объект <> Неопределено, Объект.Ссылка, Неопределено);
...
Функция ВыполнитьСценарийЗагрузкиЗапросаПредложения(ОбъектXDTO, Error) Экспорт
	...
	Если Результат.Ошибка Тогда 
		...
		ДобавитьЗаписьВЖурналОбмена(Неопределено, ФабрикаXDTO, Перечисления.ра_ВидыИнтеграций.SRM_Purchases, "Ошибка создания договора: " + Результат.ТекстОшибки, , Истина, Истина, , , , ОбъектXDTO); 
	КонецЕсли;
...
Процедура СформироватьИОтправитьПодтверждениеПоЗагрузкеДоговора(ИсходныйОбъект, Договор, НомерДоговора, тзНоменклатура, СтатусЗагрузки, СтатусТекст) Экспорт
	...
	СтруктураНастроек.Вставить("Определение", Новый WSОпределения(ИмяВременногоФайла, "C_SO_EOSQUAL", "Qazwsx123"));
	...
	//ОбъектXDTO.PurchasingContract.SellerParty.InternalID.__content			= ИсходныйОбъект.PurchasingContract.SellerParty.InternalID;
	...
	Попытка          
		...
		ДобавитьЗаписьВЖурналОбмена(Договор, ТекФабрикаXDTO, Перечисления.ра_ВидыИнтеграций.SRM_Purchases, "Подтверждение о загрузке договора успешно отправлено", ТекСервисSRM.ТочкаПодключения.Местоположение, Ложь, Истина, , , Истина, ОбъектXDTO); 
	Исключение
		...
		ДобавитьЗаписьВЖурналОбмена(Договор, ТекФабрикаXDTO, Перечисления.ра_ВидыИнтеграций.SRM_Purchases, ТекстОшибки, ТекСервисSRM.ТочкаПодключения.Местоположение, Истина, Истина, , , Истина, ОбъектXDTO); 
	КонецПопытки;
...
Функция СоздатьОбновитьДоговор(ОбъектXDTO, Результат, Рамка = Ложь)
	...
	ВнешнийНомерДоговора = PurchasingContract.ExternalContractNumber; 
	
	Попытка
		Если PurchasingContract.SellerParty.InternalID.Свойства().Получить("__content") <> Неопределено  Тогда 
			GID_Поставщик = Число(PurchasingContract.SellerParty.InternalID.__content);
		Иначе
			GID_Поставщик = Число(PurchasingContract.SellerParty.InternalID);
		КонецЕсли;
	Исключение
		GID_Поставщик = 0;
	КонецПопытки;
	...
	// Загрузка доп. соглашений
	Если ЭтоДопСоглашение Тогда 
		
		НомерОсновногоДоговора = PurchasingContract.InitialContractRegistrationNumber;

		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ra_Dogovory.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.ra_Dogovory КАК ra_Dogovory
		|ГДЕ
		|	ra_Dogovory.VneshnijNomer = &НомерОсновногоДоговора";
		
		Запрос.УстановитьПараметр("НомерОсновногоДоговора", НомерОсновногоДоговора);
		...
		Иначе
			...
			Результат.Вставить("ТекстОшибки", "Загрузка дополнительного соглашения не произведена. Не найден основной договор");		
			Возврат Неопределено;
		КонецЕсли;
	...
	Если не ЗначениеЗаполнено(Выборка.Поставщик) Тогда 
		Результат.Вставить("Ошибка"		, Истина);
		Результат.Вставить("ТекстОшибки", "Загрузка спецификации рамочной заявки не произведена. Не найден поставщик по GID: " + Строка(GID_Поставщик));
	...
	Если не ЗначениеЗаполнено(Выборка.Покупатель) Тогда 
		Результат.Вставить("Ошибка"		, Истина);
		Результат.Вставить("ТекстОшибки", "Загрузка спецификации рамочной заявки не произведена. Не найден покупатель по GID: " + Строка(GID_Покупателя));
	...
	Если не ЗначениеЗаполнено(СсылкаНаДоговор) Тогда
		...
		Договор.VneshnijNomer = ВнешнийНомерДоговора;
	...
	Договор.Ramka	  = Рамка;
	Если PurchasingContract.TargetAmount.Свойства().Получить("__content") <> Неопределено Тогда 
		Договор.SummaSNDS = ТипЧисло15_2.ПривестиЗначение(PurchasingContract.TargetAmount.__content); 
	Иначе
		Договор.SummaSNDS = ТипЧисло15_2.ПривестиЗначение(PurchasingContract.TargetAmount); 
	КонецЕсли;
	Если PurchasingContract.Свойства().Получить("VATContractSum") <> Неопределено Тогда 
	...
	Если НЕ ЭтоДопСоглашение Тогда
		Если ТипЗнч(PurchasingContract.Item) = Тип("СписокXDTO") Тогда
			...
			Для Каждого Элемент Из PurchasingContract.Item Цикл	         
				...
				Если Элемент.Свойства().Получить("Product") <> Неопределено И Элемент.Product <> Неопределено Тогда
					
					Если Элемент.Product.InternalID.Свойства().Получить("__content") <> Неопределено И 
						ЗначениеЗаполнено(Элемент.Product.InternalID.__content) Тогда
						GID = ТипЧисло15.ПривестиЗначение(Элемент.Product.InternalID.__content);
					ИначеЕсли ЗначениеЗаполнено(Элемент.Product.InternalID) Тогда 
						GID = ТипЧисло15.ПривестиЗначение(Элемент.Product.InternalID);
					Иначе
						GID = 0;
					КонецЕсли;		
				...
				СтрокаСпецификацииИзSRM.НаименованиеМТР = Элемент.Description;	
				...
				СтрокаСпецификацииИзSRM.Количество = ТипЧисло15_3.ПривестиЗначение(Элемент.TargetQuantity);
				СтрокаСпецификацииИзSRM.ЦенаСНДС   = ТипЧисло15_2.ПривестиЗначение(Элемент.PriceIncludingVAT);
				СтрокаСпецификацииИзSRM.СуммаСНДС  = ТипЧисло15_2.ПривестиЗначение(Элемент.TargetAmount);
		...
		КонецЕсли;
	Иначе
		Договор.OsnovnojDogovor = ОсновнойДоговор;
		Договор.DopolnitelnoeSoglashenie = Истина; 		
	Конецесли;
    ...
    Попытка
		...
		ДобавитьЗаписьВЖурналОбмена(Договор.Ссылка, ФабрикаXDTO, Перечисления.ра_ВидыИнтеграций.SRM_Purchases, "Договор загружен успешно",, Ложь, Истина,,,,ОбъектXDTO); 
	Исключение
		ТекстОшибки = "Ошибка при записи договора загруженного из SRM: " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Результат.Вставить("Ошибка"		, Истина);
...
Процедура ВыполнитьСценарийЗагрузкиОбновлениеСтатусаВыгруженныхВSRMНесоответствий(ПакетXDTO) Экспорт 
	...
	Исключение
		ВызватьИсключение "Не удалось преобразовать GUID переданных объектов к формату ""XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX""";	
	КонецПопытки;
