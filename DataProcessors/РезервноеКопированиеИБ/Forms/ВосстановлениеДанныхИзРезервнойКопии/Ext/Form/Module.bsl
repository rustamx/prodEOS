...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ru = 'Прервать подготовку к восстановлению данных?'; en = 'Abort preparations for data recovery?'");
...
Процедура Готово(Команда)
	...
	СоединенияИБВызовСервера.УстановитьБлокировкуСоединений(НСтр("ru = 'Выполняется восстановление информационной базы.'; en = 'Infobase data is being restored'"), "РезервноеКопирование");
...
Процедура ВыбратьФайлРезервнойКопии()
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("ru = 'Архив с резервной копией (*.zip)|*.zip'; en = 'Backup archive (*.zip)|*.zip'");
	ДиалогОткрытияФайла.Заголовок= НСтр("ru = 'Выберите файл резервной копии'; en = 'Select the backup file'");
...
Функция ПроверитьЗаполнениеРеквизитов()
	...
	Если ПустаяСтрока(ИмяФайла) Тогда
		ТекстСообщения = НСтр("ru = 'Не выбран файл с резервной копией.'; en = 'Backup file is not selected.'");
	...
	Если ФайлАрхива.Расширение <> ".zip" Тогда
		...
		ТекстСообщения = НСтр("ru = 'Выбранный файл не является архивом с резервной копией.'; en = 'Selected file is not a backup archive.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
		Возврат Ложь;
	...
	Если ZipФайл.Элементы.Количество() <> 1 Тогда
		...
		ТекстСообщения = НСтр("ru = 'Выбранный файл не является архивом с резервной копией (содержит более одного файла).'; en = 'Selected file is not a backup archive (contains more than one file).'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
		Возврат Ложь;
	...
	Если ВРЕГ(ФайлВАрхиве.Расширение) <> "1CD" Тогда
		...
		ТекстСообщения = НСтр("ru = 'Выбранный файл не является архивом с резервной копией (не содержит файл информационной базы).'; en = 'Selected file is not a backup archive (does not contain infobase file).'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
		Возврат Ложь;
	...
	Если ВРЕГ(ФайлВАрхиве.ИмяБезРасширения) <> "1CV8" Тогда
		...
		ТекстСообщения = НСтр("ru = 'Выбранный файл не является архивом с резервной копией (неправильное имя файла информационной базы).'; en = 'Selected file is not a backup archive (incorrect infobase file name).'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
		Возврат Ложь;
...
Процедура ОтменитьПодготовку()
	...
	Элементы.НадписьНеУдалось.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1.
		|Подготовка к восстановлению данных из резервной копии отменена. Информационная база разблокирована.'; en = '%1. Preparing to restore data from backup canceled. Infobase is unlocked.'"),
...
Процедура НачатьРезервноеКопирование() 
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(РезервноеКопированиеИБКлиент.СобытиеЖурналаРегистрации(), 
		"Информация",
		НСтр("ru = 'Выполняется восстановление данных информационной базы:'; en = 'Infobase data is being restored:'") + " " + ИмяГлавногоФайлаСкрипта);
