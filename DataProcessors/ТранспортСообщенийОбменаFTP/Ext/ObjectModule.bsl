...
Функция ПолучитьСообщениеОбмена()
	...
	Если ТаблицаФайловСообщенийОбмена.Количество() = 0 Тогда
		...
		СтрокаСообщения = НСтр("ru = 'Каталог обмена информацией на сервере: ""%1""'; en = 'Information exchange directory on the server: ""%1""'");
		СтрокаСообщения = СтрШаблон(СтрокаСообщения, КаталогНаFTPСервере);
		ДополнитьСообщениеОбОшибке(СтрокаСообщения);
		
		СтрокаСообщения = НСтр("ru = 'Имя файла сообщения обмена: ""%1"" или ""%2""'; en = 'Name of exchange message file: ""%1"" or ""%2""'");
		СтрокаСообщения = СтрШаблон(СтрокаСообщения, ШаблонИмениФайлаСообщения + ".xml", ШаблонИмениФайлаСообщения + ".zip");
		ДополнитьСообщениеОбОшибке(СтрокаСообщения);
...
Процедура ИнициализацияСообщенийОшибок()
	...
	// Общие коды ошибок
	СообщенияОшибок.Вставить(001, НСтр("ru = 'В каталоге обмена информацией не был обнаружен файл сообщения с данными.'; en = 'Message file with the data was not found in the catalog for data exchange.'"));
	...
	СообщенияОшибок.Вставить(101, НСтр("ru = 'Не задан путь на сервере.'; en = 'Path on the server is not specified.'"));
	СообщенияОшибок.Вставить(102, НСтр("ru = 'Ошибка инициализации подключения к FTP-серверу.'; en = 'Error initializing connection to the FTP server.'"));
	СообщенияОшибок.Вставить(103, НСтр("ru = 'Ошибка подключения к FTP-серверу, проверьте правильность задания пути и права доступа к ресурсу.'; en = 'Unable to connect to the FTP server, check the path and permissions for the resource.'"));
	СообщенияОшибок.Вставить(104, НСтр("ru = 'Ошибка при поиске файлов на FTP-сервере.'; en = 'Error locating files on the FTP server.'"));
	СообщенияОшибок.Вставить(105, НСтр("ru = 'Ошибка при получении файла с FTP-сервера.'; en = 'Error retrieving file from the FTP server.'"));
	СообщенияОшибок.Вставить(106, НСтр("ru = 'Ошибка удаления файла на FTP-сервере, проверьте права доступа к ресурсу.'; en = 'Error deleting file from the FTP server. Check the permissions.'"));
...
Функция ВыполнитьУдалениеФайлаНаFTPСервере(Знач ИмяФайла, ПроверкаПодключения = Ложь)
	...
	Исключение
		...
		Если ПроверкаПодключения Тогда
			...
			|en = 'Unable to test connection using the test file ""%1"".
			|Perhaps, the specified directory does not exist or is not accessible.
			|It is also recommended that you consult the reference information for your FTP server to configure support for filenames with Cyrillic characters.'");
			СообщениеОбОшибке = СтрШаблон(СообщениеОбОшибке, ОбменДаннымиСервер.ИмяФайлаПроверкиПодключения());
