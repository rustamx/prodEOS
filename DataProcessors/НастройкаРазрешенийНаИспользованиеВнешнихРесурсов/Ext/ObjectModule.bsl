	...
	Если ТребуетсяПрименениеРазрешенийВКластереСерверов() Тогда
		...
		Исключение
			ВызватьИсключение НСтр("ru = 'Ошибка конкурентного доступа к настройке разрешений на использование внешних ресурсов.
                                ...
                                |en = 'Error of competitive access to configure permissions for the use of external resources.
                                |Try to repeat the operation later.'");
		КонецПопытки;
...
Функция НовоеОписаниеПрофиляБезопасности(Знач ТипПрограммногоМодуля, Знач ИдентификаторПрограммногоМодуля)
	...
	Шаблон = НСтр("ru = '[ИБ %1] %2 ""%3""'; en = '[IB %1] %2 ""%3""'");
	...
	Если ПустаяСтрока(ИмяИБ) Тогда
		ВызватьИсключение НСтр("ru = 'Строка соединения информационной базы не содержит имени информационной базы!'; en = 'The infobase connection string does not contain the infobase name.'");
	КонецЕсли;
	
	Если ТипПрограммногоМодуля = Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяИБ,
			НСтр("ru = 'Профиль безопасности для информационной базы'; en = 'A security profile for the information base'"), СтрокаСоединенияИнформационнойБазы());
	Иначе
