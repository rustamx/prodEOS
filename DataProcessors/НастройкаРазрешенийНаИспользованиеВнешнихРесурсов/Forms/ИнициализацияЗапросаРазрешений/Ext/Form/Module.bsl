...
Функция НачатьОбработкуЗапросов(Знач Запросы, Знач РежимВключения, РежимОтключения, Знач РежимВосстановления, Знач РежимПроверкиПрименения)
	...
	Задание = ФоновыеЗадания.Выполнить("РаботаВБезопасномРежиме.ВыполнитьМетодКонфигурации",
			...
			,
			НСтр("ru = 'Обработка запросов на использование внешних ресурсов...'; en = 'Processing of requests for the use of external resources.'"));
...
Функция ЗапросыОбработаны(Знач ИдентификаторЗадания)
	...
	Если Задание = Неопределено Тогда
		ВызватьИсключение(НСтр("ru = 'При обработке запросов произошла ошибка - не найдено задание обработки запросов.'; en = 'An error occurred while processing your query - query processing job was not found.'"));
	КонецЕсли;
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		...
		Иначе
			ВызватьИсключение(НСтр("ru = 'При обработке запросов произошла ошибка - задание обработки запросов завершилось с неизвестной ошибкой.'; en = 'An error occurred while processing request - query processing job failed with unknown error.'"));
		КонецЕсли;
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		ВызватьИсключение(НСтр("ru = 'При обработке запросов произошла ошибка - задание обработки запросов было отменено администратором.'; en = 'An error occurred while processing your query - query processing job was canceled by the administrator.'"));
	Иначе
...
Процедура ОтменитьОбработкуЗапросов(Знач ИдентификаторЗадания)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Настройка разрешений на использование внешних ресурсов.Отмена выполнения фонового задания'; en = 'Configuring permissions for external resources usage.Scheduled job cancelled'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
