...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.УзелИнформационнойБазы) Тогда
		Текст = НСтр("ru='Настройка обмена данными не найдена.'; en = 'Data exchange settings item not found.'");
...
Процедура ДополнительнаяРегистрацияПередУдалением(Элемент, Отказ)
	...
	Если Количество>1 Тогда
		ТекстПредставления = НСтр("ru='выбранные строки'; en = 'selected rows'");
	...
	ТекстВопроса = НСтр("ru='Удалить из дополнительных данных %1 ?'; en = 'Delete %1 from additional data?'");    
	...
	ИначеЕсли ТипВыбранного= Тип("Структура") Тогда
		Если ВыбранноеЗначение.ДействиеВыбора=3 Тогда
			...
			Если Не ПустаяСтрока(ПредставлениеТекущейНастройки) И ПредставлениеНастройки<>ПредставлениеТекущейНастройки Тогда
				ТекстВопроса  = НСтр("ru='Восстановить настройки ""%1""?'; en = 'Restore settings ""%1""?'");
...
Функция РедактированиеОтбораСтрокиДополнительныйСоставСервер(СтруктураВыбора)
	...
	ТекущиеДанные.Количество   = НСтр("ru='Не рассчитано'; en = 'Not calculated'");
...
Функция ДобавитьВДополнительныйСоставВыгрузки(Элемент)
	...
	Иначе
		...
		Строка.Количество = НСтр("ru='Не рассчитано'; en = 'Not calculated'");
		ОбновитьНадписьОбщегоКоличества();
	КонецЕсли;
...
Функция ОбновитьКоличествоСервер()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = 
		НСтр("ru='Расчет количества объектов для отправки при синхронизации'; en = 'Calculating the number of objects to be sent during the synchronization'");
...
Процедура ОбновитьНадписьОбщегоКоличества(Количество = Неопределено) 
	...
	Если Количество = Неопределено Тогда
		ТекстКоличества = НСтр("ru='<не рассчитано>'; en = '<not calculated>'");
	Иначе
		ТекстКоличества = НСтр("ru = 'Объектов: %1'; en = 'Objects: %1'");
...
Процедура СброситьНадписьКоличествТаблицы()
	ТекстКоличеств = НСтр("ru='Не рассчитано'; en = 'Not calculated'");
	Для Каждого Строка Из Объект.ДополнительнаяРегистрация Цикл
