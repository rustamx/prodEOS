...
Процедура ВариантыНастроекПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса   = НСтр("ru='Удалить настройку ""%1""?'; en = 'Delete settings ""%1""?'");
...
Процедура СохранитьНастройку(Команда)
	...
	Если ПустаяСтрока(ПредставлениеТекущейНастройки) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не заполнено имя для текущей настройки.'; en = 'Enter a name for the current settings.'"), , "ПредставлениеТекущейНастройки");
		Возврат;
	КонецЕсли;
		
	Если ВариантыНастроек.НайтиПоЗначению(ПредставлениеТекущейНастройки)<>Неопределено Тогда
		ТекстЗаголовка = НСтр("ru='Подтверждение'; en = 'Confirmation'");
		ТекстВопроса   = НСтр("ru='Перезаписать существующую настройку ""%1""?'; en = 'Overwrite the existing settings ""%1""?'");
