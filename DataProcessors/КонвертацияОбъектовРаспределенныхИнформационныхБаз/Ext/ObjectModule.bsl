		...
		Попытка
			...
			Если НайденыНеУникальныеЗаписи("Справочник.ИдентификаторыОбъектовМетаданных") Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					НСтр("ru = 'Перед загрузкой идентификаторов объектов метаданных
					           ...
					           |en = 'Found non-unique record in the catalog before importing metadata 
					           |object identifiers.'"));
			КонецЕсли;
			...
			Если Отказ Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Загрузка приоритетных данных не выполнена.
					           ...
					           |%1';
					           |en = 'Import of priority data failed.
					           |Found non-unique records when importing predefined items.
					           |Unable to proceed for the following reasons:
					           |%1'"),
					ОписаниеОтказа);
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ДублиПредопределенных) Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru = 'Предопределенные элементы.Найдены не уникальные записи.'; en = 'Predefined elements.Non-unique records are found.'",
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'При загрузке предопределенных элементов найдены не уникальные записи.
						           |%1';
						           |en = 'Non-unique records found while importing predefined items.
						           |%1'"),
						ДублиПредопределенных));
			КонецЕсли;
...
Процедура ДобавитьОписаниеДублейПредопределенного(ЗаписанныйОбъект, ДублиПредопределенных, Отказ, ОписаниеОтказа)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонОшибкиНеУникальностиЗаписей(),
				НСтр("ru = 'При загрузке предопределенных элементов найдены не уникальные записи.'; en = 'Non-unique records found while importing predefined items.'"));
		КонецЕсли;
	...
	Если ЗаписатьВЖурнал Тогда
		Если КоличествоДублей = 1 Тогда
			Шаблон = НСтр("ru = '(загружаемая ссылка: %1, ссылка дубля: %2)'; en = '(imported reference: %1, duplicate reference: %2)'");
		Иначе
			Шаблон = НСтр("ru = '(загружаемая ссылка: %1, ссылки дублей: %2)'; en = '(imported reference: %1, duplicate references: %2)'");
		КонецЕсли;
...
Функция ШаблонОшибкиНеУникальностиЗаписей()
	Возврат
		НСтр("ru = 'Загрузка приоритетных данных не выполнена.
		           ...
		           |en = 'Import of priority data failed.
		           |%1
		           |Infobase requires troubleshooting.
		           |1. Open the Configurator, go to the Administration menu, select ""Verify and repair... "".
		           |2. In the opened form select only the item ""Checking infobase logical integrity"";
		           |- Select ""Verify and repair"" rather than just ""Verify only"";
		           |- Click Execute.
		           |3. Then start 1C:Enterprise and repeat the synchronization.'");
КонецФункции
...
Функция ОшибкаОткрытияФайлаСообщенияОбмена()
	...
	Возврат НСтр("ru = 'Ошибка открытия файла сообщения обмена'; en = 'Error opening exchange message file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаНачалаЧтенияФайлаСообщенияОбмена()
	
	Возврат НСтр("ru = 'Ошибка при начале чтения файла сообщения обмена'; en = 'Error while starting reading exchange message file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаНачалаЗаписиФайлаСообщенияОбмена()
	
	Возврат НСтр("ru = 'Ошибка при начале записи файла сообщения обмена'; en = 'Error while starting writing exchange message file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаЧтенияФайлаСообщенияОбмена()
	
	Возврат НСтр("ru = 'Ошибка чтения файла сообщения обмена'; en = 'Error reading exchange message file'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ОшибкаЗаписиФайлаСообщенияОбмена()
	
	Возврат НСтр("ru = 'Ошибка записи данных в файл сообщения обмена'; en = 'Error writing message data to file'");
	
КонецФункции

Функция ОшибкаВидаОбменаДанными()
	
	Возврат НСтр("ru = 'Обмен не по правилам конвертации не поддерживается'; en = 'Exchange without conversion rules is not supported'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
