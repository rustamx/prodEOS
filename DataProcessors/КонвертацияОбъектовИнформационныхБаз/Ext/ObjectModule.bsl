...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяДействия = ?(ТолькоПолучитьУзелСсылки, НСтр("ru = 'Конвертация ссылки на объект'; en = 'Converting object reference'"), НСтр("ru = 'Конвертация объекта'; en = 'Object conversion'"));
		
		ТекстСообщения = НСтр("ru = '[ИмяДействия]: [Объект]([ТипОбъекта]), ПКО: [ПКО](НаименованиеПКО)'; en = '[ИмяДействия]: [Объект]([ТипОбъекта]), DCR: [ПКО](НаименованиеПКО)'");
	...
	Если ЕстьГлобальныйОбработчикПередКонвертациейОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(64, ОписаниеОшибки(), ПКО, Источник, НСтр("ru = 'ПередКонвертациейОбъекта (глобальный)'; en = 'BeforeObjectConversion (global)'"));
		КонецПопытки;
		...
		Если УзелСсылки = Неопределено Тогда
			...
			Если СтруктураСвойств.ИмяТипа = "Перечисление"
				...
				ЗП.Текст               = НСтр("ru = 'В правиле конвертации значений (ПКЗ) необходимо сопоставить значение Источника значению Приемника.
													...
													|en = 'Value conversion rule (VCR) must map the values of the source to these of the target.
													|If the target value does not exist, specify an empty value.'");
				//
...
Процедура ВыгрузитьДанныеПоПравилу(Правило) Экспорт
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("ru = 'ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: %1 (%2)'; en = 'DATA EXPORT RULE: %1 (%2)'");
...
Процедура ЗаписатьОбъектВИБ(Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		СтрокаСообщенияОбОшибке = НСтр("ru = 'Попытка изменения неразделенных данных (%1) в разделенном сеансе.'; en = 'Attempt to change unseparated data (%1) in a separated session.'");
...
Процедура ВыполнитьВыгрузкуДанных(ОбработкаДляЗагрузкиДанных = Неопределено) Экспорт
	...
	Если ЭтоОбменЧерезВнешнееСоединение() Тогда
		...
		Если ОбработкаДляЗагрузкиДанных().ФлагОшибки() Тогда
			...
			СтрокаСообщения = НСтр("ru = 'Ошибка в базе-корреспонденте: %1'; en = 'Error in correspondent infobase: %1'");
...
Процедура ВыполнитьЗагрузкуДанных() Экспорт
	...
	Попытка
		...
		Попытка
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("ru = 'Возникли ошибки при загрузке данных.'; en = 'Errors occurred while importing data.'");
			КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(ТаблицыДляЗагрузки) Экспорт
	...
	СтрокаСообщения = НСтр("ru = 'Начало процесса обмена данными для узла: %1'; en = 'Starting data exchange process for node: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	// Запись в журнале регистрации.
	СтрокаСообщения = НСтр("ru = '%1, %2; Обработано %3 объектов'; en = '%1, %2; Processed %3 objects'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьАнализСообщенияОбмена(ПараметрыАнализа = Неопределено) Экспорт
	...
	Попытка
		...
		Попытка
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("ru = 'Возникли ошибки при анализе данных.'; en = 'There were errors during data analysis.'");
			КонецЕсли;
...
Процедура ВнешнееСоединениеОбработчикКонвертацииПередЗагрузкойДанных(Отказ) Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ru = 'ПередЗагрузкойДанных (конвертация)'; en = 'BeforeDataImport (conversion)'"));
			Отказ = Истина;
		КонецПопытки;
...
Процедура ВнешнееСоединениеПослеЗагрузкиДанных() Экспорт
	...
	Если Не ФлагОшибки() Тогда
		...
		Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
			...
			Исключение
				ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ru = 'ПослеЗагрузкиДанных (конвертация)'; en = 'AfterDataImport (conversion)'"));
			КонецПопытки;
...
Функция ПолучитьИнформациюОПравилах(ЭтоПравилаКорреспондента = Ложь) Экспорт
	...
	Если ЭтоПравилаКорреспондента Тогда
		СтрокаИнформации = НСтр("ru = 'Правила конвертации корреспондента (%1) от %2'; en = 'Correspondent conversion rules (%1) of %2'");
	Иначе
		СтрокаИнформации = НСтр("ru = 'Правила конвертации этой информационной базы (%1) от %2'; en = 'Conversion rules from this infobase (%1) of %2'");
	КонецЕсли;
...
Функция ОткрытьФайлВыгрузки()
	...
	Исключение
		ПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при открытии файла для записи сообщения обмена.
				...
				|Описание ошибки:
				|%2';
				|en = 'Error opening file to write exchange message.
				|File name ""%1"".
				|Error description:
				|%2'"),
...
Процедура ИнициализироватьВедениеПротоколаОбмена()
	...
	Исключение
		ФайлПротоколаДанных = Неопределено;
		СтрокаСообщения = НСтр("ru = 'Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2'; en = 'Error occurred when writing data to exchange log file: %1. Error description: %2'",
...
Процедура ВосстановитьПравилаИзВнутреннегоФормата() Экспорт
	...
	Если ВерсияФорматаХраненияПравил <> ВерсияФорматаХраненияПравилОбмена() Тогда
		ВызватьИсключение НСтр("ru = 'Версия формата хранения правил обмена не соответствует ожидаемой.
			...
			|en = 'Rules storage format version does not match the expected one.
			|It is necessary to import exchange rules again.'");
	КонецЕсли;
...
Процедура НачатьЧтениеСообщения(ЧтениеСообщения, АнализДанных = Ложь)
	...
	Если ФайлОбмена.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		ВызватьИсключение НСтр("ru = 'Ошибка формата сообщения обмена.'; en = 'Exchange message format error.'");
	КонецЕсли;
	...
	Если ТаблицаПравилКонвертации.Количество() = 0 Тогда
		...
		Если ФлагОшибки() Тогда
			ВызватьИсключение НСтр("ru = 'При загрузке правил обмена данными возникли ошибки.'; en = 'Errors occurred while importing data exchange rules.'");
		КонецЕсли;
	Иначе
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("ru = 'ПередЗагрузкойДанных (конвертация)'; en = 'BeforeDataImport (conversion)'"));
		КонецПопытки;
		
		Если Отказ Тогда
			ВызватьИсключение НСтр("ru = 'Отказ от загрузки сообщения обмена в обработчике ПередЗагрузкойДанных (конвертация).'; en = 'Refusal from import of exchange message in hanlder ПередЗагрузкойДанных (conversion)'");
		КонецЕсли;
	...
	Если ФайлОбмена.ЛокальноеИмя = "ИнформацияОТипахДанных" Тогда
		...
		Иначе
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("ru = 'При загрузке информации о типах данных возникли ошибки.'; en = 'Errors occurred while importing information about types.'");
			КонецЕсли;
		КонецЕсли;
		ФайлОбмена.Прочитать();
	КонецЕсли;
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ru = 'Блокировка получения данных не может быть установлена в активной транзакции.'; en = 'Locking no data import cannot be set in an active transaction.'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|Подробности:
			|%1';
			|en = 'Error setting lock on data exchange.
			|Perhaps the exchange is being executed in another session.
			|
			|Details:
			|%1'"),
...
Процедура ВыполнитьОбработчикПослеЗагрузкиДанных()
	...
	Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("ru = 'ПослеЗагрузкиДанных (конвертация)'; en = 'AfterDataImport (conversion)'"));
		КонецПопытки;
	...
	Попытка
		...
		Если Отказ = Истина Тогда
			...
			Если Не ПустаяСтрока(ПричинаОтказа) Тогда
				...
				СтрокаСообщения = НСтр("ru = 'Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация) по причине: %1'; en = 'Refusal from import of exchange message in hanlder ПередЗагрузкойДанных (conversion) because: %1'");
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ПричинаОтказа);
				ВызватьИсключение СтрокаСообщения;
			Иначе
				ВызватьИсключение НСтр("ru = 'Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация).'; en = 'Refusal from import of exchange message in hanlder ПослеЗагрузкиПараметров (conversion)'");
			КонецЕсли;
	...
	Пока НомерВариантаПоиска <= 10
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(73, ОписаниеОшибки(), "", "", 
				ТипОбъекта, Неопределено, НСтр("ru = 'Последовательность полей поиска'; en = 'Search field sequence'"));
...
Процедура ЗагрузитьТабличнуюЧасть(Объект, ИмяТабличнойЧасти, ОбщаяИнформацияОТипеДокумента, ПараметрыОбъекта, ПКО)
	...
	Исключение
		...
		Текст = НСтр("ru = 'Имя табличной части: %1'; en = 'Tabular section name: %1'");
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("ru = 'Загрузка объекта № %1'; en = 'Importing object %1'");
...
Функция ПрочитатьНаборЗаписейРегистра()
	...
	Если ЕстьГлобальныйОбработчикПередЗагрузкойОбъекта Тогда
		...
		Исключение
			...
			ТипОбъекта, Неопределено, НСтр("ru = 'ПередЗагрузкойОбъекта (глобальный)'; en = 'BeforeImportObject (global)'"));
			...
			Если ЕстьГлобальныйОбработчикПослеЗагрузкиОбъекта Тогда
				...
				Исключение
					...
					ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(54, ОписаниеОшибки(), ИмяПравила, Источник,
						ТипОбъекта, Объект, НСтр("ru = 'ПослеЗагрузкиОбъекта (глобальный)'; en = 'AfterObjectImport (global)'"));
			...
			Если ИмяТипаОбъекта = "Документ" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Исключение
						...
						СтрокаСообщения = НСтр("ru = 'Ошибка при записи документа: %1. Описание ошибки: %2'; en = 'Error saving : %1. Error description: %2'");
...
Процедура ВыполнитьПереходНаНовыйОбмен()
	...
	СтрокаСообщения = НСтр("ru = 'Выполнен автоматический переход на синхронизацию данных через формат EnterpriseData.'; en = 'Performed automatic transition to data synchronization via EnterpriseData format.'");
	ЗаписьЖурналаРегистрацииОбменДанными(СтрокаСообщения, УровеньЖурналаРегистрации.Информация);
	ПолеРезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Отменено;
	ВызватьИсключение НСтр("ru = 'Синхронизация данных по старой настройке отменена.'; en = 'Data synchronization using old settings was cancelled.'");
КонецПроцедуры
...
Процедура ВызватьСобытияПередВыгрузкойОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяСобытия = НСтр("ru = 'ВыгрузкаОбъекта: %1'; en = 'Exporting object: %1'");
	...
	Если ЕстьГлобальныйОбработчикПередВыгрузкойОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(65, ОписаниеОшибки(), Правило.Имя, НСтр("ru = 'ПередВыгрузкойОбъектаВыборки (глобальный)'; en = 'BeforeExportSelectionObject (global)'"), Объект);
		КонецПопытки;
...
Процедура ВызватьСобытияПослеВыгрузкиОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ЕстьГлобальныйОбработчикПослеВыгрузкиОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(69, ОписаниеОшибки(), Правило.Имя, НСтр("ru = 'ПослеВыгрузкиОбъектаВыборки (глобальный)'; en = 'AfterSelectionObjectExport (global)'"), Объект);
		КонецПопытки;
	КонецЕсли;
...
Процедура ОтработатьУдалениеОбъекта(ДанныеОбУдаленииОбъекта, СтрокаСообщенияОбОшибке = "")
	...
	Если Конвертация.Свойство("ПередОтправкойИнформацииОбУдалении", ТекстСобытия) Тогда
		...
		Если Не ПустаяСтрока(ТекстСобытия) Тогда
			...
			Исключение
				СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(76, ОписаниеОшибки(), НСтр("ru = 'ПередОтправкойИнформацииОбУдалении (конвертация)'; en = 'BeforeSendDeletionInfo (conversion)'"));
...
Функция ПолучитьСтруктуруРеквизитовРегистрации(ТаблицаПКС)
	...
	Для Каждого ПКС Из МассивСтрокПКС Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными.Загрузка правил конвертации'; en = 'Data exchange.Loading conversion rules'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ВыполнитьПроверкуРеквизитовРегистрацииОбъектов(ТаблицаРеквизитовРегистрации)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРеквизитовРегистрации Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru = 'Тип объекта не определен: %1'; en = 'Object type is undefined: %1'");
			...
			Для Каждого Реквизит Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если ОбщегоНазначения.ЭтоЗадача(ОбъектМД) Тогда
					...
					Если НЕ (ОбъектМД.Реквизиты.Найти(Реквизит.Ключ) <> Неопределено
						...
						СтрокаСообщения = НСтр("ru = 'Неправильно указаны реквизиты шапки объекта ""%1"". Реквизит ""%2"" не существует.'; en = 'Incorrect header attributes of  object ""%1"". Attribute ""%2"" does not exist.'");
		...
		Иначе
			...
			Если МетаТаблица = Неопределено Тогда
				ЗаписатьВПротоколВыполнения(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Табличная часть (стандартная табличная часть, движения) ""%1"" объекта ""%2"" не существует.'; en = 'Tabulat section (standard tabular section, records) ""%1"" of object ""%2"" does not exist.'"),
			...
			Для Каждого Реквизит Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если Не РеквизитНайденВТабличнойЧастиРеквизитовРегистрацииОбъектов(МетаТаблица, Реквизит.Ключ) Тогда
					ЗаписатьВПротоколВыполнения(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Реквизит ""%3"" не существует в табличной части (стандартной табличной части, движениях) ""%1"" объекта ""%2"".'; en = 'Attrubute ""%3"" does not exist in the tabular section (in standard tabular section, in movements) ""%1"" of object ""%2"".'"),
...
Процедура ОтключитьОбработкуДляОтладки()
	...
	Если ОбработкаВыгрузки <> Неопределено Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными'; en = 'Data exchange'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ИнициализацияСообщений()
	...
	СообщенияОбОшибках.Вставить(2,  НСтр("ru = 'Ошибка распаковки файла обмена. Файл заблокирован.'; en = 'Error unpacking exchange file. The file is locked.'"));
	СообщенияОбОшибках.Вставить(3,  НСтр("ru = 'Указанный файл правил обмена не существует.'; en = 'Specified exchange rules file does not exist.'"));
	СообщенияОбОшибках.Вставить(4,  НСтр("ru = 'Ошибка при создании COM-объекта Msxml2.DOMDocument'; en = 'Error creating COM object Msxml2.DOMDocument'"));
	СообщенияОбОшибках.Вставить(5,  НСтр("ru = 'Ошибка открытия файла обмена'; en = 'Error opening exchange file'"));
	СообщенияОбОшибках.Вставить(6,  НСтр("ru = 'Ошибка при загрузке правил обмена'; en = 'Error importing exchange rules'"));
	СообщенияОбОшибках.Вставить(7,  НСтр("ru = 'Ошибка формата правил обмена'; en = 'Exchange rule format error'"));
	СообщенияОбОшибках.Вставить(8,  НСтр("ru = 'Не корректно указано имя файла для выгрузки данных'; en = 'Invalid data export file name'")); // не используется
	СообщенияОбОшибках.Вставить(9,  НСтр("ru = 'Ошибка формата файла обмена'; en = 'Exchange file format error'"));
	СообщенияОбОшибках.Вставить(10, НСтр("ru = 'Не указано имя файла для выгрузки данных (Имя файла данных)'; en = 'File name for data export (Data file name) is not specified.'"));
	СообщенияОбОшибках.Вставить(11, НСтр("ru = 'Ссылка на несуществующий объект метаданных в правилах обмена'; en = 'Exchange rules contain a reference to a nonexistent metadata object'"));
	СообщенияОбОшибках.Вставить(12, НСтр("ru = 'Не указано имя файла с правилами обмена (Имя файла правил)'; en = 'File name for exchange rules (Rules file name) is not specified.'"));
			
	СообщенияОбОшибках.Вставить(13, НСтр("ru = 'Ошибка получения значения свойства объекта (по имени свойства источника)'; en = 'Error retrieving object property value (using source property name)'"));
	СообщенияОбОшибках.Вставить(14, НСтр("ru = 'Ошибка получения значения свойства объекта (по имени свойства приемника)'; en = 'Error retrieving object property value (using destination property name)'"));
	
	СообщенияОбОшибках.Вставить(15, НСтр("ru = 'Не указано имя файла для загрузки данных (Имя файла для загрузки)'; en = 'File name for data import (import file name) is not specified.'"));
			
	СообщенияОбОшибках.Вставить(16, НСтр("ru = 'Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)'; en = 'Error retrieving subordinate object property value (using source property name)'"));
	СообщенияОбОшибках.Вставить(17, НСтр("ru = 'Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)'; en = 'Error retrieving subordinate object property value (using destination property name)'"));
	СообщенияОбОшибках.Вставить(18, НСтр("ru = 'Ошибка при создании обработки с кодом обработчиков'; en = 'Error creating data processor with handlers code'"));
	СообщенияОбОшибках.Вставить(19, НСтр("ru = 'Ошибка в обработчике события ПередЗагрузкойОбъекта'; en = 'BeforeImportObject event handler error'"));
	СообщенияОбОшибках.Вставить(20, НСтр("ru = 'Ошибка в обработчике события ПриЗагрузкеОбъекта'; en = 'OnImportObject event handler error'"));
	СообщенияОбОшибках.Вставить(21, НСтр("ru = 'Ошибка в обработчике события ПослеЗагрузкиОбъекта'; en = 'AfterImportObject event handler error'"));
	СообщенияОбОшибках.Вставить(22, НСтр("ru = 'Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)'; en = 'BeforeDataImport event handler error (conversion)'"));
	СообщенияОбОшибках.Вставить(23, НСтр("ru = 'Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)'; en = 'AfterLoadingData event handler error'"));
	СообщенияОбОшибках.Вставить(24, НСтр("ru = 'Ошибка при удалении объекта'; en = 'Error deleting object'"));
	СообщенияОбОшибках.Вставить(25, НСтр("ru = 'Ошибка при записи документа'; en = 'Error saving document'"));
	СообщенияОбОшибках.Вставить(26, НСтр("ru = 'Ошибка записи объекта'; en = 'Error writing object'"));
	СообщенияОбОшибках.Вставить(27, НСтр("ru = 'Ошибка в обработчике события ПередОбработкойПравилаОчистки'; en = 'BeforeProcessClearingRule event handler error'"));
	СообщенияОбОшибках.Вставить(28, НСтр("ru = 'Ошибка в обработчике события ПослеОбработкиПравилаОчистки'; en = 'AfterProcessClearingRule event handler error'"));
	СообщенияОбОшибках.Вставить(29, НСтр("ru = 'Ошибка в обработчике события ПередУдалениемОбъекта'; en = 'BeforeDeleteObject event handler error'"));
	
	СообщенияОбОшибках.Вставить(31, НСтр("ru = 'Ошибка в обработчике события ПередОбработкойПравилаВыгрузки'; en = 'BeforeProcessExportRule event handler error'"));
	СообщенияОбОшибках.Вставить(32, НСтр("ru = 'Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки'; en = 'AfterProcessExportRule event handler error'"));
	СообщенияОбОшибках.Вставить(33, НСтр("ru = 'Ошибка в обработчике события ПередВыгрузкойОбъекта'; en = 'BeforeExportObject event handler error'"));
	СообщенияОбОшибках.Вставить(34, НСтр("ru = 'Ошибка в обработчике события ПослеВыгрузкиОбъекта'; en = 'AfterExportObject event handler error'"));
			
	СообщенияОбОшибках.Вставить(41, НСтр("ru = 'Ошибка в обработчике события ПередВыгрузкойОбъекта'; en = 'BeforeExportObject event handler error'"));
	СообщенияОбОшибках.Вставить(42, НСтр("ru = 'Ошибка в обработчике события ПриВыгрузкеОбъекта'; en = 'OnExportObject event handler error'"));
	СообщенияОбОшибках.Вставить(43, НСтр("ru = 'Ошибка в обработчике события ПослеВыгрузкиОбъекта'; en = 'AfterExportObject event handler error'"));
			
	СообщенияОбОшибках.Вставить(45, НСтр("ru = 'Не найдено правило конвертации объектов'; en = 'No conversion rule is found'"));
		
	СообщенияОбОшибках.Вставить(48, НСтр("ru = 'Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств'; en = 'BeforeProcessExport property group event handler error'"));
	СообщенияОбОшибках.Вставить(49, НСтр("ru = 'Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств'; en = 'AfterProcessExport property group event handler error'"));
	СообщенияОбОшибках.Вставить(50, НСтр("ru = 'Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)'; en = 'BeforeExport event handler error (collection object)'"));
	СообщенияОбОшибках.Вставить(51, НСтр("ru = 'Ошибка в обработчике события ПриВыгрузке (объекта коллекции)'; en = 'OnExport event handler error (collection object)'"));
	СообщенияОбОшибках.Вставить(52, НСтр("ru = 'Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)'; en = 'AfterExport event handler error (collection object)'"));
	СообщенияОбОшибках.Вставить(53, НСтр("ru = 'Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)'; en = 'BeforeImportObject global event handler error (conversion)'"));
	СообщенияОбОшибках.Вставить(54, НСтр("ru = 'Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)'; en = 'AfterImportObject global event handler error (conversion)'"));
	СообщенияОбОшибках.Вставить(55, НСтр("ru = 'Ошибка в обработчике события ПередВыгрузкой (свойства)'; en = 'BeforeExport event handler error (properties)'"));
	СообщенияОбОшибках.Вставить(56, НСтр("ru = 'Ошибка в обработчике события ПриВыгрузке (свойства)'; en = 'OnExport event handler error (properties)'"));
	СообщенияОбОшибках.Вставить(57, НСтр("ru = 'Ошибка в обработчике события ПослеВыгрузки (свойства)'; en = 'AfterExport event handler error (properties)'"));
	
	СообщенияОбОшибках.Вставить(62, НСтр("ru = 'Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)'; en = 'BeforeDataExport event handler error (conversion)'"));
	СообщенияОбОшибках.Вставить(63, НСтр("ru = 'Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)'; en = 'AfterDataExport event handler error (conversion)'"));
	СообщенияОбОшибках.Вставить(64, НСтр("ru = 'Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)'; en = 'BeforeObjectConversion global event handler error (conversion)'"));
	СообщенияОбОшибках.Вставить(65, НСтр("ru = 'Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)'; en = 'BeforeExportObject global event handler error (conversion)'"));
	СообщенияОбОшибках.Вставить(66, НСтр("ru = 'Ошибка получения коллекции подчиненных объектов из входящих данных'; en = 'Error retrieving subordinate object collection from incoming data'"));
	СообщенияОбОшибках.Вставить(67, НСтр("ru = 'Ошибка получения свойства подчиненного объекта из входящих данных'; en = 'Error retrieving subordinate object properties from incoming data'"));
	СообщенияОбОшибках.Вставить(68, НСтр("ru = 'Ошибка получения свойства объекта из входящих данных'; en = 'Error retrieving object properties from incoming data'"));
	
	СообщенияОбОшибках.Вставить(69, НСтр("ru = 'Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)'; en = 'AfterExportObject global event handler error (conversion)'"));
	
	СообщенияОбОшибках.Вставить(71, НСтр("ru = 'Не найдено соответствие для значения Источника'; en = 'The map of the Source value is not found'"));
	
	СообщенияОбОшибках.Вставить(72, НСтр("ru = 'Ошибка при выгрузке данных для узла плана обмена'; en = 'Error exporting data for exchange plan node'"));
	
	СообщенияОбОшибках.Вставить(73, НСтр("ru = 'Ошибка в обработчике события ПоследовательностьПолейПоиска'; en = 'SearchFieldSequence event handler error'"));
	СообщенияОбОшибках.Вставить(74, НСтр("ru = 'Необходимо перезагрузить правила обмена для выгрузки данных.'; en = 'Data exchange rules for export must be imported again.'"));
	
	СообщенияОбОшибках.Вставить(75, НСтр("ru = 'Ошибка в обработчике события ПослеЗагрузкиПравилОбмена (конвертация)'; en = 'AfterExchangeRulesImport event handler error (conversion)'"));
	СообщенияОбОшибках.Вставить(76, НСтр("ru = 'Ошибка в обработчике события ПередОтправкойИнформацииОбУдалении (конвертация)'; en = 'BeforeSendingDeletionData event handler error (conversion)'"));
	СообщенияОбОшибках.Вставить(77, НСтр("ru = 'Ошибка в обработчике события ПриПолученииИнформацииОбУдалении (конвертация)'; en = 'OnGetDeletionInfo event handler error (conversion)'"));
	
	СообщенияОбОшибках.Вставить(78, НСтр("ru = 'Ошибка при выполнении алгоритма после загрузки значений параметров'; en = 'Error executing algorithm after parameter value import'"));
	
	СообщенияОбОшибках.Вставить(79, НСтр("ru = 'Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл'; en = 'AfterExportObjectToFile event handler error'"));
	...
	СообщенияОбОшибках.Вставить(80, НСтр("ru = 'Ошибка установки свойства предопределенного элемента.
		...
		|en = 'Error setting property of the predefined element.
		|It is not possible to mark predefined items for deletion. Deletion mark is not set.'"));
	//
	СообщенияОбОшибках.Вставить(83, НСтр("ru = 'Ошибка обращения к табличной части объекта. Табличная часть объекта не может быть изменена.'; en = 'Error accessing object tabular section. Tabular section cannot be changed.'"));
	СообщенияОбОшибках.Вставить(84, НСтр("ru = 'Коллизия дат запрета изменения.'; en = 'Prohibition dates conflict.'"));
	
	СообщенияОбОшибках.Вставить(173, НСтр("ru = 'Ошибка блокировки узла обмена. Возможно синхронизация данных уже выполняется'; en = 'Error locking exchange node. Perhaps data synchronization is already in progress'"));
	СообщенияОбОшибках.Вставить(174, НСтр("ru = 'Сообщение обмена было принято ранее'; en = 'Exchange message was received previously'"));
	СообщенияОбОшибках.Вставить(175, НСтр("ru = 'Ошибка в обработчике события ПередПолучениемИзмененныхОбъектов (конвертация)'; en = 'BeforeReceivingModifiedObjects event handler error (conversion)'"));
	СообщенияОбОшибках.Вставить(176, НСтр("ru = 'Ошибка в обработчике события ПослеПолученияИнформацииОбУзлахОбмена (конвертация)'; en = 'AfterGetExchangeNodeDetails event handler error (conversion)'"));
		
	СообщенияОбОшибках.Вставить(1000, НСтр("ru = 'Ошибка при создании временного файла выгрузки данных'; en = 'Error creating temporary data export file'"));
...
Процедура ЗагрузитьПравилаОбмена(Источник="",
	...
	Если РежимОбмена <> "Загрузка" И Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(75, ОписаниеОшибки(), НСтр("ru = 'ПослеЗагрузкиПравилОбмена (конвертация)'; en = 'AfterExchangeRulesImport (converstion)'"));
			Отказ = Истина;
...
Процедура УдалитьОбъектПоСсылке(Ссылка, СтрокаСообщенияОбОшибке)
	...
	Если Не ПустаяСтрока(Конвертация.ПриПолученииИнформацииОбУдалении) Тогда
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(77, ОписаниеОшибки(), НСтр("ru = 'ПриПолученииИнформацииОбУдалении (конвертация)'; en = 'WhenReceivingInformationOnDeletion (conversion)'"));
			Отказ = Истина;
					...
					Если СтрокаТаблицы = Неопределено Тогда
						...
						Для Каждого Колонка Из ТаблицаДанныхСообщенияОбмена.Колонки Цикл
							...
							Если Колонка.ТипЗначения.СодержитТип(ТипСтрока) Тогда
								...
								СтрокаТаблицы[Колонка.Имя] = НСтр("ru = 'Удаление объекта'; en = 'Object deletion'");
				...
				Попытка
					...
					Если Отказ = Истина Тогда
						...
						Если Не ПустаяСтрока(ПричинаОтказа) Тогда
							...
							СтрокаСообщения = НСтр("ru = 'Загрузка данных отменена по причине: %1'; en = 'Data import cancelled because: %1'");
							СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ПричинаОтказа);
							ВызватьИсключение СтрокаСообщения;
						Иначе
							ВызватьИсключение НСтр("ru = 'Загрузка данных отменена'; en = 'Data import cancelled'");
						КонецЕсли;
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	Если Не ЗначениеЗаполнено(УзелОбменаПолучатель)
		...
		СтрокаСообщения = НСтр("ru = 'Не найден узел обмена для загрузки данных. План обмена: %1, Код: %2'; en = 'Exchange node for data import is not found. Exchange plan: %1, Code: %2'");
	...
	Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
		...
		Если НомерПринятого >= ЧтениеСообщения.НомерСообщения Тогда // Номер сообщения меньше либо равен ранее принятому.
			...
			ВызватьИсключение НСтр("ru = 'Сообщение обмена было принято ранее'; en = 'Exchange message was received previously'");
		КонецЕсли;
		...
		Если УдалитьРегистрациюИзменений Тогда // Удаляем регистрацию изменений.
			...
			Если ТранзакцияАктивна() Тогда
				ВызватьИсключение НСтр("ru = 'Удаление регистрации изменений данных не может быть выполнено в активной транзакции.'; en = 'Deletion of data changes registration cannot be done in an active transaction.'");
			КонецЕсли;
	...
	Если Не ПустаяСтрока(Конвертация.ПослеПолученияИнформацииОбУзлахОбмена) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(176, ОписаниеОшибки(), НСтр("ru = 'ПослеПолученияИнформацииОбУзлахОбмена (конвертация)'; en = 'AfterGettingInformationAboutExchangeNodes (conversion)'"));
		КонецПопытки;
	...
	Для Каждого ОбъектСоответствия Из ОбъектыДляОтложеннойЗаписи() Цикл
		...
		Попытка
			...
			Иначе
				...
				ОписаниеОшибки = НСтр("ru = 'Ошибка проверки заполнения реквизитов'; en = 'Error when checking unfilled attributes'");
...
Процедура ВыполнитьВыгрузку(СтрокаСообщенияОбОшибке = "")
	...
	Если Не ПустаяСтрока(Конвертация.ПередВыгрузкойДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(62, ОписаниеОшибки(), НСтр("ru = 'ПередВыгрузкойДанных (конвертация)'; en = 'BeforeDataExport (conversion)'"));
			Отказ = Истина;
		КонецПопытки; 
	...
	Если Не Отказ И Не ПустаяСтрока(Конвертация.ПослеВыгрузкиДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(63, ОписаниеОшибки(), НСтр("ru = 'ПослеВыгрузкиДанных (конвертация)'; en = 'AfterDataExport (conversion)'"));
		КонецПопытки;
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(ЗаписьСообщения, СтрокаСообщенияОбОшибке, ТаблицаПравилВыгрузкиИспользуемые)
	...
	Если Не ПустаяСтрока(Конвертация.ПередПолучениемИзмененныхОбъектов) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(175, ОписаниеОшибки(), НСтр("ru = 'ПередПолучениемИзмененныхОбъектов (конвертация)'; en = 'BeforeGetChangedObjects (conversion)'"));
			Возврат;
		КонецПопытки;
		...
		Если ЭтоОбменЧерезВнешнееСоединение Тогда
			Если ЗагрузкаДанныхВыполняетсяВоВнешнемСоединении Тогда
				Если ОбработкаДляЗагрузкиДанных().ИспользоватьТранзакции Тогда
					...
					Если ОбработкаДляЗагрузкиДанных().ФлагОшибки() Тогда
						ВызватьИсключение(НСтр("ru = 'Ошибка при отправке данных.'; en = 'Error while exporting data.'"));
					Иначе
						ВнешнееСоединение.ЗафиксироватьТранзакцию();
					КонецЕсли;
