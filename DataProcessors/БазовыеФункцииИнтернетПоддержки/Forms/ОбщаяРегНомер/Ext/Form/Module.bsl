...
Процедура СписокЗарегистрированныхПродуктовРегНомерНажатие(Элемент)
	...
	ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
		...
		НСтр("ru = 'Список зарегистрированных продуктов'; en = 'List of registered products'"),
...
Процедура ПояснениеКЗаголовкуРегНомерОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ru = 'Не получается ввести регистрационный номер программного продукта.'; en = 'Unable to enter the registration number of the software product.'");
		
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru = 'Интернет-поддержка. Ввод регистрационного номера.'; en = 'Online support. Entering registration number.'"),
			ТекстСообщения,
			,
			,
...
Процедура ЗаполнитьФорму()
	...
	ЗаголовокПользователя = НСтр("ru = 'Логин:'; en = 'Login:'") + " " + Параметры.login;
...
Функция ЗаполнениеПолейКорректно()
	...
	Если ПустаяСтрока(РегистрационныйНомерРегНомер) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не заполнено поле ""Регистрационный номер""'; en = 'Field ""Registration number"" is not filled in.'");
