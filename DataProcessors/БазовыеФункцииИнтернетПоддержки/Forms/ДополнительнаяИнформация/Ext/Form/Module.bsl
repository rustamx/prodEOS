...
Процедура АдресНажатие(Элемент, СтандартнаяОбработка)
	...
	Если РегионыСтраны = Неопределено Тогда
		...
		РегионыСтраны.Добавить("-1", НСтр("ru = '<не выбран>'; en = '<not selected>'"));
	КонецЕсли;
	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ru = 'Не получается ввести информацию об организации при регистрации программного продукта.
				...
				|Факс: %8';
				|en = 'Unable to enter company information while registering the software product. 
				|
				|Company name: %1 
				|Activity type: %2 
				|TIN: %3 
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru = 'Интернет-поддержка. Ввод информации об организации.'; en = 'Online support. Entering information about company.'"),
...
Процедура КомандаОтмена(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru = 'Вы уверены, что хотите отказаться от регистрации программного продукта?'; en = 'Are you sure you want to refuse registering the software product?'"),
		РежимДиалогаВопрос.ДаНет,
		,
		,
		НСтр("ru = 'Регистрация программного продукта'; en = 'Software product registration'"));
...
Процедура НастроитьОтображениеФормы()
	...
	ПредставлениеАдреса   = НСтр("ru = '<введите адрес>'; en = '<enter address>'");
...
Функция ЗаполнениеПолейКорректно()
	...
	Если Организация = "-1" Тогда
		...
		Если ПустаяСтрока(НазваниеОрганизации) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Название организации"" не заполнено.'; en = '""Company name"" field is not filled in'"),
				"НазваниеОрганизации",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(ТипДеятельности) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Тип деятельности"" не заполнено.'; en = 'Field ""Activity type"" is not filled in'"),
				"ТипДеятельности",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(ИНН) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""ИНН"" не заполнено.'; en = 'Field ""TIN"" is not filled in'"),
				"ИНН",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(Руководитель) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Руководитель"" не заполнено.'; en = 'Field ""Head"" is not filled in'"),
				"Руководитель",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(Телефон) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Телефон"" не заполнено.'; en = 'Field ""Phone"" is not filled in'"),
				"Телефон",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(АдресЭлектроннойПочты) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Адрес электронной почты"" не заполнено.'; en = 'Field ""Email address"" is not filled in'"),
				"АдресЭлектроннойПочты",
				Отказ);
		КонецЕсли;
	...
	Если ПустаяСтрока(ГдеПриобретенаПрограмма) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Место покупки"" не заполнено.'; en = 'Field ""Place of purchase"" is not filled in'"),
			"ГдеПриобретенаПрограмма",
			Отказ);
	КонецЕсли;
	
	Если ДатаПриобретенияПрограммы = '00010101' Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Дата покупки"" не заполнено.'; en = 'Field ""Purchase date"" is not filled in'"),
			"ДатаПриобретенияПрограммы",
			Отказ);
	КонецЕсли;
	
	Если ЧислоРабочихМест = 0 Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Число рабочих мест"" не заполнено.'; en = 'Field ""Number of workplaces"" is not filled in'"),
			"ЧислоРабочихМест",
			Отказ);
	КонецЕсли;
	
	Если ПустаяСтрока(Ответственный) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru = 'Поле ""Ответственный сотрудник"" не заполнено.'; en = 'Field ""Responsible employee"" is not filled in'"),
			"Ответственный",
			Отказ);
	КонецЕсли;
...
Процедура СформироватьПредставлениеАдреса()
	...
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Район, НСтр("ru = 'район'; en = 'county'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Город, НСтр("ru = 'г.'; en = 'yr'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Улица, НСтр("ru = 'ул.'; en = 'st.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Дом, НСтр("ru = 'д.'; en = 'd.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Строение, НСтр("ru = 'стр.'; en = 'bldg.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Квартира, НСтр("ru = 'кв.'; en = 'apart.'") + " ");
	
	Если ПустаяСтрока(ПредставлениеАдреса) Тогда
		Если Организация = "-1" Тогда
			ПредставлениеАдреса = НСтр("ru = '<введите адрес>'; en = '<enter address>'");
		Иначе
			ПредставлениеАдреса = НСтр("ru = '<не заполнен>'; en = '<not filled in>'");
		КонецЕсли;
	КонецЕсли;
