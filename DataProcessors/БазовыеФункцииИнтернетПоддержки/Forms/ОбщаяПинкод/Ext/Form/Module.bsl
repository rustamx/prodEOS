	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ru = 'Не получается зарегистрировать программный продукт.
				|
				|Пинкод: %1.';
				|en = 'Unable to register the software product.
				|
				|Pincode: %1.'");
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru = 'Интернет-поддержка. Регистрация продукта.'; en = 'Online support. Product registration.'"),
			ТекстСообщения,
			,
			,
...
Функция ЗаполнениеПолейКорректно()
	...
	Если ПустаяСтрока(ПинКод) Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Не заполнено поле ""Пинкод""'; en = 'Field ""PIN code"" is not filled in.'");
	...
	Если СтрДлина(ПинКодСтр) <> 16 Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Пин-код должен состоять из 16 цифр.'; en = 'PIN code must consist of 16 digits.'");
		Сообщение.Поле  = "ПинКод";
		Сообщение.Сообщить();
