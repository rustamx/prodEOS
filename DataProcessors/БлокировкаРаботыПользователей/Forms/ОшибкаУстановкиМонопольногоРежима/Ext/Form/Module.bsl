...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если УдалениеПомеченныхОбъектов Тогда
		Заголовок = НСтр("ru = 'Не удалось выполнить удаление помеченных объектов'; en = 'Failed to delete marked objects'");
		Элементы.ТекстСообщенияОбОшибке.Заголовок = НСтр("ru = 'Невозможно выполнить удаление помеченных объектов, т.к. в программе работают другие пользователи:'; en = 'It is not possible to delete the marked objects because there are other connected users:'");
	КонецЕсли;
	
	АктивныеПользователиШаблон = НСтр("ru = 'Активные пользователи (%1)'; en = 'Active users (%1)'");
...
Процедура ТаймАутЗавершенияРаботыПользователей()
	...
	Если ПродолжительностьЗавершенияРаботыПользователей >= 3 Тогда
		...
		Элементы.ТекстСообщенияОбОшибке.Заголовок = НСтр("ru = 'Невозможно выполнить обновление версии программы, т.к. не удалось завершить работу пользователей:'; en = 'It is not possible to upgrade the program because there are other connected users:'");
...
Процедура УстановитьБлокировкуФайловойБазы()
	...
	Если УдалениеПомеченныхОбъектов Тогда
		...
		Объект.СообщениеДляПользователей = НСтр("ru = 'Программа заблокирована для удаления помеченных объектов.'; en = 'Program is locked for deletion of marked objects.'");
	Иначе
		Объект.НачалоДействияБлокировки = ТекущаяДатаСеанса() + 2*60;
		Объект.ОкончаниеДействияБлокировки = Объект.НачалоДействияБлокировки + 5*60;
		Объект.СообщениеДляПользователей = НСтр("ru = 'Программа заблокирована для выполнения обновления.'; en = 'Program is locked for the purpose of upgrade.'");
	КонецЕсли;
