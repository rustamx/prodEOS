...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		Иначе
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru = 'Не заданы параметры формы. Форма не может быть открыта.'; en = 'Form parameters are not set. The form cannot be opened.'"), Отказ);
			Возврат;
		КонецЕсли;
	...
	ШаблонЗаголовка = НСтр("ru = 'Синхронизация данных с ""%1""'; en = 'Data synchronization with ""%1""'");
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(ПараметрыЗадания, ОшибкаРасхожденияВерсийПриПолученииДанных, ПроверятьРасхождениеВерсий)
	...
	Если ПараметрыЗадания.ВыполнятьЗагрузку Тогда
		...
		ШаблонНаименования = НСтр("ru = 'Выполняется загрузка данных из %1'; en = 'Importing data from %1'");
		
	Иначе
		ШаблонНаименования = НСтр("ru = 'Выполняется выгрузка данных в %1'; en = 'Exporting data to %1'");
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПолучитьРезультатНаСервере()
	...
	ТиповоеПредставлениеОшибки = СтрШаблон(
		НСтр("ru = 'Не удалось выполнить %1. Подробности см. в журнале регистрации'; en = 'Unable to execute %1. For details, see event log'"),
		?(ФоновоеЗаданиеВыполняемоеДействие = 1, НСтр("ru = 'получение данных'; en = 'importing data'"), НСтр("ru = 'отправку данных'; en = 'data export'")));
