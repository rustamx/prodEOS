...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщаяПроизводительностьСистемы.Пустая() Тогда
		Объект.ОбщаяПроизводительностьСистемы = НСтр("ru = 'Общая производительность системы'; en = 'Overall system performance'");
	Иначе
	...
	Исключение
		ТекстСообщения = НСтр("ru = 'Не удалось загрузить настройки.'; en = 'Cannot load settings.'");
...
Процедура УказатьAPDEX(Команда)
	...
	Если СтрокаТЧ[Элемент.Имя] = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Отсутствуют замеры производительности.
			...
			|en = 'No performance measurements.
			|Calculate the target time is impossible.'"));
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("УказатьAPDEXЗавершение", ЭтотОбъект);
	Подсказка = НСтр("ru = 'Введите желаемое значение APDEX'; en = 'Enter desired apdex VALUE.'"); 
	ЗначениеAPDEX = 0;
...
Процедура УказатьAPDEXЗавершение(Знач ЗначениеAPDEX, Знач ДополнительныеПараметры) Экспорт
	...
	Если 0 > ЗначениеAPDEX Или ЗначениеAPDEX > 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Введен неправильный показатель APDEX.
			...
			|en = 'APDEX index you entered is not valid.
...
Процедура ОбновитьПоказатели(ЗначенияОтбора = Неопределено)
	...
	Если ТаблицаКлючевыхОпераций = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Период установлен не верно.'; en = 'Period is set incorrectly.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДиаграмму(ТаблицаКлючевыхОпераций)
	...
	ТекстЗаголовка = НСтр("ru = 'Диаграмма производительности с %1 по %2 - шаг: %3'; en = 'Chart of performance from %1 to %2 with step %3'");
...
Процедура УдалитьФайлыНаСервере(Каталог)
	...
	Исключение
		Событие = НСтр("ru = 'Удалить файлы на сервере'; en = 'Delete files on the server'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ВычислитьДатыВременногоОтрезка(ДатаНачала, ДатаОкончания, ИндексПериода)
	...
	Если КоличествоШагов <= ИндексПериода Тогда
		ВызватьИсключение НСтр("ru = 'Количество шагов не может быть меньше индекса.'; en = 'Step count cannot be less then index.'");
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Целевое время обязательно должно быть заполнено.'; en = 'Target time must be filled.'"),
				,
...
Процедура ВыбратьФайлЭкспортаПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Состояние(НСтр("ru = 'Экспорт данных...'; en = 'Data export...'"));
