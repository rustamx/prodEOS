...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав доступа.
		                             ...
		                             |
		                             |Sheduled and background jobs
		                             | runs only by administrators.'");
	КонецЕсли;
...
Процедура ТаблицаРегламентныеЗаданияПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ВыделенныеСтроки.Количество() > 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите одно регламентное задание.'; en = 'Select one scheduled job.'"));
		
	ИначеЕсли Элемент.ТекущиеДанные.Предопределенное Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Невозможно удалить предопределенное регламентное задание.'; en = 'Cannot delete a predefined sheduled job.'") );
	Иначе
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ТаблицаРегламентныеЗаданияПередУдалениемЗавершение", ЭтотОбъект),
			НСтр("ru = 'Удалить регламентное задание?'; en = 'Delete sheduled job?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите регламентное задание.'; en = 'Select scheduled job.'"));
		Возврат;
	КонецЕсли;
	...
	Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		Если ТекущиеДанные.Параметризуемое И КоличествоВыделенныхЗаданий = 1 Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Выбранное регламентное задание нельзя выполнить вручную.'; en = 'Selected sheduled job cannot be perfomed manually.'"));
			Возврат;
		...
		Если ПараметрыВыполнения.ЗапускВыполнен Тогда
			...
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Запущена процедура регламентного задания'; en = 'Sheduled job procedure runs'"), ,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1.
					|Процедура запущена в фоновом задании %2';
					|en = '%1.
					|Procedure is started in the background job %2'"),
		...
		ИначеЕсли ПараметрыВыполнения.ПроцедураУжеВыполняется Тогда
			МассивСообщенийОбОшибках.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					|en = 'Sheduled job procedure ""%1""
					|is already running in the background job ""%2"", launched %3.'"),
					ТекущиеДанные.Наименование,
	...
	Если КоличествоОшибок > 0 Тогда
		ЗаголовокТекстаПроОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Задания выполнены с ошибками (%1 из %2)'; en = 'Jobs completed with errors (%1 of %2)'"),
...
Процедура ОтменитьФоновоеЗадание(Команда)
	...
	Если Элементы.ТаблицаФоновыеЗадания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выберите фоновое задание.'; en = 'Select background job.'"));
	Иначе
		...
		ПоказатьПредупреждение(,
			НСтр("ru = 'Задание отменено, но состояние отмены будет
			           ...
			           |en = 'Job is canceled, but the revocation status 
			           |will be set to the server only through the seconds,
			           |you may need to refresh data manually.'"));
	КонецЕсли;
...
Процедура ВыполнитьНеВФоне(Команда)
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru = 'Выполнена процедура регламентного задания'; en = 'Sheduled job procedure performed'"), ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1.
			|Процедура выполнена вручную без фонового задания';
			|en = '%1. 
			|Procedure is performed manually without background job'"),
			ТекущиеДанные.Наименование),
		БиблиотекаКартинок.ВыполнитьРегламентноеЗаданиеВручную);
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ru = '<не определено>'; en = '<not defined>'");
		...
		ПоказатьОповещениеПользователя(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1.
				           ...
				           |en = '%1. 
				           |Procedure is completed in the background job %2'"),
...
Функция ЗагрузкаРегламентныхЗаданий(ИдентификаторЗадания)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Формирование таблицы регламентных заданий'; en = 'Formation of the table of scheduled tasks'");
...
Функция СформироватьТаблицуФоновыхЗаданийВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Регламентные задания. Обновление списка фоновых заданий'; en = 'Sheduled jobs. Update the list of background jobs'");
	...
	Для каждого Задание Из ТекущаяТаблица Цикл
		...
		Если ЗначениеЗаполнено(Обновляемое.ИдентификаторРегламентногоЗадания) Тогда
			...
			Обновляемое.НаименованиеРегламентногоЗадания
				= ?(Строки.Количество() = 0, НСтр("ru = '<не найдено>'; en = '<not found>'"), Строки[0].Наименование);
		Иначе
