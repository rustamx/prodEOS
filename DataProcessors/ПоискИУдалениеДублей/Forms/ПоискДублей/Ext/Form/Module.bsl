...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтображениеСостояния.Текст = НСтр("ru = 'Поиск дублей не выполнялся. 
	                                        ...
	                                        |en = 'Search of duplicates was not performed. 
	                                        |Set the criteria and compare and click ""Find duplicates"".'");
	...
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ru = 'Поиск дублей...'; en = 'Looking for duplicates...'");
	...
	ОтображениеСостояния.Видимость = Истина;
	ОтображениеСостояния.Текст = НСтр("ru = 'Удаление дублей...'; en = 'Deleting duplicates...'");
	ОтображениеСостояния.Картинка = Элементы.ДлительнаяОперация48.Картинка;
	...
	ОтображениеСостояния.Текст = НСтр("ru = 'Не обнаружено дублей по указанным параметрам.
	                                        ...
	                                        |en = 'Not detected duplicates on the specified parameters.
	                                        |Edit filters and compare and then click ""Find duplicates""'");
	ОтображениеСостояния.Картинка = Элементы.Предупреждение32.Картинка;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
		ТекстВопроса = НСтр("ru = 'Прервать поиск дублей и закрыть форму?'; en = 'Interrupt the search of duplicates and close the form?'");
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
		ТекстВопроса = НСтр("ru = 'Прервать удаление дублей и закрыть форму?'; en = 'Abort deleting duplicate and close the form?'");
	КонецЕсли;
...
Процедура ОбновитьМестаИспользованияКандидата(Знач ИдентификаторСтроки)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для элемента ""%1"" найдено дублей: %2'; en = 'For item ""%1"" found duplicates: %2'"),
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Элемент ""%1"" не используется'; en = 'Item ""%1"" is not used'"), 
	...
	Иначе
		Элементы.МестаИспользованияКандидата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Места использования ""%1"" (%2)'; en = 'The location of ""%1"" (%2)'"), 
			ДанныеСтроки.Наименование,
			ДанныеСтроки.Количество);
...
Процедура ОбновитьМестаИспользованияНеобработанныхДубли(Знач СтрокаДанных)
	...
	Если ДанныеСтроки.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = НСтр("ru = 'Для просмотра причин выберите проблемный элемент-дубль.'; en = 'To view the reasons select the problematic item.'");
	...
	Если ДанныеСтроки.Количество = 0 Тогда
		Элементы.ОписаниеТекущейГруппыДублей1.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Замена дубля ""%1"" возможна, но была отменена из-за невозможности замены в других местах.'; en = 'Replacement double ""%1"" is possible, but was cancelled due to the impossibility of replacement elsewhere.'"), 
			ДанныеСтроки.Наименование);
		
		Элементы.СтраницыМестаИспользованияНеобработанных.ТекущаяСтраница = Элементы.ОписаниеГруппыНеобработанных;
	Иначе
		Элементы.МестаИспользованияКандидата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось заменить дубли в некоторых местах (%1)'; en = 'There are places where duplicates cannot be replaced (%1)'"), 
			ДанныеСтроки.Количество);
...
Процедура ПриСозданииНаСервереИнициализацияДанных(НастройкиФормы)
	...
	ВсеВариантыСравнения.Добавить("Равно",   НСтр("ru = 'Совпадает'; en = 'Matches'"));
	ВсеВариантыСравнения.Добавить("Подобно", НСтр("ru = 'Совпадает по похожим словам'; en = 'Matches for similar words'"));
КонецПроцедуры
...
Процедура ОбновитьОписаниеСостоянияНайденныхДублей(Форма)
	...
	Форма.ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Найдено дублей: %2 (среди элементов: %1). Все отмеченные элементы будут помечены на удаление и заменены во всех местах использования на оригиналы (отмечены стрелкой).'; en = 'Find duplicates: %2 (items: %1). All checked items will be marked for deletion and replaced at all locations use the originals (marked with an arrow).'"),
...
Процедура ИнициализироватьСценарийМастера()
	...
	КнопкиМастера.Далее.Заголовок = НСтр("ru = 'Найти дубли >'; en = 'Find duplicates >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru = 'Найти дубли по указанным критериям'; en = 'Find duplicates based on specified criteria'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ru = 'Закрыть'; en = 'Close'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru = 'Отказаться от поиска и замены дублей'; en = 'Cancel search and replacement of duplicates'");
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ru = 'Прервать поиск дублей'; en = 'Abort searching for duplicates'");
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Видимость = Ложь;
	КнопкиМастера.Далее.Заголовок = НСтр("ru = 'Удалить дубли >'; en = 'Delete duplicates >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru = 'Удалить дубли'; en = 'Delete duplicates'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ru = 'Закрыть'; en = 'Close'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru = 'Отказаться от поиска и замены дублей'; en = 'Cancel search and replacement of duplicates'");
	...
	КнопкиМастера.Далее.Видимость = Ложь;
	КнопкиМастера.Отмена.Заголовок = НСтр("ru = 'Прервать'; en = 'Terminate'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru = 'Прервать удаление дублей'; en = 'Abort duplicate deletion'");
	...
	// 4. Успешное удаление.
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Заголовок = НСтр("ru = '< Новый поиск'; en = '< New search'");
	КнопкиМастера.Назад.Подсказка = НСтр("ru = 'Начать новый поиск с другими параметрами'; en = 'Start new search with other parameters'");
	КнопкиМастера.Далее.Видимость = Ложь;
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Видимость = Ложь;
	КнопкиМастера.Далее.Заголовок = НСтр("ru = 'Повторить удаление >'; en = 'Repeat deleting >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru = 'Удалить дубли'; en = 'Delete duplicates'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ru = 'Закрыть'; en = 'Close'");
...
Процедура ШагПоискНеВыполнялсяПриАктивации(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ПредставлениеПравилОтбора) Тогда
		ПредставлениеПравилОтбора = НСтр("ru = 'Все элементы'; en = 'All items'");
	КонецЕсли;
	
	// Представление правил поиска.
	Союз = " " + НСтр("ru = 'И'; en = 'And'") + " ";
	ТекстПравил = "";
	Для Каждого Правило Из ПравилаПоиска Цикл
		Если Правило.Правило = "Равно" Тогда
			Сравнение = Правило.ПредставлениеРеквизита + " " + НСтр("ru = 'совпадает'; en = 'matches'");
		ИначеЕсли Правило.Правило = "Подобно" Тогда
			Сравнение = Правило.ПредставлениеРеквизита + " " + НСтр("ru = 'совпадает по похожим словам'; en = 'matches for similar words'");
		Иначе
			Продолжить;
		КонецЕсли;
	...
	Если ПустаяСтрока(ТекстПравил) Тогда
		ТекстПравил = НСтр("ru = 'Правила не заданы'; en = 'Rules not set'");
	КонецЕсли;
...
Процедура ШагПоискНеВыполнялсяПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ТекстПредупреждения = НСтр("ru = 'Необходимо выбрать область поиска дублей'; en = 'Select area to search for duplicates'");
...
Процедура ЗапуститьРаботуМастера()
	...
	Если НастройкиПошаговогоМастера.СтраницаСтарта = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Перед запуском мастера должна быть установлена начальная страница.'; en = 'The start page must be set before the wizard is started.'");
...
Функция ЗапуститьФоновоеЗадание()
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
		...
		НаименованиеМетода = НСтр("ru = 'Поиск и удаление дублей: Поиск дублей'; en = 'Finding and removing duplicates: Duplicates search'");
	...
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
		...
		НаименованиеМетода = НСтр("ru = 'Поиск и удаление дублей: Удаление дублей'; en = 'Finding and removing duplicates: Remove duplicates'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
			Кратко = НСтр("ru = 'При поиске дублей возникла ошибка:'; en = 'Error occurred when searching for duplicates:'");
		ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
			Кратко = НСтр("ru = 'При удалении дублей возникла ошибка:'; en = 'Error occurred when removing a duplicate :'");
		КонецЕсли;
...
Функция ЗаполнитьРезультатыПоискаДублей(Знач АдресРезультата)
	...
	МестаИспользованияКандидата.Очистить();
	Элементы.ОписаниеТекущейГруппыДублей.Заголовок = НСтр("ru = 'Дублей не найдено'; en = 'No duplicates found'");
	...
	Если ВсегоНайденоДублей = ЭлементыДерева.Количество() Тогда
		ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(Элементы.Информация16.Картинка, " ",
			НСтр("ru = 'Не обнаружено дублей по указанным условиям.'; en = 'Not detected duplicates on specified conditions.'"));
	Иначе
		ОбновитьОписаниеСостоянияНайденныхДублей(ЭтотОбъект);
	КонецЕсли;
		...
		Если ВсегоОсновных = 1 Тогда
			...
			Если ПоследнийКандидат = Неопределено Тогда
				ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Все найденные дубли (%1) успешно объединены'; en = 'All found duplicates (%1) are merged'"),
						ВсегоОбработано));
			Иначе
				...
				ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Все найденные дубли (%1) успешно объединены
							|в ""%2""';
							|en = 'All found duplicates (%1) are merged
							|in ""%2""'"),
