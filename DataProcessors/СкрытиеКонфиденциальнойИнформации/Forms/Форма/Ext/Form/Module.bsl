...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru = 'Скрытие конфиденциальной информации'; en = 'Masking of confidential information'");
	...
	Если ДеревоМетаданныхЗаполнено Тогда
		Если Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаЧастотныеВозможности Тогда
			...
			Если МакетОпределен Тогда
				Заголовок = НСтр("ru = 'Скрытие конфиденциальной информации - полные возможности'; en = 'Masking of confidential information - complete features'");
			КонецЕсли;
			Элементы.РасширенныеВозможности.Заголовок = НСтр("ru = 'Обычный вид'; en = 'Normal view'");
		Иначе
			Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаЧастотныеВозможности;
			Заголовок = НСтр("ru = 'Скрытие конфиденциальной информации'; en = 'Masking of confidential information'");
			Элементы.РасширенныеВозможности.Заголовок = НСтр("ru = 'Полные возможности'; en = 'Complete capabilities'");
		КонецЕсли;
	Иначе
		Если ДеревоМетаданныхЗаполнено() Тогда
			Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаРасширенныеВозможности;
...
Процедура ЗначениеЧислоПриИзменении(Элемент)
	Если ЗначениеЧисло = 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'Значение должно быть не нулевым.'; en = 'Value must not be zero.'");
...
Процедура ВыполнитьСкрытие(Команда)
	...
	ТекстПредупреждения = НСтр("ru = 'Скрытие конфиденциальной информации необходимо выполнять на копии рабочей базы,
		...
		|Продолжить?'; en = 'Masking of confidential information must be performed on the copy of the production database, all changes will be perfomed permanently. Do you want to continue?'");
...
Процедура Подключаемый_ВыполнитьСкрытиеЗавершение()
	...
	Если РезультатЗапуска.Статус <> "Выполнено" Тогда
		...
		КонецЕсли;
	Иначе
		СообщитьПользователю(НСтр("ru = 'Скрытие конфиденциальной информации завершено.'; en = 'Masking of confidential information is completed.'"));
	КонецЕсли;
...
Процедура СохранитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогСохранения.Фильтр = НСтр("ru = 'Файл настроек'; en = 'Settings file'") + "(*.xml)|*.xml";
...
Процедура ЗагрузитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогВыбора.Фильтр = НСтр("ru = 'Файл настроек'; en = 'Settings file'") + "(*.xml)|*.xml";
...
Процедура ЗаполнитьДеревоОбрабатываемыхОбъектов(СохраненныеНастройки, ОбработкаОбъект)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru = 'Скрытие конфиденциальной информации'; en = 'Masking of confidential information'");
...
Функция ОбрабатываемыеМетаданные()
	...
	Структура.Вставить("Константы", НСтр("ru = 'Константы'; en = 'Constants'"));
	Структура.Вставить("Справочники", НСтр("ru = 'Справочники'; en = 'Catalogs'"));
	Структура.Вставить("Документы", НСтр("ru = 'Документы'; en = 'Documents'"));
	Структура.Вставить("ПланыВидовХарактеристик", НСтр("ru = 'Планы видов характеристик'; en = 'Charts of characteristic types'"));
	Структура.Вставить("ПланыСчетов", НСтр("ru = 'Планы счетов'; en = 'Charts of accounts'"));
	Структура.Вставить("ПланыВидовРасчета", НСтр("ru = 'Планы видов расчета'; en = 'Charts of calculation types'"));
	Структура.Вставить("РегистрыСведений", НСтр("ru = 'Регистры сведений'; en = 'Information registers'"));
	Структура.Вставить("РегистрыНакопления", НСтр("ru = 'Регистры накопления'; en = 'Accumulation registers'"));
	Структура.Вставить("РегистрыБухгалтерии", НСтр("ru = 'Регистры бухгалтерии'; en = 'Accounting registers'"));
	Структура.Вставить("РегистрыРасчета", НСтр("ru = 'Регистры расчета'; en = 'Calculation registers'"));
	Структура.Вставить("БизнесПроцессы", НСтр("ru = 'Бизнес процессы'; en = 'Business processes'"));
	Структура.Вставить("Задачи", НСтр("ru = 'Задачи'; en = 'Tasks'"));
...
Функция СкрытиеКонфиденциальнойИнформации()
	...
	НаименованиеЗадания = НСтр("ru = 'Скрытие конфиденциальной информации'; en = 'Masking of confidential information'");
