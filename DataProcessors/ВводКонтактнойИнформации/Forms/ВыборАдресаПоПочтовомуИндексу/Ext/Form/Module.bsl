...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ДанныеКлассификатора.Данные.Количество() = 0 Тогда
		...
		КраткоеПредставлениеОшибки = НСтр("ru = 'Индекс не найден в адресном классификаторе.'; en = 'ZIP code is not found in the address classifier.'");
		Возврат;
	КонецЕсли;
...
Процедура ПроизвестиВыбор(Знач НомерСтроки)
	...
	ПредупреждениеНеактуальности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Адрес ""%2, %1"" неактуален.
		           |Продолжить?';
		           |en = 'Address ""%2, %1"" is not valid.
		           |Continue?'"), СокрЛП(ОбщаяЧастьПредставления), Данные.Представление);
