...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидКонтактнойИнформации.ПроверятьПоФИАС Тогда
		Команды.ПроверитьЗаполнениеАдреса.Подсказка = НСтр("ru = 'Проверить заполнение адреса в формате ФИАС'; en = 'Check address filling in FIAS format'");
	Иначе
		Команды.ПроверитьЗаполнениеАдреса.Подсказка  = НСтр("ru = 'Проверить заполнение адреса в формате КЛАДР'; en = 'Check address filling in KLADR format'");
	КонецЕсли;
	...
	Если АдресТолькоРоссийский  Тогда
		...
		Если Страна <> СтранаРоссия Тогда
			...
			ТекстПредупрежденияПриОткрытии = НСтр("ru = 'Адрес введен некорректно: допускается ввод только российских адресов. Значение поля ""Страна"" было изменено на ""Россия"", необходимо проверить остальные поля.'; en = 'Incorrect address: only Russian addresses are allowed. Value of field ""Country"" has been changed to ""Russia"", you should the rest of the fields.'"); 
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
					...
					Если СведенияОРегионе.Загружен = Ложь Тогда
						...
						ДоступностьКлассификатора.КраткоеПредставлениеОшибки = 
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Необходимо загрузить адресные сведения по региону ""%1"" (в меню ""%2"").'; en = 'It is necessary to import address information for region ""%1"" (in menu ""%2"").'"),
							XDTOКонтактная.Состав.Состав.СубъектРФ,
							?(ЭтоТакси, НСтр("ru = 'Еще'; en = 'More'"), НСтр("ru = 'Все действия'; en = 'All actions'")));
	...
	ИначеЕсли ВыбранноеЗначение.Отказ Тогда
		...
		Иначе
			Если НЕ ПодборПоИндексуДоступен Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Индекс %1 не найден в адресном классификаторе'; en = 'ZIP code %1 is not found in address classifier'"),
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		Если ЕстьПравоЗагружатьКлассификатор И РегионНеЗагружен Тогда
			...
			УправлениеКонтактнойИнформациейКлиент.ПредложениеЗагрузкиКлассификатора(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Данные для ""%1"" не загружены.'; en = 'Data for ""%1"" were not imported.'"), ВыбранноеЗначение.Представление),
...
Процедура УлицаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрыФормы.Вставить("Улица", Улица);
		ПараметрыФормы.Вставить("Заголовок", НаселенныйПункт + " " + НСтр("ru = '(не найден)'; en = '(not found)'"));
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеАдреса(Команда)
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ПоказатьПредупреждение(, НСтр("ru='Адрес не может быть проверен, так как он введен в свободной форме.'; en = 'The address could not be validated because it was entered in a free form.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВвестиАдресВСвободнойФорме(Команда)
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ТекстВопроса = НСтр("ru='Изменения, введенные вручную, будут потеряны.
		                        ...
		                        |Продолжить?';
		                        |en = 'Changes entered manually will be lost.
		                        |Source address will be moved to the comment.
		                        |Continue?'");
	Иначе
		ТекстВопроса = НСтр("ru='Ввести адрес в свободной форме?
		                        ...
		                        |en = 'Enter address in a free form?
		                        |Addresses entered in a free form can fail validation against the classifier.'");
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ВвестиАдресВСвободнойФормеЗавершение", ЭтотОбъект);
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , , НСтр("ru='Подтверждение'; en = 'Confirmation'"));
КонецПроцедуры
...
Процедура АдренсыйКлассификаторУстарел(Команда)
	...
	СообщениеОПроверкеАдреса = НСтр("ru='Адресные сведения ФИАС, загруженные в программу, устарели.
		...
		|en = 'FIAS address information imported into the program is obsolete.
		|It is recommend to upgrade the address information in order to verify addresses properly.'");
	...
	Иначе
		СообщениеОПроверкеАдреса = СообщениеОПроверкеАдреса + Символы.ПС +
			НСтр("ru = 'Для обновления адресного классфикатора необходимо обратиться к администратору.'; en = 'One should contact the administrator to update the address classifier.'");
...
Процедура ПоказатьСписокДомов(ВключитьОтбор)
	...
	Иначе
		Если ЗначениеЗаполнено(ПредставлениеАдреса) Тогда
			ТекстПредупреждения = НСтр("ru = 'Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о нумерации домов для введенного адреса.'; en = 'Selection from list is unavailable because there is no information in the address classifier about house numbers for the address entered.'");
		Иначе
			ТекстПредупреждения = НСтр("ru = 'Для просмотра список домов заполните поля адреса.'; en = 'To see the house list fill in address fields.'");
		КонецЕсли;
...
Процедура ВвестиАдресВСвободнойФормеЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		Комментарий = ?(НЕ ПустаяСтрока(Комментарий), Комментарий + Символы.ПС, "")
			+ НСтр("ru = 'Исходный адрес:'; en = 'Source address:'") + " " + ПредставлениеАдреса;
		ПодключитьОбработчикОжидания("УстановитьПиктограммуКомментария", 0.1, Истина);
	КонецЕсли;
...
Функция СписокОшибокЗаполнения(Контекст, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если Результат.Количество() = 0 // Нет ошибок
		...
		Результат.Добавить("/", НСтр("ru = 'Адрес пуст.'; en = 'Address is empty.'"));
	КонецЕсли;
...
Процедура ОтобразитьИнформациюОДатахДействияАдреса(ДействуетС, ДействуетДо)
	...
	Если ЗначениеЗаполнено(ДействуетДо) И ЗначениеЗаполнено(ДействуетС)Тогда
		Если НачалоДня(ДействуетДо -10) = НачалоДня(ДействуетС) Тогда
			ТекстИсторическийАдрес = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Адрес действовал только %1'; en = 'Address was vaild only %1'"), Формат(ДействуетС, "ДЛФ=DD"));
		Иначе
			ТекстИсторическийАдрес = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Адрес действовал с %1 по %2 (включительно)'; en = 'Address was valid from %1 to %2 (inclusive)'"), Формат(ДействуетС, "ДЛФ=DD"), Формат(ДействуетДо - 10, "ДЛФ=DD"));
		КонецЕсли;
	ИначеЕсли НЕ ЗначениеЗаполнено(ДействуетС) И ЗначениеЗаполнено(ДействуетДо) Тогда
		ТекстИсторическийАдрес = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Адрес действовал до %1 (включительно)'; en = 'Address was valid until %1 (inclusive)'"), Формат(ДействуетДо - 10, "ДЛФ=DD"));
	Иначе
		ТекстИсторическийАдрес = НСтр("ru = 'Адрес действует в настоящие время'; en = 'Address is valid now'");
	КонецЕсли;
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если КоличествоОшибок = 0 И ПредупреждатьОбОтсутствии Тогда
		...
		Если ТребуетсяОбновление Тогда
			Если ЕстьПравоЗагружатьКлассификатор Тогда
				...
				|en = 'Address is enterred correctly, but address information imported into the program is obsolete.
				|It is recommended to update the information and repeat the check.'");
				Кнопки = Новый СписокЗначений();
			...
			КонецЕсли;
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Адрес введен корректно.'; en = 'Address is entered correctly.'"));
		КонецЕсли;
	...
	Если КоличествоОшибок = 1 Тогда
		...
		Если СписокОшибок[0].Пометка = Ложь И НЕ ЭтоВебСервис Тогда
			ТекстВопроса = НСтр("ru = 'Адрес не может быть проверен.'; en = 'The address could not be verified.'") + Символы.ПС + СписокОшибок[0].Представление + Символы.ПС + НСтр("ru = 'Загрузить классификатор для проверки адреса?'; en = 'Import the classifier for address validation?'");
...
Процедура СостояниеВводаПредставленияАдреса(РедактироватьПредставлениеАдреса, ФормироватьПредставление = Истина)
	...
	Если РедактироватьПредставлениеАдреса Тогда
		...
		Элементы.ПредставлениеАдреса.Заголовок          = НСтр("ru='Адрес в свободной форме'; en = 'Address in free form'");
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если ДоступностьКлассификатора <> Неопределено Тогда
			...
			Если ДоступностьКлассификатора.Отказ Тогда
				ТекстСообщенияСервиса = НСтр("ru = 'Автоподбор и проверка адреса недоступны:'; en = 'Autocompletion and address validation unavailable:'") + Символы.ПС 
				+ ДоступностьКлассификатора.КраткоеПредставлениеОшибки;
			КонецЕсли;
		КонецЕсли;
...
Функция ПроверитьДоступностьКлассификатораВФоне(Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru = 'Проверка доступности сервиса адресного классификатора'; en = 'Test availability of address classifier service'");
