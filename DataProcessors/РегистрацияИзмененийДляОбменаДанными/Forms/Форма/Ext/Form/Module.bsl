...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			ВызватьИсключение СтрЗаменить(
				НСтр("ru = 'Некорректные параметры объектов назначения открытия команды ""%1""'; en = 'Incorrect parameters for objects when opening command ""%1""'"),
	...
	Если Не ПроконтролироватьНастройки() И ОткрытиеСУзломПараметром Тогда
		...
		ТекстСообщения = СтрЗаменить(
			НСтр("ru = 'Для ""%1"" редактирование регистрации объектов недоступно.'; en = 'For ""%1"" editing registration of objects is not available.'"),
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") 
		...
		Ошибка = НСтр("ru = 'Неожиданный результат выбора из консоли запросов'; en = 'Unexpected result of selection from a query console'");
	Иначе
	...
	Если ВыбранноеЗначение.ДействиеВыбора Тогда
		Текст = НСтр("ru = 'Зарегистрировать результат запроса
		                 |на узле ""%1""?';
		                 |en = 'Register query result
		                 |for node ""%1""?'"); 
	Иначе
		Текст = НСтр("ru = 'Отменить регистрацию результата запроса
		                 |на узле ""%1""?';
		                 |en = 'Cancel registration of query results
		                 |for node ""%1""?'");
	КонецЕсли;
...
Процедура ПоказатьРезультатВыгрузки(Команда)
	...
	Если Сериализация.Количество() > 0 Тогда
		...
		ЗаголовокТекста = НСтр("ru = 'Результат стандартной выгрузки (РИБ)'; en = 'Export result (in DIB mode)'");
...
Процедура ЗарегистрироватьИОМИПредопределенные(Команда)
	...
	ТекстВопроса     = СтрЗаменить( 
		НСтр("ru = 'Зарегистрировать данные для восстановления подчиненного узла РИБ
		     |на узле ""%1""?'; en = 'Register data to restore subordinate DIB node on node ""%1""?'"),
		"%1", УзелОбменаСсылка);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Не выгружалось'; en = 'Not exported'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Нет изменений'; en = 'No changes'"));
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Подсказка = НСтр("ru = 'Номер отправленного'; en = 'Sent message number'"); 
...
Процедура УстановитьЗаголовокНомеровСообщений()
	...
	Текст = НСтр("ru = '№ отправленного %1, № принятого %2'; en = 'sent № %1, received № %2'");
...
Процедура СообщитьОРезультатахРегистрации(Команда, Результаты)
	...
	Если ТипЗнч(Команда) = Тип("Булево") Тогда
		Если Команда Тогда
			ЗаголовокПредупреждения = НСтр("ru = 'Регистрация изменений:'; en = 'Registering changes:'");
			ТекстПредупреждения = НСтр("ru = 'Зарегистрировано %1 изменений из %2
			                           |на узле ""%0""';
			                           |en = 'Registered %1 changes of %2
			                           |for node ""%0""'");
		Иначе
			ЗаголовокПредупреждения = НСтр("ru = 'Отмена регистрации:'; en = 'Cancelling registration:'");
			ТекстПредупреждения = НСтр("ru = 'Отменена регистрация %1 изменений 
			                           |на узле ""%0"".';
			                           |en = 'Cancelled registration of %1 changes
			                           |for node ""%0"".'");
		КонецЕсли;
	Иначе
		ЗаголовокПредупреждения = НСтр("ru = 'Изменение номера сообщения:'; en = 'Changing message number:'");
...
Процедура УдалитьРегистрациюКонстантыВСписке()
	...
	ИначеЕсли Количество = 1 Тогда
		Текст = НСтр("ru = 'Отменить регистрацию ""%2""
		                 |на узле ""%1""?';
		                 |en = 'Cancel registration ""%2""
		                 |for node ""%1""?'"); 
	Иначе
		Текст = НСтр("ru = 'Отменить регистрацию выбранных констант
		                 |на узле ""%1""?';
		                 |en = 'Cancel registration of selected constants
		                 |for node ""%1""?'"); 
	КонецЕсли;
...
Процедура УдалитьРегистрациюИзСпискаСсылок()
	...
	Иначе
		Текст = НСтр("ru = 'Отменить регистрацию выбранных объектов
		                 |на узле ""%1""?';
		                 |en = 'Cancel registration of selected objects
		                 |for node ""%1""?'"); 
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", УзелОбменаСсылка);
...
Процедура ДобавитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("ru = 'Зарегистрировать %1 для выгрузки на узле ""%2""?
	                 ...
	                 |en = 'Register %1 for export to node ""%2""?
	                 |
	                 |Changing the registration of large numbers of objects can take a long time!'");
...
Процедура УдалитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("ru = 'Отменить регистрацию %1 для выгрузки на узле ""%2""?
	                 |
	                 |Изменение регистрации большого количества объектов может занять продолжительное время!';
	                 |en = 'Cancel registration %1 for export to node ""%2""?
	                 |Changing registration of large number of objects may take a long time!'");
...
Процедура ОперацияСРезультатамиЗапроса(КомандаОперации)
	...
	Если ТекИмяФормы <> Неопределено Тогда
		...
		Если КомандаОперации Тогда
			Текст = НСтр("ru = 'Регистрация изменений результата запроса'; en = 'Registering query result changes'");
		Иначе
			Текст = НСтр("ru = 'Отмена регистрации изменений результата запроса'; en = 'Cancelling query result change registration'");
		КонецЕсли;
	...
	Текст = НСтр("ru = 'В настройках не указана обработка для выполнения запросов.
	                        |Настроить сейчас?';
	                        |en = 'Processor for query execution is not specified in the settings.
	                        |Specify now?'");
	
	ЗаголовокВопроса = НСтр("ru = 'Настройки'; en = 'Settings'");
...
Функция КонтрольСсылокДляВыбораЗапросом(Адрес)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда 
		...
		Если Результат.Колонки.Найти("Ссылка") = Неопределено Тогда
			Возврат НСтр("ru = 'В последнем результате запроса отсутствует колонка ""Ссылка""'; en = 'In the last query result there is no column ""Reference""'");
		КонецЕсли;
	Иначе		
		Возврат НСтр("ru = 'Ошибка получения данных результата запроса'; en = 'Query result data getting error'");
	КонецЕсли;
...
Процедура НастроитьПустуюСтраницу(Наименование, ИмяТаблицы = Неопределено)
	...
	Текст = НСтр("ru = '%1.
	                 ...
	                 |
	                 |%2 
	                 |To register or unregister object for exchange with node
	                 |""%3""
	                 |select the object type in the metadata tree on the left, and then use 
	                 |commands ""Register"" or ""Cancel registration""'");
...
Функция ПолучитьОписаниеВыбранныхМетаданных(БезУчетаАвторегистрации, МетаИмяГруппа = Неопределено, МетаИмяУзел = Неопределено)
	...
	Если МетаИмяГруппа = Неопределено И МетаИмяУзел = Неопределено Тогда
		// Не указано ничего
		Текст = НСтр("ru = 'все объекты %1 по выбранной иерархии вида'; en = 'all objects %1 according to the selected hierarchy kind'");
	...
	Иначе
		...
		Текст = НСтр("ru = 'все объекты типа ""%3"" %1'; en = 'all objects of type ""%3"" %1'");
	...
	Если БезУчетаАвторегистрации Тогда
		ТекстФлага = "";
	Иначе
		ТекстФлага = НСтр("ru = 'с признаком авторегистрации'; en = 'with auto registration'");
	КонецЕсли;
...
Процедура ОбработатьЗапретИзмененияУзла()
	...
	Если ОперацииРазрешены Тогда
		...
		Заголовок = НСтр("ru = 'Регистрация изменений для обмена данными'; en = 'Registering changes for data exchange'");
	Иначе
		Элементы.УзелОбменаСсылка.Видимость = Ложь;
		Заголовок = СтрЗаменить(НСтр("ru = 'Регистрация изменений для обмена с  ""%1""'; en = 'Registration of changes for exchange with ""%1""'"), "%1", Строка(УзелОбменаСсылка));
	КонецЕсли;
...
Функция ПроверитьВерсиюИРежимСовместимостиПлатформы()
	...
	Если Не (Лев(Информация.ВерсияПриложения, 3) = "8.3"
		...
		ВызватьИсключение НСтр("ru = 'Обработка предназначена для запуска на версии платформы
			...
			|8.3.5 with compatibility mode off or higher'");
