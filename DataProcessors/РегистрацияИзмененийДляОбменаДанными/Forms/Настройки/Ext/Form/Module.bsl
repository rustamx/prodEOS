...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТекущийОбъект.ЭтоФайловаяБаза() Тогда
		...
		Если ТекЭлемент <> Неопределено Тогда
			ТекЭлемент.Представление = НСтр("ru = 'В каталоге:'; en = 'In directory:'");
		КонецЕсли;
	КонецЕсли;
...
#Иначе
	ВызватьИсключение(НСтр("ru = 'В WEB-клиенте данная возможность недоступна.'; en = 'This feature is not available in Web client.'"));
#КонецЕсли
...
Процедура ПослеПроверкиРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Диалог.Фильтр = НСтр("ru = 'Внешние обработки (*.epf)|*.epf'; en = 'External data processors (*.epf)|*.epf'");
...
Процедура СообщитьОбОшибке(Текст, ИмяРеквизита = Неопределено)
	...
	Если ИмяРеквизита = Неопределено Тогда
		ЗаголовокОшибки = НСтр("ru = 'Ошибка'; en = 'Error'");
