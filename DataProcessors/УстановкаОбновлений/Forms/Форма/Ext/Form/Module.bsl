...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбновлениеКонфигурации.ЕстьПраваНаУстановкуОбновления() Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав для обновления конфигурации. Обратитесь к администратору.'; en = 'Insufficient permissions to upgrade the configuration. Contact the administrator.'");
	ИначеЕсли ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		ВызватьИсключение НСтр("ru = 'Данная операция не доступна внешнему пользователю системы.'; en = 'Operation is not available to external users.'");
	КонецЕсли;
...
Процедура ПолеФайлОбновленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Диалог.Фильтр = НСтр("ru = 'Все файлы поставки (*.cf*;*.cfu)|*.cf*;*.cfu|Файлы поставки конфигурации (*.cf)|*.cf|Файлы поставки обновления конфигурации(*.cfu)|*.cfu'; en = 'All delivery files (*.cf*;*.cfu)|*.cf*;*.cfu|Configuration supply files (*.cf)|*.cf|Configuration update supply files (*.cfu)|*.cfu'");
	Диалог.Заголовок = НСтр("ru = 'Выбор поставки обновления конфигурации'; en = 'Choice of delivery configuration updates'");
...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено)
	...
	КнопкаДалее.Заголовок = ?(ФункцияКнопкиДалее, НСтр("ru = 'Далее >'; en = 'Next >'"), НСтр("ru = 'Готово'; en = 'Done'"));
	КнопкаЗакрыть.Заголовок = ?(ФункцияКнопкиЗакрыть, НСтр("ru = 'Отмена'; en = 'Cancel'"), НСтр("ru = 'Закрыть'; en = 'Close'"));
...
Процедура ПерейтиСоСтраницыФайлаОбновления()
	...
	Если НуженФайлОбновления = 1 Тогда
		Если Не ЗначениеЗаполнено(Объект.ИмяФайлаОбновления) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите файл поставки обновления конфигурации.'; en = 'Specify supplies configuration update file.'"),,"Объект.ИмяФайлаОбновления");
		...
		Если Не Файл.Существует() Или Не Файл.ЭтоФайл() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Файл поставки обновления конфигурации не найден.'; en = 'Delivery configuration update file not found.'"),,"Объект.ИмяФайлаОбновления");
			ТекущийЭлемент = Элементы.ПолеФайлОбновления;
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПерейтиКВыборуРежимаОбновления(ЭтоПереходДалее = Ложь)
	...
	Если ПараметрыАдминистрирования = Неопределено Тогда
		...
		ЗаголовокФормы = НСтр("ru = 'Установка обновления'; en = 'Upgrade installation'");
		Если ЭтоФайловаяБаза Тогда
			ПоясняющаяНадпись = НСтр("ru = 'Для установки обновления необходимо ввести
				|параметры администрирования информационной базы'; en = 'To install the update, you must enter the admin settings infobase'");
		...
		Иначе
			ПоясняющаяНадпись = НСтр("ru = 'Для установки обновления необходимо ввести параметры
				|администрирования кластера серверов и информационной базы'; en = 'To install the update, you must enter the settings for server cluster administration and infobase'");
...
Процедура ПослеПолученияПараметровАдминистрирования(Результат, ЭтоПереходДалее) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = НСтр("ru = 'Для установки обновления необходимо ввести параметры администрирования.'; en = 'To install the update, you must enter the administrator settings.'");
		...
		ТекстСообщения = НСтр("ru = 'Не удалось установить обновление программы, т.к. не были введены
			...
			|en = 'Unable to set program update, because were not entered 
			|the correct infobase administrative settings.'");
...
Функция ВозможенЗапускОбновления()
	...
	#Если ВебКлиент Тогда
		...
		ТекстСообщения = НСтр("ru = 'Обновление программы недоступно в веб-клиенте.'; en = 'Upgrade is not available in web client.'");
	#КонецЕсли
	
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		ВозможенЗапускОбновления = Ложь;
		ТекстСообщения = НСтр("ru = 'Обновление программы доступно только в клиенте под управлением ОС Windows.'; en = 'Program upgrade is available only in the Windows client.'");
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиент.КлиентПодключенЧерезВебСервер() Тогда
		ВозможенЗапускОбновления = Ложь;
		ТекстСообщения = НСтр("ru = 'Обновление программы недоступно при подключении через веб-сервер.'; en = 'Upgrade is not available when you are connected through a web server.'");
	КонецЕсли;
...
Процедура ПрочитатьДанныеВЖурналРегистрации()
	...
	Для НомерСтроки = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		Если Сред(ТекущаяСтрока, 3, 1) = "." И Сред(ТекущаяСтрока, 6, 1) = "." Тогда // Строка с датой
			...
			Если Комментарий = НСтр("ru = 'Обновление выполнено'; en = 'Upgrade completed'") Тогда
				Объект.РезультатОбновления = Истина;
				Продолжить;
			ИначеЕсли Комментарий = НСтр("ru = 'Обновление не выполнено'; en = 'Upgrade not completed'") Тогда
