...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("ru = 'Универсальный обмен данными в формате XML (%ВерсияОбработки%)'; en = 'Universal data exchange in xml FORMAT (%DataProcessorVersion%)'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Выполнить загрузку данных из файла обмена?'; en = 'Do you want to import data from the exchange file?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура КаталогИнформационнойБазыДляПодключенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ru = 'Выберите каталог информационной базы'; en = 'Select infobase directory'");
...
Процедура ИмяФайлаПравилПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ИмяФайлаПравил) Или Не Файл.Существует() Тогда
		СообщитьПользователю(НСтр("ru = 'Не найден файл правил обмена'; en = 'Exchange rule file not found'"), "ИмяФайлаПравил");
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Загрузить правила обмена данными?'; en = 'Do you want to import data exchange rules?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,НСтр("ru = 'Файл обмена'; en = 'Exchange file'"),, УникальныйИдентификатор);
...
Процедура ПолучитьИнформациюОФайлеОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			...
			СообщитьПользователю(НСтр("ru = 'Не удалось прочитать файл обмена.'; en = 'Cannot read the exchange file.'"));
			ОчиститьДанныеОФайлеДляЗагрузкиДанных();
...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Удалить выбранные данные в информационной базе?'; en = 'Do you want to delete selected data?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура УдалениеУдалитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Состояние(НСтр("ru = 'Выполняется удаление данных. Пожалуйста, подождите...'; en = 'Deleting data. Please wait ...'"));
		УдалитьНаСервере();
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаОбменаВХранилище,НСтр("ru = 'Файл обмена'; en = 'Exchange file'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если Не ЭтоWindowsКлиент() И ПрямаяВыгрузка = 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.'; en = 'Direct connection to the database is only supported in the Windows client.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаПравилВХранилище,НСтр("ru = 'Файл правил обмена'; en = 'Exchange rule file'"),, УникальныйИдентификатор);
...
Функция ИменаФайловПравилИОбменаСовпадают()
	...
	Если ВРег(СокрЛП(ИмяФайлаПравил)) = ВРег(СокрЛП(ИмяФайлаДанных)) Тогда
		...
		|en = 'Exchange rules file may not be the same as the data file.
		|Select another file for data export.'"));
		Возврат Истина;
...
Процедура ЗаполнитьСписокТиповДоступныхДляУдаления()
	...
	СтрокаДерева.Представление = НСтр("ru = 'Справочники'; en = 'Catalogs'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = НСтр("ru = 'Планы видов характеристик'; en = 'Charts of characteristic types'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = НСтр("ru = 'Документы'; en = 'Documents'");
...
Функция ВыполнитьПодключениеКИБПриемникуНаСервере()
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		СообщитьПользователю(НСтр("ru = 'Подключение успешно установлено.'; en = 'Connection established.'"));
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ru = 'Не найден файл обмена на сервере.'; en = 'Exchange file not found on the server.'"), "ИмяФайлаОбмена");
			Возврат;
...
Процедура ВыполнитьЗагрузкуИзФормыЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ru = 'Выполняется загрузка данных. Пожалуйста, подождите...'; en = 'Importing data. Please wait...'"));
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Если Объект.БезопаснаяЗагрузка Тогда
		...
		Иначе
			СообщитьПользователю(НСтр("ru = 'Не указан файл правил для загрузки данных.'; en = 'Rules file for data import is not specified.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Универсальный обмен данными в формате XML'; en = 'Universal data exchange in XML format'", ОбъектДляСервера.КодОсновногоЯзыка()),
...
Функция ИмяФайлаНаСервереИлиКлиенте(ИмяРеквизита ,Знач АдресФайла, Знач ИмяФайлаДляРасширения = ".xml",
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() И ПроверятьСуществование Тогда
			...
			СообщитьПользователю(НСтр("ru = 'Указанный файл не существует.'; en = 'The file does not exist.'"));
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Состояние(НСтр("ru = 'Выполняется выгрузка данных. Пожалуйста, подождите...'; en = 'Exporting data. Please wait...'"));
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
		...
		ИмяСохраняемогоФайла = ?(Объект.АрхивироватьФайл, НСтр("ru = 'Файл выгрузки.zip'; en = 'Export file.zip'"),НСтр("ru = 'Файл выгрузки.xml'; en = 'Export file.xml'"));
...
Функция ВыполнитьВыгрузкуНаСервере()
	...
	Если Не ПрямаяВыгрузка Тогда
		...
		Если ИмяВременногоФайлаДанных = Неопределено Тогда
			...
			СообщитьПользователю(НСтр("ru = 'Не определен файл данных'; en = 'Data file not specified'"));
	...
	Если ОбъектДляСервера.ФлагРежимОтладкиОбработчиков Тогда
		...
		Если Не Файл.Существует() Тогда
			...
			СообщитьПользователю(НСтр("ru = 'Файл внешней обработки отладчиков событий не существует на сервере'; en = 'Event debugger external data processor file does not exist on the server'"));
			Возврат Неопределено;
		...
		Если Отказ Тогда
			...
			СообщитьПользователю(НСтр("ru = 'Не удалось выгрузить обработчики событий'; en = 'Cannot export event handlers'"));
			Возврат "";
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	?(РежимРаботы, НСтр("ru = 'Режим работы (на клиенте)'; en = 'Mode (on client)'"), НСтр("ru = 'Режим работы (на сервере)'; en = 'Mode (on server)'"));
...
Функция ПроверитьВерсиюИРежимСовместимостиПлатформы()
	...
	Если Не (Лев(Информация.ВерсияПриложения, 3) = "8.3"
		...
		ВызватьИсключение НСтр("ru = 'Обработка предназначена для запуска на версии платформы
			...
			|en = 'The data processor is designed to run on the 1C:Enterprise platform of version 
			|8.3 with compatibility mode off or higher'");
...
Процедура ПоместитьФайлПравилЗагрузкиВХранилище()
	...
	НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,
		?(ЧерезХранилище, НСтр("ru = 'Файл обмена'; en = 'Exchange file'"), ИмяФайлаПравилЗагрузки), ЧерезХранилище, УникальныйИдентификатор);
