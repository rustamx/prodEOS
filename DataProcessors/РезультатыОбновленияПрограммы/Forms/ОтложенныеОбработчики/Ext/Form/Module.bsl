...
Процедура ГиперссылкаОтложенноеОбновлениеНажатие(Элемент)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru = 'Обработка данных еще не выполнялась.'; en = 'Data processing has not been executed.'");
...
Процедура Приостановить(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Остановка дополнительных процедур обработки данных
		...
		|en = 'Stoping additional data processing procedures can lead to
		|unstable work or inoperability of the program.
		|Disabling is recommended only in case of discovering an error in the procedure
		|and only after consultation with the customer service, because the data update
		|procedures may depend on each other.'");
...
Процедура СформироватьТаблицуОтложенныхОбработчиков(ВыполненыВсеОбработчики = Истина, НачальноеЗаполнение = Ложь)
	...
	Если ОбработчикиНеВыполнялись Тогда
		Элементы.ТекстПояснения.Заголовок = НСтр("ru = 'Рекомендуется запустить невыполненные процедуры обработки данных.'; en = 'It is recommended to start update procedures that have not been executed.'");
	Иначе
		Элементы.ТекстПояснения.Заголовок = НСтр("ru = 'Невыполненные процедуры рекомендуется запустить повторно.'; en = 'It is recommended to restart update procedures that have not been executed'");
	КонецЕсли;
...
Процедура ДобавитьОтложенныйОбработчик(СтрокаОбработчика, ОбработчикиНеВыполнялись, ВыполненыВсеОбработчики, НачальноеЗаполнение, ВыполняетсяИзменениеПриоритета)
	...
	ШаблонИнформацияОПроцедуреОбновления = НСтр("ru = '%1%2Процедура ""%3"" обработки данных %4.'; en = '%1%2Procedure ""%3"" that processed data %4.'");
	...
	Если СтрокаОбработчика.Статус = "Выполнено" Тогда
		...
		ПредставлениеСтатусаВыполнения = НСтр("ru = 'завершилась успешно'; en = 'completed successfully'");
		СтрокаСписка.СтатусПредставление = НСтр("ru = 'Выполнено'; en = 'Executed'");
	...
	ИначеЕсли СтрокаОбработчика.Статус = "Выполняется" Тогда
		...
		ПредставлениеСтатусаВыполнения = НСтр("ru = 'в данный момент выполняется'; en = 'is currently running'");
		Если Прогресс <> Неопределено Тогда
			ШаблонСтатус = НСтр("ru = 'Выполняется (%1%)'; en = 'Executing (%1%)'");
			СтрокаСписка.СтатусПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтатус, Прогресс)
		Иначе
			СтрокаСписка.СтатусПредставление = НСтр("ru = 'Выполняется'; en = 'Executing'");
		КонецЕсли;
	ИначеЕсли СтрокаОбработчика.Статус = "Ошибка" Тогда
		
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ru = 'Процедура ""%1"" обработки данных завершилась с ошибкой:'; en = 'Procedure ""%1"" data processing failed:'") + Символы.ПС + Символы.ПС;
		...
		СтрокаСписка.СтатусПредставление = НСтр("ru = 'Ошибка'; en = 'Error'");
	...
	ИначеЕсли СтрокаОбработчика.Статус = "Приостановлен" Тогда
		...
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ru = 'остановлена администратором'; en = 'stopped by the administrator'");
		СтрокаСписка.СтатусПредставление = НСтр("ru = 'Остановлено'; en = 'Stopped'");
		СтрокаСписка.СтатусКартинка    = БиблиотекаКартинок.ЗнакСтоп;
	Иначе
		
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("ru = 'еще не выполнялась'; en = 'not yet executed'");
		СтрокаСписка.СтатусПредставление = НСтр("ru = 'Не выполнялась'; en = 'Not executed'");
	КонецЕсли;
...
Функция ДлительностьВыполненияПроцедурыОбновления(ДлительностьВыполнения)
	...
	ШаблонСекунды = НСтр("ru = '%1 сек.'; en = '%1 s.'");
	ШаблонМинуты = НСтр("ru = '%1 мин. %2 сек.'; en = '%1 min. %2 s.'");
	ШаблонЧасы = НСтр("ru = '%1 ч. %2 мин.'; en = '%1 h. %2 min.'");
	...
	Если ДлительностьВСекундах < 1 Тогда
		Возврат НСтр("ru = 'менее секунды'; en = 'less than a second'")
