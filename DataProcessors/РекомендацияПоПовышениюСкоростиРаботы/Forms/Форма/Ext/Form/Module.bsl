...
Процедура ПриОткрытии(Отказ)
	...
	ТекстСообщения = НСтр("ru = 'На компьютере установлено %1 Гб оперативной памяти.
		...
		|en = 'It is %1 GB of RAM on your computer.
		|In order for the program to work faster,
		|we recommend that you increase the memory to %2 Gb.'");
	
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ДоступныйОбъем, РекомендуемыйОбъем);
	
	ЗаголовокСообщения = НСтр("ru = 'Рекомендация по повышению скорости работы'; en = 'Recommendation on speeding up'");
	
	ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
	ПараметрыВопроса.Заголовок = ЗаголовокСообщения;
	ПараметрыВопроса.Картинка = БиблиотекаКартинок.Предупреждение32;
	ПараметрыВопроса.Вставить("ТекстФлажка", НСтр("ru = 'Не показывать в течение двух месяцев'; en = 'Do not show within two months'"));
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("ПродолжитьРаботу", НСтр("ru = 'Продолжить работу'; en = 'Continue work'"));
