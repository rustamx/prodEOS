...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстЗаголовка = НСтр("ru = 'Период: с %1 по %2'; en = 'Period: from %1 to %2'");
	...
	ЗаголовокЗаявки = НСтр("ru = 'Заявка №%1 от %2'; en = 'Request №%1 of %2'");
	...
	Если Не ПрограммноеЗакрытие Тогда
		...
		Если Модифицированность Тогда
			...
			Если ЗавершениеРаботы Тогда
				ТекстПредупреждения =
					НСтр("ru = 'Заявка на изменение тарифа обмена электронными документами не отправлена.'; en = 'Application for change of tariff was not sent.'");
			Иначе
				ОписаниеОповещения = Новый ОписаниеОповещения(
					"ПриОтветеНаВопросОЗакрытииМодифицированнойФормы",
					ЭтотОбъект);
				ТекстВопроса = НСтр("ru = 'Данные изменены. Закрыть форму без сохранения данных?'; en = 'Data was changed. Close the form without saving any data?'");
	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ru = 'Не получается изменить тариф работы с оператором обмена ЭД.
				|
				|%1';
				|en = 'Unable to change the tarriff for EDI operator.
				|
				|%1'");
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("ru = 'Интернет-поддержка. Изменение тарифа в 1С-Такском.'; en = 'Online support. Tariff change in 1С-Taxcom.'"),
			ТекстСообщения,
			"taxcom",
			,
...
Процедура УстановитьНадписьНаГиперссылкеОбновленияСтатуса()
	...
	ТекстЗаголовка = НСтр("ru = 'Проверить выполнение заявки (осталось %1 сек.)'; en = 'Check request execution (%1 sec. left)'");
