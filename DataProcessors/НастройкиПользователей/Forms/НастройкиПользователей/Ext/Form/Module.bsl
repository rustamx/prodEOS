...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		...
		Заголовок = НСтр("ru = 'Настройки пользователя'; en = 'User settings'");
...
Процедура НастройкиПередУдалением(Элемент, Отказ)
	...
	Отказ = Истина;
	ТекстВопроса = НСтр("ru = 'Очистить выбранные настройки?'; en = 'Clear the selected settings?'");
...
Процедура ПользовательСсылкаПриИзменении(Элемент)
	...
	Если РезультатПолученияНастроек() = "НетПользователяИБ" Тогда
		ТекстСообщения = НСтр("ru = 'У выбранного пользователя ""%1"" не указан
			...
			|en = 'For the selected user ""%1"" infobase user is not specified, it is impossible to view his settings.
			|You can correct the problem in the form of the user.'");
...
Процедура СкопироватьВсеНастройки(Команда)
	...
	КопируемыеНастройки.Добавить("НастройкиОтчетов", НСтр("ru='Настройки отчетов'; en = 'Report settings'"));
	КопируемыеНастройки.Добавить("НастройкиВнешнегоВида", НСтр("ru='Настройки внешнего вида'; en = 'Appearance settings'"));
	КопируемыеНастройки.Добавить("ДанныеФорм", НСтр("ru='Данные форм'; en = 'Form data'"));
	КопируемыеНастройки.Добавить("ПерсональныеНастройки", НСтр("ru='Персональные настройки'; en = 'Personal settings'"));
	КопируемыеНастройки.Добавить("Избранное", НСтр("ru='Избранное'; en = 'Favorites'"));
	КопируемыеНастройки.Добавить("НастройкиПечати", НСтр("ru='Настройки печати'; en = 'Print settings'"));
	КопируемыеНастройки.Добавить(
		"ПрочиеПользовательскиеНастройки", НСтр("ru='Настройки дополнительных отчетов и обработок'; en = 'Additional report and data processor settings'"));
...
Процедура Очистить(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ПоказатьПредупреждение(,НСтр("ru = 'Необходимо выбрать настройки, которые требуется удалить.'; en = 'Select the settings that you want to delete.'"));
		Возврат;
		
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ОчиститьЗавершение", ЭтотОбъект, ДеревоНастроек);
	ТекстВопроса = НСтр("ru = 'Очистить выделенные настройки?'; en = 'Clear selected settings?'");
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
Процедура ОчиститьНастройкиВыбраннымПользователям(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Очистить выделенные настройки?
		...
		|A form for selecting users who need clear settings will be opened.'");
...
Процедура ОчиститьВсеНастройки(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Очистить все настройки у пользователя ""%1""?'; en = 'Clear all the settings from the user ""%1""?'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Строка(ПользовательСсылка));
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Очистить", НСтр("ru='Очистить'; en = 'Clear'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ru='Отмена'; en = 'Cancel'"));
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВида(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?'; en = 'Clear all report and appearance settings of the user ""%1""?'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Строка(ПользовательСсылка));
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Очистить", НСтр("ru='Очистить'; en = 'Clear'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ru='Отмена'; en = 'Cancel'"));
...
Процедура ОчиститьНастройкиВсехПользователей(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Сейчас будут очищены все настройки всех пользователей.
		                      |Продолжить?';
		                      |en = 'All settings will now be cleared for all users.
		                      |Do you want to continue?'");
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("ОчиститьВсе", НСтр("ru='Очистить все'; en = 'Clear all'"));
	КнопкиВопроса.Добавить("Отмена", НСтр("ru='Отмена'; en = 'Cancel'"));
...
Функция ОбновлениеСпискаНастроек()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Обновление настроек пользователей'; en = 'Update user settings'");
...
Процедура РассчитатьКоличествоНастроек()
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.НастройкиОтчетовСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Настройки отчетов (%1)'; en = 'Report settings (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.НастройкиОтчетовСтраница.Заголовок = НСтр("ru='Настройки отчетов'; en = 'Report settings'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ВнешнийВидСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Внешний вид (%1)'; en = 'Appearance (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.ВнешнийВидСтраница.Заголовок = НСтр("ru='Внешний вид'; en = 'Appearance'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ПрочиеНастройкиСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Прочие настройки (%1)'; en = 'Other settings (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.ПрочиеНастройкиСтраница.Заголовок = НСтр("ru='Прочие настройки'; en = 'Other settings'");
	КонецЕсли;
	...
	Если ВРег(ИмяСобытия) = ВРег("ВыборПользователя") Тогда
		...
		Если Параметр.КопироватьВсе Тогда
			...
			Если МассивНастроек.Количество() = 7 Тогда
				ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Скопированы все настройки %1'; en = 'All settings are copied to %1'"), ПояснениеКомуСкопированыНастройки);
			Иначе
				ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 скопированы %2'; en = '%1 copied to %2'"), ИменаНастроек, ПояснениеКомуСкопированыНастройки);
			КонецЕсли;
				
			Состояние(НСтр("ru = 'Выполняется копирование настроек'; en = 'Copying settings...'"));
			
			Отчет = Неопределено;
			СкопироватьВсеНастройкиНаСервере(ПользовательИнформационнойБазы, ПользователиПриемник, МассивНастроек, Отчет);
			
			Если Отчет <> Неопределено Тогда
				ТекстВопроса = НСтр("ru = 'Не все варианты отчетов и настройки были скопированы.'; en = 'Some report options and settings are not copied.'");
				КнопкиВопроса = Новый СписокЗначений;
				КнопкиВопроса.Добавить("Ок", НСтр("ru='ОК'; en = 'ОК'"));
				КнопкиВопроса.Добавить("ПоказатьОтчет", НСтр("ru='Показать отчет'; en = 'Show report'"));
			...
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Копирование настроек'; en = 'Сopy settings'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьНастройкиВсехПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Очистка настроек'; en = 'Clear settings'"), ,
		НСтр("ru='Очищены все настройки всех пользователя'; en = 'All settings of all users are cleared'"), БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьВсеНастройкиЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = НСтр("ru = 'Очищены все настройки пользователя ""%1""'; en = 'Cleared all user settings ""%1""'");
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВидаЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = НСтр("ru = 'Очищены все настройки отчетов и внешнего вида у пользователя ""%1""'; en = 'Cleared all reports and appearance settings of the user ""%1""'");
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	Если КоличествоНастроек = 1
		...
		ТекстПояснения = НСтр("ru = '""%1"" очищена пользователю ""%2""'; en = '""%1"" cleaned up for user ""%2""'");
	...
	ИначеЕсли КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = НСтр("ru = 'Очищено %1 пользователю ""%2""'; en = 'Cleared %1 for user ""%2""'");
	...
	Если КоличествоПользователей <> Неопределено Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			ТекстПояснения = НСтр("ru = '""%1"" очищена %2'; en = '""%1"" cleared %2'");
		...
		Иначе
			ТекстПояснения = НСтр("ru = 'Очищено %1 %2'; en = 'Cleared %1 %2'");
	...
	ПоказатьОповещениеПользователя(
		НСтр("ru = 'Очистка настроек'; en = 'Clear settings'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура СкопироватьНастройки()
	...
	Если ДеревоНастроек.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Необходимо выбрать настройки, которые требуется скопировать.'; en = 'Select the settings you want to copy.'"));
		Возврат;
	ИначеЕсли ДеревоНастроек.ВыделенныеСтроки.Количество() = 1 Тогда
		...
		Если ДеревоНастроек.ТекущиеДанные.Тип = "ВариантЛичный" Тогда
			...
			|en = 'You cannot copy a personal version of the report.
			|To make personal version of the report available to other users, you must resave it with the removed mark ""Author only"".'"));
			Возврат;
		ИначеЕсли ДеревоНастроек.ТекущиеДанные.Тип = "НастройкаЛичная" Тогда
			...
			|en = 'Cannot copy personal configuration options for the report.
			|Copying personal reporting options settings is not provided.'"));
			Возврат;
		КонецЕсли;
