...
Функция ПолучитьИнформациюОПравилах() Экспорт
	...
	СтрокаИнформации = НСтр("ru = 'Правила регистрации объектов этой информационной базы (%1) от %2'; en = 'Object registration rules for this infobase (%1) of %2'");
...
Процедура ВыполнитьПроверкуНаличияПланаОбмена()
	...
	Если Регистрация.ИмяПланаОбмена <> ИмяПланаОбменаДляЗагрузки Тогда
		...
		ОписаниеОшибки = НСтр("ru = 'В правилах регистрации указан план обмена %1, а загрузка выполняется для плана обмена %2'; en = 'Exchange plan %1 is specified in the registration rules, but export is being executed for plan %2'");
...
Функция ИнициализацияСообщений()
	...
	Сообщения.Вставить(0, НСтр("ru = 'Внутренняя ошибка'; en = 'Internal error'", КодОсновногоЯзыка));
	Сообщения.Вставить(1, НСтр("ru = 'Ошибка открытия файла правил'; en = 'Error opening rule file'", КодОсновногоЯзыка));
	Сообщения.Вставить(2, НСтр("ru = 'Ошибка при загрузке правил'; en = 'Error importing rules'", КодОсновногоЯзыка));
	Сообщения.Вставить(3, НСтр("ru = 'Ошибка формата правил'; en = 'Rule format error'", КодОсновногоЯзыка));
	Сообщения.Вставить(4, НСтр("ru = 'Ошибка при получении файла правил для чтения'; en = 'Error retrieving rules file for reading'", КодОсновногоЯзыка));
	Сообщения.Вставить(5, НСтр("ru = 'Загружаемые правила регистрации не предназначены для текущего плана обмена.'; en = 'Imported registration rules are not designed for the current exchange plan.'", КодОсновногоЯзыка));
